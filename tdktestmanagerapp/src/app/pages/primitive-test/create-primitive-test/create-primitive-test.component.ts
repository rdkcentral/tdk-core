...
/**
 * Component for creating a primitive test.
 */
@Component({
  selector: 'app-create-primitive-test',
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, NgMultiSelectDropDownModule, FormsModule, AgGridAngular],
  templateUrl: './create-primitive-test.component.html',
  styleUrl: './create-primitive-test.component.css'
})

export class CreatePrimitiveTestComponent {

  [x: string]: any;
  dropdownSettings = {};
  submitted = false;
  createPrimitiveTestForm!: FormGroup;
  dropdownList: any;
  ...
  ngOnInit(): void {
    this.createPrimitiveTestForm = this.formBuilder.group({
      testName: ['', [Validators.required, Validators.minLength(4)]],
  ...
  createPrimitiveTest(): void {
    this.submitted = true;
    if (this.createPrimitiveTestForm.invalid) {
      return
    } else {
      const tableData: any[] = [];
      ...
      let data = {
        primitiveTestname: this.createPrimitiveTestForm.value.testName,
