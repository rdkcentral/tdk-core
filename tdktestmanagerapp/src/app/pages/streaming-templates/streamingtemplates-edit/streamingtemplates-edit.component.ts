...
@Component({
  selector: 'app-streamingtemplates-edit',
  standalone: true,
  imports: [CommonModule, HttpClientModule, ReactiveFormsModule, MaterialModule, AgGridAngular],
  templateUrl: './streamingtemplates-edit.component.html',
  styleUrl: './streamingtemplates-edit.component.css'
})
export class StreamingtemplatesEditComponent {
  ...
  constructor(private fb: FormBuilder, private router: Router,
    private _snakebar: MatSnackBar, private service: StreamingTemplatesService) {
   ...
   */
  ngOnInit(): void {
    this.streamingEditForm = this.fb.group({
      streamingName: [this.user.name, Validators.required]
    });
  ...
  /**
   * Handles the update of a streaming template.
   */  
  onStreamingUpdate():void  {
    this.streamingFormSubmitted = true;
    if (this.streamingEditForm.invalid) {
      return
    } else {
      const tableData: any[] = [];
      ...
      let obj = {
        existingTemplateName: this.user.name,
        newTemplateName: this.streamingEditForm.value.streamingName,
        streamingMaps: this.streamingMapObj,
        streamingTemplateUserGroup: this.loggedinUser.userGroupName
      }
      this.service.streamingTeamplatesUpdate(obj).subscribe({
        next: (res) => {
  ...
  /**
   * Navigates back to the streaming templates list page.
   */  
  goBack():void  {
    this.router.navigate(["configure/streamingtemplates-list"]);
  }
  ...
  /**
   * Resets the streaming edit form to its initial state.
   */
  reset():void {
    this.streamingEditForm.reset();
  }
