...
preCon_IPv6() {
    ...
    if [[ "$testcaseID" == "TC_IPv6_MANUAL_02" ]] || [[ "$testcaseID" == "TC_IPv6_MANUAL_04" ]] || [[ "$testcaseID" == "TC_IPv6_MANUAL_08" ]]; then
...
getPublicIP_key_Extract() {
    ...
    if [ "$interface_flag" -eq 0 ]; then
        ...
        local json_payload_GetPublicIP=$(printf '{ "jsonrpc": "2.0", "id": 42, "method": "org.rdk.NetworkManager.1.GetPublicIP", "params": { "interface": "%s","ipversion": "IPv6" } }' "$expected_interface")
    fi    
...
TC_IPv6_MANUAL_02() {
    ...
    if [ "$interface_flag" -eq 0 ]; then
        ...
        local step_mesg="Execute the curl command to check the behaviour of getPublicIP API when connected to a IPv6 Supported SSID and ethernet is connected" 
    fi
    getPublicIP_key_Extract "$step" "$step_mesg" "$interface" 
    local getPublicIP_key_Extract_exit=$?

    if [ "$getPublicIP_key_Extract_exit" -eq 0 ]; then
        return 0
    elif [ "$interface_flag" -eq 1 ] && [ "$getPublicIP_key_Extract_exit" -eq 101 ]; then
        return 0
    else
        return 1
    fi    
...
tc_IPv6_MANUAL_testsuite() {
    ...
    #Step 1 code block for TC_IPV6_MANUAL_08
        ...
        log_generate_operations "$failed_step_num" "IPv6"
...
postCondition_Execution_IPv6() {
    ...
    if [[ "$testcaseID" == "TC_IPv6_MANUAL_04" ]] || [[ "$testcaseID" == "TC_IPv6_MANUAL_05" ]]; then
        kill_app "YouTube" "$isAppInstalled_appid"  
        local kill_app_exit=$?
...
while true; do
  ...
  printf '08. Run TestCase : TC_IPv6_MANUAL_08        :\t[ Verify the getPublicIP API response when connected to an IPv6 supported SSID and Ethernet is connected ] \n\n'
  ...
  case "$menu_choice" in 
    ...
    7)
        ...
        ;;
    8) 
        exec_start "TC_IPv6_MANUAL_08"
        tc_IPv6_MANUAL_testsuite "tc8_step" "TC_IPv6_MANUAL_08"
        ;;
    9)             
