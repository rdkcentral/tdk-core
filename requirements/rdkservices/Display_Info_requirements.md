...
| `DINFO‑REQ‑002` | SHALL return a boolean HDMI connection status (connected), and return the correct disconnected status when queried without a TV connected |
| `DINFO‑REQ‑003` | SHALL return the current display width in pixels, the display height in pixels, and the vertical frequency, each consistent with the active resolution reported by DisplaySettings; emit the updated resolution via the pre-change and post-change display update events when the resolution is changed |
...
| `DINFO‑REQ‑010` | SHALL remain functional after repeated activate/deactivate stress cycles, and emit the statechange event with the correct plugin state when activated and deactivated |
