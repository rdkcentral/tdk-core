...
| `HDCP‑REQ‑003` | SHALL emit the statechange event with the correct plugin state when the HdcpProfile plugin is activated and deactivated, and SHALL emit the all event for the same lifecycle transitions |
