...
| `OCDM‑REQ‑002` | SHALL emit a state change event when the OCDM plugin is activated and deactivated via the platform controller, with the event payload identifying the correct plugin callsign and target state value, and SHALL confirm the updated activation state after each transition |
