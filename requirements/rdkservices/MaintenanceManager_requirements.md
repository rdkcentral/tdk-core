...
| `MM‑REQ‑006` | SHALL successfully set the maintenance mode with BACKGROUND and FOREGROUND modes combined with valid opt-out values (BYPASS_OPTOUT, ENFORCE_OPTOUT, NONE), setting mode-only or opt-out-only parameters, and SHALL return an error for invalid, empty, special-character, or numeric mode or opt-out combinations |
| `MM‑REQ‑007` | SHALL return an error when getMaintenanceActivityStatus or getMaintenanceStartTime is called while maintenance is not running, and SHALL return an error when stopMaintenance is called while no maintenance is active |
