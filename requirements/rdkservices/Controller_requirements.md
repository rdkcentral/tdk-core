...
| `CTRL‑REQ‑003` | SHALL retrieve the current value of valid environment variables, and return an error response when queried with an invalid or non-existent environment variable name |
| `CTRL‑REQ‑004` | SHALL retrieve the DeviceInfo plugin configuration, successfully store an updated configuration, and successfully delete directory contents for a valid configured path; return an error response when deleteDirectoryContents is invoked with an empty path or when configuration is set with an empty value |
