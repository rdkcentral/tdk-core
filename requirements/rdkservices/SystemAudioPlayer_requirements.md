...
| `SAP‑REQ‑005` | SHALL successfully play, pause, and stop audio for MP3 and WAV sources in System and App play modes, and SHALL successfully play, pause, resume, and stop audio for MP3 and WAV sources in System and App play modes |
...
| `SAP‑REQ‑007` | SHALL successfully enable smart volume control for PCM, MP3, and WAV audio sources in both System and App play modes, with the primary audio ducking accordingly |
...
| `SAP‑REQ‑010` | SHALL return an error response when setMixerLevels is invoked with an out-of-range primary volume or player volume, and SHALL accept mixer level values at both the minimum and maximum boundary values |
...
| `SAP‑REQ‑012` | SHALL correctly manage the smart volume control lifecycle (enable, play, disable), SHALL successfully operate two simultaneous players in System and App modes, and SHALL correctly replay audio after a play/stop cycle |
