...
| `VTS‑DSVIDEOPORT‑025` | SHALL enforce the following error handling contracts across all Device Settings Video Port operations:<br>report an already-initialized error when initialization is attempted while the module is already initialized<br>report a not-initialized error when any operation is attempted without prior initialization or after the module has already been terminated<br>report an invalid-parameter error when any operation is called with an invalid or null port handle, a NULL output pointer, or an out-of-range parameter value. |
