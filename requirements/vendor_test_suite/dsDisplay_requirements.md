...
| Req ID | Test Scope |
|:-------|:--------------------------------------|
| `VTS‑DSDISPLAY‑001` | SHALL initialize the Display sub-system via `dsDisplayInit()` returning `dsERR_NONE` and terminate it via `dsDisplayTerm()` returning `dsERR_NONE`. |
...
| `VTS‑DSDISPLAY‑005` | SHALL retrieve the display aspect ratio of a valid display handle via `dsGetDisplayAspectRatio()` returning `dsERR_NONE`, with the default reported aspect ratio being `dsVIDEO_ASPECT_RATIO_16x9`. |
...
| `VTS‑DSDISPLAY‑007` | SHALL, for each valid AVI content type, set the value via `dsSetAVIContentType()` returning `dsERR_NONE`, with the value retrieved via `dsGetAVIContentType()` matching the value that was set. |
...
| `VTS‑DSDISPLAY‑010` | SHALL enforce the following error code contracts across all `dsDisplay` APIs:<br>`dsDisplayInit()` returns `dsERR_ALREADY_INITIALIZED` when it is called while the module is already initialized<br>every `dsDisplay` API returns `dsERR_NOT_INITIALIZED` when it is invoked without prior initialization or after the module has already been terminated<br>every `dsDisplay` API returns `dsERR_INVALID_PARAM` when it is called with an invalid handle, a NULL output pointer, or an out-of-range parameter value. |
