...
| Req ID | Test Scope |
|:-------|:--------------------------------------|
| `VTS‑POWERMANAGER‑001` | SHALL initialize the Power Manager HAL via `PLAT_INIT()` returning `PWRMGR_SUCCESS` and terminate the module via `PLAT_TERM()` returning `PWRMGR_SUCCESS`, supporting re-initialization after a prior termination. |
...
| `VTS‑POWERMANAGER‑008` | SHALL enforce the following error code contracts across all `plat_power` APIs:<br>`PLAT_INIT()` returns `PWRMGR_ALREADY_INITIALIZED` when it is called while the module is already initialized<br>every `plat_power` API returns `PWRMGR_NOT_INITIALIZED` when it is invoked without prior initialization or after the module has already been terminated<br>`PLAT_API_SetPowerState()`, `PLAT_API_GetPowerState()`, `PLAT_API_SetWakeupSrc()`, and `PLAT_API_GetWakeupSrc()` each return `PWRMGR_INVALID_ARGUMENT` when called with an out-of-range parameter value or a NULL output pointer. |
