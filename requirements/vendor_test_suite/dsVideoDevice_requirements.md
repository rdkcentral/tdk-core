...
| Req ID | Test Scope |
|:-------|:--------------------------------------|
| `VTS‑DSVIDEODEVICE‑001` | SHALL initialize the Video Device sub-system via `dsVideoDeviceInit()` returning `dsERR_NONE` and terminate it via `dsVideoDeviceTerm()` returning `dsERR_NONE`, supporting re-initialization after a prior termination without error. |
...
| `VTS‑DSVIDEODEVICE‑012` | SHALL enforce the following error code contracts across all `dsVideoDevice` APIs:<br>`dsVideoDeviceInit()` returns `dsERR_ALREADY_INITIALIZED` when it is called while the module is already initialized<br>every `dsVideoDevice` API returns `dsERR_NOT_INITIALIZED` when it is invoked without prior initialization or after the module has already been terminated<br>every `dsVideoDevice` API returns `dsERR_INVALID_PARAM` when it is called with a NULL output pointer, an invalid handle, or an out-of-range value. |
