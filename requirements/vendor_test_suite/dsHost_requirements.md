...
| `VTS-DSHOST-005` | SHALL enforce the following error code contracts across all `dsHost` APIs:<br>`dsHostInit()` SHALL return `dsERR_ALREADY_INITIALIZED` when it is called while the module is already initialized<br>every `dsHost` API SHALL return `dsERR_NOT_INITIALIZED` when it is invoked without prior initialization or after the module has already been terminated<br>every `dsHost` API SHALL return `dsERR_INVALID_PARAM` when it is called with a NULL output pointer or buffer. |
