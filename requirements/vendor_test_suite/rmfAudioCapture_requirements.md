...
| `VTS‑RMFAUDIOCAPTURE‑002` | SHALL start a capture session with a valid data-ready callback and stop it, with non-zero audio data delivered to the registered data-ready callback within the expected timeframe during an active capture session. |
| `VTS‑RMFAUDIOCAPTURE‑003` | SHALL retrieve the default capture settings, the capture status, and the current active settings, each as a populated structure. |
| `VTS‑RMFAUDIOCAPTURE‑004` | SHALL report an invalid-state error for operations invoked in an invalid session state (e.g., opening while already open, closing when closed, or starting/stopping capture out of order), an invalid-handle error for null or outdated handles, and an invalid-parameter error for null handle, settings, or status pointers, an invalid capture type, or out-of-range format, sampling frequency, or callback values. |
