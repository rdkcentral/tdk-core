...
| 5 | Wake DUT from Light Sleep mode | Wake up the DUT from Light Sleep mode by pressing the Power key on the Bluetooth-paired RCU (Remote Control Unit). | The DUT should wake up from Light Sleep mode and the RDK UI should turn on. |
