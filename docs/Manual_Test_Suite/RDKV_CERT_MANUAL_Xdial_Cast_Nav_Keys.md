## TestCase ID
RDKV_MANUAL_XDIAL_06
## TestCase Name
RDKV_CERT_MANUAL_Xdial_Cast_Nav_Keys
...
## Objective
To validate that the navigation keys on the smartphone function correctly on the DUT during an active YouTube XDial casting session. This test confirms that Up, Down, Left, and Right navigation inputs from the smartphone are accurately reflected on the DUT, ensuring navigation key functionality during XDial casting meets certification requirements.
...
| 3 | Select DUT from cast devices popup | Select the DUT from the cast devices popup. | The smartphone should display "Connecting to <VA Device Name>" followed by "Connected to <VA Device Name>". The YouTube Home screen should load on the TV. A "New Device Connected" notification should appear in the top corner of YouTube on the TV.|
| 4 | Tap Cast icon on smartphone for options | Tap the Cast icon again on the smartphone. | A popup should appear on the smartphone displaying the selected DUT name, volume control, Voice Search option, Remote option, and Close/Disconnect options.|
| 5 | Select Remote option from popup | Select the Remote option from the popup. | A remote control screen should appear on the smartphone displaying Up, Down, Left, Right, Play, Mic, and Back navigation buttons.|
| 6 | Use navigation buttons and verify on TV | Use the Up, Down, Left, and Right navigation buttons from the remote control screen on the smartphone. | The corresponding navigation actions should be reflected correctly on the YouTube interface displayed on the TV.|
