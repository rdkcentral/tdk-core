...
## Objective
To validate that AV1 video codec playback is functional on the DUT using the RDK media pipeline. This test confirms that the AV1 asset plays with proper audio and video output without artifacts or errors, ensuring AV1 codec support meets certification requirements.
...
| 9 | Configure AV1 stream URL | Configure the stream variable `video_src_url_short_duration_av1` in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_DASH_AV1_AAC_30Sec/master.mpd"` to point to the AV1 DASH test stream. | The `video_src_url_short_duration_av1` variable should resolve to a valid, accessible stream location. |
...
| 2 | Execute AV1 test script | Select the script `RDKV_CERT_MVS_Video_Play_Full_AV1` and click on 'Execute Now' button. | The script should start execution. |
| 3 | Verify video playback on TV | Verify if the video started playing on the TV and monitor it. | The AV1 asset should play with proper audio and video without any artifacts or errors. |
