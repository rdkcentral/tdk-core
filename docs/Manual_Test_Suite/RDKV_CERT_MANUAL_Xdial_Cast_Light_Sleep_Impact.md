...
## Objective
To validate the XDial casting session behavior when the DUT enters Light Sleep mode and is subsequently woken up during an active casting session. This test confirms that the casting session terminates upon entering Light Sleep and the DUT returns to the home screen after wake-up, ensuring XDial session handling across Light Sleep power transitions meets certification requirements.
...
| 10 | Configure Energy Saver for Light Sleep | Ensure the Energy Saver toggle is set to OFF from Settings > Other Settings > Energy Saver. | The Energy Saver toggle should be set to OFF, activating Light Sleep mode.|
...
| 7 | Put DUT into Light Sleep/Standby mode | Press the Power key on the remote to put the DUT into Light Sleep (Standby) mode. | The DUT should enter Standby mode. The active XDial casting session on the external device should be terminated.|
| 8 | Wake DUT from Standby and verify cast state | Press the Power key on the remote again to wake the DUT from Light Sleep (Standby) mode. | The DUT should wake up and display the RDK UI Home Page. The XDial casting session should not be restored, as it was terminated when the DUT entered Standby mode.|
