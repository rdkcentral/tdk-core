...
RDKV_MANUAL_AV_06
## TestCase Name
RDKV_CERT_MANUAL_AV_Codec_DASH_Playback
...
## Objective
To validate that DASH (.mpd) adaptive streaming playback is functional on the DUT using the RDK media pipeline. This test confirms that the DASH asset plays with proper audio and video output without artifacts or errors, ensuring DASH streaming support meets certification requirements.
...
| 9 | Configure DASH stream URL | Configure the stream variable `video_src_url_short_duration_dash` in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_AAC_15Sec/master.mpd"` to point to the DASH H.264 test stream. | The `video_src_url_short_duration_dash` variable should resolve to a valid, accessible stream location. |
...
| 2 | Execute DASH test script | Select the script `RDKV_CERT_MVS_Video_Play_Full_DASH_H264` and click on 'Execute Now' button. | The script should start execution. |
| 3 | Verify video playback on TV | Verify if the video started playing on the TV and monitor it. | The DASH asset should play with proper audio and video without any artifacts or errors. |
