...
| 3 |  Ensure DUT connected to Wi-Fi on different subnet  | Ensure that the DUT is already connected to a Wi-Fi network on a different subnet than the Ethernet network prior to the test. | The DUT should have an active Wi-Fi connection on a different subnet with a valid IP address.|
