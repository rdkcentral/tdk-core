...
| 3 | Pre-connect DUT to Wi-Fi | Ensure the DUT is already connected to a Wi-Fi network prior to the test. | The DUT should be already connected to a Wi-Fi network.|
...
| 6 | Select different SSID same network | Select a different SSID within the same network. | The password entry screen should launch with an on-screen keyboard and Connect/Cancel buttons.|
...
| 9 | Open network info | Select "Network Info" and press OK. | The Network Info screen should load displaying the newly connected SSID details and the wlan0 IP address with all fields correctly populated.|
