...
| 4 |  Ensure DUT connected to Wi-Fi  | Ensure that the DUT is already connected to a Wi-Fi network prior to the test. | The DUT should have an active Wi-Fi connection with a valid IP address assigned.|
| 5 |  Confirm internet access  | Confirm that internet access is functional on the DUT prior to the test. | Internet access should be confirmed as working on the DUT.|
