...
| 5 | Disconnect and reconnect HDMI cable | Disconnect the HDMI cable from the DUT. Wait approximately 10 seconds, then reconnect the HDMI cable to the DUT. | After HDMI reconnection, the YouTube video should remain in the paused state at the same playback position as before the disconnection.|
| 6 | Exit YouTube application | Exit the YouTube application by pressing the Back key on the Bluetooth remote. | The YouTube application should terminate gracefully and the RDK UI Home screen should be visible on the TV/display.|
