...
## Objective
To validate that HDCP authentication is successfully completed on the DUT during active AV playback over HDMI. This test confirms that the full HDCP authentication handshake between the DUT and the connected HDMI display completes successfully under active media streaming conditions.
...
| 4 | Verify HDCP authenticated log and reason code | Execute the command to check the device log file for the HDCP authenticated event and verify the HDCP reason code.<br>Command: <code>tail -n 500 <hdcp_logs_path> &#124; grep -F -i -o "Updated hdcp_status to AUTHENTICATED" &#124; tail -n 1</code> | The log file should contain `"Updated hdcp_status to AUTHENTICATED"` and the getHDCPStatus API should return `hdcpReason=2`, confirming that HDCP authentication has been successfully completed. |
