...
| 2 | Reboot DUT before test | Reboot the DUT and wait for the device to complete its full boot sequence before proceeding with the test. | The DUT must complete its boot sequence successfully with all RDK services running and accessible prior to test execution. |
