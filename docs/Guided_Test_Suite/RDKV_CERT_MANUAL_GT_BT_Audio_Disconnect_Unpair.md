## TestCase ID
RDKV_MANUAL_GT_BT_AUDIO_03
## TestCase Name
RDKV_CERT_MANUAL_GT_BT_Audio_Disconnect_Unpair
...
## Objective
To validate that an actively streaming external Bluetooth wearable headset can be successfully disconnected and unpaired from the DUT, with the device correctly removed from both the connected and paired device lists. This test confirms that the Bluetooth teardown workflow on the DUT correctly clears all association records for a previously active audio streaming device.
