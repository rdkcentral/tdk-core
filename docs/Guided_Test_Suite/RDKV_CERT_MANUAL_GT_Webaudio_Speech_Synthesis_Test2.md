## TestCase ID
RDKV_MANUAL_GT_WEBAUDIO_02
## TestCase Name
RDKV_CERT_MANUAL_GT_Webaudio_Speech_Synthesis_Test2
...
## Objective
To validate that the DUT's Web Audio Speech Synthesis API correctly handles supported audio language configurations and produces audible TTS output when activated through the second speech synthesis test in the Webaudio_manual application. This test confirms the DUT's TTS implementation correctly processes and speaks text using supported language voice profiles.
...
| 2 | Navigate to speech synthesis test 2 and click speak | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>— Press the **Tab** key **2 times** to navigate to speech synthesis test case 2 (supported audio languages), then press the **Enter** key **once** to select it<br>— Press the **Tab** key **2 more times** to navigate to the **[speak]** button, then press the **Enter** key **once** to activate it | The app should navigate to the speech synthesis test 2 and the [speak] button should be activated successfully. |
