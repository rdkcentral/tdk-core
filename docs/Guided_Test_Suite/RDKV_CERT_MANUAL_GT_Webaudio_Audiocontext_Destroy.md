...
## Objective
To validate that the DUT's WebKit browser can perform repeated AudioContext creation and destruction cycles via the Web Audio API without triggering a crash or unrecoverable error. This test confirms the AudioContext lifecycle management on the DUT is stable and does not produce memory leaks or fatal errors during rapid create/destroy operations.
