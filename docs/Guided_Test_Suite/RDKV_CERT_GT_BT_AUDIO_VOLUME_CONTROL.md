...
## Objective
To validate that the volume of a connected external Bluetooth device can be increased and decreased through progressive volume levels using the org.rdk.Bluetooth.setDeviceVolumeMuteInfo and getDeviceVolumeMuteInfo APIs, and that the tester perceives the corresponding volume changes audibly during active audio streaming.
