## TestCase ID
RDKV_MANUAL_GT_WEBAUDIO_03
## TestCase Name
RDKV_CERT_MANUAL_GT_Webaudio_Speech_Synthesis_Test3
...
## Objective
To validate that the DUT's Web Audio Speech Synthesis API can continuously synthesize speech across multiple audio languages using randomly selected voice profiles through the third speech synthesis test in the Webaudio_manual application. This test confirms the DUT's TTS implementation supports multi-language speech synthesis with random voice variation without errors.
...
| 2 | Navigate to speech synthesis test 3 and click speak with Random Voices | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>— Press the **Tab** key **3 times** to navigate to speech synthesis test case 3 (3 different audio languages), then press the **Enter** key **once** to select it<br>— Press the **Tab** key **2 more times** to navigate to the **[speak with Random Voices]** button, then press the **Enter** key **once** to activate it | The app should navigate to the speech synthesis test 3 and the [speak with Random Voices] button should be activated successfully. |
| 3 | Verify random voices audio output | The script prompts: *"Is text-to-speech audio with random voices heard when the [speak with Random Voices] button is pressed on the Webaudio_manual App [yes/no]:"* — the tester must listen and respond `yes` if audio with 3 different random language voices is audible. | The tester should confirm with `yes` that text-to-speech audio with random voices (3 different audio languages continuously) is audible when the [speak with Random Voices] button is pressed. |
