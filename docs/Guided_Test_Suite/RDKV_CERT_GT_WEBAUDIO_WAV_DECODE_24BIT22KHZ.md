...
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_WAV_DECODE_24BIT22KHZ
...
## Objective
To validate that the DUT's WebKit browser correctly decodes a WAV audio file encoded at 24-bit depth with 22kHz sample rate (with resampling) using the Web Audio API's `decodeAudioData` function, and that the Webaudio_manual App displays the decoded codec metadata on an inner HTML page, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test navigates to the WAV 24bit-22kHz decode test using `org.rdk.RDKWindowManager.generateKey` (Tab×14 → Enter×1) and confirms via tester input that decoded audio information is visible. This test ensures the DUT correctly handles WAV format decoding with 24-bit depth and 22kHz resampling via the Web Audio API.
