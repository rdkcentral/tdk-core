...
## Objective
To validate that the DUT's NetworkManager Ping API successfully delivers IPv6 ICMP packets to a configured IPv6 endpoint with zero packet loss when operating over a WiFi-only network with Ethernet disconnected. This test confirms IPv6 ICMP reachability and bidirectional packet delivery over the WiFi path, verifying the DUT's IPv6 Ping diagnostic capability.
