...
## Objective
To validate that the DUT's WebKit browser correctly generates FM synthesized sound using Web Audio API oscillator and gain nodes, producing audible buzzer output through the Webaudio_manual application. This test confirms the DUT's Web Audio API FM sound synthesis capability is correctly implemented and produces audible output.
...
| 2 | Navigate to FM sound synthesis test and click start synthesis | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>— Press the **Tab** key **7 times** to navigate to the Generated Sound FM test case, then press the **Enter** key **once** to select it<br>— Press the **Tab** key **1 more time** to navigate to the **[start synthesis]** button, then press the **Enter** key **once** to activate it and initiate FM sound generation | The app should navigate to the Generated Sound FM test case and the [start synthesis] button should be activated successfully. |
| 3 | Verify FM sound synthesis audio output | The script prompts: *"Is a buzzer/warning sound audio heard when the [start synthesis] button is pressed on the Webaudio_manual App [yes/no]:"* — the tester must listen and respond `yes` if a buzzer/warning sound is audible. | The tester should confirm with `yes` that a buzzer/warning sound is audible when the [start synthesis] button is pressed on the Webaudio_manual App. |
