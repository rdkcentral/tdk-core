...
RDKV_GT_WEBAUDIO_02
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_SPEECH_SYNTHESIS_TEST2
...
## Objective
To validate that the second speech synthesis test within the Webaudio_manual App — targeting supported audio languages — functions correctly by producing audible text-to-speech output when the [speak] button is activated, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test uses `org.rdk.RDKWindowManager.generateKey` navigation (Tab×2 → Enter×1 → Tab×2 → Enter×1) to select and activate the speech test, and verifies TTS audio output via tester confirmation. This test ensures the WebAudio Speech Synthesis API on the DUT correctly handles supported audio language configurations.
...
| 2 | Navigate to speech synthesis test 2 and click speak | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>**Tab×2 → Enter×1** to select speech synthesis test case 2 (supported audio languages) in the app<br>**Tab×2 → Enter×1** to click the **[speak]** button | The app should navigate to the speech synthesis test 2 and the [speak] button should be activated successfully. |
