## TestCase ID
RDKV_MANUAL_WEBAUDIO_01
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_SPEECH_SYNTHESIS_TEST1
...
## Objective
To validate that the first speech synthesis test within the Webaudio_manual App functions correctly by navigating to and activating the [speak] button, producing audible text-to-speech audio output on the DUT, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test launches the app via `org.rdk.AppManager.launchApp`, uses `org.rdk.RDKWindowManager.setFocus` and `generateKey` (Tab×1 → Enter×1 → Tab×2 → Enter×1) for navigation, and confirms audio output via tester confirmation before terminating the app with `org.rdk.AppManager.terminateApp`. This test ensures the WebAudio Speech Synthesis API implementation on the DUT correctly produces audible TTS output from the Webaudio_manual test application.
...
| 2 | Navigate to speech synthesis test 1 and click speak | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>**Tab×1 → Enter×1** to select speech synthesis test case 1 in the app<br>**Tab×2 → Enter×1** to click the **[speak]** button | The app should navigate to the speech synthesis test 1 and the [speak] button should be activated successfully. |
