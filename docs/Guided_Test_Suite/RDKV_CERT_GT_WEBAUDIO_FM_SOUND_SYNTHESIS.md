...
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_FM_SOUND_SYNTHESIS
...
## Objective
To validate that the Webaudio_manual App can generate an FM synthesized sound using the Web Audio API's oscillator and gain nodes by activating the [start synthesis] button, producing an audible buzzer/warning sound on the DUT, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test uses `org.rdk.RDKWindowManager.generateKey` navigation (Tab×7 → Enter×1 → Tab×1 → Enter×1) to select the FM sound synthesis test and activate the synthesizer, with tester confirmation of audible buzzer/warning output. This test ensures the DUT's Web Audio API FM synthesis capabilities are correctly implemented and produce audible output through the Webaudio_manual application.
...
| 2 | Navigate to FM sound synthesis test and click start synthesis | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>**Tab×7 → Enter×1** to select the Generated Sound FM test case<br>**Tab×1 → Enter×1** to click the **[start synthesis]** button to initiate FM sound generation | The app should navigate to the Generated Sound FM test case and the [start synthesis] button should be activated successfully. |
| 3 | Verify FM sound synthesis audio output | The script prompts: *"Is an (Buzzer/warning sound) Audio heard when [start synthesis] button is pressed on Webaudio_manual App [yes/no]:"* — the tester must listen and respond `yes` if a buzzer/warning sound is audible. | The tester should confirm with `yes` that a buzzer/warning sound is audible when the [start synthesis] button is pressed on the Webaudio_manual App. |
