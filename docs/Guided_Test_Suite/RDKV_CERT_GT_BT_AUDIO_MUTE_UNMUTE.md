...
## Objective
To validate that the volume mute and unmute operations on a connected external Bluetooth device can be controlled via the org.rdk.Bluetooth.setDeviceVolumeMuteInfo and getDeviceVolumeMuteInfo APIs, and that the audio streaming state on the BT device reflects the mute and unmute settings correctly.
...
| 13 | Verify muted state via getDeviceVolumeMuteInfo | The script executes the following curl command to verify the mute status of the BT device and confirms the `mute` field returns `true`:<br>`curl --header "Content-Type: application/json" --request POST --data '{"jsonrpc": "2.0", "id": 42, "method": "org.rdk.Bluetooth.getDeviceVolumeMuteInfo", "params": {"deviceID": "<device_id>", "deviceType": "WEARABLE HEADSET"}' http://127.0.0.1:9998/jsonrpc` | The getDeviceVolumeMuteInfo API response should return the `mute` field as `true`, confirming the device is in muted state. |
