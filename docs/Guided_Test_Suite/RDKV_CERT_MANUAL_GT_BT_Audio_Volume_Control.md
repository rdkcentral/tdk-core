## TestCase ID
RDKV_MANUAL_GT_BT_AUDIO_06
## TestCase Name
RDKV_CERT_MANUAL_GT_BT_Audio_Volume_Control
...
## Objective
To validate that progressive volume increase and decrease operations on a connected external Bluetooth wearable headset are correctly applied and accurately reflected via the Bluetooth volume management API during active A2DP audio streaming. This test confirms the DUT's Bluetooth volume control accurately adjusts and reports audio output levels on the connected external streaming device, with both API-level and auditory verification at each step.
