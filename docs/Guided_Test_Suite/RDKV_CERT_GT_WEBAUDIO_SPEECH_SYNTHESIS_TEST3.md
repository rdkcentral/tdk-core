...
RDKV_GT_WEBAUDIO_03
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_SPEECH_SYNTHESIS_TEST3
...
## Objective
To validate that the third speech synthesis test within the Webaudio_manual App can continuously synthesize speech across three different audio languages by activating the [speak with Random Voices] button, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test uses `org.rdk.RDKWindowManager.generateKey` navigation (Tab×3 → Enter×1 → Tab×2 → Enter×1) to select and activate the multi-language speech test, and verifies that audio with random voice variations is audible via tester confirmation. This test ensures the WebAudio Speech Synthesis API correctly supports multi-language TTS with randomly selected voice profiles.
...
| 2 | Navigate to speech synthesis test 3 and click speak with Random Voices | The script sets focus on the Webaudio_manual App via `org.rdk.RDKWindowManager.setFocus`, then sends the following key sequence via `org.rdk.RDKWindowManager.generateKey`:<br>**Tab×3 → Enter×1** to select speech synthesis test case 3 (3 different audio languages) in the app<br>**Tab×2 → Enter×1** to click the **[speak with Random Voices]** button | The app should navigate to the speech synthesis test 3 and the [speak with Random Voices] button should be activated successfully. |
| 3 | Verify random voices audio output | The script prompts: *"Is texts speaking with random voices audio heard when [speak with Random Voices] button is pressed on Webaudio_manual App [yes/no]:"* — the tester must listen and respond `yes` if audio with 3 different random language voices is audible. | The tester should confirm with `yes` that text-to-speech audio with random voices (3 different audio languages continuously) is audible when the [speak with Random Voices] button is pressed. |
