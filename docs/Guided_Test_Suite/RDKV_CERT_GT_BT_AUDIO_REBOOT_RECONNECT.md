...
## Objective
To validate that when the external Bluetooth device is rebooted during active audio streaming, the audio stops as expected, and that the DUT automatically reconnects to the external Bluetooth device within a configurable polling period after the device comes back online.
