...
## TestCase Name
RDKV_CERT_GT_WEBAUDIO_AUDIOCONTEXT_DESTROY
...
## Objective
To validate that the Webaudio_manual App can perform repeated `AudioContext` creation and destruction cycles without causing a crash or unrecoverable error on the DUT, as exercised by the `WEBAUDIO_manual_automated.sh` script. The test uses `org.rdk.RDKWindowManager.generateKey` navigation (Tab×5 → Enter×1) to load the AudioContext creation/destruction test page, and confirms via tester input that the message 'This test passes if it does not crash' is displayed. This test ensures the Web Audio API `AudioContext` lifecycle management on the DUT is stable and does not produce memory leaks or fatal errors during rapid create/destroy operations.
