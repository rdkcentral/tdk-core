...
| 2 | Validate device build version | Execute the command to retrieve and validate the build version of the device.<br>Command: <code>grep -i "imagename" /version.txt &#124; cut -d: -f2- &#124; xargs</code> | The build version should be retrieved successfully and the image name should contain the RDK build identifier. |
