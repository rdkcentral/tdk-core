...
| 3 | Verify HDCP authentication logs | Execute the commands to check the device log file for both the HDMI connect and HDCP authentication events.<br>Command 1: <code>tail -n 500 <hdcp_logs_path> &#124; grep -F -i -o "Updated hotplug to CONNECTED" &#124; tail -n 1</code><br>Command 2: <code>tail -n 500 <hdcp_logs_path> &#124; grep -F -i -o "Updated hdcp_status to AUTHENTICATED" &#124; tail -n 1</code> | The log file should contain both `"Updated hotplug to CONNECTED"` and `"Updated hdcp_status to AUTHENTICATED"`, confirming that HDCP authentication was successfully initiated after the hotplug operation. |
