...
| 4 | Set video URL | Set the configured video URL in Cobalt using the deeplink method via applicable API calls.<br>{"jsonrpc":"2.0","id":1234567890,"method":"Cobalt.1.deeplink","params":{"url":"<cobalt_test_url>"}} | Video URL should be set and Cobalt should start loading the video. |
| 5 | Validate video playback | Verify that video is playing by checking decoder process entries via applicable API calls. This checks for an active video decoder (decoder_count > 0). | Video should be playing (proc entry for video decoder should be present). |
