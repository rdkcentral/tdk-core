...
## Objective
To validate the RDKShell moveToFront functionality by launching both Cobalt and WebKitBrowser plugins and repeatedly moving them to the front of the z-order for moveto_operation_max_count iterations, verifying the z-order changes correctly and CPU/memory usage remains within expected limits.
...
|4|webkit_url must be configured in StabilityTestVariables.|
...
| 3 | Launch Cobalt and WebKitBrowser | Launch Cobalt via `launch_plugin`. Launch WebKitBrowser via `launch_plugin`. Set webkit_url in WebKitBrowser: `{"jsonrpc":"2.0","id":1234567890,"method":"WebKitBrowser.1.url","params":{"value":"<webkit_url>"}}` | Both Cobalt and WebKitBrowser should launch successfully. |
...
| 5 | MoveToFront loop (repeat for moveto_operation_max_count iterations) | For each iteration: <br>Get current z-order via `org.rdk.RDKShell.1.getZOrder`. <br>Determine which plugin is at the back of the z-order (should move to front). <br>Move the backmost plugin to front: `{"jsonrpc":"2.0","id":1234567890,"method":"org.rdk.RDKShell.1.moveToFront","params":{"client":"<plugin>"}}` <br>Get z-order again to verify the plugin moved to front. <br>Validate CPU load and memory usage via `rdkservice_validateResourceUsage`. | Plugin should be moved to the front of the z-order successfully. Z-order verification should confirm the change. CPU load and memory usage should remain within expected limits. |
...
| 8 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the moveToFront stress test. |
