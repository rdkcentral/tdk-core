...
| 5 | Validate video playback | Verify that the encrypted video is playing by checking decoder process entries via `rdkservice_validateProcEntry`. | Video should be playing (video decoder proc entry should be present). |
| 6 | Monitor resource usage (every 300 seconds for cobalt_test_duration) | Every 5 minutes for the configured test duration: <br>Validate CPU load and memory usage via `rdkservice_validateResourceUsage` using DeviceInfo.1.systeminfo. <br>Verify encrypted video is still playing by re-checking the proc entry for video decoder. | CPU load and memory usage should remain within expected limits. Encrypted video should play without interruption. |
