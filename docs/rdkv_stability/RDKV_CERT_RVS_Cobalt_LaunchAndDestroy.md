...
| 2 | Check and set plugin preconditions | Get the status of Cobalt, WebKitBrowser, and DeviceInfo plugins via applicable API calls. Set required states: Cobalt=deactivated, WebKitBrowser=deactivated, DeviceInfo=activated. | All required plugins should be in their expected states. |
| 3 | Launch and destroy loop (repeat for launch_and_destroy_max_count iterations) | For each iteration: <br>Launch Cobalt using : launch Cobalt via `org.rdk.RDKShell.1.launch` → check status is activated → destroy Cobalt via `org.rdk.RDKShell.1.destroy` → check status is deactivated. <br>Validate CPU load and memory usage via using DeviceInfo.1.systeminfo. | Cobalt should launch and be destroyed successfully in each iteration. CPU load and memory usage should remain within expected limits. |
