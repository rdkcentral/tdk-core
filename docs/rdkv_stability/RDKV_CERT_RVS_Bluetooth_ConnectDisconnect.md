...
| 2 | Check and set plugin preconditions | Get the status of org.rdk.Bluetooth and DeviceInfo plugins via applicable API calls. Activate any plugin not in the required state using applicable API calls. Required states: org.rdk.Bluetooth=activated, DeviceInfo=activated. | All required plugins should be in their expected states. |
...
| 12 | Revert plugins | Restore plugins to their original state using applicable API calls. | Plugins should be reverted to their pre-test states. |
| 13 | Check device state post-condition | Verify the device is still in a stable state after completing the test using applicable API calls. | Device should remain stable after the stress test. |
