...
## Objective
To validate AppManager stability by repeatedly launching and forcefully killing an app using the killApp method, and verifying that resource usage (CPU and memory) remains within expected limits across multiple iterations with no resource leaks.
...
|5|launch_kill_count should be configured in StabilityTestVariables with required iteration count.|
...
| 6 | Step 6 | Kill the launched app using the killApp method (forceful termination). <br>Kill API: {"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.killApp", "params": {"appId": "com.rdkcentral.google"}}<br>Wait 10 seconds for resource cleanup. | Kill API should return success (result null/None is acceptable). App should be forcefully terminated and resources released. |
| 7 | Step 7 | Validate resource usage (CPU and memory) after the launch-kill cycle. <br>{"jsonrpc": "2.0", "id": 1234567890, "method": "DeviceInfo.1.systeminfo"} | CPU load and memory usage should remain within configured thresholds. Resource usage should not show increasing trend indicating leaks. |
| 8 | Step 8 | Repeat Step 4, Step 5, Step 6, and Step 7 for configured launch_kill_count iterations (same workflow for each iteration). | Every iteration should complete successfully with launch, kill, and resource validation showing stable resource consumption. |
