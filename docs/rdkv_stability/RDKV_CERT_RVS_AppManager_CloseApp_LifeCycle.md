...
| 8 | Validate all expected close lifecycle events (Per Iteration) | Monitor the onAppLifecycleStateChanged event buffer for up to 120 seconds after the close request. Verify that both expected close lifecycle states are received for com.rdkcentral.google: APP_STATE_ACTIVE (confirming the app was active before close) and APP_STATE_PAUSED (confirming the app transitioned to paused state). The monitoring loop exits when APP_STATE_PAUSED is observed. <br>Example event: `{"jsonrpc": "2.0", "method": "client.events.1.onAppLifecycleStateChanged", "params": {"appId": "com.rdkcentral.google", "newState": "APP_STATE_PAUSED", "oldState": "APP_STATE_ACTIVE", "errorReason": "APP_ERROR_NONE"}}` | Both expected close lifecycle states (APP_STATE_ACTIVE and APP_STATE_PAUSED) should be received within the timeout period for each iteration. |
| 9 | Repeat close lifecycle validation for all iterations | Repeat Steps 6 through 8 for all `lifecycle_count` (100) configured iterations. | Every iteration should receive both expected close lifecycle events successfully, confirming a valid close lifecycle transition sequence. |
