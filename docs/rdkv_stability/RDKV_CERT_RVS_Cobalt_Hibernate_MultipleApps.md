...
| 9 | Validate resource usage | Validate CPU load and memory usage via `rdkservice_validateResourceUsage`. | CPU load and memory usage should be within expected limits. |
...
| 11 | Revert plugins | Restore plugins to their original state using `rdkservice_setPluginStatus`. | Plugins should be reverted to their pre-test states. |
| 12 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the hibernate multi-app test. |
