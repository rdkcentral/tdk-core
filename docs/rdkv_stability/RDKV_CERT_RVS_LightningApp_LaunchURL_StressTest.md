...
RDKV_STABILITY_80
## TestCase Name
RDKV_CERT_RVS_LightningApp_LaunchURL_StressTest
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate LightningApp plugin stability by repeatedly setting a video stream URL 100 times without destroying the plugin between iterations, verifying the URL is loaded correctly and CPU/memory usage remains within expected limits.
...
|2|LightningApp plugin must be in resumed state; Cobalt plugin must be deactivated; DeviceInfo plugin must be activated.|
...
| 3 | URL stress test loop (100 iterations without destroy) | For each of 100 iterations: <br>Set the HLS video stream URL in LightningApp: `{"jsonrpc":"2.0","id":1234567890,"method":"LightningApp.1.url","params":{"value":"<video_src_url_hls>"}}` <br>Verify the URL is set correctly by getting the current URL: `{"jsonrpc":"2.0","id":1234567890,"method":"LightningApp.1.url"}` <br>Validate CPU load and memory usage via `rdkservice_validateResourceUsage`. Note: LightningApp is NOT destroyed between iterations. | URL should be set and verified successfully in each iteration. CPU load and memory usage should remain within expected limits without plugin restart. |
...
| 6 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the URL stress test. |
