...
| 7 | Validate all expected launch lifecycle events (Per Iteration) | Monitor the onAppLifecycleStateChanged event buffer for up to 120 seconds. Verify that all four expected lifecycle states are received for com.rdkcentral.google: APP_STATE_LOADING, APP_STATE_INITIALIZING, APP_STATE_PAUSED, and APP_STATE_ACTIVE. <br>Example event: `{"jsonrpc": "2.0", "method": "client.events.1.onAppLifecycleStateChanged", "params": {"appId": "com.rdkcentral.google", "newState": "APP_STATE_ACTIVE", "oldState": "APP_STATE_PAUSED", "errorReason": "APP_ERROR_NONE"}}` | All four expected launch lifecycle states (APP_STATE_LOADING, APP_STATE_INITIALIZING, APP_STATE_PAUSED, APP_STATE_ACTIVE) should be observed for each iteration. |
| 8 | Terminate application (Per Iteration) | After all lifecycle events are validated, terminate the launched application using the AppManager terminateApp API. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.terminateApp", "params": {"appId": "com.rdkcentral.google"}}` | The application should terminate successfully. |
| 9 | Repeat launch lifecycle validation for all iterations | Repeat Steps 6 through 8 for all `lifecycle_count` (100) configured iterations. | Every iteration should receive all four expected launch lifecycle events and complete the terminate cycle successfully. |
