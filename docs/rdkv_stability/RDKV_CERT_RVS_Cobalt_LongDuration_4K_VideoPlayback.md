...
RDKV_STABILITY_66
## TestCase Name
RDKV_CERT_RVS_Cobalt_LongDuration_4K_VideoPlayback
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate device stability by playing a 4K video from Cobalt for a configured duration, measuring CPU load and memory usage every 5 minutes to ensure resources remain within expected limits.
...
|3|cobalt_test_url (4K stream URL) and cobalt_test_duration must be configured in StabilityTestVariables.|
...
| 4 | Set 4K video URL | Set the configured 4K video URL in Cobalt using the deeplink method.<br>{"jsonrpc":"2.0","id":1234567890,"method":"Cobalt.1.deeplink","params":{"url":"<cobalt_4k_test_url>"}} | 4K video URL should be set and Cobalt should start loading the video. |
| 5 | Validate video playback | Verify that the 4K video is playing by checking decoder process entries via `rdkservice_validateProcEntry`. | Video should be playing (video decoder proc entry should be present). |
| 6 | Monitor resource usage (every 300 seconds for cobalt_test_duration) | Every 5 minutes for the configured test duration: <br>Validate CPU load and memory usage via `rdkservice_validateResourceUsage` using DeviceInfo.1.systeminfo. <br>Verify 4K video is still playing by re-checking the proc entry for video decoder. | CPU load and memory usage should remain within expected limits throughout the test duration. 4K video should continue playing without interruption. |
