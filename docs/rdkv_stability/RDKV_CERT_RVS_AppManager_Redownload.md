...
| 3 | Configure app_download_url in PerformanceTestVariables | `app_download_url` must be set to the base URL where the validation file is hosted. The full download URL is constructed as `app_download_url + "/" + Small_Validation_File` ("Any file of size 10 MB"). | The app_download_url should point to a reachable and valid hosting location. |
