...
| 3 | Launch Cobalt and set 4K video URL | Launch Cobalt via `launch_plugin`. Set 4K video URL via deeplink: `{"jsonrpc":"2.0","id":1234567890,"method":"Cobalt.1.deeplink","params":{"url":"<cobalt_4k_url>"}}` | Cobalt should launch and start loading the 4K video. |
| 4 | Validate 4K video playback | Verify 4K video is playing via `rdkservice_validateProcEntry`. | Video decoder proc entry should be present, indicating 4K video is playing. |
...
| 8 | Validate resource usage after restore | Validate CPU load and memory usage via `rdkservice_validateResourceUsage`. | CPU load and memory usage should be within expected limits after restore. |
...
| 11 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the 4K hibernate memory test. |
