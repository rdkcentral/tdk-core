...
| 2 | Check and set plugin preconditions | Get the status of webkit_instance, Cobalt, and DeviceInfo plugins via `rdkservice_getPluginStatus`. Set required states: webkit_instance=resumed, Cobalt=deactivated, DeviceInfo=activated. | All required plugins should be in their expected states. |
...
| 7 | Monitor channel changes (until max_channel_change_count is reached) | While channel change count is less than max_channel_change_count: <br>Retrieve events from the webinspect event buffer via `getEventsBuffer()`. <br>Check for "Count" in console log output using `rdkservice_checkChannelChangeLog`. <br>Increment channel_change_count when a change event is detected. <br>Validate CPU load and memory usage via `rdkservice_validateResourceUsage` after each confirmed channel change. <br>Exit monitoring loop if continue_count exceeds 20 (no new events). | Channel changes should be detected in the console log. CPU load and memory usage should remain within limits per iteration. |
...
| 10 | Revert plugins | Restore plugins to their original state using `rdkservice_setPluginStatus`. | Plugins should be reverted to their pre-test states. |
| 11 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the stress test. |
