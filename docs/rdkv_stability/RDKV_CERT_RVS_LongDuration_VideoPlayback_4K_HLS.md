...
| 4 | Validate video playback | Verify that video is playing by checking decoder process entries via `rdkservice_validateProcEntry`. | Video decoder proc entry should be present, confirming 4K HLS video playback. |
...
| 8 | Check device state post-condition | Verify the device is still in a stable state after completing the test using `check_device_state`. | Device should remain stable after the long-duration 4K HLS playback test. |
