...
RDKV_MANUAL_AV_12
## TestCase Name
RDKV_CERT_MANUAL_AV_CODEC_EAC3_PLAYBACK
...
## Objective
To validate that EAC3 (Enhanced AC-3) audio stream playback is functional on the DUT via GStreamer.
...
|#|Step Name | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Execute GStreamer command for EAC3 | Execute the GStreamer command to play the EAC3 stream.<br>Command: `gst-launch-1.0 playbin uri=<EAC3_URL>` | The EAC3 audio stream should play without any distortion or errors. |
