...
## Objective
To validate that switching the DUT network connection from Ethernet to Wi-Fi during an active XDial casting session terminates the casting session while video playback continues on the DUT.
...
| 3 | Connect DUT via Ethernet | The DUT shall be connected via Ethernet prior to test execution. | The DUT should be connected via Ethernet with a valid IP address assigned. |
