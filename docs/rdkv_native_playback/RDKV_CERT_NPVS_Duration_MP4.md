## TestCase ID
RDKV_NATIVE_PLAYBACK_191

## TestCase Name
RDKV_CERT_NPVS_Duration_MP4
...
## Objective
Validate stream duration retrieval and accuracy for MP4 container format duration query API (). Query total stream duration in nanosecond precision and verify the returned value matches expected baseline duration within +/-250ms tolerance. Confirm MP4 demultiplexer correctly extracts duration metadata from ftyp/moov atoms and communicates accurate timing information to the playbin. Monitor playback position advancement to verify position increments correlate with query duration. Validate frame rendering statistics show consistent H.264 video frame delivery without stalls or drops during full duration playback. Verify test successfully parses duration from mediapipelinetests output log and confirms retrieved value matches expected 9-second baseline within tolerance threshold.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  MP4 container stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_MP4.mp4"` in MediaValidationVariables.py. Stream contains H.264 video and AAC audio (MP4 container format for duration testing))<br>Stream variable `video_src_url_mp4` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_MP4.mp4"`  | Verify MP4 file is accessible and readable from configured path, qtdemux parser available for duration extraction from moov atom Verify `video_src_url_mp4` resolves to valid MP4 container file with moov atom containing duration metadata |
...
| 4 | Transition Pipeline to PLAYING State | Set pipeline state to `GST_STATE_PLAYING` via `gst_element_set_state(playbin, GST_STATE_PLAYING)`<br> Poll `gst_element_get_state()` until state change completes (`GST_STATE_CHANGE_SUCCESS`)<br> Confirm `firstFrameReceived == true` callback was triggered indicating first H.264 frame rendered on sink<br> `qtdemux` element automatically demultiplexes MP4 file and initializes audio/video streams | Verify pipeline reaches `GST_STATE_PLAYING` without `GST_MESSAGE_ERROR`, state transition completes synchronously or asynchronously, first-frame signal received |
| 5 | Query Stream Duration Using GStreamer Query API | Invoke `gst_element_query_duration(playbin, GST_FORMAT_TIME, &duration)` to retrieve total MP4 stream duration in nanoseconds<br> Validate query returns TRUE (success)<br> Verify retrieved duration != -1 (indicates valid, parseable MP4 file)<br> For MP4 streams, `qtdemux` extracts duration from ftyp/moov atoms<br> Convert duration from nanoseconds to seconds via `GST_TIME_AS_SECONDS(duration)` and milliseconds via `GST_TIME_AS_MSECONDS(duration) % 1000` for logging and comparison | Verify duration query succeeds, returned value is positive integer (not -1), duration formatted correctly in M:SS.MS format |
