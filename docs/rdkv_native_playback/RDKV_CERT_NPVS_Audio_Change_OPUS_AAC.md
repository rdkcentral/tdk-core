## TestCase ID
RDKV_NATIVE_PLAYBACK_123

## TestCase Name
RDKV_CERT_NPVS_Audio_Change_OPUS_AAC
...
## Objective
Test to verify if dynamic audio codec switching (OPUS to AAC) is possible while maintaining video playback continuity. Validate that audio track can be switched between different codec types (OPUS and AAC) present in the same multi-codec stream without interrupting video playback. Specifically validate audio rendering quality, position synchronization, and codec switching timing accuracy during active playback. Verify codec identity is confirmed through tag comparison.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  Multi-codec stream containing both OPUS and AAC audio tracks must be accessible via HTTPS (`souphttpsrc`))<br>or local file system (`filesrc`))<br>with DASH manifest. Stream configured as `test_streams_base_path + "MultiCodecStreams/stream_with_opus_aac.mpd"` in MediaValidationVariables.py Stream variable `video_src_url_opus_aac` configured in `MediaValidationVariables.py` with DASH manifest containing OPUS and AAC audio tracks  | Verify multi-codec DASH stream accessible, both OPUS and AAC audio tracks present and parseable Verify `video_src_url_opus_aac` resolves to valid multi-codec stream with dual audio tracks |
...
| 2 | Configure Test Parameters and Load Multi-Codec Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` from device<br> Build `mediapipelinetests` command with test_name="test_playback_audio_change", stream_url=video_src_url_opus_aac, timeout=N | Verify device config retrieved successfully, command constructed with correct parameters |
...
| 4 | Retrieve and Verify Multiple Audio Tracks |  Query playbin to determine number of audio streams<br>via `g_object_get(playbin, "n-audio", &n_audio, NULL)`. Verify n-audio >= 2 (confirming OPUS and AAC tracks present). Query current audio index<br>via `g_object_get(playbin, "current-audio", &current_audio, NULL)` to identify default (OPUS))<br>track  | Verify n-audio returns at least 2, current-audio successfully retrieved (typically 0 for first codec) |
...
| 7 | Verify Second AAC Audio Playback and Retrieve Audio-Tags | After seek-flush completes, playback continues with AAC codec now active<br> Emit "get-audio-tags" signal for AAC track via `g_signal_emit_by_name(playbin, "get-audio-tags", 1, &tags)`<br> Parse tag list using `gst_tag_list_foreach()` to identify codec name (AAC)<br> Log audio-tags including codec type, sample rate, channels<br> Compare logged tags from Step 5 (OPUS) with current tags (AAC) to confirm different codec | Verify audio-tags successfully retrieved showing AAC codec (e.g., audio/mpeg, different from OPUS), codec change confirmed through tag comparison |
