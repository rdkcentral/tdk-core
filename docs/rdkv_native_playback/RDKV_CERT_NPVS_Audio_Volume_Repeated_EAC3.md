## TestCase ID
RDKV_NATIVE_PLAYBACK_138

## TestCase Name
RDKV_CERT_NPVS_Audio_Volume_Repeated_EAC3
...
| 2 | Stream Provisioning and Configuration | EAC3 H.264 DASH stream must be accessible via HTTPS (`souphttpsrc`) or local file system (`filesrc`) or local file system<br> Stream URL is configured via MediaValidationVariables.py variable `video_src_url_ec3` = `test_streams_base_path + "DASH_H264_EAC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd"`<br> Resolves to: `http://<TM_IP>:8080/TDK_Clear_Test_Streams_Sunrise/DASH_H264_EAC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd` Stream variable `video_src_url_ec3` must be configured in `MediaValidationVariables.py`<br> Path: `DASH_H264_EAC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd` containing EAC3 audio track with H.264 video codec | Verify DASH manifest is accessible and parseable, contains H.264 video codec streams and EAC3 audio codec streams, stream files can be downloaded via HTTP(S) Verify `video_src_url_ec3` variable resolves to valid, accessible DASH stream location with EAC3 audio codec and H.264 video codec available |
| 3 | Test Parameters and Volume Repeated Configuration |  Test execution parameters must include: volume repeated levels = `{1.0, 0.75, 0.5, 0.25, 0.0}` (CUBIC scale),<br>playback duration per level = `15` seconds (5 seconds initial + 10 seconds continued),<br>timeout = `75` seconds total (5 levels � 15 seconds each)  | Verify volume_repeated_levels are defined, 15-second duration per level configured, total timeout set to 75 seconds |
