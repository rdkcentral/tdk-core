## TestCase ID
RDKV_NATIVE_PLAYBACK_237

## TestCase Name
RDKV_CERT_NPVS_SeekBackward_FF2x_H264_29Fps
...
| 2 | Create Playbin and Configure Video Sink |  Create `playbin` element<br>via `gst_element_factory_make("playbin", NULL)`. Configure URI<br>via `g_object_set(playbin, "uri", stream_url, NULL)` with H.264 29fps stream. Create and set `westerossink` as video sink  | Playbin created, URI configured, westerossink set as video sink |
