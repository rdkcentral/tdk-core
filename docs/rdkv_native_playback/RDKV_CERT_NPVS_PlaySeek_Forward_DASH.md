## TestCase ID
RDKV_NATIVE_PLAYBACK_60

## TestCase Name
RDKV_CERT_NPVS_PlaySeek_Forward_DASH
...
| 2 | Configure Seek Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` (playback duration for each seek operation), `NATIVE_PLAYBACK_SEEK_POSITION` (target position for forward seek, default 20 seconds);<br>Retrieve stream URL from `video_src_url_dash` variable; Construct seek operation string with `seek:timeout:position`;<br>Execute `mediapipelinetests test_trickplay <URL> operations=<seek_string>` to prepare seek test | Verify mediapipelinetests initializes with DASH stream and seek operation parameters correctly configured |
| 3 | Construct H.264 DASH Pipeline and Initiate Playing State | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `video_src_url_dash` via `g_object_set()`;<br>Set `westerossink` as video sink (or `autoaudiosink` for audio-only) via `g_object_set()`;<br>Trigger state transition to `GST_STATE_PLAYING` via `gst_element_set_state()` | Verify playbin reaches `GST_STATE_PLAYING`, DASH dashdemux active and parsing stream, first frame rendered via first-video-frame-callback signal (or first audio sample for audio-only), no `GST_MESSAGE_ERROR` on bus |
