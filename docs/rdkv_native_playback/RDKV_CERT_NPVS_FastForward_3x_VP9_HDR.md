...
## TestCase ID
RDKV_NATIVE_PLAYBACK_377

## TestCase Name
RDKV_CERT_NPVS_FastForward_3x_VP9_HDR
## Table of Contents
...
| 2 | Stream Provisioning and Configuration |  DASH stream with MPD manifest must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` in MediaValidationVariables.py. Stream contains VP9 video with HDR and OPUS audio in WebM container (WebM format for fast-forward HDR testing))<br>Stream variable `video_src_url_vp9_hdr` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_HDR.webm"`  | Verify master.mpd manifest file is accessible and readable from configured path, dashdemux plugin available for MPD parsing Verify `video_src_url_vp9_hdr` resolves to valid WebM file path with VP9 HDR codec support |
