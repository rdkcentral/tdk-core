## TestCase ID
RDKV_NATIVE_PLAYBACK_359

## TestCase Name
RDKV_CERT_NPVS_Set_Rate_0.5x_HEVC_MKV
...
## Objective
Test to verify playback rate control at 0.5x speed function with HEVC H.265 in MKV container. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic polling with position increment expected at +/-25% tolerance around 0.05 seconds per 100ms interval. Verify MKV demuxer correctly parses container, audio/video remain synchronized during rate change, and frame rendering continues without discontinuities using frame statistics validation.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  HEVC H.265 encoded MKV stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_MKV.mkv"` in MediaValidationVariables.py (HEVC in Matroska container for rate control validation). Matroska demuxer (`matroskademux`))<br>must be available and functional Stream variable `video_src_url_hevc_mkv` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_HEVC_MKV.mkv` (stream must have HEVC H.265 codec in MKV container)  | Verify HEVC MKV stream file is accessible, readable, matroskademux plugin available, and stream contains minimum 60 seconds of content Verify `video_src_url_hevc_mkv` resolves to valid, accessible HEVC MKV file location with proper Matroska container format |
...
| 4 | Execute Playback Rate Change | Invoke `gst_element_seek()` with playback rate set to 0.5 and GST_SEEK_FLAG_FLUSH flag<br> Verify position progression matches 0.5x speed (half normal speed) with maintained audio/video sync | Verify test operation completes successfully with expected results |
| 5 | Monitor Playback Progress | Poll `gst_element_query_position()` at 100ms intervals to verify position advances at 0.5x rate (expected increment: 0.05 seconds per 100ms +/-25% tolerance) | Verify position increments are consistent, no stalls or backward jumps detected |
