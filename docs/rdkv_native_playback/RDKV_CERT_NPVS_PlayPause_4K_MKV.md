## TestCase ID
RDKV_NATIVE_PLAYBACK_28

## TestCase Name
RDKV_CERT_NPVS_PlayPause_4K_MKV
...
| 2 | Stream Provisioning and Configuration | MKV AV1 4K stream must be accessible via local file system or HTTP streaming<br> Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_AV1_MKV.mkv"` Stream variable `video_src_url_4k_av1_mkv` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_AV1_MKV.mkv"` | Verify stream file is accessible with valid Video: AV1, Audio: PCM, matroskademux element for MKV parsing Verify `video_src_url_4k_av1_mkv` resolves to valid, accessible stream location |
