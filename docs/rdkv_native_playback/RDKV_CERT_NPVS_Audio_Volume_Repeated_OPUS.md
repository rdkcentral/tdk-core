## TestCase ID
RDKV_NATIVE_PLAYBACK_139

## TestCase Name
RDKV_CERT_NPVS_Audio_Volume_Repeated_OPUS
...
| 2 | Stream Provisioning and Configuration | OPUS H.264 DASH stream must be accessible via HTTPS (`souphttpsrc`) or local file system (`filesrc`) or local file system<br> Stream URL is configured via MediaValidationVariables.py variable `video_src_url_opus` = `test_streams_base_path + "DASH_H264_OPUS/atfms_291_dash_tdk_avc_opus_fmp4.mpd"`<br> Resolves to: `http://<TM_IP>:8080/TDK_Clear_Test_Streams_Sunrise/DASH_H264_OPUS/atfms_291_dash_tdk_avc_opus_fmp4.mpd` Stream variable `video_src_url_opus` must be configured in `MediaValidationVariables.py`<br> Path: `DASH_H264_OPUS/atfms_291_dash_tdk_avc_opus_fmp4.mpd` containing OPUS audio track with H.264 video codec | Verify DASH manifest is accessible and parseable, contains H.264 video codec streams and OPUS audio codec streams, stream files can be downloaded via HTTP(S) Verify `video_src_url_opus` variable resolves to valid, accessible DASH stream location with OPUS audio codec and H.264 video codec available |
