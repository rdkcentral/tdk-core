## TestCase ID
RDKV_NATIVE_PLAYBACK_317

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_MKV
...
| 2 | Stream Provisioning and Configuration | AV1 stream must be accessible via HTTP/HTTPS or filesrc, configured at `test_streams_base_path + "TDK_Asset_Sunrise_AV1_MKV.mkv"` in MediaValidationVariables.py<br> Stream variable `video_src_url_4k_av1_mkv` must be configured with this path<br> (Matroska container (MKV), 1080p, 5+ mins) | Verify MKV AV1 stream is accessible and parseable, `video_src_url_4k_av1_mkv` resolves to valid, accessible stream with valid segments |
...
| 2 | Create Playbin and Configure MKV Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`<br> Set MKV file URL from `MediaValidationVariables.video_src_url_4k_av1_mkv` as URI property (local file:// or HTTP URL)<br> Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO` ; Verify playbin element created successfully, MKV stream URL configured, flags set for A/V playback |
