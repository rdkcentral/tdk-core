## TestCase ID
RDKV_NATIVE_PLAYBACK_258

## TestCase Name
RDKV_CERT_NPVS_Appsrc_Audio_Underflow_Playback_OPUS
...
| 2 | Stream Provisioning and Configuration | OPUS stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding<br> Stream file path configured as `test_streams_base_path + "DASH_VP9_OPUS_WebM/audio.webm"` in MediaValidationVariables.py Stream variable `audio_src_url_webm_opus` configured in `MediaValidationVariables.py` with path: `DASH_VP9_OPUS_WebM/audio.webm` (Appsrc requires local file access, not DASH/HLS streams) | Verify OPUS stream file is accessible and readable from filesystem Verify `audio_src_url_webm_opus` resolves to valid, accessible OPUS file location |
| 3 | Appsrc Buffer Threshold Configuration |  `BYTES_THRESHOLD` must be configured to 419338 bytes (actual threshold value from test script))<br>to trigger opus underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered  | Verify BYTES_THRESHOLD = 419338 bytes is set in Appsrc configuration or test parameters |
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State |  Set pipeline to `GST_STATE_PAUSED`<br>via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc<br>via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 419338 bytes` is reached. Transition to `GST_STATE_PLAYING`<br>via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started  | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 419338 bytes threshold, first-frame signal is detected |
