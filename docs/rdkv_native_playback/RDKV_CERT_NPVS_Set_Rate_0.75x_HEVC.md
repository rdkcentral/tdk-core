## TestCase ID
RDKV_NATIVE_PLAYBACK_87

## TestCase Name
RDKV_CERT_NPVS_Set_Rate_0.75x_HEVC
...
## Objective
Test to verify playback rate control at 0.75x speed function with HEVC H.265 codec. Validate that position advances at exactly 0.75x speed (three-quarter normal speed) through periodic polling with position increment expected at +/-25% tolerance around 0.075 seconds per 100ms interval. Verify audio/video remain synchronized during HEVC rate change and frame rendering continues without discontinuities using frame statistics validation.
## Preconditions
