## TestCase ID
RDKV_NATIVE_PLAYBACK_01

## TestCase Name
RDKV_CERT_NPVS_Play_DASH
...
## Objective
Validate H.264 video codec with AAC audio using DASH streaming. Initialize playbin with dashdemux element, configure westerossink as video sink and alsasink for audio output. Query video dimensions and audio stream count via 'current-video' and 'current-audio' properties. Execute 10-second playback monitoring position advancement and frame rendering via westerossink stats.
## Preconditions
