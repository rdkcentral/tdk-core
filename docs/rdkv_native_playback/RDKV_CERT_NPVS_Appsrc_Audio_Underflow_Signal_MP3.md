...
## TestCase Name
RDKV_CERT_NPVS_Appsrc_Audio_Underflow_Signal_MP3
...
| 2 | Stream Provisioning and Configuration | MP3 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding<br> Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_MP3.mp3"` in MediaValidationVariables.py Stream variable `audio_src_url_mp3` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_MP3.mp3` (Appsrc requires local file access, not DASH/HLS streams) | Verify MP3 stream file is accessible and readable from filesystem Verify `audio_src_url_mp3` resolves to valid, accessible MP3 file location |
| 3 | Appsrc Buffer Threshold Configuration |  `BYTES_THRESHOLD` must be configured to 811071 bytes (actual threshold value from test script))<br>to trigger mp3 underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered  | Verify BYTES_THRESHOLD = 811071 bytes is set in Appsrc configuration or test parameters |
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State |  Set pipeline to `GST_STATE_PAUSED`<br>via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc<br>via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 811071 bytes` is reached. Transition to `GST_STATE_PLAYING`<br>via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started  | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 811071 bytes threshold, first-frame signal is detected |
