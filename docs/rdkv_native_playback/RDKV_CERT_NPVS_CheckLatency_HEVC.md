## TestCase ID
RDKV_NATIVE_PLAYBACK_205

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_HEVC
...
## Objective
Measure playback latency from pipeline PLAYING state to first frame rendering on westerossink video sink using HEVC/H.265 encoded content via DASH format. Execute DASH HEVC stream through playbin and measure elapsed time when playback position reaches 1 second, validating that latency remains below configured threshold (default 100 milliseconds). Verify westerossink successfully renders HEVC frames within latency budget and logs measurement for HEVC codec compliance validation.
...
| 2 | Create Playbin and Configure DASH HEVC Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`<br> Set DASH HEVC stream URL from `MediaValidationVariables.video_src_url_hevc` as URI property<br> Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO` ; Verify playbin element created successfully, DASH HEVC stream URL configured, flags set for A/V playback |
