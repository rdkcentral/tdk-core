## TestCase ID
RDKV_NATIVE_PLAYBACK_313

## TestCase Name
RDKV_CERT_NPVS_EOS_EAC3_Only_Audio
...
| 2 | Stream Provisioning and Configuration |  EAC3 audio-only stream must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_EAC3_Audio_only_15sec.mp4"` in MediaValidationVariables.py. Stream contains 15-second EAC3 audio media (EAC3 audio codec, MP4 container format, audio-only content))<br>Stream variable `audio_src_url_short_duration_eac3` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_EAC3_Audio_only_15sec.mp4"`  | Verify EAC3 audio file is accessible and readable from configured path, appropriate audio decoder and demuxer available Verify `audio_src_url_short_duration_eac3` resolves to valid audio stream URL, MP4 container structure is valid |
...
| 4 | Platform-Specific Environment Variables |  Platform-specific environment variables (`WAYLAND_DISPLAY`, `XDG_RUNTIME_DIR`, `LD_PRELOAD` with vendor EAC3 decoder libraries, `GST_PLUGIN_PATH`))<br>must be defined in `/opt/TDK/TDK.env` for GStreamer plugin and codec access  | Verify `/opt/TDK/TDK.env` exists with all required variables, EAC3 hardware decoder plugins loaded via GST_PLUGIN_PATH |
