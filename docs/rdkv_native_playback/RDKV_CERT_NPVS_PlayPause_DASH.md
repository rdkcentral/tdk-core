...
## Objective
Validate play-pause state management and position control during DASH adaptive streaming. Execute repeated transitions between PLAYING and PAUSED states while DASH dynamically adapts bitrate and switches segments. Verify playback position halts completely during pause with continuous monitoring; verify position resumes at normal rate ( second tolerance) when resumed to PLAYING state. Monitor frame rendering statistics showing increments only during PLAYING and halts during PAUSED, confirming proper state machine behavior across adaptive streaming segment boundaries.
## Preconditions
...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_dash` variable;<br>Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` to prepare play-pause cycling test | Verify mediapipelinetests initializes with DASH stream and cycle count configured |
