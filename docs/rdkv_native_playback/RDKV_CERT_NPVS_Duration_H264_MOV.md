## TestCase ID
RDKV_NATIVE_PLAYBACK_366

## TestCase Name
RDKV_CERT_NPVS_Duration_H264_MOV
...
## Objective
Validate stream duration retrieval and accuracy for H.264 video in MOV container format duration query API (). Query total stream duration in nanosecond precision and verify the returned value matches expected baseline duration within +/-250ms tolerance. Confirm MOV demultiplexer correctly extracts duration metadata from ftyp/moov atoms and communicates accurate timing information to the playbin. Monitor playback position advancement to verify position increments correlate with query duration. Validate frame rendering statistics show consistent frame delivery without stalls or drops during full duration playback. Verify test successfully parses duration from mediapipelinetests output log and confirms retrieved value matches expected 9-second baseline within tolerance threshold.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  MOV container with H.264 video stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_H264_MOV.MOV"` in MediaValidationVariables.py. Stream contains H.264 video in MOV container format (QuickTime container for duration testing))<br>Stream variable `video_src_url_h264_mov` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_H264_MOV.MOV"`  | Verify MOV file is accessible and readable from configured path, moovrelocator and qtdemux parser available for duration extraction Verify `video_src_url_h264_mov` resolves to valid MOV container file with duration metadata |
...
| 4 | Transition Pipeline to PLAYING State | Set pipeline state to `GST_STATE_PLAYING` via `gst_element_set_state(playbin, GST_STATE_PLAYING)`<br> Poll `gst_element_get_state()` until state change completes (`GST_STATE_CHANGE_SUCCESS`)<br> Confirm `firstFrameReceived == true` callback was triggered indicating first H.264 frame rendered on sink<br> `qtdemux` element automatically demultiplexes MOV file and initializes audio/video streams | Verify pipeline reaches `GST_STATE_PLAYING` without `GST_MESSAGE_ERROR`, state transition completes synchronously or asynchronously, first-frame signal received |
| 5 | Query Stream Duration Using GStreamer Query API | Invoke `gst_element_query_duration(playbin, GST_FORMAT_TIME, &duration)` to retrieve total MOV stream duration in nanoseconds<br> Validate query returns TRUE (success)<br> Verify retrieved duration != -1 (indicates valid, parseable MOV file)<br> For MOV streams, `qtdemux` extracts duration from QuickTime movie header atom (mvhd)<br> Convert duration from nanoseconds to seconds via `GST_TIME_AS_SECONDS(duration)` and milliseconds via `GST_TIME_AS_MSECONDS(duration) % 1000` for logging and comparison | Verify duration query succeeds, returned value is positive integer (not -1), duration formatted correctly in M:SS.MS format |
