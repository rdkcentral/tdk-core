## TestCase ID
RDKV_NATIVE_PLAYBACK_203

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_H264
...
## Objective
Measure playback latency from pipeline PLAYING state to first frame rendering on westerossink video sink using H.264/AVC encoded content via DASH format. Execute DASH H.264 stream through playbin and measure elapsed time when playback position reaches 1 second, validating that latency remains below configured threshold (default 100 milliseconds). Verify westerossink successfully renders H.264 frames within latency budget and logs measurement for H.264 codec compliance validation.
...
| 2 | Stream Provisioning and Configuration | H.264/AAC stream must be accessible via HTTP/HTTPS or filesrc, configured at `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` in MediaValidationVariables.py<br> Stream variable `video_src_url_dash_h264` must be configured with this path<br> (DASH MPD with fMP4 segments, 1080p, 5+ mins) | Verify DASH H.264/AAC stream is accessible and parseable, `video_src_url_dash_h264` resolves to valid, accessible stream with valid segments |
...
| 2 | Create Playbin and Configure DASH H.264 Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`<br> Set DASH H.264 stream URL from `MediaValidationVariables.video_src_url_dash_h264` as URI property<br> Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO` ; Verify playbin element created successfully, DASH H.264 stream URL configured, flags set for A/V playback |
