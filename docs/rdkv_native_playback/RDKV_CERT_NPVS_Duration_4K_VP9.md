## TestCase ID
RDKV_NATIVE_PLAYBACK_292

## TestCase Name
RDKV_CERT_NPVS_Duration_4K_VP9
...
## Objective
Validate stream duration retrieval and accuracy for 4K VP9 video content duration query API (). Query total stream duration in nanosecond precision and verify the returned value matches expected baseline duration within +/-250ms tolerance. Confirm VP9 WebM container parsing correctly extracts duration metadata and communicates accurate timing information to the playbin. Monitor playback position advancement to verify position increments correlate with query duration. Validate frame rendering statistics show consistent frame delivery without stalls or drops during full duration playback. Verify test successfully parses duration from mediapipelinetests output log and confirms retrieved value matches expected 9-second baseline within tolerance threshold.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  WebM container with VP9 4K video must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_4K.webm"` in MediaValidationVariables.py. Stream contains VP9 video in 4K resolution with WebM container format (for 4K VP9 duration testing))<br>Stream variable `video_src_url_vp9_4k` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_4K.webm"`  | Verify WebM file is accessible and readable from configured path, matroskademux parser available for duration extraction from segment information Verify `video_src_url_vp9_4k` resolves to valid WebM container file with 4K resolution and duration metadata |
