## TestCase ID
RDKV_NATIVE_PLAYBACK_166

## TestCase Name
RDKV_CERT_NPVS_PauseSeek_Forward_HEVC
...
| 2 | Configure and Execute Test Application | Retrieve configuration and stream URL, then execute `tdk_mediapipelinetests` with test case name, stream URL, and timeout arguments<br> Load HEVC stream configuration from MediaValidationVariables.py | Verify configuration is retrieved and `tdk_mediapipelinetests` initializes playbin pipeline for HEVC |
| 3 | Construct Pipeline and Initiate Playback | Create `playbin` element with HEVC stream URI, set `westerossink` as video sink, trigger `NULLREADYPAUSEDPLAYING` state transition, verify `first-video-frame-callback` signal | Verify `playbin` reaches `GST_STATE_PLAYING` with first frame rendered from HEVC stream, no `GST_MESSAGE_ERROR` |
