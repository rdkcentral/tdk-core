## TestCase ID
RDKV_NATIVE_PLAYBACK_79

## TestCase Name
RDKV_CERT_NPVS_Set_Rate_0.5x_H264
...
## Objective
Test to verify playback rate control at 0.5x speed function with rate parameter set to 0.5 and flag. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic polling with position increment expected at +/-25% tolerance around 0.05 seconds per 100ms interval. Verify audio/video remain synchronized during rate change and frame rendering continues without discontinuities using frame statistics validation.
## Preconditions
