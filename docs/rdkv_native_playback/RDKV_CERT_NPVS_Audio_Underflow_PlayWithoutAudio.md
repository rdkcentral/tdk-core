## TestCase ID
RDKV_NATIVE_PLAYBACK_143

## TestCase Name
RDKV_CERT_NPVS_Audio_Underflow_PlayWithoutAudio
...
| 2 | Stream Provisioning and Configuration |  Media stream must be accessible via HTTPS (`souphttpsrc`))<br>or local file system (`filesrc`))<br>Stream URL must be configured in `MediaValidationVariables.py` with valid HTTPS URL or local file path  | Verify stream is accessible and appropriate demuxer can parse the stream Verify stream URL resolves to valid, accessible stream location |
