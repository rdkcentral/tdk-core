## TestCase ID
RDKV_NATIVE_PLAYBACK_202

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_DASH
...
| 2 | Create Playbin and Configure DASH Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`<br> Set DASH stream URL from `MediaValidationVariables.video_src_url_dash` as URI property<br> Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO` ; Verify playbin element created successfully, DASH stream URL configured, flags set for A/V playback |
