...
## TestCase Name
RDKV_CERT_NPVS_PlaySeek_Backward_Repeated_H264
...
## Objective
Test to verify backward seek operations on H.264 DASH stream repeated in repeated cycles. The test executes 3+ seek operations at different positions (incremented in fixed steps) to validate seek accuracy, position recovery, and playback continuity under repeated conditions. Verify playback position matches seek target within +/-1 second tolerance after each seek operation, validate frame rendering resumes correctly via westerossink stats (rendered_frames increment), and detect no GStreamer errors during all seek cycles.
## Preconditions
