## TestCase ID
RDKV_NATIVE_PLAYBACK_311

## TestCase Name
RDKV_CERT_NPVS_Set_Rate_0.5x_VP9
...
## Objective
Test to verify playback rate control at 0.5x speed function with VP9 codec. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic polling with position increment expected at +/-25% tolerance around 0.05 seconds per 100ms interval. Verify audio/video remain synchronized during VP9 rate change and frame rendering continues without discontinuities using frame statistics validation.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  VP9 encoded WebM stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_30fps.webm"` in MediaValidationVariables.py (30fps VP9 baseline stream in WebM container for rate control validation))<br>Stream variable `video_src_url_vp9_30fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_VP9_30fps.webm` (stream must have VP9 codec in WebM container at 30fps framerate)  | Verify VP9 WebM stream file is accessible, readable, and contains minimum 60 seconds of content for rate testing Verify `video_src_url_vp9_30fps` resolves to valid, accessible VP9 WebM file location with 30fps encoding |
