## TestCase ID
RDKV_NATIVE_PLAYBACK_29

## TestCase Name
RDKV_CERT_NPVS_PlayPause_4K_VP9
...
## Objective
Validate play-pause-resume state transitions during WebM VP9 OPUS 4K playback with westerossink video sink and audio sink. Test creates playbin element with configured stream, transitions pipeline between PLAYING and PAUSED states, and validates position remains stationary during pause with 0ms drift tolerance while verifying 4K resolution maintained.rifying position advances 1x rate during play +/-100ms tolerance. Confirm frame rendering statistics from westerossink property show rendered_frames increment ONLY during PLAYING state and remain static during PAUSED state, no on bus, and "Failures: 0" output for all state transitions.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation |  TDK_Package must be installed on the Device Under Test (DUT))<br>with `tdk_mediapipelinetests` binary, GStreamer plugins (matroskademux demuxer, necessary decoders), and westerossink element  | Verify TDK_Package is installed, binary is executable, matroskademux element available in GStreamer |
| 2 | Stream Provisioning and Configuration | WebM VP9 OPUS 4K stream must be accessible via local file system or HTTP streaming<br> Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "DASH_VP9_OPUS_WebM/4k.webm"` Stream variable `video_src_url_webm_4k_vp9` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "DASH_VP9_OPUS_WebM/4k.webm"` | Verify stream file is accessible with valid Video: VP9, Audio: OPUS, matroskademux element for WebM parsing Verify `video_src_url_webm_4k_vp9` resolves to valid, accessible stream location |
