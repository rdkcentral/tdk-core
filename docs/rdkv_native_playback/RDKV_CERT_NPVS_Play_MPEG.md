## TestCase ID
RDKV_NATIVE_PLAYBACK_349

## TestCase Name
RDKV_CERT_NPVS_Play_MPEG
...
| 2 | Stream Provisioning and Configuration |  MPEG MPEG2 MP3 stream (`TDK_Asset_MPEG.mpeg`))<br>must be accessible via HTTPS (`souphttpsrc`))<br>or local file system (`filesrc`))<br>element with `mpegpsdemux` capable of parsing MPEG container `video_src_url_mpeg` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_MPEG.mpeg"`  | Verify MPEG stream is accessible and contains valid MPEG2 and MP3 representations Verify `video_src_url_mpeg` resolves to valid MPEG location with correct codec support |
...
| 2 | Configure and Execute Test Application | Retrieve configuration values for `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` from device config; Retrieve stream URL from `video_src_url_mpeg` variable;<br>Execute `mediapipelinetests test_generic_playback <MPEG_URL> timeout=<seconds>` | Verify mediapipelinetests initializes with MPEG2 MPEG stream and timeout parameters |
| 3 | Construct Pipeline and Initiate Playback | Create `playbin` element via `gst_element_factory_make()` with video_src_url_mpeg URI;<br>Configure `uri` property to video_src_url_mpeg manifest/file via `g_object_set()`; Set `video-sink` property to `westerossink` via `g_object_set()`;<br>Configure `autoaudiosink` for MP3 audio; Trigger state transition from `GST_STATE_NULL` to `GST_STATE_PLAYING` via `gst_element_set_state()`;<br>Monitor for `first-video-frame-callback` signal | Verify playbin reaches `GST_STATE_PLAYING` with first frame decoded, MPEG manifest/file parsed successfully, no `GST_MESSAGE_ERROR` |
| 4 | Query Video and Audio Stream Properties | Query video dimensions via `g_object_get(westerossink, "video-height", &height, NULL)` and `g_object_get(westerossink, "video-width")`;<br>Query audio stream count via `g_object_get(playbin, "n-audio", &n_audio, NULL)` to verify MP3 stream presence; Log extracted properties | Verify video dimensions valid; Verify n-audio >= 1 confirming MP3 stream present |
