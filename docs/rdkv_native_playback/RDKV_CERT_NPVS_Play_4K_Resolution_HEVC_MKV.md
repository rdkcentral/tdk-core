## TestCase ID
RDKV_NATIVE_PLAYBACK_352

## TestCase Name
RDKV_CERT_NPVS_Play_4K_Resolution_HEVC_MKV
...
## Objective
Validate 4K (3840�2160) resolution playback function with H.265 video codec and AAC audio. Initialize playbin with matroskademux demuxing, query video sink properties and verify resolution matches expected 3840�2160 (+/-5 pixel tolerance). Execute 10-second playback monitoring position, validate frame rendering statistics. Confirm clean pipeline state transitions and error-free playback.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  MKV H.265 AAC stream (`TDK_Asset_Sunrise_HEVC_MKV.mkv`))<br>must be accessible via HTTPS (`souphttpsrc`))<br>or local file system (`filesrc`))<br>element with `matroskademux` capable of parsing MKV container `video_src_url_hevc_mkv` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_MKV.mkv"`  | Verify MKV stream is accessible and contains valid H.265 and AAC representations Verify `video_src_url_hevc_mkv` resolves to valid MKV location with correct codec support |
...
| 2 | Configure Test with Resolution Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_hevc_mkv`;<br>Execute `mediapipelinetests test_generic_playback <URL> timeout=<seconds>` | Verify mediapipelinetests initializes with 4K stream |
| 3 | Construct H.265 Pipeline with Resolution Stream | Create `playbin` element; Configure `uri` to video_src_url_hevc_mkv (MKV); Set `westerossink` as video sink;<br>Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, MKV parsed successfully |
