## TestCase ID
RDKV_NATIVE_PLAYBACK_20

## TestCase Name
RDKV_CERT_NPVS_PlayPause_23Fps
...
| 2 | Stream Provisioning and Configuration | MP4 H.264/AAC stream must be accessible via local file system or HTTP streaming<br> Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_23fps.mp4"` Stream variable `video_src_url_mp4_23fps` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_23fps.mp4"` | Verify stream file is accessible with valid Video: H.264, Audio: AAC, qtdemux element for MP4 parsing Verify `video_src_url_mp4_23fps` resolves to valid, accessible stream location |
