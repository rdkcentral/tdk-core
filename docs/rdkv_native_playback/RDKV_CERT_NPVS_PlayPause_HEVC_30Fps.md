...
## TestCase Name
RDKV_CERT_NPVS_PlayPause_HEVC_30Fps
...
| 2 | Stream Provisioning and Configuration |  MP4 container with HEVC video at 30fps must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_30fps_v2.mp4"` in MediaValidationVariables.py. Stream contains HEVC (H.265))<br>video at 30fps and AAC audio (MP4 container format for HEVC 30fps play-pause testing))<br>Stream variable `video_src_url_hevc_30fps` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_30fps_v2.mp4"`  | Verify MP4 file is accessible and readable from configured path, qtdemux and HEVC decoder plugins available for playback at 30fps Verify `video_src_url_hevc_30fps` resolves to valid MP4 container file with HEVC video at 30fps and AAC audio data |
...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_hevc_30fps` variable;<br>Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` | Verify mediapipelinetests initializes with MP4 stream and cycle count configured |
| 3 | Construct H.265 MP4 Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` to `video_src_url_hevc_30fps` via `g_object_set()`;<br>Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio; Transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, qtdemux active, first frame rendered |
