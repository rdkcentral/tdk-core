...
## TestCase Name
RDKV_CERT_NPVS_Seek_Towards_EOS_with_Pause_Webm
...
| 2 | Stream Provisioning and Configuration |  WebM container stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"` in MediaValidationVariables.py. Stream contains VP9 video and Opus audio (WebM container format for EOS seeking with pause testing))<br>Stream variable `video_src_url_direct_webm` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"`  | Verify WebM file is accessible and readable from configured path, matroskademux and VP9 decoder plugins available for playback Verify `video_src_url_direct_webm` resolves to valid WebM container file with VP9 video and Opus audio data |
