...
## TestCase Name
RDKV_CERT_NPVS_Seek_Towards_EOS_with_Pause_HLS
...
| 2 | Stream Provisioning and Configuration | HLS stream with H.264 video must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`)<br> Stream file configured as `test_streams_base_path + "HLS_H264_AAC/master.m3u8"` in MediaValidationVariables.py<br> Stream contains H.264 video and AAC audio in HLS format Stream variable `video_src_url_hls` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "HLS_H264_AAC/master.m3u8"` | Verify HLS M3U8 playlist is accessible and hlsdemux can parse playlist Verify stream URL resolves to valid, accessible HLS playlist location |
