## TestCase ID
RDKV_NATIVE_PLAYBACK_218

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_FastForward_2x_EAC3
...
## Objective
Measure playback latency when transitioning to 2.0x fast-forward playback rate on H.264/EAC3 DASH stream. Record latency from setting playback rate to 2.0x to first frame rendering at new rate, validating that latency remains below configured threshold (default 100 milliseconds). Verify audio/video remain synchronized during rate-changed playback and westerossink renders frames within latency budget.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration | H.264/EAC3 stream must be accessible via HTTP/HTTPS or filesrc, configured at `test_streams_base_path + "DASH_H264_EC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd"` in MediaValidationVariables.py<br> Stream variable `video_src_url_ec3` must be configured with this path<br> (DASH MPD with fMP4 segments, 1080p, 5+ mins) | Verify DASH H.264/EAC3 stream is accessible and parseable, `video_src_url_ec3` resolves to valid, accessible stream with valid segments |
