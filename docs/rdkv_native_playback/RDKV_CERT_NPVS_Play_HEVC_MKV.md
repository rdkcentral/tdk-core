## TestCase ID
RDKV_NATIVE_PLAYBACK_350

## TestCase Name
RDKV_CERT_NPVS_Play_HEVC_MKV
...
## Objective
Validate H.265 video codec with AAC audio using MKV streaming. Initialize playbin with matroskademux element, configure westerossink as video sink and alsasink for audio output. Query video dimensions and audio stream count via 'current-video' and 'current-audio' properties. Execute 10-second playback monitoring position advancement and frame rendering via westerossink stats.
## Preconditions
