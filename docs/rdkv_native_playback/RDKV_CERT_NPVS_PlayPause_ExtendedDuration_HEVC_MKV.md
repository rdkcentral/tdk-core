## TestCase ID
RDKV_NATIVE_PLAYBACK_357

## TestCase Name
RDKV_CERT_NPVS_PlayPause_ExtendedDuration_HEVC_MKV
...
## Objective
Validate Matroska (MKV) container playback with HEVC video codec support through extended duration content with sustained play-pause cycling. Execute multiple play-pause state cycles to verify pipeline maintains stability during repeated transitions between PLAYING and PAUSED states over extended playback duration without stalls, buffer underruns, or resource exhaustion. Monitor frame rendering statistics and verify position synchronization.r playback position advancement to verify position increments smoothly at 1x rate during PLAYING state and remains stationary during PAUSED state. Query property to validate frame rendering statistics (rendered_frames increments consistently, dropped_frames < 1% throughout extended session). Verify MKV matroskademux plugin maintains stable performance during extended playback, correctly processes HEVC video and audio streams without timeout errors, memory leaks, or synchronization loss across multiple pause-resume cycles.
## Preconditions
