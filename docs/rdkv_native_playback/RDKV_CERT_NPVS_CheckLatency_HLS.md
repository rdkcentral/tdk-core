## TestCase ID
RDKV_NATIVE_PLAYBACK_204

## TestCase Name
RDKV_CERT_NPVS_CheckLatency_HLS
...
## Objective
Measure playback latency from pipeline PLAYING state to first frame rendering on westerossink video sink using HLS (HTTP Live Streaming) format H.264/AAC content. Execute HLS stream through playbin with hls demultiplexer and measure elapsed time when playback position reaches 1 second, validating that latency remains below configured threshold (default 100 milliseconds). Verify westerossink successfully renders HLS-sourced frames within latency budget and logs measurement for streaming protocol compliance.
...
| 2 | Stream Provisioning and Configuration | H.264/AAC stream must be accessible via HTTP/HTTPS or filesrc, configured at `test_streams_base_path + "HLS_H264_AAC/master.m3u8"` in MediaValidationVariables.py<br> Stream variable `video_src_url_hls` must be configured with this path<br> (HLS M3U8 with MPEG-2 TS segments, 1080p, 5+ mins) | Verify HLS H.264/AAC stream is accessible and parseable, `video_src_url_hls` resolves to valid, accessible stream with valid segments |
| 3 | Latency Threshold Configuration |  Latency threshold must be retrieved from device configuration file: `NATIVE_PLAYBACK_LATENCY_THRESHOLD` (default: 100 milliseconds))<br>for pass/fail validation. Secondary timeout: `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` (default: 10 seconds))<br>for overall playback duration  | Verify latency threshold is set (default 100ms accepted if config unavailable) and playback timeout is accessible |
...
| 2 | Create Playbin and Configure HLS Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`<br> Set HLS stream URL from `MediaValidationVariables.video_src_url_hls` as URI property<br> Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO` ; Verify playbin element created successfully, HLS stream URL configured, flags set for A/V playback |
