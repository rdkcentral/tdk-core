## TestCase ID
RDKV_NATIVE_PLAYBACK_63

## TestCase Name
RDKV_CERT_NPVS_PlaySeek_Forward_HLS
...
## Objective
Test to verify forward seek operation on H.264 HLS stream to seek to a specified position during active playback. The test validates accurate seek positioning, proper pipeline state handling during seek, and seamless playback recovery after seek completion. Verify playback position reaches the target seek position within +/-1 second tolerance, validate frame rendering via westerossink stats showing rendered_frames increment after seek, and confirm no errors on GStreamer bus.
## Preconditions
