...
## TestCase Name
RDKV_CERT_NPVS_PlayPause_48kHz_AAC
...
| 2 | Stream Provisioning and Configuration | MP4 AAC 48kHz stream must be accessible via local file system or HTTP streaming<br> Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_30fps.mp4"` Stream variable `video_src_url_48khz_aac` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_30fps.mp4"` | Verify stream file is accessible with valid Audio: AAC/OPUS, qtdemux element for MP4 parsing Verify `video_src_url_48khz_aac` resolves to valid, accessible stream location |
