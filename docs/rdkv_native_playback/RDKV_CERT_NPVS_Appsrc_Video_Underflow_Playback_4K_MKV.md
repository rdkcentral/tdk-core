## TestCase ID
RDKV_NATIVE_PLAYBACK_268

## TestCase Name
RDKV_CERT_NPVS_Appsrc_Video_Underflow_Playback_4K_MKV
...
| 2 | Stream Provisioning and Configuration | 4K MKV stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding<br> Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_AV1_MKV.mkv"` in MediaValidationVariables.py Stream variable `video_src_url_4k_av1_mkv` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_AV1_MKV.mkv` (Appsrc requires local file access, not DASH/HLS streams) | Verify 4K MKV stream file is accessible and readable from filesystem Verify `video_src_url_4k_av1_mkv` resolves to valid, accessible 4K MKV file location |
| 3 | Appsrc Buffer Threshold Configuration |  `BYTES_THRESHOLD` must be configured to 3218827 bytes (actual threshold value from test script))<br>to trigger 4k mkv underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered  | Verify BYTES_THRESHOLD = 3218827 bytes is set in Appsrc configuration or test parameters |
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State |  Set pipeline to `GST_STATE_PAUSED`<br>via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc<br>via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 3218827 bytes` is reached. Transition to `GST_STATE_PLAYING`<br>via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started  | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 3218827 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 6437654 bytes`<br> After all media data is transferred to Appsrc, emit `end-of-stream` action via `g_signal_emit_by_name()` | Verify buffers accumulated to 6437654 bytes threshold and `end-of-stream` action is emitted |
