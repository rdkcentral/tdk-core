...
## TestCase Name
RDKV_CERT_NPVS_Seek_Towards_EOS_with_Pause_MP4
...
## Objective
Test validates seeking capability towards end-of-stream (EOS) on MP4 H.264 video with playback pause intervals by invoking to reposition playback near the stream's final position. The test verifies that the seek target position is reached within +/-1 second tolerance using position queries every 100ms, with periodic pause/resume cycles to validate state machine robustness. Validates frame rendering continues with correct PTS monotonicity after pause transitions and no errors are triggered during near-EOS seeking with pause recovery operations.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  MP4 container stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_MP4.mp4"` in MediaValidationVariables.py. Stream contains H.264 video and AAC audio (MP4 container format for EOS seeking with pause testing))<br>Stream variable `video_src_url_mp4` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_MP4.mp4"`  | Verify MP4 file is accessible and readable from configured path, qtdemux and H.264 decoder plugins available for playback Verify `video_src_url_mp4` resolves to valid MP4 container file with H.264 video and AAC audio data |
...
| 4 | Execute Seek Operation | Invoke `gst_element_seek()` with `GST_SEEK_FLAG_FLUSH` to reposition playback<br> Verify position matches the seek target within +/-250ms tolerance | Verify test operation completes successfully with expected results |
