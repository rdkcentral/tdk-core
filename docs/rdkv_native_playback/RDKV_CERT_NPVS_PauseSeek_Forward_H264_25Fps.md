## TestCase ID
RDKV_NATIVE_PLAYBACK_163

## TestCase Name
RDKV_CERT_NPVS_PauseSeek_Forward_H264_25Fps
...
| 2 | Configure and Execute Test Application | Retrieve configuration and stream URL, then execute `tdk_mediapipelinetests` with test case name, stream URL, and timeout arguments<br> Load H264 25fps format configuration from MediaValidationVariables.py | Verify configuration is retrieved and `tdk_mediapipelinetests` initializes playbin pipeline for 25fps format |
| 3 | Construct Pipeline and Initiate Playback | Create `playbin` element with H264 stream URI, set `westerossink` as video sink, trigger `NULLREADYPAUSEDPLAYING` state transition, verify<br>`first-video-frame-callback` signal at 25fps framerate | Verify `playbin` reaches `GST_STATE_PLAYING` with first frame rendered at 25fps, no `GST_MESSAGE_ERROR` |
