## TestCase ID
RDKV_NATIVE_PLAYBACK_109

## TestCase Name
RDKV_CERT_NPVS_Play_480p_Resolution_H264
...
| 2 | Stream Provisioning and Configuration |  MP4 H.264 AAC stream (`atfms_291_dash_tdk_avc_aac_fmp4_480p.mp4`))<br>must be accessible via HTTPS (`souphttpsrc`))<br>or local file system (`filesrc`))<br>element with `qtdemux` capable of parsing MP4 container `video_src_url_mp4_480p` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4_480p.mp4"`  | Verify MP4 stream is accessible and contains valid H.264 and AAC representations Verify `video_src_url_mp4_480p` resolves to valid MP4 location with correct codec support |
...
| 2 | Configure Test with Resolution Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_mp4_480p`;<br>Execute `mediapipelinetests test_generic_playback <URL> timeout=<seconds>` | Verify mediapipelinetests initializes with 480p stream |
| 3 | Construct H.264 Pipeline with Resolution Stream | Create `playbin` element; Configure `uri` to video_src_url_mp4_480p (MP4); Set `westerossink` as video sink;<br>Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, MP4 parsed successfully |
