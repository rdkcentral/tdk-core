## TestCase ID
RDKV_NATIVE_PLAYBACK_249

## TestCase Name
RDKV_CERT_NPVS_Set_Rate_0.5x_H264_24Fps
...
## Objective
Test to verify playback rate control at 0.5x speed function with H.264 24fps stream baseline. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic polling with position increment expected at +/-25% tolerance around 0.05 seconds per 100ms. Verify audio/video remain synchronized during rate change and frame rendering continues without discontinuities with 24fps framerate baseline.
## Preconditions

| # | Step Name | Step Description | Expected Result |
|----|-----------|------------------|-----------------|  
| 1 | TDK Package Installation | TDK_Package must be installed on the Device Under Test (DUT) with `tdk_mediapipelinetests` binary and all dependent libraries | Verify TDK_Package is installed, binary is executable, and all libraries are available |
| 2 | Stream Provisioning and Configuration |  H.264 encoded MP4 stream must be accessible via local file system (`filesrc`))<br>or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_24fps.mp4"` in MediaValidationVariables.py (24fps H.264 baseline stream for rate control validation))<br>Stream variable `video_src_url_mp4_24fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_24fps.mp4` (stream must have H.264 codec at 24fps framerate)  | Verify H.264 MP4 stream file is accessible, readable, and contains minimum 60 seconds of content for rate testing Verify `video_src_url_mp4_24fps` resolves to valid, accessible H.264 MP4 file location with 24fps encoding |
