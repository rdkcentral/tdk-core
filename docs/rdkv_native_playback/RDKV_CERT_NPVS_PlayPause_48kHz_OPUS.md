...
## TestCase Name
RDKV_CERT_NPVS_PlayPause_48kHz_OPUS
...
| 2 | Stream Provisioning and Configuration | MP4 OPUS 48kHz stream must be accessible via local file system or HTTP streaming<br> Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_Opus_v2.mp4"` Stream variable `video_src_url_opus_mp4` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_Opus_v2.mp4"` | Verify stream file is accessible with valid Audio: AAC/OPUS, qtdemux element for MP4 parsing Verify `video_src_url_opus_mp4` resolves to valid, accessible stream location |
