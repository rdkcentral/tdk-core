# FNCS_Playback_Appsrc_Video_Underflow_Signal_VP9 Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_245

## TestCase Name
FNCS_Playback_Appsrc_Video_Underflow_Signal_VP9
...
| 2 | VP9 video stream (720p MP4) must be available and referenced in MediaValidationVariables.video_src_url_mp4_720p_vp9 |
...
| 11 | video_src_url_mp4_720p_vp9 variable in MediaValidationVariables.py contains VP9 video stream URL (720p MP4) for appsrc underflow signal testing |
...
| 5 | Setup Appsrc Element | Configure appsrc with size property set to underflow threshold (2625307 bytes), set appropriate VP9 video caps, and connect push-buffer signal for manual feeding | Appsrc element configured successfully with threshold size and VP9 video capabilities |
...
| 7 | Reach Buffer Threshold | Continue feeding video buffers via appsrc until threshold bytes (2625307) are reached | Threshold limit reached successfully, pipeline consuming all provided video buffer data |
...
| 11 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating VP9 appsrc video underflow signal detection completed without errors |
