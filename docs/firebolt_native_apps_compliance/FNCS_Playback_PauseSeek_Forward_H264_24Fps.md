...
## TestCase Name
**FNCS_Playback_PauseSeek_Forward_H264_24Fps**
...
## Objective
To validate that DASH stream playback with H264 video codec at 24 frames per second supports forward seek operations after pause, ensuring proper playback state transitions from paused state to seek position and resumption of playback at the specified frame rate.
...
| 6 | Resume Playback After Seek | Transition playbin from paused state to PLAYING state after seek completion. Monitor playback for the configured timeout duration (default 10 seconds). Verify video frames render smoothly from the seek position at 24 Fps | Playback must resume successfully from the seek position. Video frames must render continuously at 24 Fps without interruption |
