# FNCS_Playback_Audio_Change_OPUS_AAC Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_123

## TestCase Name
FNCS_Playback_Audio_Change_OPUS_AAC
...
| 2 | Multi-codec stream with OPUS and AAC audio tracks must be available and referenced in MediaValidationVariables.video_src_url_opus_aac |
...
| 13 | video_src_url_opus_aac variable in MediaValidationVariables.py contains multi-codec stream URL (TDK_Asset_Sunrise_AAC_OPUS.mp4) with both OPUS and AAC audio tracks |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT config values, get multi-codec stream URL from MediaValidationVariables.video_src_url_opus_aac, construct tdk_mediapipelinetests command with timeout parameter, and run test_audio_change with OPUS/AAC stream | Configuration values retrieved, multi-codec stream URL obtained, command executed: `tdk_mediapipelinetests test_audio_change <OPUS_AAC_STREAM_URL> checkavstatus=no timeout=10`, and test application starts execution without errors |
...
| 7 | Initiate Audio Codec Switch | Use playbin current-audio property to switch from OPUS to AAC audio track while maintaining video playback continuity | Audio codec switch initiated successfully using current-audio property, video playback continues without interruption |
| 8 | Complete AAC Transition | Monitor transition from OPUS to AAC audio codec, verify seamless audio changeover without audio dropouts or glitches | Transition from OPUS to AAC completed successfully with seamless audio changeover and no audio artifacts |
...
| 10 | Verify Codec Detection | Check output for OPUS and AAC codec detection confirmations using checkifCodecPlayed validation for both audio codecs | Output contains successful codec detection for both "opus" and "aac" codecs indicating proper codec recognition |
| 11 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating OPUS to AAC audio codec switching completed without errors |
