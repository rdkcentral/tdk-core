...
## TestCase Name
**FNCS_Playback_Set_Rate_0.5x_Only_Audio_AAC**
...
## Objective
To validate that an audio-only AAC stream can be played back at a reduced playback rate of 0.5x (half speed), ensuring smooth slow-motion playback and proper rate transitions for audio content without video components.
...
| 4 | Apply Slow Motion Rate Transition | Execute the slowMotion0.5x operation which reduces the playback rate to 50% of normal speed (0.5x rate) for audio content | Playback rate must transition to 0.5x successfully, and audio continues playing at reduced speed (pitch-shifted) |
| 5 | Monitor Slow Motion Audio Playback | Monitor playback position progression during slow motion operation for the configured timeout duration (default 10 seconds). Verify playback position increments occur at 0.5x rate intervals. If FIREBOLT_COMPLIANCE_CHECK_AV_STATUS is enabled, verify SOC level audio decoder status | Playback position must progress at 0.5x rate smoothly. If AV status check is enabled, audio decoder must show active decoding without interruptions |
