...
## TestCase Name
**FNCS_Playback_Set_Rate_0.5x_VP9_HDR**
...
## Objective
To validate that an HDR VP9 stream can be played back at a reduced playback rate of 0.5x (half speed), ensuring smooth slow-motion playback with proper HDR color gamut handling and rate transitions.
...
| 4 | Apply Slow Motion Rate Transition | Execute the slowMotion0.5x operation which reduces the playback rate to 50% of normal speed (0.5x rate) | Playback rate must transition to 0.5x successfully, and media continues playing at reduced speed with HDR color gamut preserved |
