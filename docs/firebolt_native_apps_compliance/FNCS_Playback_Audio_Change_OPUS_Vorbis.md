# FNCS_Playback_Audio_Change_OPUS_Vorbis Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_122

## TestCase Name
FNCS_Playback_Audio_Change_OPUS_Vorbis
...
## Objective
To verify the media pipeline's capability to switch between different audio codecs (OPUS and Vorbis) present in the same stream without changing the video, ensuring seamless audio codec transitions during playback. This test validates dynamic audio codec switching functionality by monitoring playback continuity and verifying successful codec detection during runtime transitions.
...
| 2 | Multi-codec stream with OPUS and Vorbis audio tracks must be available and referenced in MediaValidationVariables.video_src_url_opus_vorbis |
...
| 13 | video_src_url_opus_vorbis variable in MediaValidationVariables.py contains multi-codec stream URL (TDK_Asset_Sunrise_Vorbis_Opus.webm) with both OPUS and Vorbis audio tracks |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT config values, get multi-codec stream URL from MediaValidationVariables.video_src_url_opus_vorbis, construct tdk_mediapipelinetests command with timeout parameter, and run test_audio_change with OPUS/Vorbis stream | Configuration values retrieved, multi-codec stream URL obtained, command executed: `tdk_mediapipelinetests test_audio_change <OPUS_VORBIS_STREAM_URL> checkavstatus=no timeout=10`, and test application starts execution without errors |
...
| 7 | Initiate Audio Codec Switch | Use playbin current-audio property to switch from OPUS to Vorbis audio track while maintaining video playback continuity | Audio codec switch initiated successfully using current-audio property, video playback continues without interruption |
| 8 | Complete Vorbis Transition | Monitor transition from OPUS to Vorbis audio codec, verify seamless audio changeover without audio dropouts or glitches | Transition from OPUS to Vorbis completed successfully with seamless audio changeover and no audio artifacts |
| 9 | Validate Vorbis Playback | Monitor Vorbis audio playback for configured timeout duration (default 10 seconds), verify audio frames and codec stability | Vorbis audio playback proceeds successfully for full timeout duration with stable audio frames and confirmed Vorbis codec detection |
| 10 | Verify Codec Detection | Check output for OPUS and Vorbis codec detection confirmations using checkifCodecPlayed validation for both audio codecs | Output contains successful codec detection for both "opus" and "vorbis" codecs indicating proper codec recognition |
| 11 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating OPUS to Vorbis audio codec switching completed without errors |
