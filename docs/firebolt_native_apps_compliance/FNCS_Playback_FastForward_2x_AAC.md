# FNCS Playback FastForward 2x AAC Test Documentation

## TestCase ID
FNCS_PLAYBACK_32

## TestCase Name
FNCS_Playback_FastForward_2x_AAC
...
This test validates the fast forward functionality of AAC audio streams at 2x playback rate using GStreamer pipeline. The test verifies that the media pipeline can successfully handle trick play operations specifically for AAC codec streams, ensuring proper playback rate manipulation and audio stream processing.
...
| 5 | video_src_url_aac variable must be configured with TDK_Asset_AAC_Stream.aac in MediaValidationVariables.py as URL or file path for the AAC audio test stream used for fast forward validation |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS config value, get AAC stream URL from MediaValidationVariables.video_src_url_aac, construct tdk_mediapipelinetests command with fastforward2x operation, and run test_trickplay with AAC stream | Configuration values retrieved, AAC stream URL obtained, command executed: `tdk_mediapipelinetests_trickplay https://<server_hosting_stream>:<port_number>/TDK_Asset_AAC_Stream.aac checkavstatus=no operations=fastforward2x:10`, and test application starts execution without errors |
...
| 5 | Execute Fast Forward Playback | Start media playback with 2x fast forward rate for the configured timeout duration and monitor audio stream processing during fast forward operation | Fast forward playback must execute smoothly for specified duration with proper audio stream handling at 2x rate |
...
| 7 | Verify Test Results | Check command output for success indicators including "Failures: 0" and "Errors: 0" or "failed: 0" strings to confirm successful test execution | Output must contain success indicators showing no failures or errors during fast forward AAC playback test |
