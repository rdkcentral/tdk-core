...
## TestCase Name
**FNCS_Playback_SeekBackward_FF2x_HLS**
...
## Objective
To validate that an HLS adaptive streaming content supports backward seek operation followed by fast-forward playback at 2x speed, ensuring proper seeking accuracy and fast playback with correct media delivery over HTTP Live Streaming protocol.
...
| 3 | Verify Playback Starts | Validate that the mediapipeline initializes and begins HLS adaptive stream playback using GStreamer playbin element with westeros-sink for rendering and native audio-sink for audio | HLS adaptive stream playback must begin successfully without pipeline errors |
