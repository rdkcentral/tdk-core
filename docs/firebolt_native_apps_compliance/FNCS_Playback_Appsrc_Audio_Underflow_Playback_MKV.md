# FNCS_Playback_Appsrc_Audio_Underflow_Playback_MKV Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_299

## TestCase Name
FNCS_Playback_Appsrc_Audio_Underflow_Playback_MKV
...
| 2 | TDK_Asset_Waterfall_Audio_MKV.mkv must be installed in the server hosting streams or installed inside the device if user is selecting filesrc instead of httpsrc (copying stream to USB like file:/tmp/usb/) or httpsrc via HTTPS server (https://<server_hosting_stream>:<port_number>/) |
...
| 11 | audio_src_url_mkv variable in MediaValidationVariables.py contains MKV audio stream URL (TDK_Asset_Waterfall_Audio_MKV.mkv) for appsrc underflow testing |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS config value, get MKV stream URL from MediaValidationVariables.audio_src_url_mkv, construct tdk_mediapipelinetests command with underflow threshold parameter (490077 bytes), and run test_appsrc_audio_underflow with MKV stream | Configuration values retrieved, MKV stream URL obtained, command executed: `tdk_mediapipelinetests test_appsrc_audio_underflow https://<server_hosting_stream>:<port_number>/TDK_Asset_Waterfall_Audio_MKV.mkv checkavstatus=no validateFullPlayback underflow_threshold=490077`, and test application starts execution without errors |
...
| 5 | Setup Appsrc Element | Configure appsrc with size property set to underflow threshold (490077 bytes), set appropriate MKV audio caps, and connect push-buffer signal for manual feeding | Appsrc element configured successfully with threshold size and MKV audio capabilities |
...
| 7 | Reach Buffer Threshold | Continue feeding audio buffers via appsrc until threshold bytes (490077) are reached | Threshold limit reached successfully, pipeline consuming all provided audio buffer data |
...
| 11 | Update Buffer Threshold | Double the buffer threshold value to 980154 bytes and prepare for additional audio buffer feeding | Buffer threshold updated successfully, ready for extended audio buffer feeding |
...
| 16 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating MKV appsrc audio underflow recovery completed without errors |
