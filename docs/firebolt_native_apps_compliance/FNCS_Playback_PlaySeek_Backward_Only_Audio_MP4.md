...
## TestCase Name
**FNCS_Playback_PlaySeek_Backward_Only_Audio_MP4**
...
## Objective
To validate that an AAC audio stream in MP4 format supports backward seek operation during playback, ensuring proper seeking accuracy and smooth playback resumption at the target position.
...
| 2 | Configure and Execute Backward Seek Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration values from Device config file. Retrieve AAC MP4 audio stream URL from MediaValidationVariables library. Construct mediapipelinetests command with test_playseek operation specifying backward seek operation. Execute command: `tdk_mediapipelinetests_playseek https://<server_hosting_stream>:<port_number>/[AAC_MP4_STREAM.mp4] checkavstatus=<yes/no> operations=seek:0` | Configuration values must be retrieved successfully. Stream URL must be obtained correctly. Command must be constructed properly. Test application must start execution without errors |
| 3 | Verify Initial Playback Starts | Validate that the mediapipeline initializes and begins AAC MP4 audio stream playback using GStreamer playbin element with native audio-sink for rendering | AAC MP4 audio stream playback must begin successfully without pipeline errors |
