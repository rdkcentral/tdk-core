# FNCS_Playback_FastForward_4x_Only_Audio_AAC Test Documentation

## TestCase ID
FNCS_PLAYBACK_277

## TestCase Name
FNCS_Playback_FastForward_4x_Only_Audio_AAC
...
## Objective
This test validates the fast forward playback functionality at 4x rate for audio-only AAC encoded content to ensure proper trick-play operations and audio pipeline handling during accelerated playback scenarios.
...
| 3 | Execute Fast Forward 4x Test | Retrieve configuration values for AV status check and playback timeout, obtain AAC audio stream URL from MediaValidationVariables, construct trickplay test command with fastforward4x operations, and execute the command `tdk_mediapipelinetests_trickplay https://&lt;server_hosting_stream&gt;:&lt;port_number&gt;/[STREAM_NAME] checkavstatus=no operations=fastforward4x:10` | Configuration values must be retrieved successfully, stream URL must be obtained correctly, command must be constructed properly, test command execution must start successfully and test application starts execution without errors |
| 4 | Monitor Fast Forward Audio Playback | Monitor audio-only AAC playback during 4x fast forward operation to verify proper audio rendering, timeline progression, and playback rate acceleration | AAC audio must render properly at 4x fast forward rate with smooth timeline progression and accelerated playback without errors |
...
## Test Attributes
**Supported Models:** Video_Accelerator, RPI  
