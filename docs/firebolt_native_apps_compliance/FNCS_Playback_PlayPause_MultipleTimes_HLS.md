...
## TestCase Name
**FNCS_Playback_PlayPause_MultipleTimes_HLS**
...
## Objective
To validate that HLS adaptive streaming content supports multiple sequential play and pause operations over stressed iterations, ensuring proper playback state transitions and smooth resumption across repeated play/pause cycles with correct adaptive bitrate handling.
...
| 10 | Monitor Overall Playback Quality | Throughout all play/pause iterations, monitor video frame rendering using westerossink "stats" property and audio rendering using native audio-sink "stats" property. Verify that video and audio frame drop rates do not exceed 1% across all state transitions. If FIREBOLT_COMPLIANCE_CHECK_AV_STATUS is enabled, verify SOC level decoders remain active | Video and audio frames must render with drop rates below 1%. All state transitions must occur without pipeline errors. If AV status check is enabled, decoders must show continuous activity |
