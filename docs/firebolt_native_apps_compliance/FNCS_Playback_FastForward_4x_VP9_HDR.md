# FNCS_Playback_FastForward_4x_VP9_HDR Test Documentation

## TestCase ID
FNCS_PLAYBACK_378

## TestCase Name
FNCS_Playback_FastForward_4x_VP9_HDR
...
## Objective
This test validates the fast forward playback functionality at 4x rate for VP9 HDR encoded video content to ensure proper trick-play operations and media pipeline handling during accelerated playback scenarios.
...
| 3 | Execute Fast Forward 4x Test | Retrieve configuration values for AV status check and playback timeout, obtain VP9 HDR stream URL from MediaValidationVariables, construct trickplay test command with fastforward4x operations, and execute the command `tdk_mediapipelinetests_trickplay https://&lt;server_hosting_stream&gt;:&lt;port_number&gt;/[STREAM_NAME] checkavstatus=no operations=fastforward4x:10` | Configuration values must be retrieved successfully, stream URL must be obtained correctly, command must be constructed properly, test command execution must start successfully and test application starts execution without errors |
| 4 | Monitor Fast Forward Playback | Monitor VP9 HDR video playback during 4x fast forward operation to verify proper frame rendering, timeline progression, and playback rate acceleration | VP9 HDR video must render properly at 4x fast forward rate with smooth timeline progression and accelerated playback without errors |
