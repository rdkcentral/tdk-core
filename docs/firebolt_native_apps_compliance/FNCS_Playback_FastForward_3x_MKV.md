# FNCS_PLAYBACK_303 - 4K AV1 MKV FastForward 3x Playback Test
...
FNCS_Playback_FastForward_3x_MKV
...
Validate the FastForward 3x trick play operation on 4K AV1 MKV container streams to ensure proper high-resolution video acceleration control and container format handling during trick play operations.
...
| 3 | Execute MediaPipeline FastForward Test | Retrieve configuration values (FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT), get 4K AV1 MKV stream URL from MediaValidationVariables.video_src_url_4k_av1_mkv, construct trickplay command with fastforward3x operation parameters, and execute tdk_mediapipelinetests_trickplay command | Configuration values must be retrieved successfully, 4K AV1 MKV stream URL must be obtained correctly, fastforward3x command must be constructed properly as `tdk_mediapipelinetests_trickplay https://<server_hosting_stream>:<port_number>/[STREAM_NAME] checkavstatus=no operations=fastforward3x:10`, and test application starts execution without errors |
| 4 | Setup GStreamer Pipeline | Create GStreamer pipeline with playbin element configured for 4K AV1 MKV stream playback using westeros-sink for video rendering and proper AV1 decoder configuration | GStreamer pipeline must be created successfully with proper 4K AV1 decoding capabilities and MKV container demuxing support |
| 5 | Configure Trick Play Operation | Set pipeline to PLAYING state and configure fastforward3x operation with specified timeout duration for accelerated 4K video playback | Pipeline must transition to PLAYING state successfully and fastforward3x operation must be configured with 3x playback rate for 4K AV1 content |
| 6 | Execute FastForward 3x Operation | Start fastforward3x playback operation and monitor 4K AV1 MKV stream processing for specified timeout duration while maintaining video quality | FastForward 3x operation must execute successfully, 4K AV1 MKV stream must play at 3x speed with proper high-resolution video processing |
| 7 | Monitor Playback and Position Validation | Track playback position progression and verify that position advances at 3x normal playback rate throughout the operation duration for 4K content | Playback position must advance consistently at 3x rate, indicating proper fastforward operation without frame drops or quality degradation for 4K AV1 content |
