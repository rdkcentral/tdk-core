# FNCS_Playback_CheckLatency_Seek_Forward_HEVC Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_220

## TestCase Name
FNCS_Playback_CheckLatency_Seek_Forward_HEVC

## Objective
Test to do forward seek operation followed by fast forward of a HEVC stream with playback rate 2 and verify the latency of the operation
...
| 6 | FIREBOLT_COMPLIANCE_TRICKPLAY_LATENCY_THRESHOLD configuration should be set to 1000 (default) in Video_Accelerator.config / RPI-Client.config |
| 7 | FIREBOLT_COMPLIANCE_SEEK_POSITION configuration should be set to 10 (default) in Video_Accelerator.config / RPI-Client.config |
