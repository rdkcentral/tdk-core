# FNCS_Playback_FastForward_3x_H264
...
## Objective
To validate the fast forward functionality of H264 video streams with 3x playback rate, ensuring proper video frame progression and timing accuracy during trick play operations.
...
| 2 | Execute MediaPipeline FastForward Test | Retrieve configuration values (FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT), get H264 stream URL from MediaValidationVariables.video_src_url_dash_h264, construct trickplay command with fastforward3x operation parameters, and execute tdk_mediapipelinetests_trickplay command | Configuration values must be retrieved successfully, H264 stream URL must be obtained correctly, fastforward3x command must be constructed properly as `tdk_mediapipelinetests_trickplay https://&lt;server_hosting_stream&gt;:&lt;port_number&gt;/[STREAM_NAME] checkavstatus=no operations=fastforward3x:10`, and test application starts execution without errors |
| 3 | Initialize GStreamer Pipeline | Create GStreamer pipeline using playbin element and configure it for H264 stream playback with httpsrc source element for network streaming | GStreamer pipeline must be created successfully with playbin configured for H264 stream processing and httpsrc elements initialized properly |
| 4 | Configure Pipeline Elements | Set up pipeline elements including video sink (westerossink), audio sink, and configure pipeline properties for H264 decoding with fast forward capabilities | All pipeline elements must be configured successfully with proper H264 decoder and sink elements ready for fast forward operations |
