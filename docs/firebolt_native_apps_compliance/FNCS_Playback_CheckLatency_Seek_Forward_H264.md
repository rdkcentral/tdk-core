# FNCS_Playback_CheckLatency_Seek_Forward_H264 Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_219

## TestCase Name
FNCS_Playback_CheckLatency_Seek_Forward_H264

## Objective
Test to do forward seek operation followed by fast forward of a H264 stream with playback rate 2
...
| 3 | video_src_url_dash_h264 variable must be configured with DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd in MediaValidationVariables.py as URL or file path for the test stream |
...
| 6 | FIREBOLT_COMPLIANCE_SEEK_POSITION configuration should be set to 10 (default) in Video_Accelerator.config / RPI-Client.config |
| 7 | FIREBOLT_COMPLIANCE_TRICKPLAY_LATENCY_THRESHOLD configuration should be set to 1000 (default) in Video_Accelerator.config / RPI-Client.config |
