# FNCS_Playback_Appsrc_Video_Underflow_Signal_4K_MKV Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_300

## TestCase Name
FNCS_Playback_Appsrc_Video_Underflow_Signal_4K_MKV
...
| 11 | video_src_url_4k_av1_mkv variable in MediaValidationVariables.py contains 4K AV1 MKV video stream URL for appsrc underflow signal testing |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS config value, get 4K AV1 MKV stream URL from MediaValidationVariables.video_src_url_4k_av1_mkv, construct tdk_mediapipelinetests command with underflow threshold parameter (3218827 bytes), and run test_appsrc_video_underflow_signal with 4K MKV stream | Configuration values retrieved, 4K AV1 MKV stream URL obtained, command executed: `tdk_mediapipelinetests test_appsrc_video_underflow_signal <4K_MKV_STREAM_URL> checkavstatus=no validateFullPlayback underflow_threshold=3218827`, and test application starts execution without errors |
...
| 7 | Reach Buffer Threshold | Continue feeding video buffers via appsrc until threshold bytes (3218827) are reached | Threshold limit reached successfully, pipeline consuming all provided video buffer data |
...
| 11 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating 4K AV1 MKV appsrc video underflow signal detection completed without errors |
