...
## Objective
To validate that AAC audio stream supports rewind playback at 3x speed (negative playback rate), ensuring proper backward playback at accelerated speed and smooth media playback with correct audio rendering.
...
| 5 | Monitor Backward Playback at 3x Speed | Monitor audio rendering during rewind playback at 3x speed. Verify that audio frames render in reverse direction at accelerated pace. Verify playback position decreases over time at approximately 3x the normal rate | Audio must render in backward direction. Position must decrease smoothly at 3x speed. No pipeline errors should occur during rewind playback |
...
| 7 | Verify Smooth Playback Transition | Confirm that rewind playback at 3x speed continues smoothly without interruptions or audio glitches until reaching the beginning of stream or timeout | Audio playback must continue seamlessly throughout rewind operation without interruption |
