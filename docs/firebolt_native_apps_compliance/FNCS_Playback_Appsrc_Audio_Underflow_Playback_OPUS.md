# FNCS_Playback_Appsrc_Audio_Underflow_Playback_OPUS Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_256

## TestCase Name
FNCS_Playback_Appsrc_Audio_Underflow_Playback_OPUS
...
| 2 | audio.webm must be installed in the server hosting streams or installed inside the device if user is selecting filesrc instead of httpsrc (copying stream to USB like file:/tmp/usb/) or httpsrc via HTTPS server (https://<server_hosting_stream>:<port_number>/) |
...
| 11 | audio_src_url_webm_opus variable in MediaValidationVariables.py contains OPUS audio stream URL (DASH_VP9_OPUS_WebM/audio.webm) for appsrc underflow testing |
...
| 11 | Update Buffer Threshold | Double the buffer threshold value to 838676 bytes and prepare for additional audio buffer feeding | Buffer threshold updated successfully, ready for extended audio buffer feeding |
...
| 16 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating OPUS appsrc audio underflow recovery completed without errors |
