...
## Objective
To validate that the system resource usage (CPU and memory) remains within the configured acceptable limits after uninstalling an application via the AppManager.
...
| 3 | Install the application | Install the downloaded application bundle without launching it, ensuring it is present on the device before uninstall measurement. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.install", "params": {"packageId": "com.rdkcentral.google", "version": "0.1.0", "additionalMetadata": [{"name": "type", "value": "native/dac-app"}], "fileLocator": "<PACKAGEMANAGER_FILE_LOCATOR>/package<download_id>"}}` | The application should be installed successfully. |
| 4 | Uninstall the application | Send an uninstall request to remove the installed application from the device. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.uninstall", "params": {"packageId": "com.rdkcentral.google"}}` | The application should be uninstalled successfully. |
| 5 | Validate system resource usage post-uninstall | After the uninstall completes, measure and validate the system CPU and memory usage against the configured resource usage thresholds. The resource validation checks that both CPU utilization and memory consumption are within the acceptable limits defined in the device configuration. Invoke the DeviceInfo systeminfo API to retrieve current system metrics: <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "DeviceInfo.1.systeminfo"}` <br>Extract CPU load from the `cpuload` field and calculate memory usage using the formula `(totalram - freeram) / totalram × 100`. | Resource usage (CPU and memory) should be within the expected limits after the uninstall operation. No resource leaks should be observed. |
