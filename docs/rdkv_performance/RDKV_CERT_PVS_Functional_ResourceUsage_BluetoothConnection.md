...
| 2 | Confirm required plugins are available | The following plugins must be present and activatable: DeviceInfo, org.rdk.Bluetooth, org.rdk.System. | All required plugins should be available in the build. |
...
| 4 | Ensure a Bluetooth emulator is available | A Bluetooth emulator device must be accessible via SSH and available for pairing during the test execution. | The Bluetooth emulator should be reachable and configurable via SSH. |
