...
## Objective
To validate the CPU and memory resource usage of the device during 4K HLS video playback in the Lightning/Unified Player application and verify that resource usage is within acceptable limits.
...
| 5 | Confirm the 4K HLS video stream URL is configured | The `video_src_url_4k_hls` variable in MediaValidationVariables must be configured with a valid 4K HLS video stream URL. | The 4K HLS video URL should be configured and accessible. |
...
|#|Step Name | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Retrieve device configuration and logging method | Read the device configuration file to obtain the `LOGGING_METHOD` setting. Configure the video test URL using the 4K HLS video source (`video_src_url_4k_hls`) with video type "hls" and HLS HEVC codec player list (`codec_hls_hevc`). Set the close operation with a 5-second interval: `setOperation("close", 5)`. | The device configuration file should be read successfully and the `LOGGING_METHOD` value should be retrieved. |
...
| 3 | Build and store 4K HLS video test URL in PersistentStore | Construct the unified player test application URL with the 4K HLS video URL, close operation, and logging arguments. Store it in PersistentStore: <br>`{"jsonrpc":"2.0","id":1,"method":"org.rdk.PersistentStore.setValue","params":{"namespace":"MVS","key":"lightningURL","value":"<test_app_url>"}}` | The 4K HLS video test URL should be stored successfully in PersistentStore under namespace MVS with key lightningURL. |
...
| 6 | Launch the unified player application with 4K HLS video | Launch the unified player application via AppManager: <br>`{"jsonrpc":"2.0","id":1,"method":"org.rdk.AppManager.launchApp","params":{"appId":"<app_name>","intent":"","launchArgs":""}}` <br>Verify the application is active: <br>`{"jsonrpc":"2.0","id":1,"method":"org.rdk.AppManager.getLoadedApps"}` | The application should be launched and appear in loaded apps with APP_STATE_ACTIVE. The application should begin loading the 4K HLS video stream. |
| 7 | Monitor application log for 4K HLS video playback confirmation | Monitor the application log (via REST_API log file or WEB_INSPECT WebSocket console) for the "URL Info:" message indicating the application has successfully loaded the 4K HLS video URL and started playback. | The "URL Info:" confirmation message should appear in the application log, indicating the application has loaded the 4K HLS video URL from PersistentStore. |
| 8 | Validate resource usage during 4K HLS video playback | After the application launches and confirms video URL loading ("URL Info:"), validate the CPU and memory usage using the resource validation step, which internally invokes `DeviceInfo.1.systeminfo`: <br>`{"jsonrpc":"2.0","id":1,"method":"DeviceInfo.1.systeminfo"}` | The CPU and memory usage should be within the expected threshold limits during 4K HLS video playback. The resource usage validation should return SUCCESS with no ERROR status. |
