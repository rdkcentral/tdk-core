...
## Objective
To validate that the time taken for play and pause operations on an EC3 (Dolby Digital Plus) video/audio stream in a Lightning application is within the expected performance threshold.
...
|3|EC3 video/audio stream URL must be configured in `MediaValidationVariables`.|
...
| 3 | Configure Video Player URL | Build the test app URL with EC3 source and operations (pause 10s, play 10s). | Test app URL constructed. |
| 4 | Launch Video Player App | Load the video player application in WebKitBrowser with EC3 stream. | Video player launches and EC3 stream starts playing. |
...
| 6 | Validate Play/Pause Time | Calculate the time taken for play and pause on EC3 stream. Compare against threshold. | Time taken for play/pause on EC3 stream is within the expected threshold. |
