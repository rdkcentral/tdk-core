...
| 6 | Configure switch threshold in device config | `APPMANAGER_LAUNCH_THRESHOLD_VALUE` and `THRESHOLD_OFFSET` must be set in the device-specific configuration file for app switch time validation. | Threshold and offset values should be correctly configured. |
