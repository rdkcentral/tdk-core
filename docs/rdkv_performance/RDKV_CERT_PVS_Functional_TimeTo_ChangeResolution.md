...
| 2 | Confirm org.rdk.DisplaySettings plugin is available | The org.rdk.DisplaySettings plugin must be present and activatable in the device build. | The DisplaySettings plugin should be available in the build. |
...
| 4 | Configure CHANGE_RESOLUTION_VALUE in device config | `CHANGE_RESOLUTION_VALUE` must be set to the target resolution (e.g., 1080p) to change to during the test in the device-specific config file. | The resolution value should be configured and must differ from the current active resolution. |
