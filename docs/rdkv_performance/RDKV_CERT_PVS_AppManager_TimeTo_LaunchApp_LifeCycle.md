...
## Objective
To measure and validate the time taken for each lifecycle state transition (APP_STATE_LOADING, APP_STATE_INITIALIZING, APP_STATE_PAUSED, APP_STATE_ACTIVE) following an AppManager launchApp request, and verify that each individual transition time as well as the total launch time are within the configured performance threshold.
