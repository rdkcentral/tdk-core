...
## Objective
To validate that the memory usage of the unified player application remains within the configured threshold while performing MP4 video play and pause operations with the WebInspect remote debugging port kept open throughout the test duration.
...
| 6 | Configure MEMORY_USAGE_THRESHOLD in device config | `MEMORY_USAGE_THRESHOLD` must be set in the device-specific config file with the maximum acceptable memory usage value in bytes. | The memory usage threshold should be correctly configured for result comparison. |
| 7 | Configure unified_player_app_download_url in MediaValidationVariables | `unified_player_app_download_url` must be set to the hosted bolt package URL for the lightning unified player application (e.g., `com.rdkcentral.lightning-unified-player+0.1.0.bolt`). | The app download URL should be configured and accessible. |
...
| 7 | Verify application installation | Confirm that the unified player application appears in the installed packages list after installation. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.1.listPackages"}` | The unified player application should appear in the installed packages list, confirming successful installation. |
...
| 13 | Monitor video play and pause events during playback | Monitor the playback event log via REST API (reading from the app log file) or via the WebInspect console socket, depending on the configured `LOGGING_METHOD`. Track expected and observed play and pause events until a TEST RESULT message is received or a timeout is reached. Tracked log entries include: `Expected Event: paused`, `Observed Event: paused`, `Expected Event: play`, `Observed Event: play`, and `TEST RESULT:`. | All expected play and pause events should be observed and the video player application should report a TEST RESULT indicating successful completion. |
| 14 | Measure memory usage via SSH | After video playback events are monitored, connect to the DUT via SSH and read the maximum memory usage from the cgroup memory controller. Compare the output value against `MEMORY_USAGE_THRESHOLD` from the device config. <br>`cd /sys/fs/cgroup/memory && cat memory.max_usage_in_bytes` | The memory usage value should be retrieved successfully. The measured memory usage in bytes should be within the configured threshold: output bytes must not exceed `MEMORY_USAGE_THRESHOLD`. |
| 15 | Terminate the unified player application | Terminate the running application using the AppManager terminateApp API. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.terminateApp", "params": {"appId": "com.rdkcentral.lightning-unified-player"}}` | The application should be terminated successfully. |
