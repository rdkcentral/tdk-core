...
## Objective
To validate that WebKitBrowser memory usage does not grow beyond acceptable limits during extended video playback, as measured using the Web Inspector tool.
...
| 3 | Connect Web Inspector | Connect the Web Inspector to the WebKitBrowser process to enable memory profiling. | Web Inspector connected. |
...
| 8 | Validate Memory Usage | Verify that memory growth during playback does not exceed the acceptable threshold. | WebKitBrowser memory usage during extended video playback is within the acceptable limit as measured by Web Inspector. |
