...
## Objective
To validate that the system resource usage (CPU and memory) remains within acceptable limits after closing a running application via the AppManager closeApp API.
...
| 4 | Launch the application | Send a launch request for the installed application so it is in the active running state before the close operation. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.launchApp", "params": {"appId": "com.rdkcentral.google", "intent": "", "launchArgs": ""}}` | The application should be installed and launched successfully. |
| 5 | Close the application | Send a close request to the AppManager to close the running application. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.closeApp", "params": {"appId": "com.rdkcentral.google"}}` | The close request should be accepted and the application should begin the close process. |
| 6 | Validate system resource usage post-close | Allow 10 seconds for cleanup after close, then measure and validate the system CPU and memory usage against the configured resource usage thresholds. Invoke the DeviceInfo systeminfo API to retrieve current system metrics: <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "DeviceInfo.1.systeminfo"}` <br>Extract CPU load from the `cpuload` field and calculate memory usage using the formula `(totalram - freeram) / totalram × 100`. | Resource usage (CPU and memory) should be within the configured acceptable limits after the close operation. No resource leaks should remain after the app is closed. |
