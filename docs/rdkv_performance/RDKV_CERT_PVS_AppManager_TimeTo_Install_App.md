...
## Objective
To validate that the time taken to install an application bundle via the AppPackageManager is within the configured performance threshold, measured from the install request to the receipt of the onAppInstalled event.
...
| 5 | Configure install threshold in device config | `APPMANAGER_INSTALL_THRESHOLD_VALUE` and `THRESHOLD_OFFSET` must be set in the device-specific configuration file with appropriate millisecond values. | Threshold and offset values should be correctly configured for performance comparison. |
| 6 | Configure PACKAGEMANAGER_FILE_LOCATOR in device config | `PACKAGEMANAGER_FILE_LOCATOR` must be set to the correct path on the device where downloaded packages are stored. | The file locator path should be correctly configured in the device-specific config file. |
...
| 2 | Check if the application is already installed | Query the list of installed packages to determine if the application is already present on the device. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.1.listPackages"}` | The installed packages list should be returned successfully. |
| 3 | Uninstall existing application if present | If the application is already installed, uninstall it to ensure a clean install measurement. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.uninstall", "params": {"packageId": "com.rdkcentral.google"}}` | The application should be uninstalled successfully, ensuring a fresh installation can be measured. |
