...
## Objective
To validate that a large application download can be paused while in progress, remains paused without progress change, and resumes with measurable progress advancement.
...
| 3 | Provide a sufficiently large validation bundle | Configure `Large_Validation_File` and `app_download_url` with a reachable large application bundle and its hosting URL. `Large_Validation_File` can be any file of approximately 1 GB in size. | The bundle should be large enough to remain in progress during the progress polling period. |
