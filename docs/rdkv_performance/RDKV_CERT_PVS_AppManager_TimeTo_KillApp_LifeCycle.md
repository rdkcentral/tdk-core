...
RDKV_PERFORMANCE_96
## TestCase Name
RDKV_CERT_PVS_AppManager_TimeTo_KillApp_LifeCycle
...
## Objective
To measure and validate the time taken for each lifecycle state transition (APP_STATE_PAUSED, APP_STATE_TERMINATING, APP_STATE_UNLOADED) following an AppManager killApp request, and verify that each transition time is within the configured performance threshold.
...
| 5 | Launch the application | Send a launch request for the installed application using AppManager. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.launchApp", "params": {"appId": "com.rdkcentral.google", "intent": "", "launchArgs": ""}}` | The application should be installed and launched successfully and reach APP_STATE_ACTIVE. |
...
| 7 | Trigger killApp and record start time | Record the current UTC timestamp as the kill start time, then send the killApp request to the AppManager. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.killApp", "params": {"appId": "com.rdkcentral.google"}}` | The killApp request should be accepted by the AppManager and return SUCCESS. |
| 8 | Monitor event buffer for kill lifecycle events | Monitor the onAppLifecycleStateChanged event buffer for up to 120 seconds. Record the timestamps of APP_STATE_PAUSED, APP_STATE_TERMINATING, and APP_STATE_UNLOADED events as they arrive. The loop exits when APP_STATE_UNLOADED is received. | All three kill lifecycle events (APP_STATE_PAUSED, APP_STATE_TERMINATING, APP_STATE_UNLOADED) should be received within the timeout period. |
| 9 | Verify all kill lifecycle states received | Confirm that all three expected lifecycle state transitions were received: APP_STATE_PAUSED, APP_STATE_TERMINATING, and APP_STATE_UNLOADED for com.rdkcentral.google. <br>Example event: `{"jsonrpc": "2.0", "method": "client.events.1.onAppLifecycleStateChanged", "params": {"appId": "com.rdkcentral.google", "newState": "APP_STATE_UNLOADED", "oldState": "APP_STATE_TERMINATING", "errorReason": "APP_ERROR_NONE"}}` | All three kill lifecycle state transitions should be confirmed for the application. |
| 10 | Calculate transition times and validate against threshold | Compute the elapsed time in milliseconds for each transition: kill start → APP_STATE_PAUSED, APP_STATE_PAUSED → APP_STATE_TERMINATING, and APP_STATE_TERMINATING → APP_STATE_UNLOADED. Compare each against `APPMANAGER_KILL_EVENT_THRESHOLD_VALUE` + `THRESHOLD_OFFSET` from device config. | Each individual transition time should be within the range: 0 < time < (APPMANAGER_KILL_EVENT_THRESHOLD_VALUE + THRESHOLD_OFFSET) milliseconds. |
