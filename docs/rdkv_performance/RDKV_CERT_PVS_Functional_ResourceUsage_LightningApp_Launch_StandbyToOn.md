...
## TestCase Name
RDKV_CERT_PVS_Functional_ResourceUsage_LightningApp_Launch_StandbyToOn
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate that CPU load and memory usage are within acceptable ranges when LightningApp is launched after transitioning the device from Standby to On power state.
...
| 5 | Launch LightningApp | Activate LightningApp plugin: <br>`{"jsonrpc":"2.0","id":1234567890,"method":"Controller.1.activate","params":{"callsign":"LightningApp"}}` | LightningApp launched. |
| 6 | Capture Resource Usage After Launch | Record CPU and memory usage after LightningApp launch following standby-to-on transition. | Resource data captured. |
| 7 | Validate Resource Usage | Verify CPU load and memory usage are within acceptable limits. | CPU load and memory usage after LightningApp launch following Standby-to-On transition are within the expected limits. |
