## TestCase ID
RDKV_PERFORMANCE_160
## TestCase Name
RDKV_CERT_PVS_HtmlApp_Video_Playback_MemoryUsage_WithWebinspect
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate that HtmlApp memory usage does not grow beyond acceptable limits during extended video playback, as measured using the Web Inspector tool.
...
| 3 | Connect Web Inspector | Connect the Web Inspector to the HtmlApp process to enable memory profiling. | Web Inspector connected. |
...
| 8 | Validate Memory Usage | Verify that memory growth during playback does not exceed the acceptable threshold. | HtmlApp memory usage during extended video playback is within the acceptable limit as measured by Web Inspector. |
