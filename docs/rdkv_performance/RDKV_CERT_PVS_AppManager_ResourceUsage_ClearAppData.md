...
| 5 | Launch the application | Send a launch request for the installed application via AppManager. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.launchApp", "params": {"appId": "com.rdkcentral.google", "intent": "", "launchArgs": ""}}` | The application should launch successfully and be in the active running state. |
| 6 | Validate resource usage before clearing app data | Measure and validate the system CPU and memory usage while the application is running, before the clearAppData operation. Invoke the DeviceInfo systeminfo API to retrieve current system metrics: <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "DeviceInfo.1.systeminfo"}` <br>Extract CPU load from the `cpuload` field and calculate memory usage using the formula `(totalram - freeram) / totalram × 100`. | Resource usage (CPU and memory) should be within the expected limits. |
| 7 | Terminate the application | Terminate the running application before clearing app data. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.terminateApp", "params": {"appId": "com.rdkcentral.google"}}` | The application should terminate successfully. |
| 8 | Clear application data | Invoke the AppManager clearAppData API to clear all stored data for the application. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.clearAppData", "params": {"appId": "com.rdkcentral.google"}}` | The clearAppData request should return SUCCESS and all application data for com.rdkcentral.google should be cleared. |
| 9 | Validate resource usage after clearing app data | Measure and validate the system CPU and memory usage again after the clearAppData operation. Invoke the DeviceInfo systeminfo API to retrieve current system metrics: <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "DeviceInfo.1.systeminfo"}` <br>Extract CPU load from the `cpuload` field and calculate memory usage using the formula `(totalram - freeram) / totalram × 100`. | Resource usage (CPU and memory) should be within the expected limits after clearing app data. No unexpected resource spikes should be observed. |
