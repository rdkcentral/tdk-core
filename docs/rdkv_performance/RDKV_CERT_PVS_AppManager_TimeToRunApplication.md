...
| 4 | Configure run application threshold in device config | `APPMANAGER_RUNAPP_THRESHOLD_VALUE` and `THRESHOLD_OFFSET` must be set in the device-specific configuration file with appropriate millisecond values. | Threshold and offset values should be correctly configured for performance comparison. |
...
| 5 | Launch the application and record start time | Record the current UTC timestamp as the run start time, then send the launch request for the application. <br>`{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppManager.launchApp", "params": {"appId": "com.rdkcentral.google", "intent": "", "launchArgs": ""}}` | The launch request should be accepted by the AppManager. |
| 6 | Wait for APP_STATE_ACTIVE event | Monitor the event buffer until the onAppLifecycleStateChanged event containing APP_STATE_ACTIVE is received or a 120-second timeout is reached. | The APP_STATE_ACTIVE event should be received within the timeout period, confirming the application is running. |
| 7 | Apply stabilization delay and record end time | After receiving the APP_STATE_ACTIVE event, apply a 3-second stabilization delay to allow the application to fully settle, then record the end timestamp. | The application should remain stable during the 3-second stabilization period. |
| 8 | Calculate run time and validate against threshold | Compute the total elapsed run time in milliseconds from the launch start time to the stabilized end time. Compare against `APPMANAGER_RUNAPP_THRESHOLD_VALUE` + `THRESHOLD_OFFSET` from device config. | The total application run time including stabilization should be within the configured performance threshold. |
