## TestCase ID
RDKV_PERFORMANCE_141
## TestCase Name
RDKV_CERT_PVS_Functional_ResourceUsage_Cobalt_Launch_StandbyToOn
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate that CPU load and memory usage are within acceptable ranges when Cobalt is launched after transitioning the device from Standby to On power state.
...
| 5 | Launch Cobalt | Activate Cobalt plugin: <br>`{"jsonrpc":"2.0","id":1234567890,"method":"Controller.1.activate","params":{"callsign":"Cobalt"}}` | Cobalt launched. |
| 6 | Capture Resource Usage After Launch | Record CPU and memory usage after Cobalt launch following standby-to-on transition. | Resource data captured. |
| 7 | Validate Resource Usage | Verify CPU load and memory usage are within acceptable limits. | CPU load and memory usage after Cobalt launch following Standby-to-On transition are within the expected limits. |
