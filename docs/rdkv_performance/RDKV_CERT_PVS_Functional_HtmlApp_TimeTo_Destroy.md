...
## Objective
To validate that the time taken to destroy the HtmlApp plugin is within the expected performance threshold.
...
| 4 | Destroy HtmlApp | Deactivate HtmlApp plugin: <br>`{"jsonrpc":"2.0","id":1234567890,"method":"Controller.1.deactivate","params":{"callsign":"HtmlApp"}}` | Deactivate request sent. |
| 5 | Record Destroy End Time | Record UTC timestamp after HtmlApp is deactivated. | End time recorded. |
| 6 | Calculate and Validate Destroy Time | Calculate destroy time = end timestamp - start timestamp. Compare against threshold. | Time to destroy HtmlApp is within the expected threshold. |
