...
| 3 | Ensure WiFi network is available | Either the DUT's default interface should already be WiFi, or a WiFi SSID with the same IP address range as Ethernet should be available for connection during the test. | A WiFi network should be accessible for the DUT to connect to and persist across reboot. |
