...
## TestCase Name
RDKV_CERT_PVS_Browser_HtmlApp_ResourceUsage_LoadInvalidURL
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To validate that resource usage remains acceptable and no crash is observed when an invalid URL is loaded in the HtmlApp plugin.
...
| 3 | Load Invalid URL in HtmlApp | Set an invalid URL in HtmlApp: <br>`{"jsonrpc":"2.0","id":1234567890,"method":"HtmlApp.1.url","params":{"url":"<invalid_url>"}}` | URL set (load may fail gracefully). |
...
| 5 | Validate HtmlApp Stability | Verify HtmlApp plugin is still in activated and responsive state: <br>`{"jsonrpc":"2.0","id":1234567890,"method":"Controller.1.status@HtmlApp"}` | HtmlApp is stable and no crash is observed. |
