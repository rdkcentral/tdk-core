...
## Objective
To validate that the DUT can successfully decode an MP3 audio file encoded at 128 kbps and 44 kHz using the WebAudio API. This test confirms that the codec is decoded correctly and the expected metadata is reported accurately, ensuring MP3 codec support meets certification requirements.
...
| 2 | Navigate to TC_WEBAUDIO_MANUAL_10 tab and verify codec info | Navigate to the TC_WEBAUDIO_MANUAL_10 tab using the Tab key and press Enter. | The TC_WEBAUDIO_MANUAL_10 HTML page should load correctly, displaying the decoded audio codec information. Expected output: Audio Codec Information: Sample Rate: 44100 Hz, Channels: 1, Duration: 19.219614512471654 seconds|
