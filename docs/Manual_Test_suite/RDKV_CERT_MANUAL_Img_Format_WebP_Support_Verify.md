...
## Objective
To validate that the WebP image format is correctly decoded and rendered on the display by the DUT via the Browser Test application. This test confirms that the WebP image is displayed without artifacts or errors, ensuring that WebP image format support meets certification requirements.
