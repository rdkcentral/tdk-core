...
## TestCase Name
RDKV_CERT_MANUAL_Pwr_Mgmt_Light_Sleep_Command_Wakeup
...
## Objective
To validate that the DUT can be set to LIGHT SLEEP mode and successfully woken up via a remote command issued from the serial console. This test confirms that the RDK UI is operational, internet is accessible, and A/V content playback resumes after wake-up, ensuring command-triggered LIGHT SLEEP wakeup meets certification requirements.
...
| 3 | Set DUT to LIGHT_SLEEP mode via API | Set the DUT to LIGHT_SLEEP mode via the following curl command from the SSH console.<br>Command: `curl --header "Content-Type: application/json" --request POST --data '{"jsonrpc":"2.0","id":"3","method":"org.rdk.System.1.setPowerState","params":{"powerState":"LIGHT_SLEEP","standbyReason":"For Testing"}}' http://127.0.0.1:9998/jsonrpc` | The API should return a successful response confirming the power state transition, and the DUT UI should turn off, indicating the device has entered Light Sleep mode.|
...
| 5 | Wake DUT to ON state via API | Wake up the DUT from LIGHT_SLEEP mode using the following curl command.<br>Command: `curl --header "Content-Type: application/json" --request POST --data '{"jsonrpc":"2.0","id":"3","method":"org.rdk.System.1.setPowerState","params":{"powerState":"ON","standbyReason":"For Testing"}}' http://127.0.0.1:9998/jsonrpc` | The API should return a successful response confirming the power state transition, and the DUT should wake up from Light Sleep mode with the RDK UI turning on.|
