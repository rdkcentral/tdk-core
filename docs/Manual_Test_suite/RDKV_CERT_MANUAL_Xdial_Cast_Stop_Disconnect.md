...
## Objective
To validate that an active XDial casting session can be stopped using the Disconnect option from the smartphone, and that video playback subsequently resumes on the smartphone only. This test confirms that the DUT returns to the home screen upon disconnection and video continues on the smartphone, ensuring XDial session disconnection behavior meets certification requirements.
...
| 7 | Tap Cast icon on smartphone for options | Tap the Cast icon again on the smartphone. | A popup should appear with the DUT name, volume control, Voice Search option, Remote option, and Close/Disconnect options.|
| 8 | Disconnect casting session | Press the Disconnect option in the popup. | A "Device Disconnected" notification should be displayed on the YouTube interface on the TV. The YouTube Home screen on the TV should exit to a screen listing available users.|
| 9 | Verify video plays on smartphone only | Play a video from the YouTube application on the smartphone. | The video should play on the smartphone only. The TV should no longer receive the cast content.|
| 10 | Close YouTube app via Back key | Close/exit the YouTube application by pressing the Back key on the remote. | The YouTube application should terminate gracefully. The RDK UI Home Page should be displayed on the TV.|
