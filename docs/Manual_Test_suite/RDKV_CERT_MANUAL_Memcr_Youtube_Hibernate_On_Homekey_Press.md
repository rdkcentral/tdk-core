## TestCase ID
RDKV_MANUAL_MEMCR_02
## TestCase Name
RDKV_CERT_MANUAL_Memcr_Youtube_Hibernate_On_Homekey_Press
...
|#|Step Name | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Launch YouTube app | Select the YouTube App tile from the My Apps / Recommended Apps section and press Enter/OK on the remote. | The YouTube App should launch successfully (cold launch or hot launch based on the app's previous state).|
...
| 3 | Query app state via API | Execute the following curl command in the DUT serial console or SSH terminal to query the current app state.<br>Command: `curl -H 'content-type:text/plain;' --data-binary '' http://127.0.0.1:9998/jsonrpc` | The YouTube App state returned by getLoadedApps should be APP_STATE_ACTIVE.|
| 4 | Press Home key on remote | Press the Home key on the remote. | The RDK UI Home screen should be displayed.|
| 5 | Query app state after Home key press | Execute the following curl command in the DUT serial console or SSH terminal to query the current app state.<br>Command: `curl -H 'content-type:text/plain;' --data-binary '' http://127.0.0.1:9998/jsonrpc` | The YouTube App state returned by getLoadedApps should be APP_STATE_HIBERNATED.|
