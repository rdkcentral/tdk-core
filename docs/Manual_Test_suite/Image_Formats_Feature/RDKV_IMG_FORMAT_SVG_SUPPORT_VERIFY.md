...
## TestCase Name
RDKV_CERT_MANUAL_IMG_FORMAT_SVG_SUPPORT_VERIFY
...
## Objective
To validate that the SVG image format is correctly rendered and displayed by the DUT via the Browser Test application.
...
| 2 |  Navigate to SVG image tile  | Use the Tab key on the keyboard to navigate to the SVG IMAGE tile under the Image Formats section and press Enter to load the image file. | The SVG IMAGE tile should load correctly. The .svg image file should be displayed on the RDK UI without errors. |
