## TestCase ID
RDKV_MANUAL_MEMCR_06
## TestCase Name
RDKV_CERT_MANUAL_Memcr_Youtubetv_Mem_Reduction_On_Homekey_Press
...
|#|Step Name | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Launch YouTube TV app | Select the YouTube TV App tile from the My Apps / Recommended Apps section and press Enter/OK on the remote. | The YouTube TV App should launch successfully. |
| 2 | Identify Cobalt process IDs | Execute the following command in the DUT serial console or SSH terminal to identify the Cobalt process IDs.<br>Command: `ps -aux \| grep Cobalt` | The output should list the running Cobalt process entries including both the DobbyInit PID and the WPEProcess (Cobalt) application PID. |
| 3 | Navigate to Cobalt process /proc directory | From the output of Step 2, identify the WPEProcess (Cobalt) application PID (not the DobbyInit PID) and navigate to its /proc directory.<br>Command: `cd /proc/<pid>` | The current directory should change to /proc/<pid>. |
| 4 | Read Cobalt RSS memory before hibernation | Execute the following command to read the current RSS memory usage of the Cobalt process.<br>Command: `grep -i 'VmRSS' status \| awk '{print $2$3}'` | The command should output the current RSS memory consumption of the Cobalt process. Record this value. |
| 5 | Press Home key and wait | Press the Home key on the remote and wait a few seconds. | The RDK UI Home screen should be displayed. |
| 6 | Read Cobalt RSS memory after hibernation | Execute the following command again to read the RSS memory usage after hibernation.<br>Command: `grep -i 'VmRSS' status \| awk '{print $2$3}'` | The RSS memory consumption should be significantly lower than the value recorded in Step 4, confirming that Memcr hibernation has reduced memory usage. |
