...
## Objective
To validate that the speech synthesis capability of the WebAudio API can synthesize audio output and enumerate the supported voice options on the DUT. This test confirms that the available speech synthesis voices are listed and speech output is generated correctly, ensuring WebAudio speech synthesis voice enumeration meets certification requirements.
...
| 2 | Navigate to TC_WEBAUDIO_MANUAL_02 tab | Navigate to the TC_WEBAUDIO_MANUAL_02 tab using the Tab key and press Enter. | The TC_WEBAUDIO_MANUAL_02 HTML page should load correctly.|
| 3 | Select voice type and click Speak button | Navigate using Tab keys, select a voice type, and click the Speak button. | Audio speech output should be heard for the text displayed in the text box using the currently selected voice.|
| 4 | Verify supported audio voices listed on screen | Verify that the audio voices used for speech synthesis are listed on the screen. | The list of supported audio voices used for speech synthesis should be displayed on the screen.|
| 5 | Close WebAudio test app | Close the WebAudio Test App by pressing the Back key on the remote. If Back key does not close it, terminate using the terminateApp API.<br>Command: `curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 15, "method": "org.rdk.AppManager.terminateApp", "params": {"appId": "<App_Id>"}}' http://127.0.0.1:9998/jsonrpc` | The WebAudio Test App should terminate gracefully and the RDK UI Home screen should be visible on the display.|
