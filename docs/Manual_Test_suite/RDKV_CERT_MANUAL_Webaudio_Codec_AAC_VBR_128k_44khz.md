...
## TestCase Name
RDKV_CERT_MANUAL_Webaudio_Codec_AAC_VBR_128k_44khz
...
## Objective
To validate that the DUT can successfully decode an AAC audio file encoded at VBR 128 kbps and 44 kHz using the WebAudio API. This test confirms that the codec is decoded correctly and the expected metadata is reported accurately, ensuring AAC VBR codec support meets certification requirements.
...
| 2 | Navigate to TC_WEBAUDIO_MANUAL_09 tab and verify codec info | Navigate to the TC_WEBAUDIO_MANUAL_09 tab using the Tab key and press Enter. | The TC_WEBAUDIO_MANUAL_09 HTML page should load correctly, displaying the decoded audio codec information. Expected output: Audio Codec Information: Sample Rate: 44100 Hz, Channels: 1, Duration: 1.0202267573696144 seconds|
