## TestCase ID
RDKV_MANUAL_RDKUI_25
## TestCase Name
RDKV_CERT_MANUAL_RDK_UI_App_Launch_Fail_Cancel_Btn
...
## Objective
To validate that when an app launch fails due to a network issue, the RDK UI displays an appropriate error overlay that can be dismissed using the Cancel button on the overlay. This test confirms that the error overlay exits cleanly and returns the user to the previous screen, ensuring app launch failure error handling via the Cancel button meets certification requirements.
...
| 3 | Dismiss error overlay via Cancel button | Exit the error overlay using cancel button on top left corner of the overlay | The overlay should exit and return the user to previous screen|
...
| 5 | Dismiss error overlay via Cancel button | Exit the error overlay using cancel button on top left corner of the overlay | The overlay should exit and return the user to previous screen|
...
| 7 | Dismiss error overlay via Cancel button | Exit the error overlay using cancel button on top left corner of the overlay | The overlay should exit|
...
| 10 | Dismiss error overlay via Cancel button | Exit the error overlay using cancel button on top left corner of the overlay | The overlay should exit and return the user to previous screen|
