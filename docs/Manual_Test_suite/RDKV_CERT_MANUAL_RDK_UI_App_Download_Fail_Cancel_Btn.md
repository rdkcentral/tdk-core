## TestCase ID
RDKV_MANUAL_RDKUI_29
## TestCase Name
RDKV_CERT_MANUAL_RDK_UI_App_Download_Fail_Cancel_Btn
...
## Objective
To validate that when an app download fails due to a network issue, the RDK UI displays an appropriate error overlay that can be dismissed using the Cancel button on the overlay. This test confirms that the error overlay exits cleanly and returns the user to the previous screen, ensuring download failure error handling via the Cancel button meets certification requirements.
...
| 3 | Exit error overlay via cancel button | Exit the error overlay by pressing the cancel button on top left of the overlay | The overlay should exit and return the user to previous screen|
...
| 8 | Exit error overlay via cancel button | Exit the error overlay by pressing the cancel button on top left of the overlay | The overlay should exit and return the user to previous screen|
