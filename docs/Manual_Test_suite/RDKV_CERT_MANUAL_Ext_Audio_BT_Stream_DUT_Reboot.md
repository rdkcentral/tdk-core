...
| 16 | Reboot DUT during active audio stream | Reboot the DUT while the external Bluetooth audio stream is active. | Audio streaming should stop and no audio should be heard from the external Bluetooth device.|
| 17 | Play YouTube video and verify auto audio stream | When the DUT comes back up, play a YouTube video and observe the audio streaming behavior. | Audio stream should start automatically and audio should be heard from the external Bluetooth device.|
