...
## Objective
To validate that the device information of a connected external Bluetooth device can be accurately retrieved from the DUT. This test confirms that the response includes the correct MAC address, manufacturer ID, device type, and device ID, ensuring that Bluetooth device information retrieval meets certification requirements.
...
| 12 | Retrieve BT device information | Execute the following curl command to retrieve the device information of the connected external Bluetooth device.<br>`curl --header "Content-Type: application/json" --request POST --data ' {"jsonrpc":"2.0", "id":"3", "method":"org.rdk.Bluetooth.getDeviceInfo", "params": {"deviceID": "<Ext_BT_deviceID>"}}' http://127.0.0.1:9998/jsonrpc` | The connected external Bluetooth device information should be listed in the response.|
| 13 | Validate MAC address in device info | Validate that the curl response contains the MAC address of the connected Bluetooth device.<br>`curl --header "Content-Type: application/json" --request POST --data ' {"jsonrpc":"2.0", "id":"3", "method":"org.rdk.Bluetooth.getDeviceInfo", "params": {"deviceID": "<Ext_BT_deviceID>"}}' http://127.0.0.1:9998/jsonrpc` | The `getDeviceInfo` API should return a successful response, and the MAC address of the connected Bluetooth device should be present in the device information.|
| 14 | Validate manufacturer ID in device info | Validate that the curl response contains the manufacturer ID of the connected Bluetooth device.<br>`curl --header "Content-Type: application/json" --request POST --data ' {"jsonrpc":"2.0", "id":"3", "method":"org.rdk.Bluetooth.getDeviceInfo", "params": {"deviceID": "<Ext_BT_deviceID>"}}' http://127.0.0.1:9998/jsonrpc` | The `getDeviceInfo` API should return a successful response, and the manufacturer ID of the connected Bluetooth device should be present in the device information.|
| 15 | Validate device type and ID in device info | Validate that the curl response contains the device type and device ID of the connected Bluetooth device.<br>`curl --header "Content-Type: application/json" --request POST --data ' {"jsonrpc":"2.0", "id":"3", "method":"org.rdk.Bluetooth.getDeviceInfo", "params": {"deviceID": "<Ext_BT_deviceID>"}}' http://127.0.0.1:9998/jsonrpc` | The `getDeviceInfo` API should return a successful response, and the device type and device ID of the connected Bluetooth device should be present in the device information.|
