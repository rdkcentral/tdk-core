## TestCase ID
RDKV_MANUAL_MEMCR_03
## TestCase Name
RDKV_CERT_MANUAL_Memcr_Youtube_Mem_Reduction_On_Homekey_Press
...
| 2 | Select 4K content and initiate YouTube playback | Select any 4K video content and initiate playback on YouTube. | A/V playback on YouTube should start successfully.|
| 3 | Identify Cobalt process IDs | Execute the following command in the DUT serial console or SSH terminal to identify the Cobalt process IDs.<br>Command: `ps -aux \| grep Cobalt` | The output should list the running Cobalt process entries including both the DobbyInit PID and the WPEProcess (Cobalt) application PID.|
| 4 | Navigate to Cobalt process /proc directory | From the output of Step 3, identify the WPEProcess (Cobalt) application PID (not the DobbyInit PID) and navigate to its /proc directory.<br>Command: `cd /proc/<pid>` | The current directory should change to /proc/<pid>.|
| 5 | Read Cobalt RSS memory before hibernation | Execute the following command to read the current RSS memory usage of the Cobalt process.<br>Command: `grep -i 'VmRSS' status \| awk '{print $2$3}'` | The command should output the current RSS memory consumption of the Cobalt process (e.g., 216624kB). Record this value.|
