...
## Objective
To validate that FM-modulated synthesized audio can be generated and played back using the WebAudio API on the DUT. This test confirms that the FM-generated sound plays back correctly without errors, ensuring WebAudio FM synthesis capability meets certification requirements.
...
| 2 | Navigate to TC_WEBAUDIO_MANUAL_07 tab | Navigate to the TC_WEBAUDIO_MANUAL_07 tab using the Tab key and press Enter. | The TC_WEBAUDIO_MANUAL_07 HTML page should load correctly, displaying a "Start Synthesis" button.|
| 3 | Click Start Synthesis button and verify audio | Navigate using Tab keys and click the Start Synthesis button. | A buzzer or warning sound should be heard when the Start Synthesis button is pressed, confirming FM-modulated audio generation via the WebAudio API.|
