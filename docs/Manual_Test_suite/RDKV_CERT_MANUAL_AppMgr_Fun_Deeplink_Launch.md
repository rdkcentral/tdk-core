...
## Objective
To validate the deeplink launch functionality on deeplink-supported installed applications via the AppManager API. This test exercises the `org.rdk.AppManager` plugin (including APIs such as `clearAppData`, `launchApp`, and `getAppStatus`) and the RDK UI Home screen navigation to drive the application lifecycle. The test confirms that expected response should be same as Step 1 - 5 on all supported Apps.
