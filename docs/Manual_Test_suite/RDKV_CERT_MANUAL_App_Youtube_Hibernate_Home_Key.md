...
| 3 | Note playback position and press Home key | Note the current video time position, then press the Home key on the remote. | The YouTube application should move to the background (hibernate) and the RDK UI Home screen should launch.|
| 4 | Verify YouTube is hibernated and home screen visible | Validate that the YouTube application is hibernated and the RDK UI Home screen is displayed. | The YouTube application should be in a hibernated state and the RDK UI Home screen should be visible on the display.|
| 5 | Launch YouTube app | Select the YouTube application tile from the My Apps/Recommended Apps section/row of the RDK UI Home screen again and press Enter/OK. | The YouTube application should launch successfully via hot launch from the last hibernated state.|
| 6 | Verify YouTube resumes from hibernated position | Validate that the YouTube AV playback resumes from the time position noted in Step 3. | The YouTube AV playback should resume from the time position noted in Step 3, confirming that hibernation is functioning as expected.|
