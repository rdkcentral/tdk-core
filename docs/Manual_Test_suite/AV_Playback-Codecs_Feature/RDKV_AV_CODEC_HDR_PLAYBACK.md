...
RDKV_MANUAL_AV_13
## TestCase Name
RDKV_CERT_MANUAL_AV_CODEC_HDR_PLAYBACK
...
## Objective
To validate that HDR video stream playback is functional on the DUT via GStreamer.
...
|#|Step Name | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Execute GStreamer command for HDR | Execute the GStreamer command to play the HDR stream.<br>Command: `gst-launch-1.0 playbin uri=<HDR_URL>` | The HDR video stream should play with proper video quality without any artifacts or errors. |
| 2 | Wait for playback to end | Wait for the video playback to end gracefully. | The video playback should end gracefully without any errors or crashes. |
