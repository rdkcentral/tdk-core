...
## Objective
To validate that the DUT can successfully switch its Wi-Fi connection to a different SSID on a different network while already connected to a Wi-Fi network.
...
| 6 |  Select SSID from different network  | Select a different SSID from a different network. | The password entry screen should launch with an on-screen keyboard and Connect/Cancel buttons. |
...
| 9 |  Verify updated network info  | Select "Network Info" and press OK. | The Network Info screen should load displaying the newly connected SSID details and the new wlan0 IP address with all fields correctly populated. |
