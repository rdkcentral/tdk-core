...
| 2 | Reboot external BT device during stream | Reboot the external Bluetooth device while the audio stream is active. | Audio streaming should stop and no audio should be heard from the external Bluetooth device|
| 3 | Verify auto-reconnection after device reboot | Once the external Bluetooth device comes back online after reboot, observe the audio streaming behavior. | Audio stream should start automatically and audio should be heard from the External BT device.|
