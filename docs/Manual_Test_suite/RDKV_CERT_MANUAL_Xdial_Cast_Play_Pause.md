## TestCase ID
RDKV_MANUAL_XDIAL_07
## TestCase Name
RDKV_CERT_MANUAL_Xdial_Cast_Play_Pause
...
## Objective
To validate that video Play and Pause controls on the smartphone function correctly on the DUT during an active YouTube XDial casting session. This test confirms that play and pause commands from the smartphone are accurately reflected in the video playback state on the DUT, ensuring play/pause control functionality during XDial casting meets certification requirements.
...
| 6 | Select video and initiate playback | Select a video using the navigation buttons and press the OK (Play/Pause) button on the remote control screen to initiate playback. | The selected video should start playing on the TV.|
