...
## Objective
To validate that the JPEG image format is correctly decoded and rendered on the display by the DUT via the Browser Test application. This test confirms that the JPEG image is displayed without artifacts or errors, ensuring that JPEG image format support meets certification requirements.
...
| 2 |  Navigate to JPEG image tile  | Use the Tab key on the keyboard to navigate to the JPEG IMAGE tile under the Image Formats section and press Enter to load the image file. | The JPEG IMAGE tile should load correctly. The .jpeg image file should be displayed on the RDK UI without errors.|
