...
## Objective
To validate that the DUT supports the HDCP protocol and correctly reports its HDCP support status via the HDCP Profile API.
...
| 5 | Query HDCP support status via API | Immediately after initiating playback, execute the following curl command to query the HDCP support status.<br>Command: `curl --data-binary '{"jsonrpc":"2.0","id":42,"method":"org.rdk.HdcpProfile.getSettopHDCPSupport"}' -H 'content-type:text/plain;' http://127.0.0.1:9998/jsonrpc` | The response should confirm HDCP support. Expected response: {"jsonrpc":"2.0","id":42,"result":{"supportedHDCPVersion":"2.2","isHDCPSupported":true,"success":true}}. The field "isHDCPSupported":true shall be present in the response. |
