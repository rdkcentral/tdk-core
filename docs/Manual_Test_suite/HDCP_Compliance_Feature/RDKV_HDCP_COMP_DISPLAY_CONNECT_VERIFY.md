...
| 5 | Query HDCP display connection status via API | Immediately after initiating playback, execute the following curl command to query the HDCP status.<br>Command: `curl --data-binary '{"jsonrpc":"2.0", "id":3, "method":"org.rdk.HdcpProfile.getHDCPStatus", "params":{}}' -H 'content-type:text/plain;' http://127.0.0.1:9998/jsonrpc` | The response should confirm the HDCP-compliant display is connected. Expected response: {"jsonrpc":"2.0","id":3,"result":{"HDCPStatus":{"isConnected":true,"isHDCPCompliant":true,"isHDCPEnabled":true,"hdcpReason":2,"supportedHDCPVersion":"2.2","receiverHDCPVersion":"2.2","currentHDCPVersion":"2.2"},"success":true}}. The field "isHDCPCompliant":true shall be present in the response. |
| 6 | Confirm uninterrupted A/V playback | Confirm that A/V playback continues without interruption on the display. | The A/V stream should continue playing with proper audio and video output. |
| 7 | Exit the application | Exit the application by pressing the Back key on the remote. | The application should terminate gracefully and the RDK UI Home screen should be visible on the display. |
