...
## Objective
To validate that MPEG-DASH stream playback is functional on the DUT. This test exercises the RDK media pipeline, AV decoder plugins, and the video player application to drive codec-specific playback scenarios. The test confirms that the MPEG-DASH asset should play with proper audio and video without any artifacts or errors.
...
| 2 | Navigate to MPEG-DASH section and initiate playback | Navigate to the section in the MVT App where the MPEG-DASH asset is present and initiate playback of the asset. | The MPEG-DASH asset should play with proper audio and video without any artifacts or errors.|
