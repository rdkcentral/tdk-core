...
RDKV_MANUAL_AV_02
## TestCase Name
RDKV_CERT_MANUAL_AV_Codec_H265_HEVC_Playback
...
## Objective
To validate that H.265/HEVC video codec playback is functional on the DUT. This test confirms that the H.265/HEVC asset plays with proper audio and video output without artifacts or errors, ensuring H.265/HEVC codec support meets certification requirements.
...
| 9 | Configure H.265/HEVC stream URL | Configure the stream variable `video_src_url_short_duration_hevc` in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_HEVC_AAC_30Sec/master.mpd"` to point to the H.265/HEVC DASH test stream. | The `video_src_url_short_duration_hevc` variable should resolve to a valid, accessible stream location. |
...
| 2 | Execute H.265/HEVC test script | Select the script `RDKV_CERT_MVS_Video_Play_Full_HEVC` and click on 'Execute Now' button. | The script should start execution. |
| 3 | Verify video playback on TV | Verify if the video started playing on the TV and monitor it. | The H265/HEVC asset should play with proper audio and video without any artifacts or errors. |
