...
| 3 | Query app state via API | Execute the following curl command in the DUT serial console or SSH terminal to query the current app state.<br>Command: `curl -H 'content-type:text/plain;' --data-binary '' http://127.0.0.1:9998/jsonrpc` | The Amazon Prime Video App state returned by getLoadedApps should be APP_STATE_ACTIVE.|
