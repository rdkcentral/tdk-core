...
| 2 | Navigate to TC_WEBAUDIO_MANUAL_03 tab | Navigate to the TC_WEBAUDIO_MANUAL_03 tab using the Tab key and press Enter. | The TC_WEBAUDIO_MANUAL_03 HTML page should load correctly, displaying a text box and a "Speak with random voices" button. |
| 3 | Click speak with random voices button | Navigate using Tab keys and click the "Speak with random voices" button. | Audio speech output should be heard for the displayed text, played sequentially using three different language voices one after another. |
| 4 | Verify different audio voices listed on screen | Verify that the different audio voices used for speech synthesis are listed on the screen. | The list of different audio voices used for speech synthesis should be displayed on the screen. |
