...
## Objective
To validate that the DUT handles Miracast session errors and disconnections gracefully, including Wi-Fi disruption and re-connection after casting termination. This test confirms that the Miracast connection is re-established successfully and screen mirroring resumes, ensuring Miracast error recovery behavior meets certification requirements.
...
| 6 | Disconnect Wi-Fi on smartphone | Disconnect the Wi-Fi SSID on the smartphone. | The Miracast session should not be terminated.|
| 7 | Reconnect Wi-Fi on smartphone | Reconnect to the Wi-Fi SSID on the smartphone. | The Miracast session should not be terminated.|
| 8 | Stop casting session from smartphone | Stop the casting session from the smartphone. | The Miracast connection should terminate and the DUT should return to its default state (RDK UI Home screen).|
| 9 | Repeat steps with other devices/conditions | Re-initiate Miracast from the same smartphone (repeat Steps 2–5). | The Miracast connection should be re-established successfully and screen mirroring should resume.|
