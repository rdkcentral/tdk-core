...
RDKV_MANUAL_AV_04
## TestCase Name
RDKV_CERT_MANUAL_AV_Codec_AAC_Playback
...
## Objective
To validate that AAC audio codec stream playback is functional on the DUT. This test confirms that the AAC audio asset plays with proper audio output without distortion or errors, ensuring AAC audio codec support meets certification requirements.
...
| 10 | Configure AAC stream URL | Configure the stream variable `video_src_url_aac` in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` to point to the AAC DASH test stream. | The `video_src_url_aac` variable should resolve to a valid, accessible stream location. |
...
| 2 | Execute AAC test script | Select the script `RDKV_CERT_MVS_Video_Play_AAC` and click on 'Execute Now' button. | The script should start execution. |
| 3 | Verify audio playback on TV | Verify if the audio started playing on the TV and monitor it. | The AAC asset should play with proper audio without any distortion or errors. |
