...
| 3 | Construct Pipeline and Initiate Playback | Create `playbin` element with HEVC stream URI, set `westerossink` as video sink, trigger `NULL→READY→PAUSED→PLAYING` state transition, verify `first-video-frame-callback` signal | Verify `playbin` reaches `GST_STATE_PLAYING` with first frame rendered from HEVC stream, no `GST_MESSAGE_ERROR` |
