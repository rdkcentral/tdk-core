...
| 6 | Validate Forward Seek Accuracy on Streaming | Monitor bus for `GST_MESSAGE_ASYNC_DONE`. Poll `gst_element_query_position()` every 100ms verifying position within Â±1000ms of target. Handle segment boundaries in playlist | Position matches target within Â±1000ms, seek confirmed on streaming content |
