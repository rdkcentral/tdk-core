...
Test to verify backward seek operations on H.264 HLS stream via `gst_element_seek()` with GST_SEEK_FLAG_FLUSH flag repeated in stress cycles. The test executes 3+ seek operations at different positions (incremented in fixed steps) to validate seek accuracy, position recovery, and playback continuity under stress conditions. Verify playback position matches seek target within Â±1 second tolerance after each seek operation, validate frame rendering resumes correctly via westerossinkâ†’stats (rendered_frames increment), and detect no GStreamer errors during all seek cycles.
