...
## TestCase Name
NPVS_Appsrc_Audio_Underflow_Playback_MKV
...
## Objective
Test to verify if buffer underflow signal is obtained upon audio/video underrun when using Appsrc and playback is smooth after buffer is filled again. Validate MKV stream playback through custom Appsrc pipeline with underflow detection and recovery capability. Specifically test with BYTES_THRESHOLD = 490077 bytes configuration to trigger controlled buffer starvation and verify `buffer-underflow-callback` signal detection and pipeline recovery via buffer refill operations.
...
| 2 | Media Stream Provisioning | MKV stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding. Stream file path configured as `test_streams_base_path + "TDK_Asset_Waterfall_Audio_MKV.mkv"` in MediaValidationVariables.py | Verify MKV stream file is accessible and readable from filesystem |
| 3 | Stream Variable Configuration | Stream variable `audio_src_url_mkv` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Waterfall_Audio_MKV.mkv` (Appsrc requires local file access, not DASH/HLS streams) | Verify `audio_src_url_mkv` resolves to valid, accessible MKV file location |
| 4 | Appsrc Buffer Threshold Configuration | `BYTES_THRESHOLD` must be configured to 490077 bytes (actual threshold value from test script) to trigger mkv underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered | Verify BYTES_THRESHOLD = 490077 bytes is set in Appsrc configuration or test parameters |
...
| 3 | Register Appsrc Callbacks and Setup Pipeline | Register `source-setup` signal via `g_signal_connect()`. Set Appsrc `size` property via `g_object_set()`. Register sink element signals `buffer-underflow-callback`, `first-audio-frame-callback` or `first-video-frame-callback`, and `pts-error-callback` via `g_signal_connect()`. Configure `video-sink` property to `westerossink` via `g_object_set()` | Verify `source-setup`, `buffer-underflow-callback`, frame callback, and `pts-error-callback` signals are registered, Appsrc `size` property is set, and `video-sink` property is configured |
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to `GST_STATE_PAUSED` via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 490077 bytes` is reached. Transition to `GST_STATE_PLAYING` via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 490077 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 980154 bytes`. After all media data is transferred to Appsrc, emit `end-of-stream` action via `g_signal_emit_by_name()` | Verify buffers accumulated to 980154 bytes threshold and `end-of-stream` action is emitted |
