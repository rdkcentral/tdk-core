...
**TestCase Name**
NPVS_PlayPause_MultipleTimes_HEVC_MKV
...
| 2 | Media Stream Provisioning | MKV H.265 AAC stream must be accessible via HTTP (`souphttpsrc`) or local file system (`filesrc`) with matroskademux element. Stream path: `TDK_Asset_Sunrise_HEVC_MKV.mkv` | Verify MKV stream is accessible and contains valid H.265 and AAC representations |
...
| 2 | Configure Stress Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` (timeout for each play/pause operation, default: 10 seconds) and `NATIVE_PLAYBACK_STRESS_REPEAT_COUNT` (number of play-pause cycles, default: 3); Retrieve stream URL from `video_src_url_hevc_mkv` variable; Construct operations string with `play:<timeout>,pause:<timeout>` repeated 3+ times; Execute `mediapipelinetests test_trickplay <URL> checkavstatus=<yes/no> operations=<operations_string>` to prepare stress test | Verify mediapipelinetests initializes with MKV stream and stress operation parameters (operations string, repeat count) correctly configured |
| 3 | Construct H.265 MKV Pipeline and Initiate Playing State | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `video_src_url_hevc_mkv` via `g_object_set()`; Set `westerossink` as video sink via `g_object_set()`; Configure `autoaudiosink` for AAC audio via `g_object_set()`; Trigger state transition to `GST_STATE_PLAYING` via `gst_element_set_state()` | Verify playbin reaches `GST_STATE_PLAYING`, MKV matroskademux active and parsing stream, first frame rendered via first-video-frame-callback signal, no `GST_MESSAGE_ERROR` on bus |
