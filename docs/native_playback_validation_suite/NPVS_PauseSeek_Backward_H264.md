...
| 2 | Configure Playback Pipeline | Create `playbin` element and set `uri` property to H264 DASH stream (`DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd` from MediaValidationVariables.py). Configure `westerossink` as video sink | Verify playbin created successfully, URI property set to H264 DASH manifest, westerossink linked to video output |
