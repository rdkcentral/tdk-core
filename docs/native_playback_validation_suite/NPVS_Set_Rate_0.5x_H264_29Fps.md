...
## Objective
Validate playback rate control at 0.5x speed (half normal) with H.264 29fps baseline. The test sets rate to 0.5 and verifies position advances at exactly half the normal speed. Confirm audio/video synchronization and frame rendering consistency at 29fps framerate during rate changes.
...
| 2 | Media Stream Provisioning | H.264 encoded MP4 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_29fps.mp4"` in MediaValidationVariables.py (29fps H.264 baseline stream for rate control validation) | Verify H.264 MP4 stream file is accessible, readable, and contains minimum 60 seconds of content for rate testing |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_mp4_29fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_29fps.mp4` (stream must have H.264 codec at 29fps framerate) | Verify `video_src_url_mp4_29fps` resolves to valid, accessible H.264 MP4 file location with 29fps encoding |
