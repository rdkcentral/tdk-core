...
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to  via . Emit `push-buffer` action on Appsrc via  until `BYTES_THRESHOLD = 1290444 bytes` is reached.  via . Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 1290444 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 2580888 bytes`. After all media data is transferred to Appsrc, emit `end-of-stream` action via  | Verify buffers accumulated to 2580888 bytes threshold and `end-of-stream` action is emitted |
