...
## Objective
Validate VP9 video codec with OPUS audio  using DASH streaming.  Query video dimensions and audio stream count and 
...
| 3 | Construct Pipeline and Initiate Playback | Create `playbin` element via  with video_src_url_vp9 URI; Configure `uri` property to video_src_url_vp9 manifest/file via ; Set `video-sink` property to `westerossink` via ; Configure `autoaudiosink` for OPUS audio; Trigger state  to  via ; Monitor for `first-video-frame-callback` signal | Verify playbin reaches  with first frame decoded, DASH manifest/file parsed successfully, no  |
| 4 | Query Video and Audio Stream Properties | Query video dimensions via  and `westerossinkâ†’video-width`; Query audio stream count via  to verify OPUS stream presence; Log extracted properties | Verify video dimensions valid; Verify n-audio >= 1 confirming OPUS stream present |
