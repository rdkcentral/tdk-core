...
| 3 | Construct AV1 MP4 Pipeline | Create `playbin` element via ; Configure `uri` to `video_src_url_av1_23fps` via ; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio;  | Verify playbin reaches , qtdemux active, first frame rendered |
