...
| 2 | Configure Test Parameters and Load Multi-Codec Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and `NATIVE_PLAYBACK_CHECK_AV_STATUS` from device. Build `mediapipelinetests` command with test_name="test_playback_audio_change_with_pause", stream_url=video_src_url_aac_eac3_pause, parameters: checkavstatus=yes/no, timeout=N | Verify device config retrieved successfully, command constructed with correct parameters |
