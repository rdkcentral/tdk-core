...
## Objective
Validate play-pause state transitions during HLS stream playback. The test verifies pausing halts rendering and position, resuming continues from pause point without gaps, and frame rendering reflects state changes. Confirm playback position advances at consistent rate during play, remains stationary during pause, and video frame statistics correlate with play-pause state transitions throughout all cycles.
...
| 3 | Construct H.264 HLS Pipeline | Create `playbin` element via ; Configure `uri` property to `video_src_url_hls` via ; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio; Trigger state  | Verify playbin reaches , HLS hlsdemux active, first frame rendered |
