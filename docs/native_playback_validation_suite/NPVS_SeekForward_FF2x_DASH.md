...
| 5 | Execute Forward Seek on DASH Stream | Query current position via . Calculate forward seek target. Invoke  handling manifest updates | Seek completes, DASH demux updates manifest, playback continues |
| 6 | Validate Forward Seek Accuracy on Streaming | Monitor bus for .  Handle segment boundaries in manifest | Position matches target within ±1000ms, seek confirmed on DASH stream |
