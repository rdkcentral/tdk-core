...
## Objective
Validate reverse playback at 4x speed (playback rate = -4) on H.264 video stream via dashdemux demuxer. Test executes seek operation to stream position (2*NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT + 30 seconds) via `gst_element_seek()` with GST_SEEK_FLAG_FLUSH, then applies negative playback rate via `gst_element_seek(playbin, -4, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET, currentPosition)`. Validate position decreases at expected rate during 4x reverse playback for NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT seconds with position tolerance ±250ms, no GST_MESSAGE_ERROR, and continuous rendered frames
...
| 3 | Stream Variable Configuration | Stream variable `video_src_url_dash_h264` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"`. dashdemux capable of parsing stream and extracting audio/video representations | Verify `video_src_url_dash_h264` resolves to valid, accessible H.264 stream location |
