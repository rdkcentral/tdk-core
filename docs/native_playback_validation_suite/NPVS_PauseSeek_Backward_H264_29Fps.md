...
| 2 | Configure Playback Pipeline | Create `playbin` element via `gst_element_factory_make("playbin", NULL)`. Set playbin `uri` property via `g_object_set(playbin, "uri", "DASH_H264_29fps/master.mpd", NULL)` from MediaValidationVariables.py. Configure `westerossink` via `g_object_set(playbin, "video-sink", westerosSink, NULL)` | Verify playbin created successfully, URI property set correctly to H264 29.97fps DASH manifest, westerossink linked to video output |
...
| 7 | Validate Frame Rendering Continuity | Query `westerossinkâ†’stats` structure via `g_object_get(westerosSink, "stats", &structure, NULL)`. Extract `rendered_frames` and `dropped_frames` via `gst_structure_get_uint64()`. Verify frame increments are consistent for 29.97fps (~33ms per frame) and dropped frames remain below acceptable threshold | Verify no frame drops at seek boundary, frame rendering continues smoothly post-seek at 29.97fps cadence |
