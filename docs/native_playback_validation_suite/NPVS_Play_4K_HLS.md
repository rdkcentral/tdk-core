...
## Objective
Validate 4K (3840Ã—2160) resolution playback via `test_generic_playback` function with H.264 video codec and AAC audio. Initialize playbin with hlsdemux demuxing, query video sink properties via `westerossinkâ†’video-height` and `westerossinkâ†’video-width`. Verify resolution matches expected 3840Ã—2160 (Â±5 pixel tolerance). Execute 10-second playback monitoring position via `gst_element_query_position()`, validate frame rendering statistics via `westerossinkâ†’stats`. Confirm clean pipeline state transitions and error-free playback.
...
| 3 | Construct H.264 Pipeline with Resolution Stream | Create `playbin` element; Configure `uri` to video_src_url_hls (HLS); Set `westerossink` as video sink; Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, HLS parsed successfully |
