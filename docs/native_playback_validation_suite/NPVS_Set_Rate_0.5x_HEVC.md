...
## Objective
Test to verify playback rate control at 0.5x speed using GStreamer `gst_element_seek()` function with HEVC H.265 codec. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic `gst_element_query_position()` polling with position increment expected at Â±25% tolerance around 0.05 seconds per 100ms interval. Verify audio/video remain synchronized during HEVC rate change and frame rendering continues without discontinuities using `westerossink` frame statistics validation.
...
| 2 | Media Stream Provisioning | HEVC H.265 encoded MP4 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_30fps_v2.mp4"` in MediaValidationVariables.py (30fps HEVC baseline stream for rate control validation) | Verify HEVC MP4 stream file is accessible, readable, and contains minimum 60 seconds of content for rate testing |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hevc_30fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_HEVC_30fps_v2.mp4` (stream must have HEVC H.265 codec at 30fps framerate) | Verify `video_src_url_hevc_30fps` resolves to valid, accessible HEVC MP4 file location with 30fps encoding |
