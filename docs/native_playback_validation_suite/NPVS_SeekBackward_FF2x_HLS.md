...
| 5 | Execute Backward Seek on HLS Stream | Query current position via . Calculate backward seek target. Invoke  handling playlist updates | Seek completes, HLS demux updates playlist, playback continues |
| 6 | Validate Backward Seek Accuracy on Streaming | Monitor bus for .  Handle segment boundaries in playlist | Position matches target within ±1000ms, backward seek confirmed on streaming content |
