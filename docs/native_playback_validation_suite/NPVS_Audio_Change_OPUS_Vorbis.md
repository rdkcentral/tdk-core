...
## Objective
Validate dynamic audio codec switching between OPUS and Vorbis during playback. The test switches between different audio codec types in a multi-codec stream while maintaining continuous video playback. Verify audio rendering quality, position synchronization, and codec switching timing accuracy without audio/video interruption.
...
| 4 | Retrieve and Verify Multiple Audio Tracks | Query playbin to determine number of audio streams via . Verify n-audio >= 2 (confirming OPUS and Vorbis tracks present). Query current audio index via  to identify default (OPUS) track | Verify n-audio returns at least 2, current-audio successfully retrieved (typically 0 for first codec) |
