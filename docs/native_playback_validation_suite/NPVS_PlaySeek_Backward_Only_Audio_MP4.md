...
## TestCase Name
NPVS_PlaySeek_Backward_Only_Audio_MP4
...
## Objective
Validate backward seek operation during audio-only MP4 playback. The test verifies accurate positioning to the target seek point within ±1 second tolerance, seamless playback recovery after seek, audio rendering without interruption, and error-free playback.
...
| 2 | Audio Stream Provisioning | AAC MP4 audio stream must be accessible  with local file path or HTTP stream accessible to qtdemux element. Stream file path: `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4_audio_1.mp4"` from MediaValidationVariables.py | Verify AAC MP4 audio stream file is accessible, readable, and contains valid audio codec data with proper headers |
| 3 | Stream Variable Configuration | `audio_src_url_mp4_aac` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4_audio_1.mp4"` with accessible URL or local file path | Verify `audio_src_url_mp4_aac` variable resolves to valid, accessible audio stream location |
...
| 4 | Transition Pipeline to PLAYING State and Verify Audio Activation | Call  to transition pipeline. Wait for state change completion via . Monitor GStreamer bus via  for state transition messages (GST_MESSAGE_STATE_CHANGED). Verify `qtdemux element` active and parsing audio stream from source. Register audio pad and verify caps via  showing valid audio codec, sample rate, and channel count | Verify playbin reaches ; `qtdemux element` active and processing stream; Audio pad connected with valid caps; No GST_MESSAGE_ERROR on bus |
