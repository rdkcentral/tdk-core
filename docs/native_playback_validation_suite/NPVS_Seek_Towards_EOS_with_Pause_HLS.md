...
| 2 | Media Stream Provisioning | HLS stream with H.264 video must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "HLS_H264_AAC/master.m3u8"` in MediaValidationVariables.py. Stream contains H.264 video and AAC audio in HLS format | Verify HLS M3U8 playlist is accessible and hlsdemux can parse playlist |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hls` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "HLS_H264_AAC/master.m3u8"` | Verify stream URL resolves to valid, accessible HLS playlist location |
