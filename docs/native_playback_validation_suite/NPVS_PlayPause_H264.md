...
| 3 | Construct H.264 DASH Pipeline | Create `playbin` element via ; Configure `uri` to `video_src_url_dash_h264` via ; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio;  | Verify playbin reaches , dashdemux active, first frame rendered |
