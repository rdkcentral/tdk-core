...
| 7 | Validate Frame Rendering Continuity | Query `westerossink→stats` structure via . Extract `rendered_frames` and `dropped_frames` via . Verify frame increments are consistent and dropped frames remain below acceptable threshold | Verify no frame drops at seek boundary, frame rendering continues smoothly post-seek from MOV container |
