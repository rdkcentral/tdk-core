...
## Objective
Measure playback latency when transitioning to 2.0x fast-forward playback rate on H.264/AAC DASH stream. Record latency from setting playback rate to 2.0x via `gst_element_seek()` to first frame rendering at new rate, validating that latency remains below configured threshold (default 100 milliseconds). Verify audio/video remain synchronized during rate-changed playback and westerossink renders frames within latency budget.
