...
## Objective
Test validates seeking capability towards end-of-stream (EOS) on MKV Matroska H.264 stream with playback pause intervals by invoking `gst_element_seek()` to reposition playback near stream's final position. The test verifies that the seek target is reached within Â±1 second tolerance, and validates state machine transitions through pause/resume cycles. Validates frame rendering continues with correct PTS monotonicity during pause-resume-seek sequences and matroskademux maintains proper cluster state through pause recovery operations.
