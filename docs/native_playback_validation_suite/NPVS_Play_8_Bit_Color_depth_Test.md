...
**TestCase Name**
NPVS_Play_8_Bit_Color_depth_Test
...
## Objective
Validate 8-bit color depth rendering via `test_color_depth` function with H.264 stream. Query video decoder hardware interface to extract source bit depth information via `/proc/brcm/video_decoder` (Broadcom), `/lib/rdk/get_avstatus.sh` (RTK), or GStreamer pad capabilities. Verify decoder reports 8-bit color depth and execute DASH playback with continuous position monitoring. Confirm color depth metadata preserved throughout 10-second playback window without errors.
...
| 2 | Media Stream Provisioning | 8-bit H.264 DASH stream with AAC audio must be accessible via HTTP(S) (`souphttpsrc`) or local file system (`filesrc`). Stream configured as `video_src_url_aac` in MediaValidationVariables.py pointing to `DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd` (DASH manifest with H.264 8-bit color depth video) | Verify DASH stream is accessible and dashdemux can parse MPD manifest |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_aac` configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` pointing to 8-bit H.264 DASH stream | Verify `video_src_url_aac` resolves to valid, accessible DASH MPD manifest with 8-bit H.264 video |
| 4 | Bit Depth Configuration and Timeout | `bit_depth` parameter must be set to "8" in test execution to specify expected color depth. `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` must be configured (default: 10 seconds) in device config for playback validation window | Verify bit_depth parameter set to 8; Verify timeout configured (minimum 10 seconds) |
...
| 2 | Configure Test with Bit Depth Parameter | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT`, `NATIVE_PLAYBACK_CHECK_AV_STATUS`, and bit depth target (8); Execute `mediapipelinetests test_color_depth <URL> bit_depth=8 timeout=<seconds>` | Verify mediapipelinetests initializes with 8-bit parameter |
| 3 | Construct H.264 DASH Pipeline | Create `playbin` element; Configure `uri` property to video_src_url_aac (DASH manifest/file); Set `westerossink` as video sink; Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, DASH demuxer active |
...
| 5 | Query Video Decoder Bit Depth | Execute platform-specific command: `sh /lib/rdk/get_avstatus.sh | grep LumaBitDepth` to extract color depth; Parse output to extract 8-bit value via `atoi()` conversion | Verify 8-bit value successfully extracted from decoder |
| 6 | Validate Bit Depth Matches Expected | Compare retrieved bit depth against 8 parameter using assertion: `assert_failure(playbin, (expected==8), "Bit depth mismatch")`; Log comparison result | Verify assertion passes; Verify Bit Depth == 8 |
