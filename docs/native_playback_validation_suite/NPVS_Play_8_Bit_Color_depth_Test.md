...
| 3 | Construct H.264 DASH Pipeline | Create `playbin` element; Configure `uri` property to video_src_url_aac (DASH manifest/file); Set `westerossink` as video sink; Trigger state  | Verify playbin reaches , DASH demuxer active |
| 4 | Execute Playback and Position Monitoring | Play stream for configured timeout (10 seconds); Monitor position via  at 100ms intervals | Verify position advances at 1x rate (±1 second tolerance) |
