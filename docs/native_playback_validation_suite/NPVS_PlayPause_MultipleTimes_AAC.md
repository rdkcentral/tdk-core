...
| 3 | Construct H.264 DASH Pipeline and Initiate Playing State | Create `playbin` element via ; Configure `uri` property to `video_src_url_aac` via ; Set `westerossink` as video sink via ; Configure `autoaudiosink` for AAC audio via ; Trigger state  via  | Verify playbin reaches , DASH dashdemux active and parsing stream, first frame rendered via first-video-frame-callback signal, no  on bus |
