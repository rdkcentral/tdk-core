...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `audio_src_url_wav_pcm` variable; Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` to prepare play-pause cycling test | Verify mediapipelinetests initializes with WAV stream and cycle count configured |
| 3 | Construct Audio-only WAV Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `audio_src_url_wav_pcm` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for PCM audio; Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, WAV wavparse active, first frame rendered |
| 4 | Query Video and Audio Stream Properties | Query `westerossink→video-height` and `westerossink→video-width` to confirm stream resolution; Query `n-audio` property via `g_object_get()` to verify PCM stream present; Log stream properties | Verify video dimensions valid; Verify n-audio >= 1 confirming PCM stream detected |
