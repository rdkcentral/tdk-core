...
| 3 | Construct Audio-only WAV Pipeline | Create `playbin` element via ; Configure `uri` property to `audio_src_url_wav_pcm` via ; Set `westerossink` as video sink; Set `autoaudiosink` for PCM audio; Trigger state  | Verify playbin reaches , WAV wavparse active, first frame rendered |
...
-audio` property via  to verify PCM stream present; Log stream properties | Verify video dimensions valid; Verify n-audio >= 1 confirming PCM stream detected |
