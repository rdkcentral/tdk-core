...
| 2 | Media Stream Provisioning | WAV audio-only stream must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_WAV_15sec.wav"` in MediaValidationVariables.py. Stream contains 15-second WAV audio media (PCM audio codec, WAV container format, audio-only content) | Verify WAV audio file is accessible and readable from configured path, appropriate audio decoder and demuxer available |
| 3 | Stream Variable Configuration | Stream variable `audio_src_url_short_duration_wav` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_WAV_15sec.wav"` | Verify `audio_src_url_short_duration_wav` resolves to valid audio stream URL, WAV container structure is valid |
...
| 5 | Platform-Specific Environment Variables | Platform-specific environment variables (`WAYLAND_DISPLAY`, `XDG_RUNTIME_DIR`, `LD_PRELOAD` with vendor WAV decoder libraries, `GST_PLUGIN_PATH`) must be defined in `/opt/TDK/TDK.env` for GStreamer plugin and codec access | Verify `/opt/TDK/TDK.env` exists with all required variables, WAV hardware decoder plugins loaded via GST_PLUGIN_PATH |
