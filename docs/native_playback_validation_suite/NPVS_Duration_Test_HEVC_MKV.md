...
## Objective
Validate MKV streaming format parsing and pipeline initialization. Test validates duration metadata accuracy for HEVC video content in MKV container format. Verifies that duration query accurately reports media duration matching the 9-second baseline, confirming proper MKV element parsing and duration extraction for HEVC-contained streams.
...
| 2 | Media Stream Provisioning | MKV container with HEVC video stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + \"TDK_Asset_Sunrise_HEVC_MKV.mkv\"` in MediaValidationVariables.py. Stream contains HEVC (H.265) video in MKV container format (for HEVC duration testing) | Verify MKV file is accessible and readable from configured path, matroskademux parser available for duration extraction from segment information |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hevc_mkv` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + \"TDK_Asset_Sunrise_HEVC_MKV.mkv\"` | Verify `video_src_url_hevc_mkv` resolves to valid MKV container file with duration metadata |
...
|----|----------|------------------|-----------------|  
| 1 | Initialize Test Environment | Source environment variables from `/opt/TDK/TDK.env` to load GStreamer HEVC decoder plugins, HEVC hardware library paths (`LD_PRELOAD` for vendor libraries), Matroska demuxer plugin, and Wayland display configuration. Establish Wayland display session via RDKWindowManager or westeros compositor. Set up logging file at `/opt/TDK/mediapipeline_test_step.log` for test execution trace | Verify environment variables load correctly, HEVC decoder and Matroska plugins available, Wayland display created successfully, logging initialized without errors |
| 2 | Retrieve Configuration and Construct Playbin Pipeline | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` from device configuration file (default 10 seconds). Retrieve stream variable `video_src_url_hevc_mkv` from MediaValidationVariables.py. Create `playbin` element via . Set URI property via  with MKV file URL. Set playback flags to  via  | Verify playbin element created successfully, stream URL configured without errors, flags set for A/V playback |
| 3 | Configure Westerossink and Register Callbacks | Create `westerossink` element via . Connect as video sink to playbin via . Register `first-video-frame-callback` signal via  to detect when HEVC frame rendering begins | Verify westerossink created successfully, connected as video sink, first-frame callback registered without errors |
| 4 | Transition Pipeline to PLAYING State | Set pipeline state to  via . Poll  until state change completes (). Confirm `firstFrameReceived == true` callback was triggered indicating first HEVC frame rendered on sink. `matroskademux` element automatically parses MKV container and initializes video/audio streams | Verify pipeline reaches  without , state transition completes synchronously or asynchronously, first-frame signal received |
| 5 | Query Stream Duration Using GStreamer Query API | Invoke  to retrieve total MKV stream duration in nanoseconds. Validate query returns TRUE (success). Verify retrieved duration != -1 (indicates valid, parseable MKV file). For MKV streams, `matroskademux` reads duration from Matroska segment seek head or duration element. Convert duration from nanoseconds to seconds via  and milliseconds via  for logging and comparison | Verify duration query succeeds, returned value is positive integer (not -1), duration formatted correctly in M:SS.MS format |
