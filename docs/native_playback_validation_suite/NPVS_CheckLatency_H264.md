...
## Objective
Measure playback latency from pipeline PLAYING state to first frame rendering using H.264 content. The test executes DASH H.264 playback and measures elapsed time when playback position reaches 1 second, validating that latency remains below the configured threshold. Verify frame rendering completes within latency budget for H.264 codec compliance.
...
| 2 | Create Playbin and Configure DASH H.264 Stream | Create playbin element via . Set DASH H.264 stream URL from `MediaValidationVariables.video_src_url_dash_h264` as URI property. Set playback flags to enable video and audio via  | Verify playbin element created successfully, DASH H.264 stream URL configured, flags set for A/V playback |
