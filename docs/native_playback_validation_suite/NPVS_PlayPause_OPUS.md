...
## Objective
Validate OPUS audio stream playback and audio pad query.
...
| 2 | Media Stream Provisioning | WebM container with VP9 video and Opus audio must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"` in MediaValidationVariables.py. Stream contains VP9 video and Opus audio in WebM container format (for VP9/Opus play-pause testing) | Verify WebM file is accessible and readable from configured path, matroskademux and VP9 decoder plugins available for playback |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_vp9` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"` | Verify `video_src_url_vp9` resolves to valid WebM container file with VP9 video and Opus audio data |
...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_vp9` variable; Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` to prepare play-pause cycling test | Verify mediapipelinetests initializes with WebM stream and cycle count configured |
| 3 | Construct VP9 WebM Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `video_src_url_vp9` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for OPUS audio; Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, WebM matroskademux active, first frame rendered |
| 4 | Query Video and Audio Stream Properties | Query `westerossink→video-height` and `westerossink→video-width` to confirm stream resolution; Query `n-audio` property via `g_object_get()` to verify OPUS stream present; Log stream properties | Verify video dimensions valid; Verify n-audio >= 1 confirming OPUS stream detected |
