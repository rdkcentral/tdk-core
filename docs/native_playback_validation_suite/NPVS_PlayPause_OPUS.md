...
## Objective
Validate play-pause state transitions during OPUS audio stream playback. The test verifies pausing halts audio playback and position advances, resuming continues from the pause point without gaps or audio discontinuities. Confirm audio output remains synchronized during state transitions and position tracking reflects pause state accurately.
...
| 3 | Construct VP9 WebM Pipeline | Create `playbin` element via ; Configure `uri` property to `video_src_url_vp9` via ; Set `westerossink` as video sink; Set `autoaudiosink` for OPUS audio; Trigger state  | Verify playbin reaches , WebM matroskademux active, first frame rendered |
| 4 | Query Video and Audio Stream Properties | Query `westerossink→video-height` and `westerossink→video-width` to confirm stream resolution; Query `playbin→n-audio` property via  to verify OPUS stream present; Log stream properties | Verify video dimensions valid; Verify n-audio >= 1 confirming OPUS stream detected |
