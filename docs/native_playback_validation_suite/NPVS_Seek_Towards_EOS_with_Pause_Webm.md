...
## Objective
Test validates seeking capability towards end-of-stream (EOS) on WebM VP9 stream with playback pause intervals by invoking `gst_element_seek()` to reposition playback near stream's final position. The test verifies that the seek target is reached within Â±1 second tolerance, with state machine transitions through pause/resume cycles. Validates frame rendering continues with correct PTS monotonicity after pause transitions and webmmux maintains proper frame positioning during pause-resume-seek sequences towards EOS.
