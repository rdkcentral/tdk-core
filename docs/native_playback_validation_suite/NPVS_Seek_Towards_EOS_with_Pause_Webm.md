...
## TestCase Name
NPVS_Seek_Towards_EOS_with_Pause_Webm
...
## Objective
Test validates seeking capability towards end-of-stream (EOS) on WebM VP9 stream with playback pause intervals by invoking  to reposition playback near stream's final position. The test verifies that the seek target is reached within ±1 second tolerance, with state machine transitions through pause/resume cycles. Validates frame rendering continues with correct PTS monotonicity after pause transitions and webmmux maintains proper frame positioning during pause-resume-seek sequences towards EOS.
...
| 2 | Media Stream Provisioning | WebM container stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"` in MediaValidationVariables.py. Stream contains VP9 video and Opus audio (WebM container format for EOS seeking with pause testing) | Verify WebM file is accessible and readable from configured path, matroskademux and VP9 decoder plugins available for playback |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_direct_webm` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus.webm"` | Verify `video_src_url_direct_webm` resolves to valid WebM container file with VP9 video and Opus audio data |
...
| 4 |  Verify position matches the seek target within ±250ms tolerance | Verify test operation completes successfully with expected results |
| 5 | Monitor Playback Progress | Poll  at 100ms intervals to verify position advances at expected rate | Verify position increments are consistent, no stalls or backward jumps detected |
