...
## Objective
Validate pipeline initialization, resource cleanup, and reinitilization capability by playing the same H.264 video stream via dashdemux demuxer twice sequentially. Test creates `playbin` with `westerossink` video-sink, executes first playback with position  Verify no decoder state errors, memory leaks, or resource stalls between playbacks and confirm "Failures: 0" output for both attempts.
