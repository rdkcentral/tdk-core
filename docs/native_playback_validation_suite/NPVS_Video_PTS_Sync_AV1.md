...
| 2 | Media Stream Provisioning | AV1 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path: `test_streams_base_path + "TDK_Asset_Sunrise_AV1_30fps.mp4"` configured in MediaValidationVariables.py  | Verify AV1 stream file is accessible, readable, and GStreamer AV1 decoder plugin is loaded |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_av1_30fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_AV1_30fps.mp4` (30fps AV1 video for accurate PTS increment validation) | Verify `video_src_url_av1_30fps` resolves to valid, accessible AV1 MP4 file location with 30fps encoding |
...
|----|----------|------------------|-----------------|  
| 1 | Initialize Test Environment | Source environment variables from `/opt/TDK/TDK.env` to load GStreamer plugins, AV1 codec library paths, and Wayland display configuration. Establish Wayland display session via RDKWindowManager or westeros compositor. Create logging file at `/opt/TDK/mediapipeline_test_step.log` for test execution trace | Verify `/opt/TDK/TDK.env` loads successfully, GST_PLUGIN_PATH includes AV1 plugin, Wayland display is active, logging file created without errors |
| 2 | Configure Playbin and Load AV1 Stream | Create `playbin` element via `gst_element_factory_make("playbin", NULL)`. Configure `uri` property to AV1 stream URL from MediaValidationVariables.video_src_url_av1_30fps via `g_object_set(playbin, "uri", url, NULL)`. Set `video-sink` property to `westerossink` via `g_object_set(playbin, "video-sink", westerossink_element, NULL)` | Verify playbin element created successfully, `uri` property set to AV1 stream path, `video-sink` configured to westerossink element |
