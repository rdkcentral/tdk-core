...
## Objective
Test to verify playback rate control at 0.5x speed using GStreamer `gst_element_seek()` function with VP9 codec. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic `gst_element_query_position()` polling with position increment expected at Â±25% tolerance around 0.05 seconds per 100ms interval. Verify audio/video remain synchronized during VP9 rate change and frame rendering continues without discontinuities using `westerossink` frame statistics validation.
...
| 2 | Media Stream Provisioning | VP9 encoded WebM stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_30fps.webm"` in MediaValidationVariables.py (30fps VP9 baseline stream in WebM container for rate control validation) | Verify VP9 WebM stream file is accessible, readable, and contains minimum 60 seconds of content for rate testing |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_vp9_30fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_VP9_30fps.webm` (stream must have VP9 codec in WebM container at 30fps framerate) | Verify `video_src_url_vp9_30fps` resolves to valid, accessible VP9 WebM file location with 30fps encoding |
