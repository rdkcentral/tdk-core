...
| 2 | Create Playbin and Configure MKV Stream | Create playbin element via . Set MKV file URL from `MediaValidationVariables.video_src_url_4k_av1_mkv` as URI property (local file:// or HTTP URL). Set playback flags to enable video and audio via  | Verify playbin element created successfully, MKV stream URL configured, flags set for A/V playback |
