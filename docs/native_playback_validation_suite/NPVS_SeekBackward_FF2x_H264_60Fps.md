...
**TestCase Name**
NPVS_SeekBackward_FF2x_H264_60Fps
...
## Objective
Validate backward seek operation to prior position within already-played stream content. The test invokes `gst_element_seek()` with target position prior to current timestamp, simulating user rewind/review behavior. Verify seek operation safely repositions without underflowing past stream start (GST_CLOCK_TIME_NONE) or causing pipeline errors. Confirm position queries return values matching the backward seek target, and validate demuxer correctly repositions to earlier segments (dashdemux/hlsdemux) or cluster boundaries (matroskademux) within the container. Resume playback and verify rendering continues without discontinuities.
...
| 2 | Media Stream Provisioning | DASH stream with MPD manifest at 60fps must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "DASH_H264_60fps/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` in MediaValidationVariables.py. Stream contains H.264 video at 60fps and AAC audio (DASH container format for 60fps backward fast-forward testing) | Verify master.mpd manifest file is accessible and readable from configured path, dashdemux plugin available for MPD parsing at 60fps |
...
| 2 | Create Playbin and Configure Video Sink | Create `playbin` element via `gst_element_factory_make("playbin", NULL)`. Configure URI via `g_object_set(playbin, "uri", stream_url, NULL)` with H.264 60fps stream. Create and set `westerossink` as video sink | Playbin created, URI configured, westerossink set as video sink |
