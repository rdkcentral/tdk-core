...
**TestCase Name**
NPVS_PlayPause_ExtendedDuration_MKV
...
## Objective
Validate MKV streaming format parsing and pipeline initialization.
...
| 2 | Media Stream Provisioning | MKV container stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_2160p.mkv"` in MediaValidationVariables.py. Stream contains H.264 video at 4K resolution and AAC audio (MKV container format for extended-duration play-pause testing) | Verify MKV file is accessible and readable from configured path, matroskademux and H.264 decoder plugins available for playback |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_4k_mkv` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_2160p.mkv"` | Verify `video_src_url_4k_mkv` resolves to valid MKV container file with video and audio data at 4K resolution |
...
| 3 | Construct H.265 MKV Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` to `video_src_url_hevc_mkv` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio; Transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, matroskademux active, first frame rendered |
