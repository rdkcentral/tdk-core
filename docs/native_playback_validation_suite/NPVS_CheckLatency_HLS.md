...
| 2 | HLS H.264/AAC Stream Provisioning | H.264/Avc With Aac Audio stream must be accessible via HTTP/HTTPS or filesrc. Stream file path configured as `test_streams_base_path + "HLS_H264_AAC/master.m3u8"`  in MediaValidationVariables.py (HLS M3U8 with MPEG-2 TS segments, 1080p, 5+ minutes) | Verify HLS H.264/AAC stream is accessible and parseable, segments downloadable |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hls` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "HLS_H264_AAC/master.m3u8"`  (format: HLS M3U8 with MPEG-2 TS segments) | Verify `video_src_url_hls` resolves to valid, accessible stream with valid segments |
...
| 2 | Create Playbin and Configure HLS Stream | Create playbin element via `gst_element_factory_make("playbin", NULL)`. Set HLS stream URL from `MediaValidationVariables.video_src_url_hls` as URI property. Set playback flags to enable video and audio via `GST_PLAY_FLAG_VIDEO \| GST_PLAY_FLAG_AUDIO` | Verify playbin element created successfully, HLS stream URL configured, flags set for A/V playback |
