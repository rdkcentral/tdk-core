...
## Objective
Test to verify playback rate control at 0.5x speed using GStreamer `gst_element_seek()` function with H.264 24fps stream baseline. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic `gst_element_query_position()` polling with position increment expected at Â±25% tolerance around 0.05 seconds per 100ms. Verify audio/video remain synchronized during rate change and frame rendering continues without discontinuities with 24fps framerate baseline.
