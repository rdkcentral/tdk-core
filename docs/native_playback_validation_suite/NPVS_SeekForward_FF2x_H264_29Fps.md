...
**TestCase Name**
NPVS_SeekForward_FF2x_H264_29Fps
...
| 2 | Media Stream Provisioning | DASH stream with MPD manifest at 29.97fps must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "DASH_H264_29fps/master.mpd"` in MediaValidationVariables.py. Stream contains H.264 video at 29.97fps and AAC audio (DASH container format for 29.97fps forward fast-forward testing) | Verify master.mpd manifest file is accessible and readable from configured path, dashdemux plugin available for MPD parsing at 29.97fps |
