...
## Objective
Test to verify the presence of `buffer-underflow-callback` signal when Appsrc buffer underflows for MP3 streams. Validate that the signal is correctly emitted and detected through the GStreamer pipeline during intentional buffer starvation with BYTES_THRESHOLD = 811071 bytes, and verify pipeline recovers after buffer refill operations.
...
| 2 | Media Stream Provisioning | MP3 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding. Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_MP3.mp3"` in MediaValidationVariables.py | Verify MP3 stream file is accessible and readable from filesystem |
| 3 | Stream Variable Configuration | Stream variable `audio_src_url_mp3` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_MP3.mp3` (Appsrc requires local file access, not DASH/HLS streams) | Verify `audio_src_url_mp3` resolves to valid, accessible MP3 file location |
| 4 | Appsrc Buffer Threshold Configuration | `BYTES_THRESHOLD` must be configured to 811071 bytes (actual threshold value from test script) to trigger mp3 underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered | Verify BYTES_THRESHOLD = 811071 bytes is set in Appsrc configuration or test parameters |
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to `GST_STATE_PAUSED` via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 811071 bytes` is reached. Transition to `GST_STATE_PLAYING` via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 811071 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 1622142 bytes`. After all media data is transferred to Appsrc, emit `end-of-stream` action via `g_signal_emit_by_name()` | Verify buffers accumulated to 1622142 bytes threshold and `end-of-stream` action is emitted |
