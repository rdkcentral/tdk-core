...
## Objective
Validate playback rate control at 0.5x speed (half normal) with HEVC in MKV container. The test sets rate to 0.5 and verifies position advances at exactly half the normal speed. Confirm MKV container parsing, audio/video synchronization, and frame rendering consistency during rate changes.
...
| 4 | Execute Playback Rate Change | Invoke  with playback rate set to 0.5 and GST_SEEK_FLAG_FLUSH flag. Verify position progression matches 0.5x speed (half normal speed) with maintained audio/video sync | Verify test operation completes successfully with expected results |
| 5 | Monitor Playback Progress | 5x rate (expected increment: 0.05 seconds per 100ms ±25% tolerance) | Verify position increments are consistent, no stalls or backward jumps detected |
