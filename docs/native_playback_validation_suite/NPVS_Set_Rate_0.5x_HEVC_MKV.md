...
## Objective
Test to verify playback rate control at 0.5x speed using GStreamer `gst_element_seek()` function with HEVC H.265 in MKV container. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic `gst_element_query_position()` polling with position increment expected at Â±25% tolerance around 0.05 seconds per 100ms interval. Verify MKV demuxer correctly parses container, audio/video remain synchronized during rate change, and frame rendering continues without discontinuities using `westerossink` frame statistics validation.
...
| 2 | Media Stream Provisioning | HEVC H.265 encoded MKV stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_MKV.mkv"` in MediaValidationVariables.py (HEVC in Matroska container for rate control validation). Matroska demuxer (`matroskademux`) must be available and functional | Verify HEVC MKV stream file is accessible, readable, matroskademux plugin available, and stream contains minimum 60 seconds of content |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hevc_mkv` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_HEVC_MKV.mkv` (stream must have HEVC H.265 codec in MKV container) | Verify `video_src_url_hevc_mkv` resolves to valid, accessible HEVC MKV file location with proper Matroska container format |
...
| 4 | Execute Playback Rate Change | Invoke `gst_element_seek()` with playback rate set to 0.5 and GST_SEEK_FLAG_FLUSH flag. Verify position progression matches 0.5x speed (half normal speed) with maintained audio/video sync | Verify test operation completes successfully with expected results |
| 5 | Monitor Playback Progress | Poll `gst_element_query_position()` at 100ms intervals to verify position advances at 0.5x rate (expected increment: 0.05 seconds per 100ms Â±25% tolerance) | Verify position increments are consistent, no stalls or backward jumps detected |
