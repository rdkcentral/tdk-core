...
## Objective
Validate AAC audio extraction and playback from DASH H.264 stream via `test_generic_playback` function. Initialize playbin with dashdemux demuxing, extract audio stream via 
-audio` property query. Execute audio-only playback path with `autoaudiosink`, continuously monitor position via `gst_element_query_position()`. Verify audio stream properly decoded and rendered without video sink, confirm clean 10-second playback window.
