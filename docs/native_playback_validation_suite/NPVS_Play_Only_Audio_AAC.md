...
## Objective
Validate AAC audio extraction and playback from DASH H.264 stream .  Execute audio-only playback path with `autoaudiosink`, continuously monitor position via . Verify audio stream properly decoded and rendered without video sink, confirm clean 10-second playback window.
