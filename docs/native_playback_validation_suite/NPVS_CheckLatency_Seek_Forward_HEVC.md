...
## Objective
Measure playback latency for forward seek operations on HEVC DASH streams. The test performs a forward seek operation and records latency from seek initiation to first frame rendering at the new position, validating that latency remains below the configured threshold. Verify seek completes accurately to the target position and frames render within latency budget.
