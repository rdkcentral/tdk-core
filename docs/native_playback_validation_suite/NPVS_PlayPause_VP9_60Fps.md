...
| 3 | Construct VP9 WebM Pipeline | Create `playbin` element via ; Configure `uri` to `video_src_url_vp9_60fps` via ; Set `westerossink` as video sink; Set `autoaudiosink` for OPUS audio;  | Verify playbin reaches , matroskademux active, first frame rendered |
