...
**TestCase Name**
NPVS_Appsrc_Video_Underflow_Playback_H264
...
## Objective
Test to verify if buffer underflow signal is obtained upon audio/video underrun when using Appsrc and playback is smooth after buffer is filled again. Validate H264 stream playback through custom Appsrc pipeline with underflow detection and recovery capability. Specifically test with BYTES_THRESHOLD = 2796910 bytes configuration to trigger controlled buffer starvation and verify `buffer-underflow-callback` signal detection and pipeline recovery via buffer refill operations.
...
| 2 | Media Stream Provisioning | H264 stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`) for Appsrc feeding. Stream file path configured as `test_streams_base_path + "TDK_Asset_Sunrise_30fps_v2.mp4"` in MediaValidationVariables.py | Verify H264 stream file is accessible and readable from filesystem |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_mp4_30fps` configured in `MediaValidationVariables.py` with path: `TDK_Asset_Sunrise_30fps_v2.mp4` (Appsrc requires local file access, not DASH/HLS streams) | Verify `video_src_url_mp4_30fps` resolves to valid, accessible H264 file location |
| 4 | Appsrc Buffer Threshold Configuration | `BYTES_THRESHOLD` must be configured to 2796910 bytes (actual threshold value from test script) to trigger h264 underflow condition. This value defines the Appsrc buffer fill level before starvation signal is triggered | Verify BYTES_THRESHOLD = 2796910 bytes is set in Appsrc configuration or test parameters |
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to `GST_STATE_PAUSED` via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 2796910 bytes` is reached. Transition to `GST_STATE_PLAYING` via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 2796910 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 5593820 bytes`. After all media data is transferred to Appsrc, emit `end-of-stream` action via `g_signal_emit_by_name()` | Verify buffers accumulated to 5593820 bytes threshold and `end-of-stream` action is emitted |
