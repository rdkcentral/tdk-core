...
## Objective
Validate MPEG2 video codec with MP3 audio via `test_generic_playback` function using MPEG streaming. Initialize playbin with `mpegpsdemux` element, configure `westerossink` as video sink and `autoaudiosink` for audio output. Query video dimensions and audio stream count via `westerossink` and 
...
| 4 | Query Video and Audio Stream Properties | Query video dimensions via `g_object_get(westerossink, "video-height", &height, NULL)` and `westerossinkâ†’video-width`; Query audio stream count via `g_object_get(playbin, "n-audio", &n_audio, NULL)` to verify MP3 stream presence; Log extracted properties | Verify video dimensions valid; Verify n-audio >= 1 confirming MP3 stream present |
