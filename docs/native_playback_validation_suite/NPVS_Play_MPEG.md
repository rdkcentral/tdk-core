...
## Objective
Validate MPEG2 video codec with MP3 audio via `test_generic_playback` function using MPEG streaming. Initialize playbin with `mpegpsdemux` element, configure `westerossink` as video sink and `autoaudiosink` for audio output. Query video dimensions and audio stream count via `westerossink` and `n-audio` properties. Execute 10-second playback monitoring position advancement, frame rendering via `westerossink→stats`, and verify audio-video synchronization throughout stream.
