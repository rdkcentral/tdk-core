...
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to  via . Emit `push-buffer` action on Appsrc via  until `BYTES_THRESHOLD = 419338 bytes` is reached.  via . Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 419338 bytes threshold, first-frame signal is detected |
