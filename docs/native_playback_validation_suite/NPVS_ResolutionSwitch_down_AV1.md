...
## TestCase Name
NPVS_ResolutionSwitch_down_AV1
...
| 2 | Media Stream Provisioning | AV1 video stream with multiple resolutions (high (4K) to low (144p) resolution) must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_AV1_resolution_down.mp4"`  | Verify TDK_Asset_Sunrise_AV1_resolution_down.mp4 is accessible and readable from filesystem with multiple embedded resolution representations |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_resolution_down_av1` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_AV1_resolution_down.mp4"` (local file access, not DASH/HLS streams) | Verify `video_src_url_resolution_down_av1` resolves to valid, accessible AV1 MP4 file location with resolution switching capability |
