...
## Objective
Validate dynamic video resolution switching during playback through `playbin` element monitoring westerossink video pad properties. Test verifies stream contains exactly 7 unique resolutions (144p, 240p, 360p, 480p, 720p, 1080p, 2160p (ascending)) and detects each resolution change via  at 100ms intervals combined with video-height/video-width property queries. Confirm pipeline transitions through all resolutions without decoding failures, GST_MESSAGE_ERROR, or frame drops during switching by polling rendered_frames/dropped_frames from westerossink sink statistics.
...
| 2 | Media Stream Provisioning | VP9 video stream with multiple resolutions (low (144p) to high (4K) resolution) must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_resolution_up.mp4"`  | Verify TDK_Asset_Sunrise_VP9_resolution_up.mp4 is accessible and readable from filesystem with multiple embedded resolution representations |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_resolution_up_vp9` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_resolution_up.mp4"` (local file access, not DASH/HLS streams) | Verify `video_src_url_resolution_up_vp9` resolves to valid, accessible VP9 MP4 file location with resolution switching capability |
...
| 2 | Configure Test Framework and Load Stream | Set test name to `test_resolution_up`, configure `ResolutionSwitchTest = true` flag in test framework. Load VP9 stream via `video_src_url_resolution_up_vp9` variable from MediaValidationVariables.py. Set test timeout to 70 seconds for complete resolution traversal | Verify test case name and flags configured, TDK_Asset_Sunrise_VP9_resolution_up.mp4 stream URI resolved successfully, timeout set to 70 seconds |
