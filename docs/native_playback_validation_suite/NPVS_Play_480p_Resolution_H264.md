...
## Objective
Validate 480p (854×480) resolution playback  with H.264 video codec and AAC audio.  Verify resolution matches expected 854×480 (±5 pixel tolerance). Execute 10-second playback monitoring position via , validate frame rendering statistics via `westerossink→stats`. Confirm clean pipeline state transitions and error-free playback.
...
| 3 | Construct H.264 Pipeline with Resolution Stream | Create `playbin` element; Configure `uri` to video_src_url_mp4_480p (MP4); Set `westerossink` as video sink; Trigger state  | Verify playbin reaches , MP4 parsed successfully |
| 4 | Query Video Dimensions and Validate Resolution | Query `westerossink→video-height` and `westerossink→video-width` via ; Extract values (854±5 height, 480±5 width expected) | Verify video-height == 480±5 pixels; Verify video-width == 854±5 pixels |
...
| 6 | Validate Frame Rendering at Resolution | Query `westerossink→stats` to verify `rendered_frames` increments consistently at 480p resolution; Verify `dropped_frames` < 1% of rendered_frames | Verify frame statistics indicate proper 480p rendering |
