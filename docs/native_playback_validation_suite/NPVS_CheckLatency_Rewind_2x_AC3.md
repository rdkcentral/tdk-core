...
| 2 | Configure Test Framework and Load Stream | Set test name to `test_trickplay`, Load AC3 stream via `video_src_url_aac` variable from MediaValidationVariables.py. Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and calculate seek duration = 2*timeout + 30 seconds | Verify test case name configured, DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd stream URI resolved successfully, timeout retrieved and seek duration calculated |
