...
## Objective
Test to verify if dynamic audio codec switching (OPUS to AAC) is possible while maintaining video playback continuity using GStreamer playbin `current-audio` property. Validate that audio track can be switched between different codec types (OPUS and AAC) present in the same multi-codec stream without interrupting video playback. Specifically validate audio rendering quality, position synchronization, and codec switching timing accuracy using `g_object_set(playbin, "current-audio", stream_index, NULL)` property during active playback. Verify codec identity using `g_signal_emit_by_name(playbin, "get-audio-tags", stream_index, &tags)` to confirm codec change.
...
| 2 | Media Stream Provisioning | Multi-codec stream containing both OPUS and AAC audio tracks must be accessible via HTTPS (`souphttpsrc`) or local file system (`filesrc`) with DASH manifest. Stream configured as `test_streams_base_path + "MultiCodecStreams/stream_with_opus_aac.mpd"` in MediaValidationVariables.py | Verify multi-codec DASH stream accessible, both OPUS and AAC audio tracks present and parseable |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_opus_aac` configured in `MediaValidationVariables.py` with DASH manifest containing OPUS and AAC audio tracks | Verify `video_src_url_opus_aac` resolves to valid multi-codec stream with dual audio tracks |
...
| 2 | Configure Test Parameters and Load Multi-Codec Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and `NATIVE_PLAYBACK_CHECK_AV_STATUS` from device. Build `mediapipelinetests` command with test_name="test_playback_audio_change", stream_url=video_src_url_opus_aac, parameters: checkavstatus=yes/no, timeout=N | Verify device config retrieved successfully, command constructed with correct parameters |
...
| 4 | Retrieve and Verify Multiple Audio Tracks | Query playbin to determine number of audio streams via `g_object_get(playbin, "n-audio", &n_audio, NULL)`. Verify n-audio >= 2 (confirming OPUS and AAC tracks present). Query current audio index via `g_object_get(playbin, "current-audio", &current_audio, NULL)` to identify default (OPUS) track | Verify n-audio returns at least 2, current-audio successfully retrieved (typically 0 for first codec) |
