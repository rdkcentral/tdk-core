...
**TestCase Name**
NPVS_CheckLatency_Rewind_2x_EAC3
...
## Objective
Validate reverse playback at 2x speed (playback rate = -2) on EAC3 audio stream via dashdemux demuxer. Test executes seek operation to stream position (2*NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT + 30 seconds) via `gst_element_seek()` with GST_SEEK_FLAG_FLUSH, then applies negative playback rate via `gst_element_seek(playbin, -2, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET, currentPosition)`. Validate position decreases at expected rate during 2x reverse playback for NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT seconds with position tolerance ±250ms, no GST_MESSAGE_ERROR, and continuous rendered frames with end-to-end latency measurement via proc entry stats.
...
| 2 | Configure Test Framework and Load Stream | Set test name to `test_trickplay`, configure `checkavstatus` parameter from `NATIVE_PLAYBACK_CHECK_AV_STATUS` device config. Load EAC3 stream via `video_src_url_aac` variable from MediaValidationVariables.py. Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and calculate seek duration = 2*timeout + 30 seconds | Verify test case name configured, DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd stream URI resolved successfully, timeout retrieved and seek duration calculated |
...
| 5 | Execute Forward Seek to Calculated Position | Calculate seek target: `seekPosition = (2 * NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT + 30) * GST_SECOND`. Execute `gst_element_seek(playbin, 1.0, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, seekPosition, GST_SEEK_TYPE_SET, GST_CLOCK_TIME_NONE)` to advance stream to seek target position. Monitor `GST_MESSAGE_ASYNC_DONE` on bus (2-second timeout for seek completion). Query new position via `gst_element_query_position()` to verify seek accuracy within ±250ms tolerance | Verify seek executed successfully, position advanced to target within tolerance, no GST_MESSAGE_ERROR on bus |
| 6 | Apply Negative Playback Rate for Rewind with Latency Measurement | Set `setRate = -2` (negative value for reverse playback). Record timestamp before seeking for latency calculation. Execute `gst_element_seek(playbin, -2, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET, currentPosition)` to initiate 2x reverse playback. Calculate latency from seek command to first frame at reversed rate. Monitor `GST_MESSAGE_ASYNC_DONE` on bus (2-second timeout). Query position to confirm reverse playback initiated | Verify rewind operation executed successfully with latency below configured threshold (default 100ms), pipeline in reverse playback mode at rate -2, position query reflects reverse progression |
| 7 | Monitor Rewind Position with Latency Validation | Execute `PlaybackValidation()` function which polls position via `gst_element_query_position(playbin, GST_FORMAT_TIME, &position)` at 100ms intervals for latency monitoring. For each poll, verify position DECREASES from previous value (reverse playback verification). Calculate position delta per 100ms interval and end-to-end latency metrics; expected rate ≈ -2/3/4 seconds per second real-time. Verify latency remains within configured threshold and position decrements stay within tolerance (±250ms per interval) | Verify position queries return valid, monotonically decreasing timestamps with acceptable latency, position deltas consistent with 2x reverse rate, no backward-forward jitter, latency metrics captured |
