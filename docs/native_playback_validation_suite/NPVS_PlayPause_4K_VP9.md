...
| 4 | Transition Pipeline to PLAYING State | Set pipeline state to  via . Monitor  on GStreamer bus via . Wait for state change completion via  polling until  | Verify pipeline transitions to PLAYING state without errors, matroskademux active, first frame rendered via westerossink callback |
