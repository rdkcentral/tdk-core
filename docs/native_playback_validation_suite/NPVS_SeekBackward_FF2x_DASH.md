...
| 2 | Create Playbin and Configure Video | Create `playbin` element via . Configure URI via  with DASH manifest. Set `westerossink` as video sink | Playbin created, DASH URI configured, westerossink set |
...
| 5 | Execute Backward Seek on DASH Stream | Query current position via . Calculate backward seek target. Invoke  handling manifest updates | Seek completes, DASH demux updates manifest, playback continues |
| 6 | Validate Backward Seek Accuracy on Streaming | Monitor bus for .  Handle segment boundaries in manifest | Position matches target within ±1000ms, backward seek confirmed on DASH stream |
...
| 8 | Monitor EOS and Validate Stream Integrity | Continue until  or timeout. Verify no  during streaming and seeking operations | EOS or timeout detected, no errors on DASH stream |
