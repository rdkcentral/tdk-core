...
| 6 | Validate Forward Seek Target Accuracy | Monitor bus for `GST_MESSAGE_ASYNC_DONE` confirming seek completion. Poll `gst_element_query_position()` every 100ms to verify position matches seek target within ±GST_SECOND (±1000ms). Confirm position stabilizes at target | Position queries show currentPosition ≈ seekPosition ±1000ms, forward seek confirmed |
