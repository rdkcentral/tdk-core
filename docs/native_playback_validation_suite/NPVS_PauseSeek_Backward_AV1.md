...
| 2 | Configure Playback Pipeline | Create `playbin` element and set `uri` property to AV1 DASH stream (`TDK_Asset_DASH_AV1_AAC/master.mpd` from MediaValidationVariables.py).  Wait for playback startup with initial media buffering. Verify `first-video-frame-callback` signal to confirm AV1 rendering started | Verify pipeline reaches , first frame detected on westerossink without errors |
...
| 8 | Release Pipeline Resources | Set pipeline to . Unreference playbin element to deallocate AV1 decoder, demuxer, and sink resources | Verify pipeline reaches , all GStreamer resources released, no memory leaks detected |
