...
Test to verify forward seek operation on H.264 DASH stream via `gst_element_seek()` with GST_SEEK_FLAG_FLUSH flag to seek to a specified position during active playback. The test validates accurate seek positioning, proper pipeline state handling during seek, and seamless playback recovery after seek completion. Verify playback position reaches the target seek position within ±1 second tolerance via `gst_element_query_position()`, validate frame rendering via westerossink→stats show rendered_frames increment after seek, and confirm no errors on GStreamer bus.
...
| 3 | Stream Variable Configuration | `video_src_url_aac` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd"` | Verify `video_src_url_aac` resolves to valid location with correct codec support |
...
| 2 | Configure Seek Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` (playback duration for each seek operation), `NATIVE_PLAYBACK_SEEK_POSITION` (target position for forward seek, default 20 seconds); Retrieve stream URL from `video_src_url_aac` variable; Construct seek operation string with `seek:timeout:position`; Execute `mediapipelinetests test_trickplay <URL> checkavstatus=<yes/no> operations=<seek_string>` to prepare seek test | Verify mediapipelinetests initializes with DASH stream and seek operation parameters correctly configured |
| 3 | Construct H.264 DASH Pipeline and Initiate Playing State | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `video_src_url_aac` via `g_object_set()`; Set `westerossink` as video sink (or `autoaudiosink` for audio-only) via `g_object_set()`; Trigger state transition to `GST_STATE_PLAYING` via `gst_element_set_state()` | Verify playbin reaches `GST_STATE_PLAYING`, DASH dashdemux active and parsing stream, first frame rendered via first-video-frame-callback signal (or first audio sample for audio-only), no `GST_MESSAGE_ERROR` on bus |
