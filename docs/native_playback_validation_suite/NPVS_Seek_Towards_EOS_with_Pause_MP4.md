...
## TestCase Name
NPVS_Seek_Towards_EOS_with_Pause_MP4
...
| 2 | Media Stream Provisioning | MP4 container stream must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_MP4.mp4"` in MediaValidationVariables.py. Stream contains H.264 video and AAC audio (MP4 container format for EOS seeking with pause testing) | Verify MP4 file is accessible and readable from configured path, qtdemux and H.264 decoder plugins available for playback |
