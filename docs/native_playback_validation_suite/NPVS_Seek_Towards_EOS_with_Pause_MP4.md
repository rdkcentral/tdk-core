...
## Objective
Test validates seeking capability towards end-of-stream (EOS) on MP4 H.264 video with playback pause intervals by invoking `gst_element_seek()` to reposition playback near the stream's final position. The test verifies that the seek target position is reached within Â±1 second tolerance using position queries every 100ms, with periodic pause/resume cycles to validate state machine robustness. Validates frame rendering continues with correct PTS monotonicity after pause transitions and no errors are triggered during near-EOS seeking with pause recovery operations.
