...
## Objective
Test validates duration metadata accuracy for 4K VP9 video content. Verifies that duration query accurately reports media duration matching the 9-second baseline, confirming proper parsing and duration extraction capability for high-resolution VP9 streams.
...
| 2 | Retrieve Configuration and Construct Playbin Pipeline | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` from device configuration file (default 10 seconds). Retrieve stream variable `video_src_url_webm_4k_vp9` from MediaValidationVariables.py. Create `playbin` element via . Set URI property via  with WebM file URL. Set playback flags to  via  | Verify playbin element created successfully, stream URL configured without errors, flags set for A/V playback |
| 3 | Configure Westerossink and Register Callbacks | Create `westerossink` element via . Connect as video sink to playbin via . Register `first-video-frame-callback` signal via  to detect when VP9 4K frame rendering begins | Verify westerossink created successfully, connected as video sink, first-frame callback registered without errors |
| 4 | Transition Pipeline to PLAYING State | Set pipeline state to  via . Poll  until state change completes (). Confirm `firstFrameReceived == true` callback was triggered indicating first VP9 4K frame rendered on sink. `webmdemux` element parses WebM container and initializes VP9 video and Opus audio streams | Verify pipeline reaches  without , state transition completes synchronously or asynchronously, first-frame signal received |
| 5 | Query Stream Duration Using GStreamer Query API | Invoke  to retrieve total WebM stream duration in nanoseconds. Validate query returns TRUE (success). Verify retrieved duration != -1 (indicates valid, parseable WebM file). For WebM streams, `webmdemux` reads duration from EBML segment or calculates from VP9 frame headers. Convert duration from nanoseconds to seconds via  and milliseconds via  for logging and comparison | Verify duration query succeeds, returned value is positive integer (not -1), duration formatted correctly in M:SS.MS format |
