...
| 2 | Media Stream Provisioning | AAC audio-only stream must be accessible via HTTPS or local file system (`filesrc`). Stream file configured as `test_streams_base_path + "DASH_H264_AAC_15Sec/audio.mp4"` in MediaValidationVariables.py. Stream contains 15-second AAC audio media (AAC audio codec, MP4 container format, audio-only content) | Verify audio.mp4 file is accessible and readable from configured path, appropriate audio decoder and demuxer available |
| 3 | Stream Variable Configuration | Stream variable `audio_src_url_short_duration_aac` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "DASH_H264_AAC_15Sec/audio.mp4"` | Verify `audio_src_url_short_duration_aac` resolves to valid audio stream URL, MP4 container structure is valid |
...
| 5 | Platform-Specific Environment Variables | Platform-specific environment variables (`WAYLAND_DISPLAY`, `XDG_RUNTIME_DIR`, `LD_PRELOAD` with vendor AAC decoder libraries, `GST_PLUGIN_PATH`) must be defined in `/opt/TDK/TDK.env` for GStreamer plugin and codec access | Verify `/opt/TDK/TDK.env` exists with all required variables, AAC hardware decoder plugins loaded via GST_PLUGIN_PATH |
