...
## Objective
Validate frame rendering performance during H.264 video playback. The test monitors frame delivery and frame drop metrics to verify frames render at the expected rate with acceptable frame loss within baseline limits throughout playback.
...
| 5 | Platform-Specific Environment Variables | Platform-specific environment variables (`WAYLAND_DISPLAY`, `XDG_RUNTIME_DIR`, `LD_PRELOAD` with vendor libraries, ) must be defined in `/opt/TDK/TDK.env` | Verify `/opt/TDK/TDK.env` exists and contains all required environment variables for GStreamer 1.16+ H264 codec support |
...
| 3 | Create Playbin Pipeline and Set Stream URI | Create `playbin` element via , set stream URI to H264 file path via , attach `westerossink` as video sink via  | Verify `playbin` element is created and configured with correct stream URI and video sink |
...
| 6 | Monitor Playback Position and Frame Rate | Query playback position via  to track playback progress at 1x rate; validate frame rendering rate matches stream fps (30fps for this H264 stream) | Verify position advances at 1x rate without stalls or backward jumps; frame increments match expected rendering rate |
