...
| 2 | Media Stream Provisioning | H.264 video stream with multiple resolutions (low (144p) to high (4K) resolution) must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file path configured in MediaValidationVariables.py as `test_streams_base_path + "TDK_Asset_Sunrise_H264_resolution_up.mp4"`  | Verify TDK_Asset_Sunrise_H264_resolution_up.mp4 is accessible and readable from filesystem with multiple embedded resolution representations |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_resolution_up_h264` configured in `MediaValidationVariables.py` with path: `test_streams_base_path + "TDK_Asset_Sunrise_H264_resolution_up.mp4"` (local file access, not DASH/HLS streams) | Verify `video_src_url_resolution_up_h264` resolves to valid, accessible H.264 MP4 file location with resolution switching capability |
...
| 2 | Configure Test Framework and Load Stream | Set test name to `test_resolution_up`, configure `ResolutionSwitchTest = true` flag in test framework. Load H.264 stream via `video_src_url_resolution_up_h264` variable from MediaValidationVariables.py. Set test timeout to 70 seconds for complete resolution traversal | Verify test case name and flags configured, TDK_Asset_Sunrise_H264_resolution_up.mp4 stream URI resolved successfully, timeout set to 70 seconds |
