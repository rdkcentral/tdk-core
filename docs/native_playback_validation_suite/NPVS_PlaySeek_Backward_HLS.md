...
## Objective
Validate backward seek operation on HLS protocol streams. The test verifies accurate positioning to the target seek point within ±1 second tolerance, seamless playback recovery after seek, frame rendering without interruption, and error-free playback.
...
| 3 | Construct H.264 HLS Pipeline and Initiate Playing State | Create `playbin` element via ; Configure `uri` property to `video_src_url_hls` via ; Set `westerossink` as video sink (or `autoaudiosink` for audio-only) via ; Trigger state  via  | Verify playbin reaches , HLS hlsdemux active and parsing stream, first frame rendered via first-video-frame-callback signal (or first audio sample for audio-only), no  on bus |
