...
| 4 | Device Configuration Parameters | Test execution parameters must be retrieved from device configuration file: `NATIVE_PLAYBACK_CHECK_AV_STATUS` (default: "no") for audio/video verification, `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` (default: 10 seconds) for playback duration, `NATIVE_PLAYBACK_UNDERFLOW_AUDIO_START_POINT` and `NATIVE_PLAYBACK_UNDERFLOW_AUDIO_END_POINT` (seek positions for underflow recovery testing) | Verify all device config parameters are accessible and contain valid values |
