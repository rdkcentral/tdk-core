...
## Objective
Validate audio stream underflow detection and playback recovery. The test detects audio buffer depletion events and verifies recovery by pausing, seeking to a restart point, and resuming playback with audio rendering properly restored without artifacts.
...
| 3 | Create Playbin Element and Configure Audio Stream | Create `playbin` element via . Configure `uri` property to audio underflow stream via  where stream_url = `test_streams_base_path + "TDK_Asset_Audio_underflow_v2.mp4"`. Register `element-setup` signal via  to discover audio decoder (brcmaudiodecoder, amlhalasink, rtkaudiosink) and audio sink elements when pipeline is constructed | Verify playbin element created successfully, `uri` property configured to audio underflow stream path, `element-setup` signal will enable audio element discovery during pipeline construction |
| 4 | Register Audio Element Underflow Callbacks | During `element-setup` signal callback, identify audio decoder and audio sink elements and register underflow detection callbacks:  and  where callback_func sets `audio_underflow_received = true` when signal is received | Verify audio element callbacks are registered successfully on correct elements (decoder and sink) |
| 5 | Transition Pipeline to Playing and Wait for Underflow Signal | Set pipeline to  via  to begin audio stream playback. Continuously  Record timestamps to detect position stalls. Play stream for duration specified in `NATIVE_PLAYBACK_UNDERFLOW_AUDIO_END_POINT` seconds while monitoring for `buffer-underflow-callback` or `underrun-callback` signals | Verify pipeline transitions to PLAYING state, playback begins without errors, `audio_underflow_received` flag becomes true when audio buffer starves (underflow signal received) |
...
| 7 | Pause Pipeline and Wait Before Recovery Seek | Set pipeline to  via  to pause audio playback. Wait 5 seconds to allow audio buffers to stabilize. This ensures buffer refill preparation before initiating seek operation | Verify pipeline transitions to PAUSED state, audio playback stops |
| 8 | Perform Seek to Audio Recovery Start Point | Execute  where audio_start_point = `NATIVE_PLAYBACK_UNDERFLOW_AUDIO_START_POINT` to seek to recovery position. FLUSH flag clears buffers and forces decoder restart. Verify seek operation completes successfully | Verify seek operation executed successfully, pipeline flushes buffers and prepares for playback from new position |
| 9 | Resume Playback After Underflow Recovery | Set pipeline to  via  to resume playback from seeked position. Continuously query audio sink statistics every 1 second via  and retrieve rendered audio frames via . Verify audio frames counter increments continuously (indicates successful audio decoding and rendering) | Verify pipeline returns to PLAYING state, audio sink renders audio frames at expected rate, frame counter increments (no stalls) |
| 10 | Validate Test Success and Release Resources | Query final playback position via . Verify test framework output shows `Failures: 0` and `Errors: 0`. Confirm audio frame statistics show continuous rendering without drops (rendered frames > threshold). Set pipeline to  via  to stop playback.  | Verify final position advances beyond seek point, test shows no failures/errors, audio frame rendering statistics confirm successful recovery, pipeline reaches GST_STATE_NULL, all resources released |
