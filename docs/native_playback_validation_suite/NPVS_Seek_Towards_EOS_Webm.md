...
| 2 | Create Playbin and Configure Video Sink | Create `playbin` element via `gst_element_factory_make("playbin", NULL)`. Configure URI via `g_object_set(playbin, "uri", webm_stream_url, NULL)`. Set `westerossink` as video sink | Playbin created, WebM URI configured, westerossink set |
