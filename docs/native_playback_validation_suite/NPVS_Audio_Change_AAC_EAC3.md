...
| 2 | Media Stream Provisioning | Multi-codec stream containing both AAC and EAC3 audio tracks must be accessible via HTTPS (`souphttpsrc`) or local file system (`filesrc`) with DASH manifest. Stream configured as `test_streams_base_path + "MultiCodecStreams/stream_with_aac_eac3.mpd"` in MediaValidationVariables.py | Verify multi-codec DASH stream accessible, both AAC and EAC3 audio tracks present and parseable |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_aac_eac3` configured in `MediaValidationVariables.py` with DASH manifest containing AAC and EAC3 audio tracks | Verify `video_src_url_aac_eac3` resolves to valid multi-codec stream with dual audio tracks |
...
| 2 | Configure Test Parameters and Load Multi-Codec Stream | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and `NATIVE_PLAYBACK_CHECK_AV_STATUS` from device. Build `mediapipelinetests` command with test_name="test_playback_audio_change", stream_url=video_src_url_aac_eac3, parameters: checkavstatus=yes/no, timeout=N | Verify device config retrieved successfully, command constructed with correct parameters |
...
| 7 | Verify Second EAC3 Audio Playback and Retrieve Audio-Tags | After seek-flush completes, playback continues with EAC3 codec now active. Emit "get-audio-tags" signal for EAC3 track via `g_signal_emit_by_name(playbin, "get-audio-tags", 1, &tags)`. Parse tag list using `gst_tag_list_foreach()` to identify codec name (EAC3). Log audio-tags including codec type, sample rate, channels. Compare logged tags from Step 5 (AAC) with current tags (EAC3) to confirm different codec | Verify audio-tags successfully retrieved showing EAC3 codec (e.g., audio/x-eac3, different from AAC), codec change confirmed through tag comparison |
