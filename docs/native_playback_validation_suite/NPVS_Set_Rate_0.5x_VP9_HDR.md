...
## Objective
Test to verify playback rate control at 0.5x speed using GStreamer `gst_element_seek()` function with VP9 HDR (High Dynamic Range) content in WebM container. Validate that position advances at exactly 0.5x speed (half normal speed) through periodic `gst_element_query_position()` polling with position increment expected at Â±25% tolerance around 0.05 seconds per 100ms interval. Verify WebM container parsing and HDR metadata preservation during rate change, audio/video remain synchronized, and frame rendering continues without discontinuities.
