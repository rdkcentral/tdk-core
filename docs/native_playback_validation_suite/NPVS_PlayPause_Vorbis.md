...
| 3 | Construct Vorbis WebM Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` property to `video_src_url_vorbis` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for Vorbis audio; Trigger state transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, WebM matroskademux active, first frame rendered |
