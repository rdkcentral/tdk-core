...
| 2 | Media Stream Provisioning | MP4 container with HEVC video at 24fps must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_24fps.mp4"` in MediaValidationVariables.py. Stream contains HEVC (H.265) video at 24fps and AAC audio (MP4 container format for HEVC 24fps play-pause testing) | Verify MP4 file is accessible and readable from configured path, qtdemux and HEVC decoder plugins available for playback at 24fps |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_hevc_24fps` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_24fps.mp4"` | Verify `video_src_url_hevc_24fps` resolves to valid MP4 container file with HEVC video at 24fps and AAC audio data |
...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_hevc_24fps` variable; Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` | Verify mediapipelinetests initializes with MP4 stream and cycle count configured |
| 3 | Construct H.265 MP4 Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` to `video_src_url_hevc_24fps` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio; Transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, qtdemux active, first frame rendered |
