...
## TestCase Name
NPVS_PlaySeek_Backward_Only_Audio_WAV
...
## Objective
Validate backward seek operation during audio-only WAV playback. The test verifies accurate positioning to the target seek point within ±1 second tolerance, seamless playback recovery after seek, audio rendering without interruption, and error-free playback.
...
| 2 | Audio Stream Provisioning | PCM WAV audio/x-wav stream must be accessible  with local file path or HTTP stream accessible to wavparse element. Stream file path: `test_streams_base_path + "TDK_Asset_Sunrise_WAV_Audio.wav"` from MediaValidationVariables.py | Verify PCM WAV audio/x-wav stream file is accessible, readable, and contains valid audio codec data with proper headers |
| 3 | Stream Variable Configuration | `audio_src_url_wav_pcm` must be configured in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_WAV_Audio.wav"` with accessible URL or local file path | Verify `audio_src_url_wav_pcm` variable resolves to valid, accessible audio stream location |
...
| 4 | Transition Pipeline to PLAYING State and Verify Audio Activation | Call  to transition pipeline. Wait for state change completion via . Monitor GStreamer bus via  for state transition messages (GST_MESSAGE_STATE_CHANGED). Verify `wavparse element` active and parsing audio stream from source. Register audio pad and verify caps via  showing valid audio codec, sample rate, and channel count | Verify playbin reaches ; `wavparse element` active and processing stream; Audio pad connected with valid caps; No GST_MESSAGE_ERROR on bus |
