...
**TestCase Name**
NPVS_Appsrc_Audio_Underflow_Playback_AAC
...
## Objective
Test to verify if buffer underflow signal is obtained upon audio/video underrun when using Appsrc and playback is smooth after buffer is filled again. Validate AAC stream playback through custom Appsrc pipeline with underflow detection and recovery capability. Specifically test with BYTES_THRESHOLD = 548409 bytes configuration to trigger controlled buffer starvation and verify `buffer-underflow-callback` signal detection and pipeline recovery via buffer refill operations.
...
| 4 | Start Appsrc Data Feeding and Transition to Playing State | Set pipeline to `GST_STATE_PAUSED` via `gst_element_set_state()`. Emit `push-buffer` action on Appsrc via `g_signal_emit_by_name()` until `BYTES_THRESHOLD = 548409 bytes` is reached. Transition to `GST_STATE_PLAYING` via `gst_element_set_state()`. Monitor first-frame signal to confirm rendering started | Verify pipeline state transitions to PAUSED then PLAYING, buffers pushed to Appsrc reach 548409 bytes threshold, first-frame signal is detected |
...
| 6 | Resume Data Feeding with Increased Buffer Threshold and Emit EOS | Resume emitting `push-buffer` action on Appsrc to accumulate `2 * BYTES_THRESHOLD = 1096818 bytes`. After all media data is transferred to Appsrc, emit `end-of-stream` action via `g_signal_emit_by_name()` | Verify buffers accumulated to 1096818 bytes threshold and `end-of-stream` action is emitted |
