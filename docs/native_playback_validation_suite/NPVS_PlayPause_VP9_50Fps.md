...
| 2 | Media Stream Provisioning | WebM container with VP9 video at 50fps must be accessible via local file system (`filesrc`) or HTTPS (`souphttpsrc`). Stream file configured as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_50fps.webm"` in MediaValidationVariables.py. Stream contains VP9 video at 50fps and Opus audio in WebM container format (for VP9 50fps play-pause testing) | Verify WebM file is accessible and readable from configured path, matroskademux and VP9 decoder plugins available for playback at 50fps |
| 3 | Stream Variable Configuration | Stream variable `video_src_url_vp9_50fps` must be configured in `MediaValidationVariables.py` with value: `test_streams_base_path + "TDK_Asset_Sunrise_VP9_50fps.webm"` | Verify `video_src_url_vp9_50fps` resolves to valid WebM container file with VP9 video at 50fps and Opus audio data |
...
| 2 | Configure and Execute Test Application | Retrieve `NATIVE_PLAYBACK_MEDIAPLAYBACK_TIMEOUT` and stream URL from `video_src_url_vp9_50fps` variable; Execute `mediapipelinetests test_play_pause_pipeline <URL> timeout=<seconds> cycles=3` | Verify mediapipelinetests initializes with WebM stream and cycle count configured |
| 3 | Construct VP9 WebM Pipeline | Create `playbin` element via `gst_element_factory_make()`; Configure `uri` to `video_src_url_vp9_50fps` via `g_object_set()`; Set `westerossink` as video sink; Set `autoaudiosink` for OPUS audio; Transition to `GST_STATE_PLAYING` | Verify playbin reaches `GST_STATE_PLAYING`, matroskademux active, first frame rendered |
