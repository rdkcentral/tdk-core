...
| 3 | Construct VP9 WebM Pipeline | Create `playbin` element via ; Configure `uri` to `video_src_url_vp9` via ; Set `westerossink` as video sink; Set `autoaudiosink` for OPUS audio;  | Verify playbin reaches , matroskademux active, first frame rendered |
| 4 | Query Video and Audio Properties | Query `westerossink→video-height/width` to confirm resolution; Query `playbin→n-audio` property to verify OPUS stream present; Log all stream properties | Verify video dimensions valid; Verify n-audio >= 1 confirming OPUS stream detected |
