...
| 3 | Construct H.265 DASH Pipeline | Create `playbin` element via ; Configure `uri` property to `video_src_url_hevc` via ; Set `westerossink` as video sink; Set `autoaudiosink` for AAC audio; Trigger state  | Verify playbin reaches , DASH dashdemux active, first frame rendered |
