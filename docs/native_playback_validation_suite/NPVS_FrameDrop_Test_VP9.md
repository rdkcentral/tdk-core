...
| 2 | Media Stream Provisioning | VP9 video stream must be accessible via local file system (`filesrc` element) or HTTPS (`souphttpsrc` element); stream file path: `TDK_Asset_Sunrise_VP9_30fps_30sec.webm` | Verify stream file is accessible and has 30fps frame rate with 30-second duration |
| 3 | Stream Variable Configuration | Stream URL must be configured in `MediaValidationVariables.py` as `video_src_url_mp4_frameDrop_vp9 = test_streams_base_path + "TDK_Asset_Sunrise_VP9_30fps_30sec.webm"` | Verify stream URL resolves to valid VP9 WebM file location |
...
| 3 | Create Playbin Pipeline and Set Stream URI | Create `playbin` element via `gst_element_factory_make()`, set stream URI to VP9 file path via `g_object_set(pipeline, "uri", ...)`, attach `westerossink` as video sink via `g_object_set(pipeline, "video-sink", westerossink, NULL)` | Verify `playbin` element is created and configured with correct stream URI and video sink |
...
| 6 | Monitor Playback Position and Frame Rate | Query playback position via `gst_element_query_position(pipeline, GST_FORMAT_TIME, &currentPosition)` to track playback progress at 1x rate; validate frame rendering rate matches stream fps (30fps for this VP9 stream) | Verify position advances at 1x rate without stalls or backward jumps; frame increments match expected rendering rate |
