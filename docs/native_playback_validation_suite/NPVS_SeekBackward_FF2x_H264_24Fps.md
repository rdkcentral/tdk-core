...
## Objective
Validate backward seek operation to prior position within already-played stream content. The test seeks backward from current position to an earlier timestamp, simulating user rewind behavior. Verify seek operation repositions without errors, position queries match the seek target within tolerance, and playback resumes seamlessly after seek without rendering discontinuities.
