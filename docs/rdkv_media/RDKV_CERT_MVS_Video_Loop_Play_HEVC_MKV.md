...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video loop playback of HEVC MKV video codec content
...
| 2 | Build the test URL using video_src_url_hevc_mkv_30sec. | Build the test URL with the video_src_url_hevc_mkv_30sec (MKV stream). | Verify that the test URL is built using video_src_url_hevc_mkv_30sec. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_hevc_mkv_30sec>.mkv&operations=checkloop(30)&autotest=true&type=mkv`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Run playback operations and validate media events. | App performs playback of HEVC MKV stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
