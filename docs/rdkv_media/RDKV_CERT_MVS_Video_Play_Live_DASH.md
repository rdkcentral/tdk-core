## TestCase ID
RDKV_Media_Validation_21
## TestCase Name
RDKV_CERT_MVS_Video_Play_Live_DASH
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of DASH live content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 60s (close_interval).<br>Build the test URL with the video_src_url_live_dash (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_live_dash>`<br>`&operations=close(60)&autotest=true&type=DASH` | Ensure that playback is configured to start content and close the player after 60s<br>(close_interval). Ensure the test URL is built with video_src_url_live_dash. Ensure the URL<br>is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Monitor and Validate Media Events | App should play the live DASH content and event video playing should occur | App should play the live DASH content and event video playing should occur. |
| 4 | Validate Event Result and Update Test Status | If expected event occurs, then app gives the validation result as SUCCESS or else FAILURE | Ensure that expected event occurs. Ensure that the test result is updated as SUCCESS. |
