...
RDKV_MEDIA_452
## TestCase Name
RDKV_CERT_MVS_Video_Repeated_Mute_UnMute_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager to play MKV video content and perform mute, unmute operations repeatedly for given number of times. The test confirms that the audio mutes and unmutes correctly, with each state change validated by a **volumechange** event. Video playback continues uninterrupted throughout both the mute and unmute operations.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_4k_mkv>&operations=mute(10),unmute(10),repeat(10)&autotest=true&type=mkv` | Ensure that the launch URL is stored in PersistentStore. |
