...
## Objective
Test Script to launch a HTML Video player application via AppManager and perform video seek position operation of MP4 content with AV1 and AAC codecs for few minutes and close the player
...
| 6 | Execute seek operations and validate media events. | App performs the provided operations and validates each operation using media events ('playing') | Ensure that expected media events are observed for the configured operations. |
