## TestCase ID
RDKV_Media_Validation_503
## TestCase Name
RDKV_CERT_MVS_Video_Mute_UnMute_4K_AV1
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager to play 4K AV1 video codec content and perform mute and unmute operations
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (30s), unmute audio (30s).<br>Build the test URL with the video_src_url_4k_av1.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_AV1_2160p.MP4`<br>`&operations=mute(30),unmute(30)&autotest=true` | Ensure that playback operations are configured correctly: Mute audio (30s), unmute audio (30s).<br>Ensure the test URL is built with video_src_url_4k_av1. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Mute / Unmute and Validate Media Events | App performs the mute and unmute operations and validates using events | Ensure that the mute and unmute operations and validates using events completes successfully<br>without errors. |
| 4 | Validate Event Result and Update Test Status | If expected event volumechange occurs for mute and unmute operations, then app gives the<br>validation result as SUCCESS or else FAILURE | Ensure that expected event volumechange occurs for mute and unmute operations. Ensure that the<br>test result is updated as SUCCESS. |
