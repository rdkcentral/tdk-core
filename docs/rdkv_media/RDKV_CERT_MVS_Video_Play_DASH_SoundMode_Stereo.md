...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of DASH content with stereo sound mode for few minutes and close the player
...
| 4 | Verify TV connection and stereo sound mode support. | TV should be connected with the test device. If the stereo sound mode is not supported then the test will be marked as failure. | Ensure that the TV is connected and stereo sound mode is supported; otherwise mark the test as failure. |
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Verify that get the current sound mode and set audio sound mode as stereo. | Get the current sound mode and set audio sound mode as stereo. | Ensure that get the current sound mode and set audio sound mode as stereo should be validated successfully. |
