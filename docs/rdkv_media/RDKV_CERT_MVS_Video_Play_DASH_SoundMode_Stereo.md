## TestCase ID
RDKV_Media_Validation_104
## TestCase Name
RDKV_CERT_MVS_Video_Play_DASH_SoundMode_Stereo
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of DASH content with stereo sound mode for few minutes and close the player
...
| 3 | Precondition Met | TV should be connected with the test device If the stereo sound mode is not supported then the<br>test will be marked as failure | Ensure that Condition is met successfully. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Execute Test Step | Get the current sound mode and set audio sound mode as stereo | Ensure that Step executed successfully. |
