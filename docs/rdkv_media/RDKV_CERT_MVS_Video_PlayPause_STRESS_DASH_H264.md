## TestCase ID
RDKV_Media_Validation_12
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_STRESS_DASH_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operations of DASH h264 video codec content continuously for given number of times
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat.<br>Build the test URL with the video_src_url_dash_h264 (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd`<br>`&operations=pause,play,repeat&autotest=true&type=DASH` | Ensure that playback operations are configured correctly: Pause, play, repeat. Ensure the test<br>URL is built with video_src_url_dash_h264. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the pause and play operation of DASH h264 video codec stream repeatedly and<br>validates using events | Ensure that the pause and play operation of DASH h264 video codec stream repeatedly and<br>validates using events completes successfully without errors. |
