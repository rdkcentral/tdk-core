...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of DASH content with 480p display resolution for few minutes and close the player
...
| 4 | Verify TV connection and 480p resolution support. | TV should be connected with the test device. If the 480p resolution is not supported then the test will be marked as failure. | Ensure that the TV is connected and 480p resolution is supported; otherwise mark the test as failure. |
