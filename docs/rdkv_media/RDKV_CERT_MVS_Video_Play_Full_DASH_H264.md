...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operations of dash h264 video codec content till the end
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Play till end of content (0s). Build the test URL with the video_src_url_short_duration_dash (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC_15Sec/master.mpd&operations=playtillend(0)&autotest=true&type=dash` | Verify that playback operations are configured correctly: Play till end of content (0s). Confirm the test URL is built with video_src_url_short_duration_dash. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
