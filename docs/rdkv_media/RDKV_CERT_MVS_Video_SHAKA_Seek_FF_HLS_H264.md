## TestCase ID
RDKV_Media_Validation_721
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Seek_FF_HLS_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Shaka player application via AppManager and perform video medium trick play operations like seek and FF of HLS h264 video codec content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward (30s), fast-forward (20s), resume playback (30s).<br>Build the test URL with the video_src_url_hls_h264.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=seekfwd(30),fastfwd(20),playnow(30)&autotest=true` | Ensure that playback operations are configured correctly: Seek forward (30s), fast-forward<br>(20s), resume playback (30s). Ensure the test URL is built with video_src_url_hls_h264.<br>Ensure the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
