## TestCase ID
RDKV_Media_Validation_169
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_STRESS_Audio_Only
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform  play pause operations of Audio only stream continuously for given number of times
...
| 6 | Audio URL Type Configured | MediaValidationVariables.audio_url_type should be set to the audio stream type value. | Ensure that MediaValidationVariables.audio_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat.<br>Build the test URL with the video_src_url_audio.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_AAC_Audio_Only/master.mpd`<br>`&operations=pause,play,repeat&autotest=true` | Ensure that playback operations are configured correctly: Pause, play, repeat. Ensure the test<br>URL is built with video_src_url_audio. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the pause and play operation repeatedly and validates using events | Ensure that the pause and play operation repeatedly and validates using events completes<br>successfully without errors. |
| 4 | Validate Event Result and Update Test Status | If expected events occurs for pause and play in all the repetition, then app gives the<br>validation result as SUCCESS or else FAILURE | Ensure that expected events occurs for pause and play in all the repetition. Ensure that the<br>test result is updated as SUCCESS. |
