...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, stop, replay from beginning, stop immediately, start playback, stop. Build the test URL for the Animation app. Store the constructed URL in PersistentStore for AppManager launch. Sample URL:<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/animations/build/index.html?operations=pause,play,stop,replay,stopNow,start,stop&autotest=true` | Verify that playback operations are configured correctly: Pause, play, stop, replay from beginning, stop immediately, start playback, stop. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Configured Operations and Validate Events | App performs the provided operations and validates each operation using events | Verify that the provided operations and validates each operation using events completes successfully without errors. |
| 4 | Validate Event Result and Update Test Status | Monitor animation progress. Validates the 'pause, play, stop, replay, stopNow, start, stop' event (animation completes successfully) and retrieves the validation result with average CPU load value. Get the event validation result from the app and update the test script status | Confirm the 'pause, play, stop, replay, stopNow, start, stop' event is received and animation completes successfully. Verify that the validation result and average CPU load value are retrieved. |
