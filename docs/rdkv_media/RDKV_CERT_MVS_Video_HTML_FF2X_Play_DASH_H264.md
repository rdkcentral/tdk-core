...
## Objective
Test script to launch a HTML5 Player application via AppManager and perform fast-forward at 2x speed and resume playback of DASH content, validating the expected media events and player behaviour.
...
| 6 | Execute fast-forward/rewind operations and validate media events. | App performs the configured operations and validates using media events. The 'playing' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
