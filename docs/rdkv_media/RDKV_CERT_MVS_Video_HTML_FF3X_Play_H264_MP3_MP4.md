...
RDKV_MEDIA_837
## TestCase Name
RDKV_CERT_MVS_Video_HTML_FF3X_Play_H264_MP3_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform video fast forward operation of MP4 content with H264 and MP3 codecs with playback rate 3x for few minutes and close the player. The test confirms that the video fast-forwards at three times the normal speed and that the video position advances at least three times faster than normal. It also confirms that normal speed playback resumes correctly after the fast-forward period.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_h264_mp3>&operations=fastfwd3x(30),playnow(10),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
