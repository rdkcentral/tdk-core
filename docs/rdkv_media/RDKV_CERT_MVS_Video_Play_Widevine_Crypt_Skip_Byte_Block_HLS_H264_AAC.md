...
RDKV_MEDIA_1095
## TestCase Name
RDKV_CERT_MVS_Video_Play_Widevine_Crypt_Skip_Byte_Block_HLS_H264_AAC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operation of Widevine DRM protected Crypt Skip Byte Block (CENC partial encryption) H264 codec HLS stream for few minutes and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 5 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_widevine_crypt_skip_block_hls_h264_aac` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "HLS_H264_Crypt_block/hls_h264_master.m3u8"`, providing the test stream used for playback testing. | Verify that `video_src_url_widevine_crypt_skip_block_hls_h264_aac` resolves to a valid, accessible stream location for this test. |
| 6 | Verify that the DRM configuration is set correctly. | DRM configuration variable `video_src_url_widevine_crypt_skip_block_hls_h264_aac_drmconfigs` must be configured in `MediaValidationVariables.py` with the Widevine license server URL and any required HTTP headers. Use the format: `com.widevine.alpha[<license_url>]\|headers[<header_name>:<header_value>]`. This variable is environment-specific and must be set before running the test. | Verify that `video_src_url_widevine_crypt_skip_block_hls_h264_aac_drmconfigs` is configured with valid Widevine DRM license server details. |
| 7 | Check whether the app is already installed on the device. | Query the installed package list using the following request: <br><code>{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.1.listPackages"}</code>. | Verify that the app is installed on the device. |
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Verify that Widevine DRM is supported on the device. | Check the OCDM plugin status and activate it if not already active. Query the list of supported DRMs using the following request: <br><code>{"jsonrpc": "2.0", "id": 1234567890, "method": "OCDM.1.drms"}</code>. Verify that Widevine is present in the returned DRM list. If Widevine is not supported, the test is marked as Not Applicable and skipped. | Widevine DRM must be supported on the device. If not supported, the test is marked as Not Applicable. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_widevine_crypt_skip_block_hls_h264_aac>&operations=close(30)&autotest=true&type=hls` | Ensure that the launch URL is stored in PersistentStore. |
