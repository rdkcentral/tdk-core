...
RDKV_MEDIA_780
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Play_Full_Audio_Only
<a name="head.TOC"></a>
## Table Of Contents
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_short_duration_audio` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_AAC_Audio_Only_30Sec/master.mpd"`, providing the test stream used for playback testing. | Verify that `video_src_url_short_duration_audio` resolves to a valid, accessible stream location for this test. |
