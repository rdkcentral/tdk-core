...
## Objective
Test Script to launch a HTML Video player application via AppManager and perform video seek backward operation of MP4 content with H264 and AC3 codecs for few minutes and close the player
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_h264_ac3>&operations=seekbwd(60),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
