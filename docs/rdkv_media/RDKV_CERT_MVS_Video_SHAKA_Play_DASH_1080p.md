...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_dash. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_dash>&operations=close(60)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
