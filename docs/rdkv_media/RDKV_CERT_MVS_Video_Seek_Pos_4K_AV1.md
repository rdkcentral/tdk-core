...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video seek to position operation of 4K AV1 video codec content
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Configure the playback operations for the scenario. | Configure the `seekpos(30:120),seekpos(30:80),close(30)` operations: `seekpos(30:120)` seeks the video between 30 and 120 seconds, `seekpos(30:80)` seeks the video between 30 and 80 seconds, and closes the player after 30 seconds. | Ensure playback operations are set as specified. |
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_4k_av1. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_4k_av1_url>.mp4&operations=seekpos(30:120),seekpos(30:80),close(30)&autotest=true&options=checkInterval(10)&type=mp4`| Ensure that the launch URL is stored in PersistentStore. |
