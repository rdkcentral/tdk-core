## TestCase ID
RDKV_Media_Validation_706
## TestCase Name
RDKV_CERT_MVS_Video_Loop_Play_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of MP4 video codec content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s). Build the test URL with the video_src_url_short_duration_mp4 (MP4 stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_MP4_30Secs.mp4&operations=checkloop(60)&autotest=true&type=mp4` | Verify that playback operations are configured correctly: Check loop state (60s). Confirm the test URL is built with video_src_url_short_duration_mp4. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
