## TestCase ID
RDKV_Media_Validation_24
## TestCase Name
RDKV_CERT_MVS_Video_Play_HLS_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of HLS h264 video codec content for few minutes and close the player
...
| 3 | Perform Play Content and Validate Media Events | App starts playing the HLS h264 video codec stream and closes the player after the provided<br>duration. The 'playing' event is monitored to validate successful playback. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
