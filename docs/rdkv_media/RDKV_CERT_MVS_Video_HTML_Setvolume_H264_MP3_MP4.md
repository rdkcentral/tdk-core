...
## Objective
Test script to launch an HTML video player application via AppManager and perform volume level operation on MP4 content with h264 and mp3 codecs for a few minutes and then close the player.
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Set volume (5s), close the player.<br>Build the test URL with the video_src_url_h264_mp3.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Set volume (5s), close the player.<br>Ensure the test URL is built with video_src_url_h264_mp3. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
