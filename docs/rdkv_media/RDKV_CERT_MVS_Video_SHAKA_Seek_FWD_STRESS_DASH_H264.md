## TestCase ID
RDKV_Media_Validation_745
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Seek_FWD_STRESS_DASH_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Shaka player application via AppManager and perform video seek forward operation of DASH h264 video codec content repeatedly for given number of times in provided interval
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, repeat.<br>Build the test URL with the video_src_url_dash_h264.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=shaka`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd`<br>`&operations=seekfwd,repeat&autotest=true` | Ensure that playback operations are configured correctly: Seek forward, repeat. Ensure the<br>test URL is built with video_src_url_dash_h264. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 3 | Perform Configured Operations and Validate Events | App performs the provided operations and validates each operation using media events ('Observed<br>Event: ') | Ensure that the app performs all configured operations and validates them using media events.<br>Operations should complete successfully without errors. |
