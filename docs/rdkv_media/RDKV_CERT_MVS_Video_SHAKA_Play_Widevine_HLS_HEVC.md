...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_hls_hevc. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<widevine_hls_hevc_url>.m3u8&drmconfigs=com.widevine(license_url)&operations=close(60)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
