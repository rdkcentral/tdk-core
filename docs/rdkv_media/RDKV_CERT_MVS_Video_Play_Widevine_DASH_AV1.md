...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operation of Widevine DRM protected AV1 codec DASH stream for few minutes and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 5 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_widevine_dash_av1` must be configured in `MediaValidationVariables.py` with the appropriate stream URL for this test. This variable is environment-specific and must be set to a valid, accessible URL before running the test. | Verify that `video_src_url_widevine_dash_av1` is configured with a valid, accessible stream URL in `MediaValidationVariables.py`. |
| 6 | Verify that the DRM configuration is set correctly. | DRM configuration variable `video_src_url_widevine_dash_av1_drmconfigs` must be configured in `MediaValidationVariables.py` with the Widevine license server URL and any required HTTP headers. Use the format: `com.widevine.alpha[<license_url>]\|headers[<header_name>:<header_value>]`. This variable is environment-specific and must be set before running the test. | Verify that `video_src_url_widevine_dash_av1_drmconfigs` is configured with valid Widevine DRM license server details. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_widevine_dash_av1>.mpd&operations=close(30)&autotest=true&type=dash` | Ensure that the launch URL is stored in PersistentStore. |
