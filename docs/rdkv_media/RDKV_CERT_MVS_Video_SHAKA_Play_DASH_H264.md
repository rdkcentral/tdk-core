## TestCase ID
RDKV_Media_Validation_51
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Play_DASH_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Shaka player application via AppManager and perform video play operation of DASH H.264 codec stream for few minutes and close the player
...
| 3 | Perform Play Content and Validate Media Events | App starts playing the DASH h. The 'playing' event is monitored to validate successful<br>playback.264 video codec stream and closes the player after the provided duration. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
