...
## Objective
Test Script to launch a lightning Shaka player application via AppManager and perform video play operation of DASH H264 codec stream for few minutes and close the player
...
| 6 | Run playback operations and validate media events. | App starts playing the dash h.264 video codec stream and closes the player after the provided duration. The 'playing' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
