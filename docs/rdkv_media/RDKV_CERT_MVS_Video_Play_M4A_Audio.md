...
RDKV_MEDIA_224
## TestCase Name
RDKV_CERT_MVS_Video_Play_M4A_Audio
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform audio play operation for M4a audio content for few minutes and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 4 | Verify that the required configuration is set correctly. | MediaValidationVariables.audio_src_url_m4a should be set to the M4A audio stream URL. | Verify that MediaValidationVariables.audio_src_url_m4a is configured with the required value. |
...
| 6 | Verify that the stream variable is configured correctly. | Stream variable `audio_src_url_m4a` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_M4A.m4a"`, providing the test stream used for playback testing. | Verify that `audio_src_url_m4a` resolves to a valid, accessible stream location for this test. |
