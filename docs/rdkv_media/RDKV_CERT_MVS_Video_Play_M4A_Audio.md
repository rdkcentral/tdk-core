...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform audio play operation for M4a audio content for few minutes and close the player.
...
| 3 | Audio Src URL M4a Configured | MediaValidationVariables.audio_src_url_m4a should be set to the M4A audio stream URL. | Ensure that MediaValidationVariables.audio_src_url_m4a is configured with the required value. |
