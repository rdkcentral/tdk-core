## TestCase ID
RDKV_Media_Validation_20
## TestCase Name
RDKV_CERT_MVS_Video_Seek_BWD_STRESS_HLS_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video seek backward operation of HLS h264 video codec content continuously for given number of times in provided interval
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek backward (420s) (2 times), seek backward, repeat.<br>Build the test URL with the video_src_url_hls_h264 (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=seekbwd(420),repeat(1),seekbwd,repeat&autotest=true&type=HLS` | Ensure that playback operations are configured correctly: Seek backward (420s) (2 times), seek<br>backward, repeat. Ensure the test URL is built with video_src_url_hls_h264. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
