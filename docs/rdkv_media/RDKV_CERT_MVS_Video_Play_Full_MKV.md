...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Play till end of content (0s).<br>Build the test URL with the video_src_url_short_duration_mkv (MKV stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_2160p_30Secs.MKV`<br>`&operations=playtillend(0)&autotest=true&type=MKV` | Ensure that playback operations are configured correctly: Play till end of content (0s).<br>Ensure the test URL is built with video_src_url_short_duration_mkv. Ensure the URL is stored<br>in PersistentStore (MVS/lightningURL) successfully. |
