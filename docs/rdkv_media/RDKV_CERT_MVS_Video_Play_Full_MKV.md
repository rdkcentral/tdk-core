...
RDKV_MEDIA_391
## TestCase Name
RDKV_CERT_MVS_Video_Play_Full_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operations of MKV content till the end. The test confirms that the stream plays from beginning to natural end without interruption. Successful playback start and natural stream completion are both validated, with the video position advancing continuously throughout.
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_short_duration_mkv` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_2160p_30Secs.mkv"`, providing the test stream used for playback testing. | Verify that `video_src_url_short_duration_mkv` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_short_duration_mkv>&operations=playtillend(0)&autotest=true&type=mkv` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Run full video playback until stream end and capture media events. | The player opens a short-duration stream and plays it through to the natural end. A **play** event confirms playback has started. The video position advances continuously, and when the stream finishes naturally, an **ended** event fires to confirm complete playback. No manual close operation is issued. | Ensure that the playback start and stream end events are captured. |
| 6 | Validate observed events for the full video playback operation and update test result. | If a **play** event fires at the start and an **ended** event fires at natural stream completion, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the full video playback event validation and proc check status. |
