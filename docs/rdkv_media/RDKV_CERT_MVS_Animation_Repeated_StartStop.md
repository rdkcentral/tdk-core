...
RDKV_MEDIA_02
## TestCase Name
RDKV_CERT_MVS_Animation_Repeated_StartStop
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Lightning Animation application via AppManager and perform animation start and stop operation repeatedly for given number of times. The test confirms that the animation app can reliably stop and restart playback across the configured number of repetitions without any degradation or errors.
...
| 5 | Run start and stop repeated operations and capture events. | The animation app repeatedly executes stop and start operations for the configured number of repeated iterations. Each stop and start cycle is validated by monitoring the expected state change events. The test confirms that the animation responds consistently across all repetitions without any degradation. | Ensure that the start and stop events are captured for all repeated iterations. |
| 6 | Validate captured start and stop events and update test result. | If the expected events are received for all stop and start cycles across all repeated iterations, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation result and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the captured start and stop repeated events. |
