## TestCase ID
RDKV_Media_Validation_60
## TestCase Name
RDKV_CERT_MVS_Video_Play_Audio_Only
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform audio play operation for few minutes and close the player
...
| 3 | Audio Close Interval Configured | MediaValidationVariables.audio_close_interval should be set to the audio close interval value<br>(in seconds). | Ensure that MediaValidationVariables.audio_close_interval is configured with the required<br>value. |
| 4 | Audio URL Type Configured | MediaValidationVariables.audio_url_type should be set to the audio stream type value. | Ensure that MediaValidationVariables.audio_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration<br>(close_interval).<br>Build the test URL with the video_src_url_audio.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_AAC_Audio_Only/master.mpd`<br>`&operations=close&autotest=true` | Ensure that playback is configured to start content and close the player after the configured<br>duration (close_interval). Ensure the test URL is built with video_src_url_audio. Ensure the<br>URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Play Content and Validate Media Events | App starts playing the audio and closes the player after the provided duration. The 'playing'<br>event is monitored to validate successful playback. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
