...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat.<br>Build the test URL with the video_src_url_hevc.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC/atfms_291_dash_tdk_hevc_aac_fmp4.mpd`<br>`&operations=pause,play,repeat&autotest=true` | Ensure that playback operations are configured correctly: Pause, play, repeat. Ensure the test<br>URL is built with video_src_url_hevc. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the pause and play operation of HEVC stream repeatedly and validates using events | Ensure that the pause and play operation of HEVC stream repeatedly and validates using events<br>completes successfully without errors. |
| 4 | Validate Event Result and Update Test Status | If expected events paused and play occurs for pause and play in all the repetition, then app<br>gives the validation result as SUCCESS or else FAILURE | Ensure that expected events paused and play occurs for pause and play in all the repetition.<br>Ensure that the test result is updated as SUCCESS. |
