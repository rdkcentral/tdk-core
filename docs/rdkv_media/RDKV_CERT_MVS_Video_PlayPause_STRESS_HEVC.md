...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operations of HEVC codec stream continuously for given number of times
...
| 6 | Run play/pause operations and validate events. | App performs the pause and play operation of HEVC stream repeatedly and validates using events. | Ensure that play/pause operations complete and expected events are observed. |
| 7 | Validate observed events and set pass/fail status. | If expected events paused and play occurs for pause and play in all the repetition, then the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
