...
|3| Av1 URL Type FPS Configured | MediaValidationVariables.av1_url_type_fps should be set to the AV1 (FPS) stream type value.| MediaValidationVariables.av1_url_type_fps is configured with the required value.|
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (96s). Build the test URL with the video_src_url_mp4_2160p_av1. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Waterfall_4K_AV1.mp4&operations=checkloop(96)&autotest=true` | Verify that playback operations are configured correctly: Check loop state (96s). Confirm the test URL is built with video_src_url_mp4_2160p_av1. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Loop / Play Content and Validate Media Events | App performs playback of 4k av1 stream. The 'playing' event is monitored to validate successful playback. since loop option is enabled, playback continues from the start after reaching the end. app validates using events and position. loop count is calculated based on number of video position 0 occurred. | Verify that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
| 4 | Validate Event Result and Update Test Status | If expected events playing occurs and video loop count is 1, then app gives the validation result as SUCCESS or else FAILURE | Confirm that expected events playing occurs and video loop count is 1. Check that the test result is updated as SUCCESS. |
