## TestCase ID
RDKV_Media_Validation_715
## TestCase Name
RDKV_CERT_MVS_Video_Loop_Play_4K_AV1
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of 4K AV1 video codec content
...
| 3 | AV1 URL Type FPS Configured | MediaValidationVariables.av1_url_type_fps should be set to the AV1 (FPS) stream type value. | Ensure that MediaValidationVariables.av1_url_type_fps is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (96s).<br>Build the test URL with the video_src_url_mp4_2160p_av1.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Waterfall_4K_AV1.MP4`<br>`&operations=checkloop(96)&autotest=true` | Ensure that playback operations are configured correctly: Ensure loop state (96s). Ensure the<br>test URL is built with video_src_url_mp4_2160p_av1. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Loop / Play Content and Validate Media Events | App performs playback of 4K AV1 stream. The 'playing' event is monitored to validate successful<br>playback. since loop option is enabled, playback continues from the start after reaching the<br>end. app validates using events and position. loop count is calculated based on number of video<br>position 0 occurred. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
