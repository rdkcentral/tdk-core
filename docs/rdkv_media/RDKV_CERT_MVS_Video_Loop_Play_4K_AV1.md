...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video loop playback of 4K AV1 video codec content
...
| 4 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.av1_url_type_fps should be set to the AV1 (FPS) stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 6 | Run playback operations and validate media events. | App performs playback of 4K AV1 stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
