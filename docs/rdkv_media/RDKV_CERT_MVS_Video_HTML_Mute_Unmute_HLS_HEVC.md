## TestCase ID
RDKV_Media_Validation_1070
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Mute_Unmute_HLS_HEVC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test script to launch a HTML5 Player application via AppManager and perform mute audio and unmute audio of HLS content, validating the expected media events and player behaviour.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_4k_hls>&operations=mute(30),unmute(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Run mute/unmute operations and validate events. | App performs the configured operations and validates using media events. The 'Observed Event: play' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
