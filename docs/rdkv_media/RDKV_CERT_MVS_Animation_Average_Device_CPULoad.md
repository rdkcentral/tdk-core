...
| 4 | Validate Event Result and Update Test Status | Monitor animation progress. Validates the 'stop' event (animation completes successfully) and retrieves the validation result with average CPU load value. | Confirm the 'stop' event is received and animation completes successfully. Verify that the validation result and average CPU load value are retrieved. |
| 5 | Check CPU Performance Against Threshold | Get the event validation result and average CPU value from the app and check whether CPU is lesser than 90 | Verify that cpu is lesser than 90 as expected. |
