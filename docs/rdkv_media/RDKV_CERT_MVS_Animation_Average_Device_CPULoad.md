...
## Objective
To launch a Lightning Animation application via AppManager and check whether the average device CPU load value calculated for 60 sec duration is as expected. The test confirms that the average device CPU load during animation rendering over the configured duration is within the expected threshold.
...
| 5 | Run animation rendering and capture CPU load metrics. | The animation app renders the configured animation for the specified duration. The device CPU load is monitored periodically during rendering. After the configured stop time, the animation halts and the average CPU load is calculated from all measurements collected during the rendering period. | Ensure that the CPU load measurements are captured during the animation rendering period. |
| 6 | Validate average CPU load result and update test result. | If the calculated average device CPU load is within the expected threshold, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on the CPU load validation result and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the CPU load validation result. |
