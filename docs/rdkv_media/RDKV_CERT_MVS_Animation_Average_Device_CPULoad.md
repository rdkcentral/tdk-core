...
| 2 | Build the animation app test URL. | Build the URL as follows:<br>`http://<TM_IP>:/tdkservice/fileStore/lightning-apps/animations/build/index.html?operations=stop(60)&autotest=true` | Verify that the test URL is built successfully. |
| 3 | Store the launch URL in PersistentStore. | Set the MVS/lightningURL in PersistentStore with the generated URL before app launch. | Ensure that the launch URL is stored in PersistentStore. |
