...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video loop playback of 4K VP9 video codec content
...
| 4 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 2 | Build the test URL using video_src_url_short_duration_4k_vp9. | Build the test URL with the video_src_url_short_duration_4k_vp9. | Verify that the test URL is built using video_src_url_short_duration_4k_vp9. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_short_duration_4k_vp9>.webm&operations=checkloop(30)&autotest=true&type=webm`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Run playback operations and validate media events. | App performs playback of 4K VP9 stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
