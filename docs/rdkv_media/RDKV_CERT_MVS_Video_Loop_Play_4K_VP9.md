...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of 4K VP9 video codec content
...
| 3 | VP9 4K URL Type Configured | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value. | Ensure that MediaValidationVariables.vp9_4k_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s).<br>Build the test URL with the video_src_url_short_duration_4k_vp9.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_VP9_2160p_30Sec.webm`<br>`&operations=checkloop(60)&autotest=true` | Ensure that playback operations are configured correctly: Ensure loop state (60s). Ensure the<br>test URL is built with video_src_url_short_duration_4k_vp9. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
