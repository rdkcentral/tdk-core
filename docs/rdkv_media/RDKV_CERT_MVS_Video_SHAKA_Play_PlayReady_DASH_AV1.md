...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_playready_dash_av1. Store the constructed URL in PersistentStore for AppManager launch. Sample URL:<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_playready_dash_av1>&operations=close&autotest=true` | Verify that playback is configured to start content and close the player after the configured duration (close_interval). Confirm the test URL is built with video_src_url_playready_dash_av1. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
