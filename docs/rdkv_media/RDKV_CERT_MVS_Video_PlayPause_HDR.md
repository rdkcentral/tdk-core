...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operation of HDR content
...
| 7 | Validate observed events and set pass/fail status. | If expected events video pause and play occurs for pause and play operations, then the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
