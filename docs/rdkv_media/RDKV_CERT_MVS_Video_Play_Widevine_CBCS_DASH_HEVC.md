...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_cbcs_dash_hevc_aac (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br><code>http://&lt;TM_IP&gt;:&lt;port&gt;/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html<br>?player=sdk<br>&url=http://&lt;TM_IP&gt;:&lt;port&gt;/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC_CBCS/master.mpd<br>&operations=close<br>&autotest=true<br>&type=DASH</code>. | Ensure that the launch URL is stored in PersistentStore. |
