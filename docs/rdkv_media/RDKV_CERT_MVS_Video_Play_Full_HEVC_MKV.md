...
RDKV_MEDIA_589
## TestCase Name
RDKV_CERT_MVS_Video_Play_Full_HEVC_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_hevc_mkv_30sec` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_HEVC_30Sec_MKV.mkv"`, providing the test stream used for playback testing. | Verify that `video_src_url_hevc_mkv_30sec` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_hevc_mkv_30sec>&operations=playtillend(0)&autotest=true&type=mkv` | Ensure that the launch URL is stored in PersistentStore. |
