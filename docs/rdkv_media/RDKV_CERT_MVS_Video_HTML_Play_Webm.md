## TestCase ID
RDKV_Media_Validation_98
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Play_Webm
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test cript to launch a HTML Video player application through AppManager and perform video play operation of direct WEBM content for few minutes and close the player
...
| 4 | Run playback operations and validate media events. | App starts playing the webm video and closes the player after the provided duration. The 'playing' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
