...
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operation of MKV codec stream
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the play pause operations of MKV codec stream and validates each operation using<br>media events ('Observed Event: play') | Ensure that the app performs all configured operations and validates them using media events.<br>Operations should complete successfully without errors. |
