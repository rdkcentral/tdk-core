...
RDKV_MEDIA_1073
## TestCase Name
RDKV_CERT_MVS_Video_HTML_SHAKA_Seek_FWD_DASH_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform seek forward of DASH content, validating the expected media events and player behaviour. The test confirms that a forward seek completes successfully, with the video repositioning to the expected location within the acceptable tolerance range. Both the seek request and seek completion are validated through media events.
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `seekfwd(30),close(30)` operations: the video player will seek forward for 30 seconds, then close the player after 30 seconds. | Ensure playback operations are set as specified. |
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_dash>.mpd&operations=seekfwd(30),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Execute seek operations and capture media events. | The player seeks the video forward by the configured seek interval from the current position. A **seeking** event fires immediately when the seek is requested, followed by a **seeked** event once the video repositions to the new location. The resulting video position is validated to be within the acceptable range of the expected forward seek target. | Ensure that the expected media events are captured for the configured operations. |
| 6 | Validate observed events for the seek forward operation and update test result. | If **seeking** and **seeked** events are received and the video position falls within the acceptable range of the forward seek target after the operation, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the seek forward event validation and proc check status. |
