...
## Objective
Test script to launch a HTML5 Player application via AppManager and perform seek backward of HLS content, validating the expected media events and player behaviour.
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek backward (60s), close the player.<br>Build the test URL with the video_src_url_hls.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Seek backward (60s), close the<br>player. Ensure the test URL is built with video_src_url_hls. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
