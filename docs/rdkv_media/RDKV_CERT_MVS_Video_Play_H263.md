...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of H263 codec stream for few minutes and close the player
...
| 5 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.h263_url_type should be set to the H263 stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_h263. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br><code>http://&lt;TM_IP&gt;:&lt;port&gt;/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html<br>?player=sdk<br>&url=http://&lt;TM_IP&gt;:&lt;port&gt;/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_H263_AAC.mov<br>&operations=close<br>&autotest=true</code>. | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Run playback operations and validate media events. | App starts playing the h. The 'playing' event is monitored to validate successful playback.263 stream video and c loses the player after the provided duration. | Ensure that expected media events are observed for the configured operations. |
