...
|4| H263 URL Type Configured | MediaValidationVariables.h263_url_type should be set to the H263 stream type value.| MediaValidationVariables.h263_url_type is configured with the required value.|
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_h263. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_H263_AAC.mov&operations=close&autotest=true` | Verify that playback is configured to start content and close the player after the configured duration (close_interval). Confirm the test URL is built with video_src_url_h263. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
