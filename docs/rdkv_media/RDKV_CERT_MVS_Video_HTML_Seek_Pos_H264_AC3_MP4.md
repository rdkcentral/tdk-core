...
## Objective
Test Script to launch a HTML Video player application through AppManager and perform video seek position operation of MP4 content with H264 and AC3 codecs for few minutes and close the player
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Set playback operations: Seek forward (30s), seek backward (30s), close the player. | Ensure playback operations are set as specified. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_h264_ac3_mp4_url>&operations=seekfwd(30),seekbwd(30),close(30)`| Ensure that the launch URL is stored in PersistentStore. |
