...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward (30s), seek backward (30s), close the player.<br>Build the test URL with the video_src_url_vorbis_vp9.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Seek forward (30s), seek backward<br>(30s), close the player. Ensure the test URL is built with video_src_url_vorbis_vp9. Ensure<br>the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
