...
## Objective
Test script to launch an HTML video player application via AppManager and perform mute and unmute operations on webm content with av1 and Vorbis codecs for a few minutes and then close the player.
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (30s), unmute audio (30s). Build the test URL with the video_src_url_av1_vorbis. Store the constructed URL in PersistentStore for AppManager launch. | Verify that playback operations are configured correctly: Mute audio (30s), unmute audio (30s). Confirm the test URL is built with video_src_url_av1_vorbis. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
