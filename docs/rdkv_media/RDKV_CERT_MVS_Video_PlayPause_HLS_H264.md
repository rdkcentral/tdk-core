...
| 6 | Execute operations and validate media events. | App performs the play, pause operations of hls h264 video codec stream and validates each operation using media events ('Observed Event: play') | Ensure that expected media events are observed for the configured operations. |
