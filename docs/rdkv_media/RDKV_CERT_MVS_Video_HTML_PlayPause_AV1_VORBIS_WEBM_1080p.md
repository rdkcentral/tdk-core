...
## Objective
Test Script to launch a HTML Video player application through AppManager and perform video play pause operation on WEBM content with AV1 and VORBIS codecs with 1080p resolution for few minutes and then close the player.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_av1_vorbis_webm_url>&operations=pause(30),play(10)`| Ensure that the launch URL is stored in PersistentStore. |
