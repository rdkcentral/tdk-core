...
## Objective
To launch a HTML Video Player application via AppManager and perform video fast forward operation of WEBM content with OPUS and VP8 codecs with playback rate 4x for few minutes and close the player. The test confirms that the video fast-forwards at four times the normal speed and that the video position advances at least four times faster than normal. It also confirms that normal speed playback resumes correctly after the fast-forward period.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_vp8>&operations=fastfwd4x(30),playnow(10),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
