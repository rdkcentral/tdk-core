...
## Objective
To launch a Lightning Animation application via AppManager and perform animation start and stop operation repeatedly for given number of times. The test confirms that the animation app can reliably stop and restart playback across the configured number of stress repetitions without any degradation or errors.
...
| 4 | Verify that the required interval configuration is set correctly. | MediaValidationVariables.pause_interval_stress should be set to the pause interval value for stress tests (in seconds). | Ensure that the required configuration value is set correctly. |
| 5 | Verify that the required interval configuration is set correctly. | MediaValidationVariables.play_interval_stress should be set to the play interval value for stress tests (in seconds). | Ensure that the required configuration value is set correctly. |
| 6 | Verify that the required configuration is set correctly. | MediaValidationVariables.repeat_count_stress should be set to the repeat count for stress tests. | Verify that MediaValidationVariables.repeat_count_stress is configured with the required value. |
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `stop(10),start(10),repeat(10)` operations: the animation will render for 10 seconds and then stop, then start the animation again after 10 seconds, then repeat the preceding operations 10 times. | Ensure playback operations are set as specified. |
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/animations/build/index.html?operations=stop(10),start(10),repeat(10)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Run start and stop stress operations and capture events. | The animation app repeatedly executes stop and start operations for the configured number of stress iterations. Each stop and start cycle is validated by monitoring the expected state change events. The test confirms that the animation responds consistently across all repetitions without any degradation. | Ensure that the start and stop events are captured for all stress iterations. |
| 6 | Validate captured start and stop events and update test result. | If the expected events are received for all stop and start cycles across all stress iterations, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation result and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the captured start and stop stress events. |
