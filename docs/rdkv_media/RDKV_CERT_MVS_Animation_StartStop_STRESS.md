...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Stop, start playback, repeat. Build the test URL for the Animation app. Store the constructed URL in PersistentStore for AppManager launch. Sample URL:<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/animations/build/index.html?operations=stop,start,repeat&autotest=true` | Verify that playback operations are configured correctly: Stop, start playback, repeat. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Configured Operations and Validate Events | App performs the stop and start operation repeatedly and validates using events | Verify that the stop and start operation repeatedly and validates using events completes successfully without errors. |
| 4 | Validate Event Result and Update Test Status | Monitor animation progress. Validates the 'stop, start, repeat' event (animation completes successfully) and retrieves the validation result with average CPU load value. Get the event validation result from the app and update the test script status | Confirm the 'stop, start, repeat' event is received and animation completes successfully. Verify that the validation result and average CPU load value are retrieved. |
