...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of 4k dash  video codec content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s). Build the test URL with the video_src_url_short_duration_4k_dash (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC_4K_Only_30Sec/master.mpd&operations=checkloop(60)&autotest=true&type=dash` | Verify that playback operations are configured correctly: Check loop state (60s). Confirm the test URL is built with video_src_url_short_duration_4k_dash. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
