...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operations of 4K AV1 codec stream continuously for given number of times
...
| 7 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.av1_url_type_fps should be set to the AV1 (FPS) stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_4k_av1_url>.mp4&operations=pause(5),play(5),repeat(15)&autotest=true&type=mp4`| Ensure that the launch URL is stored in PersistentStore. |
