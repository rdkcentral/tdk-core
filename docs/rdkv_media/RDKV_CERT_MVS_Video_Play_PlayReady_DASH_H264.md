## TestCase ID
RDKV_Media_Validation_73
## TestCase Name
RDKV_CERT_MVS_Video_Play_PlayReady_DASH_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of PlayReady DRM protected H.264 codec DASH stream for few minutes and close the player
...
| 2 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration<br>(close_interval).<br>Build the test URL with the video_src_url_playready_dash_h264 (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_playready_dash_h264>`<br>`&operations=close&autotest=true&type=DASH` | Ensure that playback is configured to start content and close the player after the configured<br>duration (close_interval). Ensure the test URL is built with<br>video_src_url_playready_dash_h264. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 4 | Perform Play Content and Validate Media Events | App starts playing the PlayReady DRM protected h. The 'playing' event is monitored to validate<br>successful playback.264 DASH stream video and closes the player after the provided duration. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
