...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of HLS live content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 60s (close_interval).<br>Build the test URL with the video_src_url_live_hls (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_live_hls>`<br>`&operations=close(60)&autotest=true&type=HLS` | Ensure that playback is configured to start content and close the player after 60s<br>(close_interval). Ensure the test URL is built with video_src_url_live_hls. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Monitor and Validate Media Events | App should play the live HLS content and event video playing should occur | App should play the live HLS content and event video playing should occur. |
