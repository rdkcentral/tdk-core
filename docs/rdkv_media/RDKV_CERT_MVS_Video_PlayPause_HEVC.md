...
| 6 | Execute operations and validate media events. | App performs the play and pause operations of hevc stream and validates each operation using media events ('Observed Event: play') | Ensure that expected media events are observed for the configured operations. |
