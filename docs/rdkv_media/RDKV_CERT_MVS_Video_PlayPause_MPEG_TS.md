...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operation of MPEG ts content
...
| 6 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.mpegts_url_type should be set to the MPEG-TS stream type value. | Ensure that the required URL type configuration should be set correctly. |
