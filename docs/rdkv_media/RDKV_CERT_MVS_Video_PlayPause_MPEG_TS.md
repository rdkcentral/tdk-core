...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operation of mpeg ts content
...
| 5 | Mpegts URL Type Configured | MediaValidationVariables.mpegts_url_type should be set to the MPEG-TS stream type value. | Ensure that MediaValidationVariables.mpegts_url_type is configured with the required value. |
