...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of H264 codec DASH stream for few seconds, then switch to VP9 codec DASH stream for few seconds and close the player
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `changeurl(60),close(60)` operations: the video player will play the first stream for 60 seconds, then switch to the second stream URL, play it for 60 seconds, and then close the player. | Ensure playback operations are set as specified. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_h264_url>.mpd&operations=changeurl(60),close(60),options=secondaryURL(<video_vp9_url>.mpd)&autotest=true&type=dash`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Execute operations and validate media events. | App performs the play operations of DASH h264 video codec stream and DASH VP9 video codec stream validates each o perati on using media events ('playing'). | Ensure that expected media events are observed for the configured operations. |
| 7 | Validate observed events and set pass/fail status. | If expected events playing occurs, then update the result to SUCCESS or FAILURE, as applicable. | Ensure that pass/fail status matches observed event validation. |
