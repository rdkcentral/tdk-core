## TestCase ID
RDKV_Media_Validation_200
## TestCase Name
RDKV_CERT_MVS_Video_Play_DASH_H264_VP9
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of H.264 codec DASH stream for few seconds, then switch to VP9 codec DASH stream  for few seconds and close the player
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Change stream URL (60s), close the player (60s).<br>Build the test URL with the video_src_url_dash_h264 (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd`<br>`&operations=changeurl(60),close(60)&autotest=true&type=DASH` | Ensure that playback operations are configured correctly: Change stream URL (60s), close the<br>player (60s). Ensure the test URL is built with video_src_url_dash_h264. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Play Content and Validate Media Events | App performs the play operations of DASH h264 video codec stream and DASH VP9 video codec<br>stream validates each operation using media events ('playing') | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
| 4 | Validate Event Result and Update Test Status | If expected events playing occurs, then update the result as SUCCESS or else FAILURE | Ensure that expected events playing occurs. Ensure that the test result is updated as SUCCESS. |
