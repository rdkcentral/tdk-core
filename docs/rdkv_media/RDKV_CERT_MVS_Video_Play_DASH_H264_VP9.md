...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operation of H264 codec DASH stream for few seconds, then switch to VP9 codec DASH stream for few seconds and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `changeurl(60),close(60)` operations: the video player will change stream URL for 60 seconds, then close the player after 60 seconds. | Ensure playback operations are set as specified. |
