...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Change stream URL (60s), close the player (60s). Build the test URL with the video_src_url_dash_h264 (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd&operations=changeurl(60),close(60)&autotest=true&type=dash` | Verify that playback operations are configured correctly: Change stream URL (60s), close the player (60s). Confirm the test URL is built with video_src_url_dash_h264. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Play Content and Validate Media Events | App performs the play operations of dash h264 video codec stream and dash vp9  video codec stream validates each operation using media events ('playing') | Verify that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
| 4 | Validate Event Result and Update Test Status | If expected events playing occurs, then update the result as SUCCESS or else FAILURE | Confirm that expected events playing occurs. Check that the test result is updated as SUCCESS. |
