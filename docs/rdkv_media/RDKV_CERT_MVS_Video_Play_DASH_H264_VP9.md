...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of H264 codec DASH stream for few seconds, then switch to VP9 codec DASH stream  for few seconds and close the player
...
| 6 | Run playback operations and validate media events. | App performs the play operations of dash h264 video codec stream and dash vp9  video codec stream validates each operation using media events ('playing') | Ensure that expected media events are observed for the configured operations. |
| 7 | Validate observed events and update test result. | If expected events playing occurs, then update the result as SUCCESS or else FAILURE. Update the test script result as SUCCESS/FAILURE based on event validation result from the app and proc check status (if applicable) | Ensure that the test result is updated as SUCCESS or FAILURE based on event validation and proc check status. |
