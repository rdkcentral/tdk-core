...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 30s (close_interval).<br>Build the test URL with the video_src_url_hls_h264.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=close(30)&autotest=true` | Ensure that playback is configured to start content and close the player after 30s<br>(close_interval). Ensure the test URL is built with video_src_url_hls_h264. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
