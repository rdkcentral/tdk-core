...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio, unmute audio, repeat.<br>Build the test URL with the video_src_url_mp4 (MP4 stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_MP4.MP4`<br>`&operations=mute,unmute,repeat&autotest=true&type=MP4` | Ensure that playback operations are configured correctly: Mute audio, unmute audio, repeat.<br>Ensure the test URL is built with video_src_url_mp4. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
