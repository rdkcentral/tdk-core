...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward at 3x speed (30s), resume playback (10s), close the<br>player.<br>Build the test URL with the video_src_url_ac3_h265.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Fast-forward at 3x speed (30s),<br>resume playback (10s), close the player. Ensure the test URL is built with<br>video_src_url_ac3_h265. Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
