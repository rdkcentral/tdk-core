## TestCase ID
RDKV_Media_Validation_470
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_4K_VP9
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operation of 4K VP9 codec stream
...
| 4 | Update Test Result Based on Event Validation | If expected event ('Observed Event: play') is observed for each operation, app gives the<br>validation result as SUCCESS or else FAILURE. Update the test script result as SUCCESS/FAILURE<br>based on event validation result from the app and proc check status (if applicable) | Ensure that the test result is updated as SUCCESS/FAILURE based on event validation result. |
