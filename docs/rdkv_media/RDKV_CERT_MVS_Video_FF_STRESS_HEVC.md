...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video fast forward operation of HEVC content repeatedly for given number of times
...
| 6 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.hevc_url_type should be set to the HEVC stream type value. | Ensure that the required URL type configuration should be set correctly. |
