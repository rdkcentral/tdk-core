...
| 6 | Verify that the DRM configuration is set correctly. | DRM configuration variable `video_src_url_widevine_cbcs_dash_av1_aac_drmconfigs` must be configured in `MediaValidationVariables.py` with the Widevine license server URL and any required HTTP headers. Use the format: `com.widevine.alpha[<license_url>]|headers[<header_name>:<header_value>]`. This variable is environment-specific and must be set before running the test. | Verify that `video_src_url_widevine_cbcs_dash_av1_aac_drmconfigs` is configured with valid Widevine DRM license server details in `MediaValidationVariables.py`. |
