...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (30s), unmute audio (30s). Build the test URL with the video_src_url_hevc_mkv (MKV stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_HEVC_MKV.mkv&operations=mute(30),unmute(30)&autotest=true&type=mkv` | Verify that playback operations are configured correctly: Mute audio (30s), unmute audio (30s). Confirm the test URL is built with video_src_url_hevc_mkv. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
