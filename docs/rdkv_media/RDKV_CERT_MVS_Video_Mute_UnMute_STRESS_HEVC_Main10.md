...
| 5 | HEVC URL Type Configured | MediaValidationVariables.hevc_url_type should be set to the HEVC stream type value. | Ensure that MediaValidationVariables.hevc_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio, unmute audio, repeat.<br>Build the test URL with the video_src_url_hevc_main10.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC_Main10/dash_hevc_aac_fmp4_main10.mpd`<br>`&operations=mute,unmute,repeat&autotest=true` | Ensure that playback operations are configured correctly: Mute audio, unmute audio, repeat.<br>Ensure the test URL is built with video_src_url_hevc_main10. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
