...
## Objective
To launch a HTML Video Player application via AppManager and perform video seek forward operation of WEBM content with OPUS and VP8 codecs for few minutes and close the player. The test confirms that a forward seek completes successfully, with the video repositioning to the expected location within the acceptable tolerance range. Both the seek request and seek completion are validated through media events.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_vp8>&operations=seekfwd(30),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
