...
RDKV_MEDIA_819
## TestCase Name
RDKV_CERT_MVS_Video_HTML_FF2X_Play_AAC_H265_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform video fast forward operation of MP4 content with H265 and AAC codecs with playback rate 2x for few minutes and close the player. The test confirms that the video fast-forwards at twice the normal speed and that the video position advances at least twice as fast as during normal playback. It also confirms that normal speed playback resumes correctly after the fast-forward period.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_aac_h265>&operations=fastfwd2x(30),playnow(10),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Validate observed events for the fast-forward operation and update test result. | If **ratechange** events are received for both the 2x fast-forward and the normal speed resumption, and the video position advances at least twice the normal rate during fast-forward, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the **fast-forward** event validation and proc check status. |
