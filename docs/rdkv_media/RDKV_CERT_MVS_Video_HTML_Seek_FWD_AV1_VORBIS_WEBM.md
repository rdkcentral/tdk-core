...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward (30s), close the player.<br>Build the test URL with the video_src_url_av1_vorbis.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Seek forward (30s), close the player.<br>Ensure the test URL is built with video_src_url_av1_vorbis. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
