...
## Objective
Test Script to launch a HTML Video player application through AppManager and perform video play pause operation on MP4 content with AV1 and AAC codecs with 1080p resolution for few minutes and then close the player.
...
| 2 | Build the test URL using video_src_url_av1_aac_1080p. | Build the test URL with the video_src_url_av1_aac_1080p. | Verify that the test URL is built using video_src_url_av1_aac_1080p. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_av1_aac_mp4_url>&operations=pause(30),play(10)`| Ensure that the launch URL is stored in PersistentStore. |
