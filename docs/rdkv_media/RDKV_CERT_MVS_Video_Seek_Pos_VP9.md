...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video seek to position operation of VP9 video codec content. The test confirms that the video seeks correctly to configured absolute positions within the stream, with the resulting video position landing within the acceptable range of the intended target for both forward and backward seek positions.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_vp9>&operations=seekpos(30:120),seekpos(30:80),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
