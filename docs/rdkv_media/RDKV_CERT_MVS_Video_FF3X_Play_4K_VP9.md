...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward at 3x speed (30s), resume playback (10s), close the player (30s). Build the test URL with the video_src_url_4k_vp9. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_VP9_2160p.webm&operations=fastfwd3x(30),playnow(10),close(30)&autotest=true` | Verify that playback operations are configured correctly: Fast-forward at 3x speed (30s), resume playback (10s), close the player (30s). Confirm the test URL is built with video_src_url_4k_vp9. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
