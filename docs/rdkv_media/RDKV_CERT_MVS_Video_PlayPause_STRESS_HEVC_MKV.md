...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operations of HEVC MKV content continuously for given number of times
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_hevc_mkv>&operations=pause(5),play(5),repeat(15)&autotest=true&type=mkv`| Ensure that the launch URL is stored in PersistentStore. |
...
| 7 | Validate observed events and set pass/fail status. | If expected events video pause and play occurs for pause and play in all the repetition, then the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
