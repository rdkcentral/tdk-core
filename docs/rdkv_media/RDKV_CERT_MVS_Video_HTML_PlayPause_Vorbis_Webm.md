## TestCase ID
RDKV_Media_Validation_117
## TestCase Name
RDKV_CERT_MVS_Video_HTML_PlayPause_OGG
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a HTML Video player application via AppManager and perform video play pause operation of ogg content and close the player
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play.<br>Build the test URL with the video_src_url_vorbis_webm.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Pause, play. Ensure the test URL is<br>built with video_src_url_vorbis_webm. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
