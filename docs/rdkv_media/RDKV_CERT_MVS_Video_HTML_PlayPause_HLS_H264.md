...
## Objective
Test script to launch a HTML5 Player application via AppManager and perform pause and play of HLS content, validating the expected media events and player behaviour.
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play. Build the test URL with the video_src_url_hls. Store the constructed URL in PersistentStore for AppManager launch. | Verify that playback operations are configured correctly: Pause, play. Confirm the test URL is built with video_src_url_hls. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
