...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operations of OGG stream continuously for given number of times
...
| 7 | Verify that the required configuration is set correctly. | MediaValidationVariables.ogg_url_type should be set to the OGG stream type value. | Verify that MediaValidationVariables.ogg_url_type is configured with the required value. |
...
| 2 | Build the test URL using video_src_url_vorbis. | Build the test URL with the video_src_url_vorbis. | Verify that the test URL is built using video_src_url_vorbis. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_vorbis>&operations=pause(10),play(10),repeat(10)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Execute operations and validate media events. | App performs the pause and play operation of ogg stream repeatedly and validates using events | Ensure that expected media events are observed for the configured operations. |
