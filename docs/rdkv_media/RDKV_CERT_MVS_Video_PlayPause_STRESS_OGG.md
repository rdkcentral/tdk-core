...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operations of Ogg stream continuously for given number of times
...
| 7 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.ogg_url_type should be set to the OGG stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 6 | Run play/pause operations and validate events. | App performs the pause and play operation of ogg stream repeatedly and validates using events. | Ensure that play/pause operations complete and expected events are observed. |
