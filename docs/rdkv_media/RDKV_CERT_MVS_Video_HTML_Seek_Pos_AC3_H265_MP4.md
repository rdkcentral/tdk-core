...
## Objective
To launch a HTML Video Player application via AppManager and perform video seek position operation of MP4 content with H265 and AC3 codecs for few minutes and close the player. The test confirms that the video seeks correctly to configured absolute positions within the stream, with the resulting video position landing within the acceptable range of the intended target for both forward and backward seek positions.
...
| 7 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_ac3_h265` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_AC3_H265.mp4"`, providing the test stream used for playback testing. | Verify that `video_src_url_ac3_h265` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_ac3_h265>&operations=seekfwd(30),seekbwd(30),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
