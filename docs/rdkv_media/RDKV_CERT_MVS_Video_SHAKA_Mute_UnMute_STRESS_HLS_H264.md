...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (10s), unmute audio (10s), repeat. Build the test URL with the video_src_url_hls_h264. Store the constructed URL in PersistentStore for AppManager launch. Sample URL:<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8&operations=mute(10),unmute(10),repeat&autotest=true` | Verify that playback operations are configured correctly: Mute audio (10s), unmute audio (10s), repeat. Confirm the test URL is built with video_src_url_hls_h264. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
