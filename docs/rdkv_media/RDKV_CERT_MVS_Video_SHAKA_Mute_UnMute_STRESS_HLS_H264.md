...
## Objective
Test Script to launch a lightning Shaka player application via AppManager to play HLS h264 video content and perform mute, unmute operations repeatedly for given number of times
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (10s), unmute audio (10s), repeat.<br>Build the test URL with the video_src_url_hls_h264.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=mute(10),unmute(10),repeat&autotest=true` | Ensure that playback operations are configured correctly: Mute audio (10s), unmute audio (10s),<br>repeat. Ensure the test URL is built with video_src_url_hls_h264. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
