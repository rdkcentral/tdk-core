...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_crypt_skip_block_dash_h264_aac (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_aac_url>.mpd&drmconfigs=com.widevine(<license_url>)&operations=close(60)&autotest=true&type=dash`| Ensure that the launch URL is stored in PersistentStore. |
