...
## Objective
Test Script to launch a lightning Shaka player application through AppManager and perform video play pause operation of DASH H264 codec stream
...
| 6 | Execute operations and validate media events. | App performs the play, pause operations and validates each operation using media events ('Observed Event: play'). | Ensure that expected media events are observed for the configured operations. |
| 7 | Validate observed events and set pass/fail status. | If expected events play, paused occurs for play, pause operation, then the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
