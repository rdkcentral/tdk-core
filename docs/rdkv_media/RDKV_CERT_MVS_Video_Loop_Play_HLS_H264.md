...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of hls h264 video codec content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (30s). Build the test URL with the video_src_url_short_duration_hls (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC_15Sec/master.m3u8&operations=checkloop(30)&autotest=true&type=hls` | Verify that playback operations are configured correctly: Check loop state (30s). Confirm the test URL is built with video_src_url_short_duration_hls. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
