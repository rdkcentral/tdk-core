...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (30s).<br>Build the test URL with the video_src_url_short_duration_hls (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC_15Sec/master.m3u8`<br>`&operations=checkloop(30)&autotest=true&type=HLS` | Ensure that playback operations are configured correctly: Ensure loop state (30s). Ensure the<br>test URL is built with video_src_url_short_duration_hls. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
