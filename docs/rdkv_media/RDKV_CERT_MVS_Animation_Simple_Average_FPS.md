...
## Objective
To launch a Lightning Object Animation application via AppManager to render combination of Rectangles and Texts and check whether the average FPS calculated for 60 sec duration value is as expected. The test confirms that the average frames per second rendered by the animation app over the configured duration is within the expected range.
