...
## Objective
Test Script to launch a lightning Video player application through AppManager to play audio only content and perform mute, unmute operations repeatedly for given number of times
...
| 6 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.audio_url_type should be set to the audio stream type value. | Ensure that the required URL type configuration should be set correctly. |
