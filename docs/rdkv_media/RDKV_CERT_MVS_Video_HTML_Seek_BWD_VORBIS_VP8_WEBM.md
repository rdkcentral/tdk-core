...
## Objective
Test Script to launch a HTML Video player application via AppManager and perform video seek backward operation of webm content with Vorbis and Vp8 codecs for few minutes and close the player
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek backward (60s), close the player. Build the test URL with the video_src_url_vorbis_vp8. Store the constructed URL in PersistentStore for AppManager launch. | Verify that playback operations are configured correctly: Seek backward (60s), close the player. Confirm the test URL is built with video_src_url_vorbis_vp8. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
