...
## Objective
Test Script to launch a lightning Shaka player application through AppManager and perform video loop playback of DASH H264 video codec content
...
| 2 | Build the test URL using video_src_url_short_duration_dash. | Build the test URL with the video_src_url_short_duration_dash. | Verify that the test URL is built using video_src_url_short_duration_dash. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<dash_h264_url>.mpd&operations=checkloop(30)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Run playback operations and validate media events. | App performs playback of DASH h264 codec stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
