...
## Objective
To launch a HTML Video Player application via AppManager and perform volume level operation on WEBM content with VORBIS and VP9 codecs for a few minutes and then close the player. The test confirms that the audio volume is set to the configured level and that the volume change is validated by a **volumechange** event. Video playback continues normally after the volume change.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_vorbis_vp9>&operations=setvolume(0.5),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
