...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operations of 4K VP9 codec stream continuously for given number of times
...
| 7 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_4k_vp9>.webm&operations=pause(5),play(5),repeat(15)&autotest=true&type=webm`| Ensure that the launch URL is stored in PersistentStore. |
