...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_cbcs_hls (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_playready_cbcs_hls>.mpd&drmconfigs=com.widevine(license_url)&operations=close(60)&autotest=true&type=hls`| Ensure that the launch URL is stored in PersistentStore. |
