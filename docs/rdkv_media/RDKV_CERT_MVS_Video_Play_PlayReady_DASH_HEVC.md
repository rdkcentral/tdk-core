...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Configure the playback operations. | Configure the `close(close_interval)` operation: the video player will start playing the content and automatically close after the configured duration (close_interval seconds). | Ensure that the playback operations are configured correctly. |
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_playready_dash_hevc (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_hevc_url>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true&type=dash`| Ensure that the launch URL is stored in PersistentStore. |
