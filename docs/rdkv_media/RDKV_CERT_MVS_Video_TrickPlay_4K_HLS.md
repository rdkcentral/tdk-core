## TestCase ID
RDKV_Media_Validation_359
## TestCase Name
RDKV_CERT_MVS_Video_TrickPlay_4K_HLS
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video trickplay operations of 4K HLS stream
...
| 2 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, fast-forward, fast-forward, resume playback, pause,<br>play, seek backward, fast-forward, resume playback.<br>Build the test URL with the video_src_url_4k_hls (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC/master.m3u8`<br>`&operations=seekfwd,fastfwd,fastfwd,playnow,pause,play,seekbwd,fastfwd,playnow&autotest=true`<br>`&type=HLS` | Ensure that playback operations are configured correctly: Seek forward, fast-forward,<br>fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback.<br>Ensure the test URL is built with video_src_url_4k_hls. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
