...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat.<br>Build the test URL with the video_src_url_hevc_hdr (MP4 stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Waterfall_HDR.MOV`<br>`&operations=pause,play,repeat&autotest=true&type=MP4` | Ensure that playback operations are configured correctly: Pause, play, repeat. Ensure the test<br>URL is built with video_src_url_hevc_hdr. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 4 | Validate Event Result and Update Test Status | If expected events video pause and play occurs for pause and play in all the repetition, then<br>app gives the validation result as SUCCESS or else FAILURE | Ensure that expected events video pause and play occurs for pause and play in all the<br>repetition. Ensure that the test result is updated as SUCCESS. |
