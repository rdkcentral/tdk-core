## TestCase ID
RDKV_Media_Validation_302
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_STRESS_HDR
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operations of HDR content continuously for given number of times
...
| 2 | Build the test URL using video_src_url_hevc_hdr. | Build the test URL with the video_src_url_hevc_hdr (MP4 stream). | Verify that the test URL is built using video_src_url_hevc_hdr. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_hevc_hdr>&operations=pause(10),play(10),repeat(10)&autotest=true&type=mp4` | Ensure that the launch URL is stored in PersistentStore. |
...
**Release Version**: M110<div align="right"><sup>[Go To Top](#head.TOC)</sup></div>