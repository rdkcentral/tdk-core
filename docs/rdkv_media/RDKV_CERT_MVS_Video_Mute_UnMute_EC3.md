...
| 4 | Verify that the required configuration is set correctly. | MediaValidationVariables.ec3_url_type should be set to the EC3 stream type value. | Verify that MediaValidationVariables.ec3_url_type is configured with the required value. |
...
| 6 | Run mute/unmute operations and validate events. | App performs the mute and unmute operations and validates using events | Ensure that expected media events are observed for the configured operations. |
