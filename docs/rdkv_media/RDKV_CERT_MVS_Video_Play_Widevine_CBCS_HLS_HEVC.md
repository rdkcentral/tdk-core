...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_cbcs_hls_hevc_aac (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br><code>http://&lt;TM_IP&gt;:&lt;port&gt;/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html<br>?player=sdk<br>&url=http://&lt;TM_IP&gt;:&lt;port&gt;/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC_CBCS/hls_hevc_master.m3u8<br>&operations=close<br>&autotest=true<br>&type=HLS</code>. | Ensure that the launch URL is stored in PersistentStore. |
