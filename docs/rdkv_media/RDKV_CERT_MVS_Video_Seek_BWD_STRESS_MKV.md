...
| 5 | Execute seek operations and capture media events. | The player repeatedly seeks the video backward for the configured number of stress iterations. Each seek fires a **seeking** event immediately when requested and a **seeked** event when the seek completes. The video position is validated after each seek to confirm correct backward repositioning within the acceptable range of the expected target. | Ensure that the expected media events are captured for the configured operations. |
| 6 | Validate observed events for the seek backward stress operation and update test result. | If **seeking** and **seeked** events fire for every backward seek iteration and the video position lands within the acceptable range of the expected target each time, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the seek backward stress event validation and proc check status. |
