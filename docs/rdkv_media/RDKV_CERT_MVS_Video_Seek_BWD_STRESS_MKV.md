## TestCase ID
RDKV_Media_Validation_392
## TestCase Name
RDKV_CERT_MVS_Video_Seek_BWD_STRESS_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video seek backward operation of MKV content continuously for given number of times in provided interval
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek backward (420s) (2 times), seek backward, repeat.<br>Build the test URL with the video_src_url_4k_mkv (MKV stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_2160p.MKV`<br>`&operations=seekbwd(420),repeat(1),seekbwd,repeat&autotest=true&type=MKV` | Ensure that playback operations are configured correctly: Seek backward (420s) (2 times), seek<br>backward, repeat. Ensure the test URL is built with video_src_url_4k_mkv. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
