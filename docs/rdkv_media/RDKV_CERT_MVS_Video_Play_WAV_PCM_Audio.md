...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform audio play operation of basic.wav pcm audio stream for few minutes and close the player
...
| 4 | Verify that the required configuration is set correctly. | MediaValidationVariables.audio_src_url_wav_pcm should be set to the WAV/PCM audio stream URL. | Ensure that MediaValidationVariables.audio_src_url_wav_pcm should be configured with the required value. |
