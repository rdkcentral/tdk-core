...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform audio play operation of basic .WAV pcm audio stream for few minutes and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 6 | Verify that the stream variable is configured correctly. | Stream variable `audio_src_url_wav_pcm` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_WAV_Audio.wav"`, providing the test stream used for playback testing. | Verify that `audio_src_url_wav_pcm` resolves to a valid, accessible stream location for this test. |
...
| 5 | Run video playback and capture media events. | App starts playing the wav pcm audio stream and closes the player after the provided duration. The 'playing' event is monitored to validate successful playback. | Ensure that the video playback events are captured for the configured duration. |
