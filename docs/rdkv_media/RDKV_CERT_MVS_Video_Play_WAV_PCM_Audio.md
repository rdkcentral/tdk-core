...
| 2 | Store the launch URL in PersistentStore. | Build the test URL for the Video app. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<audio_url>.wav&operations=close(60)&autotest=true&type=wav`| Ensure that the launch URL is stored in PersistentStore. |
