...
| 3 | Audio Src URL Wav Pcm Configured | MediaValidationVariables.audio_src_url_wav_pcm should be set to the WAV/PCM audio stream URL. | Ensure that MediaValidationVariables.audio_src_url_wav_pcm is configured with the required<br>value. |
| 4 | Audio Close Interval Configured | MediaValidationVariables.audio_close_interval should be set to the audio close interval value<br>(in seconds). | Ensure that MediaValidationVariables.audio_close_interval is configured with the required<br>value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration<br>(close_interval).<br>Build the test URL for the Video app.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<stream>&operations=close`<br>`&autotest=true&type=wav` | Ensure that playback is configured to start content and close the player after the configured<br>duration (close_interval). Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
...
| 3 | Perform Play Content and Validate Media Events | App starts playing the wav pcm audio stream and closes the player after the provided duration.<br>The 'playing' event is monitored to validate successful playback. | Ensure that the app performs all configured operations without errors. The 'playing' event<br>should be observed, confirming successful media playback. |
