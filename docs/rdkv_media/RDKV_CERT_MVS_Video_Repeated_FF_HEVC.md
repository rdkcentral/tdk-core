...
RDKV_MEDIA_256
## TestCase Name
RDKV_CERT_MVS_Video_Repeated_FF_HEVC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video fast forward operation of HEVC content repeatedly for given number of times. The test confirms that the player can reliably alternate between fast-forward speed and normal playback speed across the configured number of repetitions, with consistent speed changes and correct position advancement throughout all repeated iterations without any degradation.
...
| 6 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_hevc` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_HEVC_AAC/atfms_291_dash_tdk_hevc_aac_fmp4.mpd"`, providing the HEVC codec DASH stream used for playback testing. | Verify `video_src_url_hevc` resolves to a valid, accessible HEVC DASH stream location. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_hevc>&operations=fastfwd(10),repeat(3),playnow(10),repeat(1),fastfwd(10),repeat(3),playnow(10)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Execute fast-forward/rewind operations and capture media events. | The player repeatedly alternates between fast-forward speed and normal playback speed for the configured number of repeated iterations. A **ratechange** event is expected for each speed change. The video position is monitored after each fast-forward segment to confirm consistent acceleration without degradation across all repetitions. | Ensure that the expected media events are captured for the configured operations. |
| 6 | Validate observed events for the fast-forward repeated operation and update test result. | If **ratechange** events fire correctly for every fast-forward and normal speed resumption across all repeated iterations, and the video position advances at the expected faster rate during each fast-forward segment, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the fast-forward repeated event validation and proc check status. |
