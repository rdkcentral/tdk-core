...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play. Build the test URL with the video_src_url_ac3_h265_720p. Store the constructed URL in PersistentStore for AppManager launch. | Verify that playback operations are configured correctly: Pause, play. Confirm the test URL is built with video_src_url_ac3_h265_720p. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
