...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of  PlayReady DRM protected MultiKey encryption HLS stream for few minutes and close the player
...
| 2 | Build the test URL using video_src_url_playready_multikey_hls. | Build the test URL with the video_src_url_playready_multikey_hls (HLS stream). | Verify that the test URL is built using video_src_url_playready_multikey_hls. |
