...
## Objective
To launch a Lightning Animation application via AppManager and check whether the average FPS calculated for 60 sec duration value is as expected. The test confirms that the average frames per second rendered by the animation app over the configured duration is within the expected range.
...
| 4 | Check whether the app is already installed on the device. | Query the installed package list using the following request: <br><code>{"jsonrpc": "2.0", "id": 1234567890, "method": "org.rdk.AppPackageManager.1.listPackages"}</code>. | Verify that the app is installed on the device. |
