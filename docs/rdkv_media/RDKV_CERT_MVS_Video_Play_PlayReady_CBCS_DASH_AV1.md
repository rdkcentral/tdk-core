...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of  PlayReady DRM protected CBCS encryption DASH AV1 stream for few minutes and close the player
...
| 2 | Build the test URL using video_src_url_playready_cbcs_dash_av1. | Build the test URL with the video_src_url_playready_cbcs_dash_av1 (DASH stream). | Verify that the test URL is built using video_src_url_playready_cbcs_dash_av1. |
...
| 6 | Run playback operations and validate media events. | App starts playing the PlayReady DRM protected cbcs dash av1 stream video and closes the player after the provided duration. The 'playing' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
