## TestCase ID
RDKV_Media_Validation_97
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Play_OGG
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a HTML Video player application through AppManager and perform video play operation of ogg content for few minutes and close the player
...
| 4 | Run playback operations and validate media events. | App starts playing the ogg video and closes the player after the provided duration. The 'playing' event is monitored to validate successful playback. | Ensure that expected media events are observed for the configured operations. |
