...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_vorbis_webm. Store the constructed URL in PersistentStore for AppManager launch. | Verify that playback is configured to start content and close the player after the configured duration (close_interval). Confirm the test URL is built with video_src_url_vorbis_webm. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
