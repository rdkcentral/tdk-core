...
RDKV_MEDIA_776
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Play_Full_VP9
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Shaka Player integrated with lightning application via AppManager and perform play till end of content of VP9 content, validating the expected media events and player behaviour. The test confirms that the stream plays from beginning to natural end without interruption. Successful playback start and natural stream completion are both validated, with the video position advancing continuously throughout.
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_short_duration_vp9` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_VP9_OPUS_WebM_30Sec/master.mpd"`, providing the test stream used for playback testing. | Verify that `video_src_url_short_duration_vp9` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_short_duration_vp9>&operations=playtillend(0)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
