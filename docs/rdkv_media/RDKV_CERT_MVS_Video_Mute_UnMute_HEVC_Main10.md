## TestCase ID
RDKV_Media_Validation_1083
## TestCase Name
RDKV_CERT_MVS_Video_Mute_UnMute_HEVC_Main10
<a name="head.TOC"></a>
## Table Of Contents
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (30s), unmute audio (30s).<br>Build the test URL with the video_src_url_hevc_main10 (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC_Main10/dash_hevc_aac_fmp4_main10.mpd`<br>`&operations=mute(30),unmute(30)&autotest=true&type=DASH` | Ensure that playback operations are configured correctly: Mute audio (30s), unmute audio (30s).<br>Ensure the test URL is built with video_src_url_hevc_main10. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
