...
RDKV_MEDIA_1013
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Play_DASH_SoundMode_DolbyDigital
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Shaka Player integrated with lightning application via AppManager and perform playback, validating the expected media events and player behaviour. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 5 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_dolby` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "DASH_H264_EC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd"`, providing the test stream used for playback testing. | Verify that `video_src_url_dolby` resolves to a valid, accessible stream location for this test. |
