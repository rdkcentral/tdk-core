...
## Objective
Test Script to launch a HTML Video player application through AppManager and perform video play pause operation of HEVC content and close the player
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_hevc_url>&operations=pause(30),play(10)`| Ensure that the launch URL is stored in PersistentStore. |
