...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of HEVC video codec content
...
|3| Hevc URL Type Configured | MediaValidationVariables.hevc_url_type should be set to the HEVC stream type value.| MediaValidationVariables.hevc_url_type is configured with the required value.|
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s). Build the test URL with the video_src_url_short_duration_hevc. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_HEVC_AAC_30Sec/master.mpd&operations=checkloop(60)&autotest=true` | Verify that playback operations are configured correctly: Check loop state (60s). Confirm the test URL is built with video_src_url_short_duration_hevc. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
