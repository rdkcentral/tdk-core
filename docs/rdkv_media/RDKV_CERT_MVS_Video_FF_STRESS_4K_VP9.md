...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video fast forward operation of 4K VP9 content repeatedly for given number of times
...
| 5 | VP9 4K URL Type Configured | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value. | Ensure that MediaValidationVariables.vp9_4k_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward (4 times), resume playback (2 times), fast-forward (4<br>times), resume playback.<br>Build the test URL with the video_src_url_4k_vp9.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_VP9_2160p.webm`<br>`&operations=fastfwd,repeat(3),playnow,repeat(1),fastfwd,repeat(3),playnow&autotest=true` | Ensure that playback operations are configured correctly: Fast-forward (4 times), resume<br>playback (2 times), fast-forward (4 times), resume playback. Ensure the test URL is built with<br>video_src_url_4k_vp9. Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
