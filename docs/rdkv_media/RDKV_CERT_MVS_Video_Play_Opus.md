## TestCase ID
RDKV_Media_Validation_57
## TestCase Name
RDKV_CERT_MVS_Video_Play_Opus
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of Opus codec stream for few minutes and close the player
...
| 4 | Opus URL Type Configured | MediaValidationVariables.opus_url_type should be set to the OPUS stream type value. | Ensure that MediaValidationVariables.opus_url_type is configured with the required value. |
