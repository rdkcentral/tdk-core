...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform audio play operation for DTS audio content for few minutes and close the player.
...
| 3 | Audio Src URL Dts Configured | MediaValidationVariables.audio_src_url_dts should be set to the DTS audio stream URL. | Ensure that MediaValidationVariables.audio_src_url_dts is configured with the required value. |
