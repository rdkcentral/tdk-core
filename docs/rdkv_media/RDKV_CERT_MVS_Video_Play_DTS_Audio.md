...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform audio play operation for DTS audio content for few minutes and close the player.
...
| 4 | Verify that the required configuration is set correctly. | MediaValidationVariables.audio_src_url_dts should be set to the DTS audio stream URL. | Ensure that MediaValidationVariables.audio_src_url_dts should be configured with the required value. |
