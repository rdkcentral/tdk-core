## TestCase ID
RDKV_Media_Validation_476
## TestCase Name
RDKV_CERT_MVS_Video_TrickPlay_4K_VP9
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video trickplay operations of 4K VP9 content
...
| 4 | VP9 4K URL Type Configured | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value. | Ensure that MediaValidationVariables.vp9_4k_url_type is configured with the required value. |
...
| 2 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, fast-forward, fast-forward, resume playback, pause,<br>play, seek backward, fast-forward, resume playback.<br>Build the test URL with the video_src_url_4k_vp9.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_VP9_2160p.webm`<br>`&operations=seekfwd,fastfwd,fastfwd,playnow,pause,play,seekbwd,fastfwd,playnow&autotest=true` | Ensure that playback operations are configured correctly: Seek forward, fast-forward,<br>fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback.<br>Ensure the test URL is built with video_src_url_4k_vp9. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
