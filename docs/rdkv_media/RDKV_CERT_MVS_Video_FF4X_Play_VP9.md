...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward at 4x speed (30s), resume playback (10s), close the<br>player (30s).<br>Build the test URL with the video_src_url_vp9.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_VP9_OPUS_WebM/master.mpd`<br>`&operations=fastfwd4x(30),playnow(10),close(30)&autotest=true` | Ensure that playback operations are configured correctly: Fast-forward at 4x speed (30s),<br>resume playback (10s), close the player (30s). Ensure the test URL is built with<br>video_src_url_vp9. Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
