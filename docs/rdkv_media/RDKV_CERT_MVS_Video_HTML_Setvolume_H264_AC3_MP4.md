...
## Objective
Test script to launch an HTML video player application through AppManager and perform volume level operation on MP4 content with H264 and AC3 codecs for a few minutes and then close the player.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_h264_ac3_mp4_url>&operations=setvolume(0.5),close(30)`| Ensure that the launch URL is stored in PersistentStore. |
