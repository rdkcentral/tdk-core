...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio, unmute audio, repeat. Build the test URL with the video_src_url_4k_av1. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_AV1_2160p.mp4&operations=mute,unmute,repeat&autotest=true` | Verify that playback operations are configured correctly: Mute audio, unmute audio, repeat. Confirm the test URL is built with video_src_url_4k_av1. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
