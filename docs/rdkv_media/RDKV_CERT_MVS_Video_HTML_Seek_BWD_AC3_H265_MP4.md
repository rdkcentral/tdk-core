...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek backward (60s), close the player.<br>Build the test URL with the video_src_url_ac3_h265.<br>Store the constructed URL in PersistentStore for AppManager launch. | Ensure that playback operations are configured correctly: Seek backward (60s), close the<br>player. Ensure the test URL is built with video_src_url_ac3_h265. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
