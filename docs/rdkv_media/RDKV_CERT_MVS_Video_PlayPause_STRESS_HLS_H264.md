...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat. Build the test URL with the video_src_url_hls_h264 (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8&operations=pause,play,repeat&autotest=true&type=hls` | Verify that playback operations are configured correctly: Pause, play, repeat. Confirm the test URL is built with video_src_url_hls_h264. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the pause and play operation of hls h264 video codec stream repeatedly and validates using events | Verify that the pause and play operation of hls h264 video codec stream repeatedly and validates using events completes successfully without errors. |
| 4 | Validate Event Result and Update Test Status | If expected events paused, play occurs for pause and play in all the repetition, then app gives the validation result as SUCCESS or else FAILURE | Confirm that expected events paused, play occurs for pause and play in all the repetition. Check that the test result is updated as SUCCESS. |
