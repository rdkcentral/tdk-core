## TestCase ID
RDKV_Media_Validation_08
## TestCase Name
RDKV_CERT_MVS_Video_PlayPause_STRESS_HLS_H264
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operations of HLS h264 video codec content continuously for given number of times
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Pause, play, repeat.<br>Build the test URL with the video_src_url_hls_h264 (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=pause,play,repeat&autotest=true&type=HLS` | Ensure that playback operations are configured correctly: Pause, play, repeat. Ensure the test<br>URL is built with video_src_url_hls_h264. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the pause and play operation of HLS h264 video codec stream repeatedly and<br>validates using events | Ensure that the pause and play operation of HLS h264 video codec stream repeatedly and<br>validates using events completes successfully without errors. |
| 4 | Validate Event Result and Update Test Status | If expected events paused, play occurs for pause and play in all the repetition, then app gives<br>the validation result as SUCCESS or else FAILURE | Ensure that expected events paused, play occurs for pause and play in all the repetition. Ensure<br>that the test result is updated as SUCCESS. |
