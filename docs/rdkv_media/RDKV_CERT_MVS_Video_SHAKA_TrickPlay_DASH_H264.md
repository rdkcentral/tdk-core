...
| 6 | Execute seek and fast-forward operations and validate media events. | App performs the provided operations and validates each operation using media events ('playing') and with position and rate value for seek and fast-forward operations | Ensure that expected media events are observed for the configured operations. |
