...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, fast-forward, fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback. Build the test URL with the video_src_url_dash_h264. Store the constructed URL in PersistentStore for AppManager launch. Sample URL:<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd&operations=seekfwd,fastfwd,fastfwd,playnow,pause,play,seekbwd,fastfwd,playnow&autotest=true` | Verify that playback operations are configured correctly: Seek forward, fast-forward, fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback. Confirm the test URL is built with video_src_url_dash_h264. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
