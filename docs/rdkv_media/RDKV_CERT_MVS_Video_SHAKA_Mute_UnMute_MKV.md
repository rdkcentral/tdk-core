...
## Objective
To launch a Shaka Player integrated with lightning application via AppManager and perform mute audio and unmute audio of MKV content, validating the expected media events and player behaviour. The test confirms that the audio mutes and unmutes correctly, with each state change validated by a **volumechange** event. Video playback continues uninterrupted throughout both the mute and unmute operations.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_4k_mkv>&operations=mute(30),unmute(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
