...
RDKV_MEDIA_1025
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Seek_FWD_DASH_HEVC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform seek forward of HEVC/H265 content, validating the expected media events and player behaviour. The test confirms that a forward seek completes successfully, with the video repositioning to the expected location within the acceptable tolerance range. Both the seek request and seek completion are validated through media events.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_hevc>&operations=seekfwd(30),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
