## TestCase ID
RDKV_Media_Validation_62
## TestCase Name
RDKV_CERT_MVS_Video_Play_MPEG_TS
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of mpeg ts stream for few minutes and close the player
...
| 4 | Mpegts URL Type Configured | MediaValidationVariables.mpegts_url_type should be set to the MPEG-TS stream type value. | Ensure that MediaValidationVariables.mpegts_url_type is configured with the required value. |
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration<br>(close_interval).<br>Build the test URL with the video_src_url_mpegts.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=close&autotest=true` | Ensure that playback is configured to start content and close the player after the configured<br>duration (close_interval). Ensure the test URL is built with video_src_url_mpegts. Ensure the<br>URL is stored in PersistentStore (MVS/lightningURL) successfully. |
