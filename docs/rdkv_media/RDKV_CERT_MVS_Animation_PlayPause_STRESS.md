...
## Objective
To launch a Lightning Animation application via AppManager and perform animation play and pause operation repeatedly for given number of times. The test confirms that the animation app can reliably pause and resume playback across the configured number of stress repetitions without any degradation or errors.
...
| 5 | Execute play and pause operations and capture media events. | The animation app repeatedly executes pause and play operations for the configured number of stress iterations. Each pause and play cycle is validated by monitoring the expected state change events. The test confirms that the animation responds consistently across all repetitions without any degradation. | Ensure that the **play** and **pause** events are captured for the configured operations. |
| 6 | Validate captured **pause** and **play** events and update test result. | If the expected events are received for all pause and play cycles across all stress iterations, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation result and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the captured pause and play stress events. |
