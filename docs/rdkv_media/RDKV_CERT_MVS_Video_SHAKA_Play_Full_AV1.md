...
## Objective
Test script to launch a Lightning Unified Player application through AppManager and perform play till end of content of AV1 content, validating the expected media events and player behaviour.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<av1_video_url>.mpd&operations=playtillend(0)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
