...
RDKV_MEDIA_921
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Setvolume_AAC_H265_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform volume level operation on MP4 content with H265 and AAC codecs for a few minutes and then close the player. The test confirms that the audio volume is set to the configured level and that the volume change is validated by a **volumechange** event. Video playback continues normally after the volume change.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_aac_h265>&operations=setvolume(0.5),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Run volume change operation and capture media events. | The player sets the audio volume to the configured level. A **volumechange** event confirms that the new volume level was applied. The video continues to play normally after the volume change, confirming that adjusting the volume does not disrupt video playback. | Ensure that the **volumechange** event is captured for the configured operation. |
| 6 | Validate observed events for the volume change operation and update test result. | If a **volumechange** event fires confirming the configured volume level and the video continues to play normally, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the **volumechange** event validation and proc check status. |
