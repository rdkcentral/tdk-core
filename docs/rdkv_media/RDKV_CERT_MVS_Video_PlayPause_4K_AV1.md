...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play pause operation of 4K AV1 codec stream
...
| 5 | AV1 URL Type FPS Configured | MediaValidationVariables.av1_url_type_fps should be set to the AV1 (FPS) stream type value. | Ensure that MediaValidationVariables.av1_url_type_fps is configured with the required value. |
...
| 3 | Perform Pause / Play Content and Validate Media Events | App performs the play pause operations of 4K AV1 codec stream and validates each operation<br>using media events ('Observed Event: play') | Ensure that the app performs all configured operations and validates them using media events.<br>Operations should complete successfully without errors. |
| 4 | Validate Event Result and Update Test Status | If expected events playing, paused occurs for play and pause operation, then app gives the<br>validation result as SUCCESS or else FAILURE | Ensure that expected events playing, paused occurs for play and pause operation. Ensure that the<br>test result is updated as SUCCESS. |
