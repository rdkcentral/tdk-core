...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play pause operation of 4K AV1 codec stream
...
| 6 | Execute operations and validate media events. | App performs the play pause operations of 4K AV1 codec stream and validates each operation using media events ('O bserve d Event: play'). | Ensure that expected media events are observed for the configured operations. |
