...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video loop playback of dash ec3 codec content
...
|3| Ec3 URL Type Configured | MediaValidationVariables.ec3_url_type should be set to the EC3 stream type value.| MediaValidationVariables.ec3_url_type is configured with the required value.|
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s). Build the test URL with the video_src_url_short_duration_ec3. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_EC3_30Sec/master.mpd&operations=checkloop(60)&autotest=true` | Verify that playback operations are configured correctly: Check loop state (60s). Confirm the test URL is built with video_src_url_short_duration_ec3. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
