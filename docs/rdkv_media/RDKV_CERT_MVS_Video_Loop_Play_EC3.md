...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Check loop state (60s).<br>Build the test URL with the video_src_url_short_duration_ec3.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_EC3_30Sec/master.mpd`<br>`&operations=checkloop(60)&autotest=true` | Ensure that playback operations are configured correctly: Ensure loop state (60s). Ensure the<br>test URL is built with video_src_url_short_duration_ec3. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
