...
## Objective
Test Script to launch a lightning Shaka player application through AppManager and perform video play operations of HLS H264 video codec content till the end
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<hls_h264_url>.m3u8&operations=playtillend(0)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
