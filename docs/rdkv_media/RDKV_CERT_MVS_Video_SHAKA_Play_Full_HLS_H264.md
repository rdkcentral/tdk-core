...
RDKV_MEDIA_750
## TestCase Name
RDKV_CERT_MVS_Video_SHAKA_Play_Full_HLS_H264
<a name="head.TOC"></a>
## Table Of Contents
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_short_duration_hls` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "HLS_H264_AAC_15Sec/master.m3u8"`, providing the test stream used for playback testing. | Verify that `video_src_url_short_duration_hls` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_short_duration_hls>&operations=playtillend(0)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
