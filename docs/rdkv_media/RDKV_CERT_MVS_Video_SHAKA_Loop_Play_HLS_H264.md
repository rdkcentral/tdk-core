...
## Objective
Test Script to launch a lightning Shaka player application through AppManager and perform video loop playback of HLS H264 video codec content
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `checkloop(30)` operation: the video player will loop the content continuously, counting the number of times the video restarts within the 30-second observation window based on the content duration. | Ensure playback operations are set as specified. |
| 2 | Build the test URL using video_src_url_short_duration_hls. | Build the test URL with the video_src_url_short_duration_hls. | Verify that the test URL is built using video_src_url_short_duration_hls. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<hls_h264_url>.m3u8&operations=checkloop(30)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Run playback operations and validate media events. | App performs playback of HLS h264 codec stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
...
| 9 | Validate observed events and set pass/fail status. | Update the test script result as SUCCESS/FAILURE based on event validation result and proc check status (if applicable). | Ensure that the final test result is updated based on validation and proc-check status. |
