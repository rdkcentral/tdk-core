...
RDKV_MEDIA_392
## TestCase Name
RDKV_CERT_MVS_Video_Repeated_Seek_BWD_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video seek backward operation of MKV content continuously for given number of times in provided interval. The test confirms that repeated backward seeks complete successfully across all configured iterations, with the video position landing within the acceptable range of the intended target after each seek.
...
| 7 | Verify that the required configuration is set correctly. | MediaValidationVariables.repeat_count_stress should be set to the repeat count for repeated tests. | Verify that MediaValidationVariables.repeat_count_stress is configured with the required value. |
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `seekbwd(420),repeat(1),seekbwd(10),repeat(10)` operations: the video player will seek backward for 420 seconds, then repeat the preceding operations 1 time, then seek backward for 10 seconds, then repeat the preceding operations 10 times. | Ensure playback operations are set as specified. |
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_4k_mkv>&operations=seekbwd(420),repeat(1),seekbwd(10),repeat(10)&autotest=true&type=mkv` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Execute seek operations and capture media events. | The player repeatedly seeks the video backward for the configured number of repeated iterations. Each seek fires a **seeking** event immediately when requested and a **seeked** event when the seek completes. The video position is validated after each seek to confirm correct backward repositioning within the acceptable range of the expected target. | Ensure that the expected media events are captured for the configured operations. |
| 6 | Validate observed events for the seek backward repeated operation and update test result. | If **seeking** and **seeked** events fire for every backward seek iteration and the video position lands within the acceptable range of the expected target each time, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the seek backward repeated event validation and proc check status. |
...
**Release Version**: M126<div align="right"><sup>[Go To Top](#head.TOC)</sup></div>