...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of DASH content with 720p display resolution for few minutes and close the player
...
| 4 | Verify TV connection and 720p resolution support. | TV should be connected with the test device. If the 720p resolution is not supported then the test will be marked as failure. | Ensure that the TV is connected and 720p resolution is supported; otherwise mark the test as failure. |
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Verify that get the current video display resolution and set resolution as 720p. | Get the current video display resolution and set resolution as 720p. | Ensure that get the current video display resolution and set resolution as 720p should be validated successfully. |
