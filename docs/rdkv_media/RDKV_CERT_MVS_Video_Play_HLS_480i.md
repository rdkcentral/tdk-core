...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Execute Test Step | Get the current video display resolution and set resolution as 480i | Step executed successfully. |
| 2 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_hls (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8&operations=close&autotest=true&type=hls` | Verify that playback is configured to start content and close the player after the configured duration (close_interval). Confirm the test URL is built with video_src_url_hls. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
