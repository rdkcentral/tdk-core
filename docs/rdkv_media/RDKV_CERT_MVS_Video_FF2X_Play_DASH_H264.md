...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video fast forward operation in 2x speed for few seconds, continue video play of DASH h264 video codec content and close the player
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward at 2x speed (30s), resume playback (10s), close the<br>player (30s).<br>Build the test URL with the video_src_url_dash_h264 (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd`<br>`&operations=fastfwd2x(30),playnow(10),close(30)&autotest=true&type=DASH` | Ensure that playback operations are configured correctly: Fast-forward at 2x speed (30s),<br>resume playback (10s), close the player (30s). Ensure the test URL is built with<br>video_src_url_dash_h264. Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
