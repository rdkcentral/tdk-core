...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video medium trick play operations like seek and FF of 4K HLS content
...
| 2 | Configure Playback Operations and Test URL | Set playback operations: Seek forward (30s), fast-forward (20s), resume playback (30s).<br>Build the test URL with the video_src_url_4k_hls (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC/master.m3u8`<br>`&operations=seekfwd(30),fastfwd(20),playnow(30)&autotest=true&type=HLS` | Ensure that playback operations are configured correctly: Seek forward (30s), fast-forward<br>(20s), resume playback (30s). Ensure the test URL is built with video_src_url_4k_hls. Ensure<br>the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
