...
| 2 | Configure Playback Operations and Test URL | Set playback operations: Seek forward (30s), fast-forward (20s), resume playback (30s). Build the test URL with the video_src_url_4k_hls (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC/master.m3u8&operations=seekfwd(30),fastfwd(20),playnow(30)&autotest=true&type=hls` | Verify that playback operations are configured correctly: Seek forward (30s), fast-forward (20s), resume playback (30s). Confirm the test URL is built with video_src_url_4k_hls. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
