...
RDKV_MEDIA_848
## TestCase Name
RDKV_CERT_MVS_Video_HTML_FF4X_Play_H264_AC3_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform video fast forward operation of MP4 content with H264 and AC3 codecs with playback rate 4x for few minutes and close the player. The test confirms that the video fast-forwards at four times the normal speed and that the video position advances at least four times faster than normal. It also confirms that normal speed playback resumes correctly after the fast-forward period.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_h264_ac3>&operations=fastfwd4x(30),playnow(10),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Validate observed events for the fast-forward operation and update test result. | If **ratechange** events are received for both the 4x fast-forward and the normal speed resumption, and the video position advances at least four times the normal rate, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the **fast-forward** event validation and proc check status. |
