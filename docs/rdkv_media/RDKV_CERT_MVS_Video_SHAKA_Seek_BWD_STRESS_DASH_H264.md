...
## Objective
Test Script to launch a lightning Shaka player application through AppManager and perform video seek backward operation of DASH H264 video codec content continuously for given number of times in provided interval
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Set playback operations: Seek backward (420s) (2 times), seek backward, repeat. | Ensure playback operations are set as specified. |
...
| 7 | Validate observed events and set pass/fail status. | If expected event seeking and seeked occurs for each seekbwd operation, then the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
