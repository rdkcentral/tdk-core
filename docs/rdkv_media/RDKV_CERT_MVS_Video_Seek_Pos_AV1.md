...
|6| Av1 URL Type Configured | MediaValidationVariables.av1_url_type should be set to the AV1 stream type value.| MediaValidationVariables.av1_url_type is configured with the required value.|
...
| 2 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 30s (close_interval). Build the test URL with the video_src_url_av1. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_DASH_AV1_AAC/master.mpd&operations=close(30)&autotest=true` | Verify that playback is configured to start content and close the player after 30s (close_interval). Confirm the test URL is built with video_src_url_av1. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
