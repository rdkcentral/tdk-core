...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video seek to position operation of AV1 video codec content
...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_av1. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_url>.mpd&operations=seekpos(30:120),seekpos(30:80),close(30)&autotest=true&options=checkInterval(10)&type=dash`| Ensure that the launch URL is stored in PersistentStore. |
