...
RDKV_MEDIA_119
## TestCase Name
RDKV_CERT_MVS_Video_Repeated_Mute_UnMute_Audio_Only
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager to play audio only content and perform mute, unmute operations repeatedly for given number of times. The test confirms that the audio mutes and unmutes correctly, with each state change validated by a **volumechange** event. Video playback continues uninterrupted throughout both the mute and unmute operations.
