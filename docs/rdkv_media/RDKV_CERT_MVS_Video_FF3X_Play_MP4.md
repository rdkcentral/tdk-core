...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Fast-forward at 3x speed (30s), resume playback (10s), close the<br>player (30s).<br>Build the test URL with the video_src_url_mp4 (MP4 stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_MP4.MP4`<br>`&operations=fastfwd3x(30),playnow(10),close(30)&autotest=true&type=MP4` | Ensure that playback operations are configured correctly: Fast-forward at 3x speed (30s),<br>resume playback (10s), close the player (30s). Ensure the test URL is built with<br>video_src_url_mp4. Ensure the URL is stored in PersistentStore (MVS/lightningURL)<br>successfully. |
