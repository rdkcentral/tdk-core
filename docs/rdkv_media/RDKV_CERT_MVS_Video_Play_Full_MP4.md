...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operations of MP4 content till the end
...
| 2 | Build the test URL using video_src_url_short_duration_mp4. | Build the test URL with the video_src_url_short_duration_mp4 (MP4 stream). | Verify that the test URL is built using video_src_url_short_duration_mp4. |
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_mp4_url>&operations=playtillend(0)&autotest=true&type=mp4`| Ensure that the launch URL is stored in PersistentStore. |
