...
## Objective
Test script to launch a Lightning Unified Player application via AppManager and perform playback of HLS HEVC/H.265 content, validating the expected media events and player behaviour.
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 150s (close_interval).<br>Build the test URL with the video_src_url_widevine_clear_lead_hls_hevc_aac (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_Clear_Lead/hls_hevc_clear_master.m3u8`<br>`&operations=close(150),close(150)&autotest=true&type=DASH` | Ensure that playback is configured to start content and close the player after 150s<br>(close_interval). Ensure the test URL is built with<br>video_src_url_widevine_clear_lead_hls_hevc_aac. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
...
| 4 | Validate Event Result and Update Test Status | If expected event (playing) is observed, app gives the validation result as SUCCESS or else<br>FAILURE. | Ensure that the 'playing' event is observed as expected. Ensure that the test result is<br>updated as SUCCESS/FAILURE accordingly. |
