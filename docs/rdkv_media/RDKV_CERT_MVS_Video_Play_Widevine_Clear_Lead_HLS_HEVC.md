...
## Objective
Test script to launch a Lightning Unified Player application via AppManager and perform playback of HLS HEVC/H265 content, validating the expected media events and player behaviour.
...
| 2 | Build the test URL using video_src_url_widevine_clear_lead_hls_hevc_aac. | Build the test URL with the video_src_url_widevine_clear_lead_hls_hevc_aac (DASH stream). | Verify that the test URL is built using video_src_url_widevine_clear_lead_hls_hevc_aac. |
