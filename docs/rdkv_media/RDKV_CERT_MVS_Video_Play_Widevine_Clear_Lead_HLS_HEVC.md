...
RDKV_MEDIA_1106
## TestCase Name
RDKV_CERT_MVS_Video_Play_Widevine_Clear_Lead_HLS_HEVC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operation of Widevine DRM protected Clear Lead encrypted HEVC codec HLS stream for few minutes and close the player. The test confirms that the stream opens and plays back successfully, that playback starts without errors, and that the video progresses continuously without stalls or interruptions for the configured duration before the player closes.
...
| 4 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_widevine_clear_lead_hls_hevc_aac` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "HLS_HEVC_Clear_Lead/hls_hevc_clear_master.m3u8"`, providing the test stream used for playback testing. | Verify that `video_src_url_widevine_clear_lead_hls_hevc_aac` resolves to a valid, accessible stream location for this test. |
| 5 | Verify that the DRM configuration is set correctly. | DRM configuration variable `video_src_url_widevine_clear_lead_hls_hevc_aac_drmconfigs` must be configured in `MediaValidationVariables.py` with the Widevine license server URL and any required HTTP headers. Use the format: `com.widevine.alpha[<license_url>]\|headers[<header_name>:<header_value>]`. This variable is environment-specific and must be set before running the test. | Verify that `video_src_url_widevine_clear_lead_hls_hevc_aac_drmconfigs` is configured with valid Widevine DRM license server details. |
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_widevine_clear_lead_hls_hevc_aac>.mpd&operations=close(150)&autotest=true&type=dash` | Ensure that the launch URL is stored in PersistentStore. |
