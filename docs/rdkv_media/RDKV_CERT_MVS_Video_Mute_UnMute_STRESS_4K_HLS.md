...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio, unmute audio, repeat. Build the test URL with the video_src_url_4k_hls (HLS stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC/master.m3u8&operations=mute,unmute,repeat&autotest=true&type=hls` | Verify that playback operations are configured correctly: Mute audio, unmute audio, repeat. Confirm the test URL is built with video_src_url_4k_hls. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
