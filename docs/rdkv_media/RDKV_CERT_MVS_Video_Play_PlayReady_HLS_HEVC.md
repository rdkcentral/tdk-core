...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of PlayReady DRM protected HEVC codec HLS stream for few minutes and close the player
...
| 2 | Build the test URL using video_src_url_playready_hls_hevc. | Build the test URL with the video_src_url_playready_hls_hevc (HLS stream). | Verify that the test URL is built using video_src_url_playready_hls_hevc. |
