...
## Objective
Test Script to launch a lightning Texts Animation application through AppManager to render given number of text strings and check whether the average FPS calculated for 60 sec duration value is as expected
...
| 4 | Run animation operations and validate outcomes. | App performs objects animation by rendering given number of text strings for 60 sec and stops after that. | Ensure that objects animation by rendering given number of text strings completes successfully without errors. |
