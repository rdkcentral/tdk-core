...
## Objective
Test Script to launch a lightning Shaka player application via AppManager and perform video trickplay operations of HLS h264 video codec content
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, fast-forward, fast-forward, resume playback, pause,<br>play, seek backward, fast-forward, resume playback.<br>Build the test URL with the video_src_url_hls_h264.<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=shaka&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_H264_AAC/master.m3u8`<br>`&operations=seekfwd,fastfwd,fastfwd,playnow,pause,play,seekbwd,fastfwd,playnow&autotest=true` | Ensure that playback operations are configured correctly: Seek forward, fast-forward,<br>fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback.<br>Ensure the test URL is built with video_src_url_hls_h264. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
