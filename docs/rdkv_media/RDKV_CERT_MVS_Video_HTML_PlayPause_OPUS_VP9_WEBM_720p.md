...
RDKV_MEDIA_901
## TestCase Name
RDKV_CERT_MVS_Video_HTML_PlayPause_OPUS_VP9_WEBM_720p
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform video play pause operation on WEBM content with OPUS and VP9 codecs with 720p resolution for few minutes and then close the player. The test confirms that the video pauses correctly, with the video position remaining stationary during the pause interval, and that playback resumes normally with the video position advancing at the expected rate after resume.
...
| 6 | Verify that the stream variable is configured correctly. | Stream variable `video_src_url_vp9_opus_720p` must be defined in `MediaValidationVariables.py` as `test_streams_base_path + "TDK_Asset_Sunrise_VP9_Opus_720p.webm"`, providing the test stream used for playback testing. | Verify that `video_src_url_vp9_opus_720p` resolves to a valid, accessible stream location for this test. |
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_vp9_opus_720p>&operations=pause(30),play(10)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
