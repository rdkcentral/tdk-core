...
|6| Vp9 4k URL Type Configured | MediaValidationVariables.vp9_4k_url_type should be set to the VP9 4K stream type value.| MediaValidationVariables.vp9_4k_url_type is configured with the required value.|
...
| 2 | Configure Playback Operations and Test URL | Start playback of the content and close the player after 30s (close_interval). Build the test URL with the video_src_url_4k_vp9. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (sdk player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_VP9_2160p.webm&operations=close(30)&autotest=true` | Verify that playback is configured to start content and close the player after 30s (close_interval). Confirm the test URL is built with video_src_url_4k_vp9. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
