...
| 2 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback. Build the test URL with the video_src_url_playready_dash_h264 (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_playready_dash_h264>&operations=seekfwd,fastfwd,playnow,pause,play,seekbwd,fastfwd,playnow&autotest=true&type=dash` | Verify that playback operations are configured correctly: Seek forward, fast-forward, resume playback, pause, play, seek backward, fast-forward, resume playback. Confirm the test URL is built with video_src_url_playready_dash_h264. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
