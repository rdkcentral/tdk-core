...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video trickplay operations of PlayReady DRM protected DASH H264 video codec content. The test confirms a comprehensive set of trick-play operations including forward seek, fast-forward, normal speed resumption, pause, play resumption, and backward seek. Each individual operation is validated for correct behaviour through the appropriate media events and video position verification.
...
| 6 | Execute seek and fast-forward operations and capture media events. | The player executes a full sequence of trick-play operations in order: seeks the video forward to a new position, sets fast-forward speed, resumes normal playback speed, pauses the video, resumes playback, and seeks the video backward. Each operation is individually validated through the appropriate media events and video position verification. | Ensure that the expected media events are captured for the configured operations. |
| 7 | Validate observed events for the trick-play operation and update test result. | If the expected event fires for each trick-play operation and the resulting video position or playback speed is correct for each step, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the trick-play event validation and proc check status. |
