...
RDKV_MEDIA_1034
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Seek_Pos_DASH_VP9
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform seek forward and seek backward of VP9 content, validating the expected media events and player behaviour. The test confirms that the video seeks correctly to configured absolute positions within the stream, with the resulting video position landing within the acceptable range of the intended target for both forward and backward seek positions.
...
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_vp9>&operations=seekfwd(30),seekbwd(30),close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
