...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video loop playback of VP9 video codec content
...
| 4 | Verify that the required URL type configuration is set correctly. | MediaValidationVariables.vp9_url_type should be set to the VP9 stream type value. | Ensure that the required URL type configuration should be set correctly. |
...
| 2 | Build the test URL using video_src_url_short_duration_vp9. | Build the test URL with the video_src_url_short_duration_vp9. | Verify that the test URL is built using video_src_url_short_duration_vp9. |
...
| 6 | Run playback operations and validate media events. | App performs playback of VP9 stream. | Ensure that the app performs all configured operations without errors. The 'playing' event should be observed, confirming successful media playback. |
