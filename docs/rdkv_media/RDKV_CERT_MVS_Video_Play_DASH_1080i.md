...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of DASH content with 1080i display resolution for few minutes and close the player
...
| 4 | Verify TV connection and 1080i resolution support. | TV should be connected with the test device. If the 1080i resolution is not supported then the test will be marked as failure. | Ensure that the TV is connected and 1080i resolution is supported; otherwise mark the test as failure. |
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Verify that get the current video display resolution and set resolution as 1080i. | Get the current video display resolution and set resolution as 1080i. | Ensure that get the current video display resolution and set resolution as 1080i should be validated successfully. |
...
| 3 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_dash (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br><code>http://&lt;TM_IP&gt;:&lt;port&gt;/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html<br>?player=sdk<br>&url=http://&lt;TM_IP&gt;:&lt;port&gt;/TDK_Clear_Test_Streams_Sunrise/DASH_H264_AAC/atfms_291_dash_tdk_avc_aac_fmp4.mpd<br>&operations=close<br>&autotest=true<br>&type=DASH</code>. | Ensure that the launch URL is stored in PersistentStore. |
