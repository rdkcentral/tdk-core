## TestCase ID
RDKV_Media_Validation_67
## TestCase Name
RDKV_CERT_MVS_Video_Play_OGG
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video play operation of OGG stream for few minutes and close the player
...
| 4 | Ogg URL Type Configured | MediaValidationVariables.ogg_url_type should be set to the OGG stream type value. | Ensure that MediaValidationVariables.ogg_url_type is configured with the required value. |
