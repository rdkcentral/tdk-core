...
## Objective
Test script to launch a Lightning Unified Player application through AppManager and perform playback of DASH H264 content, validating the expected media events and player behaviour.
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations. | Configure the `close(150)` operation: the video player will start playing the content and automatically close the player after 150 seconds. | Ensure that the playback operations are set correctly. |
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_widevine_clear_lead_dash_h264_ac3 (DASH stream). Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_ac3_url>.mpd&drmconfigs=com.widevine(<license_url>)&operations=close(150)&autotest=true&type=dash`| Ensure that the launch URL is stored in PersistentStore. |
...
| 6 | Validate observed events and set pass/fail status. | If expected event (playing) is observed, the app reports SUCCESS; otherwise, it reports FAILURE. | Ensure that pass/fail status matches observed event validation. |
