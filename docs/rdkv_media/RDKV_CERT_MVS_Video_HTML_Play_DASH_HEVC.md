## TestCase ID
RDKV_Media_Validation_1023
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Play_DASH_HEVC
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test script to launch a HTML5 Player application via AppManager and perform playback of HEVC/H265 content, validating the expected media events and player behaviour.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_src_url_hevc>&operations=close(30)&autotest=true` | Ensure that the launch URL is stored in PersistentStore. |
