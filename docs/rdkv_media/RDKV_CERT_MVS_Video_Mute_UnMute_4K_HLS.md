## TestCase ID
RDKV_Media_Validation_352
## TestCase Name
RDKV_CERT_MVS_Video_Mute_UnMute_4K_HLS
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
Test Script to launch a lightning Video player application via AppManager to play 4K HLS content and perform mute and unmute operations
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Mute audio (30s), unmute audio (30s).<br>Build the test URL with the video_src_url_4k_hls (HLS stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (hlsjs player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/HLS_HEVC_AAC/master.m3u8`<br>`&operations=mute(30),unmute(30)&autotest=true&type=HLS` | Ensure that playback operations are configured correctly: Mute audio (30s), unmute audio (30s).<br>Ensure the test URL is built with video_src_url_4k_hls. Ensure the URL is stored in<br>PersistentStore (MVS/lightningURL) successfully. |
