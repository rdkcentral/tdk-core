...
|3| Precondition Met | TV should be connected with the test device. If the dolby digital (DD) sound mode is not supported then the test will be marked as failure| Condition is met successfully.|
|4| Close Interval Configured | MediaValidationVariables.close_interval should be set to the close interval value (in seconds).| MediaValidationVariables.close_interval is configured with the required value.|
|5| Dolby URL Type Configured | MediaValidationVariables.dolby_url_type should be set to the Dolby stream type value.| MediaValidationVariables.dolby_url_type is configured with the required value.|
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Execute Test Step | Get the current sound mode and set audio sound mode as dolby digital (DD) | Step executed successfully. |
| 2 | Configure Playback Operations and Test URL | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_dolby. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br>`http://<TM_IP>:<port>/rdk-test-tool/fileStore/lightning-apps/unifiedplayer/build/index.html?player=sdk&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/DASH_H264_EC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd&operations=close&autotest=true` | Verify that playback is configured to start content and close the player after the configured duration (close_interval). Confirm the test URL is built with video_src_url_dolby. Confirm the URL is stored in PersistentStore (MVS/lightningURL) successfully. |
...
| 5 | Validate Event Result and Update Test Status | If expected event ('playing') is observed then update the result as SUCCESS or else FAILURE | Confirm that expected event ('playing') is observed. Check that the test result is updated as SUCCESS. |
| 6 | Update Test Result Based on Event Validation | Update the test script result as SUCCESS/FAILURE based on event validation result and proc check status (if applicable) | Check that the test result is updated as SUCCESS/FAILURE based on event validation result. |
