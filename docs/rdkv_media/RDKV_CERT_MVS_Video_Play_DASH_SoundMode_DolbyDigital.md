...
## Objective
Test Script to launch a lightning Video player application through AppManager and perform video play operation of DASH content with Dolby Digital (DD) sound mode for few minutes and close the player
...
| 4 | Verify TV connection and dolby digital (DD) sound mode support. | TV should be connected with the test device. If the dolby digital (DD) sound mode is not supported then the test will be marked as failure. | Ensure that the TV is connected and dolby digital (DD) sound mode is supported; otherwise mark the test as failure. |
...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Verify that get the current sound mode and set audio sound mode as dolby digital (DD). | Get the current sound mode and set audio sound mode as dolby digital (DD). | Ensure that get the current sound mode and set audio sound mode as dolby digital (DD) should be validated successfully. |
...
| 3 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_dolby. Store the constructed URL in PersistentStore for AppManager launch. Sample URL (dashjs player):<br><code>http://&lt;TM_IP&gt;:&lt;port&gt;/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html<br>?player=sdk<br>&url=http://&lt;TM_IP&gt;:&lt;port&gt;/TDK_Clear_Test_Streams_Sunrise/DASH_H264_EC3/atfms_291_dash_tdk_avc_eac3_fmp4.mpd<br>&operations=close<br>&autotest=true</code>. | Ensure that the launch URL is stored in PersistentStore. |
