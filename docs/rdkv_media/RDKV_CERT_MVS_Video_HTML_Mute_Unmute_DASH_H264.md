...
## Objective
Test script to launch a HTML5 Player application through AppManager and perform mute audio and unmute audio of DASH content, validating the expected media events and player behaviour.
...
| 3 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_dash_h264_url>&operations=mute(30),unmute(30)`| Ensure that the launch URL is stored in PersistentStore. |
