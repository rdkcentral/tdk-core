...
|#| StepName | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Store the launch URL in PersistentStore. | Start playback of the content and close the player after the configured duration (close_interval). Build the test URL with the video_src_url_4k_hls. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/htmlplayer.html?url=<video_hls_hevc_url>&operations=close(30)`| Ensure that the launch URL is stored in PersistentStore. |
