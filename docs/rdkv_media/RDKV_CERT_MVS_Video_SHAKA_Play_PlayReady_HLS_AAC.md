...
| 2 | Store the launch URL in PersistentStore. | Build the test URL with the video_src_url_playready_hls_aac. Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<playready_hls_aac_url>.m3u8&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true`| Ensure that the launch URL is stored in PersistentStore. |
