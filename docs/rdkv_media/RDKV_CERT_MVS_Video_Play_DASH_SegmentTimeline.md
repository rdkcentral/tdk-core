...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Play till end of content (0s).<br>Build the test URL with the video_src_url_dash_segement_timeline (DASH stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL:<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/<video_src_url_dash_segement_timeline>`<br>`&operations=playtillend(0)&autotest=true` | Ensure that playback operations are configured correctly: Play till end of content (0s).<br>Ensure the test URL is built with video_src_url_dash_segement_timeline. Ensure the URL is<br>stored in PersistentStore (MVS/lightningURL) successfully. |
