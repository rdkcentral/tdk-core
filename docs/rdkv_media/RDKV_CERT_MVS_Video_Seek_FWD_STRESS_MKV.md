...
## Objective
Test Script to launch a lightning Video player application via AppManager and perform video seek forward operation of MKV content repeatedly for given number of times in provided interval
...
|#|StepName | Step Description| Expected Result|
|-|---------|-----------------|----------------|
| 1 | Configure Playback Operations and Test URL | Set playback operations: Seek forward, repeat.<br>Build the test URL with the video_src_url_4k_mkv (MKV stream).<br>Store the constructed URL in PersistentStore for AppManager launch.<br>Sample URL (sdk player):<br>`http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html`<br>`?player=sdk`<br>`&url=http://<TM_IP>:<port>/TDK_Clear_Test_Streams_Sunrise/TDK_Asset_Sunrise_2160p.MKV`<br>`&operations=seekfwd,repeat&autotest=true&type=MKV` | Ensure that playback operations are configured correctly: Seek forward, repeat. Ensure the<br>test URL is built with video_src_url_4k_mkv. Ensure the URL is stored in PersistentStore<br>(MVS/lightningURL) successfully. |
