...
RDKV_MEDIA_1131
## TestCase Name
RDKV_CERT_MVS_Video_TrickPlay_MKV
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video trickplay operations of MKV stream. The test confirms a comprehensive set of trick-play operations including forward seek, fast-forward, normal speed resumption, pause, play resumption, and backward seek. Each individual operation is validated for correct behaviour through the appropriate media events and video position verification.
...
|#| Step Name | Step Description | Expected Result |
|-|---------|-----------------|----------------|
| 1 | Set playback operations for the scenario. | Configure the `seekfwd(10),fastfwd(10),fastfwd(10),playnow(10),pause(10),play(10),seekbwd(10),fastfwd(10),playnow(10)` operations: the video player will seek forward for 10 seconds, then fast-forward for 10 seconds, then fast-forward for 10 seconds, then resume playback for 10 seconds, then play for 10 seconds and then pause the video, then keep paused for 10 seconds and then resume playback, then seek backward for 10 seconds, then fast-forward for 10 seconds, then resume playback for 10 seconds. | Ensure playback operations are set as specified. |
| 2 | Store the launch URL in PersistentStore. | Store the constructed URL in PersistentStore for AppManager launch. <br>Sample URL: `http://<TM_IP>:<port>/tdkservice/fileStore/lightning-apps/unifiedplayer/build/index.html?url=<video_src_url_4k_mkv>&operations=seekfwd(10),fastfwd(10),fastfwd(10),playnow(10),pause(10),play(10),seekbwd(10),fastfwd(10),playnow(10)&autotest=true&type=mkv` | Ensure that the launch URL is stored in PersistentStore. |
...
| 5 | Execute seek and fast-forward operations and capture media events. | The player executes a full sequence of trick-play operations in order: seeks the video forward to a new position, sets fast-forward speed, resumes normal playback speed, pauses the video, resumes playback, and seeks the video backward. Each operation is individually validated through the appropriate media events (**seeking**, **seeked**, **ratechange**, **play**, **paused**) and video position verification. | Ensure that the expected media events are captured for the configured operations. |
| 6 | Validate observed events for the trick-play operation and update test result. | If the expected events (**seeking**, **seeked**, **ratechange**, **play**, **paused**) fire for each trick-play operation and the resulting video position or playback speed is correct for each step, the app reports SUCCESS, otherwise FAILURE. The test result is updated as SUCCESS or FAILURE based on event validation and process check status. | Ensure that the test result is updated as SUCCESS or FAILURE based on the trick-play event validation and proc check status. |
