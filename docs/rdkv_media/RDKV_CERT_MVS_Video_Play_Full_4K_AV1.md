...
RDKV_MEDIA_509
## TestCase Name
RDKV_CERT_MVS_Video_Play_Full_4K_AV1
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play operations of 4k AV1 codec stream till the end. The test confirms that the stream plays from beginning to natural end without interruption. Successful playback start and natural stream completion are both validated, with the video position advancing continuously throughout.
