...
RDKV_MEDIA_922
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Setvolume_AC3_H265_MP4
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform volume level operation on MP4 content with H265 and AC3 codecs for a few minutes and then close the player. The test confirms that the audio volume is set to the configured level and that the volume change is validated by a **volumechange** event. Video playback continues normally after the volume change.
