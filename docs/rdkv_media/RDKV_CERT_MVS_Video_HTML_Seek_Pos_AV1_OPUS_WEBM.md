...
RDKV_MEDIA_914
## TestCase Name
RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_OPUS_WEBM
<a name="head.TOC"></a>
## Table Of Contents
...
## Objective
To launch a HTML Video Player application via AppManager and perform video seek position operation of WEBM content with AV1 and OPUS codecs for few minutes and close the player. The test confirms that the video seeks correctly to configured absolute positions within the stream, with the resulting video position landing within the acceptable range of the intended target for both forward and backward seek positions.
