...
## Objective
To launch a Video Player integrated with lightning application via AppManager and perform video play pause operation of VP9 codec stream. The test confirms that the video pauses correctly, with the video position remaining stationary during the pause interval, and that playback resumes normally with the video position advancing at the expected rate after resume.
