# FNCS_Playback_Appsrc_Audio_Underflow_Playback_EAC3 Test Case Documentation

## TestCase ID
FNCS_PLAYBACK_255

## TestCase Name
FNCS_Playback_Appsrc_Audio_Underflow_Playback_EAC3
...
| 2 | atfms_291_dash_tdk_avc_eac3_fmp4_ac3.mp4 must be installed in the server hosting streams or installed inside the device if user is selecting filesrc instead of httpsrc (copying stream to USB like file:/tmp/usb/) or httpsrc via HTTPS server (https://<server_hosting_stream>:<port_number>/) |
...
| 11 | audio_src_url_mp4_eac3 variable in MediaValidationVariables.py contains EAC3 audio stream URL (atfms_291_dash_tdk_avc_eac3_fmp4_ac3.mp4) for appsrc underflow testing |
...
| 2 | Execute Media Pipeline Test | Retrieve FIREBOLT_COMPLIANCE_CHECK_AV_STATUS config value, get EAC3 stream URL from MediaValidationVariables.audio_src_url_mp4_eac3, construct tdk_mediapipelinetests command with underflow threshold parameter (1094429 bytes), and run test_appsrc_audio_underflow with EAC3 stream | Configuration values retrieved, EAC3 stream URL obtained, command executed: `tdk_mediapipelinetests test_appsrc_audio_underflow https://<server_hosting_stream>:<port_number>/DASH_H264_EC3/atfms_291_dash_tdk_avc_eac3_fmp4_ac3.mp4 checkavstatus=no validateFullPlayback underflow_threshold=1094429`, and test application starts execution without errors |
...
| 5 | Setup Appsrc Element | Configure appsrc with size property set to underflow threshold (1094429 bytes), set appropriate EAC3 audio caps, and connect push-buffer signal for manual feeding | Appsrc element configured successfully with threshold size and EAC3 audio capabilities |
...
| 7 | Reach Buffer Threshold | Continue feeding audio buffers via appsrc until threshold bytes (1094429) are reached | Threshold limit reached successfully, pipeline consuming all provided audio buffer data |
...
| 11 | Update Buffer Threshold | Double the buffer threshold value to 2188858 bytes and prepare for additional audio buffer feeding | Buffer threshold updated successfully, ready for extended audio buffer feeding |
...
| 16 | Validate Test Output | Parse tdk_mediapipelinetests output for success indicators "Failures: 0", "Errors: 0", or "failed: 0" | Test output contains success strings indicating EAC3 appsrc audio underflow recovery completed without errors |
