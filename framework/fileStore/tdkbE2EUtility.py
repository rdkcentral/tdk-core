...
import parseDeviceconfig as deviceConfigParser

# Global variable to check whether login session is active
isSessionActive = False
isPerformanceTest = False
clientOutput_avg = 0
parsedDeviceConfigParameters = set()
...
def parseDeviceConfig(obj):
    ...
    for parameter in parsedDeviceConfigParameters:
        globals().pop(parameter, None)
    parsedDeviceConfigParameters.clear()

    status = deviceConfigParser.parseDeviceConfig(obj)
    if status.upper() != "SUCCESS":
        return status

    parsedValues = {
        parameter: getattr(deviceConfigParser, parameter)
        for parameter in deviceConfigParser.PARSED_PARAMETERS
    }
    globals().update(parsedValues)
    parsedDeviceConfigParameters.update(parsedValues)
...
def clientConnect(clientType):
    ...
    try:
        ...
        if clientType == "WLAN":
            isSessionActive = session.login(
                wlan_ip, wlan_username, wlan_password)
        elif clientType == "LAN":
...
def checkSsidAvailable(ssidName):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s is_ssid_available %s" % (
                wlan_script, ssidName)
    ...
    try:
        if wlan_os_type == "UBUNTU":
            if securityType == "Protected":
                command = "sudo sh %s wifi_ssid_connect %s %s" % (
                    wlan_script, ssidName, ssidPwd)
            else:
                command = "sudo sh %s wifi_ssid_connect_openSecurity %s" % (
                    wlan_script, ssidName)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
...
def getConnectedSsidName(wlanInterface):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_connected_ssid_name %s" % (
                wlan_script, wlanInterface)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
    except Exception as e:
            ...
            if wlan_os_type == "UBUNTU":
                ...
                if mlo_capability == "False":
                    if ssid_2ghz_name in status or ssid_5ghz_name in status or ssid_6ghz_name in status:
                        command = "sudo sh %s wifi_ssid_disconnect %s" % (
                            wlan_script, wlanInterface)
                        status = executeCommand(command)
                    else:
...
def wlanDisconnectWifiSsid(wlanInterface):
    ...
    try:
        ...
        if mlo_capability == "False":
            if wlan_2ghz_ssid_disconnect_status in status or wlan_5ghz_ssid_disconnect_status in status or wlan_6ghz_ssid_disconnect_status in status or "SSID is already disconnected" in status:
                return "SUCCESS"
            else:
...
def getWlanIPAddress(wlanInterface):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_wlan_ip_address %s %s" % (
                wlan_script, wlanInterface, wlan_inet_address)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
    except Exception as e:
...
def getWLANIPV6Address(wlanInterface):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                command = "sudo sh %s refresh_wifi_network %s" % (
                    wlan_script, wlanInterface)
                executeCommand(command)
                sleep(20)

                command = "sudo sh %s get_wlan_ipv6_address %s %s" % (
                    wlan_script, wlanInterface, wlan_inet6_address)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
...
def getWlanSubnetMask(wlanInterface):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_wlan_subnet_mask %s %s" % (
                wlan_script, wlanInterface, wlan_subnet_mask)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
    except Exception as e:
...
def getLanIPAddress(lanInterface):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                command = "sudo sh %s refresh_lan_network %s" % (
                    lan_script, lanInterface)
                executeCommand(command)
                sleep(20)

                command = "sudo sh %s get_lan_ip_address %s %s" % (
                    lan_script, lanInterface, lan_inet_address)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                ...
                command = "sudo sh %s get_lan_ipv6_address %s %s" % (
                    lan_script, lanInterface, lan_inet6_address)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                ...
                command = "sudo sh %s get_lan_subnet_mask %s %s" % (
                    lan_script, lanInterface, lan_subnet_mask)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
...
def getWlanMACAddress(wlanInterface):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_wlan_mac %s" % (
                wlan_script, wlanInterface)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
...
def getChannelNumber(ssidName):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_channel_number %s" % (
                wlan_script, ssidName)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
...
def getSecurityMode(ssidName):
    ...
    try:
        security_mode = ""
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_security_mode %s" % (
                wlan_script, ssidName)
            status = executeCommand(command)
            print(status)
            if status == "WPA2":
    ...
    if setup_type == "TDK":
        ...
        for elements in paramList:
            ...
            if elements[2] == "string":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 0}
            elif elements[2] == "boolean" or elements[2] == "bool":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 3}
            elif elements[2] == "unsignedint" or elements[2] == "unsignedInt":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 2}
            elif elements[2] == "int":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 1}
            elif elements[2] == "double":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 9}
            elif elements[2] == "long":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 6}
            elif elements[2] == "unsignedlong":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 7}
            elif elements[2] == "float":
                queryParam = {'name': elements[0],
                              'value': elements[1], 'dataType': 8}
            else:
                actualresult = "FAILURE"
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                if connectivityType == "PING":
                    command = "sudo sh %s ping_to_network %s %s %s" % (
                        script_name, source_ip, dest_ip, gateway_ip)
                if connectivityType == "PING_TO_HOST":
                    command = "sudo sh %s ping_to_host %s %s %s" % (
                        script_name, dest_ip, gateway_ip, interface)
                elif connectivityType == "WGET_HTTP":
                    command = "sudo sh %s wget_http_network %s %s %s" % (
                        script_name, source_ip, dest_ip, http_port)
                elif connectivityType == "WGET_HTTPS":
                    command = "sudo sh %s wget_https_network %s %s %s" % (
                        script_name, source_ip, dest_ip, https_port)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                if connectivityType == "IPV6_PING_TO_HOST":
                    command = "sudo sh %s ipv6_ping_to_host %s %s" % (
                        script_name, host_name, interface)
                status = executeCommand(command)
            else:
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU":
                if dest == "WLAN" and source == "WAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        wan_script, network_ip, wlan_ftp_username, wlan_ftp_password)
                elif dest == "WLAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        lan_script, network_ip, wlan_ftp_username, wlan_ftp_password)
                elif dest == "LAN" and source == "WAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        wan_script, network_ip, lan_ftp_username, lan_ftp_password)
                elif dest == "LAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        wlan_script, network_ip, lan_ftp_username, lan_ftp_password)
                elif dest == "WAN" and source == "LAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        lan_script, network_ip, wan_ftp_username, wan_ftp_password)
                elif dest == "WAN" and source == "WLAN":
                    command = "sudo sh %s ftpToClient %s %s %s" % (
                        wlan_script, network_ip, wan_ftp_username, wan_ftp_password)
                else:
...
def telnetToClient(dest, dest_ip, source="LAN"):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            ...
            if status == "SUCCESS":
                if dest == "WLAN":
                    command = "sudo sh %s telnetToClient %s %s %s" % (
                        lan_script, dest_ip, wlan_username, wlan_password)
                elif dest == "LAN":
                    command = "sudo sh %s telnetToClient %s %s %s" % (
                        wlan_script, dest_ip, lan_username, lan_password)
                elif dest == "WAN" and source == "WLAN":
                    command = "sudo sh %s telnetToClient %s %s %s" % (
                        wlan_script, dest_ip, wan_username, wan_password)
                elif dest == "WAN" and source == "LAN":
                    command = "sudo sh %s telnetToClient %s %s %s" % (
                        lan_script, dest_ip, wan_username, wan_password)
                else:
...
def getWlanAccessPoint(wlanInterface):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            command = "sudo sh %s get_wlan_accesspoint %s" % (
                wlan_script, wlanInterface)
            status = executeCommand(command)
        else:
            status = "Only UBUNTU platform supported!!!"
...
def deleteSavedWifiConnections():
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                if mlo_capability == "False":
                    command = "sudo sh %s delete_saved_wifi_connections %s %s %s" % (
                        wlan_script, ssid_2ghz_name, ssid_5ghz_name, ssid_6ghz_name)
                    status = executeCommand(command)
                else:
                    command = "sudo sh %s delete_saved_wifi_connections %s" % (
                        wlan_script, ssid_name)
                    status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                command = "sudo sh %s add_static_route %s %s %s" % (
                    script_name, destIp, gwIp, interface)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                if source == "WLAN":
                    ...
                    script_name = lan_script
                command = "sudo sh %s del_static_route %s %s %s" % (
                    script_name, destIp, gwIp, interface)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                if connectivityType == "WGET_HTTP":
                    command = "sudo sh %s wget_http_network %s %s %s" % (
                        script_name, source_ip, wan_http_ip, wan_http_port)
                elif connectivityType == "WGET_HTTPS":
                    command = "sudo sh %s wget_https_network %s %s %s" % (
                        script_name, source_ip, wan_https_ip, wan_https_port)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                if connectivityType == "WGET_HTTP":
                    command = "sudo sh %s wget_http_network %s %s %s" % (
                        script_name, source_ip, url, parentalCtl_port)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                ...
                if connectivityType == "WGET_HTTP":
                    command = "sudo sh %s wget_http_network %s %s %s" % (
                        script_name, source_ip, dest_ip, gw_port)
                elif connectivityType == "WGET_HTTPS":
                    command = "sudo sh %s wget_https_network %s %s %s" % (
                        script_name, source_ip, dest_ip, gw_port)
                status = executeCommand(command)
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                if source == "WLAN":
                    ...
                    script_name = lan_script
                command = "sudo sh %s nslookup_in_client %s %s" % (
                    script_name, domainName, serverIP)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
...
def getLanDhcpDetails(param):
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                command = "sudo sh %s get_lan_dhcp_details %s %s %s" % (
                    lan_script, lan_dhcp_location, lan_interface, param)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
...
def getThroughput(source, destination, dest_ip, src_ip, outfile, expected_perf, tdkTestObj, connectivityType="TCP"):
    ...
    try:
        ...
        status, outputValue, clientOutput = tcp_udpInClients(
            source, destination, dest_ip, src_ip, connectivityType)
...
def tcp_udpInClients(source, destination, dest_ip, src_ip, connectivityType="TCP"):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            status, clientOutput = RequestToServer(
                source, dest_ip, src_ip, connectivityType)
            if status == "SUCCESS":
...
#                       clientOutput_avg = clientOutput[-1]
                    ...
                    validationStatus, outputValue = validateTcpUdpOutput(
                        source, destination, connectivityType)
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if "TCP" in connectivityType:
                if isPerformanceTest == True:
                    command = "sudo sh %s tcp_init_server_perf %s %s %s %s" % (
                        dest_script_name, tmp_file_lan, dest_ip, perf_test_duration, perf_test_poll_interval)
                else:
                    command = "sudo sh %s tcp_init_server %s %s" % (
                        dest_script_name, tmp_file_lan, dest_ip)
            elif connectivityType == "UDP":
                command = "sudo sh %s udp_init_server %s" % (
                    dest_script_name, dest_ip)
            status = executeCommand(command)
        else:
            status = "Failed to connect to client"
...
def RequestToServer(source, dest_ip, src_ip, connectivityType):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            if "TCP" in connectivityType:
                if isPerformanceTest == True:
                    command = "sudo sh %s tcp_request_perf %s %s %s %s %s" % (
                        src_script_name, dest_ip, src_ip, tmp_file_wlan, perf_test_duration, perf_test_poll_interval)
                    output = executeCommand(command)
                    sleep(int(perf_test_duration)+10)
                    command = "sudo sh %s tcp_get_client_throughput %s %s" % (
                        src_script_name, tmp_file_wlan, perf_out_file)
                else:
                    command = "sudo sh %s tcp_request %s %s %s" % (
                        src_script_name, dest_ip, src_ip, tmp_file_wlan)
            elif connectivityType == "UDP":
                command = "sudo sh %s udp_request %s %s %s" % (
                    src_script_name, dest_ip, tmp_file_wlan, src_ip)
            output = executeCommand(command)
            if output:
                status = "SUCCESS"
            else:
    ...
    try:
        if connectivityType == "TCP":
            ...
            if status == "SUCCESS":
                ...
                command = "sudo sh %s validate_tcp_server_output %s" % (
                    dest_script_name, tmp_file_lan)
        ...
        if connectivityType == "UDP":
            ...
            if status == "SUCCESS":
                ...
                command = "sudo sh %s validate_udp_output %s" % (
                    src_script_name, tmp_file_wlan)
                outputValue = executeCommand(command)
                if outputValue:
                    status = "SUCCESS"
                else:
        ...
        if connectivityType == "TCP_Throughput":
            ...
            if status == "SUCCESS":
                # Get the bandwidth from server side and validate it.
                command = "sudo sh %s validate_tcp_server_output_throughput %s" % (
                    dest_script_name, tmp_file_lan)
                outputValue = executeCommand(command)
                if outputValue >= clientOutput:
                    status = "SUCCESS"
...
def scpLogFromClientToTM(source, outputFile):
    ...
    try:
        ...
        destinationLogPath = str(
            testObj.execID) + "_" + str(testObj.execDevId) + "_" + str(testObj.resultId) + "_"
        ...
        if wlan_os_type == "UBUNTU":
            if source == "WLAN":
                ...
                command = "scp %s@%s:%s %s" % (wlan_username,
                                               wlan_ip, outputFile, tm_logs_location)
                pswd = wlan_password
            elif source == "LAN":
                command = "scp %s@%s:%s %s" % (lan_username,
                                               lan_ip, outputFile, tm_logs_location)
                pswd = lan_password
            ...
            if ret == 0:
                ...
                out = subprocess.Popen(
                    ['ls', org_file], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                ...
                if out.returncode == 0 and "No such file" not in stdout:
                    ...
                    out = subprocess.Popen(
                        ['mv', org_file, tmFileName], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                    stdout, stderr = out.communicate()
            ...
            if lan_os_type == "UBUNTU":
                ...
                if (src == "WLAN"):
                    ...
                    if status == "SUCCESS":
                        ...
                        command = "sudo sh %s ftpFromWlan %s %s %s %s" % (
                            wlan_script, dest_ip, lan_username, lan_password, ftp_test_file)
                        status = executeCommand(command)
                        ...
                        if "230 Login successful" and "Transfer complete" in status:
                            ...
                            if status == "SUCCESS":
                                ...
                                command = "sudo sh %s validate_FTP %s" % (
                                    lan_script, ftp_test_file)
                                status = executeCommand(command)
                                if status == "SUCCESS":
                                    # Reomve the ftp_test_file created for FTP download validation
                                    command = "sudo sh %s remove_File %s" % (
                                        lan_script, ftp_test_file)
                                    status = executeCommand(command)
                    ...
                    if status == "SUCCESS":
                        ...
                        command = "sudo sh %s touch_File %s" % (
                            lan_script, ftp_test_file)
                        status = executeCommand(command)
                        if status == "SUCCESS":
                            status = clientConnect(dest)
                            if status == "SUCCESS":
                                ...
                                command = "sudo sh %s ftpFromlan %s %s %s %s" % (
                                    wlan_script, src_ip, lan_username, lan_password, ftp_test_file)
                                status = executeCommand(command)
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU":
                if source == "WLAN":
                    ...
                    script_name = wan_script
                if dest == "WLAN":
                    command = "sudo sh %s ssh_to_client %s %s %s %s %s" % (
                        script_name, wlan_password, wlan_username, dest_ip, dest_inteface, dest_inet_address)
                if dest == "LAN":
                    command = "sudo sh %s ssh_to_client %s %s %s %s %s" % (
                        script_name, lan_password, lan_username, dest_ip, dest_inteface, dest_inet_address)
                if dest == "WAN":
                    command = "sudo sh %s ssh_to_client %s %s %s %s %s" % (
                        script_name, wan_password, wan_username, dest_ip, dest_inteface, dest_inet_address)
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                if source == "LAN":
                    ...
                    script_name = wlan_script
                command = "sudo sh %s bringup_interface %s" % (
    ...
    try:
        ...
        if status == "SUCCESS":
            if wlan_os_type == "UBUNTU" and lan_os_type == "UBUNTU":
                ...
                command = "sudo sh %s trigger_port %s %s %s" % (
                    script_name, dest_ip, trigger_port, trigger_protocol)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
        else:
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU" and wlan_os_type == "UBUNTU":
                ...
                if serverType == "IPERF" and protocol == "TCP":
                    command = "sudo sh %s tcp_init_server %s %s %s" % (
                        script_name, logFile, serverBindAddr, serverPort)
                elif serverType == "IPERF" and protocol == "UDP":
                    command = "sudo sh %s udp_init_server %s %s %s" % (
                        script_name, logFile, serverBindAddr, serverPort)
                elif serverType == "NETCAT":
                    # Write Message to file
                    command = "sudo sh %s write_msgtofile \"%s\" %s" % (
                        script_name, serverMsg, logFile)
                    status = executeCommand(command)
                    if status == "SUCCESS":
                        command = "sudo sh %s netcat_init_server %s %s %s" % (
                            script_name, serverPort, protocol, logFile)
                    else:
                        status = "FAILURE"
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU" and wlan_os_type == "UBUNTU":
                ...
                if clientType == "IPERF" and protocol == "TCP":
                    command = "sudo sh %s tcp_request %s %s %s %s" % (
                        script_name, gw_wan_ip, clientBindAddr, logFile, clientPort)
                elif clientType == "IPERF" and protocol == "UDP":
                    command = "sudo sh %s udp_request %s %s %s %s" % (
                        script_name, gw_wan_ip, clientBindAddr, logFile, clientPort)
                elif clientType == "NETCAT":
                    command = "sudo sh %s netcat_request %s %s %s %s" % (
                        script_name, gw_wan_ip, clientPort, protocol, logFile)
    ...
    try:
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU" and wlan_os_type == "UBUNTU":
                ...
                command = "sudo sh %s read_fileContent %s" % (
                    script_name, logFile)
...
def tftpToClient(destIP, server, fileName, serverFileMsg, client, port="69"):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            command = "sudo sh %s write_msgtofile \"%s\" %s" % (
                script_name, serverFileMsg, serverFileFullPath)
            ...
            if status == "SUCCESS":
                status = clientConnect(client)
                if status == "SUCCESS":
                    command = "sudo sh %s tftpToClient %s %s %s %s" % (
                        wan_script, destIP, serverFileFullPath, fileName, port)
                    status = executeCommand(command)
...
def ftpToClientWithPort(dest, destIP, source, port):
    ...
    try:
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU":
                if dest == "WLAN" and source == "WAN":
                    command = "sudo sh %s ftpToClient %s %s %s %s" % (
                        wan_script, destIP, wlan_ftp_username, wlan_ftp_password, port)
                elif dest == "LAN" and source == "WAN":
                    command = "sudo sh %s ftpToClient %s %s %s %s" % (
                        wan_script, destIP, lan_ftp_username, lan_ftp_password, port)
                else:
                    return "Invalid source or destination"
        ...
        if status == "SUCCESS":
            ...
            if wlan_os_type == "UBUNTU":
                if source == "LAN":
                    ...
                    script_name = wlan_script
                command = "sudo sh %s bringdown_interface %s" % (
                    script_name, interface)
                status = executeCommand(command)
            else:
                status = "Only UBUNTU platform supported!!!"
...
def PTPreRequisite(obj, step):
    ...
    tdkTestObj, retStatus, initialEnable = getParameterValue(
        obj, "Device.NAT.X_CISCO_COM_PortTriggers.Enable")

    if expectedresult in retStatus and initialEnable != "":
        # Set the result status of execution
        tdkTestObj.setResultStatus("SUCCESS")
        print("ACTUAL RESULT %d: Device.NAT.X_CISCO_COM_PortTriggers.Enable : %s" % (
            step, initialEnable))
        print("TEST EXECUTION RESULT : SUCCESS")
        ...
        if initialEnable == "false":
            ...
            tdkTestObj, retStatus, details = setMultipleParameterValues(
                obj, setString)
            ...
            if expectedresult in retStatus and details != "":
                ...
                tdkTestObj.setResultStatus("SUCCESS")
                print(
                    "ACTUAL RESULT %d: Device.NAT.X_CISCO_COM_PortTriggers.Enable set to true successfully" % step)
                print("TEST EXECUTION RESULT : SUCCESS")
                ...
                tdkTestObj, retStatus, finalEnable = getParameterValue(
                    obj, "Device.NAT.X_CISCO_COM_PortTriggers.Enable")
                ...
                tdkTestObj.setResultStatus("FAILURE")
                print(
                    "ACTUAL RESULT %d: Device.NAT.X_CISCO_COM_PortTriggers.Enable was NOT set to %s successfully" % step)
                print("TEST EXECUTION RESULT : FAILURE")
        else:
            status = "SUCCESS"
            print(
                "Device.NAT.X_CISCO_COM_PortTriggers.Enable is already in enabled state")
    else:
        # Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE")
...
def SetPTRule(obj, obj1, triggerStart, triggerEnd, targetStart, targetEnd, triggerProtocol, targetProtocol, description, enablePTRule, step):
    ...
    tdkTestObj.addParameter(
        "paramName", "Device.NAT.X_CISCO_COM_PortTriggers.Trigger.")
    tdkTestObj.executeTestCase(expectedresult)
    actualresult = tdkTestObj.getResult()
    details = tdkTestObj.getResultDetails()
    ...
    if expectedresult in actualresult:
        # Set the result status of execution
        tdkTestObj.setResultStatus("SUCCESS")
        print("ACTUAL RESULT %d: Instance added successfully: %s" %
              (step, details))
        print("[TEST EXECUTION RESULT] : SUCCESS")
        ...
        if (instance.isdigit() and int(instance) > 0):
            # Setting the rule
            step = step + 1
            print("\nTEST STEP %d: Set the Port triggering rule - Trigger Protocol : %s, Trigger Port Start : %s, Trigger Port End : %s, Target Protocol : %s, Target Port Start : %s, Target Port End : %s, Description : %s" %
                  (step, triggerProtocol, triggerStart, triggerEnd, targetProtocol, targetStart, targetEnd, description))
            print(
                "EXPECTED RESULT %d: Port Triggering Rule should be set successfully" % step)
            setValuesList = [triggerProtocol, triggerStart, triggerEnd,
                             targetProtocol, targetStart, targetEnd, description]
            triggerProtocolParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".TriggerProtocol"
            triggerStartParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".TriggerPortStart"
            triggerEndParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".TriggerPortEnd"
            targetProtocolParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".ForwardProtocol"
            targetStartParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".ForwardPortStart"
            targetEndParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
                instance + ".ForwardPortEnd"
            descriptionParam = "Device.NAT.X_CISCO_COM_PortTriggers.Trigger." + \
            ...
            if expectedresult in actualresult:
                # Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS")
                print("ACTUAL RESULT %d: Port Triggering Rule set successfully : %s" % (
                    step, details))
                print("[TEST EXECUTION RESULT] : SUCCESS")

                # Set the PT rule enable
                step = step + 1
                print("\nTEST STEP %d: Set the enable state of PT rule to %s" %
                      (step, enablePTRule))
                print(
                    "EXPECTED RESULT %d: Should set the enable value successfully" % step)
                ...
                tdkTestObj, status, details = setMultipleParameterValues(
                    obj, setString)

                if expectedresult in status and details != "":
                    # Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS")
                    print("ACTUAL RESULT %d: PT rule set to %s successfully" %
                          (step, enablePTRule))
                    print("TEST EXECUTION RESULT : SUCCESS")
                    ...
                    ParamList = [triggerProtocolParam, triggerStartParam, triggerEndParam, targetProtocolParam,
                                 targetStartParam, targetEndParam, descriptionParam, enablePTRuleParam]
                    setValues = [triggerProtocol, triggerStart, triggerEnd,
                                 targetProtocol, targetStart, targetEnd, description, enablePTRule]
                    tdkTestObj, status, newValues = getMultipleParameterValues(
                        obj, ParamList)
                    ...
                    if expectedresult in status and setValues == newValues:
                        ...
                        tdkTestObj.setResultStatus("FAILURE")
                        print(
                            "ACTUAL RESULT %d: Values SET not reflected in GET" % step)
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    # Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE")
                    print("ACTUAL RESULT %d: PT rule not set to %s successfully" % (
                        step, enablePTRule))
                    print("TEST EXECUTION RESULT : FAILURE")
            else:
                # Set the result status of execution
        ...
        # Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE")
        print("ACTUAL RESULT %d: Instance not added successfully: %s" %
              (step, details))
        print("[TEST EXECUTION RESULT] : FAILURE")
...
def DeletePTRule(obj1, instance, step):
    ...
    tdkTestObj.addParameter(
        "paramName", "Device.NAT.X_CISCO_COM_PortTriggers.Trigger.%s." % instance)
    tdkTestObj.executeTestCase(expectedresult)
    actualresult = tdkTestObj.getResult()
    details = tdkTestObj.getResultDetails()
    ...
    if expectedresult in actualresult:
        # Set the result status of execution
        tdkTestObj.setResultStatus("SUCCESS")
        print("ACTUAL RESULT %d: Added rule deleted successfully: %s" %
              (step, details))
        print("[TEST EXECUTION RESULT] : SUCCESS")
    else:
        # Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE")
        print("ACTUAL RESULT %d: Added rule not deleted successfully: %s" %
              (step, details))
...
def wifiConnectBssid(ssidName, ssidPwd, bssid, securityType):
    ...
    try:
        if wlan_os_type == "UBUNTU":
            if securityType == "Protected":
                command = "sudo sh %s wifi_ssid_bssid_connect %s %s %s" % (
                    wlan_script, ssidName, bssid, ssidPwd)
            else:
                command = "sudo sh %s wifi_ssid_connect_openSecurity %s" % (
                    wlan_script, ssidName)
            status = executeCommand(command)
...
def wlanConnectWifiSsidBssid(ssidName, ssidPwd, bssid, wlanInterface, securityType="Protected"):
    ...
    try:
        ...
        if status == "SUCCESS":
            ...
            status = checkSsidAvailable(ssidName)
            if ssidName in status:
                status = wifiConnectBssid(
                    ssidName, ssidPwd, bssid, securityType)
                if mlo_capability == "False":
                    if wlan_2ghz_ssid_connect_status in status or wlan_5ghz_ssid_connect_status in status or wlan_6ghz_ssid_connect_status in status:
                        sleep(60)
                        status = getConnectedSsidName(wlanInterface)
        ...
        if status == "SUCCESS":
            if lan_os_type == "UBUNTU":
                ...
                if connectivityType == "PING_TO_IPV4":
                    if phase == "START":
                        command = "sudo sh %s ping_to_ipv4_start %s %s %s %s %s" % (
                            script_name, source_ip, dest_ip, gateway_ip, CONNECTIVITY_DURATION, PING_OUTPUT_FILE)
                    elif phase == "CHECK":
                        command = "sudo sh %s ping_to_ipv4_check %s" % (
                            script_name, PING_OUTPUT_FILE)
                    else:
...
def firewallSet(obj, level, step, revert="false"):
    ...
    expectedresult = "SUCCESS"
    status = "FAILURE"
    tdkTestObj, firewallResult, details = setMultipleParameterValues(
        obj, "Device.X_CISCO_COM_Security.Firewall.FirewallLevel|%s|string" % level)

    print(f"\nTEST STEP {step}: Set the firewall level to {level}")
    print(
        f"EXPECTED RESULT {step}: Should set the firewall level to {level} successfully")
    if expectedresult in firewallResult and details != "":
        print(
            f"ACTUAL RESULT {step}: Firewall level set to {level} successfully")
        print(f"TEST EXECUTION RESULT : SUCCESS")
        tdkTestObj.setResultStatus("SUCCESS")

        if revert == "false":
            # Get and cross check the SET operation
            tdkTestObj, firewallResult, details = getParameterValue(
                obj, "Device.X_CISCO_COM_Security.Firewall.FirewallLevel")
            step = step + 1
            print(
                f"\nTEST STEP {step}: Get the firewall level to cross check the SET operation")
            print(
                f"EXPECTED RESULT {step}: Should get the firewall level successfully")
            if expectedresult in firewallResult and details == level:
                print(
                    f"ACTUAL RESULT {step}: Firewall level is {level} as expected")
                print(f"TEST EXECUTION RESULT : SUCCESS")
                tdkTestObj.setResultStatus("SUCCESS")
                status = "SUCCESS"
            else:
                print(
                    f"ACTUAL RESULT {step}: Firewall level is NOT {level} as expected")
        ...
        print(
            f"ACTUAL RESULT {step}: Firewall level NOT set to {level} successfully")
        print(f"TEST EXECUTION RESULT : FAILURE")
        tdkTestObj.setResultStatus("FAILURE")
...
def lanManagementSet(obj, setList, step, revert="false"):
    ...
    expectedresult = "SUCCESS"
    status = "FAILURE"
    tdkTestObj, result, details = setMultipleParameterValues(
        obj, "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress|%s|string|Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask|%s|string|Device.DHCPv4.Server.Pool.1.MinAddress|%s|string|Device.DHCPv4.Server.Pool.1.MaxAddress|%s|string" % (setList[0], setList[1], setList[2], setList[3]))
    print(
        f"\nTEST STEP {step}: Set the values - Lan IP: {setList[0]}, Lan submet mask: {setList[1]}, Min address: {setList[2]}, Max address: {setList[3]}")
    print(
        f"EXPECTED RESULT {step}: Should set the LAN Management values to the required values successfully")
    ...
    if expectedresult in result and details != "":
        ...
        if revert == "false":
            # Get and cross check the SET operation
            tdkTestObj, result, details = getMultipleParameterValues(obj, ["Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress",
                                                                     "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask", "Device.DHCPv4.Server.Pool.1.MinAddress", "Device.DHCPv4.Server.Pool.1.MaxAddress"])
            step = step + 1
            print(
                f"\nTEST STEP {step}: Get the Lan Management parameters to cross check the SET operation")
            print(
                f"EXPECTED RESULT {step}: Should get the Lan Management parameters successfully")
            if expectedresult in result and details == setList:
                print(
                    f"ACTUAL RESULT {step}: Lan management parameters are set as expected")
                print(f"TEST EXECUTION RESULT : SUCCESS")
                tdkTestObj.setResultStatus("SUCCESS")
                status = "SUCCESS"
            else:
                print(
                    f"ACTUAL RESULT {step}: Lan Management parameters are not set as expected")
                print(f"TEST EXECUTION RESULT : FAILURE")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            status = "SUCCESS"
        ...
        print(
            f"ACTUAL RESULT {step}: Lan Management parameters are not SET successfully")
        print(f"TEST EXECUTION RESULT : FAILURE")
        tdkTestObj.setResultStatus("FAILURE")
