...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_DisableSSIDAdvertisement</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that Network name broadcast (SSID advertisement) can be disabled by admin in gateway</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_665</test_case_id>
    <test_objective>Verify that Network name broadcast (SSID advertisement) can be disabled by admin in gateway</test_objective>
...
4. Try to connect to Wifi client and check if the ssid is listing or not
5. Revert the values of SSIDAdvertisementEnabled
6. Unload the module</automation_approch>
    <expected_output>The SSID name for 6GHZ should not list when SSIDAdvertisementEnabled  is set to false</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_DisableSSIDAdvertisement</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_DisableSSIDAdvertisement')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        step += 1
        print(f"TEST STEP {step}: Get the current ssid, keypassphrase, ssidAdvertStatus")
        print(f"EXPECTED RESULT {step}: Should retrieve the current ssid, keypassphrase, and ssidAdvertStatus")
        ...
        if expectedresult in status:
            ...
            step += 1
            print(f"TEST STEP {step}: Set the ssid, keypassphrase and disable the ssid advertisement status")
            print(f"EXPECTED RESULT {step}: Should set the advertisement status, ssid and keypassphrase")
            ...
            if expectedresult in actualresult:
                ...
                step += 1
                print(f"TEST STEP {step}: Get the current ssid, keypassphrase and ssidAdvertStatus")
                print(f"EXPECTED RESULT {step}: Should retrieve the current ssid, keypassphrase and ssidAdvertStatus")
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print(f"EXPECTED RESULT {step}: SSID should not be broadcasted after disabling advertisement")
            ...
            print(f"EXPECTED RESULT {step}: Should set the original ssid, keypassphrase and ssidAdvertStatus")
