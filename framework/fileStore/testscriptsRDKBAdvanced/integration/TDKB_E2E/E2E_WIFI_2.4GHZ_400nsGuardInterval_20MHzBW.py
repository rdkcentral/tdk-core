...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_400nsGuardInterval_20MHzBW</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether Wireless client can connect to WG when Guard interval is 400ns and Channel BW is 20MHZ for frequency band 2.4GHZ</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_11</test_case_id>
    <test_objective>Verify whether Wireless client can connect to WG when Guard interval is 400ns and Channel BW is 20MHZ for frequency band 2.4GHZ</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband,Emulator,RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Device.WiFi.Radio.1.GuardInterval : 400nsec
Device.WiFi.Radio.1.OperatingChannelBandwidth : 20MHz</input_parameters>
    <automation_approch>1. Load tdkb_e2e module
2. Using tdkb_e2e_Get, get and save GuardInterval and channelBW
3. Set Guardinterval and channelBW to 400nsec and 20MHz using tdkb_e2e_SetMultipleParams
...
4. Revert the values of guardinterval and channelBW
    ...
    <test_script>E2E_WIFI_2.4GHZ_400nsGuardInterval_20MHzBW</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_400nsGuardInterval_20MHzBW');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            print("TEST STEP 1: Get the current ssid,keypassphrase, Guard interval and channel BW")
            print("EXPECTED RESULT 1: Should retrieve the current ssid,keypassphrase,Guard interval and channel BW")
            print("ACTUAL RESULT 1: %s" %orgValue);
            print("[TEST EXECUTION RESULT] : SUCCESS");

            # Set the Guard interval as "400ns" and ChannelBW as "20MHz"
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'400nsec','20MHz'];
            ...
            list3 = [GuardInterval,'400nsec','string']
            list4 = [ChannelBW,'20MHz','string']
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("TEST STEP 3: Get the current ssid,keypassphrase,Guard interval and channel BW")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,keypassphrase,Guard interval and channel BW")
