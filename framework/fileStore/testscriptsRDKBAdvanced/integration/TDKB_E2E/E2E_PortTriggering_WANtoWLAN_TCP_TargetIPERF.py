...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_561</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</test_objective>
...
13. From WAN client, send an inbound packet to the target port 5001 via the DUT GW WAN IP. The packet should be successfully forwarded to the LAN client.
14. Delete the added route
15. Delete the added port triggering rule
16. Revert Device.NAT.X_CISCO_COM_PortTriggers.Enable if required
17. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 5001 is forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF');
