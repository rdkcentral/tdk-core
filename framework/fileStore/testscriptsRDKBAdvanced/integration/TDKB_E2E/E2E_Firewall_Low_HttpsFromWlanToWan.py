...
<xml>
  ...
  <name>E2E_Firewall_Low_HttpsFromWlanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Low HTTPs traffic from WLAN to WAN should pass through</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_73</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low HTTPs traffic from WLAN to WAN should pass through</test_objective>
...
5. From the wlan client, do HTTPs to the WAN client and check whether HTTPs is success when the firewall level is set to Low
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_Firewall_Low_HttpsFromWlanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_HttpsFromWlanToWan');
...
#                                   status = addStaticRoute(tdkbE2EUtility.wan_ip,tdkbE2EUtility.cm_ip,tdkbE2EUtility.wlan_2ghz_interface);
                                    ...
                                    if expectedresult in status:
                                        ...
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("TEST STEP 10:Check the Https connectivity from WLAN to WAN")
