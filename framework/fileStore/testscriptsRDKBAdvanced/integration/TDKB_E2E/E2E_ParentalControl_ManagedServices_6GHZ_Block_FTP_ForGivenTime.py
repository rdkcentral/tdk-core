...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_FTP_ForGivenTime')
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_FTP_ForGivenTime')
wifi_obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_FTP_ForGivenTime')
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if int(instance) > 0:
                                ...
                                #SET the parameters to block FTP for given time
                                            ...
                                            if wlanIP:
                                                ...
                                                if expectedresult in status and curIPAddress:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        status = tdkbE2EUtility.addStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_6ghz_interface)
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS")
                                                            print(f"ACTUAL RESULT {step}: Static route added successfully")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS")

                                                            #Check FTP connectivity from WLAN to WAN
                                                            step += 1
                                                            print(f"\nTEST STEP {step}:Check the FTP connectivity from WLAN to WAN")
                                                            print(f"EXPECTED RESULT {step}: FTP connection should be blocked")
                                                            status = tdkbE2EUtility.ftpToClient("WAN", tdkbE2EUtility.wan_ftp_ip, "WLAN")

                                                            if expectedresult not in status:
                                                                tdkTestObj.setResultStatus("SUCCESS")
                                                                print("ACTUAL RESULT: FTP connection from WLAN to WAN is blocked")
                                                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE")
                                                                print("ACTUAL RESULT: FTP connection from WLAN to WAN is success")
                                                                print("[TEST EXECUTION RESULT] : FAILURE")
                                                            ...
                                                            status = tdkbE2EUtility.delStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_6ghz_interface)
                                                            if expectedresult in status:
                                                                tdkTestObj.setResultStatus("SUCCESS")
                                ...
                                if expectedresult in actualresult:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: Rule deleted. Details: {details}")
                                    print(f"TEST EXECUTION RESULT : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: Failed to delete added rule. Details: {details}")
                                    print(f"TEST EXECUTION RESULT : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
