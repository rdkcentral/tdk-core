...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_Firewall_High_PingFromWlanToWan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_SetMultipleParams</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to High ICMP message from WLAN to WAN IP of gateway should be blocked</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_66</test_case_id>
    <test_objective>Verify that when Firewall Config is set to High ICMP message from WLAN to WAN should be blocked</test_objective>
...
5. From the wlan client, do PING to the WAN client and check whether PING is blocked when the firewall level is set to High
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>PING from WLAN to WAN should fail</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_High_PingFromWlanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_High_PingFromWlanToWan');
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult not in status:
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print(f"\nTEST STEP {step}: failed, Ping from WLAN to WAN not blocked")
