...
obj.configureTestCase(ip,port,'E2E_WIFI_TCPFromWlanToLan_GetThroughput')
sysobj.configureTestCase(ip,port,'E2E_WIFI_TCPFromWlanToLan_GetThroughput')
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP != "":
                                        ...
                                        if expectedresult in status:
                                            ...
                                            step += 1
                                            print(f"\nTEST STEP {step}:Check TCP from WLAN to LAN")
                                            print(f"EXPECTED RESULT {step}: Should perform TCP from WLAN to LAN successfully")
                                            ...
                                            if expectedresult in status and serverOutput != "":
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: TCP from WLAN to LAN is successful")
                                                ...
                                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                                raw_threshold = str(tdkbE2EUtility.wlan_mlo_throughput_to_lan).strip()
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print(f"ACTUAL RESULT {step}: Failed to perform TCP from WLAN to LAN.")
                                                print("[TEST EXECUTION RESULT] : FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
