...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_DisableDMZ_FirewallLow_HTTPFromWanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling DMZ HTTP traffic should not passthrough from WAN to WLAN</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_186</test_case_id>
    <test_objective>Verify that on disabling DMZ HTTP traffic should not passthrough from WAN to WLAN</test_objective>
...
10. Check if the HTTP connection is success
...
13. Check if the http connection has failed.
...
16.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>On enabling DMZ, HTTP connection from WAN client to WAN ip of gateway should be success. On disabling DMZ, HTTP connection should fail.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_DisableDMZ_FirewallLow_HTTPFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_HTTPFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_HTTPFromWanToWlan');
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in actualresult:
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        print("TEST STEP 13:Check the HTTP connectivity from WAN client to WAN ip of GW")
                                                        status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "WGET_HTTP", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("HTTP from WAN to WAN ip of GW: SUCCESS")
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("SUCCESS: HTTP from WAN to WAN ip of GW is blocked")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("TEST STEP 13:HTTP from WLAN to WAN ip of GW failed")
                                                                    print("FAILURE: HTTP from WAN to WAN ip of GW is not blocked")
