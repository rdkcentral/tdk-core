...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_DisableDMZ_FirewallLow_HTTPSFromWanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling DMZ HTTPs traffic should not passthrough from WAN to WLAN</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_187</test_case_id>
    <test_objective>Verify that on disabling DMZ HTTPs traffic should not passthrough from WAN to WLAN</test_objective>
...
11. Disable DMZ usnig Device.NAT.X_CISCO_COM_DMZ.Enable
12. From wan client again try to establish ftp connection with the WAN ip of GW(it should fail)
13. Check if the https connection has failed.
14. Disconnect wifi client and disable DMZ
15. Revert the firewall level to original value
16.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>On enabling DMZ, HTTPS connection from WAN client to WAN ip of gateway should be success. On disabling DMZ, HTTPS connection should fail.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_DisableDMZ_FirewallLow_HTTPSFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_HTTPSFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_HTTPSFromWanToWlan');
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in actualresult:
                                        ...
                                        tdkTestObj1.addParameter("paramName","Device.NAT.X_CISCO_COM_DMZ.Enable");
                                        tdkTestObj.executeTestCase(expectedresult);
                                        actualresult = tdkTestObj.getResult();
                                        details = tdkTestObj.getResultDetails();
                                        if expectedresult in actualresult:
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in actualresult:
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        print("TEST STEP 13:Check the HTTPS from WAN client to WAN ip of GW")
                                                        status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "WGET_HTTPS", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("HTTPS from WAN to WAN ip of GW: SUCCESS")
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                details = tdkTestObj.getResultDetails();
                                                                print("[TEST STEP 11]: Disabling DMZ");
                                                                print("[EXPECTED RESULT 11]: Should disable DMZ");
                                                                print("[ACTUAL RESULT 11]: %s" %details);
                                                                print("[TEST EXECUTION RESULT] : %s" %actualresult);
                                                                print("DMZ is disabled\n")

                                                                print("TEST STEP 13:Check the HTTPS connectivity from WAN client to WAN ip of GW")
                                                                status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "WGET_HTTPS", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("SUCCESS: HTTPS from WAN to WAN ip of GW is blocked")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("TEST STEP 13:HTTPS from WLAN to WAN ip of GW failed")
                                                                    print("FAILURE: HTTPS from WAN to WAN ip of GW is not blocked")
                                                            else:
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                details = tdkTestObj.getResultDetails();
                                                                print("[TEST STEP 11]: Disabling DMZ");
