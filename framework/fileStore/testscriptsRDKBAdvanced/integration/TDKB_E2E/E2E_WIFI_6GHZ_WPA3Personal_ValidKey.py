...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_ValidKey</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that a Wi-Fi client can connect to the 6 GHz SSID when a valid WPA3-Personal key  is used.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_661</test_case_id>
    <test_objective>Verify that a Wi-Fi client can connect to the 6 GHz SSID when a valid WPA3-Personal key is used.</test_objective>
...
4. Try to connect to Wifi client and check whether the wifi client is  connected to the DUT using WPA3-Personal and valid wifi password
5. Revert the values if required
6. Unload the module.</automation_approch>
    <expected_output>Wi-Fi client should connect to the DUT with WPA3-Personal security mode when a valid password is used.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_ValidKey</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_ValidKey')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_ValidKey')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                step += 1
                # Set the Radio status and Keypassphrase
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        print(f"EXPECTED RESULT {step}: Connection should be successful with valid password")
                        ...
                        if expectedresult in status:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print(f"ACTUAL RESULT {step}: Wlan client connection to wifi successfull")
                            print(f"[TEST EXECUTION RESULT] : SUCCESS")
                            step += 1
                            ...
                            if wlanIP:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: WLAN Client IP Address is {wlanIP}")
                                print(f"[TEST EXECUTION RESULT] : SUCCESS")
                                step += 1

                                # Get the current LAN IP address DHCP range
                                print(f"TEST STEP {step}: Get the current LAN IP address DHCP range")
                                print(f"EXPECTED RESULT {step}: Should retrieve the LAN IP address successfully")
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print(f"ACTUAL RESULT {step}: Failed to disconnect WLAN client")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WLAN client did not get IP")
                                print(f"[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print(f"ACTUAL RESULT {step}: Failed to connect with valid password")
                            print(f"[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: {newValues}")
