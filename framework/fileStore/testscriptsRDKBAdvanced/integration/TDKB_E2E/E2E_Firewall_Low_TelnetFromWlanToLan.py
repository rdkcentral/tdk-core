...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_Firewall_Low_TelnetFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to Low Telnet access from LAN to LAN should be allowed</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_59</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low Telnet access from LAN to LAN should be allowed</test_objective>
...
5. From the wlan client, do Telnet to the LAN client and check whether Telnet is success when the firewall level is set to Low
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>From the wlan client, do Telnet to the LAN client and check whether Telnet is success when the firewall level is set to Low</except_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_TelnetFromWlanToLan');
                            ...
                            if expectedresult in status:
                                ...
                                if wlanIP:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        step = step + 1
                                        print(f"\nTEST STEP {step}: Check the Telnet from WLAN to LAN")
                                        ...
                                        if expectedresult in status:
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print(f"\nTEST STEP {step}: Telnet from WLAN to LAN failed")
