...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilterBlock5GHZ')
obj1.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilterBlock5GHZ')
obj2.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilterBlock5GHZ')
    ...
    if expectedresult in status:
        ...
        if "SUCCESS" in preRequisiteStatus:
            ...
            if macAddress and (re.match("^[0-9a-f]{2}([-:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", macAddress.lower()) is not None):
                ...
                step+=1
                print(f"\nTEST STEP {step}: Set Mac FilterAsBlacklist of 5Ghz to true.")
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult  and mode == "Deny" :
                            ...
                            if expectedresult in actualresult:
                                ...
                                if expectedresult in actualresult and bssid and (re.match("^[0-9a-f]{2}([-:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", bssid.lower()) is not None):
                                    ...
                                    if expectedresult in status and  orgValue != "":
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            if expectedresult in status and setValuesList == newValues:
                                                ...
                                                if expectedresult not in status:
                                                    ...
                                                    print(f"ACTUAL RESULT {step}: Connection from client is success via 5Ghz which is not expected.")
                                        ...
                                        print(f"\nTEST STEP {step}: Set the original ssid andkeypassphrase")
                        ...
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: Mac Filtering mode for 5Ghz is '{mode}' which is not expected")
                        #Get the result of execution
                        print("[TEST EXECUTION RESULT] : FAILURE")

                    #Reverting FilterAsBlacklist of 5GHz to false
                    step+=1
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print(f"ACTUAL RESULT {step}: Revert Mac FilterAsBlacklist of 5Ghz to false successfully.")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
