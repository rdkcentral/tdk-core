...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_XDNS_SetXDNSDefaultValues_AccessLevel3Site</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Enable xDNS and try to access the level3 wesbites with DNSIPV4 as 75.75.75.75 and DNS IPv6 as 2001:558:feed::1</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_420</test_case_id>
    <test_objective>Enable xDNS and try to access the level3 wesbites with DNSIPV4 as 75.75.75.75 and DNS IPv6 as 2001:558:feed::1</test_objective>
...
5. Add new rule in this table with LAN client MAC, default DNS ipv4 and ipv6 ips and a tag name for rule
6. If the default DNS ips are empty, set dns ip values
7. Enable Device.DeviceInfo.X_RDKCENTRAL-COM_EnableXDNS
8. From LAN client do a nslookup to any level3 site via default dns server
...
12. Unload tdkb_e2e module</automation_approch>
    <except_output>nslookup should resolve the level3 site successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_XDNS_SetXDNSDefaultValues_AccessLevel3Site</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_XDNS_SetXDNSDefaultValues_AccessLevel3Site');
obj1.configureTestCase(ip,port,'E2E_XDNS_SetXDNSDefaultValues_AccessLevel3Site');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if (instance > 0):
                                ...
                                setValuesList = [lanMAC,tdkbE2EUtility.xdns_dns_server,tdkbE2EUtility.xdns_ipv6_dns_server,"Default"];
                                print("Parameter values that are set: %s" %setValuesList)

                                list1 = [macAddress,lanMAC,'string']
                                list2 = [dnsipv4,tdkbE2EUtility.xdns_dns_server,'string']
                                list3 = [dnsipv6,tdkbE2EUtility.xdns_ipv6_dns_server,'string']
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    if expectedresult in  actualresult1:
                                        ...
                                        if expectedresult in status and setValuesList == newValues:
                                            ...
                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("TEST STEP 9:Check if the level3 site accessible with default values")
                                                print("EXPECTED RESULT 9:The level3 site should be accessible with default values")
                                                print("[TEST EXECUTION RESULT] : SUCCESS");
                                                finalStatus = "SUCCESS"
                                            else:
