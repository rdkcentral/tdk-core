...
<xml>
  ...
  <name>E2E_Firewall_Custom_PingFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Custom Ping access from LAN to WAN should not be blocked</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_299</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Custom Ping access from LAN to WAN should not be blocked</test_objective>
...
5. From the lan client, do Ping to the WAN client and check whether ping is success when the firewall level is set as custom
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Ping from LAN to WAN should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_Custom_PingFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Custom_PingFromLanToWan');
