...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_ConnectWiFi_GetAssociatedDeviceNumberOfEntries</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on connecting a Wireless Client through 5 GHz radio value of parameter (Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries) should be incremented accordingly</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_168</test_case_id>
    <test_objective>Verify that on connecting a Wireless Client through 5 GHz radio value of parameter (Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries) should be incremented accordingly</test_objective>
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries should be incremented by one after successfully connected to wifi.</except_output>
...
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_ConnectWiFi_GetAssociatedDeviceNumberOfEntries');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in status:
                            ...
                            if wlanIP:
                                ...
                                print("Current Associated number of entries: %s" %associatedNumberOfEntriesAfterWiFiConnect);
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 9: Revert the values of SSID, KeyPassphrase, Radio status, encryption method, security mode to original")
                print("EXPECTED RESULT 9: Should set the original SSID, KeyPassphrase, Radio status, encryption method, security mode");
