...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_Firewall_High_TelnetFromLanToWan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to High Telnet access from LAN to WAN should be blocked</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_287</test_case_id>
    <test_objective>Verify that when Firewall Config is set to High Telnet access from LAN to WAN should be blocked</test_objective>
...
5. From the lan client, do TELNET to the WAN client and check whether telnet is success when the firewall level is set to High
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Telnet from LAN to WAN should be blocked</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_High_TelnetFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_High_TelnetFromLanToWan');
    ...
    if expectedresult in status:
        ...
        if expectedresult in retStatus:
                    ...
                    if lanIP:
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    print("TEST STEP 6:Static route add success")

                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("TEST STEP 7:Check the Telnet from LAN to WAN")
                                    status = telnetToClient("WAN",tdkbE2EUtility.wan_ip);
                                    time.sleep(20);
                                    if expectedresult not in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: Telnet from LAN to WAN is blocked")
                                        finalStatus = "SUCCESS";
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 7: failed, Telnet from LAN to WAN is success")

                                    #delete the added route
                                    print("TEST STEP 8: Delete the static route")
                                    status = delStaticRoute(tdkbE2EUtility.wan_ip, curIPAddress, tdkbE2EUtility.lan_interface, "LAN");
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("TEST STEP 9:Static route delete success")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 9:Static route delete failed")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("TEST STEP 6:Static route add failed")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("TEST STEP 5:WLAN Client IP address is not in the same Gateway DHCP range")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP :Failed to get the WLAN Client IP address")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
            ...
            if  expectedresult in firewallResult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 10: Should set the original firewall level");
