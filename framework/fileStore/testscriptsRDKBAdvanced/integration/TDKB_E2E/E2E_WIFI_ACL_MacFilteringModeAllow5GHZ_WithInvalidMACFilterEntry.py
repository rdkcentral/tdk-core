...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeAllow5GHZ_WithInvalidMACFilterEntry')
obj1.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeAllow5GHZ_WithInvalidMACFilterEntry')
obj2.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeAllow5GHZ_WithInvalidMACFilterEntry')
            ...
            if macAddress and (re.match("^[0-9a-f]{2}([-:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", macAddress.lower()) is not None):
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print(f"ACTUAL RESULT {step}: Set invalid MAC in the filter table entry successfully.")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: Failed to set invalid MAC in the filter table entry for 5Ghz.")
                        print("[TEST EXECUTION RESULT] : FAILURE")
