...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_Firewall_Custom_SSHToWAN_FromLANAndWLAN</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to Custom SSH access from LAN to WAN and WLAN to WAN should  be success</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Verify that when Firewall Config is set to Custom SSH access from LAN to WAN and WLAN to WAN should  be success</test_objective>
...
3. Set the firewall to Custom using tdkb_e2e_SetMultipleParams
...
8.Unload tdkb_e2e module</automation_approch>
    <except_output>The SSH to WAN from WLAN and LAN should be success when firewall is Custom</except_output>
    <priority>High</priority>
    <test_stub_interface>TDKB_E2E</test_stub_interface>
    <test_script>E2E_Firewall_Custom_SSHToWAN_FromLANAndWLAN</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Custom_SSHToWAN_FromLANAndWLAN');
        ...
        if expectedresult in status and expectedresult in retStatus:
            ...
            if tdkbE2EUtility.mlo_capability == "False":
                ...
                if expectedresult in actualresult and expectedresult in firewallResult:
                    ...
                    if expectedresult in status and expectedresult in retStatus and setValuesList == newValues and newFirewallValue == "Custom":
                        tdkTestObj.setResultStatus("SUCCESS");
                        print(f"\nTEST STEP {step}: Get the current ssid,keypassphrase,Radio enable status,firewall level")
                        print(f"EXPECTED RESULT {step}: Should retrieve the current ssid,keypassphrase,Radio enable status,firewall level")
                ...
                #Set the firewall level to Custom using firewallSet function
                level = "Custom"
                step = step + 1
