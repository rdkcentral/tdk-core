...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_VerifyBSSID</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that MAC address of 5 GHz radio band obtained through parameter (Device.WiFi.SSID.2.BSSID) matches with the address obtained from Wireless Client</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_171</test_case_id>
    <test_objective>Verify that MAC address of 5 GHz radio band obtained through parameter (Device.WiFi.SSID.2.BSSID) matches with the address obtained from Wireless Client</test_objective>
...
Device.WiFi.SSID.2.BSSID</input_parameters>
...
4. Get the current value of Device.WiFi.SSID.2.BSSID
5. Get the Access point MAC address of the WIFI client after connecting to the SSID and compare the Access point MAC address of WLAN client and the value returned by Device.WiFi.SSID.2.BSSID are same
6. Revert the values of encryption method and security mode
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Verify whether the Access point MAC address of WLAN client and the value returned by Device.WiFi.SSID.2.BSSID are same</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_VerifyBSSID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_VerifyBSSID');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    getMAC = "Device.WiFi.SSID.%s.BSSID" %tdkbE2EUtility.ssid_5ghz_index
                                    ...
                                    if expectedresult in status and MAC1:
                                        ...
                                        MAC2 = getWlanAccessPoint(tdkbE2EUtility.wlan_5ghz_interface);
