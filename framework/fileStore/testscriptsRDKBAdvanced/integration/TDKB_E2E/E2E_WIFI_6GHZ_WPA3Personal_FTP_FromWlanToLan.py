...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_FTP_FromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that a large file can be successfully transferred over FTP between a  WLAN client and a LAN client when the WLAN client is associated with wpa3-psk authentication for 6GHz band.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_669</test_case_id>
    <test_objective>Verify that a large file can be successfully transferred over FTP between a  WLAN client and a LAN client when the WLAN client is associated with wpa3-psk authentication for 6GHz band.</test_objective>
...
6. From the wlan client, try to connect to the LAN client via ftp and check whether ftp connection and file transfer is success when the security mode for 6GHz is set to WPA-Personal
7. Unload tdkb_e2e module</automation_approch>
    <expected_output>ftp connection from WLAN client to LAN client and file transfer should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_FTP_FromWlanToLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_FTP_FromWlanToLan')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_FTP_FromWlanToLan')
            ...
            if exit_flag != 1:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        # Step 4: Connect to the wifi ssid from wlan client
                        step += 1
                        print(f"TEST STEP {step}: From wlan client, Connect to the wifi ssid")
                        print(f"EXPECTED RESULT {step}: WLAN client should successfully connect to the SSID")
                        ...
                        if expectedresult in status:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print(f"ACTUAL RESULT {step}: WLAN client connected to SSID successfully")
                            print("[TEST EXECUTION RESULT] : SUCCESS")

                            # Step 5: Get WLAN IP
                            step += 1
                            print(f"TEST STEP {step}: Get the IP address of the wlan client after connecting to wifi")
                            print(f"EXPECTED RESULT {step}: Should retrieve valid wlan IP address")
                            ...
                            if wlanIP:
                                ...
                                # Step 6: Get current LAN IP address DHCP range
                                step += 1
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    # Step 7: Check WLAN IP in same DHCP range
                                    step += 1
                                    ...
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}: WLAN IP is in the same DHCP range")
                                        print("[TEST EXECUTION RESULT] : SUCCESS")

                                        # Step 8: Get LAN IP after connecting
                                        step += 1
                                        ...
                                        if lanIP:
                                            ...
                                            # Step 9: Check LAN IP in same DHCP range
                                            step += 1
                                            ...
                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: LAN client IP is in the same DHCP range")
                                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                                # Step 10: FTP File Download WLAN->LAN
                                                step += 1
                                                print(f"TEST STEP {step}: Check FTP and File download request from WLAN to LAN")
                                                print(f"EXPECTED RESULT {step}: FTP file download should succeed from WLAN to LAN")
                                                status = tdkbE2EUtility.ftpToClient_File_Download("LAN", lanIP, "WLAN", wlanIP)

                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    print(f"ACTUAL RESULT {step}: ftpToClient_File_Download SUCCESS")
                                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                                    finalStatus = "SUCCESS"
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print(f"ACTUAL RESULT {step}: LAN client IP not in same DHCP range")
                                                print("[TEST EXECUTION RESULT] : FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: Failed to get LAN gateway IP")
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                        ...
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: Retrieved values {newValues} do not match set values")
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print(f"ACTUAL RESULT {step}: {details}")
