...
<xml>
  ...
  <name>E2E_XfinityWiFi_Connect5GHZ</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Enable Xfinity WiFi and connect a WiFi client to XfinityWiFi 5GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_462</test_case_id>
    <test_objective>Enable Xfinity WiFi and connect a WiFi client to XfinityWiFi 5GHZ</test_objective>
...
5. Set Device.WiFi.X_RDKCENTRAL-COM_PreferPrivate as false
6. From WiFi client machine try to connect to xfinitywiFi5GHz
7. Check if the connection attempt was success and client got ip in the expected ip range
8.Disconnect the connection
9. Revert the value of Device.WiFi.X_RDKCENTRAL-COM_PreferPrivate
10. Revert back the values of XfinityWiFi parameters
11. Unload wifiagent and tdkb_e2e module</automation_approch>
    <except_output>WiFi client should successfully connect to XfinityWiFi5GHZ</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e,wifiagent</test_stub_interface>
    <test_script>E2E_XfinityWiFi_Connect5GHZ</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_XfinityWiFi_Connect5GHZ');
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in retStatus:
                    ...
                    prefParam = "%s|false|boolean" %preference
                    ...
                    if expectedresult in prefResult:
                        ...
                        if expectedresult in retStatus and newPrefValue == "false":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            status = wlanConnectWifiSsid(tdkbE2EUtility.ssid_5ghz_public_name,"",tdkbE2EUtility.wlan_5ghz_public_ssid_interface,"Open");
                            if expectedresult in status:
                                tdkTestObj.setResultStatus("SUCCESS");

                                print("TEST STEP 7: Get the IP address of the wlan client after connecting to wifi")
                                wlanIP = getWlanIPAddress(tdkbE2EUtility.wlan_5ghz_public_ssid_interface);
                                if wlanIP:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("WLAN client successfully connected to xfinityWiFi")

                                    status = wlanDisconnectWifiSsid(tdkbE2EUtility.wlan_5ghz_public_ssid_interface);
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        finalStatus = "SUCCESS";
                                        print("TEST STEP 8:Disconnect from WIFI SSID: SUCCESS")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 8:Disconnect from WIFI SSID: FAILED")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("Failed to get the wlan IP address for xfinity5GHZ")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("Failed to connect to the xfinity5GHZ SSID")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current privatePreference")
                            print("EXPECTED RESULT 5: Should retrieve the current privatePreference")
                        ...
                        if expectedresult in prefResult and expectedresult in finalStatus:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print("EXPECTED RESULT 9: Should set the original privatePreference");
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print("TEST STEP 4: Set the privatePreference as false")
                        print("EXPECTED RESULT 4: Should set the privatePreference as false")
                        print("ACTUAL RESULT 4: %s" %details);
                        print("[TEST EXECUTION RESULT] : FAILURE");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                ...
                if expectedresult in actualresult:
                    ...
                    print("TEST STEP 10: Should revert the PublicWiFi values")
