                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    step = step + 1
                                    print(f"\nTEST STEP {step}: Check the Telnet from WLAN to WAN")
                                    status = telnetToClient("WAN",tdkbE2EUtility.wan_ip,"WLAN");

                                    if expectedresult not in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: Telnet from WLAN to WAN is blocked")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print(f"\nTEST STEP {step}: failed, Telnet from WLAN to WAN not blocked")
