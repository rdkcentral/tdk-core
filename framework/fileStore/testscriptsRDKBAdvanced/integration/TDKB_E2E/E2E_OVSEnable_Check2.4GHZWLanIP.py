...
<xml>
  ...
  <name>E2E_OVSEnable_Check2.4GHZWLanIP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the 2.4GHZ WLan client gets proper IP when OVS is enabled in gateway</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_550</test_case_id>
    <test_objective>Check if the 2.4GHZ WLan client gets proper IP when OVS is enabled in gateway</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>A WLAN client should be available</pre_requisite>
...
4. Connect wlan client to DUT's 2.4G SSID
5. Check if the connection is success and ip received by wlan client is in proper range when OVS is enabled
6. Revert OVS enabled state to its initial value
7. Unload tdkb_e2e module
</automation_approch>
    <expected_output>2.4GHZ WLan client gets proper IP when OVS is enabled in gateway</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_OVSEnable_Check2.4GHZWLanIP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_OVSEnable_Check2.4GHZWLanIP');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in retStatus:
            ...
            if expectedresult in status:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 4: Get the current SSID, KeyPassphrase")
                print("EXPECTED RESULT 4: Should retrieve the current SSID, KeyPassphrase")
                ...
                if expectedresult in actualresult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("TEST STEP 5: Set the SSID, KeyPassphrase")
                    print("EXPECTED RESULT 5: Should set the SSID, KeyPassphrase");
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 6: Get the current SSID, KeyPassphrase")
                        print("EXPECTED RESULT 6: Should retrieve the current SSID, KeyPassphrase")
                        print("ACTUAL RESULT 6: %s" %newValues);
                        print("[TEST EXECUTION RESULT] : SUCCESS")

                        #Connect to the wifi ssid from wlan client using valid wifi password
                        print("TEST STEP 7: From wlan client, Connect to the wifi ssid using valid wifi password")
                        print("EXPECTED RESULT 7: Wlan client should get connected")
                        ...
                        if expectedresult in status:
                            ...
                            print("EXPECTED RESULT 8: Should retrieve the IP address of the wlan client ");
                            wlanIP = getWlanIPAddress(tdkbE2EUtility.wlan_2ghz_interface);
                            if wlanIP:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print("ACTUAL RESULT 8: WLAN IP retrieved is %s " %wlanIP) ;
                                print("[TEST EXECUTION RESULT] : SUCCESS");

                                print("TEST STEP 9: Get the current Gateway IP address")
                                print("EXPECTED RESULT 9: Should retrieve the current Gateway IP address")
                                param = "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress"
                                tdkTestObj,status,curIPAddress = getParameterValue(obj,param)

                                if expectedresult in status and curIPAddress:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("ACTUAL RESULT 9:GW IP Address: %s" %curIPAddress);
                                    print("[TEST EXECUTION RESULT] : SUCCESS");

                                    print("TEST STEP 10: Check whether wlan ip address is in same DHCP range")
                                    print("EXPECTED RESULT 10: WLAN ip should be in valid DHCP range")
                                    status = "SUCCESS"
                                    status = checkIpRange(curIPAddress, wlanIP);
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("ACTUAL RESULT 10: WLAN ip address is in valid DHCP range")
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                        ...
                                        if expectedresult in status:
                                            print("TEST STEP 11: Connection successfully disconnected")
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                        else:
                                            print("TEST STEP 11: Failed to Disconnect")
                                            print("[TEST EXECUTION RESULT] : FAILURE");
                                            tdkTestObj.setResultStatus("FAILURE");
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("ACTUAL RESULT 10: LAN Client IP address is not in valid DHCP range")
                                        print("[TEST EXECUTION RESULT] : FAILURE");
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("ACTUAL RESULT 9 :Failed to get the Gateway IP address")
                                    print("[TEST EXECUTION RESULT] : FAILURE");
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("ACTUAL RESULT 8: Failed to get the WLAN Client IP address")
                                print("[TEST EXECUTION RESULT] : FAILURE");
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 7: Failed to connect from wlan client, to the wifi ssid using valid wifi password")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("TEST STEP 6: Get the current SSID, KeyPassphrase")
                        print("EXPECTED RESULT 6: Should retrieve the current SSID, KeyPassphrase")
                ...
                if expectedresult in actualresult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("EXPECTED RESULT 12: Should set the original SSID, KeyPassphrase");
            ...
            if OVSValue == "false":
                ...
                if  expectedresult in OVSResult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("EXPECTED RESULT 13: Should set the original OVS Enabled status");
