...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithPTRuleDisabled</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_580</test_case_id>
...
11. From WAN client, send an inbound packet to the target port 21 via the DUT GW WAN IP. The packet should NOT be successfully forwarded to the LAN client since PT rule is disabled.
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithPTRuleDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithPTRuleDisabled');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithPTRuleDisabled');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                #Trigger the port
                                step = step + 1
                                print("\nTEST STEP %d: Trigger the Port %s with protocol %s through an outbound packet from LAN client" %(step, triggerStart, triggerProtocol))
