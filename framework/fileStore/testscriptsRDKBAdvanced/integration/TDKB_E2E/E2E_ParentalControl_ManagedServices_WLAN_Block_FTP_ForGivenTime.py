...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_WLAN_Block_FTP_ForGivenTime')
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_WLAN_Block_FTP_ForGivenTime')
                                    ...
                                    if wlanIP:
                                        ...
                                        if expectedresult in status and curIPAddress:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                step = step + 1
                                                print(f"\nTEST STEP {step}: Add static route to reach FTP WAN client from WLAN client")
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print(f"EXPECTED RESULT {step}: Blocked service FTP should not be reachable during configured time")
                                                    status = ftpToClient("WAN", tdkbE2EUtility.wan_ftp_ip, "WLAN")

                                                    if expectedresult not in status:
                                                        tdkTestObj.setResultStatus("SUCCESS")
                                                        print(f"ACTUAL RESULT {step}: The blocked service 'FTP' cannot be reached")
                                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                                        finalStatus = "SUCCESS"
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE")
                                                        print(f"ACTUAL RESULT {step}: The blocked service 'FTP' can be reached")
                                                        print("[TEST EXECUTION RESULT] : FAILURE")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE")
