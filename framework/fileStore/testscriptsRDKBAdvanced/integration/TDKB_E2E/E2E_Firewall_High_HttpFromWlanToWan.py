...
<xml>
  ...
  <name>E2E_Firewall_High_HttpFromWlanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to High HTTP traffic from WLAN to WAN should pass through</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_67</test_case_id>
    <test_objective>Verify that when Firewall Config is set to High HTTP traffic from WLAN to WAN should pass through</test_objective>
...
5. From the wlan client, do HTTP to the WAN client and check whether HTTP is success when the firewall level is set to High
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>HTTP from WLAN to WAN should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_High_HttpFromWlanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_High_HttpFromWlanToWan');
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: Http connection from WLAN to WAN is success")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print(f"\nTEST STEP {step}: failed, HTTP from WLAN to WAN is blocked")
