...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_Firewall_Low_PingFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to Low ICMP message from LAN to LAN should be allowed</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_56</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low ICMP message from LAN to LAN should be allowed</test_objective>
...
5. From the wlan client, do PING to the LAN client and check whether PING is success when the firewall level is set to Low
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>From the wlan client, do PING to the LAN client and check whether PING is success when the firewall level is set to Low</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_Low_PingFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_PingFromWlanToLan');
