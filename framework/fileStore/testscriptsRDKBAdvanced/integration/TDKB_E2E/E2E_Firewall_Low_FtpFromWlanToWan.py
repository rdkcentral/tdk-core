...
<xml>
  ...
  <name>E2E_Firewall_Low_FtpFromWlanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Low FTP access from WLAN to WAN should pass through</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_75</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low FTP access from WLAN to WAN should passthrough</test_objective>
...
5. From the wlan client, do FTP to the WAN client and check whether FTP is success when the firewall level is set to Low
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>FTP from WLAN to WAN should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_Low_FtpFromWlanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_FtpFromWlanToWan');
                    ...
                    if wlanIP:
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                step = step + 1
                                status = addStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_interface);
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: FTP connection from WLAN to WAN is success")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print(f"\nTEST STEP {step}: failed, FTP from WLAN to WAN is blocked")
                                        #delete the added route
                                    step = step + 1
                                    print(f"\nTEST STEP {step}: Delete the static route")
                                    status = delStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_interface);
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
