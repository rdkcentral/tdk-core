...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>E2E_PortTriggering_WANtoWLAN_UDP_TargetNETCAT_TriggerOutOfRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 54321 is not forwarded to the WLAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the out of range 12350 port which is not configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_625</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 54321 is not forwarded to the WLAN machine (which is running an NETCAT server) after it sends an outbound packet through the GW that triggers the out of range 12350 port which is not configured in the rule.</test_objective>
...
Device.NAT.X_CISCO_COM_PortTriggers.Trigger.{i}.TriggerPortStart - 12340
Device.NAT.X_CISCO_COM_PortTriggers.Trigger.{i}.TriggerPortEnd - 12346
...
5. Check if a wlan client is connected to the GW by fetching the IP corresponding to the configuration interface.
6. Check if the wlan IP is in the expected DHCP range.
7. Add a new instance to the port triggering rule table
8. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
12. Send an outbound packet to trigger the out of range port 12350 with a UDP packet
13. From WAN client, send an inbound packet to the target port 54321 via the DUT GW WAN IP. The packet should not be forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 54321 is forwarded to the WLAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_UDP_TargetNETCAT_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetNETCAT_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetNETCAT_TriggerOutOfRange');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12340"
                                        triggerEnd = "12346"
                                        targetStart = "54321"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                status = PTinitServer("NETCAT", wlanIP, targetStart, targetProtocol, tdkbE2EUtility.server_logfile, "WLAN", message)
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    #Trigger the port which is out of range
                                                    step = step + 1
                                                    trigger_OutOfRange = "12350"
                                                    print("\nTEST STEP %d: Trigger the Out of Range Port %s through a specified outbound packet from WLAN client" %(step, trigger_OutOfRange))
                                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                                    status = triggerPort(tdkbE2EUtility.wan_ip, trigger_OutOfRange, triggerProtocol, "WLAN")
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("EXPECTED RESULT %d: NETCAT client running in WAN machine should not connect to the server running in WLAN with the GW WAN IP of DUT" %step)
                                                        status = PTClientRequest("NETCAT", tdkbE2EUtility.wan_ip, targetStart, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");
                                                        if status == "FAILURE":
                                                            tdkTestObj.setResultStatus("SUCCESS");
