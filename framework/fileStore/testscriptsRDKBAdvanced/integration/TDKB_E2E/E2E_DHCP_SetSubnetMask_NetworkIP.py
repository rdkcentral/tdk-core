...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_SetSubnetMask_NetworkIP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that subnet mask of DHCPv4 server pool cannot be set with Network IP address. (ex.10.0.0.0). Ensure that DHCP configuration on client is not affected</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_267</test_case_id>
    <test_objective>Verify that subnet mask of DHCPv4 server pool cannot be set with Network IP address. (ex.10.0.0.0). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set SubnetMask to network IP using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting subnetmask as network IP</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetSubnetMask_NetworkIP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetSubnetMask_NetworkIP');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the LanIPAddress to some network IP address"
                    subnetMask = "%s|10.0.0.0|string" %lanSubnetMask

                    tdkTestObj,status,details = setMultipleParameterValues(obj,subnetMask)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the SubnetMask to network IP, 10.0.0.0")
                        ...
                        if expectedresult in retStatus and newSubnetMask != "10.0.0.0":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            if subnetMask and "10.0.0.0" not in subnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print(" subnet mask has not changed as network ip value")
                                finalStatus = "SUCCESS";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("subnet mask has changed as network ip value")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current subnetmask")
                            print("EXPECTED RESULT 5: Should retrieve the current subnetmask")
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print("TEST STEP 4: Set the subnetMask to a network IP, 10.0.0.0")
                        print("EXPECTED RESULT 4: Should not set the subnetMask");
                        print("ACTUAL RESULT 4: %s" %details);
                        print("[TEST EXECUTION RESULT] : FAILURE");
                else:
