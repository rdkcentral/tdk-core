...
<xml>
  ...
  <name>E2E_Firewall_Medium_HttpFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Medium HTTP access from LAN to WAN should not be blocked</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
5. From the lan client, do HTTP to the WAN client and check whether http is success when the firewall level is set to Medium
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_Firewall_Medium_HttpFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Medium_HttpFromLanToWan');
