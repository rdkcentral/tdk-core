...
<xml>
  ...
  <name>E2E_ParentalControl_ManagedServices_5GHZ_Block_FTP_ForGivenTime</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that WG supports blocking of FTP service for Wi-Fi Client associated using 5GHZ radio only during specific day and time period through parental control (managed services)</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_129</test_case_id>
    <test_objective>Verify that WG supports blocking of FTP service for Wi-Fi Client associated using 5GHZ radio only during specific day and time period through parental control (managed services)</test_objective>
...
7.Check whether the WLAN client is able to establish ftp connection
8.Delete the added rule and revert all params values
9.Unload tdkb_e2e module</automation_approch>
    <expected_output>WLAN client should not be able to establish FTP connection with in the specified time</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedServices_5GHZ_Block_FTP_ForGivenTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_Block_FTP_ForGivenTime');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_Block_FTP_ForGivenTime');
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in actualresult:
                        ...
                        if (int(instance) > 0):
                            ...
                            setValuesList = ['ftp','BOTH','21','21','false',start_Time,end_Time,day];
                            print("Parameter values that are set: %s" %setValuesList)
                                        ...
                                        if wlanIP:
                                            ...
                                            if expectedresult in status and curIPAddress:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    status = addStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
                                                    if expectedresult in status:
                                                        print("TEST STEP 11:Static route add success")
                                                        tdkTestObj.setResultStatus("SUCCESS");

                                                        print("TEST STEP 12:Check the FTP connectivity from WLAN to WAN")

                                                        status = ftpToClient("WAN", tdkbE2EUtility.wan_ftp_ip, "WLAN");

                                                        if expectedresult not in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("The blocked service 'FTP' cannot be reached")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("The blocked service 'FTP' can be reached")
                                                            print("[TEST EXECUTION RESULT] : FAILURE");

                                                        #delete the added route
                                                        print("TEST STEP 13: Delete the static route")
                                                        status = delStaticRoute(tdkbE2EUtility.wan_ftp_ip, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
