...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_2.4GHZ_WPAWPA2PSK_Ping_LanToWlan_RadioDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ICMP traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_211</test_case_id>
    <test_objective>Verify that ICMP traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</test_objective>
...
4. Disable radio and send ping request to wlan client from lan client
4. Revert the values</automation_approch>
    <except_output>Ping request should fail since radio is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_WPAWPA2PSK_Ping_LanToWlan_RadioDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPAWPA2PSK_Ping_LanToWlan_RadioDisabled');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in status and setValuesList == newValues:
                                                    ...
                                                    #Send ICMP request to WLAN client from LAN clinet
