...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_DisablePF_FirewallLow_FTPFromWanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling Port Forwarding FTP should not be allowed from WAN to WLAN</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_185</test_case_id>
    <test_objective>Verify that on disabling Port Forwarding FTP should not be allowed from WAN to WLAN</test_objective>
...
10. Check if the ftp connection is success
11. Disable Device.NAT.X_Comcast_com_EnablePortMapping using AdvancedConfig_Set
12. From wan client try to establish ftp connection with the WAN ip of GW(it should fail)
...
13.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>HTTP connection from WAN client to WAN ip of gateway, after enabling port forwarding should be success. On disabling port forwarding, this connection should fail</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_DisablePF_FirewallLow_FTPFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DisablePF_FirewallLow_FTPFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_DisablePF_FirewallLow_FTPFromWanToWlan');
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            #Set the result status of execution
                                                ...
                                                if (instance1 > 0):
                                                    ...
                                                    tdkTestObj.addParameter("paramList","Device.NAT.PortMapping.%s.Enable|true|bool|Device.NAT.PortMapping.%s.ExternalPort|21|unsignedint|Device.NAT.PortMapping.%s.Protocol|BOTH|string|Device.NAT.PortMapping.%s.InternalClient|%s|string|Device.NAT.PortMapping.%s.Description|NEW_FTP_RULE|string|Device.NAT.PortMapping.%s.ExternalPortEndRange|21|unsignedint" %(instance1, instance1, instance1, instance1, wlanIP, instance1, instance1));
                                                    expectedresult="SUCCESS";
                                                    tdkTestObj.executeTestCase(expectedresult);
                                                    actualresult = tdkTestObj.getResult();
                                                    if expectedresult in actualresult:
                                                        ...
                                                        time.sleep(60);
                                                        print("TEST STEP :Check the FTP connectivity from WAN client to WAN ip of GW ")
                                                        status = ftpToClient("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN");
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                            print("FTP from WAN to GW: SUCCESS")
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                time.sleep(60);
                                                                print("TEST STEP 12:Check the FTP from WAN to WLAN after disabling port mapping")
                                                                status = ftpToClient("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN");
                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("SUCCESS, FTP from WAN to WLAN failed")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("TEST STEP 12 FAILURE: FTP from WAN to WLAN is success")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE");
                                                                details = tdkTestObj.getResultDetails();
                                                                print("[TEST STEP 11]: Disabling Port Mapping");
                                                            ...
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("TEST STEP :FTP from WAN to GW failed")
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                details = tdkTestObj.getResultDetails();
                                                print("[TEST STEP 8]: Reverting Port Mapping");
                                                print("[EXPECTED RESULT 8]: Should revert Port Mapping");
                                                print("[ACTUAL RESULT 8]: %s" %details);
                                                print("[TEST EXECUTION RESULT] : %s" %actualresult);
