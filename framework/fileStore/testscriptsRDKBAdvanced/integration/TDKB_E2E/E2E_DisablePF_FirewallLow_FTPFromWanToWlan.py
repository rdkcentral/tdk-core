                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if int(instance1) > 0:
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS")
                                                            print(f"ACTUAL RESULT {step}: FTP from WAN to GW is successful")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                step = step + 1
                                                                print(f"\nTEST STEP {step}: Check the FTP from WAN to WLAN after disabling port mapping")
                                                                print(f"EXPECTED RESULT {step}: FTP from WAN to WLAN should fail after disabling Port Mapping")
                                                                status = ftpToClient("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN")

                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                                    finalStatus = "SUCCESS"
                                                                    print(f"ACTUAL RESULT {step}: FTP from WAN to WLAN failed as expected")
                                                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE")
                                                                    print(f"ACTUAL RESULT {step}: FTP from WAN to WLAN is successful after disabling Port Mapping")
                                                                    print("[TEST EXECUTION RESULT] : FAILURE")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE")
                                                                details = tdkTestObj.getResultDetails()
