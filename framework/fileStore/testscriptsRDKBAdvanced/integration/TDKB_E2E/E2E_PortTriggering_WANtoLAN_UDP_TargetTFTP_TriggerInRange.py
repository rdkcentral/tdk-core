...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerInRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if Port Triggering rule takes effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 69 (TFTP).</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_597</test_case_id>
    <test_objective>To check if Port Triggering rule takes effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 69 (TFTP).</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 1234:1244, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule should take effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 69 (TFTP).</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerInRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerInRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerInRange');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 1234:1244, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "1234"
                        triggerEnd = "1244"
                        targetStart = "69"
