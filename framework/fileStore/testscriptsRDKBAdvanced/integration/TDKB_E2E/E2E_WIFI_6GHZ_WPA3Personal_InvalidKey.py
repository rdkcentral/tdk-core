...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_InvalidKey</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that a Wi-Fi client cannot connect to the 6 GHz SSID when an invalid WPA3-Personal key  is used.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_657</test_case_id>
    <test_objective>Verify that a Wi-Fi client cannot connect to the 6 GHz SSID when an invalid WPA3-Personal key is used.</test_objective>
...
2. Using tdkb_e2e_Get security mode
3. Set security mode to WPA3-Personal if security mode is not WPA3-Personal using tdkb_e2e_SetMultipleParams
4. Try to connect to Wifi client and check whether the wifi client is not connected to the DUT using WPA3-Personal and invalid wifi password
5. Revert the values if required
6. Unload the module.</automation_approch>
    <expected_output>Wi-Fi client should not connect to the DUT with WPA3-Personal security mode when an invalid password is used.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_InvalidKey</test_script>
    <skipped></skipped>
...
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_InvalidKey')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_InvalidKey')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        print(f"TEST STEP {step}: Get the current SSID, KeyPassphrase, Radio status, Security mode")
        print(f"EXPECTED RESULT {step}: Should retrieve the current SSID, KeyPassphrase, Radio status, Security mode")
        paramList = [ssidName, keyPassPhrase, radioEnable, securityMode]
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                step += 1
                # Set the ssid, radio, keypassphrase
                print(f"TEST STEP {step}: Set the SSID, KeyPassphrase, Radio status")
                print(f"EXPECTED RESULT {step}: Should set the SSID, KeyPassphrase, Radio status")

                setValuesList = ['true', tdkbE2EUtility.ssid_6ghz_name, tdkbE2EUtility.ssid_6ghz_pwd]
                ...
                if expectedresult in actualresult:
                    ...
                    print(f"TEST STEP {step}: Get the current SSID, KeyPassphrase, Radio status after set")
                    print(f"EXPECTED RESULT {step}: Retrieved values should match the set values")

                    newParamList = [radioEnable, ssidName, keyPassPhrase]
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        # Connect to the wifi ssid from wlan client with invalid wifi password
                        print(f"TEST STEP {step}: From wlan client, Connect to the wifi ssid using invalid wifi password")
                        print(f"EXPECTED RESULT {step}: Connection should fail with invalid wifi password")
                        status = tdkbE2EUtility.wlanConnectWifiSsid(tdkbE2EUtility.ssid_6ghz_name,tdkbE2EUtility.ssid_6ghz_invalid_pwd,tdkbE2EUtility.wlan_6ghz_interface)

                        if "SUCCESS" not in status:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print(f"ACTUAL RESULT {step}: Connection failed as expected with invalid password")
                            print(f"[TEST EXECUTION RESULT] : SUCCESS")
                            finalStatus = "SUCCESS"
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print(f"ACTUAL RESULT {step}: Connection succeeded unexpectedly with invalid password")
                ...
                step += 1
                print(f"TEST STEP {step}: Revert the values to original configuration")
                print(f"EXPECTED RESULT {step}: Should set the original SSID, KeyPassphrase, Radio status")
