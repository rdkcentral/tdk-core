...
<xml>
  ...
  <name>E2E_WIFI_5GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that udp traffic between wireless and wired clients should be denied when SSID parameter for 5GHZ [Device.WiFi.SSID.2.Enable] is disabled.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_235</test_case_id>
    <test_objective>Verify that udp traffic between wireless and wired clients should be denied when SSID parameter for 5GHZ [Device.WiFi.SSID.2.Enable] is disabled.</test_objective>
...
4. Disable 5GHZssid and usinf iperf check tcp traffic from wlan to lan
...
6. Revert the values</automation_approch>
    ...
    <test_script>E2E_WIFI_5GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan');
