...
<xml>
  ...
  <name>E2E_WIFI_5GHZ_WPA2PSK_SSIDDisabled_HTTPSTraffic</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that https traffic between WLan to LAN should be denied when wifi client assocation in wpa2-psk authentication for 5GHZ and SSID interface parameter for 5GHZ [Device.WiFi.SSID.2.Enable] is disabled</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_251</test_case_id>
    <test_objective>Verify that https traffic between WLan to LAN should be denied when wifi client assocation in wpa2-psk authentication for 5GHZ and SSID interface parameter for 5GHZ [Device.WiFi.SSID.2.Enable] is disabled</test_objective>
...
4. Disable ssid for 5GHz and send https request to lan client from wifi client
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPA2PSK_SSIDDisabled_HTTPSTraffic');
