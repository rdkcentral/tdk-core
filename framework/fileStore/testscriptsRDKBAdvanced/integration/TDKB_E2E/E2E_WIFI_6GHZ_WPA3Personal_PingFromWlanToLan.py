...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_PingFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that ICMP traffic from a wireless client to a wired LAN client is successful when the wireless client is associated using WPA3-Personal authentication on the 6 GHz band.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_659</test_case_id>
    <test_objective>Verify that ICMP traffic from a wireless client to a wired LAN client is successful when the wireless client is associated using WPA3-Personal authentication on the 6 GHz band.</test_objective>
...
6. From the wlan client, do ping to the LAN client and check whether  ping is success when the security mode for 6GHz is set to WPA3-Personal
7. Revert the security mode to original value if required
8. Unload tdkb_e2e module</automation_approch>
    <expected_output>Ping should be success between wlan client and lan client when the security mode for 6GHz is set to WPA3-Personal</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_PingFromWlanToLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_PingFromWlanToLan')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_PingFromWlanToLan')
                            ...
                            if wlanIP:
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print(f"EXPECTED RESULT {step}: Should retrive LAN client IP")
                                        ...
                                        if lanIP:
                                            ...
                                            # Check LAN IP in same DHCP range
                                            step += 1
                                            print(f"TEST STEP {step}: Check whether lan client ip address is in same DHCP range")
                                            print(f"EXPECTED RESULT {step}: LAN client IP should be in same DHCP range as gateway")
                                            status = tdkbE2EUtility.checkIpRange(curIPAddress, lanIP)
                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: LAN client IP is in DHCP range")
                                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                                # Send ping request from WLAN to LAN
                                                step += 1
                                                print(f"TEST STEP {step}: Check the PING from WLAN to LAN")
                                                print(f"EXPECTED RESULT {step}: PING should be successful")
                                                status = tdkbE2EUtility.verifyNetworkConnectivity(lanIP, "PING", wlanIP, curIPAddress)
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    ...
                                                    tdkTestObj.setResultStatus("FAILURE")
                                                    print(f"ACTUAL RESULT {step}: PING failed")
                                                    print("[TEST EXECUTION RESULT] : FAILURE")
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE")
