...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_WPAWPA2PSK_None_TCPFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association in wpawpa2-psk security wlan on 5ghz band with firewall set to None.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_453</test_case_id>
    <test_objective>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association in wpawpa2-psk security wlan on 5ghz band with firewall set to None.</test_objective>
...
3. Set the security mode for 5GHz to WPAWPA2PSK and firewall to None using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, do TCP to the LAN client and check whether  TCP is success when the security mode for 5GHz is set to WPAWPA2PSK
...
8.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WIFI_5GHZ_WPAWPA2PSK_None_TCPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPAWPA2PSK_None_TCPFromWlanToLan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'WPA-WPA2-Personal','None'];
