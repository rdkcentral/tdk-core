...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_2.4GHZ_TCPFromWanToLan_GetThroughput</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the throughput using iperf tool from the WAN to the LAN. The max speed of transfer will be 110 Mbps</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>To get the throughput using iperf tool from the WAN to the LAN. The max speed of transfer will be 110 Mbps</test_objective>
...
5. Check the TCP connection from WAN to LAN
6.Unload tdkb_e2e module</automation_approch>
    <except_output>The throughput should be less than 110Mbps</except_output>
    <priority>High</priority>
    <test_stub_interface>TDKB_E2E</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_TCPFromWanToLan_GetThroughput</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_TCPFromWanToLan_GetThroughput');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status and curIPAddress:
                        ...
                        if lanIP:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    #Verify TCP from WAN to LAN
                                    print("TEST STEP 7:Check TCP from WAN to LAN");
                                    status,serverOutput,clientOutput = tcp_udpInClients("WAN","LAN",lanIP,tdkbE2EUtility.wan_ip,"TCP_Throughput");
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if throughput < 110:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("Throughtput is less than 110Mbps")
                                            finalStatus = "SUCCESS"
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("Throughtput is more than 110Mbps")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 7:TCP from WAN to LAN failed")
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print("Failed to get the gateway ip")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current ssid,keypassphrase and securityMode")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,keypassphrase and securityMode")
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 8: Should set the original ssid,keypassphrase and securityMode");
