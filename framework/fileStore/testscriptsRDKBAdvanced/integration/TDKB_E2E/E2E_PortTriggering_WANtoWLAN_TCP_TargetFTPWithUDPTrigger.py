...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetFTPWithUDPTrigger</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_567</test_case_id>
...
11. From WAN client, send an inbound packet to the target port 21 via the DUT GW WAN IP. The packet should NOT be successfully forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetFTPWithUDPTrigger</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetFTPWithUDPTrigger');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetFTPWithUDPTrigger');
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                #Trigger the port using UDP trigger
                                                incorrectTriggerProtocol = "UDP"
                                                step = step + 1
                                                print("\nTEST STEP %d: Trigger the Port %s with incorrect protocol %s through an outbound packet from WLAN client" %(step, triggerStart, incorrectTriggerProtocol))
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    step = step + 1
                                                    print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the WLAN client that triggered with incorrect protocol" %step)
                                                    print("EXPECTED RESULT %d: FTP from WAN to the WLAN should be failure with the GW WAN IP of DUT" %step)
                                                    status = ftpToClient("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN");

                                                    if expectedresult not in status:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("ACTUAL RESULT %d: FTP from WAN to the WLAN is failed with the GW WAN IP of DUT" %step)
                                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                                    else:
