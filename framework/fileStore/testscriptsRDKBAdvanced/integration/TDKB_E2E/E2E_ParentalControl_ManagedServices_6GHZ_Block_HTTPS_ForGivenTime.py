...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_HTTPS_ForGivenTime')
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_HTTPS_ForGivenTime')
wifi_obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_6GHZ_Block_HTTPS_ForGivenTime')
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if int(instance) > 0:
                                ...
                                #SET the parameters to block HTTPS for given time
                                                ...
                                                if expectedresult in status and curIPAddress:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        if expectedresult in status:
                                                            ...
                                                            #Check HTTPS connectivity from WLAN to WAN
                                                            step += 1
                                                            print(f"\nTEST STEP {step}:Check the Https connectivity from WLAN to WAN")
                                                            print(f"EXPECTED RESULT {step}: Https connection should be blocked")
                                                            status = tdkbE2EUtility.wgetToWAN("WGET_HTTPS", wlanIP, curIPAddress)

                                                            if expectedresult not in status:
                                                                tdkTestObj.setResultStatus("SUCCESS")
                                                                print("ACTUAL RESULT: Https connection from WLAN to WAN is blocked")
                                                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE")
                                                                print("ACTUAL RESULT: Https connection from WLAN to WAN is success")
                                                                print("[TEST EXECUTION RESULT] : FAILURE")
