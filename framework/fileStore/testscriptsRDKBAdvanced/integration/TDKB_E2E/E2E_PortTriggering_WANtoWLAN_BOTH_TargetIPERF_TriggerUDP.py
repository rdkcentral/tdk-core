...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_BOTH_TargetIPERF_TriggerUDP</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_609</test_case_id>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - BOTH, Target Protocol - BOTH, Description - MyPTRule
...
17. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 5001 is forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule using UDP protocol.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_BOTH_TargetIPERF_TriggerUDP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_BOTH_TargetIPERF_TriggerUDP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_BOTH_TargetIPERF_TriggerUDP');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - BOTH, Target Protocol - BOTH, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12345"
                                        triggerEnd = "12345"
                                        targetStart = "5001"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                #Since targetProtocol BOTH is not supported in server side, passing the trigger protocol UDP
                                                target_protocol = "UDP"
                                                status = PTinitServer("IPERF", wlanIP, targetStart, target_protocol, tdkbE2EUtility.server_logfile, source="WLAN")
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("EXPECTED RESULT %d: IPERF client running in WAN machine should connect to the server running in WLAN with the GW WAN IP of DUT" %step)
                                                        #Since targetProtocol BOTH is not supported in server side, passing the target protocol UDP
                                                        status = PTClientRequest("IPERF", tdkbE2EUtility.wan_ip, targetStart, target_protocol, tdkbE2EUtility.client_logfile, "WAN");
