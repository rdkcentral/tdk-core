...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_BOTH_TargetTFTP_TriggerUDP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule with protocol as BOTH and UDP Trigger.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_614</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule  with protocol as BOTH and UDP Trigger.</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - BOTH, Target Protocol -BOTH, Description - MyPTRule
...
11. From WAN client, send an inbound packet to the target port 69 via the DUT GW WAN IP. The packet should be successfully forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 69 is forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_BOTH_TargetTFTP_TriggerUDP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_BOTH_TargetTFTP_TriggerUDP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_BOTH_TargetTFTP_TriggerUDP');
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("EXPECTED RESULT %d: TFTP from WAN to the LAN should be success with the GW WAN IP of DUT" %step)
