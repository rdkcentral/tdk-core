...
<xml>
  ...
  <name>E2E_Firewall_High_FtpFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to High FTP access from LAN to WAN should  be blocked</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_288</test_case_id>
    <test_objective>Verify that when Firewall Config is set to High FTP access from LAN to WAN should  be blocked</test_objective>
...
5. From the lan client, do FTP to the WAN client and check whether ftp is blocked when the firewall level is set to High
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>FTP from LAN to WAN should be blocked</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_High_FtpFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_High_FtpFromLanToWan ');
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult not in status:
                                        ...
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 10: failed, FTP from LAN to WAN is not blocked")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
