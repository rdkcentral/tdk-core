...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_802.11n_OperatingStandard</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that only 802.11n clients should be able to connect to WG when mode is n for frequency band 5GHZ</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_19</test_case_id>
    <test_objective>Verify that only 802.11n clients should be able to connect to WG when mode is n for frequency band 5GHZ</test_objective>
...
3. Set operating standard "n" using tdkb_e2e_SetMultipleParams
3. Try to connect to Wifi client and check whether the wifi client is connected to the DUT using operating standard "n"
4. Revert the values of operating standard</automation_approch>
    <except_output>WIFI client should be able to connect to the DUT with the using operating standard "n"</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_802.11n_OperatingStandard</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_802.11n_OperatingStandard');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'n'];
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 8: Check whether wlan client connected with 802.11n operating standard")
                                    operating_standard = getOperatingStandard(tdkbE2EUtility.ssid_5ghz_name);
                                    if operating_standard == "802.11n":
                                        tdkTestObj.setResultStatus("SUCCESS");
