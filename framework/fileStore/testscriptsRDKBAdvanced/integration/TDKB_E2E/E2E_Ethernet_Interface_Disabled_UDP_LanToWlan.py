...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_Ethernet_Interface_Disabled_UDP_LanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that UDP traffic between wired and wireless clients should be denied when Ethernet interface parameter is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_207</test_case_id>
    <test_objective>Verify that UDP traffic between wired and wireless clients should be denied when Ethernet interface parameter is disabled.</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband,Emulator,RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.The lan client should be connetcted to the first Ethernet port </pre_requisite>
...
3. Set security mode to WPA2-PSK,ethernet enable and ssid enable to true using tdkb_e2e_SetMultipleParams
3. Try to connect to 2.4GHZ from WIFI client and Lan Client
4. Disable Ethernet.Interface  and send udp request to wlan client from lan client
4. Revert the values</automation_approch>
    <except_output>udp request should fail since Ethernet.Interface is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Ethernet_Interface_Disabled_UDP_LanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Ethernet_Interface_Disabled_UDP_LanToWlan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        #Parse the sampleDevice configuration file
        status = parseSampleDeviceConfig();
        if expectedresult in status:
            obj.setLoadModuleStatus("SUCCESS");
            print("Parsed the sampleDevice configuration file successfully")
            ...
            if expectedresult in status:
                                        ...
                                        if lanIP:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in actualresult:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("TEST STEP 10: Set ethernetEnable")
                                                    ...
                                                    if expectedresult in status and setValuesList == newValues:
                                                        ...
                                                        #Send UDP request to LAN client from WLAN client
                                                        print("TEST STEP 12: Connect to WLAN UDP client and send Request to LAN Client")
                                                        status = tcp_udpInClients("LAN","WLAN",wlanIP,lanIP,"UDP");
                                                        if "SUCCESS" not in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("UDP not successful between wired and wireless clients")
                                                            finalStatus = "SUCCESS"
                                                            ...
                                                            if expectedresult in status:
                                                                ...
                                                                print("wlanDisconnectWifiSsid: Failed to disconnect the wifi ssid")
                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("UDP successful between wired and wireless clients")
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("TEST STEP 11: Get the current ethernetEnable")
                                                        print("EXPECTED RESULT 11: Should retrieve the current ethernetEnable")
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("getLanIPAddress:Failed to get lan client ip")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                ...
                if expectedresult in actualresult and expectedresult in finalStatus and expectedresult in ethernetResult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("EXPECTED RESULT 15: Should set the original ssid,keypassphrase,securityMode,ethernetEnable");
                ...
                tdkTestObj.setResultStatus("FAILURE");
                print("TEST STEP 1: Get the current ssid,keypassphrase,securityMode,ethernetEnable")
                print("EXPECTED RESULT 1: Should retrieve the current ssid,keypassphrase,securityMode,ethernetEnable")
                print("ACTUAL RESULT 1: %s" %orgValue);
                print("[TEST EXECUTION RESULT] : FAILURE");
        else:
            obj.setLoadModuleStatus("FAILURE");
            print("Failed to parse the sampleDevice configuration file")
    else:
        obj.setLoadModuleStatus("FAILURE");
