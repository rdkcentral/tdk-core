...
<xml>
  ...
  <name>E2E_WIFI_MACFilter_Allow_2.4GHZ_WIFIClient</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that WG is able to allow Access for specific Wi-Fi clients of 2.4GHZ SSID that are configured in MAC address filter</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_44</test_case_id>
    <test_objective>Verify that WG is able to allow Access for specific Wi-Fi clients of 2.4GHZ SSID that are configured in MAC address filter</test_objective>
...
6.Add new rule to Device.WiFi.AccessPoint.1.X_CISCO_COM_MacFilterTable.
7. Set the devicename and mac details of client to be blocked in the MAC filter rule
8.Try to connect the blocked client to the GW SSID, that connection attempt should be success
...
10.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WIFI_MACFilter_Allow_2.4GHZ_WIFIClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_MACFilter_Allow_2.4GHZ_WIFIClient');
obj1.configureTestCase(ip,port,'E2E_WIFI_MACFilter_Allow_2.4GHZ_WIFIClient');
