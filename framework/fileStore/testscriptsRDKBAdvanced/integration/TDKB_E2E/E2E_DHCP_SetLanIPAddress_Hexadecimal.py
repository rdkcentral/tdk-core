...
<xml>
  ...
  <name>E2E_DHCP_SetLanIPAddress_Hexadecimal</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that Gateway IP in DHCPv4 server pool cannot be set with Hexadecimal characters. (ex.123ABC). Ensure that DHCP configuration on client is not affected</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_263</test_case_id>
    <test_objective>Verify that Gateway IP in DHCPv4 server pool cannot be set with Hexadecimal characters. (ex.123ABC). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set LanIPAddress to hexadecimal characters using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting lanIPAddress as hexadecimal characters</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetLanIPAddress_Hexadecimal</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetLanIPAddress_Hexadecimal');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the LanIPAddress to some hexadecimal characters"
                    lanIPValue = "%s|123ABC|string" %lanIPAddress

                    tdkTestObj,status,details = setMultipleParameterValues(obj,lanIPValue)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the lanIPAddress to some hexadecimal characters")
                        ...
                        if expectedresult in retStatus and "123ABC" not in newlanIPValue :
                            tdkTestObj.setResultStatus("SUCCESS");
