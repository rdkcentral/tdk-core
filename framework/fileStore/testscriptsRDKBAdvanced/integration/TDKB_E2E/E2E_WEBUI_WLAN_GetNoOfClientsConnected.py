...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_WLAN_GetNoOfClientsConnected')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            print(f"TEST STEP {step}: Get the current ssid and keypassphrase and number of clients connected to WLAN 2.4GHZ")
            print(f"EXPECTED RESULT {step}: Should retrieve the current ssid and keypassphrase and  clients connected count")
            print(f"ACTUAL RESULT {step}: {orgValue}")
            print("[TEST EXECUTION RESULT] : SUCCESS")

            # Check whether MLO is enabled. If enabled, proceed with MLO SSID and Passphrase. If disabled, set 2.4 GHz SSID and password
                                ...
                                if expectedresult in status:
                                    ...
                                    if status == "SUCCESS":
                                        try:
                                            ...
                                            print(f"number of clients connected to WiFi network :{num}")
            ...
            if tdkbE2EUtility.mlo_capability == "False":
                print("Reverting the ssid and keypassphrase to original values")
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print(f"TEST STEP {step}: Get the current ssid and keypassphrase and clients connected count")
            print(f"EXPECTED RESULT {step}: Should retrieve the current ssid and keypassphrase and clients connected count")
            print(f"ACTUAL RESULT {step}: {orgValue}")
            print("[TEST EXECUTION RESULT] : FAILURE")
