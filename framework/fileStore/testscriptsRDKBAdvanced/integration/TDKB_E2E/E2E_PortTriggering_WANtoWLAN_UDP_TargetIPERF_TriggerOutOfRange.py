...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF_TriggerOutOfRange</name>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
12. Send an outbound packet to trigger the out of range port 12350 with UDP packet
13. From WAN client, send an inbound packet to the target port 5001 via the DUT GW WAN IP. The packet should not be forwarded to the LAN client.
...
17. Revert the SSID, KeyPassphrase and Radio Enable to initial values.
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 5001 is not forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule using UDP protocol.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF_TriggerOutOfRange');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12340"
                                        triggerEnd = "12346"
                                        targetStart = "5001"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("\nTEST STEP %d: Trigger the Port %s through a specified outbound packet from WLAN client" %(step, trigger_OutOfRange))
