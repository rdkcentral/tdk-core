...
<xml>
  ...
  <name>E2E_ParentalControl_ManagedServices_5GHZ_Block_HTTP_ForGivenTime</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that WG supports blocking of HTTP service for Wi-Fi Client associated using 5GHZ radio only during specific day and time period through parental control (managed services)</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_124</test_case_id>
    <test_objective>Verify that WG supports blocking of HTTP service for Wi-Fi Client associated using 5GHZ radio only during specific day and time period through parental control (managed services)</test_objective>
...
7.Check whether the WLAN client is able to establish http connection
8.Delete the added rule and revert all params values
9.Unload tdkb_e2e module</automation_approch>
    <expected_output>WLAN client should not be able to establish http connection within the specified time</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedServices_5GHZ_Block_HTTP_ForGivenTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_Block_HTTP_ForGivenTime');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_Block_HTTP_ForGivenTime');
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        if expectedresult not in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("The blocked service 'HTTP' cannot be reached")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("The blocked service 'HTTP' can be reached")
