...
<xml>
  ...
  <name>E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTPS_FromLanToWLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that HTTPS traffic between Lan to WLAN should be successful when wifi client is associated with wpa3-psk authentication for 6GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_663</test_case_id>
    <test_objective>Verify that HTTPS traffic between Lan to WLAN should be successful when wifi client is associated with wpa3-psk authentication for 6GHZ</test_objective>
...
Device.WiFi.SSID.{i}.Enable</input_parameters>
    <automation_approch>1. Load tdkb_e2e module
2. Using tdkb_e2e_Get, get and save security mode,Radio enable and ssid enable status of 6GHz
3. Set security mode to WPA3-Personal if it is not WPA3-Personal,radio enable and ssid enable to true using tdkb_e2e_SetMultipleParams
4. Try to connect to 6GHZ ssid from WIFI client.
5. send HTTPS request to wlan client from lan client
6. Revert the values</automation_approch>
    <expected_output>HTTPS request between lan and wlan should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTPS_FromLanToWLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTPS_FromLanToWLan')
wifi_obj.configureTestCase(ip,port,'E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTPS_FromLanToWLan')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        step += 1
        print(f"TEST STEP {step}: Get the current ssid,keypassphrase,securityMode,radioEnable and ssidEnable")
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                # Set radioEnable and ssidEnable for 6ghz
                ...
                step += 1
                print(f"TEST STEP {step}: Set the ssid,keypassphrase,radioEnable and ssidEnable")
                print(f"EXPECTED RESULT {step}: Should set the new values successfully")
                ...
                if expectedresult in actualresult:
                    ...
                    step += 1
                    print(f"TEST STEP {step}: Get the values after set")
                    print(f"EXPECTED RESULT {step}: Values should match the set values")
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        if expectedresult in status:
                            ...
                            if wlanIP:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"WLAN IP Address: {wlanIP}")
                                print(f"[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    step += 1
                                    print(f"TEST STEP {step}: Check whether wlan ip is in the same DHCP range")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if lanIP:
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print(f"LAN Client IP Address: {lanIP}")
                                            print(f"[TEST EXECUTION RESULT] : SUCCESS")

                                            step += 1
                                            print(f"TEST STEP {step}: Check whether lan client ip is in the same DHCP range")
                                            print(f"EXPECTED RESULT {step}: Lan clinet IP should be in the DHCP range")
                                            status = tdkbE2EUtility.checkIpRange(curIPAddress, lanIP)

                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESUTL {step}: {status}")
                                                print(f"[TEST EXECUTION RESULT] : SUCCESS")

                                                step += 1
                                                print(f"TEST STEP {step}: Send HTTPS request from LAN to WLAN client")
                                                print(f"EXPECTED RESULT {step}: HTTPS traffic should be successful")
                                                status = tdkbE2EUtility.verifyNetworkConnectivity(wlanIP, "WGET_HTTPS", lanIP, curIPAddress, "LAN")

                                                if "SUCCESS" in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    print(f"ACTUAL RESULT {step}: HTTPS traffic between LAN and WLAN clients is successful")
                                                    print(f"[TEST EXECUTION RESULT] : SUCCESS")
                                                    finalStatus = "SUCCESS"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE")
                                                    print(f"ACTUAL RESULT {step}: HTTPS traffic failed")
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print(f"ACTUAL RESULT {step}: Lan client IP not in DHCP range")
                                                print(f"[TEST EXECUTION RESULT] : FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                ...
                step += 1
                print(f"TEST STEP {step}: Revert to original values")
                print(f"EXPECTED RESULT {step}: Should set values back successfully")
    ...
    print("Failed to load tdkb_e2e module and wifi module")
    obj.setLoadModuleStatus("FAILURE")
