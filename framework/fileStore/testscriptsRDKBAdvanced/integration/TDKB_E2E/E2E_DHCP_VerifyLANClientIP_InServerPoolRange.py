...
<xml>
  ...
  <name>E2E_DHCP_VerifyLANClientIP_InServerPoolRange</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that on configuring the DHCPv4 Server pool range as 10.0.0.10 - 10.0.0.15 ensure that client receives IP address within that range</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_282</test_case_id>
    <test_objective>Verify that on configuring the DHCPv4 Server pool range as 10.0.0.10 - 10.0.0.15 ensure that client receives IP address within that range</test_objective>
...
4. Set minimum and maximum addresses as 10.0.0.10 and 10.0.0.15 using tdkb_e2e_SetMultipleParams
5.  Connect to the LAN client and check whether the ipaddress assigned is from the minimum to maximum range
5. Revert the values to original value
6.Unload tdkb_e2e module</automation_approch>
    <except_output>IP assigned to the LAN client should be from the minimum to maximum range</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_VerifyClientIP_InServerPoolRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_VerifyLANClientIP_InServerPoolRange');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = ['10.0.0.1','255.255.0.0','10.0.0.10','10.0.0.15'];
            ...
            list3 = [minAddress,'10.0.0.10','string']
            list4 = [maxAddress,'10.0.0.15','string']
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print("TEST STEP 4: Get the lan ip address")
                    lanIP = getLanIPAddress(tdkbE2EUtility.lan_interface);
                    if lanIP:
                        tdkTestObj.setResultStatus("SUCCESS");

                        print("TEST STEP 5: Check whether lan ip address is in minimum to maximum range")

                        status = "SUCCESS"
                        ipmin = "10.0.0.10";
                        ipmax = "10.0.0.15";
                        status = checkIpWithinMinMaxRange(ipmin,ipmax,lanIP)
                        if expectedresult in status:
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            print("TEST STEP 5: LanIP obtained is not in the DHCP server pool range")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
