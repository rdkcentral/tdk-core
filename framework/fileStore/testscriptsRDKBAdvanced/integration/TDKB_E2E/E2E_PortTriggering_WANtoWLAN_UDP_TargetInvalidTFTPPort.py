...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_PortTriggering_WANtoWLAN_UDP_TargetInvalidTFTPPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 6969 is forwarded to the WLAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_651</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 6969 is forwarded to the WLAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
13. From WAN client, send an inbound packet to the target port 6969 via the DUT GW WAN IP. The packet should NOT be forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 6969 is forwarded to the WLAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_UDP_TargetInvalidTFTPPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetInvalidTFTPPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetInvalidTFTPPort');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12345"
                                        triggerEnd = "12345"
                                        targetStart = "69"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("EXPECTED RESULT %d: TFTP from WAN to the WLAN should fail with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
                                                    status = tftpToClient(tdkbE2EUtility.gw_wan_ip, "WLAN", tdkbE2EUtility.tftpfile, message, "WAN", invalidPort)
