...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerInRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if Port Triggering rule takes effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 54321 (NETCAT).</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_594</test_case_id>
    <test_objective>To check if Port Triggering rule takes effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 54321 (NETCAT).</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 1234:1244, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
10. Send an outbound packet to trigger a randomly chosen port in range 1234:1244 with a TCP packet
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule should take effect when the trigger port is between the range (triggerPortStart, triggerPortEnd) and the target port is 54321 (NETCAT).</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerInRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerInRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerInRange');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 1234:1244, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "1234"
                        triggerEnd = "1244"
                        targetStart = "54321"
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("\nTEST STEP %d: Trigger the Port %s through a specified outbound packet from LAN client" %(step, portInRange))
                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                    status = triggerPort(tdkbE2EUtility.wan_ip, portInRange, triggerProtocol, "LAN")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("EXPECTED RESULT %d: NETCAT client runing in WAN machine should connect to the server running in LAN with the GW WAN IP of DUT" %step)
