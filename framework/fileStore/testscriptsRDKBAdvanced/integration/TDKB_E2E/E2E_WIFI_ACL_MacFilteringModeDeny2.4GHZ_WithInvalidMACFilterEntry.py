...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeDeny2.4GHZ_WithInvalidMACFilterEntry')
obj1.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeDeny2.4GHZ_WithInvalidMACFilterEntry')
obj2.configureTestCase(ip,port,'E2E_WIFI_ACL_MacFilteringModeDeny2.4GHZ_WithInvalidMACFilterEntry')
                ...
                if expectedresult in actualresult  and mode == "Deny" :
                    ...
                    if macAddress and (re.match("^[0-9a-f]{2}([-:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", macAddress.lower()) is not None):
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            step += 1
                            print(f"\nTEST STEP {step}: Set invalid MAC in the filter table entry for 2.4Ghz.")
                            print(f"EXPECTED RESULT {step}: Should set invalid MAC in the filter table entry for 2.4Ghz.")
                            tdkTestObj = obj.createTestStep('tdkb_e2e_Set')
                            ...
                            if expectedresult in actualresult:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: Set invalid MAC in the filter table entry for 2.4Ghz successfully.")
                                #Get the result of execution
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                if expectedresult in actualresult and bssid and (re.match("^[0-9a-f]{2}([-:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", bssid.lower()) is not None):
                                    ...
                                    if expectedresult in status and orgValue != "":
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}: Got the current ssid,keypassphrase  as {orgValue}.")
                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: Failed to Set invalid Mac Filter MacAddress Entry for 2.4Ghz.")
                                print("[TEST EXECUTION RESULT] : FAILURE")
