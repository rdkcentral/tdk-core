...
<xml>
  ...
  <name>E2E_XDNS_DualMask_PrimaryDNS_NoDnsIPv6_Level2</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Set dual dns masq with level2 primary xdns server, but skip adding ipv6 xdns server config. Check if default dns is used instead of xdns and level2 site is accessible from client.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_500</test_case_id>
    <test_objective>Set dual dns masq with level2 primary xdns server, but skip adding ipv6 xdns server config. Check if default dns is used instead of xdns and level2 site is accessible from client.</test_objective>
...
5. Add new rule in this table with LAN client MAC, level 2 DNS ipv4 primary dns ips and a tag name for rule
6. If the default DNS ips are empty, set dns ip values
7. Enable Device.DeviceInfo.X_RDKCENTRAL-COM_EnableXDNS
8. From LAN client do a ping to any level2 site
9. Ping should be success as default DNS server will be used instead of XDNS for DNS resolution level2 site
...
12. Unload tdkb_e2e module</automation_approch>
    <except_output>Default dns should be used instead of xdns and level2 site should be accessible from client.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_XDNS_DualMask_PrimaryDNS_NoDnsIPv6_Level2</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_XDNS_DualMask_PrimaryDNS_NoDnsIPv6_Level2');
obj1.configureTestCase(ip,port,'E2E_XDNS_DualMask_PrimaryDNS_NoDnsIPv6_Level2');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if (instance > 0):
                                ...
                                setValuesList = [lanMAC,tdkbE2EUtility.xdns_level2_dns_server,"Default"];
                                print("Parameter values that are set: %s" %setValuesList)

                                list1 = [macAddress,lanMAC,'string']
                                list2 = [dnsipv4,tdkbE2EUtility.xdns_level2_dns_server,'string']
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    if expectedresult in actualresult1:
                                        ...
                                        if expectedresult in status and setValuesList == newValues:
                                            ...
                                            #Check if the Level2 site is accessible
                                            status = verifyNetworkConnectivity(tdkbE2EUtility.xdns_level2_site, "PING_TO_HOST", lanIP, curIPAddress,"LAN")

                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("TEST STEP 9:Check if the level2 site accessible from the blocked device")
                                                print("EXPECTED RESULT 9:The level2 site should be accessible from the blocked device as default dns is used instead of xdns")
                                                print("[TEST EXECUTION RESULT] : SUCCESS");
                                                finalStatus = "SUCCESS"
                                            else:
