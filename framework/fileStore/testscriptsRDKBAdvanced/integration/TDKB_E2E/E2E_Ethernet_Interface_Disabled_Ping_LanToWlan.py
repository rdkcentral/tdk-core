...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_Ethernet_Interface_Disabled_Ping_LanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ICMP traffic between wired and wireless clients should be denied when Ethernet interface parameter is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_204</test_case_id>
    <test_objective>Verify that ICMP traffic between wired and wireless clients should be denied when Ethernet interface parameter is disabled.</test_objective>
...
4. Disable Ethernet.Interface  and send Ping request to wlan client from lan client
4. Revert the values</automation_approch>
    <except_output>Ping request should fail since Ethernet.Interface is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Ethernet_Interface_Disabled_Ping_LanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Ethernet_Interface_Disabled_Ping_LanToWlan');
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in actualresult:
                                                    ...
                                                    if expectedresult in status and setValuesList == newValues:
                                                        ...
                                                        print("TEST STEP 12: Connect to LAN Client and send Ping Request to WLAN Client")
                                                        status = verifyNetworkConnectivity(wlanIP,"PING",lanIP,curIPAddress,"LAN");
                                                        if "SUCCESS" not in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("Ping not successful between wired and wireless clients")
                                                            finalStatus = "SUCCESS"
                                                            ...
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("Ping successful between wired and wireless clients")
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("TEST STEP 11: Get the current ethernetEnable")
                                                        print("EXPECTED RESULT 11: Should retrieve the current ethernetEnable")
