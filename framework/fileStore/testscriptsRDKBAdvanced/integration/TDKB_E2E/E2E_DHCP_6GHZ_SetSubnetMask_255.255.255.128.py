...
<xml>
  ...
  <name>E2E_DHCP_6GHZ_SetSubnetMask_255.255.255.128</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that on configuring DHCP Server Subnet mask to 255.255.255.128, it gets reflected successfully on the Wi-Fi Client connected via 6GHz radios</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_676</test_case_id>
    <test_objective>Verify that on configuring DHCP Server Subnet mask to 255.255.255.128, it gets reflected successfully on the Wi-Fi Client connected via 6GHz radios</test_objective>
...
3. Set LanIP to 10.0.0.1,SubnetMask to 255.255.255.128,Maximum to 10.0.0.2 and minimum Address range to 10.0.0.126 using tdkb_e2e_SetMultipleParams
...
6. Unload tdkb_e2e module</automation_approch>
    <expected_output>The sub net mask obtained should be 255.255.255.128 in connected wifi client</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_6GHZ_SetSubnetMask_255.255.255.128</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,"E2E_DHCP_6GHZ_SetSubnetMask_255.255.255.128")
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_6ghz_name,tdkbE2EUtility.ssid_6ghz_pwd,"10.0.0.1","255.255.255.128","10.0.0.2","10.0.0.126"]
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status:
                        ...
                        if wlanSubnetMask:
                            ...
                            # Check whether subnet mask is 255.255.255.128
                            print(f"TEST STEP {step}: Check whether subnet mask is 255.255.255.128")
                            print(f"EXPECTED RESULT {step}: Subnet mask should be 255.255.255.128")
                            ...
                            if expectedresult in status:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: Subnet mask changed to 255.255.255.128")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                step += 1
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: Subnet mask is not 255.255.255.128")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
