...
<xml>
  ...
  <name>E2E_ParentalControl_ManagedDevices_BlockAll_Allow_LAN</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when managed devices state is set as Block All, WG supports allowing of a specific LAN client</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Verify that when managed devices state is set as Block All, WG supports allowing of a specific LAN client</test_objective>
...
3. Enable managed device, set False to Allowall
4.  Refresh LAN connection and get the MAC address of the LAN client
5.Add new rule to Device.X_Comcast_com_ParentalControl.ManagedDevices.Device. and give the MAC of LAN client to allow it
6. Set values to all fields in new rule
7.Check whether the LAN client's internet access is allowed using wget
...
9.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_ParentalControl_ManagedDevices_BlockAll_Allow_LAN</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedDevices_BlockAll_Allow_LAN');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedDevices_BlockAll_Allow_LAN');
                                                        ...
                                                        if expectedresult in status:
                                                            ...
                                                            if expectedresult in status:
                                                                ...
                                                                if expectedresult in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("Http connection from LAN to WAN is success")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("Http connection from LAN to WAN is blocked")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE");
