...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_CheckSSIDBroadcast_RadioDisabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when 6 GHz radio is disabled corresponding SSID should not be broadcasted in Client side.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_664</test_case_id>
    <test_objective>Verify that when 6 GHz radio is disabled corresponding SSID should not be broadcasted in Client side.</test_objective>
...
2. Using tdkb_e2e_Get, get and save security mode and 6GHZ radio enable status
3. Set security mode to WPA3-Personal and Disable Radio using tdkb_e2e_SetMultipleParams
4. Check whether SSID for 6GHZ is broadcasting or not
5. Revert the values to original value
6. Unload tdkb_e2e module</automation_approch>
    <expected_output>SSID for 6GHZ should not  broadcast since Radio is disabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_CheckSSIDBroadcast_RadioDisabled</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_CheckSSIDBroadcast_RadioDisabled')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_CheckSSIDBroadcast_RadioDisabled')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        step += 1
        print(f"TEST STEP {step}: Get the current ssid, keypassphrase, securityMode and radioEnable")
        print(f"EXPECTED RESULT {step}: Should retrieve the current ssid, keypassphrase, securityMode and radioEnable")
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                # Set radioEnable and keypassphrase for 6ghz
                step += 1
                print(f"TEST STEP {step}: Set the ssid, keypassphrase, and radioEnable")
                print(f"EXPECTED RESULT {step}: Should set the ssid, keypassphrase, and radioEnable")
                setValuesList = [tdkbE2EUtility.ssid_6ghz_name, tdkbE2EUtility.ssid_6ghz_pwd, 'false']
                ...
                if expectedresult in actualresult:
                    ...
                    step += 1
                    print(f"TEST STEP {step}: Get the current ssid, keypassphrase, and radioEnable")
                    print(f"EXPECTED RESULT {step}: Should retrieve the current ssid, keypassphrase, and radioEnable")
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        print(f"EXPECTED RESULT {step}: SSID should not be broadcasted on the network")
                        status = tdkbE2EUtility.wlanIsSSIDAvailable(tdkbE2EUtility.ssid_6ghz_name)

                        if expectedresult not in status:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print(f"ACTUAL RESULT {step}: Network name {tdkbE2EUtility.ssid_6ghz_name} is not broadcasted")
                            print("TEST EXECUTION RESULT : SUCCESS")
                            finalStatus = "SUCCESS"
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print(f"ACTUAL RESULT {step}: Network name {tdkbE2EUtility.ssid_6ghz_name} is broadcasted")
                            print("TEST EXECUTION RESULT : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: {newValues}")
                ...
                if sm_flag == 1:
                    ...
                    step += 1
                    print(f"TEST STEP {step}: Revert Security Mode to initial security mode {initial_secMode}")
                ...
                step += 1
                print(f"TEST STEP {step}: Revert the original ssid, keypassphrase and radioEnable")
                print(f"EXPECTED RESULT {step}: Should set the original ssid, keypassphrase and radioEnable")
