...
<xml>
  ...
  <name>E2E_PF_FirewallLow_HTTPFromWanToWlan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify Port forwarding with HTTP service running on Wi-Fi Client and Firewall mode set to Low HTTP traffic should be successful from WAN to WLAN</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_85</test_case_id>
    <test_objective>Verify Port forwarding with HTTP service running on Wi-Fi Client and Firewall mode set to Low HTTP traffic should be successful from WAN to WLAN</test_objective>
...
13.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>HTTP connection from WAN client to WAN ip of gateway, after enabling port forwarding should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_PF_FirewallLow_HTTPFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PF_FirewallLow_HTTPFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_PF_FirewallLow_HTTPFromWanToWlan');
                                            ...
                                            #Set the result status of execution
                                                ...
                                                if (instance1 > 0):
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        time.sleep(60);
                                                        print("TEST STEP 11:Check the HTTP from WLAN to LAN")
                                                        ...
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                            print("HTTP from WLAN to LAN: SUCCESS")

                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("TEST STEP 11:HTTP from WLAN to LAN failed")
