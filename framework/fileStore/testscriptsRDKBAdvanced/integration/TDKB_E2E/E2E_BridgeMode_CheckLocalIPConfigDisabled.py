...
<xml>
  ...
  <name>E2E_BridgeMode_CheckLocalIPConfigDisabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>When the gateway is in bridge mode check whether the local ip configuration is disabled. Also on disabling bridge mode local ip configuration should be enabled back</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>When the gateway is in bridge mode check whether the local ip configuration is disabled. Also on disabling bridge mode local ip configuration should be enabled back</test_objective>
...
6. Change lanmode to router
7. Again try to change local ip config. Set operation should be success
8. Revert the Lanmode and local ip config to its initial values
9. Unload tdkb_e2e module</automation_approch>
    <except_output>When device is in bridge local ip configuration option should be disabled and when bridge mode is disabled, local ip config should be enabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_BridgeMode_CheckLocalIPConfigDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_BridgeMode_CheckLocalIPConfigDisabled');
        ...
        if expectedresult in status:
            ...
            if expectedresult in status:
                ...
                if expectedresult not in actualresult:
                    ...
                    #disable bridge mode and check local ip configuration
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in actualresult:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print("TEST STEP 3: Set the Lanip, min and max addresses")
                            print("EXPECTED RESULT 3: Should set the Lanip, min and max addresses");
                            ...
                            if expectedresult in status and setValuesList == newValues:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print("TEST STEP 4: Get the current ipAddress, minAddress,maxAddress")
                                print("EXPECTED RESULT 4: Should retrieve the ipAddress ,minAddress,maxAddress")
                                print("ACTUAL RESULT 4: %s" %newValues);
                                print("[TEST EXECUTION RESULT] : SUCCESS");
                                print("SUCCESS: Local ip configuration is enabled on disabling bridge mode")

                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("TEST STEP 4: Get the current lanip,min and max addresses")
                                print("EXPECTED RESULT 4: Should retrieve the current lanip,min and max addresses")
                                print("ACTUAL RESULT 4: %s" %newValues);
            ...
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 5: Should set the original lanip,min and max addresses, lanMode")
