...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_563</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 54321 is forwarded to the LAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</test_objective>
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 54321 is forwarded to the LAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT');
