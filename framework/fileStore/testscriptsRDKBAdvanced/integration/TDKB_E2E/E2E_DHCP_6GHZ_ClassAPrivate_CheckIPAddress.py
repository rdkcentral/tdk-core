...
<xml>
  ...
  <name>E2E_DHCP_6GHZ_ClassAPrivate_CheckIPAddress</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that on configuring DHCP Server Class A Private IP range (10.x.x.x - 10.255.255.255) Wi-Fi Client connected via 6GHz radio successfully receives IP address</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_672</test_case_id>
    <test_objective>Verify that on configuring DHCP Server Class A Private IP range (10.x.x.x - 10.255.255.255) Wi-Fi Client connected via 6GHz radio successfully receives IP address</test_objective>
    <test_type>Positvie</test_type>
...
4. From Wlan client connect to the 6ghz ssid and check whether the IP obtained is in valid range
5. Revert the values to original value
6. Unload tdkb_e2e module
</automation_approch>
    <expected_output>The IP obtained by the wlan client should be in Class A private range</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_6GHZ_ClassAPrivate_CheckIPAddress</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,"E2E_DHCP_6GHZ_ClassAPrivate_CheckIPAddress")
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        print(f"EXPECTED RESULT {step}: Should retrieve the current values successfully")
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_6ghz_name,tdkbE2EUtility.ssid_6ghz_pwd,"10.0.0.1","255.255.255.0","10.0.0.2","10.0.0.253"]
            ...
            print(f"EXPECTED RESULT {step}: Should set the values successfully")
            ...
            if expectedresult in actualresult and expectedresult in actualresult1:
                ...
                print(f"TEST STEP {step}: Get the parameters after set")
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status:
                        ...
                        print(f"TEST STEP {step}: Get the IP address of wlan client")
                        print(f"EXPECTED RESULT {step}: Should successfully retrieve the IP address")
                        wlanIP = getWlanIPAddress(tdkbE2EUtility.wlan_6ghz_interface)
                        if wlanIP:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print(f"ACTUAL RESULT {step}: wlanIP Retrieved:{wlanIP}")
                            print("[TEST EXECUTION RESULT] : SUCCESS")
                            step += 1
                            ...
                            print(f"EXPECTED RESULT {step}: WLAN client IP should be in same DHCP range as Gateway")
                            LanIP = newValues[2]
                            status = checkIpRange(LanIP, wlanIP)
                            if expectedresult in status:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: WLAN Client IP is in the same DHCP range")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                step += 1
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print(f"ACTUAL RESULT {step}: Failed to get the current ssid, keypassphrase, lanIPAddress, lanSubnetMask, minAddress and maxAddress values")
            print("[TEST EXECUTION RESULT] : FAILURE")
    else:
