...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_WAN_HttpTo5GWLan_WPAWPA2PSK</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that http connection from WAN to 5GHZ WLAN client with WPAWPA2PSK security mode is not success</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_547</test_case_id>
    <test_objective>Verify that http connection from WAN to 5GHZ WLAN client with WPAWPA2PSK security mode is not success</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI</test_setup>
    <pre_requisite>WLAN client device and a WAN machine with the tdkb e2e WAN configuration should be available</pre_requisite>
...
3.Connect the WiFi client to the 5GSSID  of gateway with WPAWPA2PSK security mode
4. Check if the WiFi client's ip received from gateway is in valid range
5. From WAN client check if HTTP connection to 5G WLAN client ip is success or not
6. Disconnect the WiFi client
7.Unload tdkb_e2e module</automation_approch>
    <expected_output> HTTP connection to 2.4G WLAN client ip is not success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WAN_HttpTo5GWLan_WPAWPA2PSK</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WAN_HttpTo5GWLan_WPAWPA2PSK');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 10:Check the HTTP connection from WAN to 5GHZ WLAN client ip")
                                    expectedresult = "FAILURE"
                                    status = verifyNetworkConnectivity(wlanIP, "WGET_HTTP", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: HTTP connection not successful from WAN to 5GHZ WLAN client ip")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("FAILURE: HTTP connection successful from WAN to 5GHZ WLAN client ip")
