...
<xml>
  ...
  <name>E2E_WIFI_2.4GHZ_WPA2PSK_HttpFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that http traffic between WLAN to LAN should be succesfull when wifi client assocation in wpa2-psk authentication for 2.4GHZ .</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_241</test_case_id>
    <test_objective>Verify that http traffic between WLAN to LAN should be succesfull when wifi client assocation in wpa2-psk authentication for 2.4GHZ .</test_objective>
...
5. From the wlan client, try to connect to the LAN client via http and check whether http connection is success when the security mode for 2.4GHz is set to WPA2PSK
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WIFI_2.4GHZ_WPA2PSK_HttpFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPA2PSK_HttpFromWlanToLan');
