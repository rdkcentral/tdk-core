...
obj.configureTestCase(ip,port,'E2E_WIFI_TCPFromWlanToWan_GetThroughput')
sysobj.configureTestCase(ip,port,'E2E_WIFI_TCPFromWlanToWan_GetThroughput')
                            ...
                            if expectedresult in status and curIPAddress != "":
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        step += 1
                                        print(f"\nTEST STEP {step}:Check TCP from WLAN to WAN")
                                        print(f"EXPECTED RESULT {step}: Should perform TCP from WLAN to WAN successfully")
                                        ...
                                        if expectedresult in status and serverOutput != "":
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print(f"ACTUAL RESULT {step}: TCP from WLAN to WAN is successful")
                                            ...
                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                            raw_threshold = str(tdkbE2EUtility.wlan_mlo_throughput_to_wan).strip()
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print(f"ACTUAL RESULT {step}: Failed to perform TCP from WLAN to WAN.")
                                            print("[TEST EXECUTION RESULT] : FAILURE")

                                        step += 1
                                        #delete the added route
                                        print(f"\nTEST STEP {step}: Delete the static route added from wlan client to wan via gateway")
                                        print(f"EXPECTE RESULT {step}: Should delete the static route added from wlan client to wan via gateway successfully")
