...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_WithInvalidPort</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_630</test_case_id>
...
10. Send an outbound packet to trigger the port 12343 which is different from the port in PT rule with a TCP packet.
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_WithInvalidPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_WithInvalidPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_WithInvalidPort');
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    invalidPort = "12343"
                                    step = step + 1
                                    print("\nTEST STEP %d: Trigger with invalid Port %s through a specified outbound packet from LAN client" %(step, invalidPort))
                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                    status = triggerPort(tdkbE2EUtility.wan_ip, invalidPort, triggerProtocol, "LAN")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        step = step + 1
                                        print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the LAN client that triggered with invalid port" %step)
