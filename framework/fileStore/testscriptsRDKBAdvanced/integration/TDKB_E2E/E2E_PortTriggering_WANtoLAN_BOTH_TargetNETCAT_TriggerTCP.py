...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>E2E_PortTriggering_WANtoLAN_BOTH_TargetNETCAT_TriggerTCP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 54321 is forwarded to the LAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_610</test_case_id>
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoLAN_BOTH_TargetNETCAT_TriggerTCP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_BOTH_TargetNETCAT_TriggerTCP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_BOTH_TargetNETCAT_TriggerTCP');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                target_protocol = "TCP"
                                status = PTinitServer("NETCAT", lanIP, targetStart, target_protocol, tdkbE2EUtility.server_logfile, "LAN", message)
                                if expectedresult in status:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("EXPECTED RESULT %d: NETCAT client running in WAN machine should connect to the server running in LAN with the GW WAN IP of DUT" %step)
                                        #Since targetProtocol BOTH is not supported in server side, passing the target protocol TCP
