...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_PortTriggering_WANtoLAN_UDP_TargetIPERF_TriggerOutOfRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is not forwarded to the LAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule with protocol UDP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_622</test_case_id>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
10. Send an outbound packet to trigger the out of range port 12350 with UDP packet
11. From WAN client, send an inbound packet to the target port 5001 via the DUT GW WAN IP. The packet should not be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 5001 is not forwarded to the LAN machine (which is running
	 an IPERF server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule with UDP protocol.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_UDP_TargetIPERF_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetIPERF_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetIPERF_TriggerOutOfRange');
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("EXPECTED RESULT %d: IPERF client running in WAN machine should not connect to the server running in LAN with the GW WAN IP of DUT" %step)
