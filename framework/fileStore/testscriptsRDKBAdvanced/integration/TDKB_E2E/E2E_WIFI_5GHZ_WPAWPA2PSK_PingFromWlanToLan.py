...
<xml>
  <id/>
  <version>4</version>
  <name>E2E_WIFI_5GHZ_WPAWPA2PSK_PingFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ICMP Traffic should be successfull with destination as wired client and source as wireless client where client association in wpawpa2-psk security wlan on 5ghz band.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_157</test_case_id>
    <test_objective>Verify that ICMP Traffic should be successfull with destination as wired client and source as wireless client where client association in wpawpa2-psk security wlan on 5ghz band.</test_objective>
...
5. From the wlan client, do ping to the LAN client and check whether  ping is success when the security mode for 5GHz is set to wpawpa2-psk
6. Revert the security mode to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>From the wlan client, do ping to the LAN client and check whether ping is success when the security mode for 5GHz is set to wpawpa2-psk</except_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPAWPA2PSK_PingFromWlanToLan');
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            status = verifyNetworkConnectivity(lanIP, "PING", wlanIP, curIPAddress)
                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS");
