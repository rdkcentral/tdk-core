...
<xml>
  ...
  <name>E2E_Firewall_Low_SSHToWAN_FromLANAndWLAN</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Low SSH access from LAN to WAN and WLAN to WAN should  be success</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_471</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low SSH access from LAN to WAN and WLAN to WAN should  be success</test_objective>
...
3. Set the firewall to Low using tdkb_e2e_SetMultipleParams
...
8.Unload tdkb_e2e module</automation_approch>
    <except_output>The SSH to WAN from WLAN and LAN should be success when firewall is low</except_output>
    <priority>High</priority>
    <test_stub_interface>TDKB_E2E</test_stub_interface>
    <test_script>E2E_Firewall_Low_SSHToWAN_FromLANAndWLAN</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_SSHToWAN_FromLANAndWLAN');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status and expectedresult in retStatus:
            ...
            if tdkbE2EUtility.mlo_capability == "False":
                ...
                #Set the firewall level to Low using firewallSet function
                level = "Low"
                step = step + 1
