...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidNETCATPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 123 is forwarded to the LAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_643</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 123 is forwarded to the LAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
11. From WAN client, send an inbound packet to the target port 123 via the DUT GW WAN IP. The packet should NOT be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 123 is forwarded to the LAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidNETCATPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidNETCATPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidNETCATPort');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "12345"
                        triggerEnd = "12345"
                        targetStart = "54321"
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                #Start the NETCAT server in LAN
                                step = step + 1
                                #Custom Message to be sent from server to client
                                value = random.randint(0, 1000)
                                message = "Message from server " + str(value)
                                print("\nTEST STEP %d: Start the NETCAT server in LAN client at port %s" %(step, targetStart))
                                print("EXPECTED RESULT %d: NETCAT server should be started successfully in LAN client" %step)
                                status = PTinitServer("NETCAT", lanIP, targetStart, targetProtocol, tdkbE2EUtility.server_logfile, "LAN", message)
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        step = step + 1
                                        #invalid target NETCAT port
                                        invalidPort = "123"
                                        print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the LAN client that triggered it" %step)
                                        print("EXPECTED RESULT %d: NETCAT client runing in WAN machine should NOT connect to the server running in LAN with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
                                        status = PTClientRequest("NETCAT", tdkbE2EUtility.wan_ip, invalidPort, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");

                                        if "FAILURE" in status:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("ACTUAL RESULT %d: NETCAT from WAN to the LAN failed with the GW WAN IP of DUT" %step)
                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("ACTUAL RESULT %d: NETCAT from WAN to the LAN is success with the GW WAN IP of DUT" %step)
                                    ...
                                    status = PTServerClientPostRequisite("NETCAT", "LAN", "WAN")
