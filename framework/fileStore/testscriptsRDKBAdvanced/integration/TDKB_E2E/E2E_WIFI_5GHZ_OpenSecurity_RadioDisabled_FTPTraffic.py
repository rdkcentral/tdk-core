...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_OpenSecurity_RadioDisabled_FTPTraffic</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ftp traffic between WLan to LAN should be denied when wifi client assocation in open authentication for 5GHZ and Radio interface parameter for 5GHZ [Device.WiFi.Radio.2.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_385</test_case_id>
    <test_objective>Verify that ftp traffic between WLan to LAN should be denied when wifi client assocation in open authentication for 5GHZ and Radio interface parameter for 5GHZ [Device.WiFi.Radio.2.Enable] is disabled.</test_objective>
...
4. Disable radio and send ftp request to wifi client from lan client
4. Revert the values</automation_approch>
    <except_output>FTP request should fail since radio is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_OpenSecurity_RadioDisabled_FTPTraffic</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_OpenSecurity_RadioDisabled_FTPTraffic');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            print("EXPECTED RESULT 1: Should retrieve the current ssid,Mode,radioEnable and ssidEnable")
            ...
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 2: Set the ssid,,radioEnable and ssidEnable")
                ...
                if expectedresult in status and setValuesList == newValues:
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            #Disabling 5GHz radio interface of the WG
                                            setValuesList = ['false'];
                                            print("WIFI parameter values that are set: %s" %setValuesList)
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in status and setValuesList == newValues:
                                                    ...
                                                    #Send ftp request to WLAN client from LAN clinet
                                                    print("TEST STEP 12: Connect to LAN Client and send FTP Request to WLAN Client")
                                                    status = ftpToClient("WLAN",wlanIP);
                                                    print(status)
                                                    if "SUCCESS" not in status:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("FTP traffic not successful between wired and wireless clients")
                                                        finalStatus = "SUCCESS"
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("FTP traffic successful between wired and wireless clients")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("TEST STEP 11: Get the current radioEnable")
                                                    print("EXPECTED RESULT 11: Should retrieve the current radioEnable")
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current ssid,se,securityMode,radioEnable and ssidEnable")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,,securityMode,radioEnable and ssidEnable")
                    print("ACTUAL RESULT 3: %s" %newValues);
                    print("[TEST EXECUTION RESULT] : FAILURE");
            else:
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print("STEP 1: Get the current ssid,securityMode,radioEnable and ssidEnable")
