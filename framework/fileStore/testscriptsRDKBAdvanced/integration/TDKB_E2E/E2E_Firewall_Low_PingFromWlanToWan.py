...
<xml>
  ...
  <name>E2E_Firewall_Low_PingFromWlanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Low ICMP message from WLAN to WAN IP of gateway should pass through</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_71</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low ICMP message from WLAN to WAN IP of gateway should pass through</test_objective>
...
5. From the wlan client, do PING to the WAN client and check whether PING is success when the firewall level is set to Low
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>PING from WLAN to WAN should pass through</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_Low_PingFromWlanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_PingFromWlanToWan');
                    ...
                    if wlanIP:
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                step = step + 1
                                print(f"\nTEST STEP {step}: Check the Ping from WLAN to WAN")
                                status = verifyNetworkConnectivity(tdkbE2EUtility.wan_ping_ip, "PING", wlanIP, curIPAddress)

                                if expectedresult in status:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("SUCCESS: Ping from WLAN to WAN is success")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print(f"\nTEST STEP {step}: failed, Ping from WLAN to WAN is blocked")
