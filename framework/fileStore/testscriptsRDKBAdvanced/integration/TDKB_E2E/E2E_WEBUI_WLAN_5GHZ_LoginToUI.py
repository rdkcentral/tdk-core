...
<xml>
  ...
  <name>E2E_WEBUI_WLAN_5GHZ_LoginToUI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if we can login to the UI page of gateway from the WLAN Client after connecting 5GHZ SSID and to logout from the UI</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if we can login to the UI page of gateway from the WLAN Client after connecting 5GHZ SSID and to logout from the UI</test_objective>
...
11. unload module</automation_approch>
    <except_output>The UI page of gateway should be accessible from WLAN cleint and should be able to login to it successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WEBUI_WLAN_5GHZ_LoginToUI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_WLAN_5GHZ_LoginToUI');
                                        ...
                                        if status == "SUCCESS":
                                            try:
                                                ...
                                                if checklogout == "Gateway > Login":
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("Successfully logged out from the page")
                                                    finalStatus = "SUCCESS"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("Failed to logout from the page")
