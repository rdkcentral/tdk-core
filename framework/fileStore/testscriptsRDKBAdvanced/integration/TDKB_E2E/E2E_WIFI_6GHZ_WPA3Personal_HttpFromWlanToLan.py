...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_HttpFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that http traffic between WLAN to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6GHz band.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_670</test_case_id>
    <test_objective>Verify that http traffic between WLAN to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6GHz band.</test_objective>
...
6. From the wlan client, try to connect to the lan client via http and check whether http connection is success when the security mode for 6GHz is set to WPA-Personal
7. Unload tdkb_e2e module</automation_approch>
    <expected_output>Http connection from WLAN client to LAN client should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_HttpFromWlanToLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_HttpFromWlanToLan')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_HttpFromWlanToLan')
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if lanIP:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                #Ping LAN client from WLAN client
                                                step += 1
                                                print(f"TEST STEP {step}: Check the HTTP connectivity from WLAN to LAN")
                                                print(f"EXPECTED RESULT {step}: WLAN client should reach LAN client via HTTP")
                                                status = tdkbE2EUtility.verifyNetworkConnectivity(lanIP, "WGET_HTTP", wlanIP, curIPAddress)
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
