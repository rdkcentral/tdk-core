...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_Firewall_High_HttpsFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Firewall Config is set to High HTTPs traffic from LAN to LAN should pass through</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_53</test_case_id>
    <test_objective>Verify that when Firewall Config is set to High HTTPs traffic from LAN to LAN should pass through</test_objective>
...
5. From the wlan client, do HTTPS to the LAN client and check whether HTTPS is success when the firewall level is set to High
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>From the wlan client, do HTTPS to the LAN client and check whether HTTPS is success when the firewall level is set to High</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_High_HttpsFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_High_HttpsFromWlanToLan');
                            ...
                            if expectedresult in status:
                                ...
                                if wlanIP:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        step = step + 1
                                        print(f"\nTEST STEP {step}: Check the HTTPS from WLAN to LAN")
                                        ...
                                        if expectedresult in status:
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print(f"\nTEST STEP {step}: HTTPS from WLAN to LAN failed")
