...
<xml>
  <id/>
  <version>3</version>
  <name>E2E_WEBUI_LAN_SetFirewallLevel</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the firewall level and set it to a different value. Check if the change is reflecting in the UI page.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>To get the firewall level and set it to a different value. Check if the change is reflecting in the UI page.</test_objective>
...
12. unload module</automation_approch>
    ...
    <test_script>E2E_WEBUI_LAN_SetFirewallLevel</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_LAN_SetFirewallLevel');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            firewallParam = "%s|%s|string" %(firewall,orgValue[0])
            tdkTestObj,firewallResult,details = setMultipleParameterValues(obj,firewallParam)
            if expectedresult in firewallResult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 7: Should set the original firewall level");
