...
<xml>
  ...
  <name>E2E_XDNS_DeleteRule_ConnectThroughDefault</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>When Mac entry rule is deleted. - policy for that client should go through default values</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_427</test_case_id>
    <test_objective>When Mac entry rule is deleted. - policy for that client should go through default values</test_objective>
...
5. Add new rule in this table with LAN client MAC, level 1 DNS ipv4 and ipv6 ips and a tag name for rule
6. If the default DNS ips are empty, set dns ip values
7. Enable Device.DeviceInfo.X_RDKCENTRAL-COM_EnableXDNS
8. Now delete this added rule
9. From LAN client do a nslookup to any level1 site via default dns server
10. Nslookup should resolve successfully
11. Revert the value of  Device.DeviceInfo.X_RDKCENTRAL-COM_EnableXDNS
12. Delete the added Device.X_RDKCENTRAL-COM_XDNS.DNSMappingTable. table if the previous delete was not done
13. Unload tdkb_e2e module</automation_approch>
    <except_output>Nslookup should resolve level1 site successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_XDNS_DeleteRule_ConnectThroughDefault</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_XDNS_DeleteRule_ConnectThroughDefault');
obj1.configureTestCase(ip,port,'E2E_XDNS_DeleteRule_ConnectThroughDefault');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if (instance > 0):
                                ...
                                setValuesList = [lanMAC,tdkbE2EUtility.xdns_level1_dns_server,tdkbE2EUtility.xdns_level1_ipv6_dns_server,"Default"];
                                print("Parameter values that are set: %s" %setValuesList)

                                list1 = [macAddress,lanMAC,'string']
                                list2 = [dnsipv4,tdkbE2EUtility.xdns_level1_dns_server,'string']
                                    ...
                                    if expectedresult in actualresult1:
                                        ...
                                        if expectedresult in status and setValuesList == newValues:
                                            ...
                                            if expectedresult in actualresult:
                                                #Set the result status of execution
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("TEST STEP 9: Deleting the added rule");
                                                print("EXPECTED RESULT 9: Should delete the added rule");
                                                ...
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("TEST STEP 10:Check if the level1 site accessible from the client device using default DNS")
                                                    print("EXPECTED RESULT 10:The level1 site should be accessible from the client device")
                                                    print("[TEST EXECUTION RESULT] : SUCCESS");
                                                    finalStatus = "SUCCESS"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("TEST STEP 10:Check if the level1 site accessible from the client device using default DNS")
                                                    print("EXPECTED RESULT 10:The level1 site is not accessible from the client device")
                                ...
                                #Check if the added rule is deleted or not in the previous step
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("The added rule has been deleted already\n")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Adding new rule");
                            print("EXPECTED RESULT 5: Should add new rule");
