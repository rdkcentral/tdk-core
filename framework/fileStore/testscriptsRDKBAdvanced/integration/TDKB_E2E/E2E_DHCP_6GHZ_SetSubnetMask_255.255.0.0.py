...
<xml>
  ...
  <name>E2E_DHCP_6GHZ_SetSubnetMask_255.255.0.0</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that on configuring DHCP Server Subnet mask to 255.255.0.0, it gets reflected successfully on the Wi-Fi Client connected via 6GHz radios</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_675</test_case_id>
    <test_objective>Verify that on configuring DHCP Server Subnet mask to 255.255.0.0, it gets reflected successfully on the Wi-Fi Client connected via 6GHz radios</test_objective>
...
3. Set LanIP,SubnetMask to 255.255.0.0,Maximum and minimum Address range using tdkb_e2e_SetMultipleParams
...
6. Unload tdkb_e2e module</automation_approch>
    <expected_output>The sub net mask obtained should be 255.255.0.0 in connected wifi client
</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_6GHZ_SetSubnetMask_255.255.0.0</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,"E2E_DHCP_6GHZ_SetSubnetMask_255.255.0.0")
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_6ghz_name,tdkbE2EUtility.ssid_6ghz_pwd,"10.0.0.1","255.255.0.0","10.0.0.2","10.0.0.253"]
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status:
                        ...
                        if wlanSubnetMask:
                            ...
                            # Check whether subnet mask is 255.255.0.0
                            print(f"TEST STEP {step}: Check whether subnet mask is 255.255.0.0")
                            print(f"EXPECTED RESULT {step}: Subnet mask should be 255.255.0.0")
                            if "255.255.0.0" in wlanSubnetMask:
                                status = "SUCCESS"
                            else:
                                status = "FAILURE"
                            if expectedresult in status:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: Subnet mask changed to 255.255.0.0")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                step += 1
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: Subnet mask is not 255.255.0.0")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
