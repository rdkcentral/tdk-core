...
def getDeviceConfigValue (configKey):
    try:
        ...
        if (len (deviceConfigFile) != 0):
            ...
            if configKey == "NATIVE_PLAYBACK_FASTFORWARD_3x_4x_ENABLED":
                try:
                ...
                configValue = configParser.get('device.config', configKey)
            try:
                use_aamp_for_hls = configParser.get('device.config',"NATIVE_PLAYBACK_USE_AAMP_FOR_HLS")
            except:
                use_aamp_for_hls = "no"
            try:
                use_aamp_for_dash = configParser.get('device.config',"NATIVE_PLAYBACK_USE_AAMP_FOR_DASH")
            except:
                use_aamp_for_dash = "no"
            try:
                check_pts = configParser.get('device.config',"NATIVE_PLAYBACK_CHECK_PTS")
            except:
                check_pts = "yes"
            try:
                check_fps = configParser.get('device.config',"NATIVE_PLAYBACK_CHECK_FPS")
            except:
                check_fps = "yes"
            try:
                use_audioSink = configParser.get('device.config',"NATIVE_PLAYBACK_USE_AUDIO_SINK")
            except:
                use_audioSink = ""
            try:
                use_autoVideoSink_for_fpsdisplaysink = configParser.get('device.config',"NATIVE_PLAYBACK_USE_AUTOVIDEO_FOR_FPSDISPLAYSINK")
            except:
                use_autoVideoSink_for_fpsdisplaysink = "no"
            try:
                check_audio_fps = configParser.get('device.config',"NATIVE_PLAYBACK_CHECK_AUDIO")
            except:
                check_audio_fps = "no"
            try:
                ...
                test_streams_base_path = ""
            try:
                validateFullPlayback = configParser.get('device.config',"NATIVE_PLAYBACK_MILLISEC_VALIDATION")
            except:
                validateFullPlayback = "no"
            try:
                use_appsrc = configParser.get('device.config',"NATIVE_PLAYBACK_USE_APPSRC")
            except:
                use_appsrc = "no"
            try:
                checkAVStatus = configParser.get('device.config',"NATIVE_PLAYBACK_CHECK_AV_STATUS")
            except:
                checkAVStatus = "no"
            try:
                start_westeros = configParser.get('device.config',"NATIVE_PLAYBACK_START_WESTEROS")
            except:
                start_westeros = "no"
            try:
                create_display = configParser.get('device.config',"NATIVE_PLAYBACK_CREATE_DISPLAY")
            except:
                create_display = "no"
        else:
...
def checkifCodecPlayed(codec):
    ...
    if "e-ac-3" in  codec:
        codec_search = r'"e-ac-3\|e-ac3\|eac-3"'
    else:
        codec_search = codec
