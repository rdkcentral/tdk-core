...
def stop_nginx():
    ...
    try:
        ...
        #command = "ssh root@{NGINX_CONTAINER_IP} /nginx_delay_build/sbin/nginx -s stop"
...
def check_and_start_nginx():
    ...
    try:
        ...
        #check_command = "ssh root@{NGINX_CONTAINER_IP} ps -ax | grep '[n]ginx'"
...
def restart_nginx():
    ...
    try:
        ...
        #stop_command = "ssh root@{NGINX_CONTAINER_IP} pkill -9 nginx"
        subprocess.call(stop_command, shell=True)
        #sleep(2)  # Give it some time to terminate processes
        time.sleep(2)
        print("\n Restarting Nginx...")
        start_command = "ssh root@{} /nginx_delay_build/sbin/nginx".format(NGINX_CONTAINER_IP)
        #start_command = "ssh root@{NGINX_CONTAINER_IP} /nginx_delay_build/sbin/nginx"
        subprocess.call(start_command, shell=True)
...
def generate_nginx_config(template_path, local_destination, remote_destination, nginx_container_ip, chunk_pattern, delay_value):
    ...
    try:
        ...
        #print "Updated Nginx configuration saved to: {local_destination}"
        print("Updated Nginx configuration saved to: {}".format(local_destination))
        # Copy the file to the Nginx container using scp
        #scp_command = "scp {local_destination} root@{nginx_container_ip}:{remote_destination}"
        ...
        #print "Configuration file successfully copied to Nginx container at {remote_destination}"
        print("Configuration file successfully copied to Nginx container at {}".format(remote_destination))
    except Exception as e:
        #print "Error updating or copying Nginx configuration: {e}"
