...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_NPVS_EOS_OPUS_Only_Audio');
...
if "SUCCESS" in result.upper():
    ...
    #Retrieve the value of configuration parameter 'NATIVE_PLAYBACK_EOS_TIMEOUT' that specifies the EOS timeout in seconds
    #The default timeout of test application is 6 minutes, so for EOS test cases, if the test stream is longer than 6 minutes the total timeout in seconds ( stream duration + ~5 seconds as buffer) should be explicitly specified in configuration parameter 'NATIVE_PLAYBACK_EOS_TIMEOUT' inside device config file
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","NATIVE_PLAYBACK_EOS_TIMEOUT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    timeoutConfigValue = tdkTestObj.getResultDetails();

    #If the value of NATIVE_PLAYBACK_EOS_TIMEOUT is retrieved correctly and its value is not empty, its value should be passed as the timeout argument to the test application
