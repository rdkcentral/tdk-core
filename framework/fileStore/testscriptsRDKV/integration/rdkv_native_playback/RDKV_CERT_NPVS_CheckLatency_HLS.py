...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_NPVS_CheckLatency_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Retrieve the value of configuration parameter 'NATIVE_PLAYBACK_LATENCY_THRESHOLD' that specifies the  latency threshold in milliseconds
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","NATIVE_PLAYBACK_LATENCY_THRESHOLD")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    latencyThresholdValueConfigValue = tdkTestObj.getResultDetails();

    #If the value of NATIVE_PLAYBACK_LATENCY_THRESHOLD is retrieved correctly and its value is not empty, latency threshold value should be passed to the test application
