...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_NPVS_PlayPause_ExtendedDuration_HEVC_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Retrieve the value of configuration parameter 'NATIVE_PLAYBACK_EXTENDEDDURATION_TIMEOUT' that specifies the video playback timeout in seconds
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","NATIVE_PLAYBACK_EXTENDEDDURATION_TIMEOUT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    timeoutConfigValue = tdkTestObj.getResultDetails();

    #If the value of NATIVE_PLAYBACK_EXTENDEDDURATION_TIMEOUT is retrieved correctly and its value is not empty, timeout value should be passed to the test application
