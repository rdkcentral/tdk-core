...
<xml>
  ...
  <name>FNCS_Playback_Audio_Change_OPUS_Vorbis</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_122</test_case_id>
...
2. Test stream url for a multi codec OPUS Vorbis stream should be updated in the config variable video_src_url_opus_vorbis inside MediaValidationVariables.py library inside filestore.
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Change_OPUS_Vorbis</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Change_OPUS_Vorbis');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_change <OPUS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            tdkTestObj = obj.createTestStep('checkifCodecPlayed')
            tdkTestObj.addParameter("codec","opus")
            tdkTestObj.executeTestCase(expectedResult);
            result = tdkTestObj.getResultDetails();
            if expectedResult in result:
                tdkTestObj.setResultStatus("SUCCESS")    
                tdkTestObj = obj.createTestStep('checkifCodecPlayed')
                tdkTestObj.addParameter("codec","vorbis")
                tdkTestObj.executeTestCase(expectedResult);
                result = tdkTestObj.getResultDetails();
                if expectedResult in result:
                    tdkTestObj.setResultStatus("SUCCESS")
