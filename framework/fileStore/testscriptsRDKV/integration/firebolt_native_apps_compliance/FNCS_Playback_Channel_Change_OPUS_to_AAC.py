...
<xml>
  ...
  <name>FNCS_Playback_Channel_Change_OPUS_to_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from a OPUS stream to AAC stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_190</test_case_id>
    <test_objective>Test to do Channel Change from a OPUS stream to AAC stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for a OPUS stream and AAC stream should be updated in the config variable video_src_url_opus and video_src_url_aac respectively inside MediaValidationVariables.py library inside filestore
...
test_url - - SD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus), HD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_aac)
...
3.Retrieve the video_src_url_opus and video_src_url_aac  variable from MediaValidationVariables library
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>FNCS_PLAYBACK_Channel_Change_OPUS_to_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Channel_Change_OPUS_to_AAC');
...
if "SUCCESS" in result.upper():
    ...
    test_url = test_url + " " + MediaValidationVariables.video_src_url_aac
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <Channel_Change_OPUS_to_AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel_Change_OPUS_to_AAC Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel_Change_OPUS_to_AAC Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
