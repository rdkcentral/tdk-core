...
<xml>
  ...
  <name>FNCS_Playback_SeekForward_FF2x_H264_24Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek operation followed by fast forward of a 24Fps H264 stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_252</test_case_id>
    <test_objective>Test to do forward seek operation followed by fast forward of a 24Fps H264 stream with playback rate 2</test_objective>
...
2. Test stream url for a 24Fps H264 stream should be updated in the config variable video_src_url_dash_h264_24fps inside MediaValidationVariables.py library inside filestore
...
5. FIREBOLT_COMPLIANCE_SEEK_POSITION configuration should be set to duration in seconds to which the seek operation should be carried out
6. FIREBOLT_COMPLIANCE_VALIDATE_FULL_PLAYBACK configuration should be set as yes/no in the device config file for advanced playback validation
7. FIREBOLT_COMPLIANCE_CHECK_PTS configuration should be set as yes/no in the device config file for validating video pts
8. FIREBOLT_COMPLIANCE_CHECK_FPS configuration should be set as yes/no in the device config file for validating video frames
9. FIREBOLT_COMPLIANCE_CHECK_AUDIO configuration should be set as yes/no in the device config file for validating audio frames</pre_requisite>
    <api_or_interface_used>gstreamer-1.0</api_or_interface_used>
    <input_parameters>testcasename - "test_trickplay"
test_url -  24Fps H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_24fps)
...
operations=seek:&lt;timeout&gt;:&lt;seekposition&gt;,fastforward2x:&lt;timeout&gt; - a comma seperated string of individual seek and fastforward operations.The seek operation is a ":" seperated string to specify the seek operation to be executed , the time in seconds for which the operation should be performed and seekposition in seconds to which the seek operation should be performed. The fastforward operation is a ":" seperated string to specify the fastforwad operation to be executed and the time in seconds for which the operation should be continued. The timeout should be configured in the device configuration(FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT) from Device Config file. The seekposition should also be configured in device configuration(FIREBOLT_COMPLIANCE_SEEK_POSITION)
...
2.Retrieve the FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_SEEK_POSITION, FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT, FIREBOLT_COMPLIANCE_VALIDATE_FULL_PLAYBACK,FIREBOLT_COMPLIANCE_CHECK_PTS,FIREBOLT_COMPLIANCE_CHECK_FPS, FIREBOLT_COMPLIANCE_CHECK_AUDIO config values from Device config file.
3.Retrieve the video_src_url_dash_h264_24fps variable from MediaValidationVariables library
4. Construct the mediapipelinetests command based on the retrieved video url, testcasename, FIREBOLT_COMPLIANCE_CHECK_AV_STATUS deviceconfig value, operations, seek position and timeout
5.Execute the command in DUT. During the execution, the DUT will start av playback, the pipeline then seeks to FIREBOLT_COMPLIANCE_SEEK_POSITION and then av playback is performed for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds followed by a fast forward operation with playback rate 2 and then continue playback  at rate 2 for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds. Then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_SeekForward_FF2x_H264_24Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_SeekForward_FF2x_H264_24Fps');

#Set device configurations to default values
checkAVStatus = "no"
timeoutInSeconds = "10"
seekPositionInSeconds = "20"
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_dash_h264_24fps
    ...
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_SEEK_POSITION")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    seek_position = tdkTestObj.getResultDetails();
    ...
    #The operations specifies the operations(fastforward/rewind/seek/play/pause) to be executed from the mediapipeline trickplay test
    #The seek operation should be performed first followed by the fastforward
    #TODO: the seek operation should be done to the seek position relative to current position rather than to absolute seek position
    # Sample operations strings is "operations=seek:20:0,fastforward2x:20"
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_24Fps_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward Seek followed by Fastforward on 24Fps H264 stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward Seek followed by Fastforward on 24Fps H264 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
