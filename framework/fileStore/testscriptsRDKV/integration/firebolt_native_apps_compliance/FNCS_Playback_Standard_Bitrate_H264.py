...
<xml>
  ...
  <name>FNCS_Playback_Standard_Bitrate_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether the video is playing with Standard bitrate using H264 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_183</test_case_id>
    <test_objective>To verify whether the video is playing with Loe bitrate using H264 stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Standard_Bitrate_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Standard_Bitrate_H264');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Standard bitrate video playback using H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Standard bitrate video playback using H264 stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
