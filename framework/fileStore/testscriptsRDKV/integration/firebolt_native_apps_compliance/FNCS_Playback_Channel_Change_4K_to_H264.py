...
<xml>
  ...
  <name>FNCS_Playback_Channel_Change_4K_to_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from a 4K HLS stream to H264 DASH stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_196</test_case_id>
    <test_objective>Test to do Channel Change from a 4K HLS stream to H264 DASH stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for a H264 DASH stream and 4K HLS stream should be updated in the config variable video_src_url_dash_h264 and video_src_url_4k_hls respectively inside MediaValidationVariables.py library inside filestore
...
test_url - - SD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264), HD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_hls)
...
3.Retrieve the video_src_url_dash_h264 and video_src_url_4k_hls  variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will play with stream with HD content for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to stream with SD content FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>FNCS_PLAYBACK_Channel_Change_4K_to_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Channel_Change_4K_to_H264');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_4k_hls
    #Setting URL for Channel 2
    test_url = test_url + " " + MediaValidationVariables.video_src_url_dash_h264
    ...
    #Retrieve the value of configuration parameter 'FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT' that specifies the video playback timeout in seconds
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    secondChanneltimeoutConfigValue = tdkTestObj.getResultDetails();
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <Channel_Change_4K_to_H264_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel_Change_4K_to_H264 Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel_Change_4K_to_H264 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
