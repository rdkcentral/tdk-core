...
<xml>
  ...
  <name>FNCS_Playback_Appsrc_Video_Underflow_Signal_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_247</test_case_id>
...
2. Test stream url for an Appsrc_Video_Underflow_Signal_HEVC stream should be updated in the config variable video_src_url_hevc_30fps inside MediaValidationVariables.py library inside filestore
...
test_url - Appsrc_Video_Underflow_Signal_HEVC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_h264_30fps)
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Appsrc_Video_Underflow_Signal_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Appsrc_Video_Underflow_Signal_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <Appsrc_Video_Underflow_Signal_HEVC_STREAM_URL> checkavstatus=yes timeout=20"
