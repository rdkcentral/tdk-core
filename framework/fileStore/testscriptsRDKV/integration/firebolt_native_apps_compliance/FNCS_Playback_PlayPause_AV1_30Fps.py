...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_AV1_30Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify play pause for AV1 30 fps video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_325</test_case_id>
    <test_objective>Verify play pause for AV1 30 fps video</test_objective>
...
2. Test stream url for an DASH stream should be updated in the config variable video_src_url_av1_30fps inside MediaValidationVariables.py library inside filestore
...
test_url - DASH url from MediaValidationVariables library (MediaValidationVariables.video_src_url_av1_30fps)
...
3.Retrieve the video_src_url_av1_30fps variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_AV1_30Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_AV1_30Fps');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_av1_30fps
