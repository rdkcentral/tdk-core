...
<xml>
  ...
  <name>FNCS_Playback_8_Bit_Color_depth_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether the video is playing with 8 color bit depth</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_133</test_case_id>
    <test_objective>To verify whether the video is playing with 8 color bit depth</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_8_Bit_Color_depth_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_8_Bit_Color_depth_Test');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_color_depth <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("8 Bit color depth video playback was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("8 Bit color depth video playback was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
