...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of Dolby stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_06</test_case_id>
    <test_objective>To test the video playback of Dolby stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an Dolby stream should be updated in the config variable video_src_url_dolby inside MediaValidationVariables.py library inside filestore
...
test_url - Dolby url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dolby)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Dolby</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Dolby');
