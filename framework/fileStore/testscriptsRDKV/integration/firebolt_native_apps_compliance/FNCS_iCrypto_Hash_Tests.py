...
<xml>
  ...
  <name>FNCS_iCrypto_Hash_Tests</name>
  ...
  <test_cases>
    <test_case_id>FNCS_ICRYPTO_03</test_case_id>
    ...
    <automation_approch>1.Execute the Hash Interface tests in DUT. During the execution, the DUT will execute all the tests available in iCrypto interface tests suite.
2.During the execution, the DUT will test the Hash Ingest and Calculate mechanism. Raw data and the hash_sha256 for the data are present as a pre-requisite in the application. Raw Data is encrypted using hash->Ingest. The output of Calculate mechanism calculates the digest/hmac value of the digested data. The output of Calculate is verifed against the hash_sha256 present as pre-requisite in the app.
3.During the Hash-HMAC , the above operation is done with hash object obtained using the HMAC which is obtained using a secret keyId.
4.Verify the output from the execute command and check if the string "0 FAILED" exists in the returned output
5.Based on the ExecuteCommand() return value and the output returned from the Hash tests of "cgfacetests" application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output of Hash Tests returned from cgfacetests contains the strings "TOTAL:" and "0 FAILED"</expected_output>
    ...
    <test_script>FNCS_iCrypto_Hash_Tests</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_iCrypto_Hash_Tests');
...
if "SUCCESS" in result.upper():
    ...
    testModule = "Hash"
    #Define Module Tests
    Tests = {"Hash" : [" Hash::Hash", " Hash::HMAC"]}
    ...
    for line in test_result_string:
        if "0 FAILED" not in line and not failed:
            print ("FAILURE : Observed failures in Hash test execution");
            tdkTestObj.setResultStatus("FAILURE")
    if not failed:
        print ("SUCCESS : Hash test execution was successfull, no failures observed")
