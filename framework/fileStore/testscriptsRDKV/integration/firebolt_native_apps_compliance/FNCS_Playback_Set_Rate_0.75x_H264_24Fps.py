...
<xml>
  ...
  <name>FNCS_Playback_Set_Rate_0.75x_H264_24Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a 24 Fps H264 stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_250</test_case_id>
    <test_objective>Test to do set rate operation of a 24 Fps H264 stream with playback rate 0.75</test_objective>
...
2. Test stream url for a H264_24Fps stream should be updated in the config variable video_src_url_dash_h264_24fps inside MediaValidationVariables.py library inside filestore
3. FIREBOLT_COMPLIANCE_CHECK_AV_STATUS configuration should be set as yes/no in the device config file
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time in seconds for which the slowMotion operation should be carried out
...
test_url - H264_24Fps url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_24fps)
...
operations=slowMotion0.75x:&lt;timeout&gt; - a ":" seperated string to specify the slowMotion operation to be executed and the time in seconds for which the operation should be continued. The timeout should be configured in the device configuration(FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT) from Device Config file
...
2.Retrieve the FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT, FIREBOLT_COMPLIANCE_VALIDATE_FULL_PLAYBACK,FIREBOLT_COMPLIANCE_CHECK_PTS,FIREBOLT_COMPLIANCE_CHECK_FPS, FIREBOLT_COMPLIANCE_CHECK_AUDIO config values from Device config file.
3.Retrieve the video_src_url_dash_h264_24fps variable from MediaValidationVariables library
4.Construct the tdk_mediapipelinetests_trickplay command based on the retrieved video url, testcasename and configurations
...
7.Based on the ExecuteCommand() return value and the output returned from the tdk_mediapipelinetests_trickplay application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Set_Rate_0.75x_H264_24Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Set_Rate_0.75x_H264_24Fps');
...
if "SUCCESS" in result.upper():
    ...
    # Sample operations strings is "operations=slowMotion0.75x:20"
    tdkTestObj = obj.createTestStep('setOperations')
    tdkTestObj.addParameter("operation","slowMotion0.75x")
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_24Fps_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("24 Fps H264 Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("24 Fps H264 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
