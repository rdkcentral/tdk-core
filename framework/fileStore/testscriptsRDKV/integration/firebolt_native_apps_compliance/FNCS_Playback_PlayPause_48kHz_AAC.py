...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_48kHz_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether 48Khz audio is being rendered properly using AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_168</test_case_id>
    <test_objective>to verify whether 48Khz audio is being rendered properly using AAC stream</test_objective>
...
2. Test stream url for an AAC stream should be updated in the config variable video_src_url_48khz_aac inside MediaValidationVariables.py library inside filestore
...
test_url - AAC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_48khz_aac)
...
3.Retrieve the video_src_url_48khz_aac variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_48kHz_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_48kHz_AAC');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_48khz_aac
    ...
    #Sample command = "mediapipelinetests test_audio_sampling_rate <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("48kHz sampling rate audio playback using AAC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("48kHz sampling rate audio playback using AAC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
