...
<xml>
  ...
  <name>FNCS_Playback_Set_Rate_0.75x_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a MKV stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_306</test_case_id>
    <test_objective>Test to do set rate operation of a MKV stream with playback rate 0.75</test_objective>
...
2. Test stream url for a MKV stream should be updated in the config variable video_src_url_4k_av1_mkv inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Set_Rate_0.75x_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Set_Rate_0.75x_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <MKV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("MKV Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("MKV Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
