...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Backward_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a OPUS stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_57</test_case_id>
    <test_objective>Test to do backward seek on a OPUS stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Backward_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Backward_OPUS');
   ...
   #Construct the trickplay operation string
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on OPUS stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward seek on OPUS stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
