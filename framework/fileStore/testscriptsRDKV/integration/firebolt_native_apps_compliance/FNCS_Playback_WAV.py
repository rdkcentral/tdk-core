...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of WAV stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_226</test_case_id>
    <test_objective>To test the video playback of WAV stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_WAV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_WAV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_only_audio <WAV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("WAV Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("WAV Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
