...
<xml>
  ...
  <name>FNCS_Playback_SeekForward_FF2x_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek operation followed by fast forward of a 25Fps H264 stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_212</test_case_id>
    <test_objective>Test to do forward seek operation followed by fast forward of a 25Fps H264 stream with playback rate 2</test_objective>
...
2. Test stream url for a 25Fps H264 stream should be updated in the config variable video_src_url_dash_h264_25fps inside MediaValidationVariables.py library inside filestore
...
test_url -  25Fps H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_25fps)
...
2.Retrieve the FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_SEEK_POSITION and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT config values from Device config file.
3.Retrieve the video_src_url_dash_h264_25fps variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_SeekForward_FF2x_H264_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_SeekForward_FF2x_H264_25Fps');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_dash_h264_25fps
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_25Fps_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward Seek followed by Fastforward on 25Fps H264 stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward Seek followed by Fastforward on 25Fps H264 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
