...
<xml>
  ...
  <name>FNCS_Playback_Audio_Underflow_Signal_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test if buffer underflow callback is received upon audio underrun from westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_145</test_case_id>
...
5. FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT configuration should be set to notify the point where audio underrun happens </pre_requisite>
...
5.Execute the command in DUT. During the execution, the DUT will playback av for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds and waits for few seconds after FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT upon which buffer underflow signal must be received.
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>buffer underflow signal must be received upon audio underrun</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdk_mediapipelinetests</test_stub_interface>
    <test_script>FNCS_Playback_Audio_Underflow_Signal_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Underflow_Signal_Test');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_audio_underflow_signal"
    ...
    #Sample command = "mediapipelinetests test_audio_underflow_signal <AAC_STREAM_URL> checkavstatus=yes timeout=20"
