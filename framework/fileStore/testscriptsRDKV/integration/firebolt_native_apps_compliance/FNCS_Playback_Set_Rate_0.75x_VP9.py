...
<xml>
  ...
  <name>FNCS_Playback_Set_Rate_0.75x_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a VP9 stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_310</test_case_id>
    <test_objective>Test to do set rate operation of a VP9 stream with playback rate 0.75</test_objective>
...
2. Test stream url for a VP9 stream should be updated in the config variable video_src_url_4k_vp9  inside MediaValidationVariables.py library inside filestore
...
test_url - VP9 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_vp9 )
...
3.Retrieve the video_src_url_4k_vp9  variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Set_Rate_0.75x_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Set_Rate_0.75x_VP9');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_4k_vp9 
    ...
    #Sample command = "mediapipelinetests test_trickplay <VP9_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("VP9 Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("VP9 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
