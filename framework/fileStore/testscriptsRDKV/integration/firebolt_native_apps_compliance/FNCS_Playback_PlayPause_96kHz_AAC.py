...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_96kHz_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether 96Khz audio is being rendered properly using AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_170</test_case_id>
    <test_objective>to verify whether 96Khz audio is being rendered properly using AAC stream</test_objective>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_96kHz_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_96kHz_AAC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("96kHz sampling rate audio playback using AAC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("96kHz sampling rate audio playback using AAC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
