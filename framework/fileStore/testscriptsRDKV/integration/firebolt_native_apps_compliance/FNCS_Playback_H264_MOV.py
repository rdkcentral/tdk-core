...
<xml>
  ...
  <name>FNCS_Playback_H264_MOV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of H264 MOV stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_363</test_case_id>
    <test_objective>To test the video playback of H264 MOV stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an H264 MOV stream should be updated in the config variable video_src_url_h264_mov inside MediaValidationVariables.py library inside filestore
...
test_url - H264 MOV url from MediaValidationVariables library (MediaValidationVariables.video_src_url_h264_mov)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_H264_MOV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_H264_MOV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <H264_MOV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("H264 MOV Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("H264 MOV Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
