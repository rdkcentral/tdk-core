...
<xml>
  ...
  <name>FNCS_Playback_Audio_Change_OPUS_AAC</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_123</test_case_id>
...
2. Test stream url for a multi codec OPUS AAC stream should be updated in the config variable video_src_url_opus_aac inside MediaValidationVariables.py library inside filestore.
...
test_url - multi codec url with OPUS and AAC audio streams from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus_aac)
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Change_OPUS_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Change_OPUS_AAC');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_opus_aac
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("OPUS AAC codec switch was successfull")
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print("OPUS AAC codec switch failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
