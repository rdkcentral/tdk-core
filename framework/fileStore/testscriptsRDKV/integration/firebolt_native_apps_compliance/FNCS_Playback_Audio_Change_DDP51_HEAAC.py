...
<xml>
  ...
  <name>FNCS_Playback_Audio_Change_DDP51_HEAAC</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_126</test_case_id>
...
2. Test stream url for a multi codec OPUS EAC3 stream should be updated in the config variable video_src_url_ddp51_heaac inside MediaValidationVariables.py library inside filestore.
...
test_url - multi codec url with OPUS and EAC3 audio streams from MediaValidationVariables library (MediaValidationVariables.video_src_url_ddp51_heaac)
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Change_DDP51_HEAAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Change_DDP51_HEAAC');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_ddp51_heaac
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("DDP51 HEAAC codec switch was successfull")
            print("Mediapipeline test executed successfully")

            tdkTestObj = obj.createTestStep('checkifCodecPlayed')
            tdkTestObj.addParameter("codec","aac")
            tdkTestObj.executeTestCase(expectedResult);
            result = tdkTestObj.getResultDetails();
            if expectedResult in result:
                tdkTestObj.setResultStatus("SUCCESS")    
                tdkTestObj = obj.createTestStep('checkifCodecPlayed')
                tdkTestObj.addParameter("codec","e-ac-3")
                tdkTestObj.executeTestCase(expectedResult);
                result = tdkTestObj.getResultDetails();
                if expectedResult in result:
                    tdkTestObj.setResultStatus("SUCCESS")
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print("DDP51 HEAAC codec switch failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
