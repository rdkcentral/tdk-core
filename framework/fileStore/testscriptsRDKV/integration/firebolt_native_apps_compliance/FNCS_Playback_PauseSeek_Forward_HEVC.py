...
<xml>
  ...
  <name>FNCS_Playback_PauseSeek_Forward_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_166</test_case_id>
    <test_objective>Test to do forward seek with pause on a stream with HEVC video codec</test_objective>
...
2. Test stream url for a stream with HEVC video codec should be updated in the config variable video_src_url_hevc inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PauseSeek_Forward_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PauseSeek_Forward_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pause with Forward seek on HEVC stream was successfull")
