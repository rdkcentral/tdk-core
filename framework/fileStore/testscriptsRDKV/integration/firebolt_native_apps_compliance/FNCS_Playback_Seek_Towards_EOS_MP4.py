...
<xml>
  ...
  <name>FNCS_Playback_Seek_Towards_EOS_MP4</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_262</test_case_id>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Seek_Towards_EOS_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Seek_Towards_EOS_MP4');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_seek_EOS <MP4_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Seek operation towards EOS using MP4 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Seek operation towards EOS using MP4 stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
