...
<xml>
  ...
  <name>FNCS_Playback_Duration_Test_VP9_HDR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do retrieve playback duration of a VP9_HDR stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_375</test_case_id>
    <test_objective>Test to do retrieve playback duration of a VP9_HDR stream through playbin gst element and westerossink</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests application must be installed in DUT
2. Test stream url for an Duration_Test_VP9_HDR stream should be updated in the config variable video_src_url_vp9_hdr inside MediaValidationVariables.py library inside filestore
...
test_url - Duration_Test_VP9_HDR url from MediaValidationVariables library (MediaValidationVariables.video_src_url_vp9_hdr)
...
Checkpoint 3. Verify that duration is retrieved as expected</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Duration_Test_VP9_HDR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Duration_Test_VP9_HDR');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_duration <Duration_Test_VP9_HDR_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Duration_Test_VP9_HDR Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
