...
<xml>
  ...
  <name>FNCS_Playback_Set_Rate_0.75x_Only_Audio_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a Audio_only_AAC stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_279</test_case_id>
    <test_objective>Test to do set rate operation of a Audio_only_AAC stream with playback rate 0.75</test_objective>
...
2. Test stream url for a Audio_only_AAC stream should be updated in the config variable video_src_url_only_audio_aac inside MediaValidationVariables.py library inside filestore
...
test_url - Audio_only_AAC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_only_audio_aac)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Set_Rate_0.75x_Only_Audio_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Set_Rate_0.75x_Only_Audio_AAC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <Audio_only_AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Audio_only_AAC stream set rate 0.75 operation was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Audio_only_AAC stream set rate 0.75 operation was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
