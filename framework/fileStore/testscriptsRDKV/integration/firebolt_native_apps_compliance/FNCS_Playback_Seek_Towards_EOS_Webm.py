...
<xml>
  ...
  <name>FNCS_Playback_Seek_Towards_EOS_Webm</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do seek operation towards end of the video and verify the End Of Stream message detection </synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_264</test_case_id>
...
2. Test stream url for an Webm stream should be updated in the config variable video_src_url_direct_webm inside MediaValidationVariables.py library inside filestore
...
test_url - Webm url from MediaValidationVariables library (MediaValidationVariables.video_src_url_direct_webm)
...
3.Retrieve the video_src_url_direct_webm variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Seek_Towards_EOS_Webm</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Seek_Towards_EOS_Webm');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_direct_webm
    ...
    #Sample command = "mediapipelinetests test_seek_EOS <Webm_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Seek operation towards EOS using Webm stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Seek operation towards EOS using Webm stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
