...
<xml>
  ...
  <name>FNCS_Playback_Duration_Test_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do retrieve playback duration of a HEVC_MKV stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_353</test_case_id>
    <test_objective>Test to do retrieve playback duration of a HEVC_MKV stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an Duration_Test_HEVC_MKV stream should be updated in the config variable video_src_url_hevc_mkv inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_playback_duration"
test_url - Duration_Test_HEVC_MKV url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_mkv)
...
8.Verify duration is matched with the expected duration</automation_approch>
    <expected_output>1. Verify the API call is success
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"
Checkpoint 3. Verify that duration is retrieved as expected</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdk_mediapipelinetests</test_stub_interface>
    <test_script>FNCS_PLAYBACK_Duration_Test_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Duration_Test_HEVC_MKV');

#Set device configurations to default values
checkAVStatus = "no"
timeoutInSeconds = "10"
duration_to_be_verified = 9
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_playback_duration"
    ...
    #Sample command = "mediapipelinetests test_playback_duration <Duration_Test_HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    #Disable audio frames validation for 4k_MKV playback
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            tdkTestObj = obj.createTestStep('DurationParse')
            tdkTestObj.addParameter("output",output)
            tdkTestObj.addParameter("duration_to_be_verified",duration_to_be_verified)
            tdkTestObj.executeTestCase(expectedResult);
            executionStatus = tdkTestObj.getResultDetails()
            tdkTestObj.setResultStatus(executionStatus)

        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Duration_Test_HEVC_MKV Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
