...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 60 Fps stream through 'playbin', 'fpsdisplaysink' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_25</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for 60 Fps stream through 'playbin', 'fpsdisplaysink' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an DASH stream should be updated in the config variable video_src_url_mp4_60fps inside MediaValidationVariables.py library inside filestore
...
test_url - DASH url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_60fps)
...
3.Retrieve the video_src_url_mp4_60fps variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_60Fps');
