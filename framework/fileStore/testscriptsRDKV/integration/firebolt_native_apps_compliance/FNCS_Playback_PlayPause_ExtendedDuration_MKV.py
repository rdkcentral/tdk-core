...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_ExtendedDuration_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do extended duration play, pause operations on a MKV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_320</test_case_id>
    <test_objective>Test to do extended duration play, pause operations on a MKV stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PlayPause_ExtendedDuration_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_ExtendedDuration_MKV');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Extended duration play,pause operations on MKV stream was successfull")
