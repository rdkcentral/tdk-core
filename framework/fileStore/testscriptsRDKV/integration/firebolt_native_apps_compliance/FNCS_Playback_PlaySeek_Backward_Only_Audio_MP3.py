...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Backward_Only_Audio_MP3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a MP3 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_287</test_case_id>
    <test_objective>Test to do backward seek on a MP3 stream</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RDKTV, RPI</test_setup>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Backward_Only_Audio_MP3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Backward_Only_Audio_MP3');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on MP3 stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward seek on MP3 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
