...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_Rewind_2x_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do rewind of a EAC3 stream with playback rate -2 and verify latency</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_225</test_case_id>
    <test_objective>Test to do rewind of a EAC3 stream with playback rate -2 and verify latency</test_objective>
...
2. Test stream url for a EAC3 stream should be updated in the config variable video_src_url_ec3 inside MediaValidationVariables.py library inside filestore
...
test_url - EAC3 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ec3)
...
3.Retrieve the video_src_url_ec3 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_Rewind_2x_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_Rewind_2x_EAC3');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <EAC3_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Rewind on EAC3 stream with 2x speed was successfull")
            ...
            print("Rewind on EAC3 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
