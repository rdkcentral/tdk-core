...
<xml>
  ...
  <name>FNCS_Playback_Video_PTS_Sync_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the video pts of VP9 using 'last-sample' property is allinged with the video playback</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_346</test_case_id>
    <test_objective>To verify the video pts of VP9 using 'last-sample' property is allinged with the video playback</test_objective>
...
2. Test stream url for an VP9 stream should be updated in the config variable video_src_url_mp4_frameDrop_vp9 inside MediaValidationVariables.py library inside filestore
...
test_url - VP9 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_frameDrop_vp9)
...
3.Retrieve the video_src_url_mp4_frameDrop_vp9 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Video_PTS_Sync_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Video_PTS_Sync_VP9');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_frameDrop_vp9
    ...
    #Sample command = "tdk_mediapipelinetests test_video_PTS_sync <VP9_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Video PTS Sync validation with VP9 using 'last-sample' was Sucessfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Video PTS Sync validation with VP9 using 'last-sample' was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
