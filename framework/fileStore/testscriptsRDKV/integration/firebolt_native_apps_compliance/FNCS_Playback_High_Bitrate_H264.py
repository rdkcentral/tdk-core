...
<xml>
  ...
  <name>FNCS_Playback_High_Bitrate_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether the video is playing with High bitrate using H264 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_152</test_case_id>
    <test_objective>To verify whether the video is playing with High bitrate using H264 stream</test_objective>
...
2. Test stream url for an H264 stream should be updated in the config variable video_src_url_bitrate_h264 inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_High_Bitrate_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_High_Bitrate_H264');

#Set device configurations to default values
checkAVStatus = "no"
timeoutInSeconds = "10"
connection_speed = "15000"
height = "2160"
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj = obj.createTestStep('getDeviceDetails')
    tdkTestObj.addParameter("detail","deviceType")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    deviceType = tdkTestObj.getResultDetails();
    
    if "RPI" in deviceType:
    ...
    #Sample command = "mediapipelinetests test_video_bitrate <H264_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("High bitrate video playback was successfull using H264 stream")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("High bitrate video playback was failed using H264 stream")
    else:
        tdkTestObj.setResultStatus("FAILURE")
