...
<xml>
  ...
  <name>FNCS_Playback_PauseSeek_Forward_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek with pause on a stream with 25Fps H264 video codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_163</test_case_id>
    <test_objective>Test to do forward seek with pause on a stream with 25Fps H264 video codec</test_objective>
...
2. Test stream url for a stream with  25Fps H264 video codec should be updated in the config variable video_src_url_dash_h264_25fps inside MediaValidationVariables.py library inside filestore
...
2.test_url - 25Fps H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_25fps)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PauseSeek_Forward_H264_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PauseSeek_Forward_H264_25Fps');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pause with Forward seek on 25Fps H264 stream was successfull")
