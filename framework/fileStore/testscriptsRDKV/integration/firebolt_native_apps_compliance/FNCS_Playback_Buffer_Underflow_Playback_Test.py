...
<xml>
  ...
  <name>FNCS_Playback_Buffer_Underflow_Playback_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify buffer underflow signal is received upon video underrun and fill the video buffer by seeking to videoStart point and verify playback is as expected.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_147</test_case_id>
    <test_objective>Verify buffer underflow signal is received upon video underrun and fill the video buffer by seeking to videoStart point and verify playback is as expected.</test_objective>
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>pipeline should ply with normal playback rate  after seeking to videoStart point from videoEnd point where video underrun has occurred</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdk_mediapipelinetests</test_stub_interface>
    <test_script>FNCS_Playback_Buffer_Underflow_Playback_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Buffer_Underflow_Playback_Test');
...
if "SUCCESS" in result.upper():
    ...
    #Get videoStart point from config file
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_UNDERFLOW_VIDEO_START_POINT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    videoStart = tdkTestObj.getResultDetails();

    #Sample command = "mediapipelinetests test_buffer_underflow_playback <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    arguments = {"checkavstatus" : checkAVStatus, "timeout": timeoutInSeconds, "videoEnd": videoEnd, "videoStart": videoStart}
