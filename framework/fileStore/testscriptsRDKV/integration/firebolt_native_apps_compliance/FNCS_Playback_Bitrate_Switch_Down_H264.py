...
<xml>
  ...
  <name>FNCS_Playback_Bitrate_Switch_Down_H264</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_274</test_case_id>
...
2. Test stream url for an H264 stream should be downdated in the config variable video_src_url_bitrate_h264 inside MediaValidationVariables.py library inside filestore
...
test_url - h264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_bitrate_h264)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Bitrate_Switch_Down_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Bitrate_Switch_Down_H264');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_bitrate_switching <H264_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Bitrate Switch Down video playback using H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Bitrate Switch Down video playback using H264 stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
