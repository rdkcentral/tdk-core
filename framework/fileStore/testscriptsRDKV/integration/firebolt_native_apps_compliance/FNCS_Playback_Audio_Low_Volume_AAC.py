...
<xml>
  ...
  <name>FNCS_Playback_Audio_Low_Volume_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether audio is playing in given volume using AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_134</test_case_id>
    <test_objective>to verify whether audio is playing in given volume using AAC stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Low_Volume_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Low_Volume_AAC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_volume <AAC_STREAM_URL> checkavstatus=yes timeout=20"
