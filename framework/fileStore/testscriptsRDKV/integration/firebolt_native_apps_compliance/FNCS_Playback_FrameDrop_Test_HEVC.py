...
<xml>
  ...
  <name>FNCS_Playback_FrameDrop_Test_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of FrameDrop_Test HEVC stream through videotestsrc gst element</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_336</test_case_id>
    <test_objective>To test the video playback of FrameDrop_Test HEVC stream through videotestsrc gst element</test_objective>
...
3. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_frameDrop"
...
3.Construct the mediapipelinetests command based on the retrieved testcasename, FIREBOLT_COMPLIANCE_CHECK_AV_STATUS deviceconfig value and timeout
...
6.Ensure that no frames are dropped and all the frames are rendered properly.
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_FrameDrop_Test_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_FrameDrop_Test_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_frameDrop <FrameDrop_Test_STREAM_URL> checkavstatus=yes timeout=20"
    arguments = {"checkavstatus" : checkAVStatus, "timeout": timeoutInSeconds, "totalFrames": totalFrames}
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HEVC FrameDrop_Test Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HEVC FrameDrop_Test Playback using 'playbin' and 'westeros-sink' was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
