...
<xml>
  ...
  <name>FNCS_Playback_Bitrate_Switch_Up_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Restrict network bandwidth to certain bandwidth and verify the same by switching from lowest possible resolution to highest resolution in the manifest.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_272</test_case_id>
    <test_objective>Restrict network bandwidth to certain bandwidth and verify the same by switching from lowest possible resolution to highest resolution in the manifest</test_objective>
...
2. Test stream url for an HEVC stream should be updated in the config variable video_src_url_bitrate_hevc inside MediaValidationVariables.py library inside filestore
...
7.Using dash-demux instance the bitrate is set for the lowest resolution available in the manifest file and played for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds.
8.Application then sets the next highest bandwidth and verifies if the resolution is also as played as expected from the manifest.
9.Upon successfully playing all resolutions for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds, applications returns SUCCESS.
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Bitrate_Switch_Up_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Bitrate_Switch_Up_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_bitrate_switch_up"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Bitrate Switch UP video playback using HEVC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Bitrate Switch UP video playback using HEVC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
