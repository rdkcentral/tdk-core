...
<xml>
  ...
  <name>FNCS_Playback_PauseSeek_Forward_H264_MOV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek with pause on a stream with H264_MOV video codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case>FNCS_PLAYBACK_365</test_case_id>
    <test_objective>Test to do forward seek with pause on a stream with H264_MOV video codec</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RPI</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests application must be installed in DUT
2. Test stream url for a stream with H264_MOV video codec should be updated in the config variable video_src_url_h264_mov inside MediaValidationVariables.py library inside filestore
...
2.test_url - H264_MOV url from MediaValidationVariables library (MediaValidationVariables.video_src_url_h264_mov)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PauseSeek_Forward_H264_MOV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PauseSeek_Forward_H264_MOV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_MOV_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pause with Forward seek on H264_MOV stream was successfull")
