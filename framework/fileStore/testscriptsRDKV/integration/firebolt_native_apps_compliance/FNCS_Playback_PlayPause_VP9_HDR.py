...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_VP9_HDR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for VP9_HDR stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_374</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for VP9_HDR stream through playbin gst element and westerossink</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests application must be installed in DUT
2. Test stream url for an VP9_HDR stream should be updated in the config variable video_src_url_vp9_hdr inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_VP9_HDR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_VP9_HDR');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <VP9_HDR_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("VP9_HDR Play_pause using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("VP9_HDR Play_pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
