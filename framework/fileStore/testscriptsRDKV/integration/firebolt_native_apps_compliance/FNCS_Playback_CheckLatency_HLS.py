...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if latency observed during HLS playback is within the threshold limit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_204</test_case_id>
    <test_objective>Check if latency observed during HLS playback is within the threshold limit</test_objective>
...
2. Test stream url for an CheckLatency_HLS stream should be updated in the config variable video_src_url_hls inside MediaValidationVariables.py library inside filestore
...
5. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    <api_or_interface_used>Execute the tdk_mediapipelinetests application in DUT</api_or_interface_used>
    <input_parameters>testcasename - "test_playback_latency"
test_url - CheckLatency_HLS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hls)
...
7.Based on the ExecuteCommand() return value and the output returned from the tdk_mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_HLS');

#Set device configurations to default values
checkAVStatus = "no"
timeoutInSeconds = "10"
latencyThresholdValue = "100"
...
if "SUCCESS" in result.upper():
    ...
    #Retrieve the value of configuration parameter 'FIREBOLT_COMPLIANCE_PLAYBACK_LATENCY_THRESHOLD' that specifies the  latency threshold in milliseconds
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_PLAYBACK_LATENCY_THRESHOLD")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    latencyThresholdValueConfigValue = tdkTestObj.getResultDetails();

    #If the value of FIREBOLT_COMPLIANCE_PLAYBACK_LATENCY_THRESHOLD is retrieved correctly and its value is not empty, latency threshold value should be passed to the test application
    ...
    #Sample command = "mediapipelinetests test_playback_latency <CheckLatency_HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("CheckLatency_HLS Playback using 'playbin' and 'westeros-sink' was successfull")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("CheckLatency_HLS Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
