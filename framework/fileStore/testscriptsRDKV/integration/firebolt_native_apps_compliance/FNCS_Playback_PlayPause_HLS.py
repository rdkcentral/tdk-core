...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for HLS stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_90</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for HLS stream through playbin gst element and westerossink</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HLS Play_pause using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HLS Play_pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
