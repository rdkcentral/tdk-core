...
<xml>
  ...
  <name>FNCS_Playback_4K_Resolution_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback using HEVC_MKV with resolution set to 2160p  through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_352</test_case_id>
    <test_objective>To test the video playback using HEVC_MKV with resolution set to 2160p through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an 2160p stream should be updated in the config variable video_src_url_hevc_mkv inside MediaValidationVariables.py library inside filestore
...
test_url - 2160p hevc_mkv mp4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_mkv)
...
5.Retrieve the video_src_url_hevc_mkv variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_4K_Resolution_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_4K_Resolution_HEVC_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_resolution <HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HEVC_MKV stream Playback using 'playbin' and 'westeros-sink' with resolution %s was successfull"%resolution)
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HEVC_MKV stream Playback using 'playbin' and 'westeros-sink' with resolution %s failed"%resolution)
    else:
        tdkTestObj.setResultStatus("FAILURE")
