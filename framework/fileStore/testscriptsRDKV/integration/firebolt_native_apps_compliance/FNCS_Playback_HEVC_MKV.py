...
<xml>
  ...
  <name>FNCS_Playback_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of HEVC_MKV stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_350</test_case_id>
    <test_objective>To test the video playback of HEVC_MKV stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_HEVC_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=20"
