...
<xml>
  ...
  <name>FNCS_Playback_Set_Rate_0.75x_H264_29Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a 29 Fps H264 stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_235</test_case_id>
    <test_objective>Test to do set rate operation of a 29 Fps H264 stream with playback rate 0.75</test_objective>
...
2. Test stream url for a H264_29Fps stream should be updated in the config variable video_src_url_dash_h264_29fps inside MediaValidationVariables.py library inside filestore
...
test_url - H264_29Fps url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_29fps)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Set_Rate_0.75x_H264_29Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Set_Rate_0.75x_H264_29Fps');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_29Fps_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("29 Fps H264 Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("29 Fps H264 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
