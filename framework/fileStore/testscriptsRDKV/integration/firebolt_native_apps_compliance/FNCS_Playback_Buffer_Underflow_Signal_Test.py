...
<xml>
  ...
  <name>FNCS_Playback_Buffer_Underflow_Signal_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test if buffer underflow callback is received upon video underrun from westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_148</test_case_id>
    <test_objective>To test if buffer underflow callback is received upon video underrun from westerossink</test_objective>
...
5. FIREBOLT_COMPLIANCE_VIDEO_END_POINT configuration should be set to notify the point where video underrun happens </pre_requisite>
    ...
    <input_parameters>testcasename - "test_buffer_underflow_signal_test"
...
5.Execute the command in DUT. During the execution, the DUT will playback av for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds and waits for few seconds after FIREBOLT_COMPLIANCE_VIDEO_END_POINT upon which buffer underflow signal must be received.
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>buffer underflow signal must be received upon video underrun</expected_output>    
    <priority>High</priority>
    <test_stub_interface>tdk_mediapipelinetests</test_stub_interface>
    <test_script>FNCS_Playback_Buffer_Underflow_Signal_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Buffer_Underflow_Signal_Test');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_buffer_underflow_signal"
    ...
    #Sample command = "mediapipelinetests test_buffer_underflow_signal <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Received buffer underflow signal upon video underrun from 'westeros-sink'")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("buffer underflow signal was not received from 'westeros-sink' upon video underrun")
    else:
        tdkTestObj.setResultStatus("FAILURE")
