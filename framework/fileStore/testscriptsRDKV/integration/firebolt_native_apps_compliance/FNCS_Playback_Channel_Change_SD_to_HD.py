...
<xml>
  ...
  <name>FNCS_Playback_Channel_Change_SD_to_HD</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_195</test_case_id>
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>FNCS_PLAYBACK_Channel_Change_SD_to_HD</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Channel_Change_SD_to_HD');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <Channel_Change_SD_to_HD_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel_Change_SD_to_HD Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel_Change_SD_to_HD Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
