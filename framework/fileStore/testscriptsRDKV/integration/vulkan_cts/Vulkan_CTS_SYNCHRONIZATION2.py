...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Vulkan_CTS_SYNCHRONIZATION2');
...
info_filename = "dEQP-VK.synchronization2"
qpa_file_name = "synchronization2.qpa"
...
if expectedresult in result.upper():
    ...
    print ("Vulkan CTS SYNCHRONIZATION2 : %s" %details)

    qpa_folder_path= Vulkan_CTSVariables.qpa_folder_path
    result_dir = Vulkan_CTSVariables.result_dir

    #if "ERROR" in details.upper():
    if "FATAL ERROR" in details.upper() or "VK_ERROR" in details.upper():
        print("FAILURE : Error found in Vulkan CTS output")
