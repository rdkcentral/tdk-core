...
<xml>
  ...
  <name>HdmicecHal_Get_PhysicalAddress_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to get the physical address obtained by the driver</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_02</test_case_id>
    <test_objective>Test Script to get the physical address obtained by the driver</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG,XI3,XI6,Video_Accelerator</test_setup>
    <pre_requisite>1. TDK Agent should be up and running
2. Required a setup with TV connection. Not mandatory to have CEC enabled TV
...
3. Physical address obtained should be non F.F.F.F value
4. Based on the physical address obtained updated the test result as SUCCESS/FAILURE
5. Unload the module</automation_approch>
    <expected_output>Should get non F.F.F.F physical address</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_Get_PhysicalAddress_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Get_PhysicalAddress_HDMI');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    # Setup with TV connection is required. Not mandatory to have CEC enabled TV
