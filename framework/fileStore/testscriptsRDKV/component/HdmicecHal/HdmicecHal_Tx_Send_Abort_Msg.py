...
<xml>
  ...
  <name>HdmicecHal_Tx_Send_Abort_Msg</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to send abort message to the target device (TV). This message is reserved for testing purpose.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_14</test_case_id>
    <test_objective>Test Script to send abort message to the target device (TV). This message is reserved for testing purpose.</test_objective>
...
3. Transmit the Abort CEC frame
...
6. Unload the module</automation_approch>
    <expected_output>Should transmit and receive feature abort CEC frames</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_Tx_Send_Abort_Msg</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Tx_Send_Abort_Msg');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    # Opcode to be sent: ABORT = 0xFF
    # Operand to be received : FEATURE_ABORT = 0x00
    ...
    if expectedResult in actualResult1 and expectedResult in actualResult2:
        ...
        print("\nTEST STEP : Transmit Abort message to target device")
        print("EXEPECTED RESULT : Should receive the feature abort message from target")
        ...
        if expectedResult in actualResult:
            ...
            if version_operand in version_operand_received:
                print("ACTUAL RESULT  : Feature Abort frame received")
                print("[TEST EXECUTION RESULT] : SUCCESS\n")
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print("ACTUAL RESULT  : Feature Abort frame not received")
