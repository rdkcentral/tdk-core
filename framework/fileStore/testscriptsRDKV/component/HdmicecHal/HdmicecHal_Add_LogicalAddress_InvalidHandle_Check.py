...
<xml>
  ...
  <name>HdmicecHal_Add_LogicalAddress_InvalidHandle_Check</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>HdmicecHal_AddLogicalAddress_InvalidHandle_check</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>to validate with the invalid handle param</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_19</test_case_id>
    <test_objective>To validate with the invalid handle</test_objective>
...
3.  HdmiCecOpen should open a CEC driver instance successfully and iarmbus event should be obtained from device ready call back.</pre_requisite>
    ...
    2. Add the logical address to the driver using HdmiCecAddLogicalAddress API by passing an invalid handle.
    ...
    <test_script>HdmicecHal_Add_LogicalAddress_InvalidHandle_Check</test_script>
    <skipped>no</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Add_LogicalAddress_InvalidHandle_Check');
...
if "SUCCESS" in result.upper():
    ...
    print("\nTEST STEP1 : Send an invalid handle param to the HdmiCecAddLogicalAddress API");
    ...
    if expectedResult in actualResult:
        ...
        print(details)
        print("ACTUAL RESULT: HdmiCecAddLogicalAddress call failed due to invalid handle arg");
