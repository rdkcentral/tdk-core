...
<xml>
  ...
  <name>HdmicecHal_Remove_LogicalAddress_InvalidHandle_Check</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>HdmicecHal_Remove_LogicalAddress_InvalidHandle_Check</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>to verfiy with the invalid handle param.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_20</test_case_id>
    <test_objective>To validate removelogicaladdress api with the invalid handle</test_objective>
...
3.  HdmiCecOpen should open a CEC driver instance successfully and iarmbus event should be obtained from device ready call back.</pre_requisite>
    ...
    <input_parameters>handle - invalid handle
    logicalAddresses - logical address to be released</input_parameters>
    <automation_approch>1.Load the Hdmicec Hal module
    2. Remove the logical address to the driver using HdmiCecRemoveLogicalAddress API by passing an invalid handle.
    3. Based on the return value obtained updated the test result as SUCCESS/FAILURE
    4. Unload the module</automation_approch>
    <expected_output>Should return the invalid handle as a result</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_Remove_LogicalAddress_InvalidHandle_Check</test_script>
    <skipped>no</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Remove_LogicalAddress_InvalidHandle_Check');
...
if "SUCCESS" in result.upper():
    ...
    print("\nTEST STEP1 : Send an invalid handle param to the HdmiCecRemoveLogicalAddress API");
    ...
    tdkTestObj.addParameter("handle",handle);
    ...
    if expectedResult in actualResult:
        ...
        print(details)
        print("ACTUAL RESULT: HdmiCecRemoveLogicalAddress call failed due to invalid handle arg");
