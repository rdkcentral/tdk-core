...
<xml>
  ...
  <name>RDKV_Container_LightningApp_SetURLAndExit</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Launch LightningApp in container mode, set url and exit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_53</test_case_id>
    <test_objective>Launch Lightning App in container mode, set url and exit</test_objective>
...
8. Set URL
9.Exit from LightningApp
</automation_approch>
    <expected_output>Should be able to set URL and exit from LightningApp</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_LightningApp_SetURLAndExit</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_LightningApp_SetURLAndExit');
                        ...
                        if(html_result in expectedResult):
                            ...
                            if "SUCCESS" == result:
                                ...
                                if LightningApp_status == 'deactivated' and expectedResult in result:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("LightningApp has been destroyed successfully")
                                else:
