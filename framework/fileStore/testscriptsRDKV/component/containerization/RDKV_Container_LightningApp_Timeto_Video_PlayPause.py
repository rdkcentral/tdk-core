...
<xml>
  ...
  <test_cases>
    <test_case_id>Containerization_58</test_case_id>
    <test_objective>Start playback in LightningApp then calculate time to play and pause video</test_objective>
    ...
    LIGHTNINGAPP_PLAYBACK_URL
    LIGHTNINGAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER
    LIGHTNINGAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER</input_parameters>
...
10. Calculate time to pause and play from wpeframework.log
    ...
    <test_script>RDKV_Container_LightningApp_Timeto_Video_PlayPause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_LightningApp_Timeto_Video_PlayPause');
...
if expectedResult in result.upper():
    ...
    configKeyList = ["SSH_METHOD", "SSH_USERNAME", "SSH_PASSWORD", "LIGHTNINGAPP_DETAILS", "LIGHTNINGAPP_PLAYBACK_URL", "LIGHTNINGAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER", "LIGHTNINGAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER", "THRESHOLD_OFFSET_CONTAINER"]
    configValues = {}
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            LightningApp_pause_threshold = configValues["LIGHTNINGAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER"]
            LightningApp_play_threshold = configValues["LIGHTNINGAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER"]
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching LightningApp in container mode" in output:
                        ...
                        print("\n Set the URL : {} using LightningApp")
                        ...
                        tdkTestObj.executeTestCase(expectedResult)
                        LightningApp_result = tdkTestObj.getResult()
                        time.sleep(10)
                        if(LightningApp_result in expectedResult):
                            tdkTestObj.setResultStatus("SUCCESS")
                                                ...
                                                if video_pausedtime_in_millisec > pause_starttime_in_millisec:
                                                    ...
                                                    if result == "SUCCESS":
                                                        ...
                                                        if output != "EXCEPTION" and expectedResult in result and "old: PAUSED" in output:
                                                            ...
                                                            result1,LightningApp_pause_threshold = getDeviceConfigKeyValue(conf_file,"LIGHTNINGAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER")
                                                            result2,LightningApp_play_threshold = getDeviceConfigKeyValue(conf_file,"LIGHTNINGAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER")
                                                            offset_status,offset = getDeviceConfigKeyValue(conf_file,"THRESHOLD_OFFSET_CONTAINER")
                                                            if all(value != "" for value in (LightningApp_pause_threshold,LightningApp_play_threshold,offset)):
                                                                ...
                                                                print("\n Threshold value for time taken for play operation : {} ms".format(LightningApp_play_threshold))
                                                                if 0 < int(time_for_video_play) < (int(LightningApp_play_threshold) + int(offset)):
                                                                ...
                                                                print("\n Threshold value for time taken for pause operation : {} ms".format(LightningApp_pause_threshold))
                                                                if 0 < int(time_for_video_pause) < (int(LightningApp_pause_threshold) + int(offset)):
