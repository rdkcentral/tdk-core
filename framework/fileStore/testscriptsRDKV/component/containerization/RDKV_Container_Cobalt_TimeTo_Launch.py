...
<xml>
  ...
  <name>RDKV_Container_Cobalt_TimeTo_Launch</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the time taken to launch Cobalt plugin in container mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_54</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to launch Cobalt plugin in container mode</test_objective>
...
2. COBALT_LAUNCH_THRESHOLD_VALUE_IN_CONTAINER
...
5. Save current system time and launch Cobalt using RDKShell.
...
7. Validate the output with threshold value</automation_approch>
    ...
    <test_script>RDKV_Container_Cobalt_TimeTo_Launch</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Cobalt_TimeTo_Launch');
...
if expectedResult in result.upper():
    ...
    configKeyList = ["SSH_METHOD", "SSH_USERNAME", "SSH_PASSWORD", "COBALT_DETAILS","COBALT_LAUNCH_THRESHOLD_VALUE_IN_CONTAINER", "THRESHOLD_OFFSET_IN_CONTAINER"]
    configValues = {}
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            cobalt_launch_threshold = configValues["COBALT_LAUNCH_THRESHOLD_VALUE_IN_CONTAINER"]
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching cobalt in container mode" in output:
                        ...
                        if launched_time:
                            print("\n Validating the time taken to launch Cobalt in container mode")
                            if all(value != "" for value in (cobalt_launch_threshold,threshold_offset)):
                                ...
                                if 0 < time_taken_for_launch < (int(cobalt_launch_threshold) + int(threshold_offset)) :
