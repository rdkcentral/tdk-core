...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_System_Device_Details_Check</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To Validate the System Device Details available in the device with regex pattern</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_03</test_case_id>
    <test_objective>To Initiate the shell scripts which validate the System Device Details of deviceDetails</test_objective>
...
2.Check using regex for each parameter</automation_approch>
    ...
    <test_script>RDKV_Basic_Sanity_System_Device_Details_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_System_Device_Details_Check');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check.sh 10"
