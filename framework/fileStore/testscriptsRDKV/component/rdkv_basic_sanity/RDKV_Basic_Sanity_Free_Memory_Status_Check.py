...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_Free_Memory_Status_Check</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To compare FREE memory responses of both tr181 command and /proc/meminfo</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_13</test_case_id>
    <test_objective>To Initiate the shell scripts which compare FREE memory responses of both tr181 command and /proc/meminfo</test_objective>
    ...
    <automation_approch>Compare the responses and report if it is exceeding the threshold limit</automation_approch>
    <expected_output>If Responses from both tr181 command and /proc/meminfo are same then success or else failure.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Basic_Sanity_Free_Memory_Status_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_Free_Memory_Status_Check');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check.sh 17"
