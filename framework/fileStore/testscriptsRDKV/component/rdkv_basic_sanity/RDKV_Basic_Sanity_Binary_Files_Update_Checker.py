...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_Binary_Files_Update_Checker</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To  Compare binary files with image build date and report if any old binary files are found</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_10</test_case_id>
    <test_objective>To Initiate the shell scripts which Compare binary files with image build date and report if any old binary files are found</test_objective>
    ...
    <automation_approch>List down all the files in the given locations and filter the files based on image build date.</automation_approch>
    <expected_output>Lists Success when no old binaries are found and Failure when old binaries are found</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Basic_Sanity_Binary_Files_Update_Checker</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_Binary_Files_Update_Checker');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check.sh 14"
