...
<xml>
  ...
  <name>RDKV_Mandatory_Library_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Script to validate all mandatory libraries are present in the device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_15</test_case_id>
    <test_objective>Script to validate all mandatory libraries are present in the device</test_objective>
    ...
    <automation_approch>The mandatory libraries in the configuration files should be present in the given location</automation_approch>
    <expected_output>All mandatory libraries should be present</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Mandatory_Library_Check</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'RDKV_Mandatory_Library_Check');
...
if ("SUCCESS" in LoadStatus.upper()):
    ...
    command = "cd /opt/TDK; sh system_file_check.sh Library_check"
