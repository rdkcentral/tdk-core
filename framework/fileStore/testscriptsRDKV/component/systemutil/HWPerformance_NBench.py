...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    ...
    <pre_requisite>1. TDK Agent should be up and running 2. nbench binary should be available in DUT 3. Shell script file TDK_HWPerfTools_Executor.sh, Log parsing script HWPerf_metric_parser.sh and xml file HWPerf_metric_details.xml should be available at $TDK_PATH</pre_requisite>
    <api_or_interface_used>Executes the nbench binary</api_or_interface_used>
    <input_parameters>sh TDK_HWPerfTools_Executor.sh NBench</input_parameters>
    <automation_approch>1. Execute the TDK_HWPerfTools_Executor.sh file with the required parameters and save the log in $TDK_PATH/logs/performance.log 2. Parse the nbench log using HWPerf_metric_parser.sh script and save the metrices value as Json response in logparser-results.txt. 3. Return the metrices as Json response. Note. More details on nbench is given in corresponding manual page</automation_approch>
...
if ("SUCCESS" in sysUtilLoadStatus.upper()):
    ...
    command = "sh TDK_HWPerfTools_Executor.sh NBench"
    ...
    if expectedresult in actualresult:
        if details:
            ...
            result=hardwarePerformanceThresholdComparison(sysUtilObj,details,unit=" ",reverserscheck="false")
