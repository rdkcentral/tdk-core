...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_24_ClearAllAppData</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager clearAllAppData API</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_AppManager_clearAllAppData</test_case_id>
    <test_objective>Test AppManager clearAllAppData API - Query scenarios</test_objective>
...
3. Device should have required applications installed</pre_requisite>
    <api_or_interface_used>org.rdk.AppManager.1.clearAllAppData</api_or_interface_used>
    <input_parameters>Method specific parameters</input_parameters>
    <automation_approch>1. Activate AppManager plugin
2. Test clearAllAppData API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>clearAllAppData API should return appropriate responses for Query scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_24_ClearAllAppData</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for clearAllAppData API Query scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_24_ClearAllAppData')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    try:
        ...
        if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
            ...
            print("
[TEST] clearAllAppData API - Query scenarios")
