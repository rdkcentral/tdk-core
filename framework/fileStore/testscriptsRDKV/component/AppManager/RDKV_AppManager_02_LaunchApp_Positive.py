...
<xml>
  ...
  <status>FREE</status>
  <synopsis>Test AppManager launchApp API - Positive scenarios</synopsis>
  <groups_id />
...
4. Report test results</automation_approch>
    ...
    <test_script>RDKV_AppManager_02_LaunchApp_Positive</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for launchApp API Positive scenarios</remarks>
  </test_cases>
</xml>
'''
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: launchApp API - Positive
        print("[TEST] launchApp API - Positive scenarios")
        ...
        try:
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] launchApp API returned successful result for app: %s" % app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] launchApp API error: %s" % result.get("error"))
                obj.setLoadModuleStatus("FAILURE")
            else:
