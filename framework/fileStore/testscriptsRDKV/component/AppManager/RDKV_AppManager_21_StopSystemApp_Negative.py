...
obj.configureTestCase(ip, port, 'RDKV_AppManager_21_StopSystemApp_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        try:
            ...
            if "error" in result:
                print("[SUCCESS] stopSystemApp API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] stopSystemApp API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
