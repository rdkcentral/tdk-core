...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_09_TerminateApp_Negative</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager terminateApp API - Negative scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_AppManager_terminateApp</test_case_id>
    <test_objective>Test AppManager terminateApp API - Negative scenarios</test_objective>
...
3. Device should have required applications installed</pre_requisite>
    <api_or_interface_used>org.rdk.AppManager.1.terminateApp</api_or_interface_used>
    <input_parameters>Method specific parameters</input_parameters>
    <automation_approch>1. Activate AppManager plugin
2. Test terminateApp API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>terminateApp API should return appropriate responses for Negative scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_09_TerminateApp_Negative</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for terminateApp API Negative scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_09_TerminateApp_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: terminateApp API - Negative
        print("[TEST] terminateApp API - Negative scenarios")
        ...
        try:
            method_name = "org.rdk.AppManager.1.terminateApp"
            request_data = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": method_name,
            ...
            if "error" in result:
                print("[SUCCESS] terminateApp API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] terminateApp API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
                print("[INFO] terminateApp API response: %s" % result)
                obj.setLoadModuleStatus("SUCCESS")
        except urllib.error.URLError as e:
            print("[ERROR] Failed to call terminateApp API: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
        except Exception as e:
            print("[ERROR] Unexpected error during terminateApp API call: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
