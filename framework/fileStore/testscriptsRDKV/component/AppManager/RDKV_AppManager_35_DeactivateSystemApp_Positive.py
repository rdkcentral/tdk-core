...
obj.configureTestCase(ip, port, 'RDKV_AppManager_35_DeactivateSystemApp_Positive')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    # Test: deactivateSystemApp API - Positive
    print("[TEST] deactivateSystemApp API - Positive scenarios")
    ...
    try:
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] deactivateSystemApp API returned successful result for app: %s" % system_app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] deactivateSystemApp API error: %s" % result.get("error"))
