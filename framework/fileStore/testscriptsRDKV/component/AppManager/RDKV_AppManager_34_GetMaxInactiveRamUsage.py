...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_34_GetMaxInactiveRamUsage</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager getMaxInactiveRamUsage property</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_AppManager_getMaxInactiveRamUsage</test_case_id>
    <test_objective>Test AppManager getMaxInactiveRamUsage API - Property scenarios</test_objective>
...
3. Device should have required applications installed</pre_requisite>
    <api_or_interface_used>org.rdk.AppManager.1.getMaxInactiveRamUsage</api_or_interface_used>
    <input_parameters>Method specific parameters</input_parameters>
    <automation_approch>1. Activate AppManager plugin
2. Test getMaxInactiveRamUsage API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>getMaxInactiveRamUsage API should return appropriate responses for Property scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_34_GetMaxInactiveRamUsage</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for getMaxInactiveRamUsage API Property scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_34_GetMaxInactiveRamUsage')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: getMaxInactiveRamUsage API - Property
        print("[TEST] getMaxInactiveRamUsage API - Property scenarios")
        ...
        try:
            method_name = "org.rdk.AppManager.getMaxInactiveRamUsage"
            request_data = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": method_name,
            ...
            if "result" in result and isinstance(result.get("result"), (int, float)):
                print("[SUCCESS] getMaxInactiveRamUsage API returned value: %s" % result.get("result"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] getMaxInactiveRamUsage API error: %s" % result.get("error"))
                obj.setLoadModuleStatus("FAILURE")
            else:
                print("[INFO] getMaxInactiveRamUsage API response: %s" % result)
                obj.setLoadModuleStatus("SUCCESS")
        except urllib.error.URLError as e:
            print("[ERROR] Failed to call getMaxInactiveRamUsage API: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
        except Exception as e:
            print("[ERROR] Unexpected error during getMaxInactiveRamUsage API call: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
