...
obj.configureTestCase(ip, port, 'RDKV_AppManager_36_DeactivateSystemApp_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    # Test: deactivateSystemApp API - Negative
    print("[TEST] deactivateSystemApp API - Negative scenarios")

    try:
            method_name = "org.rdk.AppManager.1.deactivateSystemApp"
            request_data = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": method_name,
            ...
            if "error" in result:
                print("[SUCCESS] deactivateSystemApp API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] deactivateSystemApp API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
                print("[INFO] deactivateSystemApp API response: %s" % result)
                obj.setLoadModuleStatus("SUCCESS")
    except urllib.error.URLError as e:
            print("[ERROR] Failed to call deactivateSystemApp API: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
    except Exception as e:
            print("[ERROR] Unexpected error during deactivateSystemApp API call: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
