...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_03_LaunchApp_Negative</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager launchApp API - Negative scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    ...
    <test_objective>Test AppManager launchApp API - Negative scenarios</test_objective>
...
2. Test launchApp API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>launchApp API should return appropriate responses for Negative scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_03_LaunchApp_Negative</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for launchApp API Negative scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_03_LaunchApp_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: launchApp API - Negative
        print("[TEST] launchApp API - Negative scenarios")
        ...
        try:
            method_name = "org.rdk.AppManager.1.launchApp"
            request_data = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": method_name,
                "params": {"appId": "invalid.app.id"}
            }
            ...
            if "error" in result:
                print("[SUCCESS] launchApp API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] launchApp API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
                print("[INFO] launchApp API response: %s" % result)
                obj.setLoadModuleStatus("SUCCESS")
        except urllib.error.URLError as e:
            print("[ERROR] Failed to call launchApp API: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
        except Exception as e:
            print("[ERROR] Unexpected error during launchApp API call: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
