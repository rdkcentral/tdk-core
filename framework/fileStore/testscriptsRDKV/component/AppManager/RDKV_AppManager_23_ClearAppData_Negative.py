...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_23_ClearAppData_Negative</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager clearAppData API - Negative scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_AppManager_clearAppData</test_case_id>
    <test_objective>Test AppManager clearAppData API - Negative scenarios</test_objective>
...
3. Device should have required applications installed</pre_requisite>
    <api_or_interface_used>org.rdk.AppManager.1.clearAppData</api_or_interface_used>
    <input_parameters>Method specific parameters</input_parameters>
    <automation_approch>1. Activate AppManager plugin
2. Test clearAppData API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>clearAppData API should return appropriate responses for Negative scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_23_ClearAppData_Negative</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for clearAppData API Negative scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_23_ClearAppData_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: clearAppData API - Negative
        print("[TEST] clearAppData API - Negative scenarios")
        ...
        try:
            ...
            if "error" in result:
                print("[SUCCESS] clearAppData API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] clearAppData API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
