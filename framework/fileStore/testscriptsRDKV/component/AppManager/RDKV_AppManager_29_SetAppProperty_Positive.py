...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_29_SetAppProperty_Positive</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager setAppProperty API - Positive scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    ...
    <test_objective>Test AppManager setAppProperty API - Positive scenarios</test_objective>
...
4. Report test results</automation_approch>
    <expected_output>setAppProperty API should return appropriate responses for Positive scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_29_SetAppProperty_Positive</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for setAppProperty API Positive scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_29_SetAppProperty_Positive')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: setAppProperty API - Positive
        print("[TEST] setAppProperty API - Positive scenarios")
        ...
        try:
            ...
            request_data = {
                ...
                "id": 1,
                "method": method_name,
                "params": {"appId": app_id, "property": "state", "value": "active"}
            }
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] setAppProperty API returned successful result for app: %s" % app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] setAppProperty API error: %s" % result.get("error"))
                obj.setLoadModuleStatus("FAILURE")
            else:
