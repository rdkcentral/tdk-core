...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_20_StopSystemApp_Positive</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager stopSystemApp API - Positive scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    ...
    <test_objective>Test AppManager stopSystemApp API - Positive scenarios</test_objective>
...
4. Report test results</automation_approch>
    <expected_output>stopSystemApp API should return appropriate responses for Positive scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_20_StopSystemApp_Positive</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for stopSystemApp API Positive scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_20_StopSystemApp_Positive')
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        try:
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] stopSystemApp API returned successful result for app: %s" % system_app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] stopSystemApp API error: %s" % result.get("error"))
                obj.setLoadModuleStatus("FAILURE")
            else:
