...
obj.configureTestCase(ip, port, 'RDKV_AppManager_38_HibernateSystemApp_Negative')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    # Test: hibernateSystemApp API - Negative
    print("[TEST] hibernateSystemApp API - Negative scenarios")
    ...
    try:
            ...
            if "error" in result:
                print("[SUCCESS] hibernateSystemApp API correctly returned error for invalid app: %s" % result.get("error"))
                obj.setLoadModuleStatus("SUCCESS")
            elif "result" in result and result.get("result") in [False, "error"]:
                print("[SUCCESS] hibernateSystemApp API correctly rejected invalid app")
                obj.setLoadModuleStatus("SUCCESS")
            else:
