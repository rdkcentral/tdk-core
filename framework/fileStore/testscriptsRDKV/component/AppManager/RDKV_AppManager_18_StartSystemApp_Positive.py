...
<xml>
  <id></id>
  <version>1</version>
  <name>RDKV_AppManager_18_StartSystemApp_Positive</name>
  <primitive_test_id></primitive_test_id>
  <primitive_test_name>RdkService_Test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test AppManager startSystemApp API - Positive scenarios</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_AppManager_startSystemApp</test_case_id>
    <test_objective>Test AppManager startSystemApp API - Positive scenarios</test_objective>
...
3. Device should have required applications installed</pre_requisite>
    <api_or_interface_used>org.rdk.AppManager.1.startSystemApp</api_or_interface_used>
    <input_parameters>Method specific parameters</input_parameters>
    <automation_approch>1. Activate AppManager plugin
2. Test startSystemApp API with appropriate parameters
3. Verify response structure and error handling
4. Report test results</automation_approch>
    <expected_output>startSystemApp API should return appropriate responses for Positive scenarios</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkservicesstub.so</test_stub_interface>
    <test_script>RDKV_AppManager_18_StartSystemApp_Positive</test_script>
    <skipped>No</skipped>
    <release_version>M128</release_version>
    <remarks>Test case for startSystemApp API Positive scenarios</remarks>
  </test_cases>
</xml>
'''
...
obj.configureTestCase(ip, port, 'RDKV_AppManager_18_StartSystemApp_Positive')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if not thunder_is_plugin_active(plugin_name, jsonrpc_url=jsonrpc_url):
        ...
        # Test: startSystemApp API - Positive
        print("[TEST] startSystemApp API - Positive scenarios")
        ...
        try:
            method_name = "org.rdk.AppManager.1.startSystemApp"
            system_app_id = get_ai2_setting('appManager.testData.systemAppId', 'org.rdk.System')
            request_data = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": method_name,
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] startSystemApp API returned successful result for app: %s" % system_app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] startSystemApp API error: %s" % result.get("error"))
                obj.setLoadModuleStatus("FAILURE")
            else:
                print("[INFO] startSystemApp API response: %s" % result)
                obj.setLoadModuleStatus("SUCCESS")
        except urllib.error.URLError as e:
            print("[ERROR] Failed to call startSystemApp API: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
        except Exception as e:
            print("[ERROR] Unexpected error during startSystemApp API call: %s" % str(e))
            obj.setLoadModuleStatus("FAILURE")
