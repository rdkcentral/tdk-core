...
obj.configureTestCase(ip, port, 'RDKV_AppManager_25_ActivateSystemApp_Positive')
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    # Test: activateSystemApp API - Positive
    print("[TEST] activateSystemApp API - Positive scenarios")
    ...
    try:
            ...
            if "result" in result and result.get("result") in [True, "success"]:
                print("[SUCCESS] activateSystemApp API returned successful result for app: %s" % system_app_id)
                obj.setLoadModuleStatus("SUCCESS")
            elif "error" in result:
                print("[FAILURE] activateSystemApp API error: %s" % result.get("error"))
