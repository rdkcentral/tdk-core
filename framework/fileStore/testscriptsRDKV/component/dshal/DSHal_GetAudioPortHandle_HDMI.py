...
<xml>
  ...
  <name>DSHal_GetAudioPortHandle_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if audio port handle is retrieved for HDMI</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_1</test_case_id>
    <test_objective>To check if audio port handle is retrieved for HDMI</test_objective>
...
2 . DSHAL agent will invoke the api dsGetAudioPort for HDMI0
...
Checkpoint 2 Verify that handle is retrieved</expected_output>
    ...
    <test_script>DSHal_GetAudioPortHandle_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetAudioPortHandle_HDMI');
