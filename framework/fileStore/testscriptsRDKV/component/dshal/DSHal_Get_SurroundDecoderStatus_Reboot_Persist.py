...
<xml>
  ...
  <name>DSHal_Get_SurroundDecoderStatus_Reboot_Persist</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_IsSurroundDecoderEnabled</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To enable or disable the audio surround decoder status and verify in reboot scenario</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_197</test_case_id>
    <test_objective>To enable or disable the audio surround decoder status and verify in reboot scenario</test_objective>
...
capabilities - to retrieve the platform supported MS12 capability fields.
enabled - Surround Decoder enabled(1)/disabled(0) value</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2. If Surround decoder MS12 feature is supported DSHAL agent will invoke the api dsIsSurroundDecoderEnabled to get SurroundDecoder enabled status
3. TM checks if the status is correct and return SUCCESS/FAILURE status.
4. Also should enabled the surround decoder using dsEnableSurroundDecoder API and verify in reboot scenario</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the status is correct if feature is supported
Checkpoint 3 If featue is not supported on source device then it should return always dsERR_OPERATION_NOT_SUPPORTED</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_Get_SurroundDecoderStatus_Reboot_Persist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_Get_SurroundDecoderStatus_Reboot_Persist');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        #Get the current Surround decoder enabled status
        ...
        if expectedResult in actualResult:
            # if box type is in STB and surround decoder is not supported then expected o/p is OP_NOT_SUPPORTED
            if source_device_failure_handlings:
                ...
                print("Test step 2 : to Enable the Surround decoder & reboot the device");
                ...
                if expectedResult in actualResult:
                    ...
                    print("\nTEST STEP 4 : Get the surround decoder enabled status using dsIsSurroundDecoderEnabled API");
                    print("EXPECTED OUTPUT : Should get enabled status as set before reboot using dsEnableSurroundDecoder API");
                    ...
                    if expectedResult in actualResult:
                        if details:
                            ...
                            print("Surround decoder is disabled. But it was enabled before reboot");
                            tdkTestObj.setResultStatus("FAILURE");
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        details = tdkTestObj.getResultDetails();
