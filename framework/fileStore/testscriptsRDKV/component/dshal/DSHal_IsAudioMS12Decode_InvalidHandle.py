...
<xml>
  ...
  <name>DSHal_IsAudioMS12Decode_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioMS12Decode status is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_139</test_case_id>
    <test_objective>To check if AudioMS12Decode status is set for invalid handle</test_objective>
...
dsIsAudioMS12Decode(int handle, bool enabled)</api_or_interface_used>
    <input_parameters>type - Videoo port type
index- Audio port index
handle - Audio port handle</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsIsAudioMS12Decode with invalid handle
...
Checkpoint 2 Verify that the status is not retrieved</expected_output>
    ...
    <test_script>DSHal_IsAudioMS12Decode_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsAudioMS12Decode_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        expectedOutput = "AudioMS12Decode status not retrieved";
        #Prmitive test case which associated to this Script
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_IsAudioMS12Decode output: ", details);

        if expectedResult in actualResult and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioMS12Decode status not retrieved for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("AudioMS12Decode status retrieved for invalid handle which is not expected");
