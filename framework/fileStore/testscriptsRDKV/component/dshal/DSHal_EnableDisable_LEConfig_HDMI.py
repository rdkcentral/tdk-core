...
<xml>
  ...
  <name>DSHal_EnableDisable_LEConfig_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To enable and disable LEConfig for HDMI audio port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_92</test_case_id>
    <test_objective>To enable and disable LEConfig for HDMI audio port</test_objective>
...
2 . DSHAL agent will invoke the api dsEnableLEConfig to enable/disable the HDMI audio port LEConfig status
...
Checkpoint 2 Verify that the status is set</expected_output>
    ...
    <test_script>DSHal_EnableDisable_LEConfig_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_EnableDisable_LEConfig_HDMI');

#Get the result of connection with test component and STB
dshalloadModuleStatus = dshalObj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %dshalloadModuleStatus);

#Check if LoudnessEquivalence is supported by DUT
capable = deviceCapabilities.getconfig(dshalObj,"LEConfig")
