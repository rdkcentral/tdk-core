...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the media intelligent steering enable/disable status</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_190</test_case_id>
    <test_objective>To get the media intelligent steering enable/disable status for audio port</test_objective>
...
dsError_t  dsGetMISteering(intptr_t handle, bool *enabled)</api_or_interface_used>
    <input_parameters>type - Audio port type
index- Audio port index
handle - Audio port handle
capabilities - returns the platfom supported MS12 capability fields.
enabled - MI Steering enable/disable status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2. DSHAL agent will invoke the api dsGetMS12Capabilities to get check really MI Steering feature got supported in the platform.
3. DSHAL agent will invoke the api dsGetMISteering to get MISteering status
4. TM checks if the status is correct and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the status is correct
Checkpoint  If MISteering is not supported in source device then API should return dsERR_OPERATION_NOT_SUPPORTED error code</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_GetMISteering</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetMISteering');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if source_device_failure_handlings:
            ...
            print("EXPECTED OUTPUT : Should get the current MI Steering enabled status");
        ...
        if expectedResult in actualResult:
            ...
            if source_device_failure_handlings:
                ...
                if expectederrorVal in details:
                    print("GetMISteering API returned an expected value : dsERR_OPERATION_NOT_SUPPORTED");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                ...
                details = tdkTestObj.getResultDetails();
                print(details);
                enable_status = int(str(details).split(":",1)[1].strip());
                if enabled_status:
