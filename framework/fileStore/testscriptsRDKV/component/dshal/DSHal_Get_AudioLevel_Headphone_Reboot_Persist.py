...
<xml>
  ...
  <name>DSHal_Get_AudioLevel_Headphone_Reboot_Persist</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the set and get Audio level value for headphone in reboot scenario</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_204</test_case_id>
    <test_objective>To verify the set and get Audio level for headphone in reboot scenario</test_objective>
...
dsError_t  dsSetAudioLevel(intptr_t handle, float level)</api_or_interface_used>
    <input_parameters>type - Audio port type
index- Audio port index
handle - Audio port handle
level - returns the current audio vol level to be used for speaker and headphone which connected in sink devices</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsGetAudioLevel and dsSetAudioLevel to set/get the current volume level.
...
Checkpoint 3 If the connected device is video_accelerator then API should return always dsERR_OPERATION_NOT_SUPPORTED</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_Get_AudioLevel_Headphone_Reboot_Persist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_Get_AudioLevel_Headphone_Reboot_Persist');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        print("\nTEST STEP 1 : Get the current audio volume level using dsGetAudioLevel API");
