...
sysUtilObj.configureTestCase(ip,port,'DSHal_GetAndSetResolution_HDMI_1080p60');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and "SUCCESS" in sysUtilloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            display_details = tdkTestObj.getResultDetails();
            print("Display connection status: ", display_details)
            if display_details == "true" and EnableVideoPort(dshalObj):
                resolution = "1080p60";
                
                tdkTestObj = dshalObj.createTestStep('DSHal_SetResolution');
                tdkTestObj.addParameter("resolution",resolution);
                tdkTestObj.addParameter("pixelResolution",4); #dsVIDEO_PIXELRES_1920x1080
