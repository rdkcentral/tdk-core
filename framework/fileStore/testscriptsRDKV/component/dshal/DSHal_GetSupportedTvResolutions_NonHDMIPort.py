...
<xml>
  ...
  <name>DSHal_GetSupportedTvResolutions_NonHDMIPort</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_GetSupportedTvResolutions</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if supported tv resolutions is retrieved for non HDMI port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_100</test_case_id>
    <test_objective>To check if supported tv resolutions is retrieved for non HDMI port</test_objective>
...
dsSupportedTvResolutions(int handle, int *resolutions)</api_or_interface_used>
...
2 . DSHAL agent will invoke the api dsGetVideoPort to get the display handle for non HDMI port
...
Checkpoint 2 Verify that the resolutions is not retrieved</expected_output>
    ...
    <test_script>DSHal_GetSupportedTvResolutions_NonHDMIPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetSupportedTvResolutions_NonHDMIPort');
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                ...
                tdkTestObj = dshalObj.createTestStep('DSHal_GetSupportedTvResolutions');
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
                ...
                if expectedResult in actualResult and expectedOutput in resolutions:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("Supported Tv Resolutions not retrieved which is expected for non HDMI port");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("Supported Tv Resolutions retrieved which is not expected for non HDMI port");
            else:
                tdkTestObj.setResultStatus("FAILURE");
