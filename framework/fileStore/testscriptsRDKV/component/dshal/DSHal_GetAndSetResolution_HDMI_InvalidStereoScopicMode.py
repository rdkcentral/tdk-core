...
<xml>
  ...
  <name>DSHal_GetAndSetResolution_HDMI_InvalidStereoScopicMode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the resolution is set for InvalidStereoScopicMode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_114</test_case_id>
    <test_objective>To check if the resolution is set for InvalidStereoScopicMode</test_objective>
...
3 . DSHAL agent will invoke the api dsSetResolution to set the resolution for InvalidStereoScopicMode
4 . TM checks if the API call is success and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_GetAndSetResolution_HDMI_InvalidStereoScopicMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetAndSetResolution_HDMI_InvalidStereoScopicMode');
sysUtilObj.configureTestCase(ip,port,'DSHal_GetAndSetResolution_HDMI_InvalidStereoScopicMode');
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                ...
                if expectedResult in actualResult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("DSHal_SetResolution call failed for invalid stereoscopic mode which is expected");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("DSHal_SetResolution call success for invalid stereoscopic mode which is not expected");
            else:
                tdkTestObj.setResultStatus("FAILURE");
