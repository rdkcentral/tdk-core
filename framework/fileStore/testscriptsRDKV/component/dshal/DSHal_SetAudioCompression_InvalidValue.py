...
<xml>
  ...
  <name>DSHal_SetAudioCompression_InvalidValue</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioCompression is set for invalid value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if AudioCompression is set for invalid value</test_objective>
...
2 . DSHAL agent will invoke the api dsSetAudioCompression with invalid value
...
Checkpoint 2 Verify that the value is not set</expected_output>
    ...
    <test_script>DSHal_SetAudioCompression_InvalidValue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAudioCompression_InvalidValue');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        #Valid range for compression is [0-10]
        audioCompression = 15;
        #Prmitive test case which associated to this Script
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioCompression not set for invalid value which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetAudioCompression success for invalid value which is not expected");
