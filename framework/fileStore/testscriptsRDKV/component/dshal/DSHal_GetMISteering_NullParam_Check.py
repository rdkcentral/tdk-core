...
<xml>
  ...
  <name>DSHal_GetMISteering_NullParam_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the GetMISteering API with the null param being passed</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_193</test_case_id>
    <test_objective>To validate the GetMISteering API with the null param being passed</test_objective>
...
enabled - MISteering status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsGetMISteering to get MS12Decode status
...
Checkpoint 2 Verify that the status is correct</expected_output>
    ...
    <test_script>DSHal_GetMISteering_NullParam_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetMISteering_NullParam_Check');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            print(details);
            print("ACTUAL RESULT : dsGetMISteering call is failed due to null param being passed");
            tdkTestObj.setResultStatus("SUCCESS");
            print("[TEST EXECUTION RESULT] : SUCCESS");
        else:
            print("Value returned : ", details);
            print("ACTUAL RESULT : dsGetMISteering call is success eventhough null param being passed");
