...
<xml>
  ...
  <name>DSHal_GetSupportedTvResolutions_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get supported tv resolutions for HDMI port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_99</test_case_id>
    <test_objective>To get supported tv resolutions for HDMI port</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort to get the display handle for HDMI port
...
Checkpoint 2 Verify that the resolutions is valid</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_GetSupportedTvResolutions_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetSupportedTvResolutions_HDMI');
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                ...
                if expectedResult in actualResult:
                    ...
                    if int(resolutions):
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("Supported Tv Resolutions is valid");
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("Supported Tv Resolutions is not valid");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("Failed to get Supported Tv Resolutions");
            else:
                tdkTestObj.setResultStatus("FAILURE");
