...
<xml>
  ...
  <name>DSHal_SetLEConfig_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if LEConfig status is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_95</test_case_id>
    <test_objective>To check if LEConfig status is set for invalid handle</test_objective>
...
dsEnableLEConfig(int handle, const bool enabled)</api_or_interface_used>
...
2 . DSHAL agent will invoke the api dsEnableLEConfig to set the enable status with invalid handle
...
Checkpoint 2 Verify that the status is not set</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_SetLEConfig_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetLEConfig_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_EnableLEConfig output: ", details);

        if expectedResult in actualResult and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("LEConfig status not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("EnableLEConfig success for invalid handle which is not expected");
