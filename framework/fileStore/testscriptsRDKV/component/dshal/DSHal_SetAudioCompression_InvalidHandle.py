...
<xml>
  ...
  <name>DSHal_SetAudioCompression_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioCompression is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_143</test_case_id>
    <test_objective>To check if AudioCompression is set for invalid handle</test_objective>
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsSetAudioCompression(int handle, int audioCompressionMs)</api_or_interface_used>
    <input_parameters>handle - Audio port handle
audioCompression- Audio compression</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetAudioCompression with invalid handle
...
Checkpoint 2 Verify that the value is not set</expected_output>
    ...
    <test_script>DSHal_SetAudioCompression_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAudioCompression_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioCompression not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetAudioCompression success for invalid handle which is not expected");
