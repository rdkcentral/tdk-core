...
<xml>
  ...
  <name>DSHal_SetDialogEnhancement_InvalidHandle</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_SetDialogEnhancement</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if DialogEnhancement is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_135</test_case_id>
    <test_objective>To check if DialogEnhancement is set for invalid handle</test_objective>
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsSetDialogEnhancement(int handle, int level)</api_or_interface_used>
    <input_parameters>handle - Audio port handle
enable - enable value</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetDialogEnhancement with invalid handle
...
Checkpoint 2 Verify that the value is not set</expected_output>
    ...
    <test_script>DSHal_SetDialogEnhancement_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetDialogEnhancement_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_SetDialogEnhancement output: ", details);

        if expectedResult in actualResult and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("DialogEnhancement not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetDialogEnhancement success for invalid handle which is not expected");
