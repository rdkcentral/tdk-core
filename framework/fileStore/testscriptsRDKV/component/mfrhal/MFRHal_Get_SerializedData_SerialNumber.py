...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_SerialNumber</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Serial Number from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_06</test_case_id>
    <test_objective>To retrieve Serial Number from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 5 for mfrSERIALIZED_TYPE_SERIALNUMBER</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_SerialNumber</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_SerialNumber');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 5(mfrSERIALIZED_TYPE_SERIALNUMBER)
    tdkTestObj.addParameter("data_type", 5)
