...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Terminate_Already_Terminated_App')
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            ...
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                ...
                                # Step 8 : Terminate the application which is already terminated and check for the expected error
                                print("\n")
                                time.sleep(3)
                                method = "org.rdk.AppManager.1.terminateApp"
                                value = '{"appId": "'+application_name+'"}'
                                ...
                                if "error" in result and result["error"]["message"] == "ERROR_GENERAL":
                                    print("SUCCESS : terminateApp returned expected error when trying to terminate a terminated application")
                                    tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                    print("FAILURE : terminateApp did not return expected error when trying to terminate a terminated application")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
