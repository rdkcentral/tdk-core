...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Check_OnAppUnloaded_Event')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.DownloadManager": "onAppDownloadStatus"}','{"org.rdk.PackageManagerRDKEMS": "onAppInstallationStatus"}','{"org.rdk.AppManager": "onAppUnloaded"}']
            for item in events:
            ...
            if ("error" not in result and "result" in result) or result["result"] == True or installation_status == "TRUE":
                ...
                result = ast.literal_eval(result)
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    print("SUCCESS : Launch application initiated successfully")
                    ...
                    if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                        ...
                        if launch_status == "TRUE":
                            ...
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                ...
                                # Wait for the onAppUnloaded event
                                event_log = wait_for_event(event_listener)
                                if len(event_log) > 0:
                                    for entry in event_log:
                                        _, json_part = entry.split("$$$", 1)
                                        ...
                                        inner = outer["params"]
                                        #print("\nParsed Event : ", inner)
                                        terminate_appInstanceId = inner["appInstanceId"]
                                        terminate_appId = inner["appId"]
                                        if ( terminate_appId == application_name and terminate_appInstanceId == appInstanceId ):
                                            print("SUCCESS : Received onAppUnloaded event with correct appInstanceId and appId for the terminated application")
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            appunload_status = "TRUE"
                                            break
                                    if appunload_status != "TRUE":
                                        print("FAILURE : Received onAppUnloaded event with incorrect appInstanceId for the terminated application")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                            ...
                            print("FAILURE : Application is not in expected lifecycle state")
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("FAILURE : Failed to get the launched application details")
                        tdkTestObj.setResultStatus("FAILURE")              
                else:
