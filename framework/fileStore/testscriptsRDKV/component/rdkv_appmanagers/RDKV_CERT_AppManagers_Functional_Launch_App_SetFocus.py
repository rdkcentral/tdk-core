...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Launch_App_SetFocus')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.DownloadManager": "onAppDownloadStatus"}','{"org.rdk.PackageManagerRDKEMS": "onAppInstallationStatus"}','{"org.rdk.RDKWindowManager": "onFocus"}']
            for item in events:
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                        ...
                        if launch_status == "TRUE":
                            ...
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                print("SUCCESS : Set focus method executed successfully")
                                ...
                                if len(event_log) > 0:
                                    for entry in event_log:
                                        ...
                                        inner = outer["params"]
                                        #print("\nParsed Event : ", inner)
                                        focus_appInstanceId = inner["appInstanceId"]
                                        if ( focus_appInstanceId == appInstanceId ):
                                            print("SUCCESS : Set focus successful with correct appInstanceId received in event")
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            focus_status = "TRUE"
                                            break
                                    if focus_status != "TRUE":
                                        print("FAILURE : Set focus failed with incorrect appInstanceId received in event")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("FAILURE : Failed to execute set focus method")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
