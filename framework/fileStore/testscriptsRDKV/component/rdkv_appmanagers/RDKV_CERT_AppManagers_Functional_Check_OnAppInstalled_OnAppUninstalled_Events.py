...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Check_OnAppInstalled_OnAppUninstalled_Events')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        print("\n")
        pluginlist = ["org.rdk.StorageManager", "org.rdk.DownloadManager", "org.rdk.PackageManagerRDKEMS", "org.rdk.AppManager"]
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.DownloadManager": "onAppDownloadStatus"}','{"org.rdk.AppManager": "onAppInstalled"}','{"org.rdk.AppManager": "onAppUninstalled"}']
            for item in events:
            ...
            if "error" not in result and "result" in result and result["result"] == True:
                print("INFO : Package is already installed, proceeding with uninstallation steps")
                tdkTestObj.setResultStatus("SUCCESS")
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        for entry in event_log:
                            ...
                            #print("\nParsed Event : ", inner)
                            appId = inner["appId"]
                            if appId == application_name:
                                print("SUCCESS : Application uninstallation successful with correct installation status")
                                tdkTestObj.setResultStatus("SUCCESS")
                                uninstallation_status = "TRUE"
                                break
                        
                        if uninstallation_status == "TRUE":
                            ...
                            if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                                ...
                                for package in packages:
                                    if package["packageId"] == application_name and package["state"] == "UNINSTALLED":
                                        print("SUCCESS : Package found in the listPackages response after uninstallation with correct status")
                                        package_found = "TRUE"
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        break

                                if package_found != "TRUE":
                                    print("FAILURE : Package not found in the listPackages response after uninstallation or incorrect status")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("FAILURE : Failed to get the list of packages after uninstallation")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
                            print("FAILURE : Application uninstallation failed or incorrect installation status")
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                ...
                if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        ...
                        if download_status == "TRUE":
                            # Clear the event buffer before waiting for events to avoid processing old events
                            event_listener.clearEventsBuffer()
                            # Step 7 : Form filelocator URL and Install the package
                            print("\n")
                            ...
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                print("INFO : Package installation initiated, waiting for onAppInstalled event...")

                                # Wait for the onAppInstalled event
                                event_log = wait_for_event(event_listener)
                                if len(event_log) > 0:
                                    for entry in event_log:
                                        _, json_part = entry.split("$$$", 1)
                                        ...
                                        if ( appId == application_name and version == application_version ):
                                            print("SUCCESS : Package installation successful with correct installation status")
                                            tdkTestObj.setResultStatus("SUCCESS")
                                    ...
                                    if installation_status == "TRUE":
                                        ...
                                        if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                                            ...
                                            package_found = "FALSE"
                                            for package in packages:
                                                if package["packageId"] == application_name and package["state"] == "INSTALLED":
                                                    print("SUCCESS : Package found in the listPackages response after installation")
                                                    package_found = "TRUE"
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    break
                                            ...
                                            if package_found == "TRUE":
                                                ...
                                                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                                    ...
                                                    # Wait for the onAppUninstalled event
                                                    event_log = wait_for_event(event_listener)
                                                    if len(event_log) > 0:
                                                        for entry in event_log:
                                                            _, json_part = entry.split("$$$", 1)
                                                            ...
                                                            appId = inner["appId"]
                                                            if appId == application_name:
                                                                print("SUCCESS : Application uninstalled successfully with correct uninstallation status")
                                                                tdkTestObj.setResultStatus("SUCCESS")
                                                                uninstallation_status = "TRUE"
                                                                break
                                                        ...
                                                        if uninstallation_status == "TRUE":
                                                            ...
                                                            print("FAILURE : Application uninstallation failed or incorrect uninstallation status received in event")
                                                            tdkTestObj.setResultStatus("FAILURE")
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE")
                                                else:
                                                ...
                                                print("FAILURE : Package not found in the listPackages response after installation")
                                                tdkTestObj.setResultStatus("FAILURE")
                                        else:
                                            print("FAILURE : Failed to get the list of packages after installation")
                                            tdkTestObj.setResultStatus("FAILURE")
                                    else:
                                        print("FAILURE : Package installation failed or incorrect installation status received in event")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
