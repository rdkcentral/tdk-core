...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Launch_App_GenerateKey')
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            # Step 7 : Apply keycode using generateKey method
                            print("\n")
                            time.sleep(2)
                            method = "org.rdk.RDKWindowManager.1.generateKey"
                            value = '{ "clientId": "'+app_instance_id+'", "keys": [ { "keyCode": 13, "modifiers": [], "delay": 0.5 } ] }'
                            tdkTestObj = obj.createTestStep('appmanagers_setvalue')
                            tdkTestObj.addParameter("method",method)
                            tdkTestObj.addParameter("value",value)
                            tdkTestObj.executeTestCase(expectedResult)
                            ...
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                print("SUCCESS : Keycode applied successfully")
                                tdkTestObj.setResultStatus("SUCCESS")
                            else:
                                print("FAILURE : Failed to apply keycode")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
