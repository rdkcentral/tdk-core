...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Launch_App_GetZOrder_After_Terminate')
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            ...
                            result = tdkTestObj.getResultDetails()
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] > 0:
                                print("SUCCESS : Launched application is in top z-order")
                                tdkTestObj.setResultStatus("SUCCESS")
                                ...
                                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                    ...
                                    # Step 9 : Verify the z-order after application termination
                                    print("\n")
                                    time.sleep(5)
                                    method = "org.rdk.RDKWindowManager.1.getZOrder"
                                    value = '{"clientId": "'+app_instance_id+'"}'
                                    ...
                                    if "error" in result and result["error"]["message"] == "ERROR_GENERAL":
                                        print("SUCCESS : Launched application is not in top z-order after termination")
                                        tdkTestObj.setResultStatus("SUCCESS")
                                    else:
                                        print("FAILURE : Launched application is still in top z-order after termination")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    print("FAILURE : Failed to terminate the application")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("FAILURE : Launched application is not in top z-order")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
