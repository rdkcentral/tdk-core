...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Launch_App_SetVisible')
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                        ...
                        if launch_status == "TRUE":
                            # Step 7 : Set the application visibility to False and then True
                            for visible_state in [False, True]:
                                ...
                                result = ast.literal_eval(result)
                                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                    print("SUCCESS : Application visibility set to "+json.dumps(visible_state)+" successfully")
                                    ...
                                    print("FAILURE : Failed to set application visibility to "+ json.dumps(visible_state))
                                    tdkTestObj.setResultStatus("FAILURE")
                                    break
                        else:
                            print("FAILURE : Launched application is not in expected lifecycle state")
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("FAILURE : Failed to get the launched applications list")
                        tdkTestObj.setResultStatus("FAILURE")       
                else:
                    print("FAILURE : Application launch failed to initiate")
                    tdkTestObj.setResultStatus("FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
