...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Launch_App_Add_Remove_KeyIntercept')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            # Step 7 : Add key intercepts for the launched application
                            print("\n")
                            time.sleep(2)
                            ...
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                print("SUCCESS : Key intercepts added successfully")
                                tdkTestObj.setResultStatus("SUCCESS")

                                # Step 8 : Remove the key intercepts for the launched application
                                print("\n")
                                time.sleep(5)
                                ...
                                result = ast.literal_eval(result)
                                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                    print("SUCCESS : Key intercepts removed successfully")
                                    tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                    print("FAILURE : Failed to remove key intercepts")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("FAILURE : Failed to add key intercepts")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
        ...
        if launch_status == "TRUE":
            # Step 9 : Terminate the application
            print("\n")
            time.sleep(3)
