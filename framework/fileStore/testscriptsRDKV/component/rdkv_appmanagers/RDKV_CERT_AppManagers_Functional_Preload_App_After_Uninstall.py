...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Preload_App_After_Uninstall')
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            if preload_status == "TRUE":
                # Step 5 : Preload the application without installing the package and check for failure
                print("\n")
                time.sleep(3)
                method = "org.rdk.AppManager.1.preloadApp"
                value = '{"appId": "'+application_name+'"}'
                tdkTestObj = obj.createTestStep('appmanagers_setvalue')
                tdkTestObj.addParameter("method",method)
                tdkTestObj.addParameter("value",value)
                ...
                if "error" in result and "message" in result["error"]:
                    ...
                    if message_value == "ERROR_GENERAL":
                        print("SUCCESS : Preloading the application failed as expected since the application is uninstalled")
                        tdkTestObj.setResultStatus("SUCCESS")
                    else:
                        print("FAILURE : Preloading the application failed with unexpected error message: %s" %message_value)
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print("FAILURE : Preloading the application succeeded even after uninstallation or application is not present")
                    tdkTestObj.setResultStatus("FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
    else:
