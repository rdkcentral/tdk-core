...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_CloseApp_State_Transition_Active_To_Paused')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.DownloadManager": "onAppDownloadStatus"}','{"org.rdk.AppPackageManager": "onAppInstallationStatus"}','{"org.rdk.AppManager": "onAppLifecycleStateChanged"}']
            for item in events:
                            ...
                            if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                                ...
                                for app in result:
                                    if app["appId"] == application_name and app["lifecycleState"] == "APP_STATE_ACTIVE":
                                        print("SUCCESS : Application is in expected lifecycle state APP_STATE_ACTIVE")
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        app_status = "TRUE"
                                        break
                                ...
                                if app_status == "TRUE":
                                    ...
                                    result = ast.literal_eval(result)
                                    if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                        print("SUCCESS : Application close initiated successfully and waiting for lifecycle state change event...")
                                        tdkTestObj.setResultStatus("SUCCESS")

                                        # Wait for the lifecycle state change event and check the lifecycle state
                                        event_log = wait_for_event(event_listener)
                                        if len(event_log) > 0:
                                            for entry in event_log:
                                                _, json_part = entry.split("$$$", 1)
                                                ...
                                                newState = inner["newState"]
                                                errorReason = inner["errorReason"]
                                                if appId == application_name and newState == "APP_STATE_PAUSED" and errorReason == "APP_ERROR_NONE":
                                                    print("SUCCESS : Application close successful with correct lifecycle state transition from APP_STATE_ACTIVE to APP_STATE_PAUSED")
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    app_status = "TRUE"
                                                    break

                                            if app_status == "TRUE":
                                                # Step 9 : Check the application lifecycle state using getLoadedApps method
                                                print("\n")
                                                time.sleep(3)
                                                method = "org.rdk.AppManager.1.getLoadedApps"
                                                tdkTestObj = obj.createTestStep('appmanagers_getvalue')
                                                ...
                                                if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                                                    result = result["result"]
                                                    for app in result:
                                                        if app["appId"] == application_name and app["lifecycleState"] == "APP_STATE_PAUSED":
                                                            print("SUCCESS : Application is in expected lifecycle state APP_STATE_PAUSED after close")
                                                            tdkTestObj.setResultStatus("SUCCESS")
                                                            app_status = "TRUE"
                                                            break
                                                    ...
                                                    if app_status == "TRUE":
                                                        ...
                                                        if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                                            ...
                                                            print("FAILURE : Application termination failed")
                                                            tdkTestObj.setResultStatus("FAILURE")
                                                    else:
                                                        print("FAILURE : Application is not in expected lifecycle state APP_STATE_PAUSED after close")
                                                        tdkTestObj.setResultStatus("FAILURE")
                                            else:
                                                print("FAILURE : Application close failed or incorrect lifecycle state transition received in event")
                                                tdkTestObj.setResultStatus("FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                    else:
                                        print("FAILURE : Failed to initiate application close")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    print("FAILURE : Application is not in expected lifecycle state APP_STATE_ACTIVE")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
        ...
        if launch_status == "TRUE":
            # Step 11 : Check whether the launched application is present in loaded apps list before termination
            print("\n")
            time.sleep(3)
            method = "org.rdk.AppManager.1.getLoadedApps"
            tdkTestObj = obj.createTestStep('appmanagers_getvalue')
            ...
            if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                ...
                if terminate_status == "TRUE":
                    # Step 12 : Terminate the application
                    print("\n")
                    time.sleep(3)
                    method  = "org.rdk.AppManager.1.terminateApp"
                    value = '{"appId": "'+application_name+'"}'
