...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_Close_Already_Terminated_App')
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            ...
                            if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                ...
                                # Step 8 : Close the application which is already terminated and check for the expected error
                                print("\n")
                                time.sleep(3)
                                ...
                                if "error" in result and result["error"]["message"] == "ERROR_GENERAL":
                                    print("SUCCESS : closeApp returned expected error when trying to close a terminated application")
                                    tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                    print("FAILURE : closeApp did not return expected error when trying to close a terminated application")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
