...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_App_Kill_ContainerMode_Status_Check')
                    ...
                    if len(event_log) > 0:
                        ...
                        if launch_status == "TRUE":
                            ...
                            if container_status == "TRUE":
                                ...
                                result = ast.literal_eval(result)
                                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                                    print("SUCCESS : Application killed successfully")
                                    tdkTestObj.setResultStatus("SUCCESS")

                                    # Step 9 : Check the killed application is not running in container mode
                                    print("\n")
                                    time.sleep(3)
                                    container_status = "FALSE"
                                    command = "DobbyTool list"
                                    ...
                                    print("FAILURE : Failed to kill the application")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
