...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_GetInstalledApps_After_Uninstall')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.DownloadManager": "onAppDownloadStatus"}','{"org.rdk.PackageManagerRDKEMS": "onAppInstallationStatus"}']
            for item in events:
                ...
                if "error" not in result and "result" in result and result["result"] in (None, '', 'NONE'):
                    ...
                    if len(event_log) > 0:
                        for entry in event_log:
                            ...
                            version = inner[0]["version"]
                            state = inner[0]["state"]
                            if ( package_id == application_name and version == application_version and state == "UNINSTALLED" ):
                        ...
                        if uninstallation_status == "TRUE":
                            # Step 7 : Get the list of installed applications and check whether the uninstalled application is not present in the list
                            print("\n")
                            time.sleep(5)
                            ...
                            result = ast.literal_eval(result)
                            if "error" not in result and "result" in result and result["result"] not in (None, '', 'NONE'):
                                installed_apps = result["result"]
                                print("Installed Applications List : ", installed_apps)
                                app_found = "False"
                                for app in installed_apps:
                                    if app["appId"] != application_name:
                                        app_found = "True"
                                if app_found == "True":
                                    print("SUCCESS : Application uninstalled successfully and not found in the list of installed applications")
                                    tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                    print("FAILURE : Application uninstallation failed as the application is still found in the list of installed applications")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("FAILURE : Failed to get the list of installed applications")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
