...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_AppManagers_Functional_WayLand_OnConnected_Event');
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in result[1]:
        ...
        if "SUCCESS" in result:
            ...
            events = ['{"org.rdk.RDKWindowManager": "onConnected"}']
            for item in events:
                        ...
                        if "SUCCESS" in result:
                            ...
                            if wayland_mp4_md5sum_value in result:
                                ...
                                if "SUCCESS" in result:
                                    ...
                                    # Wait for the onConnected event
                                    ...
                                    if len(event_log) > 0:
                                        for entry in event_log:
                                            ...
                                            if appInstanceId == wayland_display_name:
                                                print("SUCCESS : Received onConnected event for the created display : ", appInstanceId)
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                event_status = "TRUE"
                                                break
                                    
                                        if event_status == "TRUE":
                                            # Step 8 : Check created display should not present in the list of apps after receiving onConnected event
                                            print("\n")
                                            time.sleep(5)
                                            method = "org.rdk.RDKWindowManager.1.getApps"
                                            tdkTestObj = obj.createTestStep('appmanagers_getvalue')
                                            ...
                                            print("FAILURE : onConnected event received but for different display or not received at all : ", appInstanceId)
                                            tdkTestObj.setResultStatus("FAILURE")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
