...
<xml>
  ...
  <name>RDKV_Telemetry_Event_UsedMemory_split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Device Used Memory Split report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_20</test_case_id>
    <test_objective>To send Device Used Memory Split report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Device Used Memory Split into server.
    ...
    5. Verify if device Device Used Memory Split is updated in cJSON report. </automation_approch>
    ...
    5. Device Used Memory Split must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Event_UsedMemory_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Event_UsedMemory_split');
...
if pre_requisite_set:
    event = "USED_MEM_split"
    profile_name = "RDKV-Profile-18"
    description = "Report to check Device Used Memory Split"
        ...
        if cJSON_line and component_name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    USED_MEM_split = data["Report"][0][name]
                    print("Device Used Memory Split:", USED_MEM_split)
                    print("SUCCESS : Able to obtain  \"Device Used Memory Split\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Device Used Memory Split\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Device Used Memory Split from cJSON report")
