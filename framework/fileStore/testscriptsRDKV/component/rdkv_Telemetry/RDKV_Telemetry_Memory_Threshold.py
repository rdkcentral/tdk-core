...
<xml>
  ...
  <name>RDKV_Telemetry_Memory_Threshold</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Free Memory Threshold report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_15</test_case_id>
    <test_objective>To send device Free Memory Threshold report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Free Memory Threshold into server.
    ...
    5. Verify if device Free Memory Threshold is updated in cJSON report. </automation_approch>
    ...
    5. Device Free Memory Threshold must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Memory_Threshold</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Memory_Threshold');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.MemoryStatus.X_RDKCENTRAL-COM_FreeMemThreshold"
    profile_name = "RDKV-Profile-12"
    description = "Report to check Free Memory Threshold"
    name = "FrMemThrshld"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Free Memory Threshold in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Free_Memory_Threshold = data["Report"][0][name]
                    print("Free Memory Threshold:", Free_Memory_Threshold)
                    print("SUCCESS : Able to obtain  \"Free Memory Threshold\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Free Memory Threshold\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Free Memory Threshold from cJSON report")
