...
<xml>
  ...
  <name>RDKV_Telemetry_ClearResetCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send clear reset count report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_06</test_case_id>
    <test_objective>To send clear reset count report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report clear reset count into server.
    ...
    5. Verify if clear reset count is updated in cJSON report. </automation_approch>
    ...
    5. Clear reset count must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_ClearResetCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_ClearResetCount');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.ClearResetCount"
    profile_name = "RDKV-Profile-3"
    description = "Report to check Clear Reset count"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find clear reset count in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    reset_count = data["Report"][0][name]
                    print("Clear Reset Count:", reset_count)
                    print("SUCCESS : Able to obtain  \"Clear Reser Count\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Clear Reser Count\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Clear Reser Count from cJSON report")
