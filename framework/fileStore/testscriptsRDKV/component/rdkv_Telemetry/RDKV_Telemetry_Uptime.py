...
<xml>
  ...
  <name>RDKV_Telemetry_Uptime</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send uptime report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_04</test_case_id>
    <test_objective>To send uptime report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report uptime into server.
    ...
    5. Verify if Uptime is updated in cJSON report.
    6. Get uptime from /proc/uptime file in DUT.
    7. Verify if uptime from cJSON report is correct by validating against /proc/uptime value.</automation_approch>
    ...
    5. Uptime must be successfully retrieved from cJSON report.
    6. Uptime must be successfully retrieved from /proc/uptime.
    7. difference between cJSON reported uptime and /proc/uptime must be less than 60 seconds.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Uptime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Uptime');
...
if pre_requisite_set:
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find uptime in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    uptime = data["Report"][0][name]
                    print("Uptime:", uptime)
                    print("SUCCESS : Able to obtain  \"Uptime\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Uptime\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Uptime from cJSON report")

        if uptime:
            print("\n[TEST STEP 6] :Obtain uptime from /proc/uptime")
            tdkTestObj = obj.createTestStep('telemetry_executeCmdInDUT');
            command = "awk '{print int($1)}' /proc/uptime"
            tdkTestObj.addParameter("command", command)
            tdkTestObj.executeTestCase("SUCCESS");
            details = tdkTestObj.getResultDetails()
            proc_uptime = details.splitlines()[1].strip("\r\n")

            diff = int(proc_uptime) - int(uptime)

            print("\n[TEST STEP 7] :Verifying if cJSON report uptime value is correct")
            print("Difference between both uptime is ", diff)
            if diff < 60:
                print ("SUCCESS : difference between both uptime is less than 60 seconds")
                print ("\n[TEST STEP RESULT] : SUCCESS\n");
                tdkTestObj.setResultStatus("SUCCESS");
            else:
                print ("FAILURE : difference between both uptime is more than 60 seconds")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
