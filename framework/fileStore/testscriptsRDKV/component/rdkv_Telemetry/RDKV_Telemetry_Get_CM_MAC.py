...
<xml>
  ...
  <name>RDKV_Telemetry_Get_CM_MAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Device CM MAC report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_18</test_case_id>
    <test_objective>To send Device CM MAC report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Device CM MAC into server.
    ...
    5. Verify if device Device CM MAC is updated in cJSON report. </automation_approch>
    ...
    5. Device CM MAC must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Get_CM_MAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Get_CM_MAC');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.X_COMCAST-COM_CM_MAC"
    profile_name = "RDKV-Profile-16"
    description = "Report to check Device CM MAC"
    name = "CmMac"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Device CM MAC in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    CM_MAC = data["Report"][0][name]
                    print("Device CM MAC:", CM_MAC)
                    print("SUCCESS : Able to obtain  \"Device CM MAC\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Device CM MAC\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Device CM MAC from cJSON report")
