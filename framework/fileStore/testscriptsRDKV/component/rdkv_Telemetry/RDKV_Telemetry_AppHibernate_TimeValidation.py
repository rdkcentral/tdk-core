...
obj.configureTestCase(ip,port,'RDKV_Telemetry_AppHibernate_TimeValidation')
...
event_name = "RDKAMHibernateTime_split"
component_name = "AppManager"
profile_name = "RDKV-AppHibernateProfile"
description = "Report to validate App Hibernate Time"
...
if profile_set:
    ...
    print("\n[TEST STEP5] : Hibernate Application")

    result = rdkservice_setValue("org.rdk.LifecycleManager.1.setTargetAppState",'{"appInstanceId":"' + app_instance_id +'","targetLifecycleState":"HIBERNATED"}')
    print(result)
    ...
    while timeout > 0:
        ...
        if resume_app in event and ("APP_STATE_BACKGROUND" in event or "APP_STATE_SUSPENDED" in event):
            suspended = True
            print("SUCCESS : Application Hiberanted")
            break
        if not suspended:
            print("FAILURE : Hiberante event not received")
