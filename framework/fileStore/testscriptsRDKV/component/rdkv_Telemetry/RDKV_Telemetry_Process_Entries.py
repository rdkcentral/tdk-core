...
<xml>
  ...
  <name>RDKV_Telemetry_Process_Entries</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Number of process Enteries report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_23</test_case_id>
    <test_objective>To send device Number of process Enteries report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Number of process Enteries into server.
    ...
    5. Verify if device Number of process Enteries is updated in cJSON report. </automation_approch>
    ...
    5. Device Number of process Enteries must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Process_Entries</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Process_Entries');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.ProcessStatus.ProcessNumberOfEntries"
    profile_name = "RDKV-Profile-15"
    description = "Report to check Number of process Enteries"
    name = "ProcessNumberOfEntries"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Number of process Enteries in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Process_Enteries = data["Report"][0][name]
                    print("Number of process Enteries:", Process_Enteries)
                    print("SUCCESS : Able to obtain  \"Number of process Enteries\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Number of process Enteries\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Number of process Enteries from cJSON report")
