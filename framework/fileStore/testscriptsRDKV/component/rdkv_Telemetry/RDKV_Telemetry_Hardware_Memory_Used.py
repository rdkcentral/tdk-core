...
<xml>
  ...
  <name>RDKV_Telemetry_Hardware_Memory_Used</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Hardware_Memory_Used report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_10</test_case_id>
    <test_objective>To send device Hardware_Memory_Used report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Hardware_Memory_Used into server.
    ...
    5. Device Hardware_Memory_Used must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Hardware_Memory_Used</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Hardware_Memory_Used');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.Hardware_Memory_Used"
    profile_name = "RDKV-Profile-7"
    description = "Report to check Hardware Memory Used"
    name = "MemUsed"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Hardware_Memory_Used in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Hardware_Memory_Used = data["Report"][0][name]
                    print("Hardware Memory Used:", Hardware_Memory_Used)
                    print("SUCCESS : Able to obtain  \"Hardware Memory Used\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Hardware Memory Used\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Hardware Memory Used from cJSON report")
