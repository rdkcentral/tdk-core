...
<xml>
  ...
  <name>RDKV_Telemetry_Event_FreeMemory_Split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Device Free Memory Split report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_19</test_case_id>
    <test_objective>To send Device Free Memory Split report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Device Free Memory Split into server.
    ...
    5. Verify if device Device Free Memory Split is updated in cJSON report. </automation_approch>
    ...
    5. Device Free Memory Split must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Event_FreeMemory_Split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Event_FreeMemory_Split');
...
if pre_requisite_set:
    event = "FREE_MEM_split"
    profile_name = "RDKV-Profile-17"
    description = "Report to check Device Free Memory Split"
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    FREE_MEM_split = data["Report"][0][name]
                    print("Device Free Memory Split:", FREE_MEM_split)
                    print("SUCCESS : Able to obtain  \"Device Free Memory Split\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Device Free Memory Split\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Device Free Memory Split from cJSON report")
