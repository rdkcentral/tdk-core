...
obj.configureTestCase(ip,port,'RDKV_Telemetry_AppSuspend_TimeValidation')
...
event_name = "RDKAMSuspendTime_split"
component_name = "AppManager"
profile_name = "RDKV-AppSuspendProfile"
description = "Report to validate App Suspend Time"
...
if profile_set:
    ...
    tdkTestObj = obj.createTestStep('telemetry_executeCmdInDUT')
    monitor_cmd = ('rm -f /tmp/Suspend_monitor.log; ' 'tail -F /opt/logs/telemetry2_0.txt > ''/tmp/Suspend_monitor.log 2>&1 & echo $!')
    ...
    while timeout > 0:
        ...
        if resume_app in event and ("APP_STATE_BACKGROUND" in event or "APP_STATE_SUSPENDED" in event):
            suspended = True
            print("SUCCESS : Application suspended")
            break
        if not suspended:
            print("FAILURE : Suspend event not received")
            event_listener.disconnect()
            obj.unloadModule("rdkv_telemetry")
            exit()
            ...
            try:
                ...
                if match:
                    ...
                    if len(report) == 0:
                        ...
                        print("\nApp Suspend Telemetry Data :")
                        print(result_data)
                        print("SUCCESS : App Suspend telemetry data found")
                ...
                print("FAILURE : Unable to obtain Suspend telemetry data")
                print(e)
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
