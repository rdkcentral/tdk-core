...
<xml>
  ...
  <name>RDKV_Telemetry_Enable_Speedtest</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Speedtest Enable report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_25</test_case_id>
    <test_objective>To send device Speedtest Enable or not report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Speedtest Enable into server.
    ...
    5. Verify if device Speedtest Enable is updated in cJSON report. </automation_approch>
    ...
    5. Device Speedtest Enable must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Enable_Speedtest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Enable_Speedtest');
...
if pre_requisite_set:
    param = "Device.IP.Diagnostics.X_RDKCENTRAL-COM_SpeedTest.Enable_Speedtest"
    profile_name = "RDKV-Profile-23"
    description = "Report to check Speedtest Enable"
    name = "SpeedtestEnable"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Speedtest Enable in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Speedtest_Enable = data["Report"][0][name]
                    print("Speedtest Enable:", Speedtest_Enable)
                    print("SUCCESS : Able to check whether Speedtest is enabled or not")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to check whether Speedtest is enabled or not")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to check whether Speedtest is enabled or not from cJSON report")
