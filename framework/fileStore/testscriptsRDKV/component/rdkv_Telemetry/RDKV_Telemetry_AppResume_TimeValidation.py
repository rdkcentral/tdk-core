...
obj.configureTestCase(ip,port,'RDKV_Telemetry_AppResume_TimeValidation')
...
event_name = "RDKAMAppResumeTime_split"
component_name = "AppManager"
profile_name = "RDKV-AppResumeProfile"
description = "Report to validate App Resume Time"
...
if profile_set:
    ...
    tdkTestObj = obj.createTestStep('telemetry_executeCmdInDUT')
    monitor_cmd = ('rm -f /tmp/Resume_monitor.log; ' 'tail -F /opt/logs/telemetry2_0.txt > ''/tmp/Resume_monitor.log 2>&1 & echo $!')
    ...
    if status == "SUCCESS":
        print("SUCCESS : Resume request sent")
        print("\nVerifying Application Resume")
        ...
        while timeout > 0:
            ...
            if resume_app in event and "APP_STATE_ACTIVE" in event:
                resumed = True
                print("SUCCESS : Application resumed")
                break
            if not resumed:
                print("FAILURE : Resume event not received")
                event_listener.disconnect()
                obj.unloadModule("rdkv_telemetry")
                exit()
    else:
	    print("FAILURE : Resume request failed")
            ...
            try:
                ...
                if match:
                    ...
                    if len(report) == 0:
                        ...
                        print("\nApp Resume Telemetry Data :")
                        print(result_data)
                        print("SUCCESS : App Resume telemetry data found")
                ...
                print("FAILURE : Unable to obtain Resume telemetry data")
                print(e)
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
