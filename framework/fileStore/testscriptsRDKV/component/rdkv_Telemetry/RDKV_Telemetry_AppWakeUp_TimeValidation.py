...
obj.configureTestCase(ip,port,'RDKV_Telemetry_AppWakeUp_TimeValidation')
...
event_name = "RDKAMWakeTime_split"
component_name = "AppManager"
profile_name = "RDKV-AppWakeUpProfile"
description = "Report to validate App WakeUp Time"



app_bundle_1 = PerformanceTestVariables.google_bundle
resume_app = app_bundle_1.split('+')[0]
...
if profile_set:
    ...
    tdkTestObj = obj.createTestStep('telemetry_executeCmdInDUT')
    monitor_cmd = ('rm -f /tmp/WakeUp_monitor.log; ' 'tail -F /opt/logs/telemetry2_0.txt > ''/tmp/WakeUp_monitor.log 2>&1 & echo $!')
    ...
    print("\nLaunching Resume App")
    tdkTestObj = obj.createTestStep('rdkservice_launch_app')
    tdkTestObj.addParameter("app_name", resume_app)
    tdkTestObj.executeTestCase(expectedResult)

    time.sleep(10)
    print("\nGetting Loaded Apps")
    loaded_apps = rdkservice_getValue("org.rdk.AppManager.1.getLoadedApps")
    print(loaded_apps)
    app_instance_id = ""
    try:
        apps = loaded_apps["apps"]
        for app in apps:
            if app["appId"] == resume_app:
                app_instance_id = app["appInstanceId"]
                break
    except Exception as e:
        print(e)
        print("App Instance Id :", app_instance_id)
    ...
    while timeout > 0:
        ...
        if resume_app in event and ("APP_STATE_BACKGROUND" in event or "APP_STATE_SUSPENDED" in even):
            background = True
        ...
        if not background:
            ...
            exit()
    print("\n[TEST STEP5] : Wake Application")
    result = rdkservice_setValue("org.rdk.LifecycleManager.1.setTargetAppState",'{"appInstanceId":"' + app_instance_id +'","targetLifecycleState":"ACTIVE"}')
    print(result)

    status = "SUCCESS"

    if status == "SUCCESS":
        print("\nVerifying Application Wake")
        woken = False
        timeout = 120
        while timeout > 0:
            if len(event_listener.getEventsBuffer()) == 0:
            ...
            event = event_listener.getEventsBuffer().pop(0)
            print(event)
            if resume_app in event and "APP_STATE_ACTIVE" in event:
                woken = True
                print("SUCCESS : Application woke up")
                break
            if not woken:
                print("FAILURE : App was not Woke up")
                event_listener.disconnect()
                obj.unloadModule("rdkv_telemetry")
                exit()
    else:
	    print("FAILURE : Wake Up request failed")
            ...
            try:
                ...
                if match:
                    ...
                    if len(report) == 0:
                        ...
                        print("\nApp Wake UP Telemetry Data :")
                        print(result_data)
                        print("SUCCESS : App Wake Up telemetry data found")
                ...
                print("FAILURE : Unable to obtain Wake Up telemetry data")
                print(e)
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
