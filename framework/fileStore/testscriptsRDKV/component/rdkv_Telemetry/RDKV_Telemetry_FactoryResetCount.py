...
<xml>
  ...
  <name>RDKV_Telemetry_FactoryResetCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Factory_Reset_Count report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_08</test_case_id>
    <test_objective>To send device Factory_Reset_Count report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Factory_Reset_Count into server.
    ...
    5. Verify if device Factory_Reset_Count is updated in cJSON report. </automation_approch>
    ...
    5. Device Factory_Reset_Count must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_FactoryResetCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_FactoryResetCount');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.FactoryResetCount"
    profile_name = "RDKV-Profile-5"
    description = "Report to check Factory Reset Count"
    name = "FactoryResetCount"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Factory_Reset_Count in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Factory_Reset_Count = data["Report"][0][name]
                    print("Factory Reset Count:", Factory_Reset_Count)
                    print("SUCCESS : Able to obtain  \"Factory Reset Count\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Factory Reset Count\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Factory Reset Count from cJSON report")
