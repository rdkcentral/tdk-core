...
<xml>
  ...
  <name>RDKV_Telemetry_Total_MemoryStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Total_Memory_Status report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_13</test_case_id>
    <test_objective>To send device Total_Memory_Status report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Total_Memory_Status into server.
    ...
    5. Verify if device Total_Memory_Status is updated in cJSON report. </automation_approch>
    ...
    5. Device Total_Memory_Status must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Total_MemoryStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Total_MemoryStatus');
...
if pre_requisite_set:
    ...
    profile_name = "RDKV-Profile-10"
    description = "Report to check Total Memory status"
    name = "MemStatTotal"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Total_Memory_Status in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Total_Memory_Status = data["Report"][0][name]
                    print("Total_Memory_Status:", Total_Memory_Status)
                    print("SUCCESS : Able to obtain  \"Total_Memory_Status\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Total_Memory_Status\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Total_Memory_Status from cJSON report")
