...
<xml>
  ...
  <name>RDKV_Telemetry_CPUUsage</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device CPU Usage report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_16</test_case_id>
    <test_objective>To send device CPU Usage report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device CPU Usage into server.
    ...
    5. Verify if device CPU Usage is updated in cJSON report. </automation_approch>
    ...
    5. Device CPU Usage must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_CPUUsage</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_CPUUsage');
...
if pre_requisite_set:
    ...
    profile_name = "RDKV-Profile-13"
    description = "Report to check CPU Usage"
    name = "CpuUsage"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device CPU Usage in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    CPU_Usage = data["Report"][0][name]
                    print("CPU Usage:", CPU_Usage)
                    print("SUCCESS : Able to obtain  \"CPU Usage\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"CPU Usage\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get CPU Usage from cJSON report")
