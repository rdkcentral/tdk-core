...
<xml>
  ...
  <name>RDKV_Telemetry_Event_CPUInfo_Split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Device CPU Info Split report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_21</test_case_id>
    <test_objective>To send Device CPU Info Split report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Device CPU Info Split into server.
    ...
    5. Verify if device Device CPU Info Split is updated in cJSON report. </automation_approch>
    ...
    5. Device CPU Info Split must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Event_CPUInfo_Split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Event_CPUInfo_Split');
...
if pre_requisite_set:
    event = "cpuinfo_split"
    profile_name = "RDKV-Profile-19"
    description = "Report to check Device CPU Info Split"
    component_name = "sysint"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Device CPU Info Split in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and component_name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    CPUInfo_Split = data["Report"][0][name]
                    print("Device CPU Info Split:", CPUInfo_Split)
                    print("SUCCESS : Able to obtain  \"Device CPU Info Split\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Device CPU Info Split\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Device CPU Info Split from cJSON report")
