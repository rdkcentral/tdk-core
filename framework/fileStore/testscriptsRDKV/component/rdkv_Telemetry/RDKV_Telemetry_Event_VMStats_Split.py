...
<xml>
  ...
  <name>RDKV_Telemetry_Event_VMStats_Split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Device VM Stats Split report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_22</test_case_id>
    <test_objective>To send Device VM Stats Split report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Device VM Stats Split into server.
    ...
    5. Verify if device Device VM Stats Split is updated in cJSON report. </automation_approch>
    ...
    5. Device VM Stats Split must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Event_VMStats_Split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Event_VMStats_Split');
...
if pre_requisite_set:
    event = "vmstats_split"
    profile_name = "RDKV-Profile-20"
    description = "Report to check Device VM Stats Split"
