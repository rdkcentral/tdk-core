...
<xml>
  ...
  <name>Rdkfwupgrader_GetRemoteVers_NULL_param</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the behavior when a NULL parameter is passed in the GetRemoteVers function.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADER_81</test_case_id>
    <test_objective>To verify the behavior when a NULL parameter is passed in the GetRemoteVers function.</test_objective>
    ...
    2. RDK_fwupgradeAgent will invoke GetRemoteVers API with the NULL param being passed to pRemoteVers param.
    ...
    <test_script>Rdkfwupgrader_GetRemoteVers_NULL_param</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetRemoteVers_NULL_param');
...
if "SUCCESS" in result.upper():
    ...
    if result == "FAILURE" and "crash" not in details:
        print("SUCCESS : GetRemoteVers failed as expected for null param")
        tdkTestObj.setResultStatus("SUCCESS")
    elif "crash" in details:
        print("FAILURE : GetRemoteVers crashes when called with null param")
        tdkTestObj.setResultStatus("FAILURE")
        obj.initiateReboot();
    else:
        print("FAILURE : GetRemoteVers is success for null param which is not expected")
        tdkTestObj.setResultStatus("FAILURE")
