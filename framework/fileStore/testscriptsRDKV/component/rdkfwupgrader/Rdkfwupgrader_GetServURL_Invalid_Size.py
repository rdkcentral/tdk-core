...
<xml>
  ...
  <name>Rdkfwupgrader_GetServURL_Invalid_Size</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>rdkfwupgrader_GetServURL</primitive_test_name>
  <!--  -->
  <primitive_test_version>2</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass Invalid string buffer size with control characters and spaces to rdkfwupgrader API GetServURL and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_94</test_case_id>
    <test_objective>To pass Invalid string buffer size with control characters and spaces to rdkfwupgrader API GetServURL and verify output</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Video Accelerator,RPI</test_setup>
    <pre_requisite></pre_requisite>
    <api_or_interface_used>GetServURL</api_or_interface_used>
    ...
    3. RDK_fwupgradeAgent will invoke GetServURL API with testurl data along with invalid buffer size.
    ...
    <expected_output>API must not scans URL data since the invalid buffer has been passed</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_GetServURL_Invalid_Size</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetServURL_Invalid_Size');
sysUtilObj.configureTestCase(ip,port,'Rdkfwupgrader_GetServURL_Invalid_Size');
...
serverURL_test_filepath = "/tmp/stateredrecovry.conf";
testurl = "https://www.statered.com";

def createTestFile():
    print("\n")
    tdkTestObj = sysUtilObj.createTestStep('ExecuteCommand');
    #Create test config file under tmp dir
    cmd = "touch " + serverURL_test_filepath + " ;"
    #List files to cross verify the file
    cmd = cmd + "ls " + serverURL_test_filepath;
    tdkTestObj.addParameter("command", cmd);
    tdkTestObj.executeTestCase("SUCCESS");
    details = tdkTestObj.getResultDetails()
    if "stateredrecovry.conf" in details:
        print("Test file created successfully");
        tdkTestObj = sysUtilObj.createTestStep('ExecuteCommand');
        cmd = "echo " + testurl + " > " + serverURL_test_filepath + " ;"
        cmd = cmd + "echo " + "$(cat " + serverURL_test_filepath + ")"; 
        tdkTestObj.addParameter("command", cmd);
        tdkTestObj.executeTestCase("SUCCESS");
        details = tdkTestObj.getResultDetails().strip().replace(r'\n', '\n');
        print("details", details);
        print("test_url_data", testurl);
        if testurl in details:
...
def deleteTestFile():
    ...
    cmd = " rm -rf " + serverURL_test_filepath + " ;"
    #Command to cross verify if directory is deleted
    cmd = cmd + 'if [ -d ' + serverURL_test_filepath +' ]; then echo "Files exists"; else echo "File does not exist"; fi'
    tdkTestObj.addParameter("command", cmd);
    tdkTestObj.executeTestCase("SUCCESS");
    details = tdkTestObj.getResultDetails();
...
if "SUCCESS" in result.upper() and "SUCCESS" in sysUtilLoadStatus.upper():
    if createTestFile():
        ...
        tdkTestObj = obj.createTestStep('rdkfwupgrader_GetServURL');
        ...
        if result == "SUCCESS":
            if test_url_data in details: 
                print("Buffer size is too small to hold the input URL. So, it shouldn't scans URL from a test file. But here it does.");
                tdkTestObj.setResultStatus("FAILURE");
            else:
                print("URL not copied into buffer as expected since buffer size is 0.");
                print("Expected Result : ", test_url_data);
                print("Actual Result : ", details.splitlines());
                tdkTestObj.setResultStatus("SUCCESS");
        else:
            print ("FAILURE : GetServURL failed");
            tdkTestObj.setResultStatus("FAILURE");
        deleteTestFile();
