...
<xml>
  ...
  <name>Rdkfwupgrader_GetFileContents_DoublePtr_NULL_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass NULL URL param to rdkfwupgrader API GetFileContents double ptr param and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_84</test_case_id>
    <test_objective>To pass NULL URL param to rdkfwupgrader API GetFileContents double ptr param and verify output</test_objective>
    ...
    3. RDK_fwupgradeAgent will invoke GetServerUrlFile API with the NULL param being passed to pOut param.
    4. TM will verify the output by having a expected output as invalid param and verify.
    ...
    <test_script>Rdkfwupgrader_GetFileContents_DoublePtr_NULL_Check</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetFileContents_DoublePtr_NULL_Check');
sysUtilObj.configureTestCase(ip,port,'Rdkfwupgrader_GetFileContents_DoublePtr_NULL_Check');
...
if "SUCCESS" in result.upper() and "SUCCESS" in sysUtilLoadStatus.upper():
    if createTestFile():
        ...
        tdkTestObj.addParameter("filename_null_check", 0);
        ...
        if result == "FAILURE" and "crash" not in details:
            print("SUCCESS : GetFileContents failed as expected for null param")
            tdkTestObj.setResultStatus("SUCCESS")
        elif "crash" in details:
            print("FAILURE : GetFileContents crashes when called with null param")
            tdkTestObj.setResultStatus("FAILURE")
            obj.initiateReboot();
        else:
            print("FAILURE : GetFileContents is success for null param which is not expected")
            tdkTestObj.setResultStatus("FAILURE")
