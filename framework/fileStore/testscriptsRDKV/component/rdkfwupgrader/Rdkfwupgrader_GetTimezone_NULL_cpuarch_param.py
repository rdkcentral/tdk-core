...
<xml>
  ...
  <name>Rdkfwupgrader_GetTimezone_NULL_cpuarch_param</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass NULL cpuarchitecture param to rdkfwupgrader API GetTimezone and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_37</test_case_id>
    <test_objective>To pass NULL cpuarchitecture param to rdkfwupgrader API GetTimezone and verify output</test_objective>
    ...
    cpuArch - pointer to a const char buffer to store the cpu arch input string</input_parameters>
    <automation_approch>1. TM loads the RDK_fwupgradeAgent
    2. RDK_fwupgradeAgent will invoke GetTimezone API with the NULL param being passed to cpuArch param.
    ...
    <test_script>Rdkfwupgrader_GetTimezone_NULL_cpuarch_param</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetTimezone_NULL_cpuarch_param');
...
if "SUCCESS" in result.upper():
    if True:
        ...
        tdkTestObj.addParameter("cpuarch_null_param_check", 1);
        ...
        if result == "FAILURE" and "crash" not in details:
            print("SUCCESS : GetTimezone failed as expected for cpuarch param as null")
            tdkTestObj.setResultStatus("SUCCESS")
        elif "crash" in details:
            print("FAILURE : GetTimezone crashes when called with cpuarch param as null")
            tdkTestObj.setResultStatus("FAILURE")
            obj.initiateReboot();
        else:
            print("FAILURE : GetTimezone is success for null param which is not expected")
            tdkTestObj.setResultStatus("FAILURE")
