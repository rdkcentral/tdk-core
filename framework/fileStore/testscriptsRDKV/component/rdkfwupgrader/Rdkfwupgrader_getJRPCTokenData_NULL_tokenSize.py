...
<xml>
  ...
  <name>Rdkfwupgrader_getJRPCTokenData_NULL_tokenSize</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To try to obtain tokenSize from testString via rdkfwupgrader API getJRPCTokenData with NULL tokenSize and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_16</test_case_id>
    <test_objective>To try to obtain tokenSize from testString via rdkfwupgrader API getJRPCTokenData with NULL tokenSize and verify output</test_objective>
    ...
    <expected_output>API must FAIL due to NULL tokenSize being passed</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_getJRPCTokenData_NULL_tokenSize</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_getJRPCTokenData_NULL_tokenSize');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj.addParameter("null_tokenSize", 1)
    ...
    if result == "SUCCESS" and not details:
        print ("SUCCESS : Validation success for NULL tokenSize");
        tdkTestObj.setResultStatus("SUCCESS")
    else:
        print ("FAILURE : Validation unsuccessfull")
        print ("Expected test Result : getJRPCTokenData must return empty token due to NULL tokenSize")
