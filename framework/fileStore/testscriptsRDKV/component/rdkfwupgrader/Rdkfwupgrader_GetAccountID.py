...
<xml>
  ...
  <name>Rdkfwupgrader_GetAccountID</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the retrieval of the accoundID of the device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_58</test_case_id>
    <test_objective>To verify the retrieval of the account ID of the device</test_objective>
    ...
    2. RDK_fwupgradeAgent will invoke GetAccountID API with the valid buffer size along with pointer variable to hold the accound ID.
    ...
    <expected_output>API must return account ID of the device</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_GetAccountID</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetAccountID');
...
if "SUCCESS" in result.upper():
    ...
    if result == "SUCCESS":
        if not details:
            print("Retrieved empty accound ID field. Should return the proper accound ID of the device.");
            tdkTestObj.setResultStatus("FAILURE");
        else:
            print("Successfully returns the device's account ID");
            tdkTestObj.setResultStatus("SUCCESS");
    else:
        print ("FAILURE : GetAccountID failed");
        tdkTestObj.setResultStatus("FAILURE");
