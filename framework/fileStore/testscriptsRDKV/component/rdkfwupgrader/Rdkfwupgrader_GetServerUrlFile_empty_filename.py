...
<xml>
  ...
  <name>Rdkfwupgrader_GetServerUrlFile_empty_filename</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass empty filename to rdkfwupgrader API GetServerUrlFile and verify it returns failure</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_33</test_case_id>
    <test_objective>To pass empty filename to rdkfwupgrader API GetServerUrlFile and verify it returns failure</test_objective>
    ...
    2. RDK_fwupgradeAgent will invoke GetServerUrlFile API with the empty string being passed to pFileName param.
    ...
    <expected_output>API must fail since the input filename param was empty</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_GetServerUrlFile_empty_filename</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetServerUrlFile_empty_filename');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj.addParameter("filename", "");
    ...
    if result == "SUCCESS":
        print("GetServerUrlFile API shouldn't succeed since the input Filename param is empty");
        tdkTestObj.setResultStatus("FAILURE");
    else:
        print ("GetServerUrlFile API failed as expected since the input Filename param is empty");
