...
<xml>
  ...
  <name>Rdkfwupgrader_makeHttpHttps_empty_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass empty url to rdkfwupgrader API makeHttpHttps and verify it returns failure</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_04</test_case_id>
    <test_objective>To pass empty url to rdkfwupgrader API makeHttpHttps and verify it returns failure</test_objective>
    ...
    2. RDK_fwupgradeAgent will invoke makeHttpHttps API with empty URL.
    ...
    <expected_output>makeHttpHttps must return failure as string is empty</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_makeHttpHttps_empty_URL</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_makeHttpHttps_empty_URL');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj.addParameter("string", "")
    tdkTestObj.addParameter("string_size", 100)
    ...
    if result == "FAILURE":
        print ("SUCCESS : makeHttpHttps returned FAILURE as expected for empty URL");
