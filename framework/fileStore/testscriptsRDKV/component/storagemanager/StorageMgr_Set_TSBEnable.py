...
<xml>
  ...
  <name>StorageMgr_Set_TSBEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to set TSB as enable</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.Invoke isTSBEnabled API and verify TSB is enable or not
3.If TSB is already enabled Script will success
4.Else invoke setTSBEnabled and set TSB as enabled</automation_approch>
    <expected_output>Should return TSB as enabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>libstoragemanagerstub.so.0.0.0</test_stub_interface>
    <test_script>StorageMgr_Set_TSBEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'StorageMgr_Set_TSBEnable');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    if expectedResult in actualResult and "true" in details:
        ...
        if expectedResult in actualResult:
            ...
            print("EXPECTED OUTPUT : Should get TSB as enabled")
            ...
            if expectedResult in actualResult and "true" in details: 
                tdkTestObj.setResultStatus("SUCCESS");
                print("ACTUAL RESULT : Setting TSB as enable was success")
                print("[TEST EXECUTION RESULT] : SUCCESS")
            else:
