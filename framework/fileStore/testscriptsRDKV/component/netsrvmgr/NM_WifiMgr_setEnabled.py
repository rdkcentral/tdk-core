...
<xml>
  ...
  <name>NM_WifiMgr_setEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To enable wifi adapter
Test CaseID:CT_NET_SRV_MGR_WIFI_5
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_35</test_case_id>
    <test_objective>To enable wifi adapter</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video_Accelerator</test_setup>
    <pre_requisite>1. netSrvMgr should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_setEnabled)</api_or_interface_used>
    <input_parameters>bool enable = True</input_parameters>
    <automation_approch>1. TM loads the NetSrvMgr_Agent via the test agent.
2.NetSrvMgr_Agent will enable the wifi adapter.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if wifi adapter is enabled successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_setEnabled</test_script>
    <skipped>No</skipped>
