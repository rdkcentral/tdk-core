...
<xml>
  ...
  <name>NM_WifiMgr_connect_without_password_and_securitymode_WPA</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To connect to a wifi network with the given ssid without any password and WPA security mode
Test CaseID:CT_NM_10
...
char *-ssid
password - NULL
security mode - NET_WIFI_SECURITY_WEP_64</input_parameters>
    <automation_approch>1. TM loads the NetSrvMgr_Agent via the test agent.
2. NetSrvMgr_Agent clears the saved SSIDs
3.NetSrvMgr_Agent will try to connect to the given wifi network without any password.
4.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should not connect to the network without any password</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_connect_without_password_and_securitymode_WPA</test_script>
    <skipped>No</skipped>
