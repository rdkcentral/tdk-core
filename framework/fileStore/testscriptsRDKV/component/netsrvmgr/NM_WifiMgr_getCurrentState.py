...
<xml>
  ...
  <name>NM_WifiMgr_getCurrentState</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>NetSrvMgr_WifiMgr_GetCurrentState</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To get current state of Wifi manager
Test CaseID:CT_NET_SRV_MGR_WIFI_2
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_21</test_case_id>
    <test_objective>To get current state of Wifi manager</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video_Accelerator</test_setup>
    <pre_requisite>1. netSrvMgr should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_getCurrentState)</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. TM loads the NetSrvMgr_Agent via the test agent.
2.NetSrvMgr_Agent will get current state of the Wifi manager.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if current wifi manager state is retrieved successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_getCurrentState</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_getCurrentState');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getCurrentState');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                print("GetCurrentState : %s" %actualresult);
                print("Details: [%s]"%details);

                #Set the result status of execution
                if (expectedresult in actualresult) and (WIFI_STATUS['FAILED'] not in details):
