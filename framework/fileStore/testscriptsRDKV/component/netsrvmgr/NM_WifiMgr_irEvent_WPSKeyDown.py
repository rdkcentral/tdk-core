...
<xml>
  ...
  <name>NM_WifiMgr_irEvent_WPSKeyDown</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To broadcast WPS key down IR event and check if netsrvmgr process receives it
Test CaseID:CT_NM_30
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_30</test_case_id>
    <test_objective>To broadcast WPS key down IR event and check if netsrvmgr process receives it</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_BroadcastEvent(IARM_BUS_IRMGR_NAME,IARM_BUS_IRMGR_EVENT_IRKEY,(void*)&amp;eventData,sizeof(eventData))
IARM_Bus_Disconnect()
...
4. netsrvmgr_agent broadcast the event IARM_BUS_IRMGR_EVENT_IRKEY to netsrvmgr.
...
9. IARMBUS_Agent deregisters from the IARM Bus Daemon.</automation_approch>
    ...
    <test_script>NM_WifiMgr_irEvent_WPSKeyDown</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_irEvent_WPSKeyDown');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            nmObj.configureTestCase(ip,port,'NM_WifiMgr_irEvent_WPSKeyDown');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in nmLoadStatus.upper():
                ...
                tdkTestObj.addParameter("owner", "IRMgr");
                tdkTestObj.addParameter("event_id", 0);
                tdkTestObj.addParameter("event_log","Received Key info [Type : 32768; Code : 240; isFP : 1 ]");
                tdkTestObj.addParameter("key_type", 0x00008000UL);
                tdkTestObj.addParameter("key_code", 0x000000F0UL);
                tdkTestObj.addParameter("isFP", 1);
                ...
                if "SUCCESS" in actualresult.upper():
                    ...
                    print "Broadcasting IR WPS keydown Event Result : %s"%actualresult;
