...
<xml>
  ...
  <name>NM_WifiMgr_connect_without_password_and_securitymode_none</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To connect to a wifi network with the given ssid without any password and security mode none
Test CaseID:CT_NM_9
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_25</test_case_id>
    <test_objective>To connect to a wifi network with the given ssid without any password and security mode none</test_objective>
...
char *-ssid
password - NULL
security mode - NET_WIFI_SECURITY_NONE</input_parameters>
    <automation_approch>1. TM loads the NetSrvMgr_Agent via the test agent.
2.NetSrvMgr_Agent will connect to the given wifi network without any password.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if wifi manger could successfully connect to the network without any password</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_connect_without_password_and_securitymode_none</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_connect_without_password_and_securitymode_none');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_connect_without_password_and_securitymode_none');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                if ("SUCCESS" in actualresult):
                    ...
                    tdkTestObj.addParameter("security_mode", 0);
