...
<xml>
  ...
  <name>NM_NetSrvMgr_isInterfaceEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide an info about given interface has enabled or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>true or flase</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_isInterfaceEnabled</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_isInterfaceEnabled');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_isInterfaceEnabled');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                if (expectedresult in actualresult) and ("true" in details):
                    print("[TEST EXECUTION - SUCCESS] :  WIFI/ETHERNET interface has enabled in device");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  WIFI/ETHERNET interface has disabled in device");
