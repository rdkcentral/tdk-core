...
<xml>
  ...
  <name>NM_WifiMgr_connect_with_saved_ssids</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To connect to a wifi network with the saved SSIDs
Test CaseID:CT_NM_27
Test Type: Positive</synopsis>
  ...
  <test_cases>
    ...
    <test_objective>To connect to a wifi network with the saved SSIDs</test_objective>
...
2.NetSrvMgr_Agent will save the provided wifi credentials
2.NetSrvMgr_Agent will connect to a wifi network with the previously saved credentials.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if wifi manger could successfully connect to the network with saved ssid</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_connect_with_saved_ssids</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_connect_with_saved_ssids');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_connect_with_saved_ssids');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                print("SaveSSID returned : %s" %actualresult)
                ...
                if ("SUCCESS" in actualresult):
                    ...
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
                    print("Connect with Saved SSID returns : %s" %actualresult);
                    print("Details: [%s]"%details);
