...
<xml>
  ...
  <name>NM_NetSrvMgr_getNetworkInterfaces</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide all available network network</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_1</test_case_id>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_getNetworkInterfaces)</api_or_interface_used>
...
3. NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step.</automation_approch>
    <expected_output>Available network interfaces</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_getNetworkInterfaces</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_getNetworkInterfaces');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_getNetworkInterfaces');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getNetworkInterfaces");
                expectedresult="SUCCESS"
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();

                print("getNetworkInterfaces returns : %s" %actualresult);
                print("Details: [%s]"%details);

                if ((expectedresult in actualresult) and (("ETHERNET" in details) or ("WIFI" in details))):
                    print("[TEST EXECUTION - SUCCESS] :  WIFI/ETHERNET interface found in device");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  WIFI/ETHERNET interface not found in device");
