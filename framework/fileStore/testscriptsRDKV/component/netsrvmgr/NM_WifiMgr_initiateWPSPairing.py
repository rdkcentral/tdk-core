...
<xml>
  ...
  <name>NM_WifiMgr_initiateWPSPairing</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To initiate WPS pairing
Test CaseID:CT_NM_11
Test Type: Positive</synopsis>
  ...
  <test_cases>
    ...
    <test_objective>To initiate WPS pairing</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_initiateWPSPairing)
IARM_Bus_Disconnect()
...
2.NetSrvMgr_Agent should initiate WPS pairing successfully.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should initiate WPS pairing successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_initiateWPSPairing</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_initiateWPSPairing');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_initiateWPSPairing');
