...
<xml>
  ...
  <name>NM_WifiMgr_getAvailableSSIDWithoutFreq</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide details about given SSID. This test case invoke with frequency.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_45</test_case_id>
    <test_objective>This would provide details about given SSID. This test case invoke with frequency.</test_objective>
...
IARM_Bus_Term : None</api_or_interface_used>
    <input_parameters>char *-ssid</input_parameters>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>Check if wifi manger could successfully provide details about given SSID</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_getAvailableSSIDWithoutFreq</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDWithoutFreq');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDWithoutFreq');
