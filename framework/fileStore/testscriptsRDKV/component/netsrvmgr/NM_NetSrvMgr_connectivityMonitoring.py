...
<xml>
  ...
  <name>NM_NetSrvMgr_connectivityMonitoring</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Continuous monitoring of internet connectivity with heartbeat interval that's given network interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_20</test_case_id>
    <test_objective>Enable a continuous monitoring of internet connectivity with heart beat interval thats given</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_connectivityMonitoring)</api_or_interface_used>
...
3. NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    ...
    <test_script>NM_NetSrvMgr_connectivityMonitoring</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_connectivityMonitoring');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_connectivityMonitoring');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "startConnectivityMonitoring");
                expectedresult="SUCCESS"
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();

                print("startConnectivityMonitoring returned : %s" %actualresult)
                print("Details: [%s]"%details)

                if "SUCCESS" in actualresult:
                    #Monitoring for 90 sec (default value), so wait until start to complete monitoring
                    time.sleep(90)

                    #Now invoke stop connectivity monitoring
                    tdkTestObj.addParameter("method_name", "stopConnectivityMonitoring");
                    expectedresult="SUCCESS"
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();

                    print("stopConnectivityMonitoring returned : %s" %actualresult)
                    print("Details: [%s]"%details)
                    ...
                    if "SUCCESS" in actualresult:
                        ...
                        print("[TEST EXECUTION - FAILURE] : stopConnectivityMonitoring returned failed")
                        tdkTestObj.setResultStatus("FAILURE");
                else:
                    print("[TEST EXECUTION - FAILURE] : startConnectivityMonitoring returned failed")
                    tdkTestObj.setResultStatus("FAILURE");
