...
<xml>
  ...
  <name>NM_NetSrvMgr_getSTBIPFamily</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Gets the IP address of the default interface by address family</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Gets the IP address of the default interface by address family</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_getSTBIPFamily)</api_or_interface_used>
...
3. NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    ...
    <test_script>NM_NetSrvMgr_getSTBIPFamily</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_getSTBIPFamily');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_getSTBIPFamily');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                family = configParser.get('wifi-config', 'family')

                #Prmitive test case which associated to this Script
                tdkTestObj = netsrvObj.createTestStep('NetSrvMgrAgent_NetSrvMgr_FunctionCall');

                tdkTestObj.addParameter("method_name", "getSTBip_family");
                tdkTestObj.addParameter("family", family);
                ...
                print("getSTBip_family returned : %s" %actualresult)
