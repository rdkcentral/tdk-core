...
<xml>
  ...
  <name>NM_WifiMgr_is_stopLNFWhiledisconnected</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To check if stopLNFWhileDisconnected bit is set
Test CaseID:CT_NM_19
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_19</test_case_id>
    <test_objective>To check if stopLNFWhileDisconnected bit is set</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_isStopLNFWhileDisconnected)
IARM_Bus_Disconnect()
...
2.NetSrvMgr_Agent should retrive the value of stopLNFWhileDisconnected bit.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager checks successfully if the stopLNFWhileDisconnected bit is set or not</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_is_stopLNFWhiledisconnected</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_is_stopLNFWhiledisconnected');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_is_stopLNFWhiledisconnected');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "isStopLNFWhileDisconnected");
                ...
                if "SUCCESS" in actualresult:
                    print(("stopLNFWhileDisconnected bit is " , "set" if (details) else "not set"));
                else:
