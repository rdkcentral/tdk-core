...
<xml>
  ...
  <name>Aamp_Enable_4K_Playback</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to enable 4K video playback</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <rdk_versions>
   ...
   <test_cases>
    ...
    <test_objective>Test script to enable 4K video playback</test_objective>
...
2. Aamp Agent invokes SetEnable4K to make AAMP switch to 2160p.
...
7. TM unloads the Aamp Agent.</automation_approch>
    <expected_output>Should play with 4K resolution</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Enable_4K_Playback</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Enable_4K_Playback');
...
if ("SUCCESS" in aampLoadStatus.upper()):
    ...
    tdkTestObj.addParameter("disable4K","false");
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if match:
                ...
                print("Height:", height)
                if height != 2160:
                    print("FAILURE : Playback is not happening at 4K which is not expected");
                    tdkTestObj.setResultStatus("FAILURE")
                else:
                    print("SUCCESS : Playback is happening at 4K as expected");
                    tdkTestObj.setResultStatus("SUCCESS")
            else:
