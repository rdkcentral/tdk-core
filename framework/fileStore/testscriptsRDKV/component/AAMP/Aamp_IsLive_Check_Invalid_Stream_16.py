...
<xml>
  ...
  <name>Aamp_IsLive_Check_Invalid_Stream_16</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if IsLive API returns false for an invalid stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <remarks>AAMP API IsLive can not be called when AAMP Player instance is in ERROR state</remarks>
  ...
  <test_cases>
    <test_case_id>CT_Aamp_16</test_case_id>
    <test_objective>Check if IsLive API returns false for an invalid stream</test_objective>
...
IsLive</api_or_interface_used>
    <input_parameters>Invalid URL</input_parameters>
    <automation_approch>1. TM loads the Aamp Agent and systemutil Agent.
2. Aamp Agent invokes Tune API with invalid URL
3. Aamp Agent invokes IsLive API
4. TM checks if the value returned is False and returns SUCCESS/FAILURE
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <except_output>Checkpoint 1. IsLive API returns False</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_IsLive_Check_Invalid_Stream_16</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_IsLive_Check_Invalid_Stream_16');
sysObj.configureTestCase(ip,port,'Aamp_IsLive_Check_Invalid_Stream_16');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
    ...
    #Invalid URL
    tuneURL="https://Invalid/Invalid1/invalid2.m3u8"
    ...
    if expectedResult not in actualResult:
        ...
        if expectedResult in actualResult:
            print("AAMP Tune event not recieved")
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");

            #AampIsLive call
            tdkTestObj = aampObj.createTestStep('Aamp_AampIsLive');
            expectedResult = "FAILURE";
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            #Get the result of execution
            actualResult = tdkTestObj.getResult();
            print("Live check Status: %s" %actualResult);
            ...
            print("AAMP tune event received for invalid stream")
            #Set the result status of execution
            tdkTestObj.setResultStatus("FAILURE");
