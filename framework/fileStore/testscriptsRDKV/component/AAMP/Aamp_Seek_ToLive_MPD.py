...
<xml>
  ...
  <name>Aamp_Seek_ToLive_MPD</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>Aamp_AampSeekLive</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To seek the mpd stream to live</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_24</test_case_id>
    <test_objective>To see the mpd stream to live</test_objective>
...
2. Parse the mpd stream from Aamp_Tune_Config.ini
3. Aamp Agent invokes Tune API
4. Aamp agent invokes SeekLive API
5. Aamp Agent invokes IsLive API
6. TM checks if the value returned is True and returns SUCCESS/FAILURE
7. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <except_output>Checkpoint 1. IsLive API returns true</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Seek_ToLive_MPD</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Seek_ToLive_MPD');
sysObj.configureTestCase(ip,port,'Aamp_Seek_ToLive_MPD');
