...
<xml>
  ...
  <name>Aamp_SetAndGet_Video_Zoom</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to set the different video zoom mode and validate while playing HLS stream content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_87</test_case_id>
    <test_objective>Test script to set the different video zoom mode and validate while playing HLS stream content</test_objective>
...
4. Aamp Agent invokes SetVideoZoom API to select the different zoom mode
5. Aamp Agent invokes GetVideoZoom API to get the zoom mode and check whether is it persisted properly.
6. TM checks the value and returns SUCCESS/FAILURE
7. TM unloads the Aamp Agent</automation_approch>
    <expected_output>Should persist the video zoom mode value</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_SetAndGet_Video_Zoom</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_SetAndGet_Video_Zoom');
...
if ("SUCCESS" in aampLoadStatus.upper()):
    ...
    if result and aampUtilitylib.SearchAampPlayerEvents(tdkTestObj,pattern) == "SUCCESS":
        ...
        if expectedResult in actualResult:
            ...
            details = tdkTestObj.getResultDetails();
            if expectedResult in actualResult:
                print("SetVideoZoom mode call getting succeeded");
            ...
            #set the initial zoom mode value before executing this script
            tdkTestObj = aampObj.createTestStep('Aamp_AampSetVideoZoom');
            expectedResult = "SUCCESS";
            if cur_zoommode == 0:
                tdkTestObj.addParameter("zoom","FULL");
