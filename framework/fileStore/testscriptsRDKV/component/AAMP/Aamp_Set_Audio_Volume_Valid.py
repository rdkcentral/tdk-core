...
<xml>
  ...
  <name>Aamp_Set_Audio_Volume_Valid</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to set the current audio volume</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_37</test_case_id>
    <test_objective>Test script to set the current audio volume</test_objective>
...
3. Aamp Agent invokes SetAudioVolume  API with valid volume level 50
...
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should set the  audio volume level as 50</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Set_Audio_Volume_Valid</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Valid');
sysObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Valid');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            #choosing 50 as a random value between min and max for testing
            volume_set = 50;
            ...
            if expectedResult in actualResult :
                ...
                if expectedResult in actualResult and volume == volume_set:
                    ...
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("Result :", details);
                    print("Able to set Audio volume as %s successfully" %(volume_set))
                    print("[TEST EXECUTION RESULT] : SUCCESS\n")
