...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackCueEmptyCueText</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that getting HTML from an empty text cue returns a valid document fragment</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_09</test_case_id>
    <test_objective>To ensure that getCueAsHTML returns a fragment with one empty Text node for an empty cue</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-cue-empty-cue-text.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Fragment is not null, contains one child, the child has length 0 and is a Text node</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackCueEmptyCueText</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackCueEmptyCueText');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-cue-empty-cue-text.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-cue-empty-cue-text.html file")
            grep_line = "track-cue-empty-cue-text | tail -3 | tr -d '\\n'"
            log_filename = "track-cue-empty-cue-text"
...
if "track-cue-empty-cue-text" in status_dict:
    print(f"track-cue-empty-cue-text: {status_dict['track-cue-empty-cue-text']}")
