...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcMarkup</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cues containing bold, italic, underline, ruby text, and ruby annotations are correctly parsed and recognized.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_35</test_case_id>
    <test_objective>To verify that cues with the HTML formatting tags b, i, u, rt, and ruby are properly handled and retained in the cue content.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc023-markup.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Each text track contains four cues, and all formatting tags are correctly interpreted without errors.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcMarkup</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcMarkup');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc023-markup.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc023-markup.html file")
            grep_line = "track-webvtt-tc023-markup | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc023-markup"
...
if "track-webvtt-tc023-markup" in status_dict:
    print(f"track-webvtt-tc023-markup: {status_dict['track-webvtt-tc023-markup']}")
