...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Constructors</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate the consistency of media element constructors.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_59</test_case_id>
    <test_objective>Verify that Audio and Video constructors create elements with the correct prototype hierarchy.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>constructors.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Audio objects should be instances of HTMLAudioElement and HTMLMediaElement but not HTMLVideoElement, while Video objects should be instances of HTMLVideoElement and HTMLMediaElement but not HTMLAudioElement.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Constructors</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Constructors');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/constructors.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing constructors.html file")
            grep_line = "constructors | tail -3 | tr -d '\\n'"
            log_filename = "constructors"
...
if "constructors" in status_dict:
    print(f"constructors: {status_dict['constructors']}")
