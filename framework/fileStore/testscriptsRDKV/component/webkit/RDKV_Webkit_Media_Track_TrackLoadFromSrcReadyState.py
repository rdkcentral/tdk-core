...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackLoadFromSrcReadyState</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that setting the source of an HTMLTrackElement triggers the load event and updates the TextTrack readyState</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_18</test_case_id>
    <test_objective>To verify that a TextTrack transitions to LOADED readyState when the track source is set dynamically</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-load-from-src-readyState.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>TextTrack readyState changes from zero to two and the load event is fired</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackLoadFromSrcReadyState</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackLoadFromSrcReadyState');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-load-from-src-readyState.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-load-from-src-readyState.html file")
            grep_line = "track-load-from-src-readyState | tail -3 | tr -d '\\n'"
            log_filename = "track-load-from-src-readyState"
...
if "track-load-from-src-readyState" in status_dict:
    print(f"track-load-from-src-readyState: {status_dict['track-load-from-src-readyState']}")
