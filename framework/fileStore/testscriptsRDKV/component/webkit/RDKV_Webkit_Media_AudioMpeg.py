...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_AudioMpeg</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate audio element playback support for MPEG and M4A formats.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_49</test_case_id>
    <test_objective>Verify that the audio element can correctly load and play both MPEG and M4A audio files, with the currentTime advancing when the play event is triggered.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>audio-mpeg-supported.html, audio-mpeg4-supported.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>For both MPEG and M4A files, the audio element should trigger the canplaythrough event, and upon playing, the currentTime should be greater than 0, confirming the media has started playing.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_AudioMpeg</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_AudioMpeg');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/audio-mpeg-supported.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/audio-mpeg4-supported.html'
