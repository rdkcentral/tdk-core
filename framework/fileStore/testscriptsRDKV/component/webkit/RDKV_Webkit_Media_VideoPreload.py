...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoPreload</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify media element loading behavior with different preload attribute values</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_145</test_case_id>
    <test_objective>Ensure that media buffers automatically according to the preload setting and autoplay attribute</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-preload.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Media does not buffer automatically with preload none unless play is called, buffers automatically with preload metadata or preload auto, and buffers automatically with preload none if autoplay is set to true</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoPreload</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoPreload');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-preload.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-preload.html file")
            grep_line = "video-preload | tail -3 | tr -d '\\n'"
            log_filename = "video-preload"
...
if "video-preload" in status_dict:
    print(f"video-preload: {status_dict['video-preload']}")
