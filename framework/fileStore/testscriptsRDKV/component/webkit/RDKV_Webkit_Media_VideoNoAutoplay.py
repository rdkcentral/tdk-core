...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoNoAutoplay</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that a video does not play automatically when src is set without the autoplay attribute</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_134</test_case_id>
    <test_objective>To verify that setting src alone does not trigger the play event if autoplay is not enabled</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-no-autoplay.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should remain paused and the play event should not fire after setting src</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoNoAutoplay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoNoAutoplay');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-no-autoplay.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-no-autoplay.html file")
            grep_line = "video-no-autoplay | tail -3 | tr -d '\\n'"
            log_filename = "video-no-autoplay"
...
if "video-no-autoplay" in status_dict:
    print(f"video-no-autoplay: {status_dict['video-no-autoplay']}")
