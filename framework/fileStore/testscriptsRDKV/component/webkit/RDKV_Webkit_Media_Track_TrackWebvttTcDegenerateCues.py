...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcDegenerateCues</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cues without separating blank lines are parsed correctly.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_29</test_case_id>
    <test_objective>To verify that degenerate cues without blank line separators are recognized and maintain their start and end times even if the text is empty.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc032-degenerate-cues.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Cues have the correct length, start and end times, and contain empty text.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcDegenerateCues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcDegenerateCues');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc032-degenerate-cues.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc032-degenerate-cues.html file")
            grep_line = "track-webvtt-tc032-degenerate-cues | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc032-degenerate-cues"
...
if "track-webvtt-tc032-degenerate-cues" in status_dict:
    print(f"track-webvtt-tc032-degenerate-cues: {status_dict['track-webvtt-tc032-degenerate-cues']}")
