...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaPigherPrioAudioStream</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate media pause behavior when a higher-priority audio stream takes over.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_96</test_case_id>
    <test_objective>To verify that the playing media pauses correctly without being marked as ended when interrupted by another stream.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-higher-prio-audio-stream.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The media element should transition from playing to paused, while its ended state remains false.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaPigherPrioAudioStream</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaPigherPrioAudioStream');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-higher-prio-audio-stream.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-higher-prio-audio-stream.html file")
            grep_line = "media-higher-prio-audio-stream | tail -3 | tr -d '\\n'"
            log_filename = "media-higher-prio-audio-stream"
...
if "media-higher-prio-audio-stream" in status_dict:
    print(f"media-higher-prio-audio-stream: {status_dict['media-higher-prio-audio-stream']}")
