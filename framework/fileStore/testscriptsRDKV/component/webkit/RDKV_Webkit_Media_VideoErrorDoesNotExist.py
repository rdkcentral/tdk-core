...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoErrorDoesNotExist</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates the state of a media element after a load failure</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_127</test_case_id>
    <test_objective>To verify that the media element reports an error and reflects correct properties when loading fails</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-error-does-not-exist.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should have a non-null error with correct code, networkState equal to 3, duration as NaN, currentTime zero, and empty buffered and seekable ranges</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoErrorDoesNotExist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoErrorDoesNotExist');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-error-does-not-exist.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-error-does-not-exist.html file")
            grep_line = "video-error-does-not-exist | tail -3 | tr -d '\\n'"
            log_filename = "video-error-does-not-exist"
...
if "video-error-does-not-exist" in status_dict:
    print(f"video-error-does-not-exist: {status_dict['video-error-does-not-exist']}")
