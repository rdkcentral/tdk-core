...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoPlaybackrate</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates the behavior of playbackRate and defaultPlaybackRate on the video element</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_140</test_case_id>
    <test_objective>To verify that playbackRate does not change when play is called and both playbackRate and defaultPlaybackRate always reflect the last values set</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-playbackrate.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video playbackRate and defaultPlaybackRate should maintain their expected values before, during, and after playback</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoPlaybackrate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoPlaybackrate');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-playbackrate.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-playbackrate.html file")
            grep_line = "video-playbackrate | tail -3 | tr -d '\\n'"
            log_filename = "video-playbackrate"
...
if "video-playbackrate" in status_dict:
    print(f"video-playbackrate: {status_dict['video-playbackrate']}")
