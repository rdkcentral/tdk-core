...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcLinePosition</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get tests that cue line positions and snap-to-lines settings are correctly applied from the track settings.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_33</test_case_id>
    <test_objective>To verify that each cue reports the expected line value and snap-to-lines behavior according to the settings.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc017-line-position.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Track 0 has seven cues with lines 0, 0, 50.2, 5, 100, -1, 500 and snap-to-lines false, true, false, true, false, true, true. Track 1 has six cues with lines auto except the second expected at 0.5 and all snap-to-lines true.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcLinePosition</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcLinePosition');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc017-line-position.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc017-line-position.html file")
            grep_line = "track-webvtt-tc017-line-position | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc017-line-position"
...
if "track-webvtt-tc017-line-position" in status_dict:
    print(f"track-webvtt-tc017-line-position: {status_dict['track-webvtt-tc017-line-position']}")
