...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackInBandMkvChapters</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that MKV files with in-band chapter tracks are correctly detected and cues are handled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_50</test_case_id>
    <test_objective>Ensure chapter tracks from MKV files trigger addtrack events, show cues when enabled, and retain cues after seek operations.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-in-band-mkv-chapters.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Console should show PASS logs confirming cues are added, persist after seek, all cues are equal, followed by "END OF TEST".</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackInBandMkvChapters</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackInBandMkvChapters');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/in-band/track-in-band-mkv-chapters.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-in-band-mkv-chapters.html file")
            grep_line = "track-in-band-mkv-chapters | tail -3 | tr -d '\\n'"
            log_filename = "track-in-band-mkv-chapters"
...
if "track-in-band-mkv-chapters" in status_dict:
    print(f"track-in-band-mkv-chapters: {status_dict['track-in-band-mkv-chapters']}")
