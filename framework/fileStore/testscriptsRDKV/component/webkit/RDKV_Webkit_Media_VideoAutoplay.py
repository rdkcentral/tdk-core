...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoAutoplay</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate autoplay behavior of the video element</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_114</test_case_id>
    <test_objective>To verify that a video with the autoplay attribute starts playing automatically</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-autoplay.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should have autoplay enabled and should transition to playing state without user interaction</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoAutoplay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoAutoplay');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-autoplay.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-autoplay.html file")
            grep_line = "video-autoplay | tail -3 | tr -d '\\n'"
            log_filename = "video-autoplay"
...
if "video-autoplay" in status_dict:
    print(f"video-autoplay: {status_dict['video-autoplay']}")
