...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_FullscreenApiEnabledMedia</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate video presentation mode transitions with fullscreen entry and exit.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_69</test_case_id>
    <test_objective>Verify that the video switches to fullscreen mode when entering fullscreen and reverts to inline mode after exiting.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>fullscreen-api-enabled-media-with-presentation-mode.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video’s webkitPresentationMode should update to fullscreen on entry and return to inline on exit.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_FullscreenApiEnabledMedia</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_FullscreenApiEnabledMedia');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/fullscreen-api-enabled-media-with-presentation-mode.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing fullscreen-api-enabled-media-with-presentation-mode.html file")
            grep_line = "fullscreen-api-enabled-media-with-presentation-mode | tail -3 | tr -d '\\n'"
            log_filename = "fullscreen-api-enabled-media-with-presentation-mode"
...
if "fullscreen-api-enabled-media-with-presentation-mode" in status_dict:
    print(f"fullscreen-api-enabled-media-with-presentation-mode: {status_dict['fullscreen-api-enabled-media-with-presentation-mode']}")
