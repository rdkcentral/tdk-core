...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoVolume</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate the behavior of the video volume attribute.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_159</test_case_id>
    <test_objective>To confirm that volume accepts values between 0 and 1, rejects values outside the range, and persists after reload.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-volume.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Volume correctly updates to 1, 0.5, and 0. Values above 1 or below 0 are rejected. Volume persists after load.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoVolume</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoVolume');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-volume.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-volume.html file")
            grep_line = "video-volume | tail -3 | tr -d '\\n'"
            log_filename = "video-volume"
...
if "video-volume" in status_dict:
    print(f"video-volume: {status_dict['video-volume']}")
