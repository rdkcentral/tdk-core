...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoDisplayAspectRatio</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that the video element reports correct dimensions after metadata is loaded</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_122</test_case_id>
    <test_objective>To verify that videoWidth and videoHeight update from zero to the actual media dimensions once loaded</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-display-aspect-ratio.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video element should have zero width and height before loading and correct width and height after loadedmetadata event</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoDisplayAspectRatio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoDisplayAspectRatio');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-display-aspect-ratio.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-display-aspect-ratio.html file")
            grep_line = "video-display-aspect-ratio | tail -3 | tr -d '\\n'"
            log_filename = "video-display-aspect-ratio"
...
if "video-display-aspect-ratio" in status_dict:
    print(f"video-display-aspect-ratio: {status_dict['video-display-aspect-ratio']}")
