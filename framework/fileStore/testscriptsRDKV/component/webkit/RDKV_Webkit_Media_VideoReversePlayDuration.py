...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoReversePlayDuration</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that duration remains non-zero when playing video in reverse</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_148</test_case_id>
    <test_objective>To ensure that playing backward does not reset the video duration to zero</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-reverse-play-duration.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Video duration stays greater than zero and currentTime updates correctly</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoReversePlayDuration</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoReversePlayDuration');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-reverse-play-duration.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-reverse-play-duration.html file")
            grep_line = "video-reverse-play-duration | tail -3 | tr -d '\\n'"
            log_filename = "video-reverse-play-duration"
...
if "video-reverse-play-duration" in status_dict:
    print(f"video-reverse-play-duration: {status_dict['video-reverse-play-duration']}")
