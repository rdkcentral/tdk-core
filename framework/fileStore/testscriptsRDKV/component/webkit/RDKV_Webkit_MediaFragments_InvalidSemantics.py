...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_MediaFragments_InvalidSemantics</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>
    TC0002: To ensure that media fragments with invalid semantics, where the start time equals the end time, are ignored.
    TC0003: To ensure that media fragments with invalid semantics, where the start time is greater than the end time, are ignored.
  </synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Mediafragments_05</test_case_id>
    <test_objective>
      TC0002: To verify that a media fragment with the format 't=3,3' (where start and end times are the same) is considered invalid and ignored, resulting in the media starting at the default position.
      TC0003: To verify that a media fragment with the format 't=7,3' (where the start time exceeds the end time) is treated as invalid, causing the media to start playback from the default position.
    </test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>TC0002.html, TC0003.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    ...
    <test_script>RDKV_Webkit_MediaFragments_InvalidSemantics</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_MediaFragments_InvalidSemantics');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0002.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0003.html'
