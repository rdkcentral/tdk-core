...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoAppendSource</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate that video element correctly updates its source after loading</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_113</test_case_id>
    <test_objective>To verify that the video currentSrc reflects the assigned media source after canplaythrough</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-append-source.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video currentSrc should match the expected media file URL once it is ready to play</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoAppendSource</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoAppendSource');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-append-source.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-append-source.html file")
            grep_line = "video-append-source | tail -3 | tr -d '\\n'"
            log_filename = "video-append-source"
...
if "video-append-source" in status_dict:
    print(f"video-append-source: {status_dict['video-append-source']}")
