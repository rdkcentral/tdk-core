...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaCanPlayAv1</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate canPlayType support for the AV1 codec.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_74</test_case_id>
    <test_objective>Verify that the media element correctly reports playback capability for valid AV1 codec strings and rejects invalid ones.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-can-play-av1.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The method should return probably for supported AV1 codec formats and an empty string for unsupported codec strings.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaCanPlayAv1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaCanPlayAv1');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-can-play-av1.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-can-play-av1.html file")
            grep_line = "media-can-play-av1 | tail -3 | tr -d '\\n'"
            log_filename = "media-can-play-av1"
...
if "media-can-play-av1" in status_dict:
    print(f"media-can-play-av1: {status_dict['media-can-play-av1']}")
