...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_AudioGarbageCollect</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate that active audio playback is not interrupted by garbage collection.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_48</test_case_id>
    <test_objective>Ensure that a playing audio element or its event listener is retained until playback completion.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>audio-garbage-collect.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The audio should continue playing without being garbage collected until no further audio can be produced.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_AudioGarbageCollect</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_AudioGarbageCollect');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/audio-garbage-collect.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing audio-garbage-collect.html file")
            grep_line = "audio-garbage-collect | tail -3 | tr -d '\\n'"
            log_filename = "audio-garbage-collect"
...
if "audio-garbage-collect" in status_dict:
    print(f"audio-garbage-collect: {status_dict['audio-garbage-collect']}")
