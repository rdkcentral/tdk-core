...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackModeTriggersLoading</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that a metadata track does not load automatically but loads when its mode is changed</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_21</test_case_id>
    <test_objective>To verify that changing the mode of a metadata track triggers loading and that cues are correctly populated</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-mode-triggers-loading.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Track readyState updates to loaded, mode is hidden, track contains 12 cues, and the last cue starts at 22</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackModeTriggersLoading</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackModeTriggersLoading');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-mode-triggers-loading.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-mode-triggers-loading.html file")
            grep_line = "track-mode-triggers-loading | tail -3 | tr -d '\\n'"
            log_filename = "track-mode-triggers-loading"
...
if "track-mode-triggers-loading" in status_dict:
    print(f"track-mode-triggers-loading: {status_dict['track-mode-triggers-loading']}")
