...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_AutoPlayInSandbox</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate autoplay behavior of a video element inside a sandbox with script permissions.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_54</test_case_id>
    <test_objective>Verify that setting the src with the autoplay attribute triggers the play event even in a sandboxed environment with allow-scripts.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>auto-play-in-sandbox-with-allow-scripts.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should initially be paused and then fire the play event automatically.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_AutoPlayInSandbox</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_AutoPlayInSandbox');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/auto-play-in-sandbox-with-allow-scripts.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing auto-play-in-sandbox-with-allow-scripts.html file")
            grep_line = "auto-play-in-sandbox-with-allow-scripts | tail -3 | tr -d '\\n'"
            log_filename = "auto-play-in-sandbox-with-allow-scripts"
...
if "auto-play-in-sandbox-with-allow-scripts" in status_dict:
    print(f"auto-play-in-sandbox-with-allow-scripts: {status_dict['auto-play-in-sandbox-with-allow-scripts']}")
