...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackConstants</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests the constants defined for TextTrack states in HTMLTrackElement</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_08</test_case_id>
    <test_objective>To verify that the NONE, LOADING, LOADED, and ERROR constants have the correct values</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-constants.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The TextTrack state constants correctly reflect their numeric values: NONE is 0, LOADING is 1, LOADED is 2, and ERROR is 3</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackConstants</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackConstants');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-constants.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-constants.html file")
            grep_line = "track-constants | tail -3 | tr -d '\\n'"
            log_filename = "track-constants"
...
if "track-constants" in status_dict:
    print(f"track-constants: {status_dict['track-constants']}")
