...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_RemoveFromDocument</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate behavior of media elements when removed from the DOM</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_108</test_case_id>
    <test_objective>To verify that removing a media element pauses playback without unloading loaded media and does not trigger loadstart if no media is loaded</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>remove-from-document.html, remove-from-document-no-load.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Loaded media should pause but retain networkState, and no loadstart event should occur for unloaded media</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_RemoveFromDocument</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_RemoveFromDocument');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/remove-from-document.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/remove-from-document-no-load.html'
