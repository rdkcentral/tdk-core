...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackRemoveActiveCueCrash</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests removing an active cue from a text track during playback.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_23</test_case_id>
    <test_objective>To ensure that removing a cue while it is active does not cause the browser to crash.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-remove-active-cue-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The cue is removed safely, and playback continues without errors.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackRemoveActiveCueCrash</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackRemoveActiveCueCrash');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-remove-active-cue-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-remove-active-cue-crash.html file")
            grep_line = "track-remove-active-cue-crash | tail -3 | tr -d '\\n'"
            log_filename = "track-remove-active-cue-crash"
...
if "track-remove-active-cue-crash" in status_dict:
    print(f"track-remove-active-cue-crash: {status_dict['track-remove-active-cue-crash']}")
