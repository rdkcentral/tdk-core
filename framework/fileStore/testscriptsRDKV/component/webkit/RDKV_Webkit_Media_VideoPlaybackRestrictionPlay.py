...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoPlaybackRestrictionPlay</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that starting audio playback requires a user gesture when restrictions are set</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_143</test_case_id>
    <test_objective>To verify that RequireUserGestureForAudioRateChange prevents playback without a user gesture</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-playback-restriction-play-before-load.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The play promise should be rejected, indicating playback did not start without a user gesture</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoPlaybackRestrictionPlay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoPlaybackRestrictionPlay');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-playback-restriction-play-before-load.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-playback-restriction-play-before-load.html file")
            grep_line = "video-playback-restriction-play-before-load | tail -3 | tr -d '\\n'"
            log_filename = "video-playback-restriction-play-before-load"
...
if "video-playback-restriction-play-before-load" in status_dict:
    print(f"video-playback-restriction-play-before-load: {status_dict['video-playback-restriction-play-before-load']}")
