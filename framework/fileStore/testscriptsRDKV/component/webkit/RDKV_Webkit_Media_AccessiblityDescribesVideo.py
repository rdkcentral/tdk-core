...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_AccessiblityDescribesVideo</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate audio track selection behavior in the video element.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_44</test_case_id>
    <test_objective>Verify that the video initializes with two audio tracks where only the second track is enabled.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>accessiblity-describes-video.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should report two audio tracks, with the first disabled and the second enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_AccessiblityDescribesVideo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_AccessiblityDescribesVideo');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/accessiblity-describes-video.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing accessiblity-describes-video.html file")
            grep_line = "accessiblity-describes-video | tail -3 | tr -d '\\n'"
            log_filename = "accessiblity-describes-video"
...
if "accessiblity-describes-video" in status_dict:
    print(f"accessiblity-describes-video: {status_dict['accessiblity-describes-video']}")
