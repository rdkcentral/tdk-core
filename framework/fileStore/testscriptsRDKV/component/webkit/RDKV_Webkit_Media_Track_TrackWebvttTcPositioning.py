...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcPositioning</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cue text positions are correctly applied according to their settings.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_37</test_case_id>
    <test_objective>To verify that each cue reflects the expected position value as defined in the track settings, including auto and specific numeric positions.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc015-positioning.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Text track 0 has four cues with positions 0, 49.2, auto, and 100, text track 1 has four cues with positions 0, 49.2, auto, and 100, and text track 2 has eight cues all with position auto.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcPositioning</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcPositioning');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc015-positioning.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc015-positioning.html file")
            grep_line = "track-webvtt-tc015-positioning | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc015-positioning"
...
if "track-webvtt-tc015-positioning" in status_dict:
    print(f"track-webvtt-tc015-positioning: {status_dict['track-webvtt-tc015-positioning']}")
