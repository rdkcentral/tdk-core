...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_MediaFragments_LastOccurrenceApplied</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>
    TC0055: To verify that when multiple occurrences of a fragment dimension are provided, only the last occurrence is interpreted.
    TC0058: To verify that when an invalid media fragment is provided, the entire media resource is played.
    TC0059: To ensure that when multiple fragment dimensions are provided, only the last one is interpreted.
  </synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Mediafragments_07</test_case_id>
    <test_objective>
      TC0055: To ensure that when a media fragment with multiple t dimensions ('t=7ampersandt=3') is provided, only the last occurrence of t=3 is used, and the video starts playing from 3 seconds.
      TC0058: To ensure that when a media fragment with an unrecognized fragment identifier ('T=3,7') is given, the user agent (UA) does not treat it as a valid media fragment and instead plays the entire media resource from the start.
      TC0059: To verify that if a media fragment contains multiple time dimensions (e.g., 't=smpte:00:00:01' and 't=npt:3'), the user agent (UA) correctly interprets and uses only the last specified time dimension ('t=npt:3') for playback.
    </test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>TC0055.html, TC0058.html, TC0059.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    ...
    <test_script>RDKV_Webkit_MediaFragments_LastOccurrenceApplied</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_MediaFragments_LastOccurrenceApplied');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0055.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0058.html'
webkit_test_url3 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0059.html'
