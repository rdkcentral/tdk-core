...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoSeekable</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify the behavior of the video seekable attribute before and after media is ready</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_150</test_case_id>
    <test_objective>To ensure that accessing out-of-bounds seekable ranges throws exceptions and valid ranges return correct start and end times</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-seekable.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Initial seekable length is zero, later length is one with correct start and end, invalid accesses throw errors</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoSeekable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoSeekable');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-seekable.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-seekable.html file")
            grep_line = "video-seekable | tail -3 | tr -d '\\n'"
            log_filename = "video-seekable"
...
if "video-seekable" in status_dict:
    print(f"video-seekable: {status_dict['video-seekable']}")
