...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcInterspersedNonCue</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that an empty line after a cue identifier discards the current cue and starts a new cue.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_32</test_case_id>
    <test_objective>To verify that cues are correctly restarted when an empty line follows a cue identifier.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc030-interspersed-non-cue.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Only valid cues remain with correct text, showing First and Second.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcInterspersedNonCue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcInterspersedNonCue');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc030-interspersed-non-cue.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc030-interspersed-non-cue.html file")
            grep_line = "track-webvtt-tc030-interspersed-non-cue | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc030-interspersed-non-cue"
...
if "track-webvtt-tc030-interspersed-non-cue" in status_dict:
    print(f"track-webvtt-tc030-interspersed-non-cue: {status_dict['track-webvtt-tc030-interspersed-non-cue']}")
