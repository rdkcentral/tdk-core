...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaCanPlayWavAudio</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate canPlayType support for WAV audio formats.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_84</test_case_id>
    <test_objective>Verify that the audio element correctly recognizes different WAV MIME types, with and without codec specifications.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-can-play-wav-audio.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The method should return maybe for generic WAV types and probably when the codec is specified as 1.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaCanPlayWavAudio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaCanPlayWavAudio');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-can-play-wav-audio.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-can-play-wav-audio.html file")
            grep_line = "media-can-play-wav-audio | tail -3 | tr -d '\\n'"
            log_filename = "media-can-play-wav-audio"
...
if "media-can-play-wav-audio" in status_dict:
    print(f"media-can-play-wav-audio: {status_dict['media-can-play-wav-audio']}")
