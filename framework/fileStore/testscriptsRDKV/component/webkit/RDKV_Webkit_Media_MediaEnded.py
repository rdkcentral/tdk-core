...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaEnded</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate correct firing of the ended event during media playback and source changes.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_91</test_case_id>
    <test_objective>To verify that the ended event triggers when playback completes, fires again after source change, and does not retrigger after seeking to start.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-ended.html, media-ended-fired-once.html</input_parameters>
    <automation_approch>1. Launch the html test a'ended' event should occur once per playback completion, repeat on new source, and not fire again after seeking to 0.pp in browser
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The ended event should occur once per playback completion, repeat on new source, and not fire again after seeking to 0.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaEnded</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaEnded');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-ended.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-ended-fired-once.html'
