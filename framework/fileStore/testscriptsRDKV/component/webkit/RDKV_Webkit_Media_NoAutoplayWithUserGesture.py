...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_NoAutoplayWithUserGesture</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate media autoplay restrictions when user gesture is required</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_106</test_case_id>
    <test_objective>To verify that video does not start playing automatically without a user gesture</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>no-autoplay-with-user-gesture-requirement.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should remain paused even when it is ready to play canplaythrough event</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_NoAutoplayWithUserGesture</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_NoAutoplayWithUserGesture');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/no-autoplay-with-user-gesture-requirement.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing no-autoplay-with-user-gesture-requirement.html file")
            grep_line = "no-autoplay-with-user-gesture-requirement | tail -3 | tr -d '\\n'"
            log_filename = "no-autoplay-with-user-gesture-requirement"
...
if "no-autoplay-with-user-gesture-requirement" in status_dict:
    print(f"no-autoplay-with-user-gesture-requirement: {status_dict['no-autoplay-with-user-gesture-requirement']}")
