...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_MediaCapabilities_MediacapabilitiesTypes</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that the WebKit browser correctly rejects invalid MediaCapabilities dictionary and enum types</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Mediacapabilities_01</test_case_id>
    <test_objective>Ensure decodingInfo and encodingInfo API calls with invalid parameters reject promises with appropriate TypeErrors</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>mediacapabilities-types.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Console should show a series of PASS messages with rejection errors followed by “PASS successfullyParsed is true” and “TEST COMPLETE”</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_MediaCapabilities_MediacapabilitiesTypes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_MediaCapabilities_MediacapabilitiesTypes');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/mediacapabilities/mediacapabilities-types.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing mediacapabilities-types.html file")
            grep_line = "mediacapabilities-types | tail -3 | tr -d '\\n'"
            log_filename = "mediacapabilities-types"
...
if "mediacapabilities-types" in status_dict:
    print(f"mediacapabilities-types: {status_dict['mediacapabilities-types']}")
