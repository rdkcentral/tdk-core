...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_NavigateWithPip</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate stability of video playback in picture in picture mode during page navigation</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_104</test_case_id>
    <test_objective>To verify that navigating to another page while video is in picture in picture does not cause a crash and playback continues</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>navigate-with-pip-should-not-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The media element should remain in picture in picture mode playing without interruption and the video frame should load a blank page safely</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_NavigateWithPip</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_NavigateWithPip');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/navigate-with-pip-should-not-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing navigate-with-pip-should-not-crash.html file")
            grep_line = "navigate-with-pip-should-not-crash | tail -3 | tr -d '\\n'"
            log_filename = "navigate-with-pip-should-not-crash"
...
if "navigate-with-pip-should-not-crash" in status_dict:
    print(f"navigate-with-pip-should-not-crash: {status_dict['navigate-with-pip-should-not-crash']}")
