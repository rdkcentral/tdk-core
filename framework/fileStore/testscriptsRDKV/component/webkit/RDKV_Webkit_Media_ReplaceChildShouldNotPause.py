...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_ReplaceChildShouldNotPause</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate that moving a video element in the DOM does not pause playback</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_110</test_case_id>
    <test_objective>To verify that using replaceChild to move a video element keeps it playing</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>replaceChild-should-not-pause-video.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video element should remain unpaused throughout the DOM manipulation</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_ReplaceChildShouldNotPause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_ReplaceChildShouldNotPause');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/replaceChild-should-not-pause-video.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing replaceChild-should-not-pause-video.html file")
            grep_line = "replaceChild-should-not-pause-video | tail -3 | tr -d '\\n'"
            log_filename = "replaceChild-should-not-pause-video"
...
if "replaceChild-should-not-pause-video" in status_dict:
    print(f"replaceChild-should-not-pause-video: {status_dict['replaceChild-should-not-pause-video']}")
