...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcHeaderComment</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that the optional comment area under the WEBVTT file header is ignored and that default settings or styling are treated as faulty cues.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_31</test_case_id>
    <test_objective>To verify that cues are parsed correctly despite the presence of header comments and ignored default styling.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc005-header-comment.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>All cues display with correct identifiers, start and end times, and text content, ignoring comments and default styling.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcHeaderComment</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcHeaderComment');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc005-header-comment.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc005-header-comment.html file")
            grep_line = "track-webvtt-tc005-header-comment | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc005-header-comment"
...
if "track-webvtt-tc005-header-comment" in status_dict:
    print(f"track-webvtt-tc005-header-comment: {status_dict['track-webvtt-tc005-header-comment']}")
