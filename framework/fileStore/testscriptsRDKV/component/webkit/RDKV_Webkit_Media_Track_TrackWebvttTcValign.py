...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcValign</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests cue vertical alignment direction and related positioning from settings.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_41</test_case_id>
    <test_objective>To verify that vertical directions rl, lr and none along with their alignment and position values are parsed correctly.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc021-valign.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Cues show correct vertical direction, alignment, and position or remain empty when not set.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcValign</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcValign');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc021-valign.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc021-valign.html file")
            grep_line = "track-webvtt-tc021-valign | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc021-valign"
...
if "track-webvtt-tc021-valign" in status_dict:
    print(f"track-webvtt-tc021-valign: {status_dict['track-webvtt-tc021-valign']}")
