...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackId</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that the TextTrack id attribute is appropriately set</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_15</test_case_id>
    <test_objective>To verify that the TextTrack id has the correct default value, can be looked up by id, and remains readonly</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-id.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>TextTrack id equals the default value, lookup by id returns the correct track, and attempts to modify id do not change it</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackId</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackId');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-id.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-id.html file")
            grep_line = "track-id | tail -3 | tr -d '\\n'"
            log_filename = "track-id"
...
if "track-id" in status_dict:
    print(f"track-id: {status_dict['track-id']}")
