...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaControllerMediaSourcePlayThenPause</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the media controller media source play then pause details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_37</test_case_id>
    <test_objective>To get the media controller media source play then pause details from the device browser</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-controller-media-source-play-then-pause.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the media controller media source play then pause details</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaControllerMediaSourcePlayThenPause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaControllerMediaSourcePlayThenPause');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-source/media-controller-media-source-play-then-pause.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-controller-media-source-play-then-pause.html file")
            grep_line = "media-controller-media-source-play-then-pause | tail -3 | tr -d '\\n'"
            log_filename = "media-controller-media-source-play-then-pause"
...
if "media-controller-media-source-play-then-pause" in status_dict:
    print(f"media-controller-media-source-play-then-pause: {status_dict['media-controller-media-source-play-then-pause']}")
