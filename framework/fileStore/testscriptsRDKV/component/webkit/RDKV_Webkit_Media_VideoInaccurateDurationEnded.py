...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoInaccurateDurationEnded</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that the ended event fires for videos with inaccurate duration</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_129</test_case_id>
    <test_objective>To verify that playing a video with an inaccurate duration triggers the ended event at the end of playback</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-inaccurate-duration-ended.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video ended property should be true when playback completes</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoInaccurateDurationEnded</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoInaccurateDurationEnded');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-inaccurate-duration-ended.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-inaccurate-duration-ended.html file")
            grep_line = "video-inaccurate-duration-ended | tail -3 | tr -d '\\n'"
            log_filename = "video-inaccurate-duration-ended"
...
if "video-inaccurate-duration-ended" in status_dict:
    print(f"video-inaccurate-duration-ended: {status_dict['video-inaccurate-duration-ended']}")
