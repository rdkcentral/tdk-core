...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackDeleteDuringSetup</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests the initial state of a newly created TextTrack</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_11</test_case_id>
    <test_objective>To verify that a new TextTrack is in the disabled mode and has the correct readyState</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-delete-during-setup.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The TextTrack should have a readyState of 0 indicating it is not yet loaded, and its mode should be disabled, confirming it is inactive and not displayed or processed by the player</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackDeleteDuringSetup</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackDeleteDuringSetup');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-delete-during-setup.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-delete-during-setup.html file")
            grep_line = "track-delete-during-setup | tail -3 | tr -d '\\n'"
            log_filename = "track-delete-during-setup"
...
if "track-delete-during-setup" in status_dict:
    print(f"track-delete-during-setup: {status_dict['track-delete-during-setup']}")
