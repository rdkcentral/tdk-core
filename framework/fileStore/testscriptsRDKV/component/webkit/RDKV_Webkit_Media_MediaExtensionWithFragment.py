...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaExtensionWithFragment</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate media player behavior on loading media data.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_94</test_case_id>
    <test_objective>To verify that the media player extracts the file extension from the source URL upon data load.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-extension-with-fragment.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The media player should correctly detect and report the extension from the media source URL.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaExtensionWithFragment</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaExtensionWithFragment');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-extension-with-fragment.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-extension-with-fragment.html file")
            grep_line = "media-extension-with-fragment | tail -3 | tr -d '\\n'"
            log_filename = "media-extension-with-fragment"
...
if "media-extension-with-fragment" in status_dict:
    print(f"media-extension-with-fragment: {status_dict['media-extension-with-fragment']}")
