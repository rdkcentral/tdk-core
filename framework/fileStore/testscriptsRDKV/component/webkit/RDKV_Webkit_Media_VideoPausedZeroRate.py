...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoPausedZeroRate</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that setting playbackRate to 0 works correctly</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_138</test_case_id>
    <test_objective>To verify that the video can pause playback without changing the paused state when playbackRate is set to 0</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-paused-0-rate.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video currentTime should remain at 0 and paused state should remain false</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoPausedZeroRate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoPausedZeroRate');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-paused-0-rate.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-paused-0-rate.html file")
            grep_line = "video-paused-0-rate | tail -3 | tr -d '\\n'"
            log_filename = "video-paused-0-rate"
...
if "video-paused-0-rate" in status_dict:
    print(f"video-paused-0-rate: {status_dict['video-paused-0-rate']}")
