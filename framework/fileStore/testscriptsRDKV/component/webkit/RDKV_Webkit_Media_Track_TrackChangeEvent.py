...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackChangeEvent</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that the change event is fired when the mode of a TextTrack changes.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_07</test_case_id>
    <test_objective>To verify that modifying the mode of a TextTrack triggers a change event in the TextTrackList.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-change-event.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The change event is fired with the correct target, is an instance of Event, and does not include a track property.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackChangeEvent</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackChangeEvent');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-change-event.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-change-event.html file")
            grep_line = "track-change-event | tail -3 | tr -d '\\n'"
            log_filename = "track-change-event"
...
if "track-change-event" in status_dict:
    print(f"track-change-event: {status_dict['track-change-event']}")
