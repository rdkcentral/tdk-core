...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_MediaFragments_InvalidFragmentPlayFromStart</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>
    TC0027: To ensure that when an invalid media fragment ('t=banana') is requested, the user agent plays the entire media resource instead of attempting to play a fragment.
    TC0028: To ensure that when an invalid media fragment ('t=3,banana') is requested, the user agent plays the entire media resource from the start.
    TC0029: To ensure that when an invalid media fragment ('t=banana,7') is requested, the user agent plays the entire media resource from the start.
    TC0030: To ensure that when an invalid media fragment ('t='3'') is requested, the user agent plays the entire media resource from the start.
    TC0031: To ensure that when an invalid media fragment ('t=3-7') is requested, the user agent plays the entire media resource from the start.
    TC0032: To ensure that when an invalid media fragment ('t=3:7') is requested, the user agent plays the entire media resource from the start.
    TC0033: To ensure that when an invalid media fragment ('t=3,7,9') is requested, the user agent plays the entire media resource from the start.
  </synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Mediafragments_04</test_case_id>
    <test_objective>
      TC0027: To verify that an invalid media fragment is handled by playing the entire media resource from the start, and the current time is set to 0 seconds.
      TC0028: To verify that the user agent correctly handles an invalid media fragment and plays the entire media resource.
      TC0029: To verify that the user agent correctly handles an invalid media fragment and plays the entire media resource, starting at currentTime == 0.
      TC0030: To verify that the user agent correctly handles an invalid media fragment and plays the entire media resource, starting at currentTime == 0.
      TC0031: To verify that the user agent correctly handles an invalid media fragment and plays the entire media resource, starting at currentTime == 0.
      TC0032: To verify that the user agent correctly handles an invalid media fragment and plays the entire media resource, starting at currentTime == 0.
      TC0033: To verify that the user agent correctly handles an invalid media fragment with multiple values and plays the entire media resource, starting at currentTime == 0.
    </test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>TC0027.html, TC0028.html, TC0029.html, TC0030.html, TC0031.html, TC0032.html, TC0033.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    ...
    <test_script>RDKV_Webkit_MediaFragments_InvalidFragmentPlayFromStart</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_MediaFragments_InvalidFragmentPlayFromStart');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0027.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0028.html'
webkit_test_url3 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0029.html'
webkit_test_url4 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0030.html'
webkit_test_url5 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0031.html'
webkit_test_url6 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0032.html'
webkit_test_url7 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-fragments/TC0033.html'
