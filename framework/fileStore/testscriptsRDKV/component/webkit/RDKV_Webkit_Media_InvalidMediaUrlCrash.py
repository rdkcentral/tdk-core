...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_InvalidMediaUrlCrash</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate error handling for invalid media source URLs.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_71</test_case_id>
    <test_objective>Ensure that setting an invalid src on audio or video elements raises proper errors without crashing.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>invalid-media-url-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Both audio and video should report error code 4 and set networkState to 3, with no crash occurring.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_InvalidMediaUrlCrash</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_InvalidMediaUrlCrash');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/invalid-media-url-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing invalid-media-url-crash.html file")
            grep_line = "invalid-media-url-crash | tail -3 | tr -d '\\n'"
            log_filename = "invalid-media-url-crash"
...
if "invalid-media-url-crash" in status_dict:
    print(f"invalid-media-url-crash: {status_dict['invalid-media-url-crash']}")
