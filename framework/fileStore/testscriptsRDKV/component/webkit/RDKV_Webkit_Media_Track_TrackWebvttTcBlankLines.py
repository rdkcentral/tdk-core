...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcBlankLines</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cues handle multiple newline characters and unseparated cues correctly without altering intended text.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_27</test_case_id>
    <test_objective>To verify that cues maintain proper text content, IDs, and timing despite multiple newline formats and missing whitespace between cues.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc011-blank-lines.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Cues have correct length, start and end times, IDs, and text content as defined, including merged portions when cues are unseparated.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcBlankLines</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcBlankLines');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc011-blank-lines.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc011-blank-lines.html file")
            grep_line = "track-webvtt-tc011-blank-lines | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc011-blank-lines"
...
if "track-webvtt-tc011-blank-lines" in status_dict:
    print(f"track-webvtt-tc011-blank-lines: {status_dict['track-webvtt-tc011-blank-lines']}")
