...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_ElementContainingPipVideo</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate interaction between Picture-in-Picture and fullscreen modes.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_65</test_case_id>
    <test_objective>Verify that when a video in PiP enters fullscreen, it exits PiP and switches to inline mode.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>element-containing-pip-video-going-into-fullscreen.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should transition from PiP to inline when fullscreen is activated, with correct presentation mode updates.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_ElementContainingPipVideo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_ElementContainingPipVideo');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/element-containing-pip-video-going-into-fullscreen.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing element-containing-pip-video-going-into-fullscreen.html file")
            grep_line = "element-containing-pip-video-going-into-fullscreen | tail -3 | tr -d '\\n'"
            log_filename = "element-containing-pip-video-going-into-fullscreen"
...
if "element-containing-pip-video-going-into-fullscreen" in status_dict:
    print(f"element-containing-pip-video-going-into-fullscreen: {status_dict['element-containing-pip-video-going-into-fullscreen']}")
