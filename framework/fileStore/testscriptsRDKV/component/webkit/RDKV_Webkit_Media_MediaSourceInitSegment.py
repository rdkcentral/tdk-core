...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaSourceInitSegment</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validates Media Source Extensions handling of movie fragment duration from the mvex header</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_21</test_case_id>
    <test_objective>To ensure SourceBuffer correctly interprets the mvex header to determine the media duration</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-source-init-segment-duration.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Video duration is detected as 10 confirming correct parsing of the mvex header</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaSourceInitSegment</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaSourceInitSegment');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-source/media-source-init-segment-duration.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-source-init-segment-duration.html file")
            grep_line = "media-source-init-segment-duration | tail -3 | tr -d '\\n'"
            log_filename = "media-source-init-segment-duration"
...
if "media-source-init-segment-duration" in status_dict:
    print(f"media-source-init-segment-duration: {status_dict['media-source-init-segment-duration']}")
