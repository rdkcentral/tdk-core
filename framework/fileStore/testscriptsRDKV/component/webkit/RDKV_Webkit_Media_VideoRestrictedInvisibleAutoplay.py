...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoRestrictedInvisibleAutoplay</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that invisible autoplay restriction does not pause audible media when scrolled out of view</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_147</test_case_id>
    <test_objective>To ensure video continues playing even when hidden or scrolled out of viewport</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-restricted-invisible-autoplay-allowed-if-audible.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Video remains unpaused throughout all style changes and scrolling</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoRestrictedInvisibleAutoplay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoRestrictedInvisibleAutoplay');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-restricted-invisible-autoplay-allowed-if-audible.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-restricted-invisible-autoplay-allowed-if-audible.html file")
            grep_line = "video-restricted-invisible-autoplay-allowed-if-audible | tail -3 | tr -d '\\n'"
            log_filename = "video-restricted-invisible-autoplay-allowed-if-audible"
...
if "video-restricted-invisible-autoplay-allowed-if-audible" in status_dict:
    print(f"video-restricted-invisible-autoplay-allowed-if-audible: {status_dict['video-restricted-invisible-autoplay-allowed-if-audible']}")
