...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoFastSeek</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that fastSeek commands move the video to the correct time positions</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_128</test_case_id>
    <test_objective>To verify that fastSeek correctly seeks forward or backward without overshooting or undershooting the target time</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-fast-seek.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video currentTime should match the intended seek position within expected bounds after each fastSeek operation</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoFastSeek</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoFastSeek');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-fast-seek.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-fast-seek.html file")
            grep_line = "video-fast-seek | tail -3 | tr -d '\\n'"
            log_filename = "video-fast-seek"
...
if "video-fast-seek" in status_dict:
    print(f"video-fast-seek: {status_dict['video-fast-seek']}")
