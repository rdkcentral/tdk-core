...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TracklistIsReachable</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that a TrackList with custom properties is not garbage collected</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_20</test_case_id>
    <test_objective>To verify that adding a custom property to a TrackList preserves the object and its properties</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>tracklist-is-reachable.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>TrackList length remains one and custom property retains its value after garbage collection</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TracklistIsReachable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TracklistIsReachable');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/tracklist-is-reachable.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing tracklist-is-reachable.html file")
            grep_line = "tracklist-is-reachable | tail -3 | tr -d '\\n'"
            log_filename = "tracklist-is-reachable"
...
if "tracklist-is-reachable" in status_dict:
    print(f"tracklist-is-reachable: {status_dict['tracklist-is-reachable']}")
