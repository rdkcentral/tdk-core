...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediElementPlayAfterEos</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate media element events during repeated playback cycles after end of stream</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_103</test_case_id>
    <test_objective>To verify that the playing event is emitted each time playback restarts and that pause and ended events occur correctly at the end</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-element-play-after-eos.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The media element should trigger playing on restart and send pause and ended events at playback completion each time</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediElementPlayAfterEos</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediElementPlayAfterEos');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-element-play-after-eos.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-element-play-after-eos.html file")
            grep_line = "media-element-play-after-eos | tail -3 | tr -d '\\n'"
            log_filename = "media-element-play-after-eos"
...
if "media-element-play-after-eos" in status_dict:
    print(f"media-element-play-after-eos: {status_dict['media-element-play-after-eos']}")
