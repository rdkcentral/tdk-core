...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackLegacyapiWithAutomaticMode</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests enabling a text track with webkitClosedCaptionsVisible when caption display mode is set to automatic</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_17</test_case_id>
    <test_objective>To verify that a captions track can be enabled in automatic mode, loads cues correctly, and displays them when the video plays</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-legacyapi-with-automatic-mode.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>TextTrack has mode disabled initially, then cues are loaded and visible after enabling captions</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackLegacyapiWithAutomaticMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackLegacyapiWithAutomaticMode');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-legacyapi-with-automatic-mode.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-legacyapi-with-automatic-mode.html file")
            grep_line = "track-legacyapi-with-automatic-mode | tail -3 | tr -d '\\n'"
            log_filename = "track-legacyapi-with-automatic-mode"
...
if "track-legacyapi-with-automatic-mode" in status_dict:
    print(f"track-legacyapi-with-automatic-mode: {status_dict['track-legacyapi-with-automatic-mode']}")
