...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_NetworkNoSource</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate immutability of the NETWORK_NO_SOURCE property in media elements</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_105</test_case_id>
    <test_objective>To verify that NETWORK_NO_SOURCE remains constant across HTMLMediaElement and HTMLAudioElement instances</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>network-no-source-const-shadow.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>NETWORK_NO_SOURCE should consistently equal 3 and not be modified on any element instance</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_NetworkNoSource</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_NetworkNoSource');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/network-no-source-const-shadow.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing network-no-source-const-shadow.html file")
            grep_line = "network-no-source-const-shadow | tail -3 | tr -d '\\n'"
            log_filename = "network-no-source-const-shadow"
...
if "network-no-source-const-shadow" in status_dict:
    print(f"network-no-source-const-shadow: {status_dict['network-no-source-const-shadow']}")
