...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_EventAttributes</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate video element behavior across playback, rate, volume, seeking, and error scenarios.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_66</test_case_id>
    <test_objective>Verify correct event handling during play, pause, rate/volume changes, seeking, playback completion, and error when an invalid source is set.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>event-attributes.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should trigger appropriate events for each operation and raise an error with no progress events after an invalid source is loaded.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_EventAttributes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_EventAttributes');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/event-attributes.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing event-attributes.html file")
            grep_line = "event-attributes | tail -3 | tr -d '\\n'"
            log_filename = "event-attributes"
...
if "event-attributes" in status_dict:
    print(f"event-attributes: {status_dict['event-attributes']}")
