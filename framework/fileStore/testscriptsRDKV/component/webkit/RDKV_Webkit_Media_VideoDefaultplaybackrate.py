...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoDefaultplaybackrate</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates how the muted content attribute and the muted defaultMuted IDL attributes behave in different scenarios.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_121</test_case_id>
    <test_objective>To check default values, reflection rules, and attribute independence before and after attaching the video element to the document.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-defaultplaybackrate-before-load.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The muted attribute and IDL properties reflect correctly before insertion, behave independently after changes, and stop reflecting once the muted IDL attribute is explicitly set.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoDefaultplaybackrate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoDefaultplaybackrate');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-defaultplaybackrate-before-load.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-defaultplaybackrate-before-load.html file")
            grep_line = "video-defaultplaybackrate-before-load | tail -3 | tr -d '\\n'"
            log_filename = "video-defaultplaybackrate-before-load"
...
if "video-defaultplaybackrate-before-load" in status_dict:
    print(f"video-defaultplaybackrate-before-load: {status_dict['video-defaultplaybackrate-before-load']}")
