...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcNewlines</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cues are correctly parsed when line terminators are carriage return, line feed, or both, including files without a newline at the end.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_36</test_case_id>
    <test_objective>To verify that cues remain intact and all cue properties are correctly recognized regardless of line-ending style or missing final newline.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc003-newlines.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Each text track contains one cue with the correct ID, start time, end time, and text content as Bear is Coming</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcNewlines</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcNewlines');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc003-newlines.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc003-newlines.html file")
            grep_line = "track-webvtt-tc003-newlines | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc003-newlines"
...
if "track-webvtt-tc003-newlines" in status_dict:
    print(f"track-webvtt-tc003-newlines: {status_dict['track-webvtt-tc003-newlines']}")
