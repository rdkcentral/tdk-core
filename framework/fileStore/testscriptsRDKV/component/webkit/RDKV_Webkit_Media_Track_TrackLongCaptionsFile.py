...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackLongCaptionsFile</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that all captions in a long WebVTT file are parsed correctly</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_19</test_case_id>
    <test_objective>To verify that a TextTrack contains all expected cues and that cue parsing works for large WebVTT files</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-long-captions-file.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>TextTrack has a length greater than zero, mode is hidden, and all 100 cues are present</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackLongCaptionsFile</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackLongCaptionsFile');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-long-captions-file.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-long-captions-file.html file")
            grep_line = "track-long-captions-file | tail -3 | tr -d '\\n'"
            log_filename = "track-long-captions-file"
...
if "track-long-captions-file" in status_dict:
    print(f"track-long-captions-file: {status_dict['track-long-captions-file']}")
