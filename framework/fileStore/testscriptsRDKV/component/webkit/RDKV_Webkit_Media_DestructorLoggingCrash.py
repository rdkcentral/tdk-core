...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_DestructorLoggingCrash</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate stability during video element destruction</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_64</test_case_id>
    <test_objective>Ensure that logging from the video element destructor does not cause crashes.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>destructor-logging-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video element should be destroyed cleanly with successful parsing and no crashes during destructor logging.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_DestructorLoggingCrash</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_DestructorLoggingCrash');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/destructor-logging-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing destructor-logging-crash.html file")
            grep_line = "destructor-logging-crash | tail -3 | tr -d '\\n'"
            log_filename = "destructor-logging-crash"
...
if "destructor-logging-crash" in status_dict:
    print(f"destructor-logging-crash: {status_dict['destructor-logging-crash']}")
