...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackNodeAddRemove</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests adding and removing text tracks on a video element outside and inside the DOM</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_22</test_case_id>
    <test_objective>To verify that textTracks updates correctly when tracks are added, inserted, removed, or when the video element is inserted or removed from the document</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-node-add-remove.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The correct number of tracks is maintained in order, and references to the tracks are updated as they are manipulated.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackNodeAddRemove</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackNodeAddRemove');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-node-add-remove.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-node-add-remove.html file")
            grep_line = "track-node-add-remove | tail -3 | tr -d '\\n'"
            log_filename = "track-node-add-remove"
...
if "track-node-add-remove" in status_dict:
    print(f"track-node-add-remove: {status_dict['track-node-add-remove']}")
