...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoSeeking</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify video seeking behavior and seeked event firing</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_151</test_case_id>
    <test_objective>To ensure seeking attribute is true during a seek, false after completion, and seeked event occurs for each seek</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-seeking.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Seeking becomes true when currentTime is set, false after seek completes, currentTime reflects the seeked value, and a seeked event is fired each time</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoSeeking</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoSeeking');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-seeking.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-seeking.html file")
            grep_line = "video-seeking | tail -3 | tr -d '\\n'"
            log_filename = "video-seeking"
...
if "video-seeking" in status_dict:
    print(f"video-seeking: {status_dict['video-seeking']}")
