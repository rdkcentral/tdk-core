...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackBcpLanguage</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that TextTrackBase correctly handles setting and resetting BCP 47 language tags.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_06</test_case_id>
    <test_objective>To verify that valid tags are recognized, invalid tags remain unchanged, and language values update correctly.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-bcp-language.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Valid BCP 47 tags map properly, while invalid ones do not reset, ensuring accurate language handling.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackBcpLanguage</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackBcpLanguage');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-bcp-language.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-bcp-language.html file")
            grep_line = "track-bcp-language | tail -3 | tr -d '\\n'"
            log_filename = "track-bcp-language"
...
if "track-bcp-language" in status_dict:
    print(f"track-bcp-language: {status_dict['track-bcp-language']}")
