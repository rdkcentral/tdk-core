...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackDisplayBeforeControlsCrash</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests triggering track display before media controls are created</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_12</test_case_id>
    <test_objective>To ensure that triggering track display does not cause a crash even if media controls are not yet initialized</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-display-before-controls-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Test completes successfully without errors and is marked as successfullyParsed</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackDisplayBeforeControlsCrash</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackDisplayBeforeControlsCrash');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-display-before-controls-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-display-before-controls-crash.html file")
            grep_line = "track-display-before-controls-crash | tail -3 | tr -d '\\n'"
            log_filename = "track-display-before-controls-crash"
...
if "track-display-before-controls-crash" in status_dict:
    print(f"track-display-before-controls-crash: {status_dict['track-display-before-controls-crash']}")
