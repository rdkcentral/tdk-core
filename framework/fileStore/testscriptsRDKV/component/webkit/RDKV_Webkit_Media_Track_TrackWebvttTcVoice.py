...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcVoice</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests cues containing voice markup v.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_42</test_case_id>
    <test_objective>To verify that cues with voice markup are parsed and recognized correctly.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc026-voice.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Cues lengths and content are correct for all text tracks.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcVoice</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcVoice');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc026-voice.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc026-voice.html file")
            grep_line = "track-webvtt-tc026-voice | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc026-voice"
...
if "track-webvtt-tc026-voice" in status_dict:
    print(f"track-webvtt-tc026-voice: {status_dict['track-webvtt-tc026-voice']}")
