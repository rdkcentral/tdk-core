...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcTimestamp</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that cues containing timestamp tags are parsed correctly and displayed in the intended order, ignoring out-of-range timestamps.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_39</test_case_id>
    <test_objective>To verify that cue text with timestamp tags is rendered accurately and that invalid timestamps before or after the cue boundaries are ignored.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc024-timestamp.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The first track has three cues This cue is painted on I said Bear is coming I said Bear is coming now The second track has three cues with out of order timestamps ignored</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcTimestamp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcTimestamp');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc024-timestamp.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc024-timestamp.html file")
            grep_line = "track-webvtt-tc024-timestamp | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc024-timestamp"
...
if "track-webvtt-tc024-timestamp" in status_dict:
    print(f"track-webvtt-tc024-timestamp: {status_dict['track-webvtt-tc024-timestamp']}")
