...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_MediaQueries_DynamicRange</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test checks if the device browser can understand and respond to dynamic-range and video-dynamic-range media queries</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>MediaQueries_01</test_case_id>
    <test_objective>To confirm that correct values like standard are detected as valid, while wrong or invalid values are not</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>dynamic-range.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should show PASS logs by matching valid queries and rejecting invalid ones, proving it can get dynamic range details</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_MediaQueries_DynamicRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_MediaQueries_DynamicRange');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/imported/w3c/web-platform-tests/css/mediaqueries/dynamic-range.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing dynamic-range.html file")
            grep_line = "dynamic-range | tail -3 | tr -d '\\n'"
            log_filename = "dynamic-range"
...
if "dynamic-range" in status_dict:
    print(f"dynamic-range: {status_dict['dynamic-range']}")
