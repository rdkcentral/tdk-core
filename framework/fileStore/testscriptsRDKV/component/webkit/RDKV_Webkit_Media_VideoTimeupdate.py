...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoTimeupdate</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate timeupdate event behavior when video plays in reverse.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_157</test_case_id>
    <test_objective>To ensure that playing backward to time zero triggers timeupdate without marking video as ended.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-timeupdate-reverse-play.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Timeupdate fires at zero, currentTime resets correctly, ended remains false.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoTimeupdate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoTimeupdate');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-timeupdate-reverse-play.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-timeupdate-reverse-play.html file")
            grep_line = "video-timeupdate-reverse-play | tail -3 | tr -d '\\n'"
            log_filename = "video-timeupdate-reverse-play"
...
if "video-timeupdate-reverse-play" in status_dict:
    print(f"video-timeupdate-reverse-play: {status_dict['video-timeupdate-reverse-play']}")
