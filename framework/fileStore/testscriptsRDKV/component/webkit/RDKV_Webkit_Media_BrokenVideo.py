...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_BrokenVideo</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate error handling for unsupported video sources.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_58</test_case_id>
    <test_objective>Verify that loading an invalid video triggers MEDIA_ERR_SRC_NOT_SUPPORTED and updates networkState correctly.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>broken-video.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should raise an error with code 4 and set networkState to NETWORK_NO_SOURCE</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_BrokenVideo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_BrokenVideo');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/broken-video.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing broken-video.html file")
            grep_line = "broken-video | tail -3 | tr -d '\\n'"
            log_filename = "broken-video"
...
if "broken-video" in status_dict:
    print(f"broken-video: {status_dict['broken-video']}")
