...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoLoop</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates the looping behavior of the video element</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_132</test_case_id>
    <test_objective>To verify that setting loop causes the video to restart automatically at the end and that removing loop stops automatic looping</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-loop.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The video should loop when loop is true, trigger seeked events at the restart, and end normally with ended true when loop is false</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoLoop</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoLoop');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-loop.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-loop.html file")
            grep_line = "video-loop | tail -3 | tr -d '\\n'"
            log_filename = "video-loop"
...
if "video-loop" in status_dict:
    print(f"video-loop: {status_dict['video-loop']}")
