...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaCaptions</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate the closed captions API of media elements.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_86</test_case_id>
    <test_objective>Verify that webkitHasClosedCaptions and webkitClosedCaptionsVisible reflect the presence and visibility of captions correctly for audio-only and captioned video files.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-captions.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The media element should indicate no captions for audio-only files, detect captions in video files, and correctly toggle visibility when captions are enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaCaptions</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaCaptions');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-captions.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-captions.html file")
            grep_line = "media-captions | tail -3 | tr -d '\\n'"
            log_filename = "media-captions"
...
if "media-captions" in status_dict:
    print(f"media-captions: {status_dict['media-captions']}")
