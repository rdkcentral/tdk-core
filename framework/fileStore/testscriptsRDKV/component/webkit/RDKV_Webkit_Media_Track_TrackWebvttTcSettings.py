...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcSettings</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that WebVTT cue settings such as line, position, alignment, and vertical direction are correctly applied for each cue.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_38</test_case_id>
    <test_objective>To verify that cues reflect the expected line number, position value, alignment, and vertical orientation according to their WebVTT track settings.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc013-settings.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Text track 0 has four cues with line, position, alignment, and vertical values as 100 auto start none, 15 40 center right-to-left, auto 10 center none, and 95 auto end left-to-right, and text track 1 has four cues with line, position, alignment, and vertical values as 43 10 center none, auto 50 end none, auto auto center none, and auto 90 center left-to-right.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcSettings</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcSettings');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc013-settings.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc013-settings.html file")
            grep_line = "track-webvtt-tc013-settings | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc013-settings"
...
if "track-webvtt-tc013-settings" in status_dict:
    print(f"track-webvtt-tc013-settings: {status_dict['track-webvtt-tc013-settings']}")
