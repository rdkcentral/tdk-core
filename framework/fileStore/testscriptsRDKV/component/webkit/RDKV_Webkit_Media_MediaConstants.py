...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaConstants</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate HTMLMediaElement and MediaError constants.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_87</test_case_id>
    <test_objective>To verify that all media network, readyState, and error constants are correctly defined and accessible.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-constants.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The constants should match their standard numeric values as specified by the HTMLMediaElement and MediaError APIs.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaConstants</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaConstants');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-constants.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-constants.html file")
            grep_line = "media-constants | tail -3 | tr -d '\\n'"
            log_filename = "media-constants"
...
if "media-constants" in status_dict:
    print(f"media-constants: {status_dict['media-constants']}")
