...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackAddTrack</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that the addtrack event is fired when a new text track is created.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_04</test_case_id>
    <test_objective>To verify that adding a track through an element or programmatically triggers the addtrack event.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-add-track.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The addtrack event is fired in both cases and is correctly associated with a text track.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackAddTrack</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackAddTrack');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-add-track.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-add-track.html file")
            grep_line = "track-add-track | tail -3 | tr -d '\\n'"
            log_filename = "track-add-track"
...
if "track-add-track" in status_dict:
    print(f"track-add-track: {status_dict['track-add-track']}")
