...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoPageLoadPreloadNone</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test validates that the document load event is not delayed by videos with preload set to none</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_136</test_case_id>
    <test_objective>To verify that setting preload to none on a video does not block or delay the document load event</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-page-load-preload-none.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The document load event should fire normally regardless of the video preload setting</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoPageLoadPreloadNone</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoPageLoadPreloadNone');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-page-load-preload-none.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-page-load-preload-none.html file")
            grep_line = "video-page-load-preload-none | tail -3 | tr -d '\\n'"
            log_filename = "video-page-load-preload-none"
...
if "video-page-load-preload-none" in status_dict:
    print(f"video-page-load-preload-none: {status_dict['video-page-load-preload-none']}")
