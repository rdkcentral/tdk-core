...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaCanPlayOctetStream</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate canPlayType behavior with unsupported MIME types.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_81</test_case_id>
    <test_objective>To verify that media elements correctly reject octet-stream and its codec variants.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-can-play-octet-stream.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The method should return an empty string for all tested unsupported MIME types.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaCanPlayOctetStream</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaCanPlayOctetStream');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-can-play-octet-stream.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-can-play-octet-stream.html file")
            grep_line = "media-can-play-octet-stream | tail -3 | tr -d '\\n'"
            log_filename = "media-can-play-octet-stream"
...
if "media-can-play-octet-stream" in status_dict:
    print(f"media-can-play-octet-stream: {status_dict['media-can-play-octet-stream']}")
