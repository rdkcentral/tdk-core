...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcEntities</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that special characters such as ampersand, less than, greater than, and direction or spacing marks are correctly parsed in cue text.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_30</test_case_id>
    <test_objective>To verify that cues containing special characters and non-breaking or directional marks preserve their text content accurately.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc022-entities.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>All cues display the correct text content with special characters and marks preserved as intended.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcEntities</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcEntities');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc022-entities.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc022-entities.html file")
            grep_line = "track-webvtt-tc022-entities | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc022-entities"
...
if "track-webvtt-tc022-entities" in status_dict:
    print(f"track-webvtt-tc022-entities: {status_dict['track-webvtt-tc022-entities']}")
