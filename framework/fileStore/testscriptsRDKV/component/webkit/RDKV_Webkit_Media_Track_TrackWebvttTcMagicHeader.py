...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackWebvttTcMagicHeader</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that a file with the header WEBVTT is correctly recognized as a WebVTT file and parsed into cues.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_34</test_case_id>
    <test_objective>To verify that cues are created with the correct IDs, start and end times, and text content when the WEBVTT header is present.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-webvtt-tc004-magic-header.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Tracks 0 and 1 each have two cues with IDs 1 and 2, correct start and end times, and the expected text. Track 2 has no cues.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackWebvttTcMagicHeader</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackWebvttTcMagicHeader');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-webvtt-tc004-magic-header.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-webvtt-tc004-magic-header.html file")
            grep_line = "track-webvtt-tc004-magic-header | tail -3 | tr -d '\\n'"
            log_filename = "track-webvtt-tc004-magic-header"
...
if "track-webvtt-tc004-magic-header" in status_dict:
    print(f"track-webvtt-tc004-magic-header: {status_dict['track-webvtt-tc004-magic-header']}")
