...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_VideoTrackMkvTheora</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify video track addtrack events, language attributes, and selected track behavior for MKV files with multiple audio and video tracks.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_51</test_case_id>
    <test_objective>Ensure that:
1. Video track "addtrack" events are correctly triggered for MKV with multiple video tracks.
2. Video track language attributes match the expected values (V0 = zh, V1 = ru).
3. Only one video track is selected at a time, and selection changes fire the "change" event with correct selectedIndex.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-track-mkv-theora-addtrack.html, video-track-mkv-theora-language.html, video-track-mkv-theora-selected.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>For each HTML test:
- "addtrack" events for video tracks are logged successfully.
- Language attributes for video tracks are correctly reported.
- Track selection behavior is correct: initial track selected is track 0, selecting track 1 deselects track 0, and "change" events reflect the proper selected track.
- "END OF TEST" is logged after all validations.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_VideoTrackMkvTheora</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_VideoTrackMkvTheora');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/video/video-track-mkv-theora-addtrack.html'
webkit_test_url2 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/video/video-track-mkv-theora-language.html'
webkit_test_url3 = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/video/video-track-mkv-theora-selected.html'
