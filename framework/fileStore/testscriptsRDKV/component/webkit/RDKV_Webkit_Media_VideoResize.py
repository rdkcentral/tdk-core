...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_VideoResize</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify initial resize event for video element</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_146</test_case_id>
    <test_objective>Ensure videoWidth and videoHeight update correctly when resize event fires</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>video-resize.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Initial videoWidth and videoHeight are zero, after resize they are 320 and 240 respectively</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_VideoResize</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_VideoResize');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/video-resize.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing video-resize.html file")
            grep_line = "video-resize | tail -3 | tr -d '\\n'"
            log_filename = "video-resize"
...
if "video-resize" in status_dict:
    print(f"video-resize: {status_dict['video-resize']}")
