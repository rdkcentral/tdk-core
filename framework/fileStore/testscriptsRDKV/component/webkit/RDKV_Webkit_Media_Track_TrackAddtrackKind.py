...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_Track_TrackAddtrackKind</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To tests that addTextTrack only accepts valid kind values.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Track_05</test_case_id>
    <test_objective>To verify that invalid kind values throw an error and valid keywords are correctly accepted.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>track-addtrack-kind.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>Invalid kind results in a type error, while valid kinds successfully create new text tracks.</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_Track_TrackAddtrackKind</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_Track_TrackAddtrackKind');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/track/track-addtrack-kind.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing track-addtrack-kind.html file")
            grep_line = "track-addtrack-kind | tail -3 | tr -d '\\n'"
            log_filename = "track-addtrack-kind"
...
if "track-addtrack-kind" in status_dict:
    print(f"track-addtrack-kind: {status_dict['track-addtrack-kind']}")
