...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaCanPlayMp3</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate canPlayType support for MP3 MIME types.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_78</test_case_id>
    <test_objective>Verify that the media element correctly identifies standard and non-standard MP3 codec strings.</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-can-play-mp3.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The method should return probably for recognized MP3 codecs, maybe for generic audio</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaCanPlayMp3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaCanPlayMp3');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-can-play-mp3.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-can-play-mp3.html file")
            grep_line = "media-can-play-mp3 | tail -3 | tr -d '\\n'"
            log_filename = "media-can-play-mp3"
...
if "media-can-play-mp3" in status_dict:
    print(f"media-can-play-mp3: {status_dict['media-can-play-mp3']}")
