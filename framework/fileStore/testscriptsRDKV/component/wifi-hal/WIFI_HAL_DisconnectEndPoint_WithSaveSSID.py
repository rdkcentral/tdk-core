...
<xml>
  <id/>
  <version>1</version>
  <name>WIFI_HAL_DisconnectEndPoint_WithSaveSSID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFI_HAL_ConnectEndpoint</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the last connected endpoint details are retrieved after reboot also after enabling the saveSSID flag</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFI_HAL_31</test_case_id>
...
3. Using WIFI_HAL_ConnectEndpoint, invoke wifi_connectEndpoint() api and connect to a new access point with saveSSID = 1
...
6.If the ssid details returned wifi_lastConnected_Endpoint() and the connected SSID details are same, then return SUCCESS,else FAILURE.
7.Unload the module."</automation_approch>
    <except_output>ssid details returned from wifi_lastConnected_Endpoint() and the connected SSID details should be same</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>WIFI_HAL_DisconnectEndPoint_WithSaveSSID</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'WIFI_HAL_DisconnectEndPoint_WithSaveSSID');
