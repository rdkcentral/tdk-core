...
<xml>
  <id/>
  <version>2</version>
  <name>WIFI_HAL_5GHzGetRadioExtChannel</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFI_HAL_8</test_case_id>
...
2.Invoke the api wifi_getRadioExtChannel() to get the secondary extension channel position for radio 5GHz.
3.If the position is from the list of expected positions return SUCCESS,else FAILURE.
4.Unload the module.</automation_approch>
    <except_output>To get the secondary extension channel position for radio 5GHz using wifi_getRadioExtChannel HAL API.</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFI_HAL</test_stub_interface>
    <test_script>WIFI_HAL_5GHzGetRadioExtChannel</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'WIFI_HAL_5GHzGetRadioExtChannel');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if Channel in ChannelList:
            ...
            print("TEST STEP 1: Get the Radio Extension Channel for 5GHz");
            print("EXPECTED RESULT 1: Should get the Radio Extension Channel either as 'AboveControlChannel' or 'BelowControlChannel' or 'Auto'for 5GHz");
