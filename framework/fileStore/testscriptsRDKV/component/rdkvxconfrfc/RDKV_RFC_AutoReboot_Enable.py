...
<xml>
  ...
  <name>RDKV_RFC_AutoReboot_Enable</name>
  ...
  <test_cases>
    <test_case_id>rdkvxconfrfc_01</test_case_id>
    <test_objective>Verify whether the given xconf server setting for Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.Enable is reflected in the box</test_objective>
...
5: AutoReboot.Enable RFC parameter value should be checked and noted before making changes
...
10: Confirm that the value set for the AutoReboot.Enable RFC parameter on the Xconf server is correctly applied and reflected in the device
11: AutoReboot.Enable RFC parameter value must be reverted to its original state
12: Feature and its corresponding rule must be deleted from the Xconf server</expected_output>
    ...
    <test_script>RDKV_RFC_AutoReboot_Enable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_RFC_AutoReboot_Enable');
        ...
        if "FAILURE" not in detail:
            ...
            if "FAILURE" not in detail:
                ...
                if "FAILURE" not in detail:
                    ...
                    rfcparameter="Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AutoReboot.Enable"
