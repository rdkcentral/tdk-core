...
<xml>
  ...
  <name>RDKV_Memcr_Check_YouTubeTV_App_Compressed_File_Availability</name>
  ...
  <test_cases>
    <test_case_id>rdkvmemcr_16</test_case_id>
    <test_objective>Verify that the compressed app file (.img format) is created during suspension, and confirm that it is no longer present after restoring</test_objective>
...
5. Verify that the compressed app file (.img format) is created during suspension
6. After restoring, ensure that the compressed app file is no longer present</automation_approch>
...
5. A compressed app file in .img format should be created when the app is suspended
6. After the app is restored, the compressed .img file should no longer be present</expected_output>
    <priority>Medium</priority>
    <test_stub_interface>Nil</test_stub_interface>
    <test_script>RDKV_Memcr_Check_YouTubeTV_App_Compressed_File_Availability</test_script>
    <skipped>Nil</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Memcr_Check_YouTubeTV_App_Compressed_File_Availability');
                            ...
                            if "SUCCESS" in result:
                                ...
                                if "SUCCESS" in result:
                                    ...
                                    if str(success).lower() == "true":
                                        ...
                                        tdkTestObj.addParameter("checkfile","nonemptyfilecheck")
                                        tdkTestObj.executeTestCase(expectedResult)
                                        result = tdkTestObj.getResultDetails()
                                        if "SUCCESS" in result:
                                            tdkTestObj.setResultStatus("SUCCESS")
