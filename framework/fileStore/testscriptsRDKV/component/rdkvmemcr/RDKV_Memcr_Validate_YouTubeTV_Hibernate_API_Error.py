...
<xml>
  ...
  <name>RDKV_Memcr_Validate_YouTubeTV_Hibernate_API_Error</name>
  ...
  <test_cases>
    <test_case_id>rdkvmemcr_22</test_case_id>
...
5. Check if the youtubetv app enters a hibernated state by using the Home keycode(36), without relying on the suspend API
6. Simulating a hibernate state and then triggering a hibernate API call to verify error handling</automation_approch>
...
6. If the YouTube TV app is already in a hibernated state, invoking the hibernate API should result in a proper error message</expected_output>
    <priority>Medium</priority>
    <test_stub_interface>Nil</test_stub_interface>
    <test_script>RDKV_Memcr_Validate_YouTubeTV_Hibernate_API_Error</test_script>
    <skipped>Nil</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Memcr_Validate_YouTubeTV_Hibernate_API_Error');
