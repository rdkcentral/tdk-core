...
<xml>
  ...
  <name>RDKV_WebPA_ModelName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the device model name using the WebPA interface.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_23</test_case_id>
    <test_objective>To validate the device model name using the WebPA interface.</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.ModelName will fetch the WebPA server url from the device.
...
2. API must return the Model Name</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_ModelName</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_ModelName')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.ModelName"
