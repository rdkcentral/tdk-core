...
<xml>
  ...
  <name>RDKV_WebPA_SpeakerAudioPortEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the SPEAKER audio port enable status using the WebPA server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_25</test_case_id>
    <test_objective>To validate the SPEAKER audio port enable status using the WebPA server</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.DisplaySettings.AudioPort.SPEAKER.Enabled will fetch the WebPA server url from the device.
...
2. API must return the audio port is enable or not</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_SpeakerAudioPortEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_SpeakerAudioPortEnable')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.DisplaySettings.AudioPort.SPEAKER.Enabled"
    testValue = "1"
