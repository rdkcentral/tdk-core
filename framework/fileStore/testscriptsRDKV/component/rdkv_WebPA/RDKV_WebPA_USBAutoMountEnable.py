...
<xml>
  ...
  <name>RDKV_WebPA_USBAutoMountEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the USB AutoMount feature enable status using the WebPA server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_20</test_case_id>
    <test_objective>To validate the USB AutoMount feature enable status using the WebPA server</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.USB_AutoMount.Enable will fetch the WebPA server url from the device.
...
2.RFC parameter via Webpa server should return Auto Mount of USB is ebaled or not</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_USBAutoMountEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_USBAutoMountEnable')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.USB_AutoMount.Enable"
    testValue = "1"
