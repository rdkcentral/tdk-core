...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the CPU temperature reported by the device using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_22</test_case_id>
    <test_objective>To validate the CPU temperature reported by the device using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM.CPUTemp will fetch the WebPA server url from the device.
...
2. API must return the CPU Temperature</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_CPUTemp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_CPUTemp')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM.CPUTemp"
    threshold_value = "100"
    ...
    if "SUCCESS" in detail:
        ...
        print("CPU Temperature : ", original_value)
        data_type = detail[2]
        if "SUCCESS" in detail:
            tdkTestObj.setResultStatus("SUCCESS")
            ...
            if "SUCCESS" in detail:
                tdkTestObj.setResultStatus("SUCCESS")
                print("\n CPU Temperature is obtained successfully")
            else:
