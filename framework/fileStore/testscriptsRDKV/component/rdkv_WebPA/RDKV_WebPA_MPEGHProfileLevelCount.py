...
<xml>
  ...
  <name>RDKV_WebPA_MPEGHProfileLevelCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the MPEG-H Part2 VideoDecoder profile level entries using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_24</test_case_id>
    <test_objective>To validate the MPEG-H Part2 VideoDecoder profile level entries using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.Services.STBService.1.Capabilities.VideoDecoder.X_RDKCENTRAL-COM_MPEGHPart2.ProfileLevelNumberOfEntries will fetch the WebPA server url from the device.
...
2. API must return the vdeo decoder profiles</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_MPEGHProfileLevelCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_MPEGHProfileLevelCount')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.Services.STBService.1.Capabilities.VideoDecoder.X_RDKCENTRAL-COM_MPEGHPart2.ProfileLevelNumberOfEntries"
