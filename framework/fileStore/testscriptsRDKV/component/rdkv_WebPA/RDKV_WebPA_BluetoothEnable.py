...
<xml>
  ...
  <name>RDKV_WebPA_BluetoothEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the Bluetooth enable status using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_26</test_case_id>
    <test_objective>To validate the Bluetooth enable status using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM_xBlueTooth.Enabled will fetch the WebPA server url from the device.
...
2. API must return the Bluetooth is enabled or not</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_BluetoothEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_BluetoothEnable')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM_xBlueTooth.Enabled"
