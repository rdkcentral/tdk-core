...
<xml>
  ...
  <name>RDKV_WebPA_DbgServicesEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the debug services enabled the flag or not using the WebPA server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_14</test_case_id>
    <test_objective>To validate the debug services enabled the flag or not using the WebPA server</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Identity.DbgServices.Enable will fetch the WebPA server url from the device.
...
2.RFC parameter via WebPA server should return DbgService is enabled or not</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_DbgServicesEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_DbgServicesEnable')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Identity.DbgServices.Enable"
    testValue = "1"
