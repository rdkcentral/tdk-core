...
<xml>
  ...
  <name>RDKV_WebPA_Get_SerialNumber</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the device serial number using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_32</test_case_id>
    <test_objective>To get the device serial number using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.SerialNumber will fetch the WebPA server url from the device.
...
2. API must return the serial number of the device</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_Get_SerialNumber</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_Get_SerialNumber')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in detail:
        ...
        print("Serial Number : ", original_value)
        data_type = detail[2]
        if "SUCCESS" in detail:
            tdkTestObj.setResultStatus("SUCCESS")
