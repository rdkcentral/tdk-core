...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>XUPNP_GetTimeZone</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_GetUpnpResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get time zone value from xdiscovery process.
Testcase ID: CT_XUPNP_33</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_33</test_case_id>
    <test_objective>To get time zone value from xdiscovery process.</test_objective>
...
IARM_Bus_Term : None</api_or_interface_used>
    <input_parameters>string paramName=timezone</input_parameters>
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>XUPNP_GetTimeZone</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_GetTimeZone');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_GetTimeZone');
            #Get the result of connection with test component and STB
