...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>XUPNP_GetDeviceName</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_GetUpnpResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the Device Name value from xdiscovery process
Testcase ID: CT_XUPNP_62</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_62</test_case_id>
    <test_objective>To get the Device Name value from xdiscovery process
</test_objective>
...
IARM_Bus_Term : None</api_or_interface_used>
    <input_parameters>string paramName = deviceName</input_parameters>
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>XUPNP_GetDeviceName</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_GetDeviceName');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_GetDeviceName');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in xupnpLoadStatus.upper():
                ...
                tdkTestObj.addParameter("paramName","deviceName");
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                details = details.replace('\\','')
                details = details.replace('\"','')
                print("GetDeviceName Result : %s"%actualresult);
                print("GetDeviceName Details : %s"%details);
