...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>XUPNP_BcastTunerReadyEvt</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_BroadcastEvent</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To broadcast Tuner Ready event and check if xcal-device process receives it.
Testcase ID: CT_XUPNP_40</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_40</test_case_id>
    <test_objective>To broadcast Tuner Ready event and check if xcal-device process receives it.</test_objective>
...
6. The stub will invokes the RPC method for checking the tuner ready event is registered and send the results.
...
9. IARMBUS_Agent deregisters from the IARM Bus Daemon.</automation_approch>
    ...
    <test_script>XUPNP_BcastTunerReadyEvt</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_BcastTunerReadyEvt');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_BcastTunerReadyEvt');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in xupnpLoadStatus.upper():
                ...
                tdkTestObj.addParameter("eventLog","Tune Ready Update Received");
