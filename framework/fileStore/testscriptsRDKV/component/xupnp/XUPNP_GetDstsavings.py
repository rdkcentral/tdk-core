...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>XUPNP_GetDstsavings</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_GetUpnpResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the dst savings value from xdiscovery process.
Testcase ID: CT_XUPNP_56</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_56</test_case_id>
    <test_objective>To get the dst savings value from xdiscovery process.
</test_objective>
...
IARM_Bus_Term : None</api_or_interface_used>
    <input_parameters>string paramName = dstsavings</input_parameters>
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>XUPNP_GetDstsavings</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_GetDstsavings');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_GetDstsavings');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in xupnpLoadStatus.upper():
                ...
                tdkTestObj.addParameter("paramName","dstsavings");
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                details = details.replace('\\','')
                details = details.replace('\"','')
                print("GetDstsavings Result : %s"%actualresult);
                print("GetDstsavings Details : %s"%details);
