...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>XUPNP_GetBcastMacAddress</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_GetUpnpResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the broadcast mac address value from xdiscovery process.
Testcase ID: CT_XUPNP_54</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_54</test_case_id>
    <test_objective>To get the broadcast mac address value from xdiscovery process.
</test_objective>
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>XUPNP_GetBcastMacAddress</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_GetBcastMacAddress');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_GetBcastMacAddress');
            #Get the result of connection with test component and STB
