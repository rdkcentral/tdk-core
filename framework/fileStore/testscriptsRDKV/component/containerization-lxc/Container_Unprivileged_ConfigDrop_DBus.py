...
<xml>
  ...
  <name>Container_Unprivileged_ConfigDrop_DBus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check unprivileged containers configuration drop super-user capabilities for dbus</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Container_08</test_case_id>
    <test_objective>Check unprivileged containers configuration drop super-user capabilities for dbus</test_objective>
...
2. SystemUtilAgent will check the file content of "/containers/dbus/base-namespaces.conf" 
...
4. TM unloads the SystemUtilAgent</automation_approch>
    <except_output>Checkpoint 1.Check the file "/containers/dbus/base-namespaces.conf" contains an entry as "lxc.cap.keep = none"</except_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>Container_Unprivileged_ConfigDrop_DBus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Container_Unprivileged_ConfigDrop_DBus');
...
if "SUCCESS" in loadStatus.upper():
	...
	if result:
		...
		if status:
			...
			print "dbus capabilities not dropped";
	else:
