...
TestcaseID: CT_DS191</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_191</test_case_id>
    <test_objective>This tests the API used to get the frame rate of the given video output port.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>const FrameRate &amp; getFrameRate()</api_or_interface_used>
    <input_parameters>string port_name("HDMI0")</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the frame rate of video output port.
3.Device_Settings_Agent will check if frame rate of video output port is retrievd successfully.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if frame rate of video output port is retrievd successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_VideoOutputPort_getFrameRate_191</test_script>
    <skipped>No</skipped>
