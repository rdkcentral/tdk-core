...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>DS_VideoOutputPort_getPixelResolution_194</name>
  <primitive_test_id/>
  <primitive_test_name>DS_VOPCONFIG_getPixelResolution</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This tests API used to get the pixel resolution of the given video output port.
TestcaseID: CT_DS194</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_DS_194</test_case_id>
    <test_objective>This tests API used to get the pixel resolution of the given video output port.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>const PixelResolution &amp; getPixelResolution()</api_or_interface_used>
    <input_parameters>string port_name("HDMI0")</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the pixel resolution of video output port.
3.Device_Settings_Agent will check if pixel resolution of video output port is retrievd successfully.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if pixel resolution of video output port is retrievd successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_VideoOutputPort_getPixelResolution_194</test_script>
    <skipped>No</skipped>
...
dsObj.configureTestCase(ip,port,'DS_VideoOutputPort_getPixelResolution_194');
        ...
        if "SUCCESS" in result:
                ...
                if "TRUE" in result:
                        ...
                        if expectedresult in actualresult:
                        		...
                        		tdkTestObj = dsObj.createTestStep('DS_VOPCONFIG_getPixelResolution');
                        		...
                        		print "Expected Result: [%s] Actual Result: [%s]"%(expectedresult,actualresult)
                       	 		print "Port name: %s PixelResolution: %s"%(portName,details);
