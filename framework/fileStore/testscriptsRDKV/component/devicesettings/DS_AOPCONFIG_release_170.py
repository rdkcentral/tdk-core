...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>DS_AOPCONFIG_release_170</name>
  <primitive_test_id/>
  <primitive_test_name>DS_AOPCONFIG_release</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Objective:
Test Case ID: CT_DS_170
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_170</test_case_id>
    <test_objective>This API clears the instance of audio encoding, compression, stereo modes and audio port types.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>void release()</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will release instance of audio encoding, compression, stereo modes and audio port types.
3.Device_Settings_Agent will check if audio output ports and port types are not supported.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if audio output ports and port types are not supported</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_AOPCONFIG_release_170</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DS_AOPCONFIG_release_170');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        if expectedresult in actualresult:
                ...
                #calling Device Settings - Audio Load reset to default values.
