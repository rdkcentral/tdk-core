...
TestcaseID: CT_DS188</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_188</test_case_id>
    <test_objective>This tests the API used to get video output port instance using port Id.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>VideoOutputPort getPort(int id)</api_or_interface_used>
...
3.Device_Settings_Agent will check if video output port id retrieved using video output port instance is same as port id provided.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if port id retrieved using video output port instance is same as port id provided</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_VideoOutputPort_getPortFromId_188</test_script>
    <skipped>No</skipped>
