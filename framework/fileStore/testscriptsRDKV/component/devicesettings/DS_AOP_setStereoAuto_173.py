...
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_173</test_case_id>
    <test_objective>This API is used to set the stereo mode to be auto.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>void setStereoAuto(const bool autoMode)</api_or_interface_used>
    <input_parameters>int autoMode(0,1)</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the audio output port stereo auto mode.
3.Device_Settings_Agent will set the audio output port stereo auto mode.
4.Device_Settings_Agent will get the  audio output port stereo auto mode.
5.Device_Settings_Agent will check the current stereo auto mode with the auto mode set and expect it to match.
6.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check the stereo auto mode before and after setting it.</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_AOP_setStereoAuto_173</test_script>
    <skipped>No</skipped>
