...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>DS_HOST_setPreferredSleepMode_142</name>
  <primitive_test_id/>
  <primitive_test_name>DS_HOST_setPreferredSleepMode</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This test script is used to set the Preferred sleep Mode and implements the low power mode for RDK platform. 
TestcaseID: CT_DS142</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_DS_142</test_case_id>
    <test_objective>This test script is used to set the Preferred sleep Mode and implements the low power mode for RDK platform.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>int setPreferredSleepMode(const SleepMode mode)
SleepMode getPreferredSleepMode()</api_or_interface_used>
    <input_parameters>string sleepMode ("LIGHT_SLEEP","DEEP_SLEEP")</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the  list of supported preferred sleep modes.
3.Device_Settings_Agent will set the preferred sleep mode to supported values.
...
6.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check the preferred sleep mode before and after setting it.</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_HOST_setPreferredSleepMode_142</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'DS_HOST_setPreferredSleepMode_142');
...
if "SUCCESS" in loadmodulestatus.upper():
		...
		if "SUCCESS" in actualresult.upper():
			...
			for mode in modeList:
				print "Setting the Sleep mode : ", mode
