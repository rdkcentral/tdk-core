...
Test Type: Positive.</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_165</test_case_id>
    <test_objective>This API is used to get the type of the audio output port</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>AudioOutputPortType &amp;getPortType(int id)</api_or_interface_used>
    <input_parameters>int port_id (0)</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the audio output port type by port id.
3.Device_Settings_Agent will check if audio output port id retrieved using audio output port type is same as port id provided.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if audio output port id retrieved using audio output port type is same as port id provided</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_AOPCONFIG_getPortType_165</test_script>
    <skipped>No</skipped>
