...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_SampleAccurateScheduling</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the SampleAccurateScheduling details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_51</test_case_id>
    <test_objective>To get the SampleAccurateScheduling details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>sample-accurate-scheduling.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the SampleAccurateScheduling details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_SampleAccurateScheduling</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_SampleAccurateScheduling');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/sample-accurate-scheduling.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing sample-accurate-scheduling.html file")
            grep_line = "sample-accurate-scheduling | tail -5"
            log_filename = "sample-accurate-scheduling"
...
if "sample-accurate-scheduling" in status_dict:
    print(f"sample-accurate-scheduling: {status_dict['sample-accurate-scheduling']}")
