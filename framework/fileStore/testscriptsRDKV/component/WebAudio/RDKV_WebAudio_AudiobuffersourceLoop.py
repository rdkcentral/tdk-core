...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_AudiobuffersourceLoop</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Tests AudioBufferSource node looping behavior across different loop ranges, playback rates, offsets, and error conditions.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_77</test_case_id>
    <test_objective>To validate that looping produces correct repeated sequences, handles offsets and playback rates properly, and ensures invalid loop configurations or extreme playback rates fall back to safe defaults without errors.</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>audiobuffersource-loop-comprehensive.html, audiobuffersource-loop-grain-no-duration.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>All looping cases match expected output arrays, tails resolve to silence, illegal configurations revert to whole-buffer playback or silence, and loop count and delayed-start behave as specified. All assertions pass successfully.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_AudiobuffersourceLoop</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_AudiobuffersourceLoop');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/AudioBufferSource/audiobuffersource-loop-comprehensive.html'
webaudio_test_url2 = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/AudioBufferSource/audiobuffersource-loop-grain-no-duration.html'
