...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_AudioPlayback_AAC</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To test if the AAC audio playback is working fine in device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>To test if the AAC audio playback is working fine in device browser</test_objective>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    ...
    <test_script>RDKV_WebAudio_AudioPlayback_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_AudioPlayback_AAC');
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if status == "SUCCESS":
            navigate_keys = ["9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","13"]
            select_key = ["9","13"]
            keys = [navigate_keys,select_key]
            ...
            if log_check_method == "WebinspectPageLogs":
                ...
                webinspect_logs=WebAudiolib.webaudio_getLogs_webinspectpage(obj,app_name,keys)
                ...
                webinspect_logs = WebAudiolib.webaudio_getLogs_fromDevicelogs(obj,app_name,grep_line,keys)
            if webinspect_logs != "":
