...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_WebAudio_AdditiveSynthesisTest</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the browser plays different harmonics using Webaudio</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_06</test_case_id>
    <test_objective>To check if the browser plays different harmonics using Webaudio</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>AdditiveSynthesisTest.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to play the different harmonic sounds using WebAudio</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_AdditiveSynthesisTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_AdditiveSynthesisTest');
    ...
    if expectedResult in pre_req_status:
        ...
        if status == "SUCCESS":
            ...
            if log_check_method == "WebinspectPageLogs":
                ...
                webinspect_logs=WebAudiolib.webaudio_getLogs_webinspectpage(obj,app_name, [["9","13"],["9","13"]])
                webinspect_logs = ', '.join(webinspect_logs)
            else:
                print("\n Script is using wpeframework log to validate the webaudio test")
                grep_line="'AdditiveSynthesisTest' | tail -9 | tr -d '\\n'"
                webinspect_logs = WebAudiolib.webaudio_getLogs_fromDevicelogs(obj,app_name,grep_line,[["9","13"],["9","13"]])
            if webinspect_logs != "":
                if "TDK_LOGS" in webinspect_logs and "AdditiveSynthesisTest" in webinspect_logs:
                    print("\n ", webinspect_logs)
                    print("\nSUCCESS: Successfully fetched the logs from the Html test App")
                    tdkTestObj.setResultStatus("SUCCESS")

                    success_logs=["Synthesis started","Harmonic 1 started","Harmonic 2 started","Harmonic 3 started","Harmonic 4 started","Harmonic 5 started","Harmonic changed","Synthesis stopped"]
