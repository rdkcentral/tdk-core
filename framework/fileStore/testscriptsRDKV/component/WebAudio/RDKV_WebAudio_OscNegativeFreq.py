...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_OscNegativeFreq</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the osc negative freq details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_123</test_case_id>
    <test_objective>To get the osc negative freq details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>osc-negative-freq.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the osc negative freq details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_OscNegativeFreq</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_OscNegativeFreq');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/osc-negative-freq.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing osc-negative-freq.html file")
            grep_line = "osc-negative-freq | tail -3 | tr -d '\\n'"
            log_filename = "osc-negative-freq"
...
if "osc-negative-freq" in status_dict:
    print(f"osc-negative-freq: {status_dict['osc-negative-freq']}")
