...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_RequireTransientActivation</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the RequireTransientActivation details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_49</test_case_id>
    <test_objective>To get the RequireTransientActivation details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>require-transient-activation.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the RequireTransientActivation details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_RequireTransientActivation</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_RequireTransientActivation');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/require-transient-activation.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing require-transient-activation.html file")
            grep_line = "require-transient-activation | tail -10"
            log_filename = "require-transient-activation"
...
if "require-transient-activation" in status_dict:
    print(f"require-transient-activation: {status_dict['require-transient-activation']}")
