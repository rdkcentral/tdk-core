...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_WebAudio_WebAudioSupport</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the device browser supports WebAudio</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_01</test_case_id>
    <test_objective>To check if the device browser supports WebAudio</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>CheckWebAudioSupport.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The device must support webaudio apis</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_WebAudioSupport</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_CheckWebAudioSupportTest');
    ...
    if expectedResult in pre_req_status:
        ...
        if status == "SUCCESS":
            if log_check_method == "WebinspectPageLogs":
                ...
                webinspect_logs=WebAudiolib.webaudio_getLogs_webinspectpage(obj,app_name, [["9","9","13"],[]])
                if webinspect_logs != '':
                    webinspect_logs=webinspect_logs[0]
            else:
                print("\n Script is using dacapp log to validate the webaudio test")
                grep_line="CheckWebAudioSupport | tail -1"
                webinspect_logs = WebAudiolib.webaudio_getLogs_fromDevicelogs(obj,app_name,grep_line,[["9","9","13"],[]])

            if webinspect_logs != "":
                if "TDK_LOGS" in webinspect_logs and "CheckWebAudioSupport" in webinspect_logs:
                    print("\n ", webinspect_logs)
                    print("\nSUCCESS: Successfully fetched the logs from the Html test App")
                    tdkTestObj.setResultStatus("SUCCESS")

                    if "Your browser supports the Web Audio API" in webinspect_logs:
                        print("\nSUCCESS: The device browser supports WebAudio APIs\n")
                        tdkTestObj.setResultStatus("SUCCESS")
                    else:
                        print("\nFAILURE: The device browser does not support WebAudio APIs \n")
