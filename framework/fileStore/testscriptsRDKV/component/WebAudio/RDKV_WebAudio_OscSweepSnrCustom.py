...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_OscSweepSnrCustom</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the osc sweep snr custom details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_124</test_case_id>
    <test_objective>To get the osc sweep snr custom details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>osc-sweep-snr-custom.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the osc sweep snr custom details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_OscSweepSnrCustom</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_OscSweepSnrCustom');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/osc-sweep-snr-custom.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing osc-sweep-snr-custom.html file")
            grep_line = "osc-sweep-snr-custom | tail -3 | tr -d '\\n'"
            log_filename = "osc-sweep-snr-custom"
...
if "osc-sweep-snr-custom" in status_dict:
    print(f"osc-sweep-snr-custom: {status_dict['osc-sweep-snr-custom']}")
