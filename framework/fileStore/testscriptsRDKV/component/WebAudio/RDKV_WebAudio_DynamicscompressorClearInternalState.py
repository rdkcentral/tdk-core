...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_DynamicscompressorClearInternalState</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the dynamicscompressor clear internal state details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_103</test_case_id>
    <test_objective>To get the dynamicscompressor clear internal state details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>dynamicscompressor-clear-internal-state.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the dynamicscompressor clear internal state details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_DynamicscompressorClearInternalState</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_DynamicscompressorClearInternalState');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/dynamicscompressor-clear-internal-state.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing dynamicscompressor-clear-internal-state.html file")
            grep_line = "dynamicscompressor-clear-internal-state | tail -3 | tr -d '\\n'"
            log_filename = "dynamicscompressor-clear-internal-state"
...
if "dynamicscompressor-clear-internal-state" in status_dict:
    print(f"dynamicscompressor-clear-internal-state: {status_dict['dynamicscompressor-clear-internal-state']}")
