#!/bin/bash
##########################################################################
# PackageManager Comprehensive Validation Script - v2 (Fixed)
# Run directly on device to validate all PackageManager API methods
# Device: 192.168.29.123
# Date: February 10, 2026
#
# Usage:
#   ./validate_all_packagemanager_apis_v2.sh [true|false]
#
# Parameters:
...
#
# Examples:
#   ./validate_all_packagemanager_apis_v2.sh              # Run all tests
#   ./validate_all_packagemanager_apis_v2.sh true         # Explicit: run all tests
#   ./validate_all_packagemanager_apis_v2.sh false        # Skip negative tests
##########################################################################
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║     PackageManager API Comprehensive Test Suite (v2 - Fixed)   ║"
...
# TEST 0: RATE LIMIT - Set BEFORE downloading to slow down downloads
echo "Test 0/20: RATE_LIMIT API - Set bandwidth limit (PRIORITY)"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 0,
...
execute_test "downloadLimit (Set to 512 Kbps)" "$PKG_REQUEST" "Set rate limit to slow down downloads" "positive"
sleep 1

# TEST 1: Download Package (API #1) - Cobalt
...
COBALT_FILE_PATH="$DOWNLOAD_DIR/package$COBALT_DOWNLOAD_ID"
echo "Extracted Cobalt Download ID: $COBALT_DOWNLOAD_ID (File path will be: $COBALT_FILE_PATH)"
sleep 2
...
YOUTUBE_FILE_PATH="$DOWNLOAD_DIR/package$YOUTUBE_DOWNLOAD_ID"
echo "Extracted YouTube Download ID: $YOUTUBE_DOWNLOAD_ID (File path will be: $YOUTUBE_FILE_PATH)"
sleep 3

# TEST 3: Wait for downloads to complete (check progress)
echo "Test 3/20: GET_PROGRESS API - Monitor Cobalt download"
...
execute_test "getProgress (Cobalt)" "$PKG_REQUEST" "Get progress of download ID: $COBALT_DOWNLOAD_ID" "positive"
sleep 1
...
# TEST 5: List Packages (BEFORE install to get actual package IDs)
...
echo "DESC: List all installed packages to determine package IDs"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if echo "$RESPONSE" | grep -q "INSTALLED"; then
    # Extract first INSTALLED package ID
    INSTALLED_PACKAGE_ID=$(echo "$RESPONSE" | grep -o '"packageId":"[^"]*"' | grep -v "INSTALL_FAILURE" | head -n 1 | cut -d'"' -f4)
    INSTALLED_PACKAGE_VERSION=$(echo "$RESPONSE" | grep -A1 "\"packageId\":\"$INSTALLED_PACKAGE_ID\"" | grep '"version"' | cut -d'"' -f4)
    echo "✓ TEST PASSED - Found installed package: $INSTALLED_PACKAGE_ID (version: $INSTALLED_PACKAGE_VERSION)"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
...
# TEST 8: Cancel Download YouTube (to test cancel)
echo "Test 8/20: CANCEL API"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 8,
...
execute_test "cancel" "$PKG_REQUEST" "Cancel download ID: $YOUTUBE_DOWNLOAD_ID" "positive"
sleep 1

# TEST 9: Delete cancelled download
echo "Test 9/20: DELETE API"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 9,
...
execute_test "delete (YouTube)" "$PKG_REQUEST" "Delete cancelled download ID: $YOUTUBE_DOWNLOAD_ID" "positive"
sleep 1

# TEST 10: Wait for Cobalt download to complete
echo "Test 10/20: Waiting for Cobalt download to complete..."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# TEST 11: Install Package (using actual file path from download directory)
echo "Test 11/20: INSTALL API - Cobalt"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 11,
...
execute_test "install (Cobalt)" "$PKG_REQUEST" "Install Cobalt package from $COBALT_FILE_PATH" "positive"
sleep 2

# TEST 12: List Packages (AFTER install)
echo "Test 12/20: LIST_PACKAGES API - Verify installed package"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 12,
    "method": "org.rdk.PackageManagerRDKEMS.listPackages",
...
# Extract installed package ID (use the one with status INSTALLED, not INSTALL_FAILURE)
# Try to match packageId with cobalt and INSTALLED state
INSTALLED_COBALT=$(echo "$RESPONSE" | grep -o '"packageId":"[^"]*cobalt[^"]*"[^}]*"state":"INSTALLED"' | head -n 1 | grep -o '"packageId":"[^"]*"' | cut -d'"' -f4)
if [ -z "$INSTALLED_COBALT" ]; then
    # Fallback: use sed to extract any cobalt packageId with INSTALLED state
...
# TEST 13: Package State (use actual installed package ID)
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 13/20: PACKAGE_STATE API - Using package: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 13,
...
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 15/20: GET_CONFIG_FOR_PACKAGE API - Using: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 15,
    ...
    execute_test "getConfigForPackage" "$PKG_REQUEST" "Get configuration for: $INSTALLED_PACKAGE_ID" "positive"
    sleep 1
fi

# TEST 16: Lock Package
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 16/20: LOCK API - Package: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 16,
...
# TEST 17: Get Locked Info
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 17/20: GET_LOCKED_INFO API - Package: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 17,
...
# TEST 18: Unlock Package
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 18/20: UNLOCK API - Package: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 18,
    ...
    execute_test "unlock" "$PKG_REQUEST" "Unlock package: $INSTALLED_PACKAGE_ID" "positive"
    sleep 1
fi

# TEST 19: Uninstall Package
if [ -n "$INSTALLED_PACKAGE_ID" ]; then
    echo "Test 19/20: UNINSTALL API - Package: $INSTALLED_PACKAGE_ID"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 19,
...
# TEST 20: DELETE downloaded files
if [ -n "$COBALT_DOWNLOAD_ID" ]; then
    echo "Test 20/20: DELETE API - Clean up Cobalt download"
    PKG_REQUEST='{
        "jsonrpc": "2.0",
        "id": 20,
        "method": "org.rdk.PackageManagerRDKEMS.delete",
    ...
    execute_test "delete (Cobalt)" "$PKG_REQUEST" "Delete Cobalt download ID: $COBALT_DOWNLOAD_ID" "positive"
    sleep 1
fi
...
# NEGATIVE TEST 2-5: Invalid Download IDs
echo "Negative Test 2: GET_PROGRESS API - Invalid Download ID"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 102,
    "method": "org.rdk.PackageManagerRDKEMS.getProgress",
...
# NEGATIVE TEST 5
echo "Negative Test 5: CANCEL API - Invalid Download ID"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 105,
    "method": "org.rdk.PackageManagerRDKEMS.cancel",
...
# NEGATIVE TEST 6: Install with Invalid Package
echo "Negative Test 6: INSTALL API - Invalid Package"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 106,
    "method": "org.rdk.PackageManagerRDKEMS.install",
    "params": {
        ...
        "version": "0.0.0",
        "fileLocator": "/invalid/path/package.bolt"
    }
}'
execute_test "install (Invalid Package)" "$PKG_REQUEST" "Install with invalid package ID and path" "negative"
sleep 1

# NEGATIVE TEST 7: Uninstall with Invalid Package
echo "Negative Test 7: UNINSTALL API - Invalid Package"
PKG_REQUEST='{
    "jsonrpc": "2.0",
    "id": 107,
    "method": "org.rdk.PackageManagerRDKEMS.uninstall",
...
echo "  • Cobalt Download ID: $COBALT_DOWNLOAD_ID (File: $COBALT_FILE_PATH)"
echo "  • YouTube Download ID: $YOUTUBE_DOWNLOAD_ID (File: $YOUTUBE_FILE_PATH)"
