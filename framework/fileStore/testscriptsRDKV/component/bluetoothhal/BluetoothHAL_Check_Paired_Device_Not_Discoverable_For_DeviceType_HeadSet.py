...
<xml>
  ...
  <name>BluetoothHAL_Check_Paired_Device_Not_Discoverable_For_DeviceType_HeadSet</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To confirm that the paired device will not list in discovered devices list again until it is unpaired when device type is HeadSet</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_36</test_case_id>
    <test_objective>To confirm that the paired device will not list in discovered devices list again until it is unpaired when device type is HeadSet</test_objective>
...
Checkpoint 2. Verify that the bluetooth client device  is not present in the discovered devices list afetr it is paired with the DUT.</expected_output>
    ...
    <test_script>BluetoothHAL_Check_Paired_Device_Not_Discoverable_For_DeviceType_HeadSet</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Check_Paired_Device_Not_Discoverable_For_DeviceType_HeadSet');
