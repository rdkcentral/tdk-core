...
<xml>
  ...
  <name>BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_MobileAudioIn</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether discoverable devices are listed in the discovered devices list when the device type is MobileAudioIn</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_22</test_case_id>
    <test_objective>To check whether discoverable devices are listed in the discovered devices list when the device type is MobileAudioIn</test_objective>
...
2. power_status - 1 (ON)
3. device_type - 2, for enBTRCoreMobileAudioIn
...
Checkpoint 2. Verify that the bluetooth client device  is present in the discovered devices list</expected_output>
    ...
    <test_script>BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_MobileAudioIn</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_MobileAudioIn');
        ...
        if (adapterPath):
            ...
            if (actualresult == expectedresult):
                ...
                if "FAILURE" not in str(bluetoothctlResult):
                    ...
                    #Set the device type as 2 - MobileAudioIn
                    tdkTestObj.addParameter("device_type", 2)
                                ...
                                if result and "NO_DEVICES_FOUND" != result and "deviceName" in result:
                                    ...
                                    for device in scannedDevices:
                                        if (device["deviceName"] == bluetoothhallib.deviceName):
                                            print("Client device of type MobileAudioIn is successfully discovered in DUT")
