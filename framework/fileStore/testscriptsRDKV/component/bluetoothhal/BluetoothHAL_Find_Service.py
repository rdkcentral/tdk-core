...
<xml>
  ...
  <name>BluetoothHAL_Find_Service</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>BluetoothHal_FindService</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To confirm if a service exists in a paired device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
9. enBTRCoreRet BTRCore_FindService (tBTRCoreHandle hBTRCore, tBTRCoreDevId aBTRCoreDevId, const char* UUID, char* XMLdata, int* found);
...
5. device_id - retrieved using BTRCore_GetListOfScannedDevices()
6. UUID - UUID of the service, passing "0x110b" corresponding to Audio Sink service
7. XMLData - Service name string, currently passing NULL value/empty string</input_parameters>
...
10.  Confirm if the Service exist in client device using BTRCore_FindService() api
...
Checkpoint 2. Verify that the Service exist in the bluetooth client device using BTRCore_FindService() api</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Find_Service</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Find_Service');
                                    ...
                                    if True == deviceDiscovered:
                                        ...
                                        if (actualresult == expectedresult):
                                            ...
                                            if True == devicePaired:
                                                ...
                                                tdkTestObj = bluetoothhalObj.createTestStep('BluetoothHal_FindService');
                                                #Set device ID as the bluetooth client device ID
                                                tdkTestObj.addParameter("device_id", deviceID)
                                                #Pass the UUID corresponding to Audio Sink service
                                                tdkTestObj.addParameter("uuid", "0x110b")
