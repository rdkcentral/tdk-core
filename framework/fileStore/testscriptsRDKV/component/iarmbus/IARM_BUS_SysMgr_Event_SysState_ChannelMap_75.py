...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1342</id>
  <version>4</version>
  <name>IARM_BUS_SysMgr_Event_SysState_ChannelMap_75</name>
  ...
  <status>FREE</status>
  <synopsis>IARMBUS – Broadcasting and Receiving “IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE” event and setting data for IARM_BUS_SYSMGR_SYSSTATE_CHANNELMAP and getting back using IARM_BUS_SYSMGR_API_GetSystemStates RPC call.
Test case Id - CT_IARMBUS_75</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_75</test_case_id>
    <test_objective>IARMBUS – Broadcasting and Receiving “IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE” event and setting data for IARM_BUS_SYSMGR_SYSSTATE_CHANNELMAP and getting back using IARM_BUS_SYSMGR_API_GetSystemStates RPC call.</test_objective>
...
Void * - eventdata(IARM_BUS_SYSMGR_SYSSTATE_CHANNELMAP), size_t – sizeof (eventdata)
...
6.TestMgr_IARMBUS_BusCall</test_stub_interface>
    <test_script>IARM_BUS_SysMgr_Event_SysState_ChannelMap_75</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CT_IARMBUS_75');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if ("SUCCESS" in actualresult):
        ...
        if expectedresult in actualresult:
            ...
            #passing parameter for receiving SYSMgr event sys state channel map
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    if str(setstate) in details and str(seterror) in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("SUCCESS:RPC method invoked and channel map value for state and error properly set");
                    else :
                        tdkTestObj.setResultStatus("FAILURE");
                        print("FAILURE:RPC method invoked and channel map value for state and error has not been set");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
