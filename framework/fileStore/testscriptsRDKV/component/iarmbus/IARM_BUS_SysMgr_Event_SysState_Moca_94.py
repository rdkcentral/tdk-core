...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1397</id>
  <version>1</version>
  <name>IARM_BUS_SysMgr_Event_SysState_Moca_94</name>
  ...
  <status>FREE</status>
  <synopsis>IARMBUS – Broadcasting and Receiving “IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE” event and setting data for IARM_BUS_SYSMGR_SYSSTATE_MOCA and getting back using IARM_BUS_SYSMGR_API_GetSystemStates RPC call.
Test case Id - CT_IARMBUS_94</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_94</test_case_id>
    <test_objective>IARMBUS – Broadcasting and Receiving “IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE” event and setting data for IARM_BUS_SYSMGR_SYSSTATE_MOCA and getting back using IARM_BUS_SYSMGR_API_GetSystemStates RPC call.</test_objective>
...
Void * - eventdata(IARM_BUS_SYSMGR_SYSSTATE_MOCA), size_t – sizeof (eventdata)
...
6.TestMgr_IARMBUS_BusCall</test_stub_interface>
    <test_script>IARM_BUS_SysMgr_Event_SysState_Moca_94</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CT_IARMBUS_94');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if ("SUCCESS" in actualresult):
        ...
        if expectedresult in actualresult:
            ...
            #passing parameter for receiving SYSMgr event sys state MOCA
            ...
            if expectedresult in actualresult:
                ...
                tdkTestObj.addParameter("owner_name","SYSMgr");
                tdkTestObj.addParameter("event_id",0);
                tdkTestObj.addParameter("newState",19);
                setstate=11;
                seterror=22;
                setpayload="MOCA";
                ...
                if expectedresult in actualresult:
                    if str(setstate) in details and  str(seterror) in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("SUCCESS: RPC method invoked and MOCA value for state and error properly set");
                    else :
                        tdkTestObj.setResultStatus("FAILURE");
                        print("FAILURE: RPC method invoked and MOCA value for state and error has not been set");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
