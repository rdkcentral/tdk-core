...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_RVS_LightningApp_LoadURL_OnStressLaunch</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateResourceUsage</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to launch and exit LightningApp for 99 times. On 100th time load LightningApp and verify if the load URL is happening properly.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_53</test_case_id>
    <test_objective>The objective of this test is to launch and exit LightningApp for 99 times. On 100th time load LightningApp and verify if the load URL is happening properly.</test_objective>
...
1. Launch LightningApp using RDKShell.
2. Check the status
...
10. Destroy the plugin</automation_approch>
    ...
    <test_script>RDKV_CERT_RVS_LightningApp_LoadURL_OnStressLaunch</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_LightningApp_LoadURL_OnStressLaunch')
...
if expectedResult in (result.upper() and pre_condition_status):
    ...
    htmlapp_post_condition = {"LightningApp":"deactivated"}
    ...
    if status == "SUCCESS" and lightningapp_test_url != "":
        ...
        for count in range(0,max_iterations):
            ...
            if expectedResult in result and details == "SUCCESS" :
                ...
                print("\n Error while validating load URL in LightningApp")
                tdkTestObj.setResultStatus("FAILURE")
        ...
        if status_dict and status_dict[plugin] in 'deactivated':
            print("\n Successfully deactivated LightningApp")
            tdkTestObj.setResultStatus("SUCCESS")
        else:
            print("\n Unable to deactivate LightningApp, current status: ",status_dict[plugin])
            tdkTestObj.setResultStatus("FAILURE")
    else:
