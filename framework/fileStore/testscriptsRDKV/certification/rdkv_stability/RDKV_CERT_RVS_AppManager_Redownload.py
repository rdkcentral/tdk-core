...
if expectedResult in result.upper():
    ...
    if status == "SUCCESS":
        ...
        config_status, offset = getDeviceConfigKeyValue(conf_file,"MEMORY_OFFSET")
        package_size = (StabilityTestVariables.Redownload_Package_Size_MB)*1024
        ...
        if ssh_params != "" or ssh_params != "{}":
            ...
            if not ssh_method or not credentials:
                ...
                app_bundle_name = StabilityTestVariables.Small_Validation_File
                    ...
                    if status == "SUCCESS":
                        ...
                        if "onAppDownloadStatus" in str(event):
                            ...
                            if download_id not in download_id_List:
                                ...
                                output = rdkv_performancelib.rdkservice_getRequiredLog(ssh_method, credentials, cmd)
                                output  = int(output.strip().splitlines()[-1])
                                                        ...
                                                        if download_id not in download_id_List:
                                                            ...
                                                            if output == package_size:
                                                                if ( int(event_download_id) == download_id and event_download_id in filelocator_url):
                                                                    ...
                                                                    output = rdkv_performancelib.rdkservice_getRequiredLog(ssh_method, credentials, cmd)
                                                                    output = output.strip().splitlines()[-1]
                                                                    current_used_memory = int(float(output))
                                                                    print("Current Disk usage :\n", current_used_memory)
                                                                    if current_used_memory < (initial_used_memory + package_size + int(offset)):
                                                                    ...
                                                                    if status == "SUCCESS":
                                                                        ...
                                                                        if ("No such file or directory" in output) or ("cannot access" in output.lower()):
                                                                            ...
                                                                            print(f"Iteration {iteration+1}: failed: re-downloaded package still exists on DUT. ls output:\n", output)
                                                                            tdkTestObj.setResultStatus("FAILURE")
                                                                            break    
                                                                    else:
                                                                        print(f"Iteration {iteration+1}: Failed to delete the re-downloaded package from the device")
                                                                        tdkTestObj.setResultStatus("FAILURE")
                                                                        break          
                                                                else:
                                                                    print(f"Iteration {iteration+1}: Download ID from the event does not match with the download ID from the download API")
                                                                    tdkTestObj.setResultStatus("FAILURE")    
                                                                    break  
                                                            else:
                                                                print(f"Iteration {iteration+1}: Size of the downloaded package failed to match with the configured size")    
                                                                tdkTestObj.setResultStatus("FAILURE") 
                                                                break          
                                                        else:
