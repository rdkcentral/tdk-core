...
<xml>
  ...
  <name>RDKV_CERT_SVS_PortScan_TCP_MAIMON</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Scanning the TCP ports of the device with MAIMON scan technique to find out if additional ports are open other than the  expected list of open ports</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_SECURITY_17</test_case_id>
    <test_objective>Scanning the TCP ports of the device  with MAIMON technique to find out if additional ports are open other than the expected list of open ports</test_objective>
...
2. Open ports list corresponding to the device should be added in the device configuration file (variable $TCP_PORTS) available in fileStore/tdkvRDKServiceConfig/device.config file</pre_requisite>
    ...
    <automation_approch>1. Execute the nmap with MAIMON scan type to scan all TCP ports of the DUT
...
4. Pass/fail the test based on the absence/presence of additional open ports</automation_approch>
    ...
    <test_script>RDKV_CERT_SVS_PortScan_TCP_MAIMON</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_SVS_PortScan_TCP_MAIMON');
...
if expectedResult in result.upper():
    ...
    command = "nmap -sM " + obj.IP
    print("COMMAND : %s" %(command))
    tdkTestObj.addParameter("command", command);

    #Execute the test case in DUT
    print("Scanning the TCP open ports using Maimon scan type")
