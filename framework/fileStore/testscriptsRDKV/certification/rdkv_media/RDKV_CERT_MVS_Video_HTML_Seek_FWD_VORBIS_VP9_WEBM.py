...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Seek_FWD_VORBIS_VP9_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video seek forward operation of webm content with Vorbis and Vp9 codecs for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_701</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video seek forward operation of webm content with Vorbis and Vp9 codecs for few minutes and close the player</test_objective>
...
3. Launch webkit instance with html player test app with the video src url and the seek forward operations to be performed.
...
7. Revert all values</automation_approch>
    <expected_output>Player should perform the seek forward operation correctly, the expected events should occur, playback should resume from the seek forward position, and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_HTML_Seek_FWD_VORBIS_VP9_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Seek_FWD_VORBIS_VP9_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_vorbis_vp9_webm_url>&operations=seekfwd(30),close(30)
