...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek backward operation of hls h264 video codec content continuously for given number of times in provided interval</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_753</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek backward operation of hls h264 video codec content continuously for given number of times in provided interval     </test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<hls_h264_url>.m3u8&operations=seekbwd(180),repeat(1),seekbwd(10),repeat(15)&options=seekInterval(20)&autotest=true
