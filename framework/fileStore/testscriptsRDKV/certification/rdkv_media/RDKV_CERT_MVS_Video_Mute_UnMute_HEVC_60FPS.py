...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Mute_UnMute_HEVC_60FPS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance to play hevc 60 fps video content and perform mute and unmute operations</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_487</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance to play hevc 60 fps video content and perform mute and unmute operations</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Mute_UnMute_HEVC_60FPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Mute_UnMute_HEVC_60FPS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_60fps>&operations=mute(30),unmute(30)&autotest=true&type=mp4&options=collectfps(WebKitBrowser),deviceport(9998),expectedfps(57)
