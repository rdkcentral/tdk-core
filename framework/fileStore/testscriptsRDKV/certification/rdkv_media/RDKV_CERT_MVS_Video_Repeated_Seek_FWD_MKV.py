...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_Seek_FWD_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video seek forward operation of mkv content repeatedly for given number of times in provided interval</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video seek forward operation of mkv content repeatedly for given number of times in provided interval</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Repeated_Seek_FWD_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_Seek_FWD_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_mkv>.mkv&operations=seekfwd(10),repeat(15)&autotest=true&type=mkv
