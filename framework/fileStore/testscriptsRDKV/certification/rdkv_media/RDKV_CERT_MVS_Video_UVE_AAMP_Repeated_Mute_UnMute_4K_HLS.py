...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_Mute_UnMute_4K_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play video stream with 4k hls stream and perform mute, unmute operations repeatedly for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_466</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play a video stream with 4k hls stream and perform mute, unmute operations repeatedly for given number of times</test_objective>
...
4. App performs the mute and unmute operations of 4k hls stream repeatedly and validates using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should mute and unmute the video stream with 4k hls stream, expected volume level  should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_Mute_UnMute_4K_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_Mute_UnMute_4K_HLS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_hls>&operations=mute(5),unmute(5),repeat(15)&autotest=true
