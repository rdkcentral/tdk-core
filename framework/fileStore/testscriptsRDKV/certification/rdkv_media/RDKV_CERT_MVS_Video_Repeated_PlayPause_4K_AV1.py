...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_PlayPause_4K_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of 4K AV1 codec stream continuously for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_515</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of 4k AV1 codec stream continuously for given number of times           </test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Player pause and play should happen for 4k AV1 codec stream and expected events should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Repeated_PlayPause_4K_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_PlayPause_4K_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_4k_av1_url>.mp4&operations=pause(5),play(5),repeat(15)&autotest=true&type=mp4
