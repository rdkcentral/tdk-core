...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Complete_Video_Audio_Segment_HLS</name>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_612</test_case_id>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning UVE AAMP player App URL: string   webkit_instance:string webinspect_port: string  video_audio_src_url_complete_hls: string</input_parameters>
...
10. Streaming server should be stopped.</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Complete_Video_Audio_Segment_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Complete_Video_Audio_Segment_HLS')
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        if "SUCCESS" in launch_status:
            ...
            if "FAILURE" in test_result:
                    ...
                    tdkTestObj.setResultStatus("SUCCESS")   
            #if hang_detected == 1 and "FAILURE" in test_result:
                   #print("Hang detected during playback and test result is FAILURE  expected behavior.")
                   #print("[TEST EXECUTION RESULT]: SUCCESS")
                   #tdkTestObj.setResultStatus("SUCCESS")
            ...
            #elif  hang_detected == 1:
                        #print("Hang detected during playback")
                        #print("[TEST EXECUTION RESULT]: FAILURE")
                        #tdkTestObj.setResultStatus("FAILURE")
            else:
