...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Repeated_PlayPause_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operations of hls H.264 codec stream continuously for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_719</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operations of hls H.264 codec stream continuously for given number of times</test_objective>
...
3. Launch webkit instance with shaka test app url with the operations play, pause and repeat info.
4. App performs the pause and play operation repeatedly and validates using events
5. If expected events paused, play occurs for pause and play in all the repetition, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player should play, pause hls h264 video codec stream and expected events play, paused should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Repeated_PlayPause_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Repeated_PlayPause_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hls_h264_url>.m3u8&operations=pause(5),play(5),repeat(15)&autotest=true
