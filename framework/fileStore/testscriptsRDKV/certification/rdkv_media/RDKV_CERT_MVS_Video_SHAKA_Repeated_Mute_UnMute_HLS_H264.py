...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance to play hls h264 video content and perform mute, unmute operations repeatedly for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_720</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance to play hls h264 video content and perform mute, unmute operations repeatedly for given number of times</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player audio mute and unmute should happen, expected event volume change should occur for all the repetition for hls h264 content and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hls_h264_url>.m3u8&operations=mute(10),unmute(10),repeat(10)&autotest=true
