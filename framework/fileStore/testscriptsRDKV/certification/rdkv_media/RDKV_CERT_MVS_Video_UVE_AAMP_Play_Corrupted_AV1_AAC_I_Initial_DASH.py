...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_Initial_DASH</name>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_762</test_case_id>
...
webinspect_port: string  video_src_url_corrupted_av1_aac_I_initial_dash: string</input_parameters>
...
8. Revert all values</automation_approch>
    <expected_output>The player should play the entire segment of the video, including initial corrupted I-frames of AV1_AAC from 0 to 12 seconds without altering its state. Users should be able to see the distortion caused by the corrupted frames while the video plays, without the player crashing or changing from the "playing" state in the AAMP player</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_Initial_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_Initial_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupted_av1_aac_I_initial_dash
