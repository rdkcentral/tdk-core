...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Text_Track_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of multi text tracks stream, switch to another text track and after given duration close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_584</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of multi text tracks stream, switch to another text track and after given duration close the player</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,RPI,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning UVE AAMP Player app should be hosted
3. Need a stream with multiple text tracks</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Lightning player App URL: string
webkit_instance:string
webinspect_port: string
video_src_dash_url_multi_text_tracks: string</input_parameters>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the multi text tracks stream and switch to another text  track except the current text track after 30 seconds
4. App performs the play operations of multi text tracks stream and changes text track, validates using the text track index values
5. If expected text track index and observed text track index are same, then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the multi text tracks stream and changes text track and play for provided duration, expected text track index should be obtained and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Text_Track_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Change_Text_Track_DASH')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_dash_url_multi_text_tracks
        # Setting VideoPlayer Operations
        # Play the multi text tracks stream, after 30 sec switch to another text track and close after 30 sec
        setOperation("changetext","30")
        setOperation("close","30")
        operations = getOperations()
        # Setting VideoPlayer test app URL arguments
        setURLArgument("url",videoURL)
        setURLArgument("operations",operations)
        setURLArgument("options","checkInterval(10),enableCC(true)")
        ...
        #url=<video_url>.mpd&operations=changetext(30),close(30),options=checkInterval(10)&autotest=true
        ...
        if "SUCCESS" in launch_status:
            ...
            test_result,proc_check_list = monitorVideoTest(obj,webkit_console_socket,validation_dict,"Observed Text Track Index");
