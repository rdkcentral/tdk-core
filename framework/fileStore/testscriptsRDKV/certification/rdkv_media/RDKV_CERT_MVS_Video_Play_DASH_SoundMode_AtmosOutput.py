...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_DASH_SoundMode_AtmosOutput</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with atmos audio output mode for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_106</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with atmos audio output mode for few minutes and close the player   </test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning Player app should be hosted
3.TV should be connected with the test device If the audio atmos  is not supported then the test will be marked as failure</pre_requisite>
...
2. Enable audio atmos output sound mode
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_DASH_SoundMode_AtmosOutput</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_DASH_SoundMode_AtmosOutput')
...
if expectedResult in result.upper():
    ...
    mode_pre_requisite_status = setAudioAtmosOutputModePreRequisites(obj,mode)
    if mode_pre_requisite_status:
        print("Atmos Audio setting are done successfully\n")
