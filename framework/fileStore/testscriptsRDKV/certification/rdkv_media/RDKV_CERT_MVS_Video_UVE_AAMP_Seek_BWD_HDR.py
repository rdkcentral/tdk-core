...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HDR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek backward operation of hdr content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_656</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek backward operation of hdr content </test_objective>
...
3. Launch webkit instance with uve test app with the video src url, operations to be performed, seek backward with interval and duration for close.
4. App performs  seek backward of hdr content and validates using seeking, seeked events and with position value.
5. If expected events video seeking, seeked  observed and position is within the expected range then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should seek backward of hdr content and expected events seeking, seeked  should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HDR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HDR')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_hdr>.mp4&operations=seekbwd(60)&autotest=true
