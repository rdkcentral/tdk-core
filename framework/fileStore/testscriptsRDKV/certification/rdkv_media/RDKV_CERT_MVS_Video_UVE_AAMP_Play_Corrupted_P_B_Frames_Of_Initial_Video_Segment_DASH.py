...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_P_B_Frames_Of_Initial_Video_Segment_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted P and B frames from the initial video segment of a clear stream,served via streaming server.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_438</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted P and B frames from the initial video segment of a clear stream,served via streaming server.</test_objective>
...
webinspect_port: string  video_src_url_corrupt_P_B_initial_dash: string</input_parameters>
...
8. Revert all values
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_P_B_Frames_Of_Initial_Video_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_P_B_Frames_Of_Initial_Video_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupt_P_B_initial_dash
    if pre_requisite_status == "SUCCESS":
            print("The content is corrupted with P and B frame at initial segment of the video")
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the P and B frames during the initial segment of the video from 0 to 12 seconds *************\n")
