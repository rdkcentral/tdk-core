...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_HLS_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of PlayReady DRM protected HEVC codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_92</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of PlayReady DRM protected HEVC codec hls stream for few minutes and close the player            </test_objective>
...
video_src_url_playready_hls_hevc: string
video_src_url_playready_hls_hevc_drmconfigs:string
...
4. App starts playing the PlayReady DRM protected hevc hls stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should play the PlayReady hevc hls video for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_HLS_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_HLS_HEVC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_playready_hls_hevc
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_playready_hls_hevc_drmconfigs)
        ...
        #url=<video_hevc_url>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true
