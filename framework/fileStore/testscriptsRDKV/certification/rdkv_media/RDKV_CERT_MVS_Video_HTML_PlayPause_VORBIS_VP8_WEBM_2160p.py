...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_PlayPause_VORBIS_VP8_WEBM_2160p</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation on webm content with Vorbis and Vp8 codecs with 2160p resolution for few minutes and then close the player.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_907</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation on webm content with Vorbis and Vp8 codecs with 2160p resolution for few minutes and then close the player.</test_objective>
...
video_src_url_vorbis_vp8_2160p: string
play_interval: int
pause_interval:int
</input_parameters>
...
3. Launch webkit instance with html player test app with the video src url, and perform play and pause operations at 2160p resolution at the given interval.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_PlayPause_VORBIS_VP8_WEBM_2160p</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_PlayPause_VORBIS_VP8_WEBM_2160p')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_vorbis_vp8_2160p
        ...
        #url=<video_vorbis_vp8_webm_url>&operations=pause(30),play(10)
