...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_OPUS_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video seek position operation of webm content with av1 and Opus codecs for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_914</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video seek position operation of webm content with av1 and Opus codecs for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_OPUS_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_OPUS_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_av1_opus
        ...
        #url=<video_av1_opus_webm_url>&operations=seekfwd(30),seekbwd(30),close(30)
