...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_P_End_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a WebKit instance. The objective is to assess the media player's response when it receives corrupted content from the end video segment of a clear AV1 with AAC codec stream, served via Streaming server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_782</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a WebKit instance. The objective is to assess the media player's response when it receives corrupted content from the end video segment of a clear AV1 with AAC codec stream, served via Streaming server</test_objective>
...
webinspect_port: string  video_src_url_corrupted_av1_aac_I_P_end_dash: string</input_parameters>
...
8. Revert all values</automation_approch>
    <expected_output>The player should play the entire segment of the video, including end corrupted I and P -frames of AV1_AAC from 78 to 90 seconds without altering its state. Users should be able to see the distortion caused by the corrupted frames while the video plays, without the player crashing or changing from the "playing" state in the AAMP player</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_P_End_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_P_End_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupted_av1_aac_I_P_end_dash
    if pre_requisite_status == "SUCCESS":
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the I and P frames during the end segment of the video from 78 to 90 seconds *************\n")
