...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Seek_FF_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video medium trick play operations like seek and FF of mkv content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_449</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video medium trick play operations like seek and FF of mkv content</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Seek_FF_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Seek_FF_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_mkv>.mkv&operations=seekfwd(30),fastfwd(10),playnow(5)&autotest=true&options=checkInterval(5),loop&type=mkv
