...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_FF3X_Play_VORBIS_VP9_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with Vorbis and Vp9 codecs with playback rate 3x for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_841</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with Vorbis and Vp9 codecs with playback rate 3x for few minutes and close the player</test_objective>
...
3. Launch webkit instance with html player test app with the video src url and the fast forward operations with playback rate 3x to be performed.
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the video and perform the fast forward operation with playback rate 3x correctly, the expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_HTML_FF3X_Play_VORBIS_VP9_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_FF3X_Play_VORBIS_VP9_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_vorbis_vp9_webm_url>&operations=fastfwd3x(30),playnow(10),close(30)
