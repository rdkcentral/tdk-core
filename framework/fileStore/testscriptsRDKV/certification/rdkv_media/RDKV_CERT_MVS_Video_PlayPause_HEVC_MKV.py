...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_PlayPause_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of HEVC MKV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_586</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of HEVC MKV stream</test_objective>
...
4. App performs the play pause operations of hevc mkv stream and validates each operation using events
5. If expected events playing, paused occurs for play and pause operation, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Player should pause and play hevc mkv stream, expected events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_PlayPause_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_PlayPause_HEVC_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_mkv>.mkv&operations=pause(30),play(10)&autotest=true&type=mkv
