...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Seek_BWD_AV1_VORBIS_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video seek backward operation of webm content with av1 and Vorbis codecs for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_683</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video seek backward operation of webm content with av1 and Vorbis codecs for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_Seek_BWD_AV1_VORBIS_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Seek_BWD_AV1_VORBIS_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_av1_vorbis_webm_url>&operations=seekbwd(60),close(30)
