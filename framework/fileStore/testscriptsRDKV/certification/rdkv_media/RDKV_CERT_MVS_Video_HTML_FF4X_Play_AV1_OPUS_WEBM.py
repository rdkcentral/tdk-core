...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_FF4X_Play_AV1_OPUS_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with av1 and Opus codecs with playback rate 4x for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_846</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with av1 and Opus codecs with playback rate 4x for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_FF4X_Play_AV1_OPUS_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_FF4X_Play_AV1_OPUS_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_av1_opus_webm_url>&operations=fastfwd4x(30),playnow(10),close(30)
