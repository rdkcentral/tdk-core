...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_HEVC_50FPS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of 50 fps hevc content for few minutes, check average fps and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_298</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of 50 fps hevc content for few minutes, check average fps and close the player</test_objective>
...
devicePort:int
video_src_url_hevc_50fps: string
close_interval: int
</input_parameters>
...
4. App starts playing the 50fps hevc video, collects the fps for every 1 second and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the 50 fps hevc video for provided duration, expected event playing should occur, average fps should be around 50 and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_HEVC_50FPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_HEVC_50FPS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS" and config_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_hevc_50fps
        ...
        #url=<video_hevc_url>&operations=close(30)&autotest=true&type=mp4&options=collectfps(WebKitBrowser),deviceport(9998),expectedfps(47)
