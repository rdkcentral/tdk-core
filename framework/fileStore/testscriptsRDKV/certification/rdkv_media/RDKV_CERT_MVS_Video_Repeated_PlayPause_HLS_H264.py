...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_PlayPause_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of hls h264 video codec content continuously for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of hls h264 video codec content continuously for given number of times</test_objective>
...
4. App performs the pause and play operation of hls h264 video codec stream repeatedly and validates using events
...
7. Revert all values</automation_approch>
    <expected_output>Player should play, pause hls h264 video codec stream and expected events play, paused should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Repeated_PlayPause_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_PlayPause_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.m3u8&operations=pause(5),play(5),repeat(15)&autotest=true&type=hls
