...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_DASH_SoundMode_DolbyDigital</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with Dolby Digital (DD) sound mode for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_105</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with Dolby Digital (DD) sound mode for few minutes and close the player</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning Player app should be hosted
3.TV should be connected with the test device. If the dolby digital (DD) sound mode is not supported then the test will be marked as failure</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Lightning player App URL: string
webkit_instance:string
webinspect_port: string
video_src_url_dolby: string
dolby_url_type:string
close_interval: int
</input_parameters>
    <automation_approch>1. As pre requisite, launch webkit instance via RDKShell, open websocket conntion to webinspect page
2. Get the current sound mode and set audio sound mode as dolby digital (DD)
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_DASH_SoundMode_DolbyDigital</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_DASH_SoundMode_DolbyDigital')
...
if expectedResult in result.upper():
    ...
    # Setting the pre-requisites for media test. Launching the webkit inspect via RDKShell and
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_dolby
        ...
        setURLArgument("type",MediaValidationVariables.dolby_url_type)
        ...
        #url=<video_dolby_url>.mpd&operations=close(60)&autotest=true&type=dash
        ...
        # Reverting the sound mode
        post_requisite_status = setMediaTestPostRequisites(MediaValidationVariables.unified_player_app_id)
        mode_post_requisite_status = setSoundModePostRequisites(obj)
        if not mode_post_requisite_status:
