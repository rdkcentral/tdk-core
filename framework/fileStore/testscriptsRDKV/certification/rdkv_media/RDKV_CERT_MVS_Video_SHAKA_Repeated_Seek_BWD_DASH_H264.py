...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek backward operation of dash h264 video codec content continuously for given number of times in provided interval</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_747</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek backward operation of dash h264 video codec content continuously for given number of times in provided interval       </test_objective>
...
3. Launch webkit instance with shaka test app with the src url, operations to be performed, seekbwd with given interval and repeat count.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Repeated_Seek_BWD_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<dash_h264_url>.mpd&operations=seekbwd(180),repeat(1),seekbwd(10),repeat(15)&options=seekInterval(20)&autotest=true
