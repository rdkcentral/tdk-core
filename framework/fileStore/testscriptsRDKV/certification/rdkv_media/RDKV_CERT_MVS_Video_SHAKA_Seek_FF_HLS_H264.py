...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Seek_FF_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video medium trick play operations like seek and FF of hls h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_721</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video medium trick play operations like seek and FF of hls h264 video codec content</test_objective>
...
fastfwd_check_interval: int</input_parameters>
...
3. Launch webkit instance with shaka test app url with the src url, operations to be performed, seek forward and fast-forward with interval.
...
7. Revert all values</automation_approch>
    <expected_output>All the provided player operations and expected events should occur for hls h264 content and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Seek_FF_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Seek_FF_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        # Video seeked forward after 30 seconds, then FF after 20 seconds in 2x speed and play after 30 seconds
        ...
        #url=<video_hls_h264_url>.m3u8&operations=seekfwd(30),fastfwd(10),playnow(5)&options=checkInterval(5),loop&autotest=true
