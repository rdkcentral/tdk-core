...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance to play dash h264 video content and perform mute, unmute operations repeatedly for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_725</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance to play dash h264 video content and perform mute, unmute operations repeatedly for given number of times</test_objective>
...
3. Launch webkit instance with shaka test app with the src url, operations to be performed, mute and unmute with given interval.
4. App performs the mute and unmute operations repeatedly and validates using events
5. If expected event volume change occurs for mute and unmute operations in all the repetition, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player audio mute and unmute should happen, expected event volume change should occur for all the repetition for dash h264 content and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Repeated_Mute_UnMute_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        # Setting Shaka Player Operations
        setOperation("mute","10")
        setOperation("unmute","10")
        ...
        #url=<video_dash_h264_url>.mpd&operations=mute(10),unmute(10),repeat(10)&autotest=true
