...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_3Sec_End_Video_Audio_Segment_DASH</name>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_638</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance. The objective is to assess the Player's response when delays are introduced for all audio and video  segment files throughout the stream, served via the Nginx streaming server with an intentional delay configuration.</test_objective>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning UVE AAMP player App URL: string   webkit_instance:string webinspect_port: string  video_audio_src_url_complete_dash: string</input_parameters>
...
10. Streaming server should be stopped.</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_3Sec_End_Video_Audio_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_3Sec_End_Video_Audio_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    video_chunk_pattern = "chunk-stream0-00011"   # Example video chunk pattern
    audio_chunk_pattern = "chunk-stream1-00015"  # Example audio chunk pattern (adjust based on audio stream ID)
    #combined_chunk_pattern = f"{video_chunk_pattern}|{audio_chunk_pattern}"  # Using regex OR (|) to match both
    ...
    generate_nginx_config(template_path, local_destination, remote_destination, nginx_container_ip, combined_chunk_pattern,delay3sec_value)
    # Check and start Nginx if necessary
    check_and_start_nginx()
    # Media source URL
    # Delay value
    videoURL = MediaValidationVariables.video_src_url_delay_dash
    delay = MediaValidationVariables.delay3sec_value 
