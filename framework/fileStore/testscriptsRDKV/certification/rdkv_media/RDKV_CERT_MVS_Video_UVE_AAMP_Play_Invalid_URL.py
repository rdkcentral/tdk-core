...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Invalid_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform playback of invalid stream url, check for playback Failed event and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_204</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform playback of invalid stream url, check for playback Failed event and close the player</test_objective>
...
video_src_url_invalid string</input_parameters>
...
3. Launch webkit instance with uve test app with the invalid video src url.
4. App tries to play the invalid stream and gives the playback failure event and error message.
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should take the invalid stream, expected event playback failed should occur</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Invalid_URL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Invalid_URL')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_invalid
        ...
        #url=<video_invalid_url>.mpd&operations=close(60)&autotest=true
            ...
            if logging_method == "REST_API":
                        ...
                        if len(lines) != lastIndex:
                            ...
                            for i in range(lastIndex,len(lines)):
                                ...
                                if "Event Occurred: playbackFailed" in lines[i]:
                                    event_status = 1
                                if "AAMP: Manifest Download failed" in lines[i]:
                                    error_status = 1
                ...
                while True:
                    ...
                    if "Event Occurred: playbackFailed" in console_log:
                        event_status = 1
                    if "AAMP: Manifest Download failed" in console_log:
                        error_status = 1
