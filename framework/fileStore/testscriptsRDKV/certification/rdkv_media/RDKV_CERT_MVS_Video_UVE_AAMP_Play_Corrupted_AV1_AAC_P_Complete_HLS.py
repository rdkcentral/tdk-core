...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_P_Complete_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>est script to initiate the TDK Lightning UVE AAMP Player application through a WebKit instance. The objective is to assess the media player's response when it receives corrupted content from the complete video segment of a clear AV1 with AAC codec stream, served via Streaming server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_769</test_case_id>
...
webinspect_port: string  video_src_url_corrupted_av1_aac_P_complete_hls: string</input_parameters>
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_P_Complete_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_P_Complete_HLS')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupted_av1_aac_P_complete_hls
    if pre_requisite_status == "SUCCESS":
            ...
            #url=<video_av1_url>.m2u8&operations=close(90)&autotest=true
