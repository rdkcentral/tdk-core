...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_Mute_UnMute_4K_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance to play 4k av1 video codec  content and perform mute, unmute operations repeatedly for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_517</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance to play 4k av1 video codec content and perform mute, unmute operations repeatedly for given number of times     </test_objective>
...
5. If expected event volumechange occurs for mute and unmute operations in all the repetition, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Player mute and unmute should happen, expected event volumechange should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Repeated_Mute_UnMute_4K_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_Mute_UnMute_4K_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_4k_av1_url>.mp4&operations=mute(5),unmute(5),repeat(15)&autotest=true&type=mp4
