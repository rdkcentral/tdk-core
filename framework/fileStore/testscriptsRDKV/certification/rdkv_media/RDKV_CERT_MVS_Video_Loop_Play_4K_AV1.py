...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Loop_Play_4K_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of 4K AV1 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_715</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of 4K AV1 video codec content</test_objective>
...
video_src_url_mp4_2160p_av1: string
</input_parameters>
...
3. Launch webkit instance with video test app url with the loop option.
4. App performs playback of 4k av1 stream. since loop option is enabled, playback continues from the start after reaching the end. app validates using events and position. loop count is calculated based on number of video position 0 occurred.
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the 4k av1 stream, expected events playing should occur, loop count should be 1 and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Loop_Play_4k_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Loop_Play_4K_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_mp4_2160p_av1
        # Setting VideoPlayer Operations
        # Check whether video loop happened after 60 sec. 30 sec video src should play twice
        setOperation("checkloop","96")
        ...
        setURLArgument("type",MediaValidationVariables.av1_url_type_fps)
        appArguments = getURLArguments()

        # Getting the complete test app URL for selected players
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_4k_av1).split(",")
        ...
        #url=<video_src_url_mp4_2160p_av1>.mp4&operations=checkloop(30)&autotest=true&type=mp4
