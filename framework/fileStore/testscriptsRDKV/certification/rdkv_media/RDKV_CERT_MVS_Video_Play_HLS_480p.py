...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_HLS_480p</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls content with 480p display resolution for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_208</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls content with 480p display resolution for few minutes and close the player</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,RPI, Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning Video Player app should be hosted
3.TV should be connected with the test device. If the 480p resolution is not supported then the test will be marked as failure</pre_requisite>
...
2. Get the current video display resolution and set resolution as 480p
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_HLS_480p</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_HLS_480p')
