...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_Ad_insertion_same_H264_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to validate the TDK Lightning UVE AAMP Player's ability to play DASH/HLS content through a Webkit instance, handle Ad insertion during playback, and seamlessly recover back to the main content even when the primary stream contains corruption.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_739</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance. The objective is to assess the player’s response during Ad insertion when a corrupted main content segment is encountered in a clear stream served via the streaming server.</test_objective>
...
webinspect_port: string  video_src_url_same_hls: string</input_parameters>
...
3. Launch webkit instance with video test app with the live dash/hls url.
4. App should play the live dash/hls content and event video playing should occur
5.App should play the live HLS content, an Ad segment should be inserted during playback, and playback should resume to the main content after the Ad
6. If expected event video playing is observed after Ad insertion and resuming to the main content, then update the result as SUCCESS or else FAILURE.
...
8.Revert all values.</automation_approch>
    <expected_output>Playback of the live DASH/HLS stream with Ad insertion should continue smoothly from main content to Ad and back to main content until the actual end of stream. </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_Ad_insertion_same_H264_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_Ad_insertion_same_H264_HLS')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_ad_same_hls
    if pre_requisite_status == "SUCCESS":
            ...
            #url=<video_ad_url>.m3u8&operations=close(60)&autotest=true
