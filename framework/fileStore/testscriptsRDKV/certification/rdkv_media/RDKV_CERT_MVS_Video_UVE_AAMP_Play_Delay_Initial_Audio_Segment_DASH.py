...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Initial_Audio_Segment_DASH</name>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_605</test_case_id>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning UVE AAMP player App URL: string   webkit_instance:string webinspect_port: string  audio_src_url_initial_dash: string</input_parameters>
...
6. App should play the dash dash content and event video playing should occur.
...
10. Streaming server should be stopped.</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Initial_Audio_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Delay_Initial_Audio_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    chunk_pattern = "chunk-stream1-00001"  # Example chunk pattern
    ...
    if pre_requisite_status == "SUCCESS":
            ...
            if "SUCCESS" in launch_status:
                ...
                if "FAILURE" in test_result:
                    print ("Original test result was FAILURE, but overriding to SUCCESS")
