...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_PlayPause_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of hevc mkv content continuously for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_390</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operations of hevc mkv content continuously for given number of times</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Repeated_PlayPause_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_PlayPause_HEVC_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_mkv>&operations=pause(5),play(5),repeat(15)&autotest=true&type=mkv
