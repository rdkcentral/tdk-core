...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_PlayPause_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play pause operations of mkv stream continuously for given number of times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play pause operations of mkv stream continuously for given number of times</test_objective>
...
4. App performs the pause and play operation of mp4 content repeatedly and validates using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should play and pause the mkv video, expected events should occur for all the repetition and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_PlayPause_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Repeated_PlayPause_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_mkv>&operations=pause(5),play(5),repeat(15)&autotest=true
