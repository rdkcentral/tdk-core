...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_PlayPause_AV1_VORBIS_WEBM_480p</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation on webm content with av1 and Vorbis codecs with 480p resolution for few minutes and then close the player.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_885</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation on webm content with av1 and Vorbis codecs with 480p resolution for few minutes and then close the player.</test_objective>
...
video_src_url_av1_vorbis_480p: string
play_interval: int
pause_interval:int
</input_parameters>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_PlayPause_AV1_VORBIS_WEBM_480p</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_PlayPause_AV1_VORBIS_WEBM_480p')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_av1_vorbis_480p
