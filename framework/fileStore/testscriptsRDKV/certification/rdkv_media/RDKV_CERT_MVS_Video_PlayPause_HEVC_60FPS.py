...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_PlayPause_HEVC_60FPS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of hevc 60 fps stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_484</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of hevc 60 fps stream</test_objective>
...
4. App performs the play and pause operations of hevc 60fps stream and validates each operation using events
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_PlayPause_HEVC_60FPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_PlayPause_HEVC_60FPS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_60fps>&operations=pause(30),play(10)&autotest=true&type=mp4&options=collectfps(WebKitBrowser),deviceport(9998),expectedfps(57)
