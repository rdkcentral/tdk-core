...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_Middle_Video_Segment_DASH</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_media_test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the middle video segment of a clear stream,served via streaming server.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_423</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the middle video segment of a clear stream,served via streaming server.</test_objective>
...
webinspect_port: string  video_src_url_corrupt_I_B_middle_dash: string</input_parameters>
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_Middle_Video_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_Middle_Video_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupt_I_B_middle_dash
    if pre_requisite_status == "SUCCESS":
            print("The content is corrupted with I and B  frame at middle segment of the video")
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the I and B frames during the middle segment of the video from 30 to 42 seconds *************\n")
