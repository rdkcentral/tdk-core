...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Objects Animation application to render given number of rectangles and check whether the average FPS calculated for 60 sec duration value is as expected</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_22</test_case_id>
    <test_objective>Test Script to launch a lightning Objects Animation application to render given number of rectangles and check whether the average FPS calculated for 60 sec duration value is as expected</test_objective>
...
2.Lightning Animation app should be hosted</pre_requisite>
    ...
    <input_parameters>Lightning Objects Animation App URL: string
...
3. Launch LightningApp webkit instance with Objects animation test app url with the duration 60 sec
4. App performs objects animation by rendering given number of rectangles for 60 sec and stops after that.
5. App gives average FPS value after duration of 60 sec
...
7. Revert all values</automation_approch>
    <expected_output>Object Animation should happen for 60 sec and average FPS should be grater than or equal to expected fps value.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Animation_Objects_Average_FPS</test_script>
    <skipped>No</skipped>
        ...
        if "SUCCESS" in launch_status:
            ...
            if "SUCCESS" in test_result:
                ...
                if "NaN" in str(average_fps):
                    ...
                    print("%d Rectangles are rendered for around 60 sec and average FPS is as expected" %(MediaValidationVariables.objects_count))
                    print("[TEST EXECUTION RESULT]: SUCCESS")
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("Average FPS is < %f" %(minfps))
                    print("%d Rectangles rendered for around 60 sec and average FPS is not as expected" %(MediaValidationVariables.objects_count))
                    print("[TEST EXECUTION RESULT]: FAILURE")
                    tdkTestObj.setResultStatus("FAILURE");
            else:
                tdkTestObj.setResultStatus("FAILURE");
