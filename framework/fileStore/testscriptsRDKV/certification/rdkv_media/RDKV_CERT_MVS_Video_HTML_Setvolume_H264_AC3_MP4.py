...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Setvolume_H264_AC3_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to launch an HTML video player application via WebKit instance and perform volume level operation on mp4 content with h264 and AC3 codecs for a few minutes and then close the player.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_926</test_case_id>
    <test_objective>Test script to launch an HTML video player application via WebKit instance and perform volume level operation on mp4 content with h264 and AC3 codecs for a few minutes and then close the player.</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_Setvolume_H264_AC3_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Setvolume_H264_AC3_MP4')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_h264_ac3_mp4_url>&operations=setvolume(0.5),close(30)
