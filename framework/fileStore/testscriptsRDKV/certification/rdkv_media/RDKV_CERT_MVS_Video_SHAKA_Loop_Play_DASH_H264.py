...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Loop_Play_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video loop playback of dash h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_743</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video loop playback of dash h264 video codec content</test_objective>
...
video_src_url_short_duration_dash: string
</input_parameters>
    <automation_approch>1. As pre requisite, launch webkit instance via RDKShell
2. Store the details of other launched apps. Move the webkit instance to front, if its z-order is low.
3. Launch webkit instance with shaka test app url with the loop option.
4. App performs playback of dash h264 codec stream. since loop option is enabled, playback continues from the start after reaching the end. app validates using events and position. loop count is calculated based on number of video position 0 occurred.
5. If expected events playing occurs and video loop count is 1, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player should play the dash h264 video codec stream, expected events playing should occur, loop count should be 1 and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Loop_Play_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Loop_Play_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_short_duration_dash
        # Setting VideoPlayer Operations
        # Check whether video loop happened after 30 sec. 15 sec video src should play twice
        setOperation("checkloop","30")
        ...
        #url=<dash_h264_url>.mpd&operations=checkloop(30)&autotest=true
