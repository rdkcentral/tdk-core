...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Seek_FWD_AV1_AAC_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video seek forward operation of mp4 content with av1 and aac codecs for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_693</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video seek forward operation of mp4 content with av1 and aac codecs for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_Seek_FWD_AV1_AAC_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Seek_FWD_AV1_AAC_MP4')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_av1_aac_mp4_url>&operations=seekfwd(30),close(30)
