...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_DASH_1080i</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with 1080i display resolution for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_102</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of dash content with 1080i display resolution for few minutes and close the player</test_objective>
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_DASH_1080i</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_DASH_1080i')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #http://*testManagerIP*/rdk-test-tool/fileStore/lightning-apps/tdvideoplayer/build/index.html?
