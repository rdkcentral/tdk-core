...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_P_Middle_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a WebKit instance. The objective is to assess the media player's response when it receives corrupted content from the middle video segment of a clear AV1 with AAC codec stream, served via Streaming server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
  <test_case_id>RDKV_Media_Validation_780</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a WebKit instance. The objective is to assess the media player's response when it receives corrupted content from the middle video segment of a clear AV1 with AAC codec stream, served via Streaming server</test_objective>
...
webinspect_port: string  video_src_url_corrupted_av1_aac_I_P_middle_dash: string</input_parameters>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_AV1_AAC_I_P_Middle_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupted_av1_aac_I_P_middle_dash
    if pre_requisite_status == "SUCCESS":
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the I and P frames during the middle segment of the video from 30 to 42 seconds *************\n")
