...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek to position operation of hls h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_724</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek to position operation of hls h264 video codec content</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player should perform seek to position of h264 hls stream and seeking, seeked events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hls_h264_url>.m3u8&operations=seekpos(30:120),seekpos(30:75),close(30)&options=checkInterval(10),loop&autotest=true
