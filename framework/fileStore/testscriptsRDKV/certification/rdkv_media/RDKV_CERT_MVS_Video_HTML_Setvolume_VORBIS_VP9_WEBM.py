...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Setvolume_VORBIS_VP9_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to launch an HTML video player application via WebKit instance and perform volume level operation on webm content with Vorbis and Vp9 codecs for a few minutes and then close the player.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_931</test_case_id>
    <test_objective>Test script to launch an HTML video player application via WebKit instance and perform volume level operation on webm content with Vorbis and Vp9 codecs for a few minutes and then close the player.</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_Setvolume_VORBIS_VP9_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Setvolume_VORBIS_VP9_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_vorbis_vp9_webm_url>&operations=setvolume(0.5),close(30)
