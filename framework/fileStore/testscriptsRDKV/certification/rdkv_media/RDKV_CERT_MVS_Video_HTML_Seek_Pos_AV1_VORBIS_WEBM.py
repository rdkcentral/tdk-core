...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_VORBIS_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video seek position operation of webm content with av1 and Vorbis codecs for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_915</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video seek position operation of webm content with av1 and Vorbis codecs for few minutes and close the player</test_objective>
...
video_src_url_av1_vorbis: string
...
3. Launch webkit instance with html player test app with the video src url and the seek position operations to be performed.
...
7. Revert all values</automation_approch>
    <expected_output>Player should perform the seek position operation correctly, the expected events should occur, playback should resume from the seek position operation, and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_VORBIS_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Seek_Pos_AV1_VORBIS_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_av1_vorbis
        ...
        setURLArgument("player","HTMLPlayer")
        ...
        #url=<video_av1_vorbis_webm_url>&operations=seekfwd(30),seekbwd(30),close(30)
        ...
        if "SUCCESS" in launch_status:
            ...
            test_result,proc_check_list = monitorVideoTest(obj,webkit_console_socket,validation_dict,"Observed Event: seeking,seeked");
