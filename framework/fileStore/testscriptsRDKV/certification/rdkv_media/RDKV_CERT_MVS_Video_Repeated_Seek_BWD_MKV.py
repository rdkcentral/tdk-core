...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Repeated_Seek_BWD_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video seek backward operation of mkv content continuously for given number of times in provided interval</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_392</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video seek backward operation of mkv content continuously for given number of times in provided interval</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Repeated_Seek_BWD_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Repeated_Seek_BWD_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_mkv>.mkv&operations=seekbwd(180),repeat(1),seekbwd(10),repat(15)&options=seekInterval(20)&autotest=true&type=mkv
