...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Mute_UnMute_Audio_Only</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance to play audio only content and perform mute and unmute operations</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_118</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance to play audio only content and perform mute and unmute operations</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Mute_UnMute_Audio_Only</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Mute_UnMute_Audio_Only')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<audio_url>.mpd&operations=mute(30),unmute(30)&autotest=true&type=dash
