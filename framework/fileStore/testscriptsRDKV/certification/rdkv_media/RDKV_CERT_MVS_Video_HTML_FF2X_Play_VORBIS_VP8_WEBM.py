...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_FF2X_Play_VORBIS_VP8_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with Vorbis and Vp8 codecs with playback rate 2x for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_828</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video fast forward operation of webm content with Vorbis and Vp8 codecs with playback rate 2x for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_FF2X_Play_VORBIS_VP8_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_FF2X_Play_VORBIS_VP8_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_vorbis_vp8_webm_url>&operations=fastfwd2x(30),playnow(10),close(30)
