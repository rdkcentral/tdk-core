...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_Mute_Unmute_AV1_VORBIS_WEBM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to launch an HTML video player application via WebKit instance and perform mute and unmute operations on webm content with av1 and Vorbis codecs for a few minutes and then close the player.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_799</test_case_id>
    <test_objective>Test script to launch an HTML video player application via WebKit instance and perform mute and unmute operations on webm content with av1 and Vorbis codecs for a few minutes and then close the player.</test_objective>
...
3. Launch the WebKit instance with the HTML player test app using the video src URL, and perform mute and unmute operations.
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the video and perform mute and unmute operations correctly. The expected playing event should occur, and if proc validation is applicable, the expected data should be available in the proc file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_HTML_Mute_Unmute_AV1_VORBIS_WEBM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_Mute_Unmute_AV1_VORBIS_WEBM')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_av1_vorbis_webm_url>&operations=mute(30),unmute(30)
