...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_TerminateApp_LifeCycle');
    ...
    if status == "SUCCESS":
        ...
        if status == "SUCCESS":
            ...
            if status == "SUCCESS":
                ...
                if result == "SUCCESS":
                    event_dict = {"APP_STATE_ACTIVE": False, "APP_STATE_PAUSED" : False, "APP_STATE_TERMINATING" : False}
                    tdkTestObj.setResultStatus("SUCCESS")
                    continue_count = 0
                    while True:
                        ...
                        if app_name in event and "onAppLifecycleStateChanged" in str(event):
                            ...
                            if "APP_STATE_TERMINATING" in event:
