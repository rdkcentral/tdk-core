...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PVS_Functional_HtmlApp_TimeTo_SuspendResume</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_setValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to validate the time taken to suspend and resume HtmlApp plugin</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_79</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to suspend and resume HtmlApp plugin</test_objective>
...
2. The time in Test Manager should be in sync with UTC </pre_requisite>
    ...
    <automation_approch>1. Resume the HtmlApp plugin.
...
10. Verify the time values are within the expected range</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Functional_HtmlApp_TimeTo_SuspendResume</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_HtmlApp_TimeTo_SuspendResume')
...
if expectedResult in result.upper():
    ...
    plugin_status_needed = {"HtmlApp":"resumed","Cobalt":"deactivated","WebKitBrowser":"deactivated"}
                    ...
                    if htmlapp_status == 'resumed' and expectedResult in result:
                        ...
                        if (len(event_listener.getEventsBuffer())!= 0):
                            for event_log in event_listener.getEventsBuffer():
                                ...
                                if json_msg["params"]["client"] == "HtmlApp":
                            ...
                            if suspended_time and resumed_time:
                                ...
                                suspend_config_status,suspend_threshold = getDeviceConfigKeyValue(conf_file,"HTMLAPP_SUSPEND_TIME_THRESHOLD_VALUE")
                                Summ_list.append('HTMLAPP_SUSPEND_TIME_THRESHOLD_VALUE :{}ms'.format(suspend_threshold))
                                resume_config_status,resume_threshold = getDeviceConfigKeyValue(conf_file,"HTMLAPP_RESUME_TIME_THRESHOLD_VALUE")
                                Summ_list.append('HTMLAPP_RESUME_TIME_THRESHOLD_VALUE :{}ms'.format(resume_threshold))
