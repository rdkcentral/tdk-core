...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_TimeTo_KillApp_LifeCycle');
            ...
            if result == "SUCCESS":
                ...
                if all(event_dict.values()):
                    try:
                        ...
                        time_paused_ms = (paused_time - kill_start_time).total_seconds() * 1000
                        ...
                        time_total_ms = (unload_time - kill_start_time).total_seconds() * 1000
                        ...
                        config_status,kill_threshold = getDeviceConfigKeyValue(conf_file,"APPMANAGER_KILL_EVENT_THRESHOLD_VALUE")
                        config_status,kill_offset = getDeviceConfigKeyValue(conf_file,"THRESHOLD_OFFSET")
                        
                        Summ_list.append('KILL_LIFECYCLE_THRESHOLD_VALUE :{}ms'.format(kill_threshold))
                        ...
                        try:
                            if 0 < int(time_paused_ms) < (int(kill_threshold) + int(kill_offset)):
                        ...
                        try:
                            if 0 < int(time_terminated_ms) < (int(kill_threshold) + int(kill_offset)):
                        ...
                        try:
                            if 0 < int(time_unload_ms) < (int(kill_threshold) + int(kill_offset)):
