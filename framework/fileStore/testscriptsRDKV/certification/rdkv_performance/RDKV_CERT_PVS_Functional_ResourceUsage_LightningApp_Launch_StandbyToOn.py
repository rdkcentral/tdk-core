...
<xml>
  ...
  <name>RDKV_CERT_PVS_Functional_ResourceUsage_LightningApp_Launch_StandbyToOn</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to calculate the resource usage during Html launch in standby mode to ON</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to calculate the resource usage during LightningApp launch in standby mode to ON.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI,Accelerator</test_setup>
    <pre_requisite>1. wpeframework should be running in DUT.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. As a prerequisite enable System plugin disable WebKit and lightning app plugins.
2. Get the current power state.
3. Set the power state to STANDBY
4. Set the power state to ON and launch LightningApp
...
Resource usage should be within the expected range.</expected_output>
    ...
    <test_script>RDKV_CERT_PVS_Functional_ResourceUsage_LightningApp_Launch_StandbyToOn</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_ResourceUsage_LightningApp_Launch_StandbyToOn');
            ...
            if expectedResult in result and current_power_state == "STANDBY":
                ...
                if expectedResult in result:
                    ...
                    if expectedResult in result and current_power_state == "ON":
                        ...
                        print("\n Current power state : \n",current_power_state)
                        tdkTestObj.setResultStatus("SUCCESS")
                        LightningApp_launch_status,launch_time = launch_plugin(obj,"LightningApp")
                        time.sleep(10)
                        if LightningApp_launch_status == "SUCCESS":
                            time.sleep(5)
