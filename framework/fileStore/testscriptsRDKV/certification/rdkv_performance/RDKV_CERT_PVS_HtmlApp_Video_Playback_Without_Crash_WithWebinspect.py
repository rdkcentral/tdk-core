...
<xml>
  ...
  <name>RDKV_CERT_PVS_HtmlApp_Video_Playback_Without_Crash_WithWebinspect</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The Objective of the test is to check for any crashes for the Html application by keeping the web inspect port open for play and pause operations of mp4 video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_162</test_case_id>
    <test_objective>The Objective of the test is to check for any crashes for the Html application by keeping the web inspect port open for play and pause operations of mp4 video</test_objective>
    ...
    <automation_approch>1.Open Html app web inspect port
...
4.Check for any crashes during playback</automation_approch>
    <expected_output>video playback should happen without crash</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_performance</test_stub_interface>
    <test_script>RDKV_CERT_PVS_HtmlApp_Video_Playback_Without_Crash_WithWebinspect</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_HtmlApp_Video_Playback_Without_Crash_WithWebinspect');
deviceIP =str(ip)
                     ...
                     if ssh_param_dict != {}:
                        while int(round(time.time() * 1000)) < time_limit:
                            ...
                            if ("crash" or "CRASH" or "Crash") in output:
                                ...
                                print("\n Validate the status of HtmlApp to confirm the crash:\n")
                                ...
                                if output != 'deactivated':
                                   ...
                                   tdkTestObj.setResultStatus("SUCCESS")
                                    ...
                                    print("Crash is observed during video playback and plugin is deactivated")
                                    tdkTestObj.setResultStatus("FAILURE")
