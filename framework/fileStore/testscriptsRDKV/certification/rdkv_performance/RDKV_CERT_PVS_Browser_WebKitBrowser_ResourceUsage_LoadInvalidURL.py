...
<xml>
  ...
  <name>RDKV_CERT_PVS_Browser_WebKitBrowser_ResourceUsage_LoadInvalidURL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the resource usage after loading Invalid URL in WebkitBrowser and check whether WebkitBrowser is stable, and no crash is observed.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to validate the resource usage after loading InvalidURL in WebkitBrowser and check whether WebkitBrowser is stable, and no crash is observed.</test_objective>
    ...
    <input_parameters>webkit_page_url: string</input_parameters>
    <automation_approch>1. Launch WebkitBrowser using RDKShell
2. Set a Invalid URL in WebkitBrowser .
...
6. Revert the status of WebkitBrowser.</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Browser_WebKitBrowser_ResourceUsage_LoadInvalidURL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Browser_WebKitBrowser_ResourceUsage_LoadInvalidURL')
...
if expectedResult in result.upper():    
    ...
    revert="NO"
    status = "SUCCESS"
    plugins_list = ["WebKitBrowser","Cobalt","WebKitBrowser"]
    plugin_status_needed = {"WebKitBrowser":"deactivated","Cobalt":"deactivated","WebKitBrowser":"deactivated"}
        ...
        if launch_status == expectedResult:
            ...
            if webkit_status == 'resumed' and expectedResult in result:
                ...
                if browser_invalid_test_url != "" and ssh_param_dict != {}:
                    ...
                    if current_url != None and expectedResult in result:
                        ...
                        if expectedResult in result:
                            ...
                            if output != "EXCEPTION" and "Failed to load resource" in output and url_to_grep in output:
                                print("\n Invalid URL is not loaded in WebKitBrowser \n")
                                ...
                                if not crash_status :
                                    print("Crash is not observed while loading invalid URL in WebKitBrowser")
                                    print("\n Validate the status of WebKitBrowser plugin :\n")
                                    tdkTestObj = obj.createTestStep('rdkservice_getPluginStatus')
                                    tdkTestObj.addParameter("plugin","WebKitBrowser")
                                    #Execute the test case in DUT
                                    tdkTestObj.executeTestCase(expectedResult);
                                    ...
                                    if output != 'deactivated':
                                        ...
                                        print(" WebKitBrowser got deactivated automatically while loading an invalid URL")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                    ...
                    print("\nFailed to get the URL in WebKitBrowser")
                    tdkTestObj.setResultStatus("FAILURE")
            else:
                print("\n Error while resuming WebKitBrowser, current status: ",webkit_status)
                tdkTestObj.setResultStatus("FAILURE")
