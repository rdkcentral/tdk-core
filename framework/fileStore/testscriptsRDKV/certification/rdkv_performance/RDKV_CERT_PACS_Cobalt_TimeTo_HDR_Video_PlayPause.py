...
<xml>
  ...
  <name>RDKV_CERT_PACS_Cobalt_TimeTo_HDR_Video_PlayPause</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the time taken to play and pause a HDR video on YouTube.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to validate the time taken to play and pause a HDR video on YouTube.</test_objective>
...
2. Set an HDR video URL using deeplink method.
...
10. Deactivate the Cobalt plugin.
    ...
    <test_script>RDKV_CERT_PACS_Cobalt_TimeTo_HDR_Video_PlayPause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PACS_Cobalt_TimeTo_HDR_Video_PlayPause');
...
if expectedResult in result.upper():
    ...
    status = "SUCCESS"
    revert = "NO"
    cobalt_test_url = PerformanceTestVariables.HDRvideo_test_url
    if cobalt_test_url == "":
        print ("\n Please configure the HDRvideo_test_url value\n")
