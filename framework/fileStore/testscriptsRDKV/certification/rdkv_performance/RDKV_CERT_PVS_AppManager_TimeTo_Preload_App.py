...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_TimeTo_Preload_App');
...
if expectedResult in result.upper():
    ...
    if status == "SUCCESS":
        ...
        if status == "SUCCESS":
            ...
            print("Register for the preload event")
            ...
            if status == "SUCCESS":
                ...
                while True:
                    ...
                    if app_name in event and "onAppLifecycleStateChanged" in str(event):
                        if "APP_STATE_PAUSED" in event:
                            print("Received preload event")
                            tdkTestObj.setResultStatus("SUCCESS")
                            break   
                if  "APP_STATE_PAUSED" in event:
                    print("Received preload event successfully")
                    tdkTestObj.setResultStatus("SUCCESS")
                    ...
                    Summ_list.append('App preload initiated at :{}'.format(start_time))
                    Summ_list.append('App preloaded at :{}'.format(launch_time))
                    time_taken_for_launch = launched_time - launch_start_time
                    time_taken_for_launch = time_taken_for_launch.total_seconds() * 1000
                    print("\nPreload initiated at ", start_time)
                    print("\n Preload completed at ", launch_time)
                    print("\n Time taken to preload the app : {}(ms)".format(time_taken_for_launch))
                    Summ_list.append('Time taken to preload app :{}ms'.format(time_taken_for_launch))
                    print("\n Threshold value for time taken to preload the app: {} ms".format(launch_threshold))
                    print("\n Validate the time:")
                    ...
                    print("Failed to receive preload event")
                    tdkTestObj.setResultStatus("FAILURE")
                print("\n Terminating the app")
                ...
                if result == "SUCCESS":
                    print(f"\nSuccessfully terminated {app_name}")
                    tdkTestObj.setResultStatus("SUCCESS")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
