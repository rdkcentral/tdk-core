...
<xml>
  ...
  <name>RDKV_CERT_PVS_Browser_Speedometer</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>rdkservice_getBrowserScore_Speedometer</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the responsiveness of WebApplications from the browser using Speedometer</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
<!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_99</test_case_id>
    <test_objective>The objective of this test is to validate the responsiveness of WebApplications from the browser using Speedometer</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>1. Threshold value of Speedometer
2. Speedometer test URL</input_parameters>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Browser_Speedometer')
...
if expectedResult in result.upper():
    app_bundle_name=BrowserPerformanceVariables.speed_app_bundle_name
    app_download_url=BrowserPerformanceVariables.speed_app_download_url
    app_name = "com.rdkcentral.speed"
    status = rdkservice_install_launch_app(obj, app_bundle_name, app_name, app_download_url)
    if status == "SUCCESS":                
        browsertest_keypress(obj,app_name,[["9","13"],[]])
        time.sleep(2000)
        tdkTestObj = obj.createTestStep('rdkservice_getBrowserScore_Speedometer')
        ...
        if browser_score_dict["main_score"] != "Unable to get the browser score" and expectedResult in result:
            ...
            result1, speedometer_threshold_value = getDeviceConfigKeyValue(conf_file,"SPEEDOMETER_THRESHOLD_VALUE")
            if speedometer_threshold_value != "":
                print("\n Browser score from test: ",browser_score)
                Summ_list.append('Browser score from test is :{}'.format(browser_score))
                print("\n Threshold value for browser score:",speedometer_threshold_value)
                Summ_list.append('Threshold value for browser score: {}'.format(speedometer_threshold_value))
                if float(browser_score) > float(speedometer_threshold_value):
