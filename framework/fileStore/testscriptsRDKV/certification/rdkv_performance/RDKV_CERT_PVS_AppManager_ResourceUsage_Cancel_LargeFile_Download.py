...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_ResourceUsage_Cancel_LargeFile_Download');
...
if expectedResult in result.upper():
    ...
    print(f"Current plugin status : {curr_plugins_status_dict}")
    ...
    if status == "SUCCESS":
        ...
        file_locator = file_locator.split("/package")[0] 
        ...
        print(f"getSSHParams status : {status}")
        if status == expectedResult and ssh_params not in ("", "{}"):
            ...
            if not ssh_method or not credentials:
                ...
                print(f"initial disk usage command status : {status}, output : {output}")
                if status == expectedResult:
                    tdkTestObj.setResultStatus("SUCCESS")
                    output = output.strip().splitlines()[-1]
                    if output != "":
                        print(f"Initial disk usage for {file_locator} retrieved successfully.")
                        ...
                        print(f"download_app_bundle status : {status}, result : {result}")
                        if status == "SUCCESS":
                            tdkTestObj.setResultStatus("SUCCESS")
                            ...
                            status = tdkTestObj.getResult()   
                            result = tdkTestObj.getResultDetails()           
                            print(f"progress check status : {status}, result : {result}")
                            if status == expectedResult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"Successfully retrieved the download progress for {app_bundle_name}")
                                ...
                                if progress > 0 and progress < 100:
                                    ...
                                    tdkTestObj.executeTestCase(expectedResult)
                                    status = tdkTestObj.getResult()
                                    print(f"cancel download status : {status}")
                                    if status == "SUCCESS":
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        ...
                                        while True:
                                            ...
                                            print("\nEvent:", event)
                                            break
                                        print(f"wait loop continue_count : {continue_count}, event : {event}")
                                        if "onAppDownloadStatus" in str(event) and "DOWNLOAD_FAILURE" in str(event):
                                            ...
                                            if int(event_download_id) == download_id and str(download_id) in filelocator_url:
                                                ...
                                                tdkTestObj.executeTestCase(expectedResult)
                                                status = tdkTestObj.getResult()
                                                print(f"delete package status : {status}")
                                                if status == "SUCCESS":
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    ...
                                                    status = tdkTestObj.getResult()
                                                    output = tdkTestObj.getResultDetails()
                                                    print(f"verify deletion status : {status}, output : {output}")
                                                    if status == expectedResult and "No such file or directory" in output:
                                                        ...
                                                        if status == expectedResult:
                                                            ...
                                                            if output != "":
                                                                ...
                                                                print(f"Failed to retrieve disk usage after deletion of the package")
                                                                tdkTestObj.setResultStatus("FAILURE")
                                                        else:
                                                            print(f"Failed to retrieve disk usage after deletion of the package")
                                                            tdkTestObj.setResultStatus("FAILURE")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"Failed to get the download progress for {app_bundle_name}")
                        else:
                            print(f"Failed to download {app_bundle_name} from {app_download_url}")
                            tdkTestObj.setResultStatus("FAILURE") 
                    else:
                        print(f"Failed to initiate the download for {app_bundle_name}")
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print(f"Download of {app_bundle_name} did not start as expected")
                    tdkTestObj.setResultStatus("FAILURE")
