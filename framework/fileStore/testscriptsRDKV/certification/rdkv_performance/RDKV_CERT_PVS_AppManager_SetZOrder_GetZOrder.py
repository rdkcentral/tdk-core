...
if expectedResult in result.upper():
    ...
    if status == "SUCCESS":
        ...
        if status == "SUCCESS":
            ...
            if app_instance_id:
                print("appInstanceId: %s" % app_instance_id)
                set_step = obj.createTestStep("rdkservice_setValue")
                set_step.addParameter("method", "org.rdk.RDKWindowManager.setZOrder")
                set_step.addParameter(
                    "value",
                    '{"clientId": "%s", "zOrder": %d}' % (app_instance_id, requested_zorder)
                )
                set_step.executeTestCase(expectedResult)
                if expectedResult in set_step.getResult():
                    get_step = obj.createTestStep("rdkservice_setValue")
                    get_step.addParameter("method", "org.rdk.RDKWindowManager.getZOrder")
                    get_step.addParameter("value", '{"clientId": "%s"}' % app_instance_id)
                    get_step.executeTestCase(expectedResult)
                    if expectedResult in get_step.getResult():
                        try:
                            actual_zorder = ast.literal_eval(get_step.getResultDetails())
                        ...
                        print("Failed to get the application z-order")
                else:
                    print("Failed to set the application z-order")
            else:
                print("Active application instance id was not received")
        else:
            print("Failed to install or launch the application")
    else:
        print("AppManager plugins are not active")

    terminate_step = obj.createTestStep("rdkv_terminate_app")
    terminate_step.addParameter("app_id", app_name)
    terminate_step.executeTestCase(expectedResult)
    if terminate_step.getResult() != expectedResult:
        print("Failed to terminate the application")
        test_status = "FAILURE"

    terminate_step.setResultStatus(test_status)
