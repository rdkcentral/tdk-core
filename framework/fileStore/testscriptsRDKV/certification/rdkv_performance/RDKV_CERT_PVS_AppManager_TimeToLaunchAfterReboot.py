...
if expectedResult in result.upper():
    ...
    # ========================================================
    # WAIT FOR DEVICE STABILITY AFTER REBOOT
    # ========================================================
    print("\nWaiting for device to stabilize after reboot...\n")
    time.sleep(30)
    ...
    event_listener = createEventListener(
        ip,
        thunder_port,
        ['{"jsonrpc": "2.0","id": 5,"method": "org.rdk.LifecycleManager.1.register","params": {"event": "onAppLifecycleStateChanged", "id": "client.events.1" }}'],
        "/jsonrpc",
        False
    )
    ...
    if final_status == "SUCCESS":
        ...
        print("\nTime taken to launch app after reboot: {} ms".format(launch_time))

        Summ_list.append("Launch time after reboot: {} ms".format(launch_time))
        ...
        if threshold and offset:
            ...
            if 0 < launch_time < (int(threshold) + int(offset)):
                print("\nLaunch time within threshold\n")
            else:
                print("\nLaunch time exceeded threshold\n")
                final_status = "FAILURE"
        else:
