...
if expectedResult in result.upper():
    ...
    if missing_plugins:
        ...
        if status == "SUCCESS":
            ...
            # Get app name and download URL from configuration file
            ...
            if app_name_status != "SUCCESS" or not app_id:
                ...
                # Use TDK test steps for download/install (if needed) and then launch/close
                download_timeout = 120  # 2 minutes
                install_timeout = 120   # 2 minutes
                launch_timeout = 120    # 2 minutes
                close_timeout = 120     # 2 minutes
                ...
                # Step 2: Launch the app (either freshly installed or already existing)
                if status == "SUCCESS":
                print(f"\n Starting app launch for {app_id} (pre-requisite for close test)... \n")
                tdkTestObj = obj.createTestStep('rdkservice_setValue')
                ...
                if launch_result == "SUCCESS":
                    ...
                    if launched_time and launch_success:
                        ...
                        # Step 3: Close the launched app (Main focus of this test)
                        print(f"\n Starting app close for {app_id}... \n")
                        tdkTestObj = obj.createTestStep('rdkservice_setValue')
                        tdkTestObj.addParameter("method", "org.rdk.AppManager.1.terminateApp")
                        tdkTestObj.addParameter("value", '{"appId": "' + app_id + '"}')
                        close_start_time = str(datetime.utcnow()).split()[1]
                        tdkTestObj.executeTestCase(expectedResult)
                        close_result = tdkTestObj.getResult()
                        
                        if close_result == "SUCCESS":
                            print(f"\n Close initiated successfully for {app_id} \n")
                            
                            # Wait for close completion (using lifecycle events)
                            continue_count = 0
                            closed_time = ""
                            close_success = False
                            
                            while continue_count < close_timeout:
                                if len(event_listener.getEventsBuffer()) == 0:
                                    continue_count += 1
                                    time.sleep(1)
                                    continue
                                ...
                                event_log = event_listener.getEventsBuffer().pop(0)
                                print(f"\n Close event: {event_log} \n")
                                
                                if app_id in event_log and "onAppLifecycleStateChanged" in str(event_log):
                                    event_data = json.loads(event_log.split('$$$')[1])
                                    if event_data.get("state") in ["stopped", "terminated", "suspended"]:
                                        print(f"\n Event: App lifecycle changed to {event_data.get('state')} for {app_id} \n")
                                        closed_time = event_log.split('$$$')[0]
                                        close_success = True
                                        break
                                        
                            if closed_time and close_success:
                                close_start_time_in_millisec = getTimeInMilliSec(close_start_time)
                                closed_time_in_millisec = getTimeInMilliSec(closed_time)
                                close_time_taken = closed_time_in_millisec - close_start_time_in_millisec
                                results_dict["close_time"] = close_time_taken
                                results_dict["close_success"] = True
                                status = "SUCCESS"
                                print(f"\n App close completed successfully in {close_time_taken}ms \n")
                            else:
                                print(f"\n App close event not received or close failed for {app_id} \n")
                                tdkTestObj.setResultStatus("FAILURE")
                                status = "FAILURE"
                        else:
                            print(f"\n Failed to initiate close for {app_id} \n")
                            tdkTestObj.setResultStatus("FAILURE")
                            status = "FAILURE"
                    else:
                ...
                if status == "SUCCESS" and results_dict["close_success"]:
                    test_app_id = app_id
                    time_taken_for_close = results_dict["close_time"]
                    
                    print(f"\n Launch Time: {results_dict['launch_time']}ms \n")
                    print(f"\n Close Time: {time_taken_for_close}ms \n")
                    
                    Summ_list.append(f'Time taken to launch app: {results_dict["launch_time"]}ms')
                    Summ_list.append(f'Time taken to close app: {time_taken_for_close}ms')
                    
                    # Add download and install times to summary if app was not pre-installed
                    ...
                    config_status, app_close_threshold = getDeviceConfigKeyValue(conf_file, "APP_CLOSE_THRESHOLD_VALUE")
                    offset_status, offset = getDeviceConfigKeyValue(conf_file, "THRESHOLD_OFFSET")
                    
                    Summ_list.append('APP_CLOSE_THRESHOLD_VALUE: {}ms'.format(app_close_threshold))
                    Summ_list.append('THRESHOLD_OFFSET: {}ms'.format(offset))
                    
                    if all(value != "" for value in (app_close_threshold, offset)):
                        print(f"\n Threshold value for time taken to close app: {app_close_threshold} ms")
                        print("\n Validate the time: \n")
                        
                        if 0 < time_taken_for_close < (int(app_close_threshold) + int(offset)):
                            print(f"\n Time taken for closing {test_app_id} is within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("SUCCESS")
                        else:
                            print(f"\n Time taken for closing {test_app_id} is not within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("\n Please configure the APP_CLOSE_THRESHOLD_VALUE in device configuration file \n")
                        tdkTestObj = obj.createTestStep('rdkservice_setValue')
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print(f"\n Launch/Close operation failed for app {app_id} \n")
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    tdkTestObj.setResultStatus("FAILURE")
