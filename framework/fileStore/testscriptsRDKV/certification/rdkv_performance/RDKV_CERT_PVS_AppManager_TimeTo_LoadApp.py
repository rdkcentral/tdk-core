    ...
    if missing_plugins:
        ...
        if status == "SUCCESS":
            ...
            if config_status != "SUCCESS" or not app_download_url:
                ...
                # Use TDK test steps for download/install (if needed) and then load app
                download_timeout = 120  # 2 minutes
                install_timeout = 120   # 2 minutes
                launch_timeout = 120    # 2 minutes
                load_timeout = 180      # 3 minutes (loading can take longer)
                ...
                # Step 2: Launch and Load the app (Main focus of this test)
                if status == "SUCCESS":
                    print(f"\n Starting app launch for loading test {app_id}... \n")
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    tdkTestObj.addParameter("method", "org.rdk.AppManager.1.launchApp")
                    tdkTestObj.addParameter("value", '{"appId": "' + app_id + '"}')
                    launch_start_time = str(datetime.utcnow()).split()[1]
                    load_start_time = launch_start_time  # Load timing starts with launch
                    tdkTestObj.executeTestCase(expectedResult)
                    ...
                    if launch_result == "SUCCESS":
                        ...
                        # Wait for app launch and then full loading (using lifecycle events)
                        continue_count = 0
                        launched_time = ""
                        loaded_time = ""
                        launch_success = False
                        ...
                        while continue_count < load_timeout:
                            if len(event_listener.getEventsBuffer()) == 0:
                                continue_count += 1
                                time.sleep(1)
                                continue
                            ...
                            event_log = event_listener.getEventsBuffer().pop(0)
                            print(f"\n Load event: {event_log} \n")
                            
                            if app_id in event_log and "onAppLifecycleStateChanged" in str(event_log):
                                event_data = json.loads(event_log.split('$$$')[1])
                                event_time = event_log.split('$$$')[0]
                                
                                if event_data.get("state") == "running" and not launch_success:
                                    print(f"\n Event: App lifecycle changed to running for {app_id} \n")
                                    launched_time = event_time
                                    app_instance_id = event_data.get("instanceId", "")
                                    launch_success = True
                                    results_dict["app_instance_id"] = app_instance_id
                                    # Continue monitoring for fully loaded state
                                elif event_data.get("state") in ["ready", "loaded", "active"] and launch_success:
                                    print(f"\n Event: App fully loaded (state: {event_data.get('state')}) for {app_id} \n")
                                    loaded_time = event_time
                                    load_success = True
                                    break
                                elif event_data.get("state") == "running" and launch_success and continue_count > 30:
                                    # If no explicit "loaded" state after 30 seconds, consider running as loaded
                                    print(f"\n App {app_id} appears to be fully loaded (running state maintained) \n")
                                    loaded_time = event_time
                                    load_success = True
                                    break
                        ...
                        if launched_time and launch_success:
                            ...
                            if loaded_time and load_success:
                                load_start_time_in_millisec = getTimeInMilliSec(load_start_time)
                                loaded_time_in_millisec = getTimeInMilliSec(loaded_time)
                                load_time_taken = loaded_time_in_millisec - load_start_time_in_millisec
                                results_dict["load_time"] = load_time_taken
                                results_dict["load_success"] = True
                                status = "SUCCESS"
                                print(f"\n App loading completed successfully in {load_time_taken}ms \n")
                            else:
                                print(f"\n App loading event not received or load failed for {app_id} \n")
                                tdkTestObj.setResultStatus("FAILURE")
                                status = "FAILURE"
                        else:
                ...
                if status == "SUCCESS" and results_dict["load_success"]:
                    test_app_id = app_id
                    time_taken_for_load = results_dict["load_time"]
                    
                    print(f"\n Launch Time: {results_dict['launch_time']}ms \n")
                    print(f"\n Load Time: {time_taken_for_load}ms \n")
                    
                    Summ_list.append(f'Time taken to launch app: {results_dict["launch_time"]}ms')
                    Summ_list.append(f'Time taken to load app: {time_taken_for_load}ms')
                    ...
                    Summ_list.append('APP_LOAD_THRESHOLD_VALUE: {}ms'.format(app_load_threshold))
                    Summ_list.append('THRESHOLD_OFFSET: {}ms'.format(offset))
                    
                    if all(value != "" for value in (app_load_threshold, offset)):
                        print(f"\n Threshold value for time taken to load app: {app_load_threshold} ms")
                        print("\n Validate the time: \n")
                        
                        if 0 < time_taken_for_load < (int(app_load_threshold) + int(offset)):
                            print(f"\n Time taken for loading {test_app_id} is within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("SUCCESS")
                        else:
                            print(f"\n Time taken for loading {test_app_id} is not within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("\n Please configure the APP_LOAD_THRESHOLD_VALUE in device configuration file \n")
                        tdkTestObj = obj.createTestStep('rdkservice_setValue')
                        tdkTestObj.setResultStatus("FAILURE")
                    
                    # Terminate the loaded app using TDK test step for cleanup
                    if app_instance_id:
                        print(f"\n Terminating app {test_app_id} for cleanup \n")
                        tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    ...
                    print(f"\n Launch/Load operation failed for app {app_id} \n")
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    tdkTestObj.setResultStatus("FAILURE")
