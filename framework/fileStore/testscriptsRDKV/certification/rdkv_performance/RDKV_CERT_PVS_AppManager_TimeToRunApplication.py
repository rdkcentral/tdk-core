...
if expectedResult in result.upper():
    ...
    event_listener = createEventListener(
        ip,
        thunder_port,
        ['{"jsonrpc": "2.0","id": 6,"method": "org.rdk.LifecycleManager.1.register","params": {"event": "onAppLifecycleStateChanged", "id": "client.events.1" }}'],
        "/jsonrpc",
        False
    )
    ...
    # ========================================================
    # CAPTURE RUNNING STATE EVENT
    # ========================================================
    if final_status == "SUCCESS":

        print("\nWaiting for app running state...\n")
        ...
        while True:
            ...
            # look for running/resumed state
            if app_id in str(event) and ("running" in str(event).lower() or "resumed" in str(event).lower()):
                end_time = getCurrentTime()
                break

        if not end_time:
            print("\nRunning state event not captured\n")
            final_status = "FAILURE"
    ...
    if final_status == "SUCCESS":
        ...
        print("\nTime taken for app to run: {} ms".format(run_time))

        Summ_list.append("App run time: {} ms".format(run_time))
        ...
        _, threshold = getDeviceConfigKeyValue(conf_file, "APP_RUN_THRESHOLD_VALUE")
        ...
        if threshold and offset:
            ...
            if 0 < run_time < (int(threshold) + int(offset)):
                print("\nApp run time within threshold\n")
            else:
                print("\nApp run time exceeded threshold\n")
                final_status = "FAILURE"
        else:
