...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_TimeTo_UnInstallApp')
    ...
    if missing_plugins:
        ...
        if status == "SUCCESS":
            ...
            if config_status != "SUCCESS" or not app_download_url:
                    ...
                    try:
                        ...
                        if "result" in installed_apps_data and "apps" in installed_apps_data["result"]:
                            ...
                            if app_id in installed_app_ids:
                                ...
                                print(f"\n App {app_id} is already installed, can proceed with uninstall test \n")
                            else:
                ...
                # Use TDK test steps for download/install (if needed) and then uninstall
                download_timeout = 120  # 2 minutes
                install_timeout = 120   # 2 minutes
                uninstall_timeout = 120 # 2 minutes
                ...
                # Step 1: Download and Install app if not already installed (prerequisite for uninstall test)
                if not app_already_installed:
                    print(f"\n Starting app download for {app_id} (prerequisite for uninstall test)... \n")
                    ...
                    if download_result == "SUCCESS":
                        ...
                        if downloaded_time and download_success:
                            ...
                            # Install the downloaded app (prerequisite for uninstall)
                            print(f"\n Starting app installation for {app_id} (prerequisite for uninstall test)... \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                ...
                # Step 2: Uninstall the app (Main focus of this test)
                if status == "SUCCESS":
                    print(f"\n Starting app uninstallation for {app_id}... \n")
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    ...
                    if uninstall_result == "SUCCESS":
                        print(f"\n Uninstall initiated successfully for {app_id} \n")
                        ...
                        while continue_count < uninstall_timeout:
                            ...
                            if app_id in event_log and "onAppUninstalled" in str(event_log):
                                print(f"\n Event: onAppUninstalled triggered for {app_id} \n")
                                uninstalled_time = event_log.split('$$$')[0]
                                uninstall_success = True
                                break
                            elif app_id in event_log and "onAppLifecycleStateChanged" in str(event_log):
                                # Alternative: Monitor lifecycle for app removal
                                event_data = json.loads(event_log.split('$$$')[1])
                                if event_data.get("state") in ["removed", "uninstalled"]:
                                    print(f"\n Event: App lifecycle changed to {event_data.get('state')} for {app_id} \n")
                                    uninstalled_time = event_log.split('$$$')[0]
                                    uninstall_success = True
                                    break
                                    
                        if uninstalled_time and uninstall_success:
                            uninstall_start_time_in_millisec = getTimeInMilliSec(uninstall_start_time)
                            uninstalled_time_in_millisec = getTimeInMilliSec(uninstalled_time)
                            uninstall_time_taken = uninstalled_time_in_millisec - uninstall_start_time_in_millisec
                            results_dict["uninstall_time"] = uninstall_time_taken
                            results_dict["uninstall_success"] = True
                            status = "SUCCESS"
                            print(f"\n App uninstallation completed successfully in {uninstall_time_taken}ms \n")
                        else:
                            print(f"\n App uninstall event not received or uninstall failed for {app_id} \n")
                            tdkTestObj.setResultStatus("FAILURE")
                            status = "FAILURE"
                    else:
                        print(f"\n Failed to initiate uninstall for {app_id} \n")
                        tdkTestObj.setResultStatus("FAILURE")
                        status = "FAILURE"
                
                if status == "SUCCESS" and results_dict["uninstall_success"]:
                    test_app_id = app_id
                    time_taken_for_uninstall = results_dict["uninstall_time"]
                    
                    print(f"\n Uninstall Time: {time_taken_for_uninstall}ms \n")
                    
                    Summ_list.append(f'Time taken to uninstall app: {time_taken_for_uninstall}ms')
                    
                    # Add download and install times to summary if operations were performed
                    ...
                    config_status, app_uninstall_threshold = getDeviceConfigKeyValue(conf_file, "APP_UNINSTALL_THRESHOLD_VALUE")
                    offset_status, offset = getDeviceConfigKeyValue(conf_file, "THRESHOLD_OFFSET")
                    
                    Summ_list.append('APP_UNINSTALL_THRESHOLD_VALUE: {}ms'.format(app_uninstall_threshold))
                    Summ_list.append('THRESHOLD_OFFSET: {}ms'.format(offset))
                    
                    if all(value != "" for value in (app_uninstall_threshold, offset)):
                        print(f"\n Threshold value for time taken to uninstall app: {app_uninstall_threshold} ms")
                        print("\n Validate the time: \n")
                        
                        if 0 < time_taken_for_uninstall < (int(app_uninstall_threshold) + int(offset)):
                            print(f"\n Time taken for uninstalling {test_app_id} is within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("SUCCESS")
                        else:
                            print(f"\n Time taken for uninstalling {test_app_id} is not within the expected range \n")
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("\n Please configure the APP_UNINSTALL_THRESHOLD_VALUE in device configuration file \n")
                        tdkTestObj = obj.createTestStep('rdkservice_setValue')
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print(f"\n Uninstall operation failed for app {app_id} \n")
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    tdkTestObj.setResultStatus("FAILURE")
