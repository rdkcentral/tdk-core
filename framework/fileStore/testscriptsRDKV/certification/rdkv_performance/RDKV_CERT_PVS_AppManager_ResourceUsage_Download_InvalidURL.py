...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_AppManager_ResourceUsage_Download_InvalidURL');
    ...
    if status == "SUCCESS":
        ...
        if status == expectedResult and ssh_params not in ("", "{}"):
            ...
            if not ssh_method or not credentials:   
                ...
                valid_app_download_url = PerformanceTestVariables.app_download_url.rstrip("/") + "/" + app_bundle_name
                failing_app_download_url = PerformanceTestVariables.invalid_download_url.rstrip("/") + "/" + app_bundle_name
                            ...
                            if "onAppDownloadStatus" in str(event) and "DOWNLOAD_FAILURE" not in str(event):
                                ...
                                if int(event_download_id) == download_id and event_download_id in filelocator_url:
                                    ...
                                    if status == "SUCCESS":
                                        ...
                                        print("Failed to delete the downloaded package from the device")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                                    print(f"Failed to download package with download ID :{download_id}")
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print("Failed to receive the download event")
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
