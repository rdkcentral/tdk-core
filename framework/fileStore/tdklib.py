...
def get_ipv6_address(self):
    ...
    response = urllib.request.urlopen(ipv6_url).read()
...
class PrimitiveTestCase:
    ...
    def __init__(self, name, url, execId, execDeviceId, execResId, ipAddr, realPath, logPath, tcpClient, logTransferPort, testcaseId, deviceId, standAlone=False):
        try:
            ...
            self.jsonMsgValue = urllib.request.urlopen(url).read()
            decoded_jsonMsgValue=self.jsonMsgValue.decode('utf-8')
            self.jsonMsgValue='{"jsonrpc":"2.0","id":"2","method":"executeTestCase","params":'+decoded_jsonMsgValue+'}'
    ...
    # Return Value : None
        ...
        for key in list(paramsJson.keys()):
...
#                self.resultDetails=message
    ...
    # Return Value : client moca ip address
        ...
        try:
            ...
            result = self.tcpClient.recv(1048).decode()
...
#                       self.addTxtEle(envEl,"url",url)
            ...
            try:
                loadstring = urllib.request.urlopen(url).read()
                decoded_data = loadstring.decode('utf8')
                if "null" in decoded_data:
                    self.logTransferPort = logTransferPort
            ...
            if self.isStandAlone is False :
                ...
                self.tcpClient.send(final.encode())
    ...
    # Set the status of load module as success or failure
    ...
    # Return Value: NIL
        ...
        data = setURL.split("&")
        #print setURL
        loadstring = requests.get(setURL)
        return
    ...
    #            time.sleep(5)
                self.tcpClient = self.getSocketInstance()
                self.tcpClient.connect((self.IP, self.portValue))
                json_data = '{"jsonrpc":"2.0","id":"2","method":"unloadModule","params":{"param1":"'+str(cName)+'","version":"1.3","ScriptSuiteEnabled":"false"}}\r\n'
                self.tcpClient.send(json_data.encode())
                unloadmoduleresult = self.tcpClient.recv(1048).decode()
                sys.stdout.flush()
                self.tcpClient.close()
...
def Create_ExecuteTestcase(obj,primitivetest,expectedresult,verifyList, **kwargs):
    ...
    if Expectedresult in actualresult:
        ...
        if verifyList:
            ...
            for name,value in list(verifyList.items()):
