...
class ScriptService {
				...
				dirList.each{ directory ->
					...
					if(scriptsDir.exists()){
						...
						modules.each { module ->
							...
							if((!moduleNameString.contains(Constants.RDKSERVICES)) && (!moduleNameString.contains(Constants.RDKV_DOBBY)) && (!moduleNameString.contains(Constants.RDKV_APPARMOR)) && (!moduleNameString.contains(Constants.RDKV_BASIC_SANITY))){
								def category = null
	...
	def initializeRdkServiceScripts(def realPath){
		try {
			...
			if(realPathDir.exists()){
				...
				List testGroupList = [Constants.COMPONENT, Constants.CERTIFICATION]
				testGroupList.each{testGroup->
					File testGroupDirectory = new File( "${realPath}//fileStore//"+Constants.TESTSCRIPTS_RDKV+"//"+testGroup+"//")
					if(testGroupDirectory.exists()){
						...
						modules.each{folder->
							...
							if(moduleDirectory.exists()){
								if((testGroup == Constants.COMPONENT && folder == Constants.RDKSERVICES) || (testGroup == Constants.CERTIFICATION) || (testGroup == Constants.COMPONENT && folder == Constants.RDKV_DOBBY) || (testGroup == Constants.COMPONENT && folder == Constants.RDKV_APPARMOR) || (testGroup == Constants.COMPONENT && folder == Constants.RDKV_BASIC_SANITY)){
									List sLstEmpty = []
									scriptGroupMap.put(folder,sLstEmpty)
									List sLst = []
									File [] testscriptfiles = moduleDirectory.listFiles(new FilenameFilter() {
										@Override
	...
	/**
	 * Method to initialize thunder Scripts
	 *
	 */
	...
	/***
	 *  Retrieves thunder script from the fileStore
	 */
				 ...
				 for(String testscriptfile : testscriptfiles){
					 if(testscriptfile.contains(".js") ){
					     ...
					     if(fileName.equals(name)){
							 thunderPath = fileStorePath + directory + File.separator + fileName
							 scriptFound = true
							 break
						 }
					 }
	  ...
	  * @return
	  */
	 def getScriptFileListStorm(){
		 if(totalThunderScriptList == null || totalThunderScriptList.size() == 0){
	...
	def getCategoryMap(def realPath, def category){
		...
		if("RDKV".equals(category)){
			...
			directories.each { dirName ->
				...
				["component", "integration", "certification"].each{ directory ->
			...
			if(file.exists()){
				...
				if(moduleObj){
					if(moduleObj?.testGroup?.groupValue.equals(TestGroup.E2E.groupValue)){
						...
						primitiveDirName = Constants.CERTIFICATION
					}
				}
