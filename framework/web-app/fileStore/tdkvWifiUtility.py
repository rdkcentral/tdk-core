...
# Return Value: SUCCESS/FAILURE
    try:
        ...
        device_Config = deviceName + ".config"
        ...
        setup_type = config.get(wifi_config, 'SETUP_TYPE')

        global ssid_2ghz_name
        ssid_2ghz_name = config.get(wifi_config, "SSID_2GHZ_NAME")

        global ssid_2ghz_pwd
        ssid_2ghz_pwd = config.get(wifi_config, "SSID_2GHZ_PWD")

        global ssid_2ghz_invalid_pwd
        ssid_2ghz_invalid_pwd = config.get(wifi_config, "SSID_2GHZ_INVALID_PWD")
        ...
        if setup_type == "TDK":
            ssid_2ghz_index = config.get(wifi_config, "TDK_SSID_2GHZ_INDEX")
            radio_2ghz_index = config.get(wifi_config, "TDK_RADIO_2GHZ_INDEX")
        ...
        ssid_5ghz_pwd = config.get(wifi_config, "SSID_5GHZ_PWD")

        global ssid_5ghz_invalid_pwd
        ssid_5ghz_invalid_pwd = config.get(wifi_config, "SSID_5GHZ_INVALID_PWD")

        global wlan_2ghz_ssid_connect_status
        wlan_2ghz_ssid_connect_status = config.get(wifi_config, "WLAN_2GHZ_SSID_CONNECT_STATUS")

        global wlan_5ghz_ssid_connect_status
        wlan_5ghz_ssid_connect_status = config.get(wifi_config, "WLAN_5GHZ_SSID_CONNECT_STATUS")

        global wlan_2ghz_ssid_disconnect_status
        wlan_2ghz_ssid_disconnect_status = config.get(wifi_config, "WLAN_2GHZ_SSID_DISCONNECT_STATUS")

        global wlan_5ghz_ssid_disconnect_status
        wlan_5ghz_ssid_disconnect_status = config.get(wifi_config, "WLAN_5GHZ_SSID_DISCONNECT_STATUS")

        global ssid_invalid_name
        ssid_invalid_name = config.get(wifi_config, "SSID_INVALID_NAME")
	...
	ap_2ghz_wep_key = config.get(wifi_config, "AP_2GHZ_WEP_KEY")
        ...
        ssid_2ghz_name_new = config.get(wifi_config, "SSID_2GHZ_NAME_NEW")

        global ssid_5ghz_pwd_new
        ssid_5ghz_pwd_new = config.get(wifi_config, "SSID_5GHZ_PWD_NEW")

        global ssid_2ghz_pwd_new
        ssid_2ghz_pwd_new = config.get(wifi_config, "SSID_2GHZ_PWD_NEW")

        global ap_5ghz_preshared_key_new
        ap_5ghz_preshared_key_new = config.get(wifi_config, "AP_5GHZ_PRESHARED_KEY_NEW")

        global ap_2ghz_preshared_key_new
        ap_2ghz_preshared_key_new = config.get(wifi_config, "AP_2GHZ_PRESHARED_KEY_NEW")

        global ap_5ghz_key_passphrase_new
        ap_5ghz_key_passphrase_new = config.get(wifi_config, "AP_5GHZ_KEY_PASSPHRASE_NEW")

        global ap_2ghz_key_passphrase_new
        ap_2ghz_key_passphrase_new = config.get(wifi_config, "AP_2GHZ_KEY_PASSPHRASE_NEW")

        global ap_5ghz_security_mode_new
        ap_5ghz_security_mode_new = config.get(wifi_config, "AP_5GHZ_SECURITY_MODE_NEW")

        global ap_2ghz_security_mode_new
        ap_2ghz_security_mode_new = config.get(wifi_config, "AP_2GHZ_SECURITY_MODE_NEW")

        global ap_5ghz_wep_key_new
        ap_5ghz_wep_key_new = config.get(wifi_config, "AP_5GHZ_WEP_KEY_NEW")
