...
#-----------------------------------------------------------------------------
#PRE-REQUISITE CHECK FOR WEBKIT TESTING
#-----------------------------------------------------------------------------
def webkit_prerequisite(VariableList):
    pre_requsite_status="SUCCESS"
    VariableList = VariableList.split(",")
    #Check all required variable values from WebkitVariables.py
    for var in VariableList:
        value = getattr(WebkitVariables, var, None)
        if value is None or value == "":
            print("ERROR: Please configure ", var , "in WebkitVariables.py file")
...
#GET PLUGIN STATUS
#-----------------------------------------------------------------
def webkit_getPluginStatus(plugin):
...
#----------------------------------------------------------------------------
def webkit_rdkshellStatus():
    activated = False
    rdkshell_status = webkit_getPluginStatus("org.rdk.RDKShell")
    if "activated" == rdkshell_status:
        activated = True
    elif "deactivated" == rdkshell_status:
        set_status = webkit_setPluginStatus("org.rdk.RDKShell","activate")
        time.sleep(2)
        rdkshell_status = webkit_getPluginStatus("org.rdk.RDKShell")
...
#-----------------------------------------------------------------------------
def webkit_setPluginStatus(plugin,status,uri=''):
    data = ''
    rdkshell_activated = webkit_rdkshellStatus()
...
#------------------------------------------------------------------
def webkit_setValue(method,value):
...
#---------------------------------------------------------------------------------
def webkit_create_socket_connection():

    port = WebkitVariables.webinspect_port
    ...
    def open_chrome_browser(webinspect_url):
        ...
        os.environ["PATH"] = WebkitVariables.chromedriver_path
        try:
...
#-----------------------------------------------------------------------------------------
#LAUNCH WEBKIT TEST APP IN BROWSER TO START THE TEST
#-----------------------------------------------------------------------------------------
def webkit_launch_testApp(obj, url,browser):
    result = "FAILURE"
...
#-----------------------------------------------------------------------------------------
def webkit_keypress(obj,browser,keys):
    result="FAILURE"
    param='['
    index=0
    for key in keys:
    ...
    params = '{"keys":'+ param + '}'
    tdkTestObj = obj.createTestStep('webkit_setValue')
...
#-----------------------------------------------------------------------------------------
def webkit_getLogs_webinspectpage(obj, url, browser,keys=[]):
    log_message = []
    continue_count = 0
    ...
    result, driver, webkit_console_socket = webkit_create_socket_connection()
    if expectedResult in result:
        print("SUCCESS: Successfully established websocket connection to webinspect page of the device")
        time.sleep(30)

        print("\n Launch webkit test url in ", browser)
        result = webkit_launch_testApp(obj,url,browser)
        if keys !=[] and expectedResult in result:
            print ("\n Navigate through webkit test UI and start test ")
            result=webkit_keypress(obj,browser,keys)
        if expectedResult in result:
...
#-------------------------------------------------------------------
def webkit_getRequiredLog(ssh_method,credentials,command):
    output = ""
...
#------------------------------------------------------------------------------------
def webkit_getLogs_fromDevicelogs(obj,url,browser,grep_line,keys=[]):
   log_message=""
   print("\n Check for required configurations to ssh to the device") 
   tdkTestObj = obj.createTestStep('webkit_getSSHParams')
   tdkTestObj.executeTestCase(expectedResult)
   result = tdkTestObj.getResult()
   ...
   if ssh_param_dict != {} and expectedResult in result:
       ...
       if expectedResult in result:
           ...
           tdkTestObj = obj.createTestStep('webkit_getRequiredLog')
           ...
           print("FAILURE: Failed to launch webkit test url in the device")
           tdkTestObj.setResultStatus("FAILURE")
   else:
