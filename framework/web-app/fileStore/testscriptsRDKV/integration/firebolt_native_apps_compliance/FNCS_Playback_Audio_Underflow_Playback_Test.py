...
<xml>
  ...
  <name>FNCS_Playback_Audio_Underflow_Playback_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify buffer underflow signal is received upon audio underrun and fill the audio buffer by seeking to audioStart point and verify playback is as expected.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_144</test_case_id>
    <test_objective>Verify buffer underflow signal is received upon audio underrun and fill the audio buffer by seeking to audioStart point and verify playback is as expected.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RDK TV</test_setup>
...
5. FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT configuration should be set to notify the point where audio underrun happens
6. FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_START_POINT configuration should be set to notify the point where audio starts after underrun</pre_requisite>
    <api_or_interface_used>Execute the mediapipelinetests application in DUT</api_or_interface_used>
    <input_parameters>testcasename - "test_audio_underflow_playback"
test_url - underflow stream from MediaValidationVariables library (MediaValidationVariables.video_src_url_audio_underflow)
...
"audioEnd" - time in seconds to notify the point where audio is ended and only audio plays.
"audioStart" - time in seconds to notify the point where audio begins after underrun</input_parameters>
    <automation_approch>1.Initiate SSH connection to execute command remotely.
2.Retrieve the FIREBOLT_COMPLIANCE_CHECK_AV_STATUS, FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT, FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT and FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_START_POINT config values from Device config file.
3.Retrieve the video_src_url_underflow_stream variable from MediaValidationVariables library
4.Construct the mediapipelinetests command based on the retrieved video url, testcasename, FIREBOLT_COMPLIANCE_CHECK_AV_STATUS deviceconfig value, FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT deviceconfig value, FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_START_POINT deviceconfig value and timeout
5.Execute the command in DUT. During the execution, the DUT will playback av for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds and waits for few seconds after FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_START_POINT upon which buffer underflow signal must be received. Pipeline is PAUSED after underrun and then seeked to the audioStart point and verified whetehr playback is as expected.
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>pipeline should ply with normal playback rate  after seeking to audioStart point from audioEnd point where audio underrun has occurred</expected_output>    
    <priority>High</priority>
    <test_stub_interface>tdk_mediapipelinetests</test_stub_interface>
    <test_script>FNCS_Playback_Audio_Underflow_Playback_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Underflow_Playback_Test');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_audio_underflow_playback"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_audio_underflow
    ...
    #Get audioEnd point from config file
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_END_POINT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    audioEnd = tdkTestObj.getResultDetails();


    #Get audioStart point from config file
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_UNDERFLOW_AUDIO_START_POINT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    audioStart = tdkTestObj.getResultDetails();

    #Sample command = "mediapipelinetests test_audio_underflow_playback <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    arguments = {"checkavstatus" : checkAVStatus, "timeout": timeoutInSeconds, "audioEnd": audioEnd, "audioStart": audioStart}
