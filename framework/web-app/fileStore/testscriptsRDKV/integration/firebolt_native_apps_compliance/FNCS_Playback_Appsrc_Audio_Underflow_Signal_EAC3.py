...
<xml>
  ...
  <name>FNCS_Playback_Appsrc_Audio_Underflow_Signal_EAC3</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_253</test_case_id>
...
2. Test stream url for an Appsrc_Audio_Underflow_Signal_AV1 stream should be updated in the config variable audio_src_url_mp4_eac3 inside MediaValidationVariables.py library inside filestore
...
5. FIREBOLT_COMPLIANCE_CHECK_PTS configuration should be set as yes/no in the device config file for validating audio pts
6. FIREBOLT_COMPLIANCE_CHECK_FPS configuration should be set as yes/no in the device config file for validating audio frames</pre_requisite>
    <api_or_interface_used>gstreamer-1.0</api_or_interface_used>
    <input_parameters>testcasename - "test_appsrc_audio_underflow_signal"
test_url - Appsrc_Audio_Underflow_Signal_AV1 url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_mp4_eac3)
"checkavstatus=yes" - argument to do the audio playback verification from SOC side . This argument can be yes/no based on a device configuration(FIREBOLT_COMPLIANCE_CHECK_AV_STATUS) from Device Config file
"checkFPS=yes" argument to get the audio frames. This argument can be yes/no based on a device configuration(FIREBOLT_COMPLIANCE_CHECK_FPS)
...
"checkPTS=yes" - argument to get the audio PTS . This argument can be yes/no based on a device configuration(FIREBOLT_COMPLIANCE_CHECK_PTS)
...
underflow_threshold - this is the amount of bytes pushed via "appsrc" into pipeline to trigger audio underflow</input_parameters>
...
3.Retrieve the audio_src_url_mp4_720p_av1 variable from MediaValidationVariables library
4.Construct the tdk_mediapipelinetests command based on the retrieved audio url, testcasename and configurations
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Appsrc_Audio_Underflow_Signal_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Appsrc_Audio_Underflow_Signal_EAC3');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_appsrc_audio_underflow_signal"
    ...
    if expectedResult in actualresult.upper() and check_av_status_flag == "yes":
        print("Audio Decoder proc check is added")
        checkAVStatus = check_av_status_flag

    #Sample command = "mediapipelinetests test_generic_playback <Appsrc_Audio_Underflow_Signal_AV1_STREAM_URL> checkavstatus=yes timeout=20"
