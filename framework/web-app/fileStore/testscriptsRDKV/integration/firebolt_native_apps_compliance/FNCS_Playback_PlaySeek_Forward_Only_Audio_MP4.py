...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Forward_Only_Audio_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a only audio AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_282</test_case_id>
    <test_objective>Test to do forward seek on a only audio MP4 stream</test_objective>
...
2. Test stream url for an MP4 stream should be updated in the config variable video_src_url_44khz_aac inside MediaValidationVariables.py library inside filestore
...
test_url - MP4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_44khz_aac)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Forward_Only_Audio_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Forward_Only_Audio_MP4');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on adio only MP4 stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek on adio only MP4 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
