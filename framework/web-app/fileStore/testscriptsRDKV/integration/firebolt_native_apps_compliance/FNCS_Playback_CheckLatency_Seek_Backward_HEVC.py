...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_Seek_Backward_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a HEVC stream and verify latency </synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_222</test_case_id>
    <test_objective>Test to do backward seek on a HEVC stream and verify latency</test_objective>
...
2. Test stream url for an HEVC stream should be updated in the config variable video_src_url_hevc inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_Seek_Backward_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_Seek_Backward_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on HEVC stream successfull")
            ...
            print("Backward seek on HEVC stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
