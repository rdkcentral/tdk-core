...
<xml>
  ...
  <name>FNCS_Playback_ResolutionSwitch_up_VP9</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_270</test_case_id>
    <test_objective>To test the video playback by switching resolutions from 144p resolution to very high resolution (4K) through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a resolution switch up stream should be updated in the config variable video_src_url_resolution_up_vp9 inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_resolution_up"
test_url - special vp9 mp4 url with multiple resolutions from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_up_vp9)
...
3.Retrieve the video_src_url_resolution_up_vp9 variable from MediaValidationVariables library
...
6.Pipeline created using 'playbin' and 'westerossink' will play the video set, 'westerossink' configures itself to play at 144p initially.
7.Upon reaching 20 seconds point, video will switch from 144p to 240p. 'westerossink' must configure itself automatically to render video at 240p.
8.Subsequently after 10seconds points, resolution will be upgraded in this sequence 144p -> 240p -> 360p -> 480p -> 720p -> 1080p -> 2160p.
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_ResolutionSwitch_up_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_ResolutionSwitch_up_VP9');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_resolution_up"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_resolution_up_vp9
    ...
    #Sample command = "mediapipelinetests test_resolution_up <VP9_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Playback using 'playbin' and 'westeros-sink' for resolution switch up is successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Playback using 'playbin' and 'westeros-sink' for resolution switch up failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
