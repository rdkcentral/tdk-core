...
<xml>
  ...
  <name>FNCS_Playback_Bitrate_Switch_Up_H264</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_271</test_case_id>
...
2. Test stream url for an H264 stream should be updated in the config variable video_src_url_dash inside MediaValidationVariables.py library inside filestore
...
test_url - h264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Bitrate_Switch_Up_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Bitrate_Switch_Up_H264');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Bitrate Switch UP video playback using H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Bitrate Switch UP video playback using H264 stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
