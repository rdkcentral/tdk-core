...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Backward_Only_Audio_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a audio only MP4 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_286</test_case_id>
    <test_objective>Test to do backward seek on a audio only MP4 stream</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RPI</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests application must be installed in DUT
2. Test stream url for an audio only MP4 stream should be updated in the config variable video_src_url_44khz_aac inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Backward_Only_Audio_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Backward_Only_Audio_MP4');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <MP4_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on only audio MP4 stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward seek on only audio MP4 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
