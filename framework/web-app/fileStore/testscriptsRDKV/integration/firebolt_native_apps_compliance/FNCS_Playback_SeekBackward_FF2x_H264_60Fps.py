...
<xml>
  ...
  <name>FNCS_Playback_SeekBackward_FF2x_H264_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek operation followed by fast forward of a 60Fps H264 stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_213</test_case_id>
    <test_objective>Test to do backward seek operation followed by fast forward of a 60Fps H264 stream with playback rate 2</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_SeekBackward_FF2x_H264_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_SeekBackward_FF2x_H264_60Fps');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward Seek followed by Fastforward on 60Fps H264 stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward Seek followed by Fastforward on 60Fps H264 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
