...
<xml>
  ...
  <version>1</version>
  <!-- Do not edit version. This will be auto incremented while downdating. If you are adding a new script you can keep the vresion as 1 -->
  <name>FNCS_Playback_ResolutionSwitch_down_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback by switching resolutions from 4K resolution to very low resolution (144p) through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <grodowns_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_269</test_case_id>
    <test_objective>To test the video playback by switching resolutions from 4K resolution to very low resolution (144p) through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setdown>RDK TV,Video Accelerator</test_setdown>
    <pre_requisite>1.TDK Agent should be down and running in the DUT
2. Test stream url for a resolution switch down stream should be downdated in the config variable video_src_url_resolution_down_vp9 inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_resolution_down"
test_url - special vp9 mp4 url with multiple resolutions from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_down_vp9)
...
3.Retrieve the video_src_url_resolution_down_vp9 variable from MediaValidationVariables library
...
6.Pipeline created using 'playbin' and 'westerossink' will play the video set, 'westerossink' configures itself to play at 2160p initially.
7.Upon reaching 20 seconds point, video will switch from 2160p to 1080p. 'westerossink' must configure itself automatically to render video at 1080p.
8.Subsequently after 10seconds points, resolution will be downgraded in this sequence 2160p -> 1080p -> 720p -> 480p -> 360p -> 240p -> 144p.
9.'westerossink' must configure itself to respective resolutions and play with out any pipeline errors.
10.Verify the output from the execute command and check if the strings "Failures: 0" and "Errors: 0", or "failed: 0" exists in the returned output.
11.Based on the ExecuteCommand() return value and the output returned from the tdk_mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_ResolutionSwitch_down_VP9</test_script>
    <skipped>No</skipped>
    <release_version>M124</release_version>
    <remarks></remarks>
  </test_cases>
</xml>
'''
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_ResolutionSwitch_down_VP9');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_resolution_down"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_resolution_down_vp9
    ...
    #Sample command = "mediapipelinetests test_resolution_down <VP9_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Playback using 'playbin' and 'westeros-sink' for resolution switch down is successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Playback using 'playbin' and 'westeros-sink' for resolution switch down failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
