...
<xml>
  ...
  <name>FNCS_Playback_480p_Resolution_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback with resolution set to 480p through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_109</test_case_id>
    <test_objective>To test the video playback with resolution set to 480p through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an 480p stream should be updated in the config variable video_src_url_mp4_480p inside MediaValidationVariables.py library inside filestore
...
test_url - 480p h264 mp4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_480p)
...
5.Retrieve the video_src_url_mp4_480p variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_480p_Resolution_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_480p_Resolution_H264');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_480p
