...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_Seek_Forward_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek operation followed by fast forward of a HEVC stream with playback rate 2 and verify latency of the operation</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_220</test_case_id>
    <test_objective>Test to do forward seek operation followed by fast forward of a HEVC stream with playback rate 2 and verify the latency of the operation</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_CheckLatency_Seek_Forward_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_Seek_Forward_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_TRICKPLAY_LATENCY_THRESHOLD")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    latencyThresholdValueConfigValue = tdkTestObj.getResultDetails();
    ...
    #Sample command = "mediapipelinetests test_trickplay <HEVC_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward Seek followed by Fastforward on HEVC stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")

            tdkTestObj = obj.createTestStep('parseLatency')
            tdkTestObj.addParameter("latencyThreshold",latencyThresholdValue)
            tdkTestObj.addParameter("output",output)
            tdkTestObj.executeTestCase(expectedResult);
            ...
            print("Forward Seek followed by Fastforward on HEVC stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
