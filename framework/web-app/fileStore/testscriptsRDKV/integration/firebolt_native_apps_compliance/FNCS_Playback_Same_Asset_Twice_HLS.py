...
<xml>
  ...
  <name>FNCS_Playback_Same_Asset_Twice_HLS</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_14</test_case_id>
...
2. Test stream url for an HLS stream should be updated in the config variable video_src_url_hls inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Same_Asset_Twice_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Same_Asset_Twice_HLS');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print("HLS Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
