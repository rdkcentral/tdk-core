...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_Rewind_2x_AC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do rewind of a AC3 stream with playback rate -2 and verify latency</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_224</test_case_id>
    <test_objective>Test to do rewind of a AC3 stream with playback rate -2 and verify latency</test_objective>
...
2. Test stream url for a AC3 stream should be updated in the config variable video_src_url_ac3 inside MediaValidationVariables.py library inside filestore
...
test_url - AC3 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ac3)
...
3.Retrieve the video_src_url_ac3 variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will playback av for 2*FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT+30 seconds so that there is enough duration for rewind, then do a rewind operation with playback rate -2 and then continue playback  at rate -2 for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds. Then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_Rewind_2x_AC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_Rewind_2x_AC3');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_ac3
    ...
    # There should be enough duration available for rewind operation to perform correctly, so seeking the stream for 2(since we are doing rewind with 2x speed) * timeoutInSeconds + 30(buffer) duration. Ensure that the straem being tested is having adequate duration
    ...
    rewind_arguments = "%s,%s" % (timeoutInSeconds, str (playduration))
    ...
    tdkTestObj.addParameter("arguments",str(rewind_arguments))
    tdkTestObj.addParameter("operation","rewind2x")
    ...
    #Sample command = "mediapipelinetests test_trickplay <AC3_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Rewind on AC3 stream with 2x speed was successfull")
            ...
            print("Rewind on AC3 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
