...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_MP4</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_91</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for MP4 stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an MP4 stream should be updated in the config variable video_src_url_mp4 inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_MP4');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <MP4_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("MP4 Play_pause using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("MP4 Play_pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
