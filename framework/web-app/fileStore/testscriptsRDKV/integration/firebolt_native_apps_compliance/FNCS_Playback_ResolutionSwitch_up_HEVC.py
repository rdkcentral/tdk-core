...
<xml>
  ...
  <name>FNCS_Playback_ResolutionSwitch_up_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_115</test_case_id>
...
2. Test stream url for an 1080p stream should be updated in the config variable video_src_url_resolution_up_hevc inside MediaValidationVariables.py library inside filestore
...
test_url - 1080p hevc mp4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_up_hevc)
...
5.Retrieve the video_src_url_resolution_up_hevc variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_ResolutionSwitch_up_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_ResolutionSwitch_up_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_resolution_up <HEVC_STREAM_URL> checkavstatus=yes timeout=20"
