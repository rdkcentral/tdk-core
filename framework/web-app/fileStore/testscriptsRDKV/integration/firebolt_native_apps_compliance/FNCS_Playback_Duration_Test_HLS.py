...
<xml>
  ...
  <name>FNCS_Playback_Duration_Test_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do retrieve playback duration of a HLS stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_193</test_case_id>
    <test_objective>Test to do retrieve playback duration of a HLS stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an Duration_Test_HLS stream should be updated in the config variable video_src_url_hls inside MediaValidationVariables.py library inside filestore
...
test_url - Duration_Test_HLS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hls)
...
Checkpoint 3. Verify that duration is retrieved as expected</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Duration_Test_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Duration_Test_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_duration <Duration_Test_HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Duration_Test_HLS Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
