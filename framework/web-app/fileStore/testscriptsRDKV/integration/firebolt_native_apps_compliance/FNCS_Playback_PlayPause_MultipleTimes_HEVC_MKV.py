...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_MultipleTimes_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple short duration play, pause operations on a HEVC_MKV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_358</test_case_id>
    <test_objective>Test to do multiple short duration play, pause operations on a HEVC_MKV stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PlayPause_MultipleTimes_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_MultipleTimes_HEVC_MKV');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_STRESS_REPEAT_COUNT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    ...
    #Sample command = "mediapipelinetests test_trickplay <HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Multiple play,pause operations on HEVC_MKV stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Multiple play,pause operations on HEVC_MKV stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
