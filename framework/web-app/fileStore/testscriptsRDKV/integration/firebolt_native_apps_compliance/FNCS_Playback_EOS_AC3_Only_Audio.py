...
<xml>
  ...
  <name>FNCS_Playback_EOS_AC3_Only_Audio</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the End Of Stream detection scenario for a Only Audio AC3 stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_314</test_case_id>
    <test_objective>To test the End Of Stream detection scenario for a Only Audio AC3 stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an Only Audio AC3 stream should be updated in the config variable audio_src_url_short_duration_ac3 inside MediaValidationVariables.py library inside filestore
...
test_url - Only Audio AC3 url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_short_duration_ac3)
...
3.Retrieve the audio_src_url_short_duration_ac3 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_EOS_AC3_Only_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_EOS_AC3_Only_Audio');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.audio_src_url_short_duration_ac3
    ...
    #Sample command = "mediapipelinetests test_EOS <Only Audio AC3_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("EOS Detection for Only Audio AC3 stream using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("EOS Detection for Only Audio AC3 stream using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
