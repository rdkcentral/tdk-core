...
<xml>
  ...
  <name>FNCS_Playback_FastForward_2x_Only_Audio_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do fast forward of a only audio AAC stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_275</test_case_id>
    <test_objective>Test to do fast forward of a only audio AAC stream with playback rate 2</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_FastForward_2x_Only_Audio_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_FastForward_2x_Only_Audio_AAC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Fast forward 2x operation was successfull using only_audio AAC stream")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Fast forward 2x operation was failed using only_audio AAC stream")
    else:
        tdkTestObj.setResultStatus("FAILURE")
