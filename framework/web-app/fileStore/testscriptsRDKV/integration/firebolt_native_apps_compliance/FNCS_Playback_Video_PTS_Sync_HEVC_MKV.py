...
<xml>
  ...
  <name>FNCS_Playback_Video_PTS_Sync_HEVC_MKV_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the video pts of HEVC_MKV using 'last-sample' property is allinged with the video playback</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_348</test_case_id>
    <test_objective>To verify the video pts of HEVC_MKV using 'last-sample' property is allinged with the video playback</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests and TDK.env should be installed in the DUT
2. Test stream url for an HEVC_MKV stream should be updated in the config variable video_src_url_hevc_mkv_30sec inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_video_PTS_sync"
test_url - HEVC_MKV url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_mkv_30sec)
...
3.Retrieve the video_src_url_hevc_mkv_30sec variable from MediaValidationVariables library
...
6.Verify the Video is playing untill it reaches the EOS, get the video PTS with "last-sample" property of 'westeros-sink' element
7.Verifying the video PTS value is alligned with the Video Playback poistion
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Video_PTS_Sync_HEVC_MKV_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Video_PTS_Sync_HEVC_MKV_MKV');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_video_PTS_sync"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_hevc_mkv_30sec
    ...
    #Sample command = "tdk_mediapipelinetests test_video_PTS_sync <HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Video PTS Sync validation with HEVC_MKV using 'last-sample' was Sucessfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Video PTS Sync validation with HEVC_MKV using 'last-sample' was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
