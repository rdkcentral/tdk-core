...
<xml>
  ...
  <name>FNCS_Playback_1080p_Resolution_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback with resolution set to 1080p through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_106</test_case_id>
    <test_objective>To test the video playback with resolution set to 1080p through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.Test stream url for an 1080p stream should be updated in the config variable video_src_url_mp4_1080p_av1 inside MediaValidationVariables.py library inside filestore
...
test_url - 1080p av1 mp4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_1080p_av1)
...
2.Retrieve the video_src_url_mp4_1080p_av1 variable from MediaValidationVariables library
3. Construct the mediapipelinetests command based on the retrieved video url, testcasename, FIREBOLT_COMPLIANCE_CHECK_AV_STATUS deviceconfig value and timeout
4.Execute the command in DUT. During the execution, the DUT will playback av for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds then application exits by closing the pipeline
...
6.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_1080p_Resolution_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_1080p_Resolution_AV1');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_1080p_av1
    ...
    #Sample command = "mediapipelinetests test_resolution <AV1_STREAM_URL> checkavstatus=yes timeout=20"
