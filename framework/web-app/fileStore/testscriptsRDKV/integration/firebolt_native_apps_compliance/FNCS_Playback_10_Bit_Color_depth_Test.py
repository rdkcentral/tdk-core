...
<xml>
  ...
  <name>FNCS_Playback_10_Bit_Color_depth_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether the video is playing with 10 color bit depth</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_132</test_case_id>
    <test_objective>To verify whether the video is playing with 10 color bit depth</test_objective>
...
2. Test stream url for an HDR stream should be updated in the config variable video_src_url_mp4_hevc_hdr inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_color_depth"
test_url - HDR url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_hevc_hdr)
...
3.Retrieve the video_src_url_mp4_hevc_hdr variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_10_Bit_Color_depth_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_10_Bit_Color_depth_Test');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_color_depth <HDR_STREAM_URL> checkavstatus=yes timeout=20"
    arguments = {"checkavstatus" : checkAVStatus, "timeout": timeoutInSeconds, "bit_depth": bit_depth}
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("10 Bit color depth video playback was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("10 Bit color depth video playback was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
