...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_Vorbis</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for Vorbis stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_89</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for Vorbis stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an Vorbis stream should be updated in the config variable video_src_url_ogg inside MediaValidationVariables.py library inside filestore
...
test_url - Vorbis url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ogg)
...
3.Retrieve the video_src_url_ogg variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_Vorbis</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_Vorbis');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_ogg
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <Vorbis_STREAM_URL> checkavstatus=yes timeout=20"
