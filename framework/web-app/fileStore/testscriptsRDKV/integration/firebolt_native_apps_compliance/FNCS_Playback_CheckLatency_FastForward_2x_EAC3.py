...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_FastForward_2x_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if latency observed during EAC3 trickplay is within the threshold limit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_218</test_case_id>
    <test_objective>Check if latency observed during EAC3 trickplay is within the threshold limit</test_objective>
...
2. Test stream url for an EAC3 stream should be updated in the config variable video_src_url_ec3 inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_FastForward_2x_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_FastForward_2x_EAC3');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("EAC3 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
