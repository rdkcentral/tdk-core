...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of AAC stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_03</test_case_id>
    <test_objective>To test the video playback of AAC stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_AAC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("AAC Playback using 'playbin' and 'westeros-sink' was successfull")
