...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Backward_Only_Audio_WAV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a audio only WAV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_284</test_case_id>
    <test_objective>Test to do backward seek on a audio only WAV stream</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RPI</test_setup>
    <pre_requisite>1.tdk_mediapipelinetests application must be installed in DUT
2. Test stream url for an audio only WAV stream should be updated in the config variable audio_src_url_wav_pcm inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Backward_Only_Audio_WAV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Backward_Only_Audio_WAV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <WAV_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on only audio WAV stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward seek on only audio WAV stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
