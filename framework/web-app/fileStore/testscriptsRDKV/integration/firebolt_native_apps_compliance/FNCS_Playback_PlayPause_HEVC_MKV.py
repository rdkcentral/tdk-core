...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for HEVC_MKV stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_351</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for HEVC_MKV stream through playbin gst element and westerossink</test_objective>
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <HEVC_MKV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HEVC_MKV Play_pause using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HEVC_MKV Play_pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
