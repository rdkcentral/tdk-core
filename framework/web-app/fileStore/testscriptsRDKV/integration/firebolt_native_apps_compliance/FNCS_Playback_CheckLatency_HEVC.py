...
<xml>
  ...
  <name>FNCS_Playback_CheckLatency_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if latency observed during HEVC playback is within the threshold limit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_205</test_case_id>
    <test_objective>Check if latency observed during HEVC playback is within the threshold limit</test_objective>
...
2. Test stream url for an CheckLatency_HEVC stream should be updated in the config variable video_src_url_hevc inside MediaValidationVariables.py library inside filestore
...
test_url - CheckLatency_HEVC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc)
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_CheckLatency_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_CheckLatency_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_latency <CheckLatency_HEVC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("CheckLatency_HEVC Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
