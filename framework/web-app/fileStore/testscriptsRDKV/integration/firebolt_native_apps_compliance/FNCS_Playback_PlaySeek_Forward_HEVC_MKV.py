...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Forward_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a HEVC_MKV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_361</test_case_id>
    <test_objective>Test to do forward seek on a HEVC_MKV stream</test_objective>
...
2. Test stream url for an HEVC_MKV stream should be updated in the config variable video_src_url_hevc_mkv inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Forward_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Forward_HEVC_MKV');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on HEVC_MKV stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek on HEVC_MKV stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
