...
<xml>
  ...
  <name>FNCS_Playback_PlaySeek_Forward_Only_Audio_MP3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a only audio MP3 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_283</test_case_id>
    <test_objective>Test to do forward seek on a only audio MP3 stream</test_objective>
...
2. Test stream url for an MP3 stream should be updated in the config variable audio_src_url_mp3 inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlaySeek_Forward_Only_Audio_MP3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlaySeek_Forward_Only_Audio_MP3');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <MP3_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on MP3 stream successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek on MP3 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
