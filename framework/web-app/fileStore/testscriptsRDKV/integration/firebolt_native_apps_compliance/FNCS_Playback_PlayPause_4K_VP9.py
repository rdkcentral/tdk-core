...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_4K_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 4K_VP9 stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_291</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for 4K_VP9 stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an 4K_VP9 stream should be updated in the config variable video_src_url_webm_4k_vp9 inside MediaValidationVariables.py library inside filestore
...
test_url - 4K_VP9 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_webm_4k_vp9)
...
timeout - a string to specify the time in seconds for which the video playback should be done . This argument is the value of device configuration(FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT) from Device Config file</input_parameters>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_4K_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_4K_VP9');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <4K_VP9_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("4K_VP9 Play_pause using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("4K_VP9 Play_pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
