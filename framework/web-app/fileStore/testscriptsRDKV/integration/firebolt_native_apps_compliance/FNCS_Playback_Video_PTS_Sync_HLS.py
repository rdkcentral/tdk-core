...
<xml>
  ...
  <name>FNCS_Playback_Video_PTS_Sync_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the video pts of HLS using 'last-sample' property is allinged with the video playback</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_347</test_case_id>
    <test_objective>To verify the video pts of HLS using 'last-sample' property is allinged with the video playback</test_objective>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Video_PTS_Sync_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Video_PTS_Sync_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "tdk_mediapipelinetests test_video_PTS_sync <HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Video PTS Sync validation with HLS using 'last-sample' was Sucessfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Video PTS Sync validation with HLS using 'last-sample' was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
