...
<xml>
  ...
  <name>FNCS_Playback_Audio_Low_Volume_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether audio is playing in given volume using EAC3 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_136</test_case_id>
    <test_objective>to verify whether audio is playing in given volume using EAC3 stream</test_objective>
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_audio_volume"
test_url - ec3 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ec3)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Low_Volume_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Low_Volume_EAC3');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_volume <EAC3_STREAM_URL> checkavstatus=yes timeout=20"
    arguments = {"checkavstatus" : checkAVStatus, "timeout": timeoutInSeconds, "volume_set": volume_set}
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Audio playback was successfull with given volume")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Audio playback was failed with given volume")
    else:
        tdkTestObj.setResultStatus("FAILURE")
