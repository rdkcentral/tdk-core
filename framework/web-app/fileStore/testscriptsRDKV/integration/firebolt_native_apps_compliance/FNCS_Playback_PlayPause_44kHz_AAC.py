...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_44kHz_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether 44Khz audio is being rendered properly using AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_167</test_case_id>
    <test_objective>to verify whether 44Khz audio is being rendered properly using AAC stream</test_objective>
...
2. Test stream url for an AAC stream should be updated in the config variable video_src_url_aac inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_PlayPause_44kHz_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_44kHz_AAC');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("44kHz sampling rate audio playback using AAC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("44kHz sampling rate audio playback using AAC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
