...
<xml>
  ...
  <name>FNCS_Playback_Only_Audio_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the pipeline is playing only audio using AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_156</test_case_id>
    <test_objective>To test the pipeline is playing only audio using AAC stream</test_objective>
...
2. Test stream url for an AAC stream should be updated in the config variable video_src_url_44khz_aac inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_only_audio"
test_url - AAC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_44khz_aac)
...
3.Retrieve the video_src_url_44khz_aac variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Only_Audio_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Only_Audio_AAC');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_only_audio <AAC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pipeline playing only audio was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Pipeline played only audio was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
