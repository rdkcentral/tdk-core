...
<xml>
  ...
  <name>FNCS_Playback_FrameDrop_Test_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of FrameDrop_Test AV1 stream through videotestsrc gst element</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_337</test_case_id>
    <test_objective>To test the video playback of FrameDrop_Test AV1 stream through videotestsrc gst element</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_FrameDrop_Test_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_FrameDrop_Test_AV1');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("AV1 FrameDrop_Test Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("AV1 FrameDrop_Test Playback using 'playbin' and 'westeros-sink' was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
