...
<xml>
  ...
  <name>FNCS_Playback_Appsrc_Video_Underflow_Playback_4K_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify playback after pushing additionial video buffers from "appsrc" once underflow state is reached  through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_288</test_case_id>
    <test_objective>To verify playback after pushing additionial video buffers from "appsrc" once underflow state is reached  through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an Appsrc_Video_Underflow_Playback_VP9 stream should be updated in the config variable video_src_url_webm_4k_vp9 inside MediaValidationVariables.py library inside filestore
...
6. FIREBOLT_COMPLIANCE_CHECK_FPS configuration should be set as yes/no in the device config file for validating video frames</pre_requisite>
    ...
    <input_parameters>testcasename - "test_appsrc_video_underflow"
test_url - Appsrc_Video_Underflow_Playback_4k_VP9 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_webm_4k_vp9)
...
3.Retrieve the video_src_url_webm_4k_vp9 variable from MediaValidationVariables library
...
7.An underflow signal will be triggered upon reaching the end of the buffers sent to appsrc. Verify that underflow signal is captured by westerossink.
8.Pause the pipeline once underflow signal is captured and push additionial buffers into the pipeline via "appsrc" and set the pipeline to PLAYING state.
9.Verify is playback is happening from expected position after transitioning from PAUSED to PLAYING state.
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Appsrc_Video_Underflow_Playback_4K_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Appsrc_Video_Underflow_Playback_4K_VP9');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_appsrc_video_underflow"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_webm_4k_vp9
    ...
    #Sample command = "tdk_mediapipelinetests test_appsrc_video_underflow <Appsrc_Video_Underflow_Playback_VP9_STREAM_URL> checkavstatus=yes timeout=20"
