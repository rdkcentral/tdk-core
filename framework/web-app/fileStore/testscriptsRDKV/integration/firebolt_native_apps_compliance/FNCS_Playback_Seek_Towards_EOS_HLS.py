...
<xml>
  ...
  <name>FNCS_Playback_Seek_Towards_EOS_HLS</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_263</test_case_id>
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Seek_Towards_EOS_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Seek_Towards_EOS_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_seek_EOS <HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Seek operation towards EOS using HLS stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Seek operation towards EOS using HLS stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
