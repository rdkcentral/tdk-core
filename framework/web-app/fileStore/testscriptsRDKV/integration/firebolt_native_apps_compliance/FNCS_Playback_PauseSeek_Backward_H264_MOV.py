...
<xml>
  ...
  <name>FNCS_Playback_PauseSeek_Backward_H264_MOV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek with pause on a stream with H264_MOV video codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_370</test_case_id>
    <test_objective>Test to do backward seek with pause on a stream with H264_MOV video codec</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PauseSeek_Backward_H264_MOV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PauseSeek_Backward_H264_MOV');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pause with Backward seek on H264_MOV stream was successfull")
