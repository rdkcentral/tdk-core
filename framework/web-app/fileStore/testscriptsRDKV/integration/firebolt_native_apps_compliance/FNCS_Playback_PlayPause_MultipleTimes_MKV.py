...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_MultipleTimes_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple short duration play, pause operations on a MKV stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_321</test_case_id>
    <test_objective>Test to do multiple short duration play, pause operations on a MKV stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PlayPause_MultipleTimes_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_MultipleTimes_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <MKV_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Multiple play,pause operations on MKV stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Multiple play,pause operations on MKV stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
