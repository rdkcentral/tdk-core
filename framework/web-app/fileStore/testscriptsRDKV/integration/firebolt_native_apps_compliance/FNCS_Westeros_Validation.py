...
<xml>
  ...
  <name>FNCS_Westeros_Validation</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to execute the Westeros_TDKTestApp test app  for EGL and westeros validations and verify the results</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <rdk_versions />
  <test_cases>
    <test_case_id>FNCS_GRAPHICS_03</test_case_id>
    ...
    <pre_requisite>Westeros_TDKTestApp test binary should be available in the device</pre_requisite>
    <api_or_interface_used>Execute the Westeros_TDKTestApp application in DUT</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1.Execute the RunWesterosTest script which executes "Westeros_TDKTestApp" command along with "display" (display can be set to any custom name and it is set to "wayland-0" in the test) and "timeout" (timeout option for 20 seconds) in DUT.
...
Checkpoint 2. Verify that the output returned from Westeros_TDKTestApp "error" statements.</expected_output>
    ...
    <test_stub_interface>Westeros_TDKTestApp</test_stub_interface>
    <test_script>FNCS_Westeros_Validation</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Westeros_Validation');
...
if "SUCCESS" in result.upper():
    ...
    if not output:
        ...
        tdkTestObj.addParameter("test_app","Westeros_TDKTestApp")
        tdkTestObj.executeTestCase(expectedResult);
        output = tdkTestObj.getResultDetails()
