...
<xml>
  ...
  <name>FNCS_Playback_PauseSeek_Forward_H264</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_162</test_case_id>
    <test_objective>Test to do forward seek with pause on a stream with H264 video codec</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PauseSeek_Forward_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PauseSeek_Forward_H264');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Pause with Forward seek on H264 stream was successfull")
