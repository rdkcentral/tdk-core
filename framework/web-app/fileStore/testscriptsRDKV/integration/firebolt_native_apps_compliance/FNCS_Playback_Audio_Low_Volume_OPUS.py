...
<xml>
  ...
  <name>FNCS_Playback_Audio_Low_Volume_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether audio is playing in given volume using OPUS stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_135</test_case_id>
    <test_objective>to verify whether audio is playing in given volume using OPUS stream</test_objective>
...
2. Test stream url for an OPUS stream should be updated in the config variable video_src_url_opus inside MediaValidationVariables.py library inside filestore
...
test_url - opus url from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Audio_Low_Volume_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Audio_Low_Volume_OPUS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_volume <OPUS_STREAM_URL> checkavstatus=yes timeout=20"
