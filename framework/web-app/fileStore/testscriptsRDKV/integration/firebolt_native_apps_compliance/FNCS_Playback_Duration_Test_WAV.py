...
<xml>
  ...
  <name>FNCS_Playback_Duration_Test_WAV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do retrieve playback duration of a WAV stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_228</test_case_id>
    <test_objective>Test to do retrieve playback duration of a WAV stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an Duration_Test_WAV stream should be updated in the config variable audio_src_url_wav_pcm inside MediaValidationVariables.py library inside filestore
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_audio_duration"
test_url - Duration_Test_WAV url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_wav_pcm)
...
3.Retrieve the audio_src_url_wav_pcm variable from MediaValidationVariables library
...
Checkpoint 3. Verify that duration is retrieved as expected</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Duration_Test_WAV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Duration_Test_WAV');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.audio_src_url_wav_pcm
    ...
    #Sample command = "mediapipelinetests test_audio_duration <Duration_Test_WAV_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Duration_Test_WAV Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
