...
<xml>
  ...
  <name>FNCS_Playback_Duration_Test_MP3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do retrieve playback duration of a MP3 stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_215</test_case_id>
    <test_objective>Test to do retrieve playback duration of a MP3 stream through playbin gst element and westerossink</test_objective>
...
2. Test stream url for an Duration_Test_MP3 stream should be updated in the config variable audio_src_url_mp3 inside MediaValidationVariables.py library inside filestore
...
test_url - Duration_Test_MP3 url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_mp3)
...
Checkpoint 3. Verify that duration is retrieved as expected</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Duration_Test_MP3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Duration_Test_MP3');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_duration <Duration_Test_MP3_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Duration_Test_MP3 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
