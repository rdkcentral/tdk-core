...
<xml>
  ...
  <name>FNCS_Playback_Appsrc_Audio_Underflow_Playback_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify playback after pushing additionial audio buffers from "appsrc" once underflow state is reached  through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_255</test_case_id>
    <test_objective>To verify playback after pushing additionial audio buffers from "appsrc" once underflow state is reached  through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an Appsrc_Video_Underflow_Playback_AV1 stream should be updated in the config variable audio_src_url_mp4_eac3 inside MediaValidationVariables.py library inside filestore
...
6. FIREBOLT_COMPLIANCE_CHECK_FPS configuration should be set as yes/no in the device config file for validating video frames</pre_requisite>
    ...
    <input_parameters>testcasename - "test_appsrc_audio_underflow"
test_url - Appsrc_Video_Underflow_Playback_AV1 url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_mp4_eac3)
...
3.Retrieve the audio_src_url_mp4_eac3 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from tdk_mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Appsrc_Audio_Underflow_Playback_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Appsrc_Audio_Underflow_Playback_EAC3');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_appsrc_audio_underflow"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.audio_src_url_mp4_eac3
