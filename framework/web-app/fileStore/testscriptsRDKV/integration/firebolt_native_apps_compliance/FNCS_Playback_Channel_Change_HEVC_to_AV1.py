...
<xml>
  ...
  <name>FNCS_Playback_Channel_Change_HEVC_to_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from a HEVC stream to AV1 stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_201</test_case_id>
    <test_objective>Test to do Channel Change from a HEVC stream to AV1 stream through playbin gst element and westerossink</test_objective>
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>FNCS_PLAYBACK_Channel_Change_HEVC_to_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Channel_Change_HEVC_to_AV1');
...
if "SUCCESS" in result.upper():
    ...
    test_url = test_url + " " + MediaValidationVariables.video_src_url_av1
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <Channel_Change_HEVC_to_AV1_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel_Change_HEVC_to_AV1 Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel_Change_HEVC_to_AV1 Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
