...
<xml>
  ...
  <name>FNCS_Playback_4K_Resolution_MKV</name>
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_297</test_case_id>
...
2. Test stream url for an 2160p stream should be updated in the config variable video_src_url_4k_av1_mkv inside MediaValidationVariables.py library inside filestore
...
test_url - 2160p mkv mp4 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_av1_mkv)
...
5.Retrieve the video_src_url_4k_av1_mkv variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_4K_Resolution_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_4K_Resolution_MKV');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_resolution <4K_MKV_STREAM_URL> checkavstatus=yes timeout=20"
