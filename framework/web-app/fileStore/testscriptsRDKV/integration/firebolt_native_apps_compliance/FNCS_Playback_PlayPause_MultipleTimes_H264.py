...
<xml>
  ...
  <name>FNCS_Playback_PlayPause_MultipleTimes_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple short duration play, pause operations on a H264 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_181</test_case_id>
    <test_objective>Test to do multiple short duration play, pause operations on a H264 stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>    
    ...
    <test_script>FNCS_Playback_PlayPause_MultipleTimes_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_PlayPause_MultipleTimes_H264');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Multiple play,pause operations on H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Multiple play,pause operations on H264 stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
