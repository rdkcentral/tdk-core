...
<xml>
  ...
  <name>FNCS_Playback_Low_Bitrate_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether the video is playing with Low bitrate using HEVC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_155</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Low_Bitrate_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Low_Bitrate_HEVC');

#Set device configurations to default values
checkAVStatus = "no"
timeoutInSeconds = "10"
connection_speed = "550"
height = "288"
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Low bitrate video playback using HEVC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Low bitrate video playback using HEVC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
