...
<xml>
  ...
  <name>FNCS_Playback_Rewind_3x_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do rewind of a HLS stream with playback rate -3</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_100</test_case_id>
    <test_objective>Test to do rewind of a HLS stream with playback rate -3</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Rewind_3x_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Rewind_3x_HLS');
...
if "SUCCESS" in result.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Rewind on HLS stream with 3x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Rewind on HLS stream with 3x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
