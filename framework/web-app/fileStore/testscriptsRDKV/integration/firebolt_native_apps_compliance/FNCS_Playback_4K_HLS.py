...
<xml>
  ...
  <name>FNCS_Playback_4k_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of HLS 4K stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_12</test_case_id>
    <test_objective>To test the video playback of HLS 4K stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an HLS 4K stream should be updated in the config variable video_src_url_4k_hls inside MediaValidationVariables.py library inside filestore
...
test_url - HLS 4K url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_hls)
...
3.Retrieve the video_src_url_4k_hls variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_4K_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_4K_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <HLS 4K_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HLS 4K Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HLS 4K Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
