...
<xml>
  ...
  <name>FNCS_Playback_Bitrate_Switch_Down_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Restrict network bandwidth to certain bandwidth and verify the same by switching from highest possible resolution to lowest resolution in the manifest.</synopsis>
  <!--  -->
  <grodowns_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_273</test_case_id>
    <test_objective>Restrict network bandwidth to certain bandwidth and verify the same by switching from highest possible resolution to lowest resolution in the manifest</test_objective>
...
2. Test stream url for an HEVC stream should be downdated in the config variable video_src_url_bitrate_hevc inside MediaValidationVariables.py library inside filestore
3. FIREBOLT_COMPLIANCE_CHECK_AV_STATUS configuration should be set as yes/no in the device config file</pre_requisite>
    <api_or_interface_used>Execute the mediapipelinetests application in DUT</api_or_interface_used>
    <input_parameters>testcasename - "test_bitrate_switching"
test_url - hevc url from MediaValidationVariables library (MediaValidationVariables.video_src_url_bitrate_hevc)
...
3.Retrieve the video_src_url_bitrate_hevc variable from MediaValidationVariables library
...
6.During the execution , tdk_mediapipelinetests will parse the maifest file and retrieve the resolutions and corresponding bandwidths.
7.Using dash-demux instance the bitrate is set for the highest resolution available in the manifest file and played for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds.
8.Application then sets the next lowest bandwidth and verifies if the resolution is also as played as expected from the manifest.
9.Downon successfully playing all resolutions for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds, applications returns SUCCESS.
10.Verify the output from the execute command and check if the  "Failures: 0" and "Errors: 0" string exists or "failed: 0" string exists in the returned output
11.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_Bitrate_Switch_Down_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Bitrate_Switch_Down_HEVC');
...
if "SUCCESS" in result.upper():
    ...
    test_name = "test_bitrate_switch_down"
    #Test url for the stream to be played is retrieved from MediaValidationVariables library
    test_url = MediaValidationVariables.video_src_url_bitrate_hevc
    ...
    #Sample command = "mediapipelinetests test_bitrate_switching <HEVC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Bitrate Switch Down video playback using HEVC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Bitrate Switch Down video playback using HEVC stream was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
