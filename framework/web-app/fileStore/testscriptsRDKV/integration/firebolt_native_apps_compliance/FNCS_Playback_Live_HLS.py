...
<xml>
  ...
  <name>FNCS_Playback_Live_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the video playback of LIVE_HLS stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_09</test_case_id>
    <test_objective>To test the video playback of LIVE_HLS stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an LIVE_HLS stream should be updated in the config variable video_src_url_live_hls inside MediaValidationVariables.py library inside filestore
...
test_url - LIVE_HLS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_live_hls)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FNCS_PLAYBACK_Live_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_Live_HLS');
...
if "SUCCESS" in result.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <LIVE_HLS_STREAM_URL> checkavstatus=yes timeout=20"
