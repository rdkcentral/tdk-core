...
<xml>
  ...
  <name>FNCS_Playback_EOS_OPUS_Only_Audio</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the End Of Stream detection scenario for a Only Audio OPUS stream through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FNCS_PLAYBACK_315</test_case_id>
    <test_objective>To test the End Of Stream detection scenario for a Only Audio OPUS stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
2. Test stream url for an Only Audio OPUS stream should be updated in the config variable audio_src_url_short_duration_webm_opus inside MediaValidationVariables.py library inside filestore
...
test_url - Only Audio OPUS url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_short_duration_webm_opus)
...
3.Retrieve the audio_src_url_short_duration_webm_opus variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0"</expected_output>
    ...
    <test_script>FNCS_Playback_EOS_OPUS_Only_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FNCS_Playback_EOS_OPUS_Only_Audio');
...
if "SUCCESS" in result.upper():
    ...
    test_url = MediaValidationVariables.audio_src_url_short_duration_webm_opus
    ...
    #The default timeout of test application is 6 minutes, so for EOS test cases, if the test stream is longer than 6 minutes the total timeout in seconds ( stream duration + ~5 seconds as buffer) should be explicitly specified in configuration parameter 'FIREBOLT_COMPLIANCE_EOS_TIMEOUT' inside device config file
    tdkTestObj = obj.createTestStep('getDeviceConfigValue')
    tdkTestObj.addParameter("configKey","FIREBOLT_COMPLIANCE_EOS_TIMEOUT")
    tdkTestObj.executeTestCase(expectedResult);
    actualresult = tdkTestObj.getResult();
    ...
    #Sample command = "mediapipelinetests test_EOS <Only Audio OPUS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    #Add only_audio option since stream doesn't contain any video
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("EOS Detection for Only Audio OPUS stream using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("EOS Detection for Only Audio OPUS stream using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
