...
<xml>
  ...
  <name>FCS_Playback_Set_Rate_0.75x_H264_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a 60 Fps H264 stream with playback rate 0.75</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_198</test_case_id>
    <test_objective>Test to do set rate operation of a 60 Fps H264 stream with playback rate 0.75</test_objective>
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time in seconds for which the slowMotion operation should be carried out</pre_requisite>
...
operations=slowMotion0.75x:&lt;timeout&gt; - a ":" seperated string to specify the slowMotion operation to be executed and the time in seconds for which the operation should be continued. The timeout should be configured in the device configuration(FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT) from Device Config file</input_parameters>
...
5.Execute the command in DUT. During the execution, the DUT will start av playback, then do a slow Motion operation with playback rate 0.75 and then continue playback  at rate 0.75 for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds. Then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Set_Rate_0.75x_H264_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Set_Rate_0.75x_H264_60Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    setOperations ("slowMotion0.75x", timeoutInSeconds)
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=slowMotion0.75x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Playback with 0.75x playback rate on 60fps H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Playback with 0.75x playback rate on 6gfps H264 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
