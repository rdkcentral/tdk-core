...
<xml>
  ...
  <name>FCS_Playback_PauseSeek_Backward_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a 25 Fps H264 stream with pause operation</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_202</test_case_id>
    <test_objective>Test to do backward seek with pause on a stream with 25 Fps H264 video codec</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a stream with H264 video codec should be updated in the config variable video_src_url_dash_h264_25fps inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PauseSeek_Backward_H264_25Fps</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PauseSeek_Backward_H264_25Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Pause with Backward seek on 25 FPS H264 stream was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Pause with Backward seek on 25 Fps H264 stream failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
