...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_Stress_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple forward seek on a AAC stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_71</test_case_id>
    <test_objective>Test to do multiple forward seek on a AAC stream</test_objective>
...
2.test_url - AAC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_aac)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_Stress_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_Stress_AAC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <AAC_STREAM_URL> checkavstatus=yes operations=seek:3:10,seek:3:20,seek:3:30,seek:3:40,seek:3:50,seek:3:60"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek stress on AAC stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek stress on AAC stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
