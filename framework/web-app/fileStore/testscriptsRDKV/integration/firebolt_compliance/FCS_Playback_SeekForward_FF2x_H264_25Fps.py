...
<xml>
  ...
  <name>FCS_Playback_SeekForward_FF2x_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek operation followed by fast forward of a H264 25fps stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_196</test_case_id>
    <test_objective>Test to do forward seek operation followed by fast forward of a H264 25fps stream with playback rate 2</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a H264 stream should be updated in the config variable video_src_url_dash_h264_25fps inside MediaValidationVariables.py library inside filestore
...
test_url - H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_25fps)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_SeekForward_FF2x_H264_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_SeekForward_FF2x_H264_25Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    # Sample operations strings is "operations=seek:20:60,fastforward2x:20"
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=seek:20:60,fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward Seek followed by Fastforward on H264 25fps stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward Seek followed by Fastforward on H264 25fps stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
