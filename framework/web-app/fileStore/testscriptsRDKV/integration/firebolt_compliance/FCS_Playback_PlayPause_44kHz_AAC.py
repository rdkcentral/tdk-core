...
<xml>
  ...
  <name>FCS_Playback_PlayPause_44kHz_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify whether 44Khz audio is being rendered properly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_179</test_case_id>
    <test_objective>to verify whether 44Khz audio is being rendered properly</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_44kHz_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_44kHz_AAC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_sampling_rate <AAC_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "44kHz sampling rate video playback was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "44kHz sampling rate video playback was failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
