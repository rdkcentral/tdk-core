...
<xml>
  ...
  <name>FCS_Playback_10_Bit_Color_depth_Test</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_194</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_10_Bit_Color_depth_Test</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_10_Bit_Color_depth_Test')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_color_depth <HDR_STREAM_URL> checkavstatus=yes timeout=30"
    command = getMediaPipelineTestCommand (test_name, test_url, checkavstatus = checkAVStatus, timeout = timeoutInSeconds)
    command = command + " bit_depth=" + bit_depth
