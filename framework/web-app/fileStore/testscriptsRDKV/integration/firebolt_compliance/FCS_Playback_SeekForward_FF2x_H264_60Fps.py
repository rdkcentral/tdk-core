...
<xml>
  ...
  <name>FCS_Playback_SeekForward_FF2x_H264_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek operation followed by fast forward of a H264 60fps stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_200</test_case_id>
    <test_objective>Test to do forward seek operation followed by fast forward of a H264 60fps stream with playback rate 2</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a H264 stream should be updated in the config variable video_src_url_dash_h264_60fps inside MediaValidationVariables.py library inside filestore
...
test_url - H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_60fps)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_SeekForward_FF2x_H264_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_SeekForward_FF2x_H264_60Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward Seek followed by Fastforward on H264 60fps stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward Seek followed by Fastforward on H264 60fps stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
