...
<xml>
  ...
  <name>FCS_Playback_PauseSeek_Forward_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek with pause on a stream with HEVC video codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_172</test_case_id>
    <test_objective>Test to do forward seek with pause on a stream with HEVC audio codec</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a stream with HEVC audio codec should be updated in the config variable video_src_url_hevc inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PauseSeek_Forward_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PauseSeek_Forward_HEVC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <HEVC_STREAM_URL> checkavstatus=yes operations=seek:20:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Forward seek with pause on stream with HEVC audio codec was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Forward seek on stream with HEVC audio codec failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
