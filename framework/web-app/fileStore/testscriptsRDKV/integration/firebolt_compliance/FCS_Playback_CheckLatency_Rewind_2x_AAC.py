...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_Rewind_2x_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if latency observed during AAC trickplay is within the threshold limit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_Playback_106</test_case_id>
    <test_objective>Check if latency observed during AAC trickplay is within the threshold limit</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video_Accelerator,RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a live AAC stream should be updated in the config variable video_src_url_aac inside MediaValidationVariables.py library inside filestore.
...
test_url - aac stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_aac)
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_Rewind_2x_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_Rewind_2x_AAC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <AAC_STREAM_URL> checkavstatus=yes operations=rewind2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("AAC Playback was successfull")
            parseLatency(tdkTestObj,latencyThresholdValue);
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("AAC Playback failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
