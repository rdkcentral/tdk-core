...
<xml>
  ...
  <name>FCS_Playback_Audio_Volume_Stress_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether audio is playing in decreasing order using EAC3 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_209</test_case_id>
    <test_objective>to verify whether audio volume is playing in decreasing order using EAC3 stream</test_objective>
...
4. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_audio_volume_stress"
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Volume_Stress_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Volume_Stress_EAC3')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_volume_stress <EAC3_STREAM_URL> checkavstatus=yes timeout=30"
