...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a stream with OPUS audio codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_84</test_case_id>
    <test_objective>Test to do forward seek on a stream with OPUS audio codec</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a stream with OPUS audio codec should be updated in the config variable video_src_url_opus_mp4 inside MediaValidationVariables.py library inside filestore
...
2.test_url - OPUS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus_mp4)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_OPUS');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample oprations strings is "operations=seek:20:30" where 20 is time for which playback should happen and 30 is seek position/duration
    ...
    #Sample command = "mediapipelinetests test_trickplay <OPUS_STREAM_URL> checkavstatus=yes operations=seek:20:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on stream with OPUS audio codec was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek on stream with OPUS audio codec failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
