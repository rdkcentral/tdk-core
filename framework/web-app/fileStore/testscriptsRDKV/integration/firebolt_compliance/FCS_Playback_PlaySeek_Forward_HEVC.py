...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a stream with HEVC audio codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_86</test_case_id>
    <test_objective>Test to do forward seek on a stream with HEVC audio codec</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_HEVC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on stream with HEVC audio codec was successfull")
