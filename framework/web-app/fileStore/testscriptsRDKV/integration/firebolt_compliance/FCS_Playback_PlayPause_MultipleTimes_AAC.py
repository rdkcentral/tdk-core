...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>FCS_Playback_PlayPause_MultipleTimes_AAC</name>
  <primitive_test_id/>
  <primitive_test_name>FireboltCompliance_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to do multiple short duration play, pause operations on a AAC stream</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_40</test_case_id>
    <test_objective>Test to do multiple short duration play, pause operations on a AAC stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_MultipleTimes_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_MultipleTimes_AAC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <AAC_STREAM_URL> checkavstatus=yes operations=play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Multiple play,pause operations on AAC stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
