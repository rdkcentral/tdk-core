...
<xml>
  ...
  <name>FCS_Playback_Init_Shutdown</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_50</test_case_id>
...
5.Verify the output from the execute command and check if the strings "Failures: 0" and "Errors: 0", or "failed: 0" exists in the returned output
6.Retrieve the FIREBOLT_COMPLIANCE_CHECK_AV_STATUS and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT config values from Device config file.
7.Construct the mediapipelinetests command based on the retrieved video url, testcasename("test_generic_playback"), value of FIREBOLT_COMPLIANCE_CHECK_AV_STATUS flag and FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT timeout value
8.Execute the command in DUT. During the execution, the DUT will playback the stream for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds and then the application exits by closing the pipeline
...
Checkpoint 2. Verify that the outputs returned from mediapipelinetests during both command executions, contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Init_Shutdown</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Init_Shutdown')
