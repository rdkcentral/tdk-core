...
<xml>
  ...
  <name>FCS_Playback_4K_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do videoplayback of a DASH 4K stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_32</test_case_id>
    <test_objective>To test the video playback of a DASH 4K stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a DASH 4K stream should be updated in the config variable video_src_url_4k_dash inside MediaValidationVariables.py library inside filestore
...
test_url - dash 4k stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_dash)
...
3.Retrieve the video_src_url_4k_dash variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_4K_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_4K_DASH')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_4k_dash
    ...
    #Sample command = "mediapipelinetests test_generic_playback <DASH_4K_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("DASH 4K Playback using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("DASH 4K Playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
