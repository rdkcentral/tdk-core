...
<xml>
  ...
  <name>FCS_Playback_SeekBackward_FF2x_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek operation followed by fast forward of a H264 stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_222</test_case_id>
    <test_objective>Test to do backward seek operation followed by fast forward of a H264 stream with playback rate 2</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_SeekBackward_FF2x_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_SeekBackward_FF2x_H264');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=seek:20:0,fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward Seek followed by Fastforward on H264 stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward Seek followed by Fastforward on H264 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
