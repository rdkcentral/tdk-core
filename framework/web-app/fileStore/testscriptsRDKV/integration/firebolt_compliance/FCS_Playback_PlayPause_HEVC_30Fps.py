...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HEVC_30Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify play pause for HEVC 30 fps video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_162</test_case_id>
    <test_objective>To test whether the framerate is rendered properly during play-pause scenario of pipeline for HEVC 30 fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an HEVC 30 fps stream should be updated in the config variable video_src_url_hevc_30fps inside MediaValidationVariables.py library inside filestore
...
test_url - hevc url from MediaValidationVariables library (MediaValidationVariables.video_src_hevc_30fps)
...
6.Retrieve the video_src_url_hevc_30fps variable from MediaValidationVariables library
...
Checkpoint 3. Verify video playback happened at expected frame rate.</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_HEVC_30Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HEVC_30Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_fps <30 fps_HEVC_STREAM_URL> checkavstatus=yes timeout=30"
