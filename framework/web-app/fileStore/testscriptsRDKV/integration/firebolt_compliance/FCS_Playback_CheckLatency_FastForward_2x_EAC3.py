...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_FastForward_2x_EAC3</name>
  ...
  <test_cases>
    <test_case_id>FCS_Playback_105</test_case_id>
...
2. Test stream url for a live EAC3 stream should be updated in the config variable video_src_url_ec3 inside MediaValidationVariables.py library inside filestore.
...
test_url - ec3 stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ec3)
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_FastForward_2x_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_FastForward_2x_EAC3')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <EAC3_STREAM_URL> checkavstatus=yes operations=fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("EAC3 Playback was successfull")
            parseLatency(tdkTestObj,latencyThresholdValue);
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("EAC3 Playback failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
