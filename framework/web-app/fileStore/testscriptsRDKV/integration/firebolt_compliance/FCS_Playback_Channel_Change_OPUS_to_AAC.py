...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_OPUS_to_AAC</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_83</test_case_id>
...
2. Test stream url for a AAC stream and OPUS stream should be updated in the config variable video_src_url_dash_h264 and video_src_url_OPUS_hls respectively inside MediaValidationVariables.py library inside filestore
...
test_url(s) - AAC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_aac), OPUS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus)
...
5.Execute the command in DUT. During the execution, the DUT will play with stream with OPUS codec for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to stream with AAC codec FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_OPUS_to_AAC</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_OPUS_to_AAC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <OPUS_STREAM_URL> <AAC_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change test from stream with OPUS codec to stream with AAC codec was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change test from stream with OPUS codec to stream with AAC codec failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
