...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_H264_to_4K</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from  HLS H264 stream and  DASH 4K stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_79</test_case_id>
    <test_objective>Test to do Channel Change from DASH H264 stream to  HLS 4K stream through playbin gst element and westerossink</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream urls for a DASH H264 stream  and HLS 4K stream should be updated in the config variable video_src_url_dash_h264 and video_src_url_4k_hls respectively inside MediaValidationVariables.py library inside filestore
...
5.Execute the command in DUT. During the execution, the DUT will play H264 DASH STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to 4K HLS STREAM FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_H264_to_4K</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_H264_to_4K')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and timeoutConfigValue != "" and secondChannelTimeout!= "":
        ...
        secondChannelTimeout = timeoutConfigValue
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <HLS_H264_STREAM_URL> <DASH_4K_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change from H264 DASH stream to 4K HLS stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change from H264 DASH stream to 4K HLS stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
