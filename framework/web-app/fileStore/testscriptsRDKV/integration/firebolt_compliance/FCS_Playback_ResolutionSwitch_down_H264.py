...
<xml>
  ...
  <name>FCS_Playback_ResolutionSwitch_down_H264</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_145</test_case_id>
...
2. Test stream url for an mp4 stream should be updated in the config variable video_src_url_resolution_down_h264 inside MediaValidationVariables.py library inside filestore
...
test_url - url from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_down_h264)
...
3.Retrieve the video_src_url_resolution_down_h264 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_ResolutionSwitch_down_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_ResolutionSwitch_down_H264')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_resolution_down_h264
    ...
    #Sample command = "mediapipelinetests test_resolution_down <video_src_url_resolution_down_h264> checkavstatus=yes timeout=20"
