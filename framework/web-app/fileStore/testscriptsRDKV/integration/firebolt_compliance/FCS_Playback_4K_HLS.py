...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do videoplayback of a HLS 4K stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
test_url - hls 4k stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_4k_hls)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_4K_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_4K_HLS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_generic_playback <HLS_4K_STREAM_URL> checkavstatus=yes timeout=20"
