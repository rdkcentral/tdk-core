...
<xml>
  ...
  <name>FCS_Playback_ResolutionSwitch_down_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do videoplayback by switching resolutions from 4K resolution to very low resolution (144p) through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_146</test_case_id>
...
2. Test stream url for an mp4 stream should be updated in the config variable video_src_url_resolution_down_av1 inside MediaValidationVariables.py library inside filestore
...
test_url - url from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_down_av1)
"checkavstatus=yes" - argument to do the video playback verification from SOC side . This argument can be yes/no based on a device configuration(FIREBOLT_COMPLIANCE_CHECK_AV_STATUS) from Device Config file</input_parameters>
    <automation_approch>
1.Load the systemutil module
...
3.Retrieve the video_src_url_resolution_down_av1 variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will playback av for 70 seconds(10 seconds for each resolution) checking whether all the resolutions are played 144p, 240p, 360p, 480p, 720p, 1080p and 2160p then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_ResolutionSwitch_down_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_ResolutionSwitch_down_AV1')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_resolution_down_av1
    ...
    #Sample command = "mediapipelinetests test_resolution_down <video_src_url_resolution_up_av1> checkavstatus=yes timeout=20"
