...
<xml>
  ...
  <name>FCS_Playback_PauseSeek_Forward_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek with pause on a stream with 25 Fps H264 video codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_204</test_case_id>
    <test_objective>Test to do forward seek on a 25 Fps H264 stream</test_objective>
...
2.test_url - H264 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264_25fps)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PauseSeek_Forward_H264_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PauseSeek_Forward_H264_25Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample oprations strings is "operations=pause:10,seek:10:20,play:10" where 10 is time for which pause should happen and 20 is seek position/duration
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=seek:20:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek with pause on 25 Fps H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek on 25 Fps H264 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
