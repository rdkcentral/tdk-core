...
<xml>
  ...
  <name>FCS_Playback_PlayPause_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 25 fps stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_123</test_case_id>
    <test_objective>To test whether the framerate is rendered properly during play-pause scenario of pipeline for 25 fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an 25 fps stream should be updated in the config variable video_src_url_mp4_25fps inside MediaValidationVariables.py library inside filestore
...
test_url - hls url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_25fps)
...
6.Retrieve the video_src_url_mp4_25fps variable from MediaValidationVariables library
...
Checkpoint 3. Verify video playback happened at expected framerate</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_25Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_25fps
    ...
    #Sample command = "mediapipelinetests test_playback_fps <25 fps_STREAM_URL> checkavstatus=yes timeout=30"
