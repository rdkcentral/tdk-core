...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_4K_to_H264</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_80</test_case_id>
...
3.Retrieve the video_src_url_4k_hls and video_src_url_dash_h264  variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will play 4K HLS STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to H264 DASH STREAM FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_4K_to_H264</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_4K_to_H264')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <HLS_4K_STREAM_URL> <DASH_H264_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change test from HLS 4K to DASH H264 was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change test from HLS 4K to DASH H264 failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
