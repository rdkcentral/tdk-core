...
<xml>
  ...
  <name>FCS_Playback_Audio_Change_OPUS_AC3</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_110</test_case_id>
...
2. Test stream url for a multi codec AC3 OPUS stream should be updated in the config variable video_src_url_opus_ac3 inside MediaValidationVariables.py library inside filestore.
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Change_OPUS_AC3</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Change_OPUS_AC3');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_change <OPUS_AC3_URL> checkavstatus=yes timeout=10"
