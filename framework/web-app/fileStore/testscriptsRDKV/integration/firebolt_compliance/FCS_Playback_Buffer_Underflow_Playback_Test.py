...
<xml>
  ...
  <name>FCS_Playback_Buffer_Underflow_Playback_Test</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_134</test_case_id>
...
7.Based on the ExecuteCommand() return value and the output returned from the mediapipelinetests application, TM return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>FCS_Playback_Buffer_Underflow_Playback_Test</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Buffer_Underflow_Playback_Test')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    actualresult, videoStart = getDeviceConfigValue (sysUtilObj, 'FIREBOLT_COMPLIANCE_UNDERFLOW_VIDEO_START_POINT')
