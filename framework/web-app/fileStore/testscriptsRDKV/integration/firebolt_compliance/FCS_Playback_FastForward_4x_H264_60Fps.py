...
<xml>
  ...
  <name>FCS_Playback_FastForward_4x_H264_60Fps</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_221</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_FastForward_4x_H264_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_FastForward_4x_H264_60Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_60Fps_STREAM_URL> checkavstatus=yes operations=fastforward4x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Fastforward on 60 Fps H264 stream with 4x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Fastforward on 60 Fps H264 stream with 4x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
