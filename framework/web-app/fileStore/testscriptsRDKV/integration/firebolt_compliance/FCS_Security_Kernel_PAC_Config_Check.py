...
<xml>
  ...
  <name>FCS_Security_Kernel_PAC_Config_Check</name>
  ...
  <test_cases>
    <test_case_id>FCS_Security_25</test_case_id>
    <test_objective>Check PAC cofigurations are enabled or not</test_objective>
    ...
    <automation_approch>Check if expected PAC configurations are enabled by extracting the /proc/config.gz file.</automation_approch>
    <expected_output>Expected PAC configurations must be enabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>FCS_Security_Kernel_PAC_Config_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_Security_Kernel_PAC_Config_Check')
...
# Test option
testOption = "check_PAC_CONFIG "

# Helper function to get MODEL_NUM from DUT
def check_if_pointer_authentication_support_enabled(obj):
    result,value = getDeviceConfigValue(obj, 'PLATFORM_SUPPORTS_POINTER_AUTHENTICATION')
    if value == "yes":
        return True
    else:
        return False
...
if "SUCCESS" in result.upper():
    ...
    if result and details:
        ...
        if not details:
            ...
            print("TEST STEP 2: Check if expected kernel configurations are enabled/disabled")
            ...
            if "Extracted config file could not be found" in details:
                ...
                supported = check_if_pointer_authentication_support_enabled(obj)

                if not supported:
                    print("PLATFORM does not support Pointer Authentication.")

                    # Expected configs and their status for BCM model
                    configs_to_check = {
                            "CONFIG_STACKPROTECTOR_STRONG": "enabled",
                            "CONFIG_ARM64_PTR_AUTH": "not set",
                            "CONFIG_ARM64_PTR_AUTH_KERNEL": "not found"
                    }
                else:

                    # For pointer authenticated models, all configurations must be enabled
                    configs_to_check = {
                            "CONFIG_STACKPROTECTOR_STRONG": "enabled",
                            "CONFIG_ARM64_PTR_AUTH": "enabled",
                            "CONFIG_ARM64_PTR_AUTH_KERNEL": "enabled"
                    }

                # Clean up the output details to ensure no extra characters are present
                output_lines = details.splitlines()
                output_lines = [line.strip() for line in output_lines]  # Strip any extra whitespace/newlines
                test_failed = False
