...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HEVC_HDR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test whether the HDR playback is smooth during play-pause scenario of pipeline through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_131</test_case_id>
    <test_objective>To test whether the HDR playback is smooth during play-pause scenario of pipeline through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an 25 fps stream should be updated in the config variable video_src_url_mp4_hevc_hdr inside MediaValidationVariables.py library inside filestore
...
test_url - hls url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_hevc_hdr)
...
6.Retrieve the video_src_url_mp4_hevc_hdr variable from MediaValidationVariables library
...
Checkpoint 3. Verify video playback happened at expected framerate</expected_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>FCS_Playback_PlayPause_HEVC_HDR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HEVC_HDR')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_hevc_hdr
    ...
    #Sample command = "mediapipelinetests test_play_pause_pipeline <HEVC HDR stream url> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HEVC_HDR video playback  using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HEVC_HDR video playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
