...
<xml>
  ...
  <name>FCS_Playback_PlayPause_48kHz_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether 48Khz audio is being rendered properly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_182</test_case_id>
    <test_objective>to verify whether 48Khz audio is being rendered properly</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_48kHz_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_48kHz_OPUS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_sampling_rate <OPUS_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("48kHz sampling rate video playback was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("48kHz sampling rate video playback was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
