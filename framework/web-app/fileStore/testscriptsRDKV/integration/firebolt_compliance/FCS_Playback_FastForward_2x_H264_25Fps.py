...
<xml>
  ...
  <name>FCS_Playback_FastForward_2x_H264_25Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do fast forward of a 25 Fps H264 stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_193</test_case_id>
    <test_objective>Test to do fast forward of a 25 Fps H264 stream with playback rate 2</test_objective>
...
5.Execute the command in DUT. During the execution, the DUT will start av playback, then do a fast forward operation with playback rate 2 and then continue playback  at rate 2 for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds. Then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_FastForward_2x_H264_25Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_FastForward_2x_H264_25Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Fastforward on 25fps H264 stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Fastforward on 25fps H264 stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
