...
<xml>
  ...
  <name>FCS_Playback_Audio_Low_Volume_EAC3</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_207</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Low_Volume_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Low_Volume_EAC3')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_volume <EAC3_STREAM_URL> checkavstatus=yes timeout=30"
    command = getMediaPipelineTestCommand (test_name, test_url, checkavstatus = checkAVStatus, timeout = timeoutInSeconds)
    command = command + " volume_set=" + volume_set
