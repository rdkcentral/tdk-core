...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do forward seek on a DASH stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_28</test_case_id>
    <test_objective>Test to do forward seek on a DASH stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_DASH');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <DASH_STREAM_URL> checkavstatus=yes operations=seek:20:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek on DASH stream was successfull")
