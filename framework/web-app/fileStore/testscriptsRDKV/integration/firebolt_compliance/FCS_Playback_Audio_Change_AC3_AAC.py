...
<xml>
  ...
  <name>FCS_Playback_Audio_Change_AC3_AAC</name>
...
2. Test stream url for a multi codec AAC AC3 stream should be updated in the config variable video_src_url_ac3_aac inside MediaValidationVariables.py library inside filestore.
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Change_AC3_AAC</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Change_AC3_AAC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_change <AC3_AAC_URL> checkavstatus=yes timeout=10"
