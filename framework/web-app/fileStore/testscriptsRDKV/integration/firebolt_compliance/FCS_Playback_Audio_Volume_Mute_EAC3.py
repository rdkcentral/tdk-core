...
<xml>
  ...
  <name>FCS_Playback_Audio_Volume_Mute_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to verify whether audio is playing in mute state using EAC3 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_206</test_case_id>
    <test_objective>to verify whether audio is playing in mute state using EAC3 stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Volume_Mute_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Volume_Mute_EAC3')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Mute Audio playback was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Mute Audio playback was failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
