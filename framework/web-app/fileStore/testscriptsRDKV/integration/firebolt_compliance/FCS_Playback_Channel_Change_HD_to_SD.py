...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_HD_to_SD</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_143</test_case_id>
...
test_url(s) - SD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_360p), HD url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_1080p)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_HD_to_SD</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_HD_to_SD')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <HD_STREAM_URL> <SD_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Channel Change test from stream with HD content to stream with SD content was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Channel Change test from stream with HD content to stream with SD content failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
