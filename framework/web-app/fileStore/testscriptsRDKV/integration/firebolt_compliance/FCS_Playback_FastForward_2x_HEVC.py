...
<xml>
  ...
  <name>FCS_Playback_FastForward_2x_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do fast forward of a HEVC stream with playback rate 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_FastForward_2x_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_FastForward_2x_HEVC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <HEVC_STREAM_URL> checkavstatus=yes operations=fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Fastforward on HEVC stream with 2x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Fastforward on HEVC stream with 2x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
