...
<xml>
  ...
  <name>FCS_Playback_PauseSeek_Backward_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a HEVC stream with pause operation</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_177</test_case_id>
    <test_objective>Test to do backward seek with pause on a stream with HEVC video codec</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PauseSeek_Backward_HEVC</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PauseSeek_Backward_HEVC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <HEVC_STREAM_URL> checkavstatus=yes operations=seek:20:0"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "Pause with Backward seek on HEVC stream was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "Pause with Backward seek on HEVC stream failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
