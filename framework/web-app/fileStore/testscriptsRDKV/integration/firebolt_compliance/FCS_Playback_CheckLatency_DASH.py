...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check latency for DASH playback</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_Playback_96</test_case_id>
...
2. Test stream url for a live DASH stream should be updated in the config variable video_src_url_live_dash inside MediaValidationVariables.py library inside filestore.
...
test_url - dash live stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_live_dash)
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_DASH')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_latency <DASH_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("DASH Playback was successfull")
            parseLatency(tdkTestObj,latencyThresholdValue);
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("DASH playback failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
