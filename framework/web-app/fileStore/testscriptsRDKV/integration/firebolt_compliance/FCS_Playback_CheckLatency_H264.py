...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_H264</name>
  ...
  <test_cases>
    <test_case_id>FCS_Playback_97</test_case_id>
...
2. Test stream url for a live H264 stream should be updated in the config variable video_src_url_dash_h264 inside MediaValidationVariables.py library inside filestore.
...
test_url - h264 live stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_dash_h264)
...
3.Retrieve the video_src_url_h264 variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will playback av for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds and check if the latency during playback is within in the FIREBOLT_COMPLIANCE_PLAYBACK_LATENCY_THRESHOLD milliseconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_H264')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    actualresult, latencyThresholdValueConfigValue = getDeviceConfigValue (sysUtilObj, 'FIREBOLT_COMPLIANCE_PLAYBACK_LATENCY_THRESHOLD')
    ...
    #Sample command = "mediapipelinetests test_playback_latency <H264_STREAM_URL> checkavstatus=yes timeout=20"
