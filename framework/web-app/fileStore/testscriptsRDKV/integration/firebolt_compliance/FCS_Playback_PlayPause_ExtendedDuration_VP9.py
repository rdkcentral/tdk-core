...
<xml>
  ...
  <name>FCS_Playback_PlayPause_ExtendedDuration_VP9</name>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_ExtendedDuration_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_ExtendedDuration_VP9');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <VP9_STREAM_URL> checkavstatus=yes operations=play:360,pause:360,play:5"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Extended duration play,pause operations on VP9 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
