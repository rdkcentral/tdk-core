...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>FCS_Playback_PlayPause_MultipleTimes_VP9</name>
  <primitive_test_id/>
  <primitive_test_name>FireboltCompliance_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to do multiple short duration play, pause operations on a VP9 stream</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_41</test_case_id>
    <test_objective>Test to do multiple short duration play, pause operations on a VP9 stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_MultipleTimes_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_MultipleTimes_VP9');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <VP9_STREAM_URL> checkavstatus=yes operations=play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30,play:30,pause:30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Multiple play,pause operations on VP9 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Multiple play,pause operations on VP9 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
