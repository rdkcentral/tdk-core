...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_DASH_to_HLS</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_78</test_case_id>
    <test_objective>Test to do Channel Change DASH stream to HLS stream through playbin gst element and westerossink</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a DASH stream and HLS stream should be updated in the config variable video_src_url_dash and video_src_url_hls inside MediaValidationVariables.py library inside filestore
...
5.Execute the command in DUT. During the execution, the DUT will play DASH STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to HLS STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_DASH_to_HLS</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_DASH_to_HLS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <DASH_STREAM_URL> <HLS_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change from DASH stream to HLS stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change from DASH stream to HLS stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
