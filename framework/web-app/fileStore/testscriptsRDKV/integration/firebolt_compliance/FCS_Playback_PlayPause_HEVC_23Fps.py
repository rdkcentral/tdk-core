...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HEVC_23Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify play pause for HEVC 23 fps   video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_165</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for 23 Fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an HEVC stream should be updated in the config variable video_src_url_hevc_23fps inside MediaValidationVariables.py library inside filestore
...
test_url - hls url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_23fps)
...
3.Retrieve the video_src_url_hevc_23fps variable from MediaValidationVariables library
...
8.Verify if video is playing at 23 Frames per second on average</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_HEVC_23Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HEVC_23Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_hevc_23fps
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "23Fps video playback  using 'playbin' and 'westeros-sink' was successfull"
            print "Mediapipeline test executed successfully"
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print "23Fps video playback using 'playbin' and 'westeros-sink' failed"
    else:
        tdkTestObj.setResultStatus("FAILURE")
