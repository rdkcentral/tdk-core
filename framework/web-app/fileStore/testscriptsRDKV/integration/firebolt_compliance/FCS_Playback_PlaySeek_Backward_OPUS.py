...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Backward_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do backward seek on a stream with OPUS audio codec</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_85</test_case_id>
    <test_objective>Test to do backward seek on a stream with OPUS audio codec</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Backward_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Backward_OPUS');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <OPUS_STREAM_URL> checkavstatus=yes operations=seek:20:0"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Backward seek on stream with OPUS audio codec was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Backward seek on stream with OPUS audio codec failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
