...
<xml>
  ...
  <name>FCS_Security_Kernel_KASLR_Config_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Script to check the KASLR config is enabled and the random space value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_Security_23</test_case_id>
    <test_objective>Script to check the KASLR config is enabled and the random space value</test_objective>
    ...
    <api_or_interface_used>systemutils</api_or_interface_used>
    <input_parameters></input_parameters>
    <automation_approch>Check if expected KASLR configurations are enabled by extracting the /proc/config.gz file and random_space value in the /proc/sys/kernel/randomize_va_space</automation_approch>
    <expected_output>Congigurations should be enable</expected_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>FCS_Security_Kernel_KASLR_Config_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_Security_Kernel_KASLR_Config_Check');
...
if "SUCCESS" in result.upper():
    ...
    if result and details:
        ...
        if not details:
            ...
            print("TEST STEP 2: Check if KASLR is enabled or not")
            ...
            if "Extracted config file could not be found" in details:
                ...
                for line in details.splitlines():
                    if "is enabled" in line:
                        print(f"{line}.")
                        print("KASLR Configuration is enabled as expected")
                        ...
                        KaslrValue = int(details)
                        print("Checking ASLR status")
                        print("[RESPONSE FROM DEVICE]: %s" %(details))
                        if KaslrValue is 2:
                            print("SUCCESS: KASLR is enabled")
                            tdkTestObj.setResultStatus("SUCCESS");
                        else:
                            print("FAILURE: KASLR status %s" %(KaslrValue))
                            print("KASLR is Disabled")
                            tdkTestObj.setResultStatus("FAILURE");

                    if not config_found:
                        print("KASLR Configuration is disabled")
                        tdkTestObj.setResultStatus("FAILURE")
            else:
