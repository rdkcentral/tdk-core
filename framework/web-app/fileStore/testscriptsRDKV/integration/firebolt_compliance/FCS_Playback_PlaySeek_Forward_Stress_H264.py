...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_Stress_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple forward seek on a H264 stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_Stress_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_Stress_H264');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_STREAM_URL> checkavstatus=yes operations=seek:3:10,seek:3:20,seek:3:30,seek:3:40,seek:3:50,seek:3:60"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek stress on H264 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek stress on H264 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
