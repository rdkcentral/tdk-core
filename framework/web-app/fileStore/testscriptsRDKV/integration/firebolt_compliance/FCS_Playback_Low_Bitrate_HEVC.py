...
<xml>
  ...
  <name>FCS_Playback_Low_Bitrate_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_188</test_case_id>
    <test_objective>To verify whether the video is playing with low bitrate using hevc stream</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Low_Bitrate_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Low_Bitrate_HEVC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Low bitrate video playback was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Low bitarte video playback was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
