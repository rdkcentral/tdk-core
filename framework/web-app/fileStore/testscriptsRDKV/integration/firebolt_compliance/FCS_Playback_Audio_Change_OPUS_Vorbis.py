...
<xml>
  ...
  <name>FCS_Playback_Audio_Change_OPUS_Vorbis</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_128</test_case_id>
...
2. Test stream url for a multi codec Vorbis OPUS stream should be updated in the config variable video_src_url_opus_vorbis inside MediaValidationVariables.py library inside filestore.
...
test_url - multi codec url with OPUS and Vorbis audio streams from MediaValidationVariables library (MediaValidationVariables.video_src_url_opus_vorbis)
...
3.Retrieve the video_src_url_opus_vorbis variable from MediaValidationVariables library
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Change_OPUS_Vorbis</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Change_OPUS_Vorbis');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_opus_vorbis
    ...
    #Sample command = "mediapipelinetests test_audio_change <OPUS_Vorbis_URL> checkavstatus=yes timeout=10"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("OPUS Vorbis codec switch was successfull")
            print("Mediapipeline test executed successfully")
            checkifCodecPlayed(tdkTestObj,"opus")
            checkifCodecPlayed(tdkTestObj,"vorbis")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("OPUS Vorbis codec switch failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
