...
<xml>
  ...
  <name>FCS_Playback_Set_Rate_0.5x_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do set rate operation of a AV1 stream with playback rate 0.5</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_153</test_case_id>
    <test_objective>Test to do set rate operation of a AV1 stream with playback rate 0.5</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Set_Rate_0.5x_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Set_Rate_0.5x_AV1');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <AV1_STREAM_URL> checkavstatus=yes operations=slowMotion0.5x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Playback with 0.5x playback rate on AV1 stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Playback with 0.5x playback rate on AV1 stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
