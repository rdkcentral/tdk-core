...
<xml>
  ...
  <name>FCS_Playback_FastForward_3x_H264_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do fast forward of a H264_60Fps stream with playback rate 3</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_219</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_FastForward_3x_H264_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_FastForward_3x_H264_60Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <H264_60Fps_STREAM_URL> checkavstatus=yes operations=fastforward3x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Fastforward on 60 Fps H264 stream with 3x speed was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Fastforward on 60 Fps H264 stream with 3x speed failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
