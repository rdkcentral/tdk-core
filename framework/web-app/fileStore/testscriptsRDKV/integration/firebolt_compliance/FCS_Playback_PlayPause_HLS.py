...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HLS</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_08</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for HLS stream through 'playbin' and 'westerossink' gst elements</test_objective>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HLS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HLS Play-pause using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
