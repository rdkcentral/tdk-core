...
<xml>
  ...
  <name>FCS_Security_Kernel_CFI_Config_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to check the CFI Configs are enabled or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_Security_21</test_case_id>
    <test_objective>Test to check the CFI Configs are enabled in DUT</test_objective>
    ...
    <automation_approch>Check if expected CFI Configurations are enabled by extracting the /proc/config.gz file.</automation_approch>
    <expected_output>All configurations should be enabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>FCS_Security_Kernel_CFI_Config_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_Security_Kernel_CFI_Config_Check');
...
if "SUCCESS" in result.upper():
    ...
    if result and details:
        ...
        if not details:
            ...
            print("TEST STEP 2: Check if expected CFI configurations enabled or not")
