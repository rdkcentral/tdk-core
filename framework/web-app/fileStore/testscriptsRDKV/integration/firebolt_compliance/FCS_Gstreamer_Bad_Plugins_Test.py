...
<xml>
  ...
  <name>FCS_Gstreamer_Bad_Plugins_Test</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Execute gst-plugin-bad test applications</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <rdk_versions />
  <test_cases>
    <test_case_id>FCS_Gstreamer_03</test_case_id>
    <test_objective>Execute opensource gst-plugin-bad test applications</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>OpenSource gstreamer bad plugins test applications must be present in OPENSOURCE_TEST path</pre_requisite>
    <api_or_interface_used>ExecuteSuite.sh -- MASTER_SUITE to execute opensource test applications</api_or_interface_used>
    <input_parameters>Type of gstreamer plugins to be tested - gst_plugin_bad</input_parameters>
    <automation_approch>1.Load the firebolt_compliance module.
    2.Execute the "gst_plugin_bad" test applications using the MASTER_SUITE.
    ...
    <expected_output>No test suites should fail as part of gst_plugin_bad</expected_output>
    <priority>Medium</priority>
    <test_stub_interface>libfirebolt_compliance.so.0</test_stub_interface>
    <test_script>FCS_Gstreamer_Bad_Plugins_Test</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_Gstreamer_Bad_Plugins_Test');
...
if "success" in result.lower():
    ...
    # Configuring the test object for gst-plugin-bad test suites execution
    tdkTestObj.addParameter("GStreamer_plugins_type","gst-plugin-bad");
    #Execute the test case in STB
    expectedresult="Test Suite Executed"
    tdkTestObj.executeTestCase(expectedresult);
    #Get the result of execution
    actualresult = tdkTestObj.getResult();
    print("Gst plugin bad Test Result : %s" %actualresult);

    #To Validate the Execution of Test Suites
    details = tdkTestObj.getResultDetails();
    if "TotalSuite" in details:
        print("Gst plugin bad status details : %s" %details);
        ...
        #Get the log path of the Gst plugin bad Testsuite
        logpath =tdkTestObj.getLogPath();
        if "TestSummary.log" in logpath:
            print("Log Path :%s"%logpath);

            #Transferring the Gst plugin bad Testsuite Logs
            filepath = tdkTestObj.transferLogs( logpath, "false" );

            #Parsing the output of gst-plugin-bad test apps
            data = open(filepath,'r');
            message = data.read()
            print("\n**************GStreamer_plugins_bad Execution Log - Begin*************\n\n")
            print(message)
            data.close()
            print("\n**************GStreamer_plugins_bad Execution - End*************\n\n")
            failures = getNumberOfFailures(filepath);
            if failures:
                print("Observed Failures during Execution")
                tdkTestObj.setResultStatus("FAILURE");
            else:
                print("Successfully executed all applications under gst-plugin-bad")
                tdkTestObj.setResultStatus("SUCCESS");
