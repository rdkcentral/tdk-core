...
<xml>
  ...
  <name>FCS_iCrypto_Vault_Tests</name>
...
6.Based on the ExecuteCommand() return value and the output returned from the Vault tests of "cgfacetests" application, TM return SUCCESS/FAILURE status.</automation_approch>
...
Checkpoint 2. Verify that the output of Vault Tests returned from cgfacetests contains the strings "TOTAL:" and "0 FAILED"</expected_output>
    ...
    <test_script>FCS_iCrypto_Vault_Tests</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_iCrypto_Vault_Tests');
