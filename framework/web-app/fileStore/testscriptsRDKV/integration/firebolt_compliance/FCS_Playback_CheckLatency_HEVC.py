...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FCS_Playback_98</test_case_id>
...
2. Test stream url for a live HEVC stream should be updated in the config variable video_src_url_hevc inside MediaValidationVariables.py library inside filestore.
...
test_url - hevc stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc)
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_HEVC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_playback_latency <HEVC_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HEVC Playback was successfull")
            parseLatency(tdkTestObj,latencyThresholdValue);
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("HEVC Playback failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
