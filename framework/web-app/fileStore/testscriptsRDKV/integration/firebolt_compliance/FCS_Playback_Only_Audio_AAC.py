...
<xml>
  ...
  <name>FCS_Playback_Only_Audio_AAC</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_184</test_case_id>
...
2. Test stream url for an AAC stream should be updated in the config variable audio_src_url_mp3 inside MediaValidationVariables.py library inside filestore
...
test_url - aac url from MediaValidationVariables library (MediaValidationVariables.audio_src_url_mp3)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Only_Audio_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Only_Audio_AAC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_only_audio <mp3_STREAM_URL> checkavstatus=yes timeout=30"
