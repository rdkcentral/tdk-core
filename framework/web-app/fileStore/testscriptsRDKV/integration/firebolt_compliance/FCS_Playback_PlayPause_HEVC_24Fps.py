...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HEVC_24Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify play pause for HEVC 24 fps video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_160</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for HEVC 24 Fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an HEVC stream should be updated in the config variable video_src_hevc_mp4_24fps inside MediaValidationVariables.py library inside filestore
...
test_url - hevc url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_24fps)
...
3.Retrieve the video_src_url_hevc_24fps variable from MediaValidationVariables library
...
8.Verify if video is playing at 24 Frames per second on average</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_HEVC_24Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HEVC_24Fps');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_hevc_24fps
    ...
    #Sample command = "mediapipelinetests test_playback_fps <HEVC_STREAM_URL> checkavstatus=yes timeout=30"
