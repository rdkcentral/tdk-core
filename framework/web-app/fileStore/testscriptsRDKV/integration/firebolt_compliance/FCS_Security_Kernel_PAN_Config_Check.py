...
<xml>
  ...
  <name>FCS_Security_Kernel_PAN_Config_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check PAN cofigurations are enabled or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_Security_26</test_case_id>
    <test_objective>Check PAN cofigurations are enabled or not</test_objective>
    ...
    <automation_approch>Check if expected PAN configurations are enabled by extracting the /proc/config.gz file.</automation_approch>
    <expected_output>Expected PAN configurations must be enabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>libsystemutilstub.so.0</test_stub_interface>
    <test_script>FCS_Security_Kernel_PAN_Config_Check</test_script>
    <skipped>No</skipped>
...
#This will be replaced with corresponding DUT Ip and port while executing scrip
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'FCS_Security_Kernel_PAN_Config_Check');
...
if "SUCCESS" in result.upper():
    ...
    if result and details:
        ...
        if not details:
            ...
            print("TEST STEP 2: Check if expected configurations are enabled or not")
            ...
            if "Extracted config file could not be found" in details:
                ...
                for line in details.splitlines():
                    if "is enabled" in line:
                        print(f"{line}.")
                        print("PAN Config is enabled as expected")
                        tdkTestObj.setResultStatus("SUCCESS")
                        config_found = True
                        break  # Exit after the first enabled config is found
                    if not config_found:
                        print("PAN Config is disabled")
                        tdkTestObj.setResultStatus("FAILURE")
            else:
