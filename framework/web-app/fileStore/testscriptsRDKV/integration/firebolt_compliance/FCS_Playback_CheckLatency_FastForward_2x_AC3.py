...
<xml>
  ...
  <name>FCS_Playback_CheckLatency_FastForward_2x_AC3</name>
  ...
  <test_cases>
    <test_case_id>FCS_Playback_104</test_case_id>
...
2. Test stream url for a live AC3 stream should be updated in the config variable video_src_url_ac3 inside MediaValidationVariables.py library inside filestore.
...
5. FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT configuration should be set to time to wait before checking for AV playback</pre_requisite>
    ...
    <input_parameters>testcasename - "test_trickplay_latency"
test_url - ac3 stream url from MediaValidationVariables library (MediaValidationVariables.video_src_url_ac3)
...
Checkpoint 2. Verify that the latency during playback is within the latency threshold</expected_output>
    ...
    <test_script>FCS_Playback_CheckLatency_FastForward_2x_AC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_CheckLatency_FastForward_2x_AC3')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_name = "test_trickplay_latency"
    ...
    #Sample command = "mediapipelinetests test_trickplay <AC3_STREAM_URL> checkavstatus=yes operations=fastforward2x:20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("AC3 Playback was successfull")
            parseLatency(tdkTestObj,latencyThresholdValue);
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("AC3 Playback failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
