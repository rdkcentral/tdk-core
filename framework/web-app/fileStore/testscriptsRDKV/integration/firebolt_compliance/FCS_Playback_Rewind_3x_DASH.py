...
<xml>
  ...
  <name>FCS_Playback_Rewind_3x_DASH</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_22</test_case_id>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Rewind_3x_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Rewind_3x_DASH');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    # There should be enough duration available for rewind operation to perform correctly, so seeking the stream for 3 * timeoutInSeconds + 30(buffer) duration. Ensure that the straem being tested is having adequate duration
    ...
    #Sample command = "mediapipelinetests test_trickplay <DASH_STREAM_URL> checkavstatus=yes operations=play:350,rewind3x:20"
