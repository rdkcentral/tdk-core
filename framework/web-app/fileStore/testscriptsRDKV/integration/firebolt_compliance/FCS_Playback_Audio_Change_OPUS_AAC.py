...
<xml>
  ...
  <name>FCS_Playback_Audio_Change_OPUS_AAC</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_127</test_case_id>
...
2. Test stream url for a multi codec AAC OPUS stream should be updated in the config variable video_src_url_opus_aac inside MediaValidationVariables.py library inside filestore.
...
3.Retrieve the video_src_url_opus_aac variable from MediaValidationVariables library
...
2.Check if Audio switch is happened properly.</expected_output>
    ...
    <test_script>FCS_Playback_Audio_Change_OPUS_AAC</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Audio_Change_OPUS_AAC');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_change <OPUS_AAC_URL> checkavstatus=yes timeout=10"
