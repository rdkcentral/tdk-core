...
<xml>
  ...
  <name>FCS_Playback_Same_Asset_Twice_HLS</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_51</test_case_id>
...
2. Test stream url for an HLS stream should be updated in the config variable video_src_url_hls inside MediaValidationVariables.py library inside filestore(We are initialising the pipeline and setting the url, but won't do playback)
...
2.Retrieve the video_src_url_hls variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the outputs returned from mediapipelinetests during both command executions, contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Same_Asset_Twice_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Same_Asset_Twice_HLS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("HLS Playback using 'playbin' and 'westeros-sink' was successfull in first attempt")
            ...
            if expectedResult in actualresult.upper() and output:
                ...
                if expectedResult in executionStatus:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print("HLS Playback using 'playbin' and 'westeros-sink' was successfull in second attempt")
                    print("Second Mediapipeline test executed successfully")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print("HLS Playback using 'playbin' and 'westeros-sink' failed during second attempt")
            else:
                tdkTestObj.setResultStatus("FAILURE")
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print("HLS Playback using 'playbin' and 'westeros-sink' failed in first attempt")
    else:
        tdkTestObj.setResultStatus("FAILURE")
