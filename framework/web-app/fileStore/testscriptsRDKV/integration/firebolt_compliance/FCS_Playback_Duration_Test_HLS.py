...
<xml>
  ...
  <name>FCS_Playback_Duration_Test_HLS</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_216</test_case_id>
    <test_objective>Test to do retrieve playback duration of a HLS stream through playbin gst element and westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an HLS stream should be updated in the config variable video_src_url_aac inside MediaValidationVariables.py library inside filestore
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Duration_Test_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Duration_Test_HLS')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_audio_duration <HLS_STREAM_URL> checkavstatus=yes timeout=20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print "HLS Playback Duration retrieved using successfully using gst_element_query_duration"
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print "HLS Playback Duration retrieval failed using gst_element_query_duration"
    else:
        tdkTestObj.setResultStatus("FAILURE")
