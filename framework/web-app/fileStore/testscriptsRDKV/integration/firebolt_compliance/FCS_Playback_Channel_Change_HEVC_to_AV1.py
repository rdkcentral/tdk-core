...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_HEVC_to_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from  HEVC stream to  AV1 stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_130</test_case_id>
    <test_objective>Test to do Channel Change from HEVC stream to  AV1 stream through playbin gst element and westerossink</test_objective>
...
test_url(s) - AV1 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_av1), HEVC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc)
...
3.Retrieve the video_src_url_hevc and video_src_url_av1  variable from MediaValidationVariables library
...
5.Execute the command in DUT. During the execution, the DUT will play HEVC STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to AV1 STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_HEVC_to_AV1</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_HEVC_to_AV1')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <HEVC_STREAM_URL> <AV1_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change from HEVC to AV1 was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change from HEVC to AV1 failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
