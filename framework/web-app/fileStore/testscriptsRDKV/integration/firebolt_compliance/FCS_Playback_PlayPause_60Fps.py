...
<xml>
  ...
  <name>FCS_Playback_PlayPause_60Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 60 fps stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_126</test_case_id>
    <test_objective>To test whether the framerate is rendered properly during play-pause scenario of pipeline for 60 fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an 60 fps stream should be updated in the config variable video_src_url_mp4_60fps inside MediaValidationVariables.py library inside filestore
...
6.Retrieve the video_src_url_mp4_60fps variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_60Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_60Fps')
