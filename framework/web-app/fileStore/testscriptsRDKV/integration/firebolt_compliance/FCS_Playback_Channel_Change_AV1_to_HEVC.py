...
<xml>
  ...
  <name>FCS_Playback_Channel_Change_AV1_to_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do Channel Change from  AV1 stream to  HEVC stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_129</test_case_id>
    <test_objective>Test to do Channel Change from AV1 stream to  HEVC stream through playbin gst element and westerossink</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for a HEVC stream  and AV1 stream should be updated in the config variable video_src_url_hevc and video_src_url_av1 respectively inside MediaValidationVariables.py library inside filestore
...
test_url(s) - HEVC url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc), AV1 url from MediaValidationVariables library (MediaValidationVariables.video_src_url_av1)
...
5.Execute the command in DUT. During the execution, the DUT will play AV1 STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT and then switch to HEVC STREAM for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_SECOND_CHANNEL_TIMEOUT  seconds then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Channel_Change_AV1_to_HEVC</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Channel_Change_AV1_to_HEVC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_channel_change_playback <AV1_STREAM_URL> <HEVC_STREAM_URL> checkavstatus=yes timeout=20,20"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Channel Change from AV1 to HEVC was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Channel Change from AV1 to HEVC failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
