...
<xml>
  ...
  <name>FCS_Playback_ResolutionSwitch_up_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do videoplayback by switching resolutions from very low (144p) to 4K resolution through 'playbin' and 'westerossink' gst elements</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_147</test_case_id>
    <test_objective>To test the video playback by switching resolutions from very low (144p) to 4K resolution through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator, RDK TV</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an mp4 stream should be updated in the config variable video_src_url_resolution_up_av1 inside MediaValidationVariables.py library inside filestore
...
test_url - url from MediaValidationVariables library (MediaValidationVariables.video_src_url_resolution_up_av1)
...
3.Retrieve the video_src_url_resolution_up_av1 variable from MediaValidationVariables library
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0", or "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_ResolutionSwitch_up_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_ResolutionSwitch_up_AV1')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_resolution_up_av1
    ...
    #Sample command = "mediapipelinetests test_resolution_up <video_src_url_resolution_up_av1> checkavstatus=yes timeout=20"
