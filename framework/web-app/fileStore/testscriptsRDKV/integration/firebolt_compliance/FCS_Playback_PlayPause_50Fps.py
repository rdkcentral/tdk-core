...
<xml>
  ...
  <name>FCS_Playback_PlayPause_50Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 50 fps stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_125</test_case_id>
    <test_objective>To test whether the framerate is rendered properly during play-pause scenario of pipeline for 50 fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an 50 fps stream should be updated in the config variable video_src_url_mp4_50fps inside MediaValidationVariables.py library inside filestore
...
test_url - hls url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_50fps)
...
6.Retrieve the video_src_url_mp4_50fps variable from MediaValidationVariables library
...
Checkpoint 3. Verify video playback happened at expected frame rate</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_50Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_50Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_50fps
    ...
    #Sample command = "mediapipelinetests test_playback_fps <50 fps_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("50Fps video playback  using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("50Fps video playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
