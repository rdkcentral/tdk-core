...
<xml>
  ...
  <name>FCS_Playback_PlaySeek_Forward_Stress_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to do multiple forward seek on a HLS stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_48</test_case_id>
    <test_objective>Test to do multiple forward seek on a HLS stream</test_objective>
...
5. FIREBOLT_COMPLIANCE_SEEK_STEP configuration should be set to step value in seconds with which the seek operation should be carried out each time increasing the seek position with step value staring from 0
...
2.test_url - HLS url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hls)
...
4.operations=seek:&lt;timeout&gt;:&lt;seekposition&gt;,seek:&lt;timeout&gt;:&lt;seekposition&gt;,seek:&lt;timeout&gt;:&lt;seekposition&gt;,seek:&lt;timeout&gt;:&lt;seekposition&gt;,seek:&lt;timeout&gt;:&lt;seekposition&gt;,seek:&lt;timeout&gt;:&lt;seekposition&gt; - a ":" seperated string to specify the seek operation to be executed , the time in seconds for which the operation should be performed and seekposition in seconds to which the seek operation should be performed. seekposition will be 0 + FIREBOLT_COMPLIANCE_SEEK_STEP in first seek, then it would be seekposition + FIREBOLT_COMPLIANCE_SEEK_STEP for the next seek operations, incrementing the seek position with FIREBOLT_COMPLIANCE_SEEK_STEP in each seek.The seek operation should be repeated for FIREBOLT_COMPLIANCE_STRESS_REPEAT_COUNT number of time.The timeout should be configured in the device configuration(FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT) from Device Config file The seek step should also be configured in device configuration(FIREBOLT_COMPLIANCE_SEEK_STEP).The seek repeat count should be configured in device configuration(FIREBOLT_COMPLIANCE_STRESS_REPEAT_COUNT)</input_parameters>
...
5.Execute the command in DUT. During the execution, the DUT will start av playback, then pipeline seeks to FIREBOLT_COMPLIANCE_SEEK_STEP and then av playback is performed for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds, then seeks to FIREBOLT_COMPLIANCE_SEEK_STEP + FIREBOLT_COMPLIANCE_SEEK_STEP and performs av playback for FIREBOLT_COMPLIANCE_MEDIAPLAYBACK_TIMEOUT seconds.This seek opeartion is repeated FIREBOLT_COMPLIANCE_STRESS_REPEAT_COUNT times, each time increasing the seek position by FIREBOLT_COMPLIANCE_SEEK_STEP seconds.Then application exits by closing the pipeline
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlaySeek_Forward_Stress_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlaySeek_Forward_Stress_HLS');
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_trickplay <HLS_STREAM_URL> checkavstatus=yes operations=seek:3:10,seek:3:20,seek:3:30,seek:3:40,seek:3:50,seek:3:60"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Forward seek stress on HLS stream was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Forward seek stress on HLS stream failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
