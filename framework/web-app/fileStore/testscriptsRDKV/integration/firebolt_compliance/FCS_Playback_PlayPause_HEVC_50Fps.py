...
<xml>
  ...
  <name>FCS_Playback_PlayPause_HEVC_50Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>verify play pause for HEVC 50 fps video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_163</test_case_id>
    <test_objective>To test whether the framerate is rendered properly during play-pause scenario of pipeline for HEVC 50 fps stream through 'playbin' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an hevc 50 fps stream should be updated in the config variable video_src_url_hevc_50fps inside MediaValidationVariables.py library inside filestore
...
test_url - hevc url from MediaValidationVariables library (MediaValidationVariables.video_src_url_hevc_50fps)
...
6.Retrieve the video_src_url_hevc_50fps variable from MediaValidationVariables library
...
Checkpoint 3. Verify video playback happened at expected frame rate</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_HEVC_50Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_HEVC_50Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_hevc_50fps
    ...
    #Sample command = "mediapipelinetests test_playback_fps <50 fps_HEVC_STREAM_URL> checkavstatus=yes timeout=30"
