...
<xml>
  ...
  <name>FCS_Playback_Standard_Bitrate_HEVC</name>
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_189</test_case_id>
    <test_objective>To verify whether the video is playing with standard bitrate using HEVC stream</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video Accelerator</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an AAC stream should be updated in the config variable video_src_url_bitrate_hevc inside MediaValidationVariables.py library inside filestore
...
test_url - aac url from MediaValidationVariables library (MediaValidationVariables.video_src_url_bitrate_hevc)
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_Standard_Bitrate_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_Standard_Bitrate_HEVC')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    #Sample command = "mediapipelinetests test_video_bitrate <HEVC_bitrate_STREAM_URL> checkavstatus=yes timeout=30"
    command = getMediaPipelineTestCommand (test_name, test_url, checkavstatus = checkAVStatus, timeout = timeoutInSeconds)
    command = command + " connection_speed="  + connection_speed + " height=" + height
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("Standard bitrate video playback was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("Standard bitarte video playback was failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
