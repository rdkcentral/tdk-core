...
<xml>
  ...
  <name>FCS_Playback_PlayPause_24Fps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the play-pause scenario of pipeline for 24 Fps stream through playbin gst element and westerossink</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>FCS_PLAYBACK_120</test_case_id>
    <test_objective>To test the play-pause scenario of pipeline for 24 Fps stream through 'playbin', 'fpsdisplaysink' and 'westerossink' gst elements</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDK TV,Video Accelerator, RPI</test_setup>
    <pre_requisite>1.TDK Agent should be up and running in the DUT
2. Test stream url for an HLS stream should be updated in the config variable video_src_url_mp4_24fps inside MediaValidationVariables.py library inside filestore
...
test_url - hls url from MediaValidationVariables library (MediaValidationVariables.video_src_url_mp4_24fps)
...
3.Retrieve the video_src_url_mp4_24fps variable from MediaValidationVariables library
...
8.Verify if video is playing at 24 Frames per second on average using fpsdisplaysink</automation_approch>
...
Checkpoint 2. Verify that the output returned from mediapipelinetests contains the strings "Failures: 0" and "Errors: 0" or it contains the string "failed: 0"</expected_output>
    ...
    <test_script>FCS_Playback_PlayPause_24Fps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysUtilObj.configureTestCase(ip,port,'FCS_Playback_PlayPause_24Fps')
...
if "SUCCESS" in sysutilloadModuleStatus.upper():
    ...
    test_url = MediaValidationVariables.video_src_url_mp4_24fps
    ...
    #Sample command = "mediapipelinetests test_playback_fps <HLS_STREAM_URL> checkavstatus=yes timeout=30"
    ...
    if expectedResult in actualresult.upper() and output:
        ...
        if expectedResult in executionStatus:
            ...
            print("24Fps video playback  using 'playbin' and 'westeros-sink' was successfull")
            print("Mediapipeline test executed successfully")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            print("24Fps video playback using 'playbin' and 'westeros-sink' failed")
    else:
        tdkTestObj.setResultStatus("FAILURE")
