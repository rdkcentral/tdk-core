...
<xml>
  ...
  <name>RDKV_CERT_SVS_Check_Kernel_Mmap_Min_Address</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Protect against kernel based NULL pointer attacks by enforcing a minimum value of mmap_min_addr.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_SECURITY_27</test_case_id>
    <test_objective>Protect against kernel based NULL pointer attacks by enforcing a minimum value of mmap_min_address
    </test_objective>
    ...
    <automation_approch>1.Check the value in /proc/sys/vm/mmap_min_addr
 2.Pass/fail the test based on the value in </automation_approch>
    <expected_output>If it is 4096 or greater, it is PASS else FAIL
</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_security</test_stub_interface>
    <test_script>RDKV_CERT_SVS_Check_Kernel_Mmap_Min_Address</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_SVS_Check_Kernel_Mmap_Min_Address');
