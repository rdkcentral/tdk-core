...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PVS_Functional_LightningApp_ResourceUsage_Launch</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_setPluginStatus</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to validate the resource usage after launching LightningApp plugin.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_77</test_case_id>
    <test_objective>The objective of this test is to validate the resource usage after launching LightningApp plugin.</test_objective>
...
2. Validate CPU load and memory usage using DeviceInfo.1.systeminfo method</automation_approch>
    <expected_output>LightningApp should launch and CPU load and memory usage must be within the expected range.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_performance</test_stub_interface>
    <test_script>RDKV_CERT_PVS_Functional_LightningApp_ResourceUsage_Launch</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_LightningApp_ResourceUsage_Launch')
