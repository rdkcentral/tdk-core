...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PVS_Functional_LightningApp_Reboot_OnLoadURL</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_setValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to load URL in LightningApp, reboot the device, check whether LightningApp is stable and able to load URL again in it once the device is online.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_85</test_case_id>
    <test_objective>The objective of this test is to load URL in LightningApp, reboot the device, check whether LightningApp is stable and able to load URL again in it once the device is online.</test_objective>
...
10. Destroy the plugin</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Functional_LightningApp_Reboot_OnLoadURL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_LightningApp_Reboot_OnLoadURL')
...
if expectedResult in result.upper():
    ...
    plugin = "LightningApp"
    plugins_list = ["Cobalt","WebKitBrowser","LightningApp"]
    plugin_status_needed = {"Cobalt":"deactivated","LightningApp":"deactivated","WebKitBrowser":"deactivated"}
    curr_plugins_status_dict = get_plugins_status(obj,plugins_list)
    time.sleep(10)
    status = "SUCCESS"
    if not lightningapp_test_url:
        print("\n Pls configure Lightningapp URL")
        status = "FAILURE"
                    ...
                    if expectedResult in result:
                        ...
                        if count == 1:
                            ...
                            while url_change_count < 2:
                                ...
                                if "urlchange" in event_log and lightningapp_test_url in event_log.replace("\\",""):
