...
<xml>
  ...
  <name>RDKV_CERT_PVS_Functional_ResourceUsage_Cobalt_Launch_StandbyToOn</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to calculate the resource usage during Cobalt launch in standby mode to ON</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_141</test_case_id>
    <test_objective>The objective of this test is to calculate the resource usage during Cobalt launch in standby mode to ON.</test_objective>
...
Resource usage should be within the expected range.</expected_output>
    ...
    <test_script>RDKV_CERT_PVS_Functional_ResourceUsage_Cobalt_Launch_StandbyToOn</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_ResourceUsage_Cobalt_Launch_StandbyToOn');
                    ...
                    if expectedResult in result and current_power_state == "ON":
                        ...
                        if cobal_launch_status == "SUCCESS":
                            ...
                            if Cobalt_status == 'resumed' and expectedResult in result:
                                ...
                                print("\n Error while checking Cobalt status, current status: ",Cobalt_status)
                                tdkTestObj.setResultStatus("FAILURE")
                        else:
