...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Destroy</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_setPluginStatus</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to validate the time taken to destroy LightningApp plugin.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_76</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to destroy LightningApp plugin.</test_objective>
...
2. Launch LightningApp using RDKShell
3. Save current system time and destroy LightningApp using destroy method of RDKShell plugin
...
5. Validate the output with threshold value</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Destroy</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Destroy')
    ...
    if status == "SUCCESS":
        ...
        if launch_status == expectedResult:
            ...
            if lightningapp_status == 'resumed' and expectedResult in result:
                ...
                time.sleep(10)
                #Deactivate LightningApp
                print("\n Destroying LightningApp \n")
                tdkTestObj = obj.createTestStep('rdkservice_setPluginStatus')
                tdkTestObj.addParameter("plugin","LightningApp")
                tdkTestObj.addParameter("status","deactivate")
                destroy_start_time = str(datetime.now()).split()[1]
                tdkTestObj.executeTestCase(expectedResult)
                result = tdkTestObj.getResult()
                if result == "SUCCESS":
                    tdkTestObj.setResultStatus("SUCCESS")
                    ...
                    if lightningapp_status == 'deactivated' and expectedResult in result:
                        print("\n LightningApp destroyed successfully")
                        tdkTestObj.setResultStatus("SUCCESS")
                        destroyed_time = ""
                        continue_count = 0
                        while True:
                            ...
                            if ("LightningApp" in event_log and "onDestroyed" in str(event_log)):
                                print("\n Event :onDestroyed is triggered during LightningApp destroy")
                                destroyed_time = event_log.split('$$$')[0]
                                break
                        if destroyed_time:
                            conf_file,file_status = getConfigFileName(obj.realpath)
                            config_status,htmlapp_destroy_threshold = getDeviceConfigKeyValue(conf_file,"LIGHTNINGAPP_DESTROY_THRESHOLD_VALUE")
                            Summ_list.append('LIGHTNINGAPP_DESTROY_THRESHOLD_VALUE :{}ms'.format(htmlapp_destroy_threshold))
                            ...
                            if all(value != "" for value in (htmlapp_destroy_threshold,offset)):
                                ...
                                Summ_list.append('Time taken to destroy LightningApp :{}ms'.format(time_taken_for_destroy))
                                print("\n Threshold value for time taken to destroy LightningApp: {}(ms)".format(htmlapp_destroy_threshold))
                                print("\n Validate the time:")
                            ...
                            print("\n onDestroyed event not triggered for during LightningApp destroy")
                            tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("\n Unable to destroy LightningApp, current status: ",lightningapp_status)
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print("\n Error while destroying LightningApp")
                    tdkTestObj.setResultStatus("FAILURE")
            else:
                print("\n Unable to launch LightningApp, current status: ",lightningapp_status)
                tdkTestObj.setResultStatus("FAILURE")
        else:
