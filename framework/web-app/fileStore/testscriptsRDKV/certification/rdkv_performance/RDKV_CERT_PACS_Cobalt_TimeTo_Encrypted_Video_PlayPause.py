...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PACS_Cobalt_TimeTo_Encrypted_Video_PlayPause</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_getRequiredLog</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to validate the time taken to play and pause an encrypted video on YouTube.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_124</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to play and pause an encrypted video on YouTube.</test_objective>
...
2. Set an encrypted video URL using deeplink method.
...
10. Deactivate the Cobalt plugin.
    ...
    <test_script>RDKV_CERT_PACS_Cobalt_TimeTo_Encrypted_Video_PlayPause</test_script>
    <skipped>No</skipped>
...
if expectedResult in result.upper():
    ...
    status = "SUCCESS"
    revert = "NO"
    cobalt_test_url = PerformanceTestVariables.encrypt_test_url
    if cobalt_test_url == "":
        print("\n Please configure the encrypt_test_url value\n")
