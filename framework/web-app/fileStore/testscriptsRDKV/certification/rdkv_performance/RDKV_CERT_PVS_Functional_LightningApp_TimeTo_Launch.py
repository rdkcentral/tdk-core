...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Launch</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_75</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to launch LightningApp plugin.</test_objective>
...
2. Save current system time and launch LightningApp using RDKShell.
...
4. Validate the output with threshold value</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Launch</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_LightningApp_TimeTo_Launch')
        ...
        if launch_status == expectedResult:
            ...
            if lightningapp_status == 'resumed' and expectedResult in result:
                ...
                if launched_time:
                    ...
                    config_status,lightningapp_launch_threshold = getDeviceConfigKeyValue(conf_file,"LIGHTNINGAPP_LAUNCH_THRESHOLD_VALUE")
                    Summ_list.append('LIGHTNINGAPP_LAUNCH_THRESHOLD_VALUE :{}ms'.format(lightningapp_launch_threshold))
