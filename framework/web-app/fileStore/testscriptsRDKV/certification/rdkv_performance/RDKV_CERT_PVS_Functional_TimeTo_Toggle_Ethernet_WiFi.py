...
<xml>
  ...
  <name>RDKV_CERT_PVS_Functional_TimeTo_Toggle_Ethernet_WiFi</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the time to switch to ethernet from 2.4Ghz and vice versa.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_47</test_case_id>
    <test_objective>The objective of this test is to validate the time to switch to ethernet from 2.4Ghz and vice versa.</test_objective>
...
3a). If new interface is WIFI, connect to 2.4GHZ SSID, get the current system time and set WIFI as default interface.
...
b) Get netsrvmgr logs to calculate the time taken to set WIFI as default interface,
5. Validate the time taken in both cases with threshold values.
</automation_approch>
    <expected_output>The time taken must be within the expected range.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_performance</test_stub_interface>
    <test_script>RDKV_CERT_PVS_Functional_TimeTo_Toggle_Ethernet_WiFi</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Functional_TimeTo_Toggle_Ethernet_WiFi');
