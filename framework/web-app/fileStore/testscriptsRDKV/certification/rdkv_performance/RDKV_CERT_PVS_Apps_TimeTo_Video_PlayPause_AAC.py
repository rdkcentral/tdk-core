...
<xml>
  ...
  <name>RDKV_CERT_PVS_Apps_TimeTo_Video_PlayPause_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This script is to get the performance of the Lightning application by calculating the time taken for play and pause operations of aac video.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_112</test_case_id>
    <test_objective>This script is to get the performance of the Lightning application by calculating the time taken for play and pause operations of aac video.</test_objective>
...
3. Get the time taken to play/pause the video</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Apps_TimeTo_Video_PlayPause_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Apps_TimeTo_Video_PlayPause_AAC');
...
if expectedResult in result.upper():
    ...
    videoURL_type = MediaValidationVariables.aac_url_type
