...
<xml>
  ...
  <name>RDKV_CERT_PVS_Browser_LightningApp_ResourceUsage_LoadInvalidURL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the resource usage after loading Invalid URL in Lightning App and check whether Lightning App is stable, and no crash is observed.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_PERFORMANCE_143</test_case_id>
    <test_objective>The objective of this test is to validate the resource usage after loading InvalidURL in Lightning App and check whether Lightning App is stable, and no crash is observed.</test_objective>
    ...
    <input_parameters>lightning_page_url: string</input_parameters>
    <automation_approch>1. Launch Lightning App using RDKShell
2. Set a Invalid URL in LightningApp using LightningApp.1.url method.
3. Verify whether existing URL is changed 
4. Validate the resource usage by DeviceInfo.1.systeminfo method
5.Check for crashes
6. Revert the status of Lightning App.</automation_approch>
    ...
    <test_script>RDKV_CERT_PVS_Browser_LightningApp_ResourceUsage_LoadInvalidURL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_PVS_Browser_LightningApp_ResourceUsage_LoadInvalidURL')
                    ...
                    if current_url != None and expectedResult in result:
                        ...
                        if expectedResult in result:
                            ...
                            if output != "EXCEPTION" and "Failed to load resource" in output and url_to_grep in output:
                                print("\n Invalid URL is not loaded in LightningApp \n")
                                ...
                                if not crash_status :
                                    print("Crash is not observed while loading invalid URL in LightningApp")
                                    print("\n Validate the status of LightningApp plugin :\n")
                                    tdkTestObj = obj.createTestStep('rdkservice_getPluginStatus')
                                    tdkTestObj.addParameter("plugin","LightningApp")
                                    #Execute the test case in DUT
                                    tdkTestObj.executeTestCase(expectedResult);
                                    ...
                                    if output != 'deactivated':
                                        ...
                                        print("LightningApp  got deactivated automatically while loading an invalid URL")
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
                        ...
                        print("\nUnable to set URL in LightningApp")
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print("\nFailed to get the URL in LightningApp")
                    tdkTestObj.setResultStatus("FAILURE")
            else:
