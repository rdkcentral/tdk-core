...
<xml>
  ...
  <name>RDKV_CERT_RVS_Cobalt_Hibernate_MultipleApps</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test validates the memory usage of multiple apps and compares them before and after hibernated state</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_72</test_case_id>
    <test_objective>This test validates the memory usage of multiple apps and compares them before and after hibernated state</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI, Accelerator</test_setup>
    <pre_requisite>Wpeframework process should be up and running in the device.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>1. hibernate_test_duration
2. Maximum allowed Memory usage</input_parameters>
    <automation_approch>1. As prerequisite, disable all the other plugins and enable Cobalt only. and get memory usage
2. Hibernate Cobalt by suspending it using RDKShell plugin and get memory usage 
3. Restore Cobalt using RDKShell plugin.
4.Launch YouTube tv app and get memory usage
5. Hibernate YouTube tv by suspending it using RDKShell plugin and get memory usage 
6. Check if the Memory usage is within the expected value.
7. Revert all values</automation_approch>
    <expected_output>All apps should be successfully hibernated and restored, and memory usage is within expected limit</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_stability</test_stub_interface>
    <test_script>RDKV_CERT_RVS_Cobalt_Hibernate_MultipleApps</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_Cobalt_Hibernate_MultipleApps');
...
if expectedResult in (result.upper() and pre_condition_status):    
    ...
    print("\n######################## Execution Summary#######################\n")
    if validation ==True:
        print(f"SUCCESS: After hibernating the app, its memory consumption decreased to {Cobalt_AfterHibernate_appusage} kb from {Cobalt_BeforeHibernate_appusage} kb before hibernating.\n")
        print(f"SUCCESS: After hibernating the app, its memory consumption decreased to {YoutubeTV_AfterHibernate_appusage} kb from {YoutubeTV_BeforeHibernate_appusage} kb before hibernating.\n")
    else:
        print(f"FAILURE: After hibernating the app, its memory consumption decreased  to {Cobalt_AfterHibernate_appusage} kb from {Cobalt_BeforeHibernate_appusage} kb before hibernating but it is not under threshold limit.\n")
        print(f"FAILURE: After hibernating the app, its memory consumption decreased  to {YoutubeTV_AfterHibernate_appusage} kb from {YoutubeTV_BeforeHibernate_appusage} kb before hibernating but it is not under threshold limit.\n")
