...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_RVS_WebKitBrowser_LoadURL_OnStressLaunch</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateResourceUsage</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to launch and exit WebKit for 99 times. On 100th time load webkit and verify if the load URL is happening properly.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_50</test_case_id>
    <test_objective>The objective of this test is to launch and exit WebKit for 99 times. On 100th time load webkit and verify if the load URL is happening properly.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI,Accelerator</test_setup>
    <pre_requisite>1. wpeframework should be up and running</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>webkit_test_url:string</input_parameters>
...
6. Launch WebKitBrowser using RDKShell.
7. Check the status
8. Set a URL
...
10. Destroy the plugin
    ...
    <test_script>RDKV_CERT_RVS_WebKitBrowser_LoadURL_OnStressLaunch</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_WebKitBrowser_LoadURL_OnStressLaunch');
...
if expectedResult in (result.upper() and pre_condition_status):
    ...
    if status == "SUCCESS" and webkit_test_url != "":
        ...
        json_file.close()
        status = set_plugins_status(obj,webkit_post_condition)
        time.sleep(10)
