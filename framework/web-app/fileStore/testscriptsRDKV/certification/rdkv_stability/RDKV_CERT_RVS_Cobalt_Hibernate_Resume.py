...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_CERT_RVS_Cobalt_Hibernate_Resume</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateResourceUsage</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to hibernate and resume the cobalt plugin for a given number of times.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_67</test_case_id>
    <test_objective>The objective of this test is to hibernate and resume the Cobalt plugin for a given number of times.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>hibernate_resume_max_count : int</input_parameters>
...
4.Resume Cobalt using RDKShell plugin.
5. Check if the CPU load and Memory usage is within the expected value.
6. Repeat steps 2 to 4 for given number of times.
7. Revert all values.</automation_approch>
    <expected_output>Hibernate and Resume should happen and the cpu load and memory usage must be within the expected values.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_stability</test_stub_interface>
    <test_script>RDKV_CERT_RVS_Cobalt_Hibernate_Resume</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_Cobalt_Hibernate_Resume');
...
result_dict_list = []
cpu_mem_info_dict = {}
max_count = StabilityTestVariables.hibernate_resume_max_count
        ...
        while iteration < max_count:
            ...
            if result == expectedResult:
                ...
                if cobalt_status == 'hibernated' and expectedResult in result:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print("\nCobalt plugin hibernated successfully\n")
                    print("\nRestore the cobalt plugin\n")                    
                    params = '{"callsign":"Cobalt"}'
                    tdkTestObj = obj.createTestStep('rdkservice_setValue')
                    tdkTestObj.addParameter("method","org.rdk.RDKShell.1.restore")
                    tdkTestObj.addParameter("value",params)
                    tdkTestObj.executeTestCase(expectedResult)
                    time.sleep(5)
                    result = tdkTestObj.getResult()
                    if result == expectedResult:
                        ...
                        if cobalt_status == 'suspended' and expectedResult in result:
                            print("\nCobalt plugin restored successfully now trying to resume cobalt\n")
                            ...
                            print("Cobalt plugin is not in suspended state, current Cobalt Status: ",cobalt_status)
                            tdkTestObj.setResultStatus("FAILURE")
                            completed = False
                            break
                    else:
                        print("Unable to restore Cobalt plugin to resumed state")
                        tdkTestObj.setResultStatus("FAILURE")
                        completed = False
                        break
                else:
                    print("Cobat is not in hibernated state, current Cobalt Status: ",cobalt_status)
                    tdkTestObj.setResultStatus("FAILURE")
                    completed = False
                    break
            else:
                print("Unable to set Cobalt plugin to hibernated state")
                tdkTestObj.setResultStatus("FAILURE")
                completed = False
                break
        if(completed):
