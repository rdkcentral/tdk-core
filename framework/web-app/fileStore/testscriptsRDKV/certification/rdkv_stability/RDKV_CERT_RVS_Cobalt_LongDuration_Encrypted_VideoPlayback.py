...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>RDKV_CERT_RVS_Cobalt_LongDuration_Encrypted_VideoPlayback</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateProcEntry</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to do the stability testing by playing an encrypted video from Cobalt for a given amount of time and get the cpu load in every 5 mins.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to do the stability testing by playing an encrypted video from Cobalt for a given amount of time and get the cpu load in every 5 mins.</test_objective>
...
6. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_RVS_Cobalt_LongDuration_Encrypted_VideoPlayback</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_Cobalt_LongDuration_Encrypted_VideoPlayback');
...
if expectedResult in (result.upper() and pre_condition_status):
    encrypt_test_url = StabilityTestVariables.encrypt_test_url;
    ...
    if status == "SUCCESS" and cobalt_launch_status == "SUCCESS" and validation_dict != {} and ssh_param_dict != {} :
        time.sleep(30)
        print("\nPre conditions for the test are set successfully")
        print("\n Set the URL : {} using Cobalt deeplink method".format(encrypt_test_url))
        tdkTestObj = obj.createTestStep('rdkservice_setValue')
        tdkTestObj.addParameter("method","Cobalt.1.deeplink")
        tdkTestObj.addParameter("value",encrypt_test_url)
        ...
        if(cobalt_result == expectedResult):
            ...
            if "SUCCESS" == (result1 and result2):
                tdkTestObj.setResultStatus("SUCCESS")
                test_time_in_mins = int(StabilityTestVariables.encrypt_test_duration)
                ...
                while int(round(time.time() * 1000)) < time_limit:
                    ...
                    #Check in every half an hour for video duration
                    encrypt_video_duration = int(StabilityTestVariables.encrypt_video_duration)
                    if ((iteration % 6) == 0):
                        video_played_time = video_played_time + 30
                        if(encrypt_video_duration < video_played_time):
                            tdkTestObj = obj.createTestStep('rdkservice_setValue')
                            tdkTestObj.addParameter("method","Cobalt.1.deeplink")
                            tdkTestObj.addParameter("value",encrypt_test_url)
                            tdkTestObj.executeTestCase(expectedResult)
                            ...
                            if result == "SUCCESS":
                                ...
                                print("Unable to reload the url")
                                tdkTestObj.setResultStatus("FAILURE")
                                completed = False
                                break
                if(completed):
