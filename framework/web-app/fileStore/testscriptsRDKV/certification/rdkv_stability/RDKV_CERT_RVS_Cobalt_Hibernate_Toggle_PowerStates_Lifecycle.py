...
<xml>
  ...
  <name>RDKV_CERT_RVS_Cobalt_Hibernate_Toggle_PowerStates_Lifecycle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to perform continuous toggling of power states as ON and Standby for given number of times when cobalt is present in Hibernate state and check whether cobalt state persisted over toggling of power states and perform cobalt lifecycle management after cobalt restore</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_75</test_case_id>
    <test_objective>The objective of this test is to perform continuous toggling of power states as ON and Standby for given number of times when cobalt is present in Hibernate state and check whether cobalt state persisted over toggling of power states and perform cobalt lifecycle management after cobalt restore</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI,Accelerator</test_setup>
    <pre_requisite> Wpeframework process should be up and running in the device.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters> max_power_state_changes</input_parameters>
    <automation_approch>1.Set cobalt in Hibernated state
2. Check the current power state and current Preferred Standby mode
In a loop given number of times
3. Set the preferred mode as LIGHT SLEEP
4. Check if the preferred mode is LIGHTSLEEP
5. If LIGHTSLEEP, toggle the power state.
6. Get power state and confirm
7.Check cobalt Hibernated state is persisted
8. Restore cobalt app and perform cobalt lifecycle management.
9. Check if the CPU and Memory usage is within 90%.
</automation_approch>
    <expected_output>The cobalt should persist in hibernate state and device must be stable after power toggling stress test and should successfully pass lifecycle management process</expected_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_Cobalt_Hibernate_Toggle_PowerStates_Lifecycle');
                ...
                if expectedResult in result:
                   ...
                   if expectedResult in result:
                       ...
                       for count in range(0,max_powerstate_changes):
                               ...
                               if expectedResult in result and preferred_standby == "LIGHT_SLEEP":
                                  ...
                                  if expectedResult in result:
                                     ...
                                     if expectedResult in result and current_power_state == new_power_state:
                                         ...
                                         tdkTestObj.setResultStatus("SUCCESS")
                                         print("\n Validating Cobalt current state after change in power state")
                                         tdkTestObj = obj.createTestStep('rdkservice_getPluginStatus')
                                         tdkTestObj.addParameter("plugin","Cobalt")
                                         tdkTestObj.executeTestCase(expectedResult)
                                         result = tdkTestObj.getResult()
                                         ...
                                         if cobalt_status == 'hibernated' and expectedResult in result:
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print("\n Cobalt remained in Hibernated state during power state change")
                                         else:
                                             print("\n Cobalt failed to remain in hibernated state during power state change")
                                             tdkTestObj.setResultStatus("FAILURE")
                                             break
                                         result_dict = {}
                                         #get the cpu load
                           ...
                           print("\n Successfully completed {} iterations \n".format(max_powerstate_changes,"cobalt state remained:",cobalt_status))
                       ...
                       if cobalt_status == 'hibernated':
                          ...
                          if restore_status == expectedResult:
                             ...
                             if cobalt_status == 'suspended' and expectedResult in result:
                                ...
                                if result == "SUCCESS" and cobalt_test_url != "" and validation_dict != {}:
                                   tdkTestObj.setResultStatus("SUCCESS")                                  
