...
<xml>
  ...
  <name>RDKV_CERT_RVS_Cobalt_Hibernate_Reboot_VideoPlayback</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to Hibernate cobalt and calculate the resource usage of the device after rebooting, launching Cobalt and initiating video playback multiple times</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_69</test_case_id>
    <test_objective>The objective of this test is to Hibernate cobalt and calculate the resource usage of the device after rebooting, launching Cobalt and initiating video playback multiple times</test_objective>
...
4. Maximum allowed Memory usage</input_parameters>
    <automation_approch>1. As a prerequisite, disable all the other plugins and enable Cobalt only
2.Hibernate the cobalt app
3.Reboot the device.
4. Set the URL of video to be played after launching cobalt
5. Validate if the video is playing using proc entries
6. Validate CPU load
7. Validate memory usage
8. Repeat steps 3 to 7 in a loop for configured number of iterations</automation_approch>
    ...
    <test_stub_interface>The resource usage calculated after multiple reboots should be within the expected limit.</test_stub_interface>
    <test_script>RDKV_CERT_RVS_Cobalt_Hibernate_Reboot_VideoPlayback</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_Cobalt_Hibernate_Reboot_VideoPlayback');
...
if expectedResult in (result.upper() and pre_condition_status):
    ...
    plugin_status_needed = {"WebKitBrowser":"deactivated","DeviceInfo":"activated","Cobalt":"resumed"}
    ...
    if status == "SUCCESS" and validation_dict != {} and ssh_param_dict != {} :
        ...
        while iteration <= reboot_times:
            ...
            if cobalt_launch_status in expectedResult:
               print("\n Cobalt launched successfully ,trying to hibernate it")
                            ...
                            if "SUCCESS" == (result1 and result2):
                                ...
                                if validation_dict["validation_required"]:
                                   ...
                                   if result_val == "SUCCESS" :
                                      print("result_val",result_val,"validation_dict",validation_dict)
                                      tdkTestObj.setResultStatus("SUCCESS")
                                    ...
                                    print("Proc validation is None so proceeding with wpeframework validation\n")
