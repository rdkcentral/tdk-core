...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>RDKV_CERT_RVS_WebKitBrowser_LifeCycleManagement</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_executeLifeCycle</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to launch WebKitBrowser then set URL, suspend, resume, move behind and front then destroy. The script will repeat this for a minimum of 1000 times.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_38</test_case_id>
    <test_objective>The objective of this test is to launch WebKitBrowser then set URL, suspend, resume, move behind and front then destroy. The script will repeat these operations for a minimum of 1000 times.</test_objective>
...
2. Set given URL using WebKitBrowser.1.url method.
...
9. Validate the CPU load and memory usage using DeviceInfo.1.systeminfo method
    ...
    <test_script>RDKV_CERT_RVS_WebKitBrowser_LifeCycleManagement</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_WebKitBrowser_LifeCycleManagement');
...
if expectedResult in (result.upper() and pre_condition_status):
    ...
    plugins_list = ["WebKitBrowser","DeviceInfo"]
    print("\n Check Pre conditions")
    plugin_status_needed = {"WebKitBrowser":"deactivated","DeviceInfo":"activated"}
