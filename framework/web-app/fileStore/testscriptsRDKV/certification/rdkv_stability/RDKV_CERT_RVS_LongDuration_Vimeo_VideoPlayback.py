...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>RDKV_CERT_RVS_LongDuration_Vimeo_VideoPlayback</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateProcEntry</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to do the stability testing by playing a video from Vimeo for a given amount of time and get the cpu load in every 5 mins.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to do the stability testing by playing a video from Vimeo for a given amount of time and get the cpu load in every 5 mins.</test_objective>
...
4. Maximum allowed Memory usage</input_parameters>
    <automation_approch>1. As a pre requisite, disable all the other plugins and enable webkitinstance.
...
6. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_RVS_LongDuration_Vimeo_VideoPlayback</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_LongDuration_Vimeo_VideoPlayback');
...
if expectedResult in (result.upper() and pre_condition_status):
    vimeo_test_url = StabilityTestVariables.vimeo_test_url;
    ...
    if any(curr_plugins_status_dict[plugin] == "FAILURE" for plugin in plugins_list):
        ...
        if new_plugins_status != plugin_status_needed:
            status = "FAILURE"
    launch_status,start_time = launch_plugin(obj,webkit_instance)
    validation_dict = get_validation_params(obj)
    if status == "SUCCESS" and launch_status == "SUCCESS" and validation_dict != {} :
        time.sleep(30)
        print("\nPre conditions for the test are set successfully")
        print("\n Set the URL : {} using".format(vimeo_test_url))
        ...
        if(result == expectedResult):
            ...
            params = '{"keys":[ {"keyCode": 13,"modifiers": [],"delay":1.0,"callsign": '+webkit_instance+',"client": '+webkit_instance+'}]}'
            tdkTestObj = obj.createTestStep('rdkservice_setValue')
            ...
            if "SUCCESS" == result1:
                tdkTestObj.setResultStatus("SUCCESS")
                test_time_in_mins = int(StabilityTestVariables.vimeo_test_duration)
