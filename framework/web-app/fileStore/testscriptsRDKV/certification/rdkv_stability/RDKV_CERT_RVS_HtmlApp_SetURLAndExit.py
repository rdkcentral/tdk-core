...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_RVS_HtmlApp_SetURLAndExit</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateResourceUsage</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to continuously Launch HtmlApp plugin, load a URL and exit for a minimum of 1000 times</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_58</test_case_id>
    <test_objective>The objective of this test is to continuously Launch HtmlApp plugin, load a URL and exit for a minimum of 1000 times</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI, Accelerator</test_setup>
    <pre_requisite>1.wpeframework should be up and running</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>html_page_url : string
launch_and_destroy_max_count:integer</input_parameters>
    <automation_approch>In a loop of 1000:
1. Launch HtmlApp using launch method of  RDKShell
2. Check status of HtmlApp
3. Set the given URL
4. Destroy HtmlApp using destroy method of RDKShell
5. Validate the resource usage using DeviceInfo.1.systeminfo
    ...
    <test_script>RDKV_CERT_RVS_HtmlApp_SetURLAndExit</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_HtmlApp_SetURLAndExit')
    ...
    if status == "SUCCESS" and htmlapp_test_url != "":
        ...
        for count in range(0,max_iterations):
            ...
            if expectedResult in result and details == "SUCCESS" :
                print("\n Successfully completed launching and setting URL in HtmlApp")
