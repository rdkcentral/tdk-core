...
<xml>
  ...
  <name>RDKV_CERT_RVS_HtmlApp_LaunchURL_StressTest</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to stress test by loading URL in HtmlApp for 100 times without destroying the application.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_79</test_case_id>
    <test_objective>The objective of this test is to stress test by loading URL in HtmlApp for 100 times without destroying the application. </test_objective>
    ...
    <automation_approch>1. As a prerequisite disable all plugins and enable HtmlApp and DeviceInfo plugins
...
3. Revert the plugins</automation_approch>
    ...
    <test_script>RDKV_CERT_RVS_HtmlApp_LaunchURL_StressTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>  
port = <port>  
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_HtmlApp_LaunchURL_StressTest');
...
if expectedResult in (result.upper() and pre_condition_status):
    ...
    revert = "NO"
    plugins_list = ["HtmlApp", "Cobalt", "DeviceInfo"]
    plugin_status_needed = {"HtmlApp": "resumed", "Cobalt": "deactivated", "DeviceInfo": "activated"}
    ...
    if status == "SUCCESS" and test_url != "":
        ...
        if current_url != None and expectedResult in result:
            ...
            for count in range(1, max_count):
                ...
                if result == "SUCCESS":
                    ...
                    if test_url in new_url and expectedResult in result:
                        print(f"URL : {test_url} is set in HtmlApp")
                        tdkTestObj.setResultStatus("SUCCESS")
                    else:
            ...
            tdkTestObj.setResultStatus("FAILURE")
            print("\nUnable to get the current URL loaded in HtmlApp")
    else:
