...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_RVS_LightningApp_SuspendResume</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_validateResourceUsage</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to suspend and resume the LightningApp plugin for a minimum of  1000 times.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_STABILITY_52</test_case_id>
    <test_objective>The objective of this test is to suspend and resume the LightningApp plugin for a minimum of  1000 times.</test_objective>
    ...
    <automation_approch>1. Launch the LightningApp using RDKShell plugin
In a loop of 1000:
2. Suspend LightningApp using org.rdk.RDKShell.1.suspend method
3. Check the status of LightningApp
4. Resume LightningApp plugin using org.rdk.RDKShell.1.launch method
5. Check the status of LightningApp
6. Validate the resource usage using DeviceInfo.1.systemInfo</automation_approch>
    ...
    <test_script>RDKV_CERT_RVS_LightningApp_SuspendResume</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_LightningApp_SuspendResume')
        ...
        for count in range(0,max_count):
            ...
            if suspend_status == expectedResult:
                ...
                if lightningapp_status == 'suspended' and expectedResult in result:
                    ...
                    print("\n LightningApp plugin suspended successfully")
                    time.sleep(5)
                    ...
                    if resume_status == expectedResult:
                        ...
                        if lightningapp_status == 'resumed' and expectedResult in result:
                            print("\nLightningApp plugin resumed successfully\n")
                            tdkTestObj.setResultStatus("SUCCESS")
                            result_dict = {}
                            ...
                            print("\n LightningApp plugin is not in resumed state, current LightningApp status: ",lightningapp_status)
                            tdkTestObj.setResultStatus("FAILURE")
                            break
                    else:
