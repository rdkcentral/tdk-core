...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_CERT_RVS_HtmlApp_LifeCycleManagement</name>
  <primitive_test_id/>
  <primitive_test_name>rdkservice_executeLifeCycle</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>The objective of this test is to launch HtmlApp plugin then set URL, suspend, resume, move behind and front then destroy. The script will repeat this for a minimum of 1000 times.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>The objective of this test is to launch HtmlApp plugin then set URL, suspend, resume, move behind and front then destroy. The script will repeat this for a minimum of 1000 times.</test_objective>
...
9. Validate the CPU load and memory usage using DeviceInfo.1.systeminfo method
    ...
    <test_script>RDKV_CERT_RVS_HtmlApp_LifeCycleManagement</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_RVS_HtmlApp_LifeCycleManagement');
...
if expectedResult in (result.upper() and pre_condition_status) :
    ...
    html_app_test_url = obj.url+'/fileStore/lightning-apps/VideoResizeTest.html'
    plugins_list = ["DeviceInfo","HtmlApp"]
    print("\n Check Pre conditions")
    curr_plugins_status_dict = get_plugins_status(obj,plugins_list)
    time.sleep(20)
    plugin_status_needed = {"HtmlApp":"deactivated","DeviceInfo":"activated"}
