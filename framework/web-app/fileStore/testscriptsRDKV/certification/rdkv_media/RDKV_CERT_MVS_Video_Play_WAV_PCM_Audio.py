...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_WAV_PCM_Audio</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation of basic .wav pcm audio stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_203</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation of basic .wav pcm audio stream for few minutes and close the player</test_objective>
...
audio_src_url_wav_pcm: string
audio_close_interval: int</input_parameters>
...
3. Launch webkit instance with video test app with the audio src url and duration for close.
4. App starts playing the wav pcm audio stream and closes the player after the provided duration.
5. If expected event playing is observed then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Video Player should play the wav pcm audio stream for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_WAV_PCM_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_WAV_PCM_Audio')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        audioURL  = MediaValidationVariables.audio_src_url_wav_pcm
        # Setting VideoPlayer Operations
        setOperation("close",MediaValidationVariables.audio_close_interval)
        operations = getOperations()
        # Setting VideoPlayer test app URL arguments
        setURLArgument("url",audioURL)
        setURLArgument("operations",operations)
        setURLArgument("autotest","true")
        setURLArgument("type","wav")
        appArguments = getURLArguments()

        # Getting the complete test app URL for selected players
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_audio_wav_pcm).split(",")
        ...
        #url=<audio_url>.wav&operations=close(60)&autotest=true&type=wav
        ...
        for video_test_url in video_test_urls:
            ...
            if "SUCCESS" in launch_status:
                # Monitoring the app progress, checking whether app plays the audio properly or any hang detected in between,
                ...
                if "SUCCESS" in test_result and "FAILURE" not in proc_check_list:
                    print("Audio play is fine")
                    print("[TEST EXECUTION RESULT]: SUCCESS")
                    tdkTestObj.setResultStatus("SUCCESS");
                elif "SUCCESS" in test_result and "FAILURE" in proc_check_list:
                    print("Decoder proc entry check returns failure.Audio not playing fine")
