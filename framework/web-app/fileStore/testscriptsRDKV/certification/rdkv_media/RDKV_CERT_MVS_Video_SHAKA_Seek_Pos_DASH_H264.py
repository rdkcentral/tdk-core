...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek to position operation of dash h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_726</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video seek to position operation of dash h264 video codec content</test_objective>
...
4. Launch webkit instance with shaka test app url with the src url, operations to be performed, seek position with forward and backward positions.
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player should perform seek to position of h264 dash stream and seeking, seeked events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Seek_Pos_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_dash_h264_url>.mpd&operations=seekpos(30:120),seekpos(30:75),close(30)&options=checkInterval(10),loop&autotest=true
