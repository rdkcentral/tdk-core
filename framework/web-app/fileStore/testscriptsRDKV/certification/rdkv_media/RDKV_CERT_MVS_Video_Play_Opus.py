...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Opus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of Opus codec stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_57</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of Opus codec stream for few minutes and close the player           </test_objective>
...
video_src_url_opus: string
opus_url_type:string
close_interval: int
</input_parameters>
...
4. App starts playing the Opus stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_Opus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Opus')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_opus
        ...
        setURLArgument("type",MediaValidationVariables.opus_url_type)
        ...
        #url=<video_opus_url>.mpd&operations=close(60)&autotest=true&type=dash
