...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_TrickPlay_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video trickplay operations of dash h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_746</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video trickplay operations of dash h264 video codec content</test_objective>
...
3. Launch webkit instance with shaka test app url with the operations to be performed, play, pause, seek forward/backward and fast-forward with interval.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_SHAKA_TrickPlay_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_TrickPlay_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<dash_h264_url>.mpd&operations=seekfwd(10),fastfwd(10),fastfwd(10),playnow(10),pause(10),play(10),seekbwd(10),fastfwd(10)&options=checkInterval(5),loop&autotest=true
