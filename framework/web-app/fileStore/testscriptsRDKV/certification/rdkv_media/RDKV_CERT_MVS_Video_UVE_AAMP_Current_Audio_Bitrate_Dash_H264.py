...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Current_Audio_Bitrate_Dash_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of H.264 codec dash stream for few minutes, get the current audio bitrate and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_309</test_case_id>
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of H.264 codec dash stream for few minutes,get current audio bitrate and close the player</test_objective>
...
4. App starts playing the h.264 dash stream video, gets the current audio bitrate and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>UVE Player should play the video for provided duration, expected event playing should occur, fetch the current audio bitrate and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Current_Audio_Bitrate_Dash_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Current_Audio_Bitrate_Dash_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        setOperation("curraudiobitrate","20")
        ...
        #url=<video_h264_url>.mpd&operations=curraudiobitrate(20)&autotest=true
