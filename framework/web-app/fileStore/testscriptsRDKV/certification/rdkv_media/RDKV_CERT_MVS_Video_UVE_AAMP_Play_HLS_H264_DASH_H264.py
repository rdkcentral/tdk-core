...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_H264_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of H.264 codec hls stream for few seconds, then switch to H.264 codec dash stream for few seconds and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_197</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of H.264 codec hls stream for few seconds, then switch to H.264 codec dash stream for few seconds and close the player</test_objective>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the h264 hls stream and switch to h264 dash stream after 60 seconds
4. App performs the play operations of h264 hls stream and h264 dash stream validates each operation using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the h264 hls stream and switch to h264 dash  stream and play for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_H264_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_HLS_H264_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        secondaryURL  = MediaValidationVariables.video_src_url_dash_h264
        # Setting VideoPlayer Operations
        # Play the primary H264 hls stream URL,after 60 sec switch to secondary H264 dash stream URL, play for 60 sec & close
        ...
        #url=<video_h264_url>.m3u8&operations=changeurl(30),close(30),options=secondaryURL(<video_h264_url>.mpd)&autotest=true
