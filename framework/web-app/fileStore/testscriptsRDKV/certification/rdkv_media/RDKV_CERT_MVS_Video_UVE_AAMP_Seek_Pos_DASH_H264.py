...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_Pos_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek pos operation of dash content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_324</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek pos operation of dash content</test_objective>
...
3. Launch webkit instance with uve test app with the video src url, operations to be performed, seek pos with interval and duration for close.
4. App starts playing the h.264 hls stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>All the provided player operations and expected events seeking, seeked  should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_Pos_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Seek_Pos_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=seekfwd(30)seekbwd(30)&autotest=true
