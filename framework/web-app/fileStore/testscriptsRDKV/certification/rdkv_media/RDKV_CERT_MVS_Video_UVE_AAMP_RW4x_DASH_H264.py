...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_RW4x_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video rewind operation in 4x speed  and continue video play of h264 dash content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_159</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video rewind operation in 4x speed  and continue video play of h264 dash content and close the player</test_objective>
...
video_src_url_dash_h264_iframe: string</input_parameters>
...
3. Launch webkit instance with uve test app with the src url, operations to be performed, rewind 4x  with interval and duration for close.
...
7. Revert all values</automation_approch>
    <expected_output>Video should be rewind in 4x speed and video play should be continued in 1x speed, expected event ratechange should occur for rw4x and play in 1x speed and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_RW4x_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_RW4x_DASH_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_dash_h264_iframe
        # Setting VideoPlayer Operations
        setOperation("rewind4x","180")
        setOperation("close","30")
        operations = getOperations()
        # Setting VideoPlayer test app URL arguments
        setURLArgument("url",videoURL)
        setURLArgument("operations",operations)
        setURLArgument("options","checkInterval(7)")
        ...
        #url=<video_url>.mpd&operations=rewind4x(180),close(30)&options=checkInterval(7)&autotest=true
        ...
        if "SUCCESS" in launch_status:
            ...
            test_result,proc_check_list = monitorVideoTest(obj,webkit_console_socket,validation_dict,"Video Player Rate Change to 1");
