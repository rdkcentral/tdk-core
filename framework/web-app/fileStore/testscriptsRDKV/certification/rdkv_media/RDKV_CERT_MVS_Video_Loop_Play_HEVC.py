...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Loop_Play_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of HEVC video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_488</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of HEVC video codec content</test_objective>
...
video_src_url_short_duration_hevc: string
</input_parameters>
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the hevc video codec stream, expected events playing should occur, loop count should be 1 and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Loop_Play_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Loop_Play_HEVC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_short_duration_hevc
        ...
        video_test_urls = []
        test_counter = 0
        if MediaValidationVariables.hevc_url_type == "hls":
        ...
        #url=<video_src_url_short_duration_hevc>.mpd&operations=checkloop(30)&autotest=true&type=dash
