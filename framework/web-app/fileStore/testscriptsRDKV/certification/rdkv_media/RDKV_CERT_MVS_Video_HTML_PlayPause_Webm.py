...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_HTML_PlayPause_Webm</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation of webm content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_116</test_case_id>
    <test_objective>Test Script to launch a HTML Video player application via Webkit instance and perform video play pause operation of webm content and close the player               </test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_HTML_PlayPause_Webm</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_HTML_PlayPause_Webm')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_webm_url>&operations=pause(30),play(10)
