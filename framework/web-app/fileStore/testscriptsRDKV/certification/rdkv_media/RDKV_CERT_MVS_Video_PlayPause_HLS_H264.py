...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_PlayPause_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of hls h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play pause operation of hls h264 video codec content</test_objective>
...
4. App performs the play, pause operations of hls h264 video codec stream and validates each operation using events
...
7. Revert all values</automation_approch>
    <expected_output>Player should play, pause the hls h264  video codec stream, expected events play, paused should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_PlayPause_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_PlayPause_HLS_H264')
