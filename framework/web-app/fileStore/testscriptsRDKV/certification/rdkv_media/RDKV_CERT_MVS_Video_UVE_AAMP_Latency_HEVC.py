...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Latency_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of Hevc codec dash stream for few minutes and validate the time to play the video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_291</test_case_id>
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of Hevc codec dash stream for few minutes and validate the time to play video</test_objective>
...
4. App starts playing the hevc dash stream video
5. Validate the time taken to play the video
...
8. Revert all values</automation_approch>
    <expected_output>UVE Player should play the video for provided duration, expected event playing should occur and the time taken to play video must be within the expected limit</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Latency_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Latency_HEVC')
