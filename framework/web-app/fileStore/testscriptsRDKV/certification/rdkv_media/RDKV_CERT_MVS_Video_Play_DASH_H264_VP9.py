...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_DASH_H264_VP9</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of H.264 codec dash stream for few seconds, then switch to VP9 codec dash stream  for few seconds and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_200</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of H.264 codec dash stream for few seconds, then switch to VP9 codec dash stream  for few seconds and close the player</test_objective>
...
2.Lightning Video Player app should be hosted</pre_requisite>
    ...
    <input_parameters>Lightning video player App URL: string
webkit_instance:string
webinspect_port: string
video_src_url_dash_h264: string
video_src_url_vp9:string</input_parameters>
...
3. Launch webkit instance with video test app url with the operations to be performed, play the h264 stream and switch to vp9 stream after 60 seconds
4. App performs the play operations of dash h264 video codec stream and dash vp9  video codec stream validates each operation using events
5. If expected events playing occurs, then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Video Player should play the h264 stream and switch to vp9 stream and play for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_DASH_H264_VP9</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_DASH_H264_VP9')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        secondaryURL  = MediaValidationVariables.video_src_url_vp9
        # Setting VideoPlayer Operations
        # Play the primary H264 dash stream URL,after 60 sec switch to secondary VP9 dash stream URL, play for 60 sec & close
        setOperation("changeurl","60")
        setOperation("close","60")
        operations = getOperations()
        # Setting VideoPlayer test app URL arguments
        setURLArgument("url",videoURL)
        setURLArgument("operations",operations)
        setURLArgument("options","checkInterval(10),secondaryURL("+secondaryURL +")")
        ...
        #url=<video_h264_url>.mpd&operations=changeurl(60),close(60),options=secondaryURL(<video_vp9_url>.mpd)&autotest=true&type=dash
