...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_FF4X_Play_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of av1 content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_263</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of av1 content and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_FF4X_Play_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_FF4X_Play_AV1')
...
if expectedResult in result.upper() and FFEnabledStatus.lower() != "no" and config_status != "FAILURE":
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=fastfwd4x(30),playnow(10),close(30)&options=checkInterval(5)&autotest=true&type=dash
