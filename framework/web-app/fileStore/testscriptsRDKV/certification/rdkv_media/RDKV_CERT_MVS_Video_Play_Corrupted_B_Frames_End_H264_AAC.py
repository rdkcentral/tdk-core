...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Corrupted_B_Frames_End_H264_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted B frames from the end video segment of a clear stream,served via streaming server.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_530</test_case_id>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning SDK player App URL: string webkit_instance:string webinspect_port: string  video_src_url_corrupt_B_end_h264_aac: string</input_parameters>
...
8. Revert all values
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_Corrupted_B_Frames_End_H264_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Corrupted_B_Frames_End_H264_AAC')
...
if expectedResult in result.upper():
	...
	if pre_requisite_status == "SUCCESS":
		...
		videoURL  = MediaValidationVariables.video_src_url_corrupt_B_end_h264_aac
