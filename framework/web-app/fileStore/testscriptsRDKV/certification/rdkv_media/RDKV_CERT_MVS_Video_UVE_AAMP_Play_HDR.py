...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HDR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of HDR codec stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_316</test_case_id>
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of HDR stream for few minutes and close the player</test_objective>
...
4. App starts playing the HDR stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HDR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_HDR')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_mp4_hevc_hdr>.mpd&operations=close(60)&autotest=true
