...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Get_Audio_Info</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>est Script to launch a lightning UVE player application via Webkit instance and perform video play operation of H.264 codec dash stream for few minutes, get the audio track info and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_312</test_case_id>
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play operation of H.264 codec dash stream for few minutes,get audio track info and close the player</test_objective>
...
4. App starts playing the h.264 dash stream video, get the audio track info and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>UVE Player should play the video for provided duration, expected event playing should occur, fetch the audio track info and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Get_Audio_Info</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Get_Audio_Info')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        setOperation("audiotrackinfo","20")
        ...
        #url=<video_h264_url>.mpd&operations=audiotrackinfo(20)&autotest=true
