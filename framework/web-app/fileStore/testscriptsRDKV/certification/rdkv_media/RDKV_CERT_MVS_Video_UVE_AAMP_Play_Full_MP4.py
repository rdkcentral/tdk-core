...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Full_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning  UVE AAMP player application via Webkit instance and perform video play operations of mp4 content till the end</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_648</test_case_id>
    <test_objective>Test Script to launch a lightning  UVE AAMP player application via Webkit instance and perform video play operations of mp4 content till the end</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI-Client,Accelerator</test_setup>
...
video_src_url_short_duration_mp4: string</input_parameters>
...
4. App plays the entire mp4 video and validates using video event ended.
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should play the mp4 video till the end, ended event should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Full_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Full_MP4')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_short_duration_mp4>.mp4&operations=playtillend(0)&autotest=true
