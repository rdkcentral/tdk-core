...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Loop_Play_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of MKV video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_710</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video loop playback of MKV video codec content</test_objective>
...
video_src_url_short_duration_mkv: string
</input_parameters>
...
4. App performs playback of mkv stream. since loop option is enabled, playback continues from the start after reaching the end. app validates using events and position. loop count is calculated based on number of video position 0 occurred.
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the mkv stream, expected events playing should occur, loop count should be 1 and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Loop_Play_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Loop_Play_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_short_duration_mkv
        ...
        #url=<video_src_url_short_duration_mkv>.mkv&operations=checkloop(30)&autotest=true&type=mkv
