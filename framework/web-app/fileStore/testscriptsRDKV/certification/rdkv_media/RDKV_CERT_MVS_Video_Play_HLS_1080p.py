...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_HLS_1080p</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls content with 1080p display resolution for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_206</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls content with 1080p display resolution for few minutes and close the player</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,RPI, Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning Video Player app should be hosted
3.TV should be connected with the test device. If the 1080p resolution is not supported then the test will be marked as failure</pre_requisite>
...
2. Get the current video display resolution and set resolution as 1080p
...
5. App starts playing the hls video and closes the player after the provided duration.
...
8. Revert all values</automation_approch>
    <expected_output>Player should play the hls video for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_HLS_1080p</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_HLS_1080p')
