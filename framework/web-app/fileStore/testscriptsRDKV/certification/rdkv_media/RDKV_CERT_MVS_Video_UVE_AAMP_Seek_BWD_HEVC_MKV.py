...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek backward operation of video stream with hevc mkv video</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_382</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek backward operation of video stream with hevc mkv</test_objective>
...
video_src_url_hevc_mkv: string
seekbwd_interval:int
seekbwd_check_interval:int
close_interval: int</input_parameters>
    <automation_approch>1. As pre requisite, disable all the other plugins and launch webkit instance via RDKShell.
2. Store the details of other launched apps. Move the webkit instance to front, if its z-order is low.
3. Launch webkit instance with uve aamp test app url with the src url, operations to be performed, seek forward with interval and duration for close.
4. App performs the seek backward operation of video stream with hevc mkv video and validates using events and position.
5.If expected events seeking and seeked occurs and seek position is within expected range, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should seek backward the video stream with hevc mkv video, expected events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Seek_BWD_HEVC_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_hevc_mkv
        ...
        # Video seeked backward after 60 seconds, then close the player after 30 seconds
        setOperation("seekbwd","60")
        ...
        #url=<video_src_url_hevc_mkv>.mkv&operations=seekbwd(60),close(30)&autotest=true&options=checkInterval(5),seekInterval(10)
