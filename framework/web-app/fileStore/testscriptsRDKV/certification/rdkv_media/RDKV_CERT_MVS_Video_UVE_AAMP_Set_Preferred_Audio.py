...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Preferred_Audio</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation and set the preferred audio and after given duration close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_322</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of stream and set preferred and close the player</test_objective>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the dash h264 stream andset video track
4. App performs the play operations of stream and set preferred audio language
5. If expected video track and set video track are same, then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the stream and set preferred audio  and obtained preferred audio is same and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Preferred_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Set_Preferred_Audio')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        # Play the dash stream, after 30 sec set preferred audio language
        setOperation("prefaudio","30")
        setOperation("close","20")
        ...
        #url=<video_url>.mpd&operations=prefaudio(30),close(20),options=checkInterval(10)&autotest=true
