...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_H265_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of H.265 video codec stream for few seconds, then switch to AV1 video codec stream  for few seconds and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_199</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of H.265 video codec stream for few seconds, then switch to AV1 video codec stream  for few seconds and close the player</test_objective>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the h265 stream and switch to av1 stream after 60 seconds
4. App performs the play operations of  h265 video codec stream and av1 video codec stream validates each operation using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the h265 stream and switch to av1 stream and play for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_H265_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_H265_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        # Play the primary H265 stream URL,after 60 sec switch to secondary AV1 stream URL, play for 60 sec & close
        ...
        #url=<video_h265_url>.mpd&operations=changeurl(60),close(60),options=secondaryURL(<video_av1_url>)&autotest=true
