...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Seek_Pos_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of HEVC MKV video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_657</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of HEVC MKV video codec content</test_objective>
...
2. As pre requisite, launch webkit instance via RDKShell.
...
8. Revert all values</automation_approch>
    <expected_output>Video Player should perform seek to position of hevc mkv video codec stream and seeking, seeked events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Seek_Pos_HEVC_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_mkv>.mkv&operations=seekpos(30:120),seekpos(30:80),close(30)&autotest=true&options=checkInterval(10)&type=mkv
