...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_End_Video_Segment_DASH</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_media_test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the end video segment of a clear stream,served via streaming server.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_424</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the end video segment of a clear stream,served via streaming server.</test_objective>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
 ...
 webinspect_port: string  video_src_url_corrupt_I_B_end_dash: string</input_parameters>
...
8. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_End_Video_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_B_Frames_Of_End_Video_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupt_I_B_end_dash
    if pre_requisite_status == "SUCCESS":
            print("The content is corrupted with I and B frame at end segment of the video")
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the I and B frames during the end segment of the video from 78 to 90 seconds *************\n")
