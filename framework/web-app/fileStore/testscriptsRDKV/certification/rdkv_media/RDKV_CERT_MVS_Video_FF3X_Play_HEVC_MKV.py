...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_FF3X_Play_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 3x speed for few seconds, continue video play of hevc mkv content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_651</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 3x speed for few seconds, continue video play of hevc mkv content and close the player</test_objective>
...
3. Launch webkit instance with video test app with the src url, operations to be performed, fastforward 3x and playnow 1x with interval.
...
7. Revert all values</automation_approch>
    <expected_output>Video should be fastfoward in 3x speed for given interval and video play should be continued in 1x speed, expected event ratechange should occur for ff3x and playnow 1x operations and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_FF3X_Play_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_FF3X_Play_HEVC_MKV')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_mkv>.mkv&operations=fastfwd3x(30),playnow(10),close(30)&options=checkInterval(5)&autotest=true&type=mkv
