...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_PlayPause_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operation of HLS H.264 codec stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_717</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operation of hls H.264 codec stream</test_objective>
...
3. Launch webkit instance with shaka test app url with the operations to be performed, play and pause with given interval.
4. App performs the play, pause operations and validates each operation using events
5. If expected events  play, paused occurs for play, pause operation, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Shaka should play and pause hls h264 video codec stream, expected events play, paused should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_PlayPause_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_PlayPause_HLS_H264')
...
if expectedResult in result.upper():
    ...
    # Setting the pre-requisites for media test. Launching the wekit instance via RDKShell and
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hls_h264_url>.m3u8&operations=pause(30),play(10)&autotest=true
