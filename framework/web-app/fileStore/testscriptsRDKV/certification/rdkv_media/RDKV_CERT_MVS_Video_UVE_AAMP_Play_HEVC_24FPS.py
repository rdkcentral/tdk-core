...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_24FPS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning AAMP player application via Webkit instance and perform video play operation of 24 fps hevc content for few minutes, check average fps and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_338</test_case_id>
    <test_objective>Test Script to launch a lightning AAMP player application via Webkit instance and perform video play operation of 24 fps hevc content for few minutes, check average fps and close the player</test_objective>
...
4. App starts playing the 24fps hevc video, collects the fps for every 1 second and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_24FPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_24FPS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS" and config_status == "SUCCESS":
        ...
        #url=<video_hevc_24fpsurl>.mpd&operations=close(60)&autotest=true
