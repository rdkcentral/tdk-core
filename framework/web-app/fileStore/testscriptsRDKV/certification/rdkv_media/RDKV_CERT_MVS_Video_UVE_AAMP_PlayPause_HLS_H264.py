...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_PlayPause_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE player application via Webkit instance and perform video play pause operation of H.264 codec hls stream and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_113</test_case_id>
    <test_objective>Test Script to launch a lightning UVE player application via Webkit instance and perform video play pause operation of H.264 codec hls stream and close the player          </test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_PlayPause_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_PlayPause_HLS_H264')
