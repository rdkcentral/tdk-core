...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Video_Track</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_media_test</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation and set the video track and after given duration close the player</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_310</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of dash h264 stream set video track and close the player</test_objective>
...
4. App performs the play operations of dash h264 stream and set video track
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the dash h264 stream and set video track and obtained video track is same and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Video_Track</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Set_Video_Track')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=setvideotrack(30),close(20),options=checkInterval(10)&autotest=true
