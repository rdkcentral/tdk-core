...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_MSE_Conformance_Tests</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to launch the MSE conformance test url in Webkit browser and execute the tests.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_136</test_case_id>
    <test_objective>Test script to launch the MSE conformance test url in Webkit browser and execute the tests.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,RPI,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2. Ensure that the mse_conformance_test_app_url and mse_key_sequence parameters in the MediaValidationVariables file has latest values.</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>webinspect_port: string
mse_conformance_test_app_url:string
mse_key_sequence:string</input_parameters>
    <automation_approch>1. As pre requisite, launch webkit browser via RDKShell, open websocket conntion to webinspect page
2. Store the details of other launched apps. Move the webkit browser to front, if its z-order is low.
3. Read the mse url and key sequence info from the MVS config file
4.Launch the webkit browser with mse conformance test url
...
8. Revert all values</automation_approch>
    <expected_output>MSE conformance test should run properly</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_MSE_Conformance_Tests</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_MSE_Conformance_Tests')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        print("\nSet MSE Conformance test app url...")
        #Setting device config file
        test_app_url = MediaValidationVariables.mse_conformance_test_app_url
        print("\n=================================================")
        print("URL Used: ",test_app_url)
        print("=================================================\n")

        # Setting the MSE test url in webkit instance using RDKShell
        ...
        if "SUCCESS" in launch_status:
            ...
            # Monitoring the MSE test with hang detection time-out of 2 mins (120 seconds)
            test_result,failed_test_list = monitorConformanceTest(obj,webkit_console_socket,120)
            tdkTestObj = obj.createTestStep('rdkv_media_test');
            tdkTestObj.executeTestCase(expectedResult);
            if "SUCCESS" in test_result:
                print("All MSE Conformance Tests Success")
                print("[TEST EXECUTION RESULT]: SUCCESS")
                tdkTestObj.setResultStatus("SUCCESS");
            else:
                print("MSE Conformance Tests Failed")
                print("[TEST EXECUTION RESULT]: FAILURE")
                tdkTestObj.setResultStatus("FAILURE");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("Unable to load the MSE Conformance Test URL in Webkit\n")
