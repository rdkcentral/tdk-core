...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_25FPS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning AAMP player application via Webkit instance and perform video play operation of 25 fps hevc content for few minutes, check average fps and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_339</test_case_id>
    <test_objective>Test Script to launch a lightning AAMP player application via Webkit instance and perform video play operation of 25 fps hevc content for few minutes, check average fps and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_25FPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_HEVC_25FPS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS" and config_status == "SUCCESS":
        ...
        #url=<video_hevc_25fpsurl>.mpd&operations=close(60)&autotest=true
