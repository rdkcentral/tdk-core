...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_PlayPause_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operation of dash H.264 codec stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_52</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play pause operation of dash H.264 codec stream</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Shaka should play and pause dash h264 video codec stream, expected events play, paused should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_PlayPause_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_PlayPause_DASH_H264')
