...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Seek_Pos_DASH_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of dash h264 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_191</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of dash h264 video codec content</test_objective>
...
8. Revert all values</automation_approch>
    <expected_output>Video Player should perform seek to position of h264 dash stream and seeking, seeked events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Seek_Pos_DASH_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Seek_Pos_DASH_H264')
