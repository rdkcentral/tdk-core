...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning  UVE AAMP player application via Webkit instance and perform video seek forward operation mp4 content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_348</test_case_id>
    <test_objective>Test Script to launch a lightning  UVE AAMP player application via Webkit instance and perform  video seek forward operations of mp4 content</test_objective>
...
4. App performs the seek forward operation of mp4 stream and validates using events and position.
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should seek forward the mp4 stream, expected events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_MP4</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_MP4')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url_mp4>.mp4&operations=seekfwd(30),close(30)&autotest=true&options=checkInterval(5),seekInterval(10)
