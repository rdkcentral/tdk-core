...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_FF3X_Play_4K_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 3x speed for few seconds, continue video play of 4k hls content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_367</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 3x speed for few seconds, continue video play of 4k hls content and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_FF3X_Play_4K_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_FF3X_Play_4K_HLS')
...
if expectedResult in result.upper() and FFEnabledStatus.lower() != "no" and config_status != "FAILURE":
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url_4k>.m3u8&operations=fastfwd3x(30),playnow(10),close(30)&options=checkInterval(5)&autotest=true&type=hls
