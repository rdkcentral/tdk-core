...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_Frames_Of_Complete_Video_Segment_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I frames from the complete video segment of a clear stream,served via streaming server.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_397</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning UVE AAMP Player application through a Webkit instance.The objective is to assess the player  response when it receives corrupted I frames from the complete video segment of a clear stream,served via streaming server.</test_objective>
...
webinspect_port: string  video_src_url_corrupt_I_complete_dash: string</input_parameters>
...
4. Launch webkit instance with video test app with the live Dash url.
5.App should play the  live Dash  content and event video playing should occur
...
8. Revert all values
    ...
    <expected_output>The player should play the entire segment of the video, including complete corrupted I-frames from 0 to 90 seconds without altering its state. Users should be able to see the distortion caused by the corrupted frames while the video plays, without the player crashing or changing from the "playing" state in the  player.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_Frames_Of_Complete_Video_Segment_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_Corrupted_I_Frames_Of_Complete_Video_Segment_DASH')
...
if expectedResult in result.upper():
    ...
    videoURL  = MediaValidationVariables.video_src_url_corrupt_I_complete_dash
    if pre_requisite_status == "SUCCESS":
            print("The content is corrupted with I frame at complete segment of the video")
            ...
            if "SUCCESS" in launch_status:
                ...
                test_counter += 1
                print("\n************ Corruption occurs in the I frames during the complete segment of the video from 0 to 90 seconds *************\n")
