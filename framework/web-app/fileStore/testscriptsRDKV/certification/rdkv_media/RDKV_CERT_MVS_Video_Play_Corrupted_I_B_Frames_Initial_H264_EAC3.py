...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Corrupted_I_B_Frames_Initial_H264_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the initial video segment of a clear stream,served via streaming server.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_564</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted I and B frames from the initial video segment of a clear stream,served via streaming server.</test_objective>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning SDK player App URL: string webkit_instance:string webinspect_port: string  video_src_url_corrupt_I_B_initial_h264_eac3: string</input_parameters>
...
8. Revert all values
    ...
    <expected_output>The player should play the entire segment of the video, including end corrupted I and B-frames from 0 to 12 seconds without altering its state. Users should be able to see the distortion caused by the corrupted frames while the video plays, without the player crashing or changing from the "playing" state in the SDK player. </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media
</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_Corrupted_I_B_Frames_Initial_H264_EAC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Corrupted_I_B_Frames_Initial_H264_EAC3')
...
if expectedResult in result.upper():
	...
	if pre_requisite_status == "SUCCESS":
		...
		videoURL  = MediaValidationVariables.video_src_url_corrupt_I_B_initial_h264_eac3
