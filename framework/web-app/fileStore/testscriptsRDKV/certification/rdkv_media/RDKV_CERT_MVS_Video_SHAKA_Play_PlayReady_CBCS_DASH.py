...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Play_PlayReady_CBCS_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play operation of PlayReady DRM protected cbcs dash encryption stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_732</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play operation of  PlayReady DRM protected cbcs encryption dash stream for few minutes and close the player</test_objective>
...
video_src_url_playready_cbcs_dash: string
video_src_url_playready_cbcs_dash_drmconfigs:string
...
3. Launch webkit instance with shaka test app with the video src url and duration for close.
4. App starts playing the PlayReady DRM protected cbcs dash stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    <expected_output>Shaka Player should play the video for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Play_PlayReady_CBCS_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Play_PlayReady_CBCS_DASH')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_playready_cbcs_dash
        ...
        setURLArgument("drmconfigs",
MediaValidationVariables.video_src_url_playready_cbcs_dash_drmconfigs)
        ...
        #url=<playready_cbcs_dash_url>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true
