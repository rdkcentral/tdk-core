...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_EC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek forward operation of video stream with ec3 audio</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_239</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video seek forward operation of video stream with ec3 audio</test_objective>
...
4. App performs the seek forward operation of video stream with ec3 audio and validates using events and position.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_EC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Seek_FWD_EC3')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=seekfwd(30),close(30)&autotest=true&options=checkInterval(5),seekInterval(10)
