...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Live_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls live content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_28</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of hls live content</test_objective>
...
3. Launch webkit instance with video test app with the live hls url.
4. App should play the live hls content and event video playing should occur
...
7. Revert all values</automation_approch>
    <expected_output>Live hls video should be played for provided duration and expected event video playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_Live_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Live_HLS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_live_hls
        ...
        #url=<video_url>.m3u8&operations=close(60)&options=live&autotest=true&type=hls
