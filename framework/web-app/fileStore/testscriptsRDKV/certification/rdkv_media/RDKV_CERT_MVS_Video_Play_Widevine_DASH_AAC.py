...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Widevine_DASH_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  Widevine DRM protected AAC codec dash stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_79</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  Widevine DRM protected AAC codec dash stream for few minutes and close the player</test_objective>
...
video_src_url_widevine_dash_aac: string
video_src_url_widevine_dash_aac_drmconfigs:string
...
4. App starts playing the Widevine DRM protected aac dash stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_Widevine_DASH_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Widevine_DASH_AAC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_widevine_dash_aac
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_widevine_dash_aac_drmconfigs)
