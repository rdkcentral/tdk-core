...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_TrickPlay_HEVC_MKV</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video trickplay operations of hevc MKV content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_588</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video trickplay operations of hevc mkv stream</test_objective>
...
operation_max_interval: int</input_parameters>
    <automation_approch>1. As pre requisite, disable all the other plugins 1. As pre requisite, launch webkit instance via RDKShell.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_TrickPlay_HEVC_MKV</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_TrickPlay_HEVC_MKV')
...
if expectedResult in result.upper() and FFEnabledStatus.lower() != "no" and config_status != "FAILURE":
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_hevc_mkv>&operations=seekfwd(10),fastfwd(10),fastfwd(10),playnow(10),pause(10),play(10),seekbwd(10),fastfwd(10)
