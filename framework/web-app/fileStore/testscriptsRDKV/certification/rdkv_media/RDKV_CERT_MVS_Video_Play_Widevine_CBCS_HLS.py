...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Widevine_CBCS_HLS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  widevine DRM protected cbcs encryption hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_259</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  widevine DRM protected cbcs encryption hls stream for few minutes and close the player</test_objective>
...
video_src_url_widevine_cbcs_hls: string
video_src_url_widevine_cbcs_hls_drmconfigs:string
...
4. App starts playing the widevine DRM protected cbcs hls stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_Widevine_CBCS_HLS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Widevine_CBCS_HLS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_widevine_cbcs_hls
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_widevine_cbcs_hls_drmconfigs)
	...
	# Getting the complete test app URL for selected players
        ...
        #url=<video_src_url_playready_cbcs_hls>.mpd&drmconfigs=com.widevine(license_url)&operations=close(60)&autotest=true&type=hls
