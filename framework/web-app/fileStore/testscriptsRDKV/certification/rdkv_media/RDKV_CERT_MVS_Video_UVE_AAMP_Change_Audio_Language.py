...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Audio_Language</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of multi audio language stream, switch to another audio language and after given duration close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_201</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of multi audio language stream, switch to another audio language and after given duration close the player</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RDKTV,RPI,Accelerator</test_setup>
    <pre_requisite>1. Wpeframework process should be up and running in the device.
2.Lightning UVE AAMP Player app should be hosted
3. Need a stream with multiple audio languages</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Lightning player App URL: string
webkit_instance:string
webinspect_port: string
video_src_url_multi_audio_tracks: string</input_parameters>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the multi audio tracks stream and switch to another audio except the current audio language after 30 seconds
4. App performs the play operations of multi audio tracks stream and changes audio language validates using the audio track index values
5. If expected audio track index and observed audio track index are same, then update the result as SUCCESS or else FAILURE
6. Update the test script result as SUCCESS/FAILURE based on audio track index validation result from the app and proc check status (if applicable)
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the multi audio tracks stream and changes audio language and play for provided duration, expected audio track index should be obtained and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Audio_Language</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Change_Audio_Language')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_multi_audio_tracks
        # Setting VideoPlayer Operations
        # Play the multi audio tracks stream, after 30 sec switch to another audio track and close after 30 sec
        setOperation("changeaudio","30")
        setOperation("close","30")
        operations = getOperations()
        # Setting VideoPlayer test app URL arguments
        setURLArgument("url",videoURL)
        setURLArgument("operations",operations)
        setURLArgument("options","checkInterval(10)")
        ...
        #url=<video_url>.mpd&operations=changeaudio(30),close(30),options=checkInterval(10)&autotest=true
        ...
        if "SUCCESS" in launch_status:
            ...
            test_result,proc_check_list = monitorVideoTest(obj,webkit_console_socket,validation_dict,"Observed Audio Track Index");
