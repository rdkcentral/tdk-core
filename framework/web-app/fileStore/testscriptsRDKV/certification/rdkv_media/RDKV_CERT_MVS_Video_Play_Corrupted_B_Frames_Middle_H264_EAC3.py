...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Corrupted_B_Frames_Middle_H264_EAC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to initiate the TDK Lightning SDK Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted B frames from the Middle video segment of a clear stream,served via streaming server.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_557</test_case_id>
    <test_objective>Test script to initiate the TDK Lightning Player application through a Webkit instance.The objective is to assess the player response when it receives corrupted B frames from the middle video segment of a clear stream,served via streaming server.</test_objective>
...
3.Streaming server should be up and running in the docker.</pre_requisite>
    ...
    <input_parameters>Lightning SDK player App URL: string webkit_instance:string webinspect_port: string  video_src_url_corrupt_B_middle_h264_eac3: string</input_parameters>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Corrupted_B_Frames_Middle_H264_EAC3')
...
if expectedResult in result.upper():
	...
	if pre_requisite_status == "SUCCESS":
		...
		videoURL  = MediaValidationVariables.video_src_url_corrupt_B_middle_h264_eac3
