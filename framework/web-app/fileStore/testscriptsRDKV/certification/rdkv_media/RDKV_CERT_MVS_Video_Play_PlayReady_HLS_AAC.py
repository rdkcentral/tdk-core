...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_PlayReady_HLS_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  PlayReady DRM protected AAC codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_90</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  PlayReady DRM protected AAC codec hls stream for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_PlayReady_HLS_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_PlayReady_HLS_AAC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_playready_hls_aac_drmconfigs)
        ...
        #url=<video_aac_url>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true&type=hls
