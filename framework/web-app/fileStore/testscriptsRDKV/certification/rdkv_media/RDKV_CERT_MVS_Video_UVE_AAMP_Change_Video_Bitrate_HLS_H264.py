...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Video_Bitrate_HLS_H264</name>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_280</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of hls h264 stream, switch to another video bit rate and after given duration close the player</test_objective>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the hls h264 stream and switch to another video bit rate except the current video bit rate after 30 seconds
4. App performs the play operations of hls stream and changes video bit rate, validates using the video bit rate events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the hls h264 stream and changes video bit rate and play for provided duration, expected video bit rate should be obtained and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Change_Video_Bitrate_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Change_Video_Bitrate_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        # Play the hls h264 stream, after 30 sec switch to another video bit rate and close after 30 sec
