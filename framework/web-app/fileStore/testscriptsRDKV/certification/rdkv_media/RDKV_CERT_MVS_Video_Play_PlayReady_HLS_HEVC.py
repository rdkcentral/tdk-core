...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_PlayReady_HLS_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of PlayReady DRM protected Hevc codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_276</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of PlayReady DRM protected Hevc codec hls stream for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_PlayReady_HLS_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_PlayReady_HLS_HEVC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hevc_url>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true&type=hls
