...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_FF4X_Play_MP4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of mp4 content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_649</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of mp4 content and close the player</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_FF4X_Play_MP4')
...
if expectedResult in result.upper() and FFEnabledStatus.lower() != "no" and config_status != "FAILURE":
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_mp4>.mp4&operations=fastfwd4x(30),playnow(10),close(30)&options=checkInterval(5)&autotest=true&type=mp4
