...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_M4A_Audio</name>
  ...
  <status>FREE</status>
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation of m4a audio stream for few minutes and close the player</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_224</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation for M4a audio content for few minutes and close the player.</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the M4A audio for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_M4A_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_M4A_Audio')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        audioURL  = MediaValidationVariables.audio_src_url_m4a
        ...
        setURLArgument("type","m4a")
        appArguments = getURLArguments()

        # Getting the complete test app URL for selected players
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_audio_m4a).split(",")
