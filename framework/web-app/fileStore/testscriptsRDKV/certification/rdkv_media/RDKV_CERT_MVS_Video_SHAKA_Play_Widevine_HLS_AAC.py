...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_Play_Widevine_HLS_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play operation of Widevine DRM protected AAC codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_758</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video play operation of Widevine DRM protected AAC codec hls stream for few minutes and close the player</test_objective>
...
video_src_url_widevine_hls_aac: string
video_src_url_widevine_hls_aac_drmconfigs: string
...
4.  App starts playing the Widevine DRM protected aac hls stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_SHAKA_Play_Widevine_HLS_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_Play_Widevine_HLS_AAC')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_widevine_hls_aac
...
MediaValidationVariables.video_src_url_widevine_hls_aac_drmconfigs)
        ...
        #url=<widevine_hls_aac_url>.m3u8&drmconfigs=com.widevine(license_url)&operations=close(60)&autotest=true
