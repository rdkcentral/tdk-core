...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_DASH_AAC_EC3</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of AAC audio codec stream for few seconds, then switch to EC3 audio codec codec stream  for few seconds and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_195</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of AAC audio codec stream for few seconds, then switch to EC3 audio codec codec stream  for few seconds and close the player</test_objective>
...
video_src_url_ec3:string</input_parameters>
...
3. Launch webkit instance with uve aamp test app url with the operations to be performed, play the aac audio codec stream and switch to ec3 audio codec stream after 60 seconds
4. App performs the play operations of aac audio codec stream and ec3 stream validates each operation using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the aac stream and switch to ec3 stream and play for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_DASH_AAC_EC3</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_DASH_AAC_EC3')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        secondaryURL  = MediaValidationVariables.video_src_url_ec3
        # Setting VideoPlayer Operations
        # Play the primary AAC stream URL,after 60 sec switch to secondary EC3 stream URL, play for 60 sec & close
        ...
        #url=<video_aac_url>.mpd&operations=changeurl(30),close(30),options=secondaryURL(<video_ec3_url>.mpd)&autotest=true
