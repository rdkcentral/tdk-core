...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Mute_Seek_4K_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play 4k av1 video stream and perform mute,seek and unmute operations</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_619</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play 4k av1 video stream and perform mute,seek and unmute operations</test_objective>
...
4. App performs the mute,seek and unmute operations of 4k av1 video stream and validates using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should mute,seek and unmute 4k av1 video stream, expected volume level event should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Mute_Seek_4K_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Mute_Seek_4K_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_src_url_4k_av1>&operations=mute(30),checkmute(30),seekfwd(30),checkmute(30),unmute(30)&autotest=true
