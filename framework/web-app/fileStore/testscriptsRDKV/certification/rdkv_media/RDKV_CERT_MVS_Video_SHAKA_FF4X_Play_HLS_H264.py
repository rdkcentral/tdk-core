...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_FF4X_Play_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of hls h264 video codec content and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_741</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video fast forward operation in 4x speed for few seconds, continue video play of hls h264 video codec content and close the player</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Video should be fastforward in 4x speed for given interval and video play should be continued in 1x speed, expected event ratechange should occur for ff4x and playnow 1x operations for hls h264 content and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_SHAKA_FF4X_Play_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_FF4X_Play_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_hls_h264_url>.m3u8&operations=fastfwd4x(30),playnow(10),close(30)&options=checkInterval(5),loop&autotest=true
