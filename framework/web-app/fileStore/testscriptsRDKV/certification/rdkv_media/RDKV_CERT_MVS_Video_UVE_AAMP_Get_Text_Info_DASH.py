...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Get_Text_Info_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of multi text tracks stream, switch to another text track, get text track info and after given duration close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
6. Update the test script result as SUCCESS/FAILURE based on audio track index validation result from the app, get text track info and proc check status (if applicable)
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the multi text tracks stream and changes text track and play for provided duration, expected text track index should be obtained, get text track info and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Get_Text_Info_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Get_Text_Info_DASH')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_url>.mpd&operations=changetext(30),texttrackinfo(30),close(30),options=checkInterval(10)&autotest=true
