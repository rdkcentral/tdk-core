...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Seek_Pos_4K_AV1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of 4k AV1 video codec content</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_505</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video seek to position operation of 4k av1 video codec content</test_objective>
...
8. Revert all values</automation_approch>
    <expected_output>Video Player should perform seek to position of 4k av1 video codec stream and seeking, seeked events should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Seek_Pos_4K_AV1</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Seek_Pos_4K_AV1')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        #url=<video_4k_av1_url>.mp4&operations=seekpos(30:120),seekpos(30:80),close(30)&autotest=true&options=checkInterval(10)&type=mp4
