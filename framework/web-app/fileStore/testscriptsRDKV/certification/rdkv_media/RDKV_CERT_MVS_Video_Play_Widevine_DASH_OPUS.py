...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_Widevine_DASH_OPUS</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  Widevine DRM protected OPUS codec dash stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_85</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  Widevine DRM protected OPUS codec dash stream for few minutes and close the player</test_objective>
...
video_src_url_widevine_dash_opus: string
video_src_url_widevine_dash_opus_drmconfigs:string
...
4. App starts playing the Widevine DRM protected opus dash stream video and closes the player after the provided duration.
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_Widevine_DASH_OPUS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_Widevine_DASH_OPUS')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_widevine_dash_opus
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_widevine_dash_opus_drmconfigs)
        ...
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_opus).split(",")
        ...
        #url=<video_opus_url>.mpd&drmconfigs=com.widevine(<license_url>)&operations=close(60)&autotest=true&type=dash
