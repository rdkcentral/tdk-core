...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_DTS_Audio</name>
  ...
  <status>FREE</status>
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation of dts audio stream for few minutes and close the player.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_225</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation for DTS audio content for few minutes and close the player.</test_objective>
...
5. If expected event audio playing is observed then update the result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the DTS audio for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_DTS_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_DTS_Audio')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        audioURL  = MediaValidationVariables.audio_src_url_dts
        ...
        setURLArgument("type","dts")
        appArguments = getURLArguments()

        # Getting the complete test app URL for selected players
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_audio_dts).split(",")
        ...
        #url=<audio_url>.dts&operations=close(60)&autotest=true&type=dts
