...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_MP3_Audio</name>
  ...
  <status>FREE</status>
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation of mp3 audio stream for few minutes and close the player.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_226</test_case_id>
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform audio play operation for MP3 audio content for few minutes and close the player.</test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>Player should play the MP3 audio for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_Play_MP3_Audio</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_MP3_Audio')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        audioURL  = MediaValidationVariables.audio_src_url_mp3
        ...
        setURLArgument("type","mp3")
        appArguments = getURLArguments()

        # Getting the complete test app URL for selected players
        video_test_urls = []
        test_counter = 0
        players_list = str(MediaValidationVariables.codec_audio_mp3).split(",")
        ...
        #url=<audio_url>.mp3&operations=close(60)&autotest=true&type=mp3
