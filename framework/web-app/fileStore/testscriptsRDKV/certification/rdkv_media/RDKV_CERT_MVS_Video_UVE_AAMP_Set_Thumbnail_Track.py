...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Thumbnail_Track</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation and set the thumbnail track and after given duration close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_311</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of dash h264 stream set thumbnail track and close the player</test_objective>
...
video_src_url_multi_thumbnail_tracks: string</input_parameters>
...
4. App performs the play operations of dash h264 stream and set thumbnail track
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP  Player should play the dash h264 stream and set thumbnail track and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Set_Thumbnail_Track</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Set_Thumbnail_Track')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_multi_thumbnail_tracks
        # Setting VideoPlayer Operations
        # Play the dash h264 stream, after 30 sec set video track
        setOperation("setthumbnail","30")
        ...
        #url=<video_url>.mpd&operations=setthumbnail(30),close(20),options=checkInterval(10)&autotest=true
