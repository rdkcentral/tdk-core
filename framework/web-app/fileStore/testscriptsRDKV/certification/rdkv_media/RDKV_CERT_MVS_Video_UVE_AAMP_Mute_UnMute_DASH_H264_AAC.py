...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Mute_UnMute_DASH_H264_AAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play h264 dash with aac audio content and perform mute and unmute operations</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_175</test_case_id>
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance to play h264 dash with aac audio content and perform mute and unmute operations</test_objective>
...
4. App performs the mute and unmute operations of h264 dash with aac audio content and validates using events
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should mute and unmute  h264 dash with aac audio stream, expected volume level should occur and if proc validation is applicable, then expected data should be available in proc file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Mute_UnMute_DASH_H264_AAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Mute_UnMute_DASH_H264_AAC')
