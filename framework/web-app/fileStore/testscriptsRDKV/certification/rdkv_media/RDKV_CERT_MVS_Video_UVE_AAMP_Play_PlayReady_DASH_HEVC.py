...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_DASH_HEVC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of PlayReady DRM protected HEVC codec dash stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning UVE AAMP player application via Webkit instance and perform video play operation of PlayReady DRM protected HEVC codec dash stream for few minutes and close the player           </test_objective>
...
7. Revert all values</automation_approch>
    <expected_output>UVE AAMP Player should play the PlayReady hevc dash video for provided duration, expected event playing should occur and if proc validation is applicable, then expected data should be available in proc file </expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_media</test_stub_interface>
    <test_script>RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_DASH_HEVC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_UVE_AAMP_Play_PlayReady_DASH_HEVC')
