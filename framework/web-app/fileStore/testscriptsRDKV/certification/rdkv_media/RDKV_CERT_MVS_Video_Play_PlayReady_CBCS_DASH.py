...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_Play_PlayReady_CBCS_DASH</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  PlayReady DRM protected cbcs dash encryption stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test Script to launch a lightning Video player application via Webkit instance and perform video play operation of  PlayReady DRM protected cbcs encryption dash stream for few minutes and close the player</test_objective>
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_Play_PlayReady_CBCS_DASH</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_Play_PlayReady_CBCS_DASH')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_playready_cbcs_dash_drmconfigs)
        ...
        #url=<video_src_url_playready_cbcs_dash>.mpd&drmconfigs=com.playready(license_url)&operations=close(60)&autotest=true&type=dash
