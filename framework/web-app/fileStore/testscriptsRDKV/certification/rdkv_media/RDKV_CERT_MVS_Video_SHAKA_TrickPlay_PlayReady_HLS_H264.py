...
<xml>
  ...
  <name>RDKV_CERT_MVS_Video_SHAKA_TrickPlay_PlayReady_HLS_H264</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to launch a lightning Shaka player application via Webkit instance and perform video trickplay operations of PlayReady DRM protected H.264 codec hls stream for few minutes and close the player</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Media_Validation_757</test_case_id>
    <test_objective>Test Script to launch a lightning Shaka player application via Webkit instance and perform video trickplay operations of Playready DRM protected hls h264 video codec content</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Accelerator</test_setup>
    <pre_requisite>1.Wpeframework process should be up and running in the device.
2.Lightning Shaka Player app should be hosted</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Lightning Shaka player App URL: string
webkit_instance:string
webinspect_port: string
video_src_url_playready_hls_h264: string
video_src_url_playready_hls_h264_drmconfigs: string
operation_max_interval: int</input_parameters>
    <automation_approch>1. As pre-requisite, disable all the other plugins and launch webkit instance via RDKShell, open websocket connection to web inspect page
2. Store the details of other launched apps. Move the webkit instance to front, if its z-order is low.
3. Launch webkit instance with video test app url with the operations to be performed, play, pause, seek forward/backward and fast-forward with interval.
...
5. If expected events occurs for each operation and expected position, rate values occur for seek and fast-forward, then app gives the validation result as SUCCESS or else FAILURE
...
7. Revert all values</automation_approch>
    ...
    <test_script>RDKV_CERT_MVS_Video_SHAKA_TrickPlay_PlayReady_HLS_H264</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_MVS_Video_SHAKA_TrickPlay_PlayReady_HLS_H264')
...
if expectedResult in result.upper():
    ...
    if pre_requisite_status == "SUCCESS":
        ...
        videoURL  = MediaValidationVariables.video_src_url_playready_hls_h264
        ...
        setURLArgument("drmconfigs",MediaValidationVariables.video_src_url_playready_hls_h264_drmconfigs)
        ...
        #url=<playready_hls_h264_url>.m3u8&drmconfigs=com.playready(license_url)&seekfwd(10),fastfwd(10),playnow(10),pause(10),play(10),seekbwd(10),fastfwd(10)
        #&options=checkInterval(5),loop&autotest=true
