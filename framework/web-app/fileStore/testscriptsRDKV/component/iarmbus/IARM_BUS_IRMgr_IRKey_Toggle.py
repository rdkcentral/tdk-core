...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1589</id>
  <version>1</version>
  <name>IARM_BUS_IRMgr_IRKey_Toggle</name>
  ...
  <status>FREE</status>
  <synopsis>Check if multiple IR event notification is received for repeated key press/release for digit3 key.
Testcase ID: CT_IARMBUS_108</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_108</test_case_id>
    <test_objective>Check if sending multiple KEY_DOWN and KEY_UP events in a row and check if the events are handled successfully</test_objective>
...
5.IARMBUS_Agent will broadcast digit3 key press and key release event 50 times to the bus.
...
8.TestMgr_IARMBUS_GetLastReceivedEventDetails</test_stub_interface>
    <test_script>IARM_BUS_IRMgr_IRKey_Toggle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'IARM_BUS_IRMgr_IRKey_Toggle');
    ...
    if ("SUCCESS" in actualresult):
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                #Broadcast IR Key event (DIGIT3) from IR Mgr
                keyCode=0x00000033
                keyCodeStr="DIGIT3"
                # Repeat Key Press/Release for 50 times
                for x in range(0,50):
                    print('\n******** Iteration %d **********' % x) ;
