...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1604</id>
  <version>5</version>
  <name>IARM_BUS_IRMgr_IRKey_CheckTrickplay</name>
  ...
  <status>FREE</status>
  <synopsis>Check if multiple IR event notification is received for repeated key press/release for keys: Rewind, FastForward, Play, Stop, Pause.
Testcase ID: CT_IARMBUS_110</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_110</test_case_id>
    <test_objective>Check if multiple IR event notification is received for repeated key press/release for trickplay keys (REW/FF/PLAY/STOP/PAUSE)</test_objective>
...
5.IARMBUS_Agent will broadcast (REW/FF/PLAY/STOP/PAUSE) key press and key release event 5 times to the bus.
...
8.TestMgr_IARMBUS_GetLastReceivedEventDetails</test_stub_interface>
    <test_script>IARM_BUS_IRMgr_IRKey_CheckTrickplay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'IARM_BUS_IRMgr_IRKey_CheckTrickplay');
        ...
        if ("SUCCESS" in actualresult):
                ...
                if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult:
                                ...
                                #Broadcast IR Key event (REW/FF/PLAY/STOP/PAUSE) from IR Mgr
                                for keyCode in range(0x00000097,0x0000009C):
				    if keyCode == 0x00000097:
					keyCodeStr="REWIND"
                                    elif keyCode == 0x00000098:
					keyCodeStr="FASTFORWARD"
                                    elif keyCode == 0x00000099:
					keyCodeStr="PLAY"
                                    elif keyCode == 0x0000009A:
					keyCodeStr="STOP"
                                    elif keyCode == 0x0000009B:
					keyCodeStr="PAUSE"
                                    # Repeat Key Press/Release 6 times
                                    for x in range(0,6):
					print '\n******** Iteration %d **********' % x ;
