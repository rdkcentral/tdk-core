...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>IARMBus_Register_NULL_EventHandler_122</name>
  ...
  <status>FREE</status>
  <synopsis>IARMBUS – Registering a NULL event handler shall fail</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_122</test_case_id>
    <test_objective>IARMBUS – Registering a NULL event handler shall fail</test_objective>
...
4.IARMBUS_Agent will register for “IARM_BUS_EVENT_RESOLUTIONCHANGE” event and waits on event using a NULL event handler.(should fail)
...
8.TestMgr_IARMBUS_InvokeSecondApplication</priority>
    <test_stub_interface>IARMBus_Register_NULL_EventHandler_122.py</test_stub_interface>
    <test_script>IARMBus_Register_NULL_EventHandler_122</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks>M35</remarks>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'IARMBus_Register_NULL_EventHandler_122');
                        ...
                        if expectedresult in actualresult:
                                ...
                                if expectedresult in actualresult :
                                        if "INVALID_PARAM" in details.upper():
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print "SUCCESS :Event Handler did not register NULL Handler %s" %details;
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print "FAILURE: IARM_Bus_RegisterEventHandler returned %s error for NULL Handler." %details;
                                else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print "FAILURE: IARM_Bus_RegisterEventHandler Successfully registered NULL Handler. %s" %details;
