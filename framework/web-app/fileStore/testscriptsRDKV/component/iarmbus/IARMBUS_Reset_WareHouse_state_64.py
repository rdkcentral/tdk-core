...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1315</id>
  <version>1</version>
  <name>IARMBUS_Reset_WareHouse_state_64</name>
  <primitive_test_id>8</primitive_test_id>
  <primitive_test_name>IARMBUS_BusCall</primitive_test_name>
  <primitive_test_version>8</primitive_test_version>
  <status>FREE</status>
  <synopsis>IARMBUS – Reset the box to warehouse state using IARM_BUS_PWRMGR_API_WareHouseReset RPC call
Test case Id - CT_IARMBUS_64</synopsis>
  <groups_id/>
  <execution_time>3</execution_time>
  <long_duration>false</long_duration>
  <remarks>Skipped since STB loses activation because of this test</remarks>
  <skip>true</skip>
  <box_types>
  ...
  <test_cases>
    <test_case_id>CT_IARMBUS_64</test_case_id>
    <test_objective>IARMBUS – Reset the box to warehouse state using IARM_BUS_PWRMGR_API_WareHouseReset RPC call</test_objective>
...
3.IARMBUS_Agent Invoke the RPC call for reset the box to ware house state..
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CT_IARMBUS_64');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if ("SUCCESS" in actualresult):
        ...
        if expectedresult in actualresult:
            ...
            # parameter for Warehouse box reset
            tdkTestObj.addParameter("owner_name","PWRMgr");
            tdkTestObj.addParameter("method_name","WareHouseReset");
            expectedresult="SUCCESS"
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details=tdkTestObj.getResultDetails();
            ...
            if expectedresult in actualresult:
                ...
                print("DUT for WarehouseBox reset:") ;
                print(details);
            else:
