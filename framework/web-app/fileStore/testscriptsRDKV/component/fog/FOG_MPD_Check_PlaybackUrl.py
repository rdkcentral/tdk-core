...
<xml>
  ...
  <name>FOG_MPD_Check_PlaybackUrl</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the PlaybackUrl in the recording details is correct for fog mpd stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_21</test_case_id>
    <test_objective>Check if the PlaybackUrl in the recording details is correct for fog mpd stream</test_objective>
...
Checkpoint 2.  PlaybackUrl retrieved using curl command is same as that in the  fog log</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_MPD_Check_PlaybackUrl</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_MPD_Check_PlaybackUrl');
aampObj.configureTestCase(ip,port,'FOG_MPD_Check_PlaybackUrl');
