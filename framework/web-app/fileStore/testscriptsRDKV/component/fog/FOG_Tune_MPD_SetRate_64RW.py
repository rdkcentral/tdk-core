...
<xml>
  ...
  <name>FOG_Tune_MPD_SetRate_64RW</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>check if event is received for setrate value -64 for fog mpd stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_16</test_case_id>
    <test_objective>check if event is received for setrate value -64 for fog mpd stream</test_objective>
...
4. Aamp Agent invokes SetRate API with value -64
5. TM checks if  the corresponding event is received and returns SUCCESS/FAILURE 
6. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <except_output>Checkpoint 1. Event is received for Fog MPD URL tune
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Tune_MPD_SetRate_64RW</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_64RW');
aampObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_64RW');
