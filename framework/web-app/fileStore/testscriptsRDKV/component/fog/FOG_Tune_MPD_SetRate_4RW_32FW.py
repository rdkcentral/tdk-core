...
<xml>
  ...
  <name>FOG_Tune_MPD_SetRate_4RW_32FW</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if "AAMP_EVENT_SPEED_CHANGED" event is received while invoking setrate with value 32.0 for fog MPD url</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_19</test_case_id>
    <test_objective>Check if "AAMP_EVENT_SPEED_CHANGED" event is received while invoking setrate with value 32.0 for fog MPD url</test_objective>
...
4. Aamp Agent invokes SetRate API with value -4.0
5. Aamp Agent invokes SetRate API with value 32.0
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Tune_MPD_SetRate_4RW_32FW</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_4RW_32FW');
aampObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_4RW_32FW');
    	...
    	if expectedResult in actualResult:
			...
			sleep(20);
				...
				tdkTestObj = sysObj.createTestStep('ExecuteCommand');
                                cmd = "tail -n +`grep -n -m 1 \"AampSetRate Speed is: 32.0\" " + logName + " | cut -f1 -d:` " + logName + " | grep " + pattern + " | grep -v grep" ;
                                print cmd;
