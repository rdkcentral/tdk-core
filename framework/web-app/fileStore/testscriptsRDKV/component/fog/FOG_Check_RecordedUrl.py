...
<xml>
  ...
  <name>FOG_Check_RecordedUrl</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the RecordedUrl in the recording details is correct</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_03</test_case_id>
    <test_objective>Check if the RecordedUrl in the recording details is correct</test_objective>
...
4. TM gets the RecordedUrl from the recording details using curl command
5. TM checks if RecordedUrl is same as that in the  tune url and returns SUCCESS/FAILURE 
...
Checkpoint 2.  RecordedUrl retrieved using curl command is same as that in the  tune url</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Check_RecordedUrl</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Check_RecordedUrl');
aampObj.configureTestCase(ip,port,'FOG_Check_RecordedUrl');
