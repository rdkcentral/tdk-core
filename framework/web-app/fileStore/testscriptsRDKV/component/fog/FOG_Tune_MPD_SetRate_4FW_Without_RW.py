...
<xml>
  ...
  <name>FOG_Tune_MPD_SetRate_4FW_Without_RW</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>check if setrate fails for FF without doing RW for MPD stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_18</test_case_id>
    <test_objective>check if setrate fails for FF without doing RW for MPD stream</test_objective>
...
4. Aamp Agent invokes SetRate API with value 4
...
Checkpoint 2. Event is not received for Fog URL setrate</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Tune_MPD_SetRate_4FW_Without_RW</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_4FW_Without_RW');
aampObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_4FW_Without_RW');
    	...
    	if expectedResult in actualResult:
			...
			sleep(20);
                        if expectedResult in result:
                                ...
                                if expectedResult in result:
                                        print "FF without RW failed for live FOG Url"
                                        print "[TEST EXECUTION RESULT] : SUCCESS";
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                        print "FF without RW success for live FOG Url which is not expected"
