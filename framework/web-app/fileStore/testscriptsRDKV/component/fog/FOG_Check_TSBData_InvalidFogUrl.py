...
<xml>
  ...
  <name>FOG_Check_TSBData_InvalidFogUrl</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if tsb location size is increasing for non fog url</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_13</test_case_id>
    <test_objective>Check if tsb location size is increasing for non fog url</test_objective>
...
2. Aamp Agent invokes Tune API with Non Fog URL
3. TM checks if the corresponding event is received.
4. TM gets the tsb location from fog.log.
5. TM checks if the tsb location size is increasing  and returns FAILURE/SUCCESS
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Check_TSBData_InvalidFogUrl</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Check_TSBData_InvalidFogUrl');
aampObj.configureTestCase(ip,port,'FOG_Check_TSBData_InvalidFogUrl');
