...
<xml>
  ...
  <name>FOG_Tune_MPD_SetRate_Pause</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>check if event is received for setrate for pause for fog mpd stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_26</test_case_id>
    <test_objective>check if event is received for setrate for pause for fog mpd stream</test_objective>
...
4. Aamp Agent invokes SetRate API with value 0
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Tune_MPD_SetRate_Pause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_Pause');
aampObj.configureTestCase(ip,port,'FOG_Tune_MPD_SetRate_Pause');
