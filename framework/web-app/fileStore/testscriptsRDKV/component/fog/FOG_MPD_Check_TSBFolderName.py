...
<xml>
  ...
  <name>FOG_MPD_Check_TSBFolderName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check whether the tsb folder name and recording id are equal for fog mpd stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_23</test_case_id>
    <test_objective>Check if tsb folder name and recording id are equal for fog mpd stream</test_objective>
...
5. TM checks if a folder is created in the tsb location and the folder name is same as the recording id and returns SUCCESS/FAILURE
...
Checkpoint 2. TSB folder name is same as recording id</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_MPD_Check_TSBFolderName</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_MPD_Check_TSBFolderName');
aampObj.configureTestCase(ip,port,'FOG_MPD_Check_TSBFolderName');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
    ...
    print("Failed to load aamp/systemutil");
