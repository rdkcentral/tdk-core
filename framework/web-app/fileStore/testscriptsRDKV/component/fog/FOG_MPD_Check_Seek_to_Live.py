...
<xml>
  ...
  <name>FOG_MPD_Check_Seek_to_Live</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Play fog asset and seek to live , pipeline must play from live point</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_33</test_case_id>
    <test_objective>Play fog asset and seek to live , pipeline must play from live point</test_objective>
...
7. Call AampSeekToLive and seek to LIVE point.
8. Call AampGetPlaybackPosition and verify if pipeline seeked to LIVE point.
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_MPD_Check_Seek_to_Live</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_MPD_Check_Seek_to_Live');
aampObj.configureTestCase(ip,port,'FOG_MPD_Check_Seek_to_Live');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            print"\nTEST STEP 8:Seeking to LIVE point"
            #Aamp seek live call
            tdkTestObj = aampObj.createTestStep('Aamp_AampSeekLive');
            expectedResult = "SUCCESS";
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            #Get the result of execution
            actualResult = tdkTestObj.getResult();
            print "RESULT : Aamp_AampSeekLive : " , actualResult;

            print"\nTEST STEP 9:Verify Seek to LIVE point"
            tdkTestObj = aampObj.createTestStep('Aamp_AampGetPlaybackPosition');
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            #Get the result of execution
            ...
            if expectedResult not in actualResult:
                ...
                exitFromScript()
            new_position = (float)(details.split(":")[1])
            print "Position after seek :",new_position

            if (new_position > previous_position):
                previous_position = new_position
                print "SUCCESS : Pipeline seeked to LIVE point";
                tdkTestObj.setResultStatus("SUCCESS")
                print "[TEST EXECUTION RESULT] : %s" %result;
            else:
                print "FAILURE : Pipeline is not able to seek to LIVE point";
                tdkTestObj.setResultStatus("FAILURE")
                print "[TEST EXECUTION RESULT] : FAILURE";
                exitFromScript()

            sleep(10)
            print"\nTEST STEP 10:Verify pipeline in playing state after seeking to LIVE point"
            tdkTestObj = aampObj.createTestStep('Aamp_AampGetPlaybackPosition');
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            #Get the result of execution
            ...
            if (new_position > previous_position):
                print "SUCCESS : Pipeline is in playing state after seeking to LIVE point";
                tdkTestObj.setResultStatus("SUCCESS")
                print "[TEST EXECUTION RESULT] : %s\n" %result;
            else:
                print "FAILURE : Pipeline is not in playing state after seek to LIVE point";
                tdkTestObj.setResultStatus("FAILURE")
                print "[TEST EXECUTION RESULT] : FAILURE\n";
                exitFromScript()
        else:
