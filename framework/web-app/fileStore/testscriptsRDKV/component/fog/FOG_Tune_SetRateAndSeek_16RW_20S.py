...
<xml>
  ...
  <name>FOG_Tune_SetRateAndSeek_16RW_20S</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if event is received while invoking setrateandseek with rate 16.0 and time 20s for fog url</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_FOG_14</test_case_id>
    <test_objective>Check if event is received while invoking setrateandseek with rate 16.0 and time 20s for fog url</test_objective>
...
4. Aamp Agent invokes SetRateAndSeek API with rate 16.0 and time 20s
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>FOG_Tune_SetRateAndSeek_16RW_20S</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'FOG_Tune_SetRateAndSeek_16RW_20S');
aampObj.configureTestCase(ip,port,'FOG_Tune_SetRateAndSeek_16RW_20S');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
	...
	#Execute the test case in STB
    	...
    	if expectedResult in actualResult:
			...
			tdkTestObj.addParameter("seconds",20.0);
                        #Execute the test case in STB
                        tdkTestObj.executeTestCase(expectedResult);
                        #Get the result of execution
			...
			sleep(40);
				...
				cmd = "sed -n '/AampSetRateAndSeek Entry/,/AampSetRateAndSeek Exit/p' " + logName + " | grep " + pattern + " |grep -v grep";
                                print cmd;
