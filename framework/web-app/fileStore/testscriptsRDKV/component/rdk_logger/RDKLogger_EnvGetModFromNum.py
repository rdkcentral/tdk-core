...
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_RDKLogger_07</test_case_id>
    <test_objective>To get the name of the specified environment variable based on its registered number</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1-1/XI3-1</test_setup>
    <pre_requisite>RDK debug manager module should be intialized</pre_requisite>
    <api_or_interface_used>rdk_logger_envGetModFromNum()</api_or_interface_used>
    <input_parameters>rdk_logger_envGetModFromNum:   
int – number (e.g., 27)</input_parameters>
    <automation_approch>1. TM loads RDKLoggerStub_agent via the test agent.  
2. TM will invoke “TestMgr_RDKLogger_EnvGetModFromNum” in RDKLoggerStub_agent. 
3. RDKLoggerStub_agent will call rdk_logger_init() API of the component and get the result.  
4. On success of rdk_logger_init() API, RDKLoggerStub_agent will call rdk_logger_envGetModFromNum() API of the component and get the result. 
5. Depending on the return value of API, RDKLoggerStub_Agent will send SUCCESS or FAILURE to TM.</automation_approch>
    <except_output>Checkpoint 1..Check for the module name returned by the API.
Checkpoint 2.Check the return value of API for success status.</except_output>
    <priority>High</priority>
    <test_stub_interface>librdkloggerstub.so 
TestMgr_RDKLogger_EnvGetModFromNum</test_stub_interface>
    <test_script>RDKLogger_EnvGetModFromNum</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
