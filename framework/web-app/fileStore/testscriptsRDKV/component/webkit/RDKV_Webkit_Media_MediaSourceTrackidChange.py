...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaSourceTrackidChange</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the media source trackid change details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_31</test_case_id>
    <test_objective>To get the media source trackid change details from the device browser</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-source-trackid-change.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the media source trackid change details</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaSourceTrackidChange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaSourceTrackidChange');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-source/media-source-trackid-change.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-source-trackid-change.html file")
            grep_line = "media-source-trackid-change | tail -3 | tr -d '\\n'"
            log_filename = "media-source-trackid-change"
...
if "media-source-trackid-change" in status_dict:
    print(f"media-source-trackid-change: {status_dict['media-source-trackid-change']}")
