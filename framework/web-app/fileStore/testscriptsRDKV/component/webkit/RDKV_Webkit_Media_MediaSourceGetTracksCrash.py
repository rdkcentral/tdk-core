...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>RDKV_Webkit_Media_MediaSourceGetTracksCrash</name>
  <primitive_test_id/>
  <primitive_test_name>webkit_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the media source get tracks crash details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>Media_39</test_case_id>
    <test_objective>To get the media source get tracks crash details from the device browser</test_objective>
...
All the variables in WebkitVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>media-source-get-tracks-crash.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the media source get tracks crash details</expected_output>
    <priority>High</priority>
    <test_stub_interface>webkit</test_stub_interface>
    <test_script>RDKV_Webkit_Media_MediaSourceGetTracksCrash</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Webkit_Media_MediaSourceGetTracksCrash');
...
webkit_test_url = obj.url+WebkitVariables.wpe_webkit_testcases_path+'/media/media-source/media-source-get-tracks-crash.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing media-source-get-tracks-crash.html file")
            grep_line = "media-source-get-tracks-crash | tail -3 | tr -d '\\n'"
            log_filename = "media-source-get-tracks-crash"
...
if "media-source-get-tracks-crash" in status_dict:
    print(f"media-source-get-tracks-crash: {status_dict['media-source-get-tracks-crash']}")
