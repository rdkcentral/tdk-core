...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_Storage_Fill_Check_Before_After_Reboot</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To Fill the storage of specified path to maximun and check for functionalities of logfile and apps.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_09</test_case_id>
    <test_objective>To Initiate the shell scripts to fill the storage of specified path to maximun and check for functionalities of logfile and apps.</test_objective>
    ...
    <automation_approch>Respective Scripts are executed where reboot is involved and results are available in Logfile, Filling a specified path to its maximum storage capacity and test log file and apps functionalities </automation_approch>
    <expected_output> Success if logfiles and apps are working after storage is full</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Basic_Sanity_Storage_Fill_Check_Before_After_Reboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_Storage_Fill_Check_Before_After_Reboot');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check_before_reboot.sh 3"
