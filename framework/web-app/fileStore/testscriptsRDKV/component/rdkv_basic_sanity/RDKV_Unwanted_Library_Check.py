...
<xml>
  ...
  <name>RDKV_Unwanted_Library_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Script to test whether any unwanted libraries are present in the device or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_16</test_case_id>
    <test_objective>Script to test whether any unwanted libraries are present in the device or not</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Video_Accelerator, RDKTV, RPI-Client</test_setup>
    <pre_requisite>1. Ensure the shell script and config file is present in the device</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters></input_parameters>
    <automation_approch>The unwanted libs are present in the device or not</automation_approch>
    <expected_output>All uunwanted libraries should not be present</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Unwanted_Library_Check</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'RDKV_Unwanted_Library_Check');
...
if ("SUCCESS" in LoadStatus.upper()):
    ...
    command = "cd /opt/TDK; sh system_file_check.sh Unwanted_lib_check"
