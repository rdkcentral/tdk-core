...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_Core_Files_Check_Before_After_Reboot</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if any core files are generated after sometime in DUT.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_08</test_case_id>
    <test_objective>To Initiate the shell scripts to check if any core files are generated after sometime in DUT</test_objective>
    ...
    <automation_approch> Report if any corefiles are generated</automation_approch>
    <expected_output>Respective Scripts are executed where reboot is involved and results are available in Logfile, Display if corefiles are generated within timelimit specified</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Basic_Sanity_Core_Files_Check_Before_After_Reboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_Core_Files_Check_Before_After_Reboot');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check_before_reboot.sh 2"
