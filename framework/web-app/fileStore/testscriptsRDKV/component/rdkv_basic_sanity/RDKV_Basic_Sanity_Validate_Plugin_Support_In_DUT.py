...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_Basic_Sanity_Validate_Plugin_Support_In_DUT</name>
  <primitive_test_id/>
  <primitive_test_name>rdkv_basic_sanity_executeInDUT</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis> To check the status of plugins and report if it's not supported in DUT.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>RDKV_Basic_Sanity_12</test_case_id>
    <test_objective>To Initiate the shell scripts which Validate Plugin Support In DUT</test_objective>
    ...
    <automation_approch>Check the status of given list of plugins and validate support in the DUT.</automation_approch>
    <expected_output>List supported and not supported plugins in DUT.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_basic_sanity</test_stub_interface>
    <test_script>RDKV_Basic_Sanity_Validate_Plugin_Support_In_DUT</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Basic_Sanity_Validate_Plugin_Support_In_DUT');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"] :
            ...
            command = "sh " + configValues["SANITY_SCRIPT_PATH"] + "/system_sanity_check.sh 16"
