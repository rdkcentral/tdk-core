...
<xml>
  ...
  <name>NM_WifiMgr_stopProgressiveWifiScanning</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would stop scanning SSID process.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NET_SRV_MGR_WIFI_5</test_case_id>
    <test_objective>To stop wifi scanning</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_stopProgressiveWifiScanning)</api_or_interface_used>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>Wifi scanning should be stopped</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_StopProgressiveWifiScanning</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_stopProgressiveWifiScanning');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_stopProgressiveWifiScanning');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getAvailableSSIDsAsync");
                tdkTestObj.executeTestCase("SUCCESS");
                actualresult = tdkTestObj.getResult();
                print("[TEST EXECUTION RESULT] : %s" %actualresult);

                if "SUCCESS" in actualresult:
                   tdkTestObj.addParameter("method_name", "stopProgressiveWifiScanning");
                   ...
                   if expectedresult in actualresult:
                      print("[TEST EXECUTION - SUCCESS] :  WIFI stop scan was returned successfully");
                      tdkTestObj.setResultStatus("SUCCESS");
                   else:
                      print("[TEST EXECUTION - FAILURE] :  Failed to WIFI stop scan");
                      tdkTestObj.setResultStatus("FAILURE");

                   netsrvObj.unloadModule("netsrvmgr");
                else:
                   print("Start wifi scanning was failed");
                   tdkTestObj.setResultStatus("FAILURE");
            else:
