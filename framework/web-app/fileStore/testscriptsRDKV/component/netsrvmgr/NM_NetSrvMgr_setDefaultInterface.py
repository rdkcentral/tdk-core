...
<xml>
  ...
  <name>NM_NetSrvMgr_setDefaultInterface</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would set given network interfaces like Ethernet/Wifi as default interface.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NET_SRV_MGR_WIFI_15</test_case_id>
    <test_objective>This would set given interface as default</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_setDefaultInterface)</api_or_interface_used>
    <input_parameters>char* - methodName</input_parameters>
    <automation_approch>1.1.TM loads the NetSrvMgr_Agent via the test agent.
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>network interface</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_setDefaultInterface</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_setDefaultInterface');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_setDefaultInterface');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "setDefaultInterface");
                ...
                if expectedresult in actualresult:
                    print("[TEST EXECUTION - SUCCESS] :  WIFI was set as default interface");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  WIFI wasn't set as default interface");
