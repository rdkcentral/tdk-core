...
<xml>
  ...
  <name>NM_WifiMgr_getAvailableSSIDsAsync</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would help to scan available SSIDs</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_4</test_case_id>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_getAvailableSSIDsAsync)</api_or_interface_used>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>To scan available SSID</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_getAvailableSSIDsAsync</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDsAsync');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDsAsync');
