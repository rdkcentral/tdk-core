...
<xml>
  ...
  <name>NM_WifiMgr_saveSSID_with_valid_ssid_and_password</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To save the given ssid and password
Test CaseID:CT_NM_12
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_27</test_case_id>
    <test_objective>To save the given ssid</test_objective>
...
2.NetSrvMgr_Agent should save the provided ssid and password successfully.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should save the provided ssid and password successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_saveSSID_with_valid_ssid_and_password</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_saveSSID_with_valid_ssid_and_password');
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_saveSSID_with_valid_ssid_and_password');
