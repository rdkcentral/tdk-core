...
<xml>
  ...
  <name>NM_NetSrvMgr_getSTBip</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide IP address of current active network interface.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NET_SRV_MGR_WIFI_13</test_case_id>
    <test_objective>To provide ip address of device</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_getSTBip)</api_or_interface_used>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step.</automation_approch>
    <expected_output>ip address</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_getSTBip</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_getSTBip');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_getSTBip');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getSTBip");
                ...
                if ((expectedresult in actualresult) and (ip in details)):
                    print("[TEST EXECUTION - SUCCESS] :  Device IP address is: [%s]"%ip);
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  Device IP address is: [%s]"%ip);
