...
<xml>
  ...
  <name>NM_NetSrvMgr_isConnectedToInternet</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide an info about internet has active or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_11</test_case_id>
    <test_objective>This would provide internet connection status</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_isConnectedToInternet)</api_or_interface_used>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>Connection status</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_isConnectedToInternet</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_isConnectedToInternet');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_isConnectedToInternet');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "isConnectedToInternet");
                expectedresult="SUCCESS"
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();

                print("isConnectedToInternet returned : %s" %actualresult);
                print("Details: [%s]"%details);

                if (expectedresult in actualresult) and ("true" in details):
                    print("[TEST EXECUTION - SUCCESS] :  Device is connected to internet [%s]"%ip);
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  Device has not connected to internet [%s]"%ip);
