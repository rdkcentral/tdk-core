...
<xml>
  ...
  <name>NM_WifiMgr_saveSSID_without_ssid</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To try saving without any SSID and pssword
Test CaseID:CT_NM_14
...
2.NetSrvMgr_Agent should try to save without any ssid and password.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should fail to save saying empty data</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_saveSSID_without_ssid</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in iarmLoadStatus.upper():
        ...
        if "SUCCESS" in result:
                ...
                if "SUCCESS" in result:
                        ...
                        netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_saveSSID_without_ssid');
