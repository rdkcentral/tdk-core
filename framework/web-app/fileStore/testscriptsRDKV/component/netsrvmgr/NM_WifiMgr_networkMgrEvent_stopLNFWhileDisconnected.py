...
<xml>
  ...
  <name>NM_WifiMgr_networkMgrEvent_stopLNFWhileDisconnected</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To broadcast network manager stop LNF while disconnected event and check if netsrvmgr process receives it
Test CaseID:CT_NM_33
Test Type: Positive</synopsis>
  ...
  <test_cases>
    ...
    <test_objective>To broadcast network manager stop LNF while disconnected event and check if netsrvmgr process receives it</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_BroadcastEvent(IARM_BUS_NM_SRV_MGR_NAME,IARM_BUS_NETWORK_MANAGER_EVENT_STOP_LNF_WHILE_DISCONNECTED,(void*)&amp;eventData,sizeof(eventData))
IARM_Bus_Disconnect()
...
4. netsrvmgr_agent broadcast the event IARM_BUS_NETWORK_MANAGER_EVENT_STOP_LNF_WHILE_DISCONNECTED to netsrvmgr.
...
9. IARMBUS_Agent deregisters from the IARM Bus Daemon.</automation_approch>
    ...
    <test_script>NM_WifiMgr_networkMgrEvent_stopLNFWhileDisconnected</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_networkMgrEvent_stopLNFWhileDisconnected');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            nmObj.configureTestCase(ip,port,'NM_WifiMgr_networkMgrEvent_stopLNFWhileDisconnected');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in nmLoadStatus.upper():
                ...
                tdkTestObj.addParameter("event_log","starting LnF since stopLNFWhileDisconnected value is 0.");
                ...
                if "SUCCESS" in actualresult.upper():
                    ...
                    print("Broadcasting Netwrok Manager stop LNF while disconnected Event Result : %s"%actualresult);
