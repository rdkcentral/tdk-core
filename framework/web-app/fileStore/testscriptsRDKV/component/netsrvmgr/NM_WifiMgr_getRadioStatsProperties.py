...
<xml>
  ...
  <name>NM_WifiMgr_getRadioStatsProperties</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To get the radio status properties of wifi manager
Test CaseID:CT_NM_22
...
IARM_Bus_Connect()
IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_getRadioStatsProps)
IARM_Bus_Disconnect()
...
2.NetSrvMgr_Agent should retrieve the radio status properties of wifi manager
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should retrieve the radio  status properties successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_getRadioStatsProperties</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getRadioStatsProperties');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getRadioStatsProps");
