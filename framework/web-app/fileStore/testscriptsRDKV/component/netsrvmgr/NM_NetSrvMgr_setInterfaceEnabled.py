...
<xml>
  ...
  <name>NM_NetSrvMgr_setInterfaceEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would set given network interface (Either/Wifi) to device.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NET_SRV_MGR_WIFI_6</test_case_id>
    <test_objective>To enable given network interface</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_setInterfaceEnabled)</api_or_interface_used>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>Enable given interface</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_setInterfaceEnabled</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_setInterfaceEnabled');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_setInterfaceEnabled');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "setInterfaceEnabled");
                ...
                if expectedresult in actualresult:
                    print("[TEST EXECUTION - SUCCESS] :  WIFI/ETHERNET interface was enabled successfully");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("[TEST EXECUTION - FAILURE] :  Faailed to WIFI/ETHERNET interface");
