...
<xml>
  ...
  <name>NM_WifiMgr_getAvailableSSIDWithName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To fetch data on given SSID</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_NM_44</test_case_id>
    <test_objective>To fetch data on given SSID</test_objective>
...
IARM_Bus_Term : None</api_or_interface_used>
    <input_parameters>char *-ssid
char *-frequency</input_parameters>
...
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    ...
    <test_script>NM_WifiMgr_getAvailableSSIDWithName</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDWithName');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getAvailableSSIDWithName');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                frequency = configParser.get('wifi-config', 'frequency')

                #Prmitive test case which associated to this Script
                tdkTestObj = netsrvObj.createTestStep('NetSrvMgr_WifiMgr_getAvailableSSIDWithName');

                tdkTestObj.addParameter("frequency", int(frequency));
