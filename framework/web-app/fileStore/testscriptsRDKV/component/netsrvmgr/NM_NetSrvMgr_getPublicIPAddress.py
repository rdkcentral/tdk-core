...
<xml>
  ...
  <name>NM_NetSrvMgr_getPublicIPAddress</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This functionality would provide public IP addresses.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>It allows either zero parameter or with only interface and ipv6 parameter to determine WAN ip address</test_objective>
    ...
    <api_or_interface_used>IARM_Bus_Call(IARM_BUS_NETSRVMGR_API_getPublicIPAddress)</api_or_interface_used>
...
3. NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    ...
    <test_script>NM_NetSrvMgr_getPublicIPAddress</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_getPublicIPAddress');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_getPublicIPAddress');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getPublicIP");
                ...
                print("getPublicIP returned : %s" %actualresult)
                print("Details: [%s]"%details)

                #This would return Public IP.
                #Public IP would differ based on AP, so just check return value.
