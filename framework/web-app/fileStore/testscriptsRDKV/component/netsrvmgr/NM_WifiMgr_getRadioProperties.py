...
<xml>
  ...
  <name>NM_WifiMgr_getRadioProperties</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To get the radio properties of wifi manager
Test CaseID:CT_NM_20
Test Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_37</test_case_id>
    <test_objective>To get the radio properties of wifi manager</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_getRadioProps)
IARM_Bus_Disconnect()
IARM_Bus_Term</api_or_interface_used>
    <input_parameters>char* - methodName</input_parameters>
    <automation_approch>1. TM loads the NetSrvMgr_Agent via the test agent.
2.NetSrvMgr_Agent should retrieve the radio properties of wifi manager
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1.Wifi manager should retrieve the radio properties successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_getRadioProperties</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_getRadioProperties');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_getRadioProperties');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                tdkTestObj.addParameter("method_name", "getRadioProps");
                ...
                print("getRadioProps returns : %s" %actualresult)
                print("Details: [%s]"%details)

                FrequencyBands = ["2.4GHz","5GHz"]

                outputparams = details.split(", ")
                lenable = outputparams[0]
                lstatus = outputparams[1]
                lname = outputparams[2]
                loperatingFrequencyBand = outputparams[3]
                loperatingStandards = outputparams[4]

                enable = lenable.split(":")[1].strip(" ").strip("\\n")
                status = lstatus.split(":")[1].strip(" ").strip("\\n")
                name = lname.split(":")[1].strip(" ").strip("\\n")
                opFreq = loperatingFrequencyBand.split(":")[1].strip(" ").strip("\\n")
                opStand = loperatingStandards.split(":")[1].strip(" ").strip("\\n")
                frequency = loperatingFrequencyBand.split(":")[1].strip(" ").strip("\\n")

                print("Frequency : %s" %frequency)

                #Set the result status of execution
                if (expectedresult in actualresult) and (frequency in FrequencyBands):
                    print("[TEST EXECUTION RESULT] - SUCCESS")
                    print("Enable : %s" %enable)
                    print("Status : %s" %status)
                    print("Name   : %s" %name)
                    print("Op frequency : %s" %opFreq)
                    print("Op standards : %s" %opStand)
