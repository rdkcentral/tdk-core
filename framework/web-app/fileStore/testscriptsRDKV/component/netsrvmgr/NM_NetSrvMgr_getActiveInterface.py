...
<xml>
  ...
  <name>NM_NetSrvMgr_getActiveInterface</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This would provide active network interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
3. NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <expected_output>Active network interfaces</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_NetSrvMgr_getActiveInterface</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_NetSrvMgr_getActiveInterface');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_NetSrvMgr_getActiveInterface');
