...
<xml>
  ...
  <name>NM_WifiMgr_connect_without_credentials</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Objective:To connect to a wifi network without providing any credentials
Test CaseID:CT_NM_8
Test Type: Negative</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_NM_23</test_case_id>
    <test_objective>To connect to a wifi network without providing any credentials</test_objective>
...
IARM_Bus_Connect()
IARM_Bus_Call(IARM_BUS_WIFI_MGR_API_clearSSID)
...
2.NetSrvMgr_Agent will clear saved wifi credentials
2.NetSrvMgr_Agent will try to connect to a wifi network without any credentials.
3.NetSrvMgr_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Wifi manger should fail to connect to the network without credentials</except_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_connect_without_credentials</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_connect_without_credentials');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_connect_without_credentials');
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                if ("SUCCESS" in actualresult):
                    ...
                    print("Connect returned : %s" %actualresult);
