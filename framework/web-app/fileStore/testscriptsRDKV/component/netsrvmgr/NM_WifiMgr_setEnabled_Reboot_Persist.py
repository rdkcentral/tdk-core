...
<xml>
  ...
  <name>NM_WifiMgr_setEnabled_Reboot_Persist</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To enable wifi adapter and verify after reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
3. Connect to Wifi and set default interface as wifi</pre_requisite>
...
2.NetSrvMgr_Agent should enable WiFi adapter
3.GetCurrent state API should return Wifi is connected or not
4.Reboot the device and check the device is still connected to WIFI or not</automation_approch>
    <expected_output>WiFi should connected after reboot</expected_output>
    <priority>High</priority>
    <test_stub_interface>libnetsrvmgrstub.so</test_stub_interface>
    <test_script>NM_WifiMgr_setEnabled_Reboot_Persist</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'NM_WifiMgr_setEnabled');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            netsrvObj.configureTestCase(ip,port,'NM_WifiMgr_setEnabled');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in netsrvLoadStatus.upper():
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and "Wifi connected" in details:
                        ...
                        if expectedresult in actualresult and "Wifi connected" in details:
                            ...
                            print("ACTUAL RESULT : Wifi Connected reboot was success")
                            print("[TEST EXECUTION RESULT] : SUCCESS")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("ACTUAL RESULT : Wifi Connected reboot was failed")
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("ACTUAL RESULT : Wifi is not connected")
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("ACTUAL RESULT : Unable to Wifi as connected")
                    print("[TEST EXECUTION RESULT] : FAILURE")
