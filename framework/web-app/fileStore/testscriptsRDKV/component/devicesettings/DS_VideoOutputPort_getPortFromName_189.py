...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>DS_VideoOutputPort_getPortFromName_189</name>
  <primitive_test_id/>
  <primitive_test_name>DS_VideoOutputPort_getPortFromName</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This tests the API used to get video output port instance using port name.
TestcaseID: CT_DS189</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_DS_189</test_case_id>
    <test_objective>This tests the API used to get video output port instance using port name.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>VideoOutputPort getPort(const std::string &amp; name)</api_or_interface_used>
    <input_parameters>string port_name("HDMI0")</input_parameters>
    <automation_approch>1. TM loads the Device_Settings_Agent via the test agent.
2.Device_Settings_Agent will get the video output port instance by port name.
3.Device_Settings_Agent will check if video output port name retrieved using video output port instance is same as port name provided.
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    <except_output>Checkpoint 1. Check if port name retrieved using video output port instance is same as port name provided</except_output>
    <priority>High</priority>
    <test_stub_interface>none</test_stub_interface>
    <test_script>DS_VideoOutputPort_getPortFromName_189</test_script>
    <skipped>No</skipped>
...
dsObj.configureTestCase(ip,port,'DS_VideoOutputPort_getPortFromName_189');
        ...
        if "SUCCESS" in result:
                ...
                if "TRUE" in result:
                        ...
                        if expectedresult in actualresult:
                        		...
                        		tdkTestObj = dsObj.createTestStep('DS_VideoOutputPort_getPortFromName');
