...
Test Case ID : CT_DS_18</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_DS_18</test_case_id>
    <test_objective>Device Setting –  Check if HDCP is supported for the given HDMI port.</test_objective>
...
VideoOutputPort::isHDTPSupported()
VideoOutputPortType::enabledHDCP()
Device::Manager::DeInitialize()</api_or_interface_used>
    <input_parameters>null</input_parameters>
    <automation_approch>1.TM loads the Device_Settings_Agent via the test agent
2.Device_Settings_Agent will check for  HDCP protection support(either true/false) for the given port.
3.Device_Settings_Agent will check for the status of HDCP in the port and it will return SUCCESS or FAILURE based on the result.</automation_approch>
    <except_output>Checkpoint 1. Check if the API is executed successfully.</except_output>
    <priority>Medium</priority>
    <test_stub_interface>TestMgr_DS_managerInitialize
TestMgr_DS_VOPTYPE_isHDCPSupported
TestMgr_DS_managerDeinitialize</test_stub_interface>
    <test_script>DS_IsHDCPSupported_test_18</test_script>
    <skipped>No</skipped>
