...
<xml>
  ...
  <name>DS_setHDMI_PASSTHRUFormat_reboot_persistent_8005_2</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Device Setting –  Get and set stereo mode to PASSTHRU in HDMi and check the value is persisting after device reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DS_setHDMI_PASSTHRUFormat_reboot_persistent_8005_2');
		...
		stereomode="PASSTHRU";
                ...
                if expectedresult not in actualresult:
					...
					tdkTestObj.addParameter("persist",1);
                                        ...
                                        #Check for SUCCESS/FAILURE return value of DS_SetStereoMode
                                        if expectedresult not in actualresult:
                                                print "FAILURE: Application Failed to get the PASSTHRU mode to HDMI";
                                                tdkTestObj.setResultStatus("FAILURE");
                                        else:
                                                if stereomode in stereomodedetails:
                                                        tdkTestObj.setResultStatus("SUCCESS");
