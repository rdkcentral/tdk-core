...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>DS_VideoOutputPortType_getPorts_197</name>
  <primitive_test_id/>
  <primitive_test_name>DS_VOPTYPE_getPorts</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This tests function used to get the list of video output port type.
TestcaseID: CT_DS197</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_DS_197</test_case_id>
    <test_objective>This tests function used to get the list of video output port type.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XI3-1/XG1-1</test_setup>
    <pre_requisite>1. dsMgrMain should be up and running.
2. IARMDaemonMain should be up and running.</pre_requisite>
    <api_or_interface_used>const List&lt;VideoOutputPort&gt;getPorts()</api_or_interface_used>
...
4.Device_Settings_Agent will return SUCCESS or FAILURE based on the result from the above step</automation_approch>
    ...
    <test_script>DS_VideoOutputPortType_getPorts_197</test_script>
    <skipped>No</skipped>
...
dsObj.configureTestCase(ip,port,'DS_VideoOutputPortType_getPorts_197');
...
if "SUCCESS" in dsLoadStatus.upper():
        ...
        if "SUCCESS" in result:
            ...
            tdkTestObj = dsObj.createTestStep('DS_VOPTYPE_getPorts');
            port_id = 0
            ...
            #print "PortId: %d getPorts: %s"%(port_id,details);
            print "VideoOutputPort ",details
            #Check for SUCCESS/FAILURE return value
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "Successfully obtained the details of video output port"
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print "Failed to obtain the details of video output port"

            #Calling DS_ManagerDeInitialize to DeInitialize API
            result = devicesettings.dsManagerDeInitialize(dsObj)
        else:
            print "Device settings initialization failed"
            tdkTestObj.setResultStatus("FAILURE");
