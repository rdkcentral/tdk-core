...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_SoftwareVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Software Version from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_08</test_case_id>
    <test_objective>To retrieve Software Version from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 7 for mfrSERIALIZED_TYPE_SOFTWAREVERSION</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_SoftwareVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_SoftwareVersion');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 7(mfrSERIALIZED_TYPE_SOFTWAREVERSION)
    tdkTestObj.addParameter("data_type", 7)
    ...
    if (actualresult == expectedresult):
        ...
        softwareVersion = str(tdkTestObj.getResultDetails())
        if (softwareVersion):
            print "MfrHal_GetSerializedData: Software Version : ", softwareVersion
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "Software Version data retrieved is empty or incorrect"
