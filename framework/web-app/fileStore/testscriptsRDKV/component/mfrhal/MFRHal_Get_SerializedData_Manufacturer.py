...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_Manufacturer</name>
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_01</test_case_id>
    <test_objective>To retrieve Manufacturer name from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 0 for mfrSERIALIZED_TYPE_MANUFACTURER</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_Manufacturer</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_Manufacturer');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 0(mfrSERIALIZED_TYPE_MANUFACTURER)
    tdkTestObj.addParameter("data_type", 0)
    ...
    if (actualresult == expectedresult):
        ...
        manufacturer = str(tdkTestObj.getResultDetails())
        if (manufacturer):
            print("MfrHal_GetSerializedData: Manufacturer : ", manufacturer)
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print("Manufacturer data retrieved is empty or incorrect")
