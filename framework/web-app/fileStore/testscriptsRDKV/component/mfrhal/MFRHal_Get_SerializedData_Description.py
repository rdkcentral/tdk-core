...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_Description</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve device description from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_04</test_case_id>
    <test_objective>To retrieve device description from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 3 for mfrSERIALIZED_TYPE_DESCRIPTION</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_Description</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_Description');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 3(mfrSERIALIZED_TYPE_DESCRIPTION)
    tdkTestObj.addParameter("data_type", 3)
    ...
    if (actualresult == expectedresult):
        ...
        description = str(tdkTestObj.getResultDetails())
        if (description):
            print "MfrHal_GetSerializedData: Description : ", description 
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "Description data retrieved is empty or incorrect"
