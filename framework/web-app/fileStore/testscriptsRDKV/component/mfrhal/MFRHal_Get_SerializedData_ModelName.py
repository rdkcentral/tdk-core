...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_ModelName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Model Name from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_ModelName</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_ModelName');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 2(mfrSERIALIZED_TYPE_MODELNAME)
    tdkTestObj.addParameter("data_type", 2)
    ...
    if (actualresult == expectedresult):
        ...
        modelName = str(tdkTestObj.getResultDetails())
        if (modelName):
            print("MfrHal_GetSerializedData: ModelName : ", modelName)
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print("ModelName data retrieved is empty or incorrect")
