...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_ImageName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve imagename info from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_17</test_case_id>
    <test_objective>To retrieve imagename info from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 26 for mfrSERIALIZED_TYPE_IMAGENAME</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_ImageName</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_ImageName');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 26(mfrSERIALIZED_TYPE_IMAGENAME)
    tdkTestObj.addParameter("data_type", 26)
    ...
    if (actualresult == expectedresult):
        ...
        imageName_info = str(tdkTestObj.getResultDetails())
        if (imageName_info):
            print("MfrHal_GetSerializedData: ImageName Info : ", imageName_info)
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print("ImageName Info retrieved is empty or incorrect")
