...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_FirstUseDate</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve First Use Date from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_10</test_case_id>
    <test_objective>To retrieve First Use Date from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 9 for mfrSERIALIZED_TYPE_FIRSTUSEDATE</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_FirstUseDate</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_FirstUseDate');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 9(mfrSERIALIZED_TYPE_FIRSTUSEDATE)
    tdkTestObj.addParameter("data_type", 9)
    ...
    if (actualresult == expectedresult):
        ...
        firstUseDate = str(tdkTestObj.getResultDetails())
        if (firstUseDate):
            print "MfrHal_GetSerializedData: FirstUseDate : ", firstUseDate
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "FirstUseDate data retrieved is empty or incorrect"
