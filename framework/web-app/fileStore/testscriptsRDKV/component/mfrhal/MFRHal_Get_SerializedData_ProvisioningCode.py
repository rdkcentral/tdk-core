...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_ProvisioningCode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Provisioning Code from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_09</test_case_id>
    <test_objective>To retrieve Provisioning Code from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 8 for mfrSERIALIZED_TYPE_PROVISIONINGCODE</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_ProvisioningCode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_ProvisioningCode');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 8(mfrSERIALIZED_TYPE_PROVISIONINGCODE)
    tdkTestObj.addParameter("data_type", 8)
    ...
    if (actualresult == expectedresult):
        ...
        provisioningCode = str(tdkTestObj.getResultDetails())
        if (provisioningCode):
            print "MfrHal_GetSerializedData: Provisioning Code : ", provisioningCode
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "Provisioning Code data retrieved is empty or incorrect"
