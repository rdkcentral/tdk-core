...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_HDMIHDCP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve HDMI HDCP from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_12</test_case_id>
    <test_objective>To retrieve HDMI HDCP from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 12 for mfrSERIALIZED_TYPE_HDMIHDCP</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_HDMIHDCP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_HDMIHDCP');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 12(mfrSERIALIZED_TYPE_HDMIHDCP)
    tdkTestObj.addParameter("data_type", 12)
    ...
    if (actualresult == expectedresult):
        ...
        hdmiHdcp = str(tdkTestObj.getResultDetails())
        if (hdmiHdcp):
            print("MfrHal_GetSerializedData: HDMI HDCP : ", hdmiHdcp)
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print("HDMI HDCP data retrieved is empty or incorrect")
