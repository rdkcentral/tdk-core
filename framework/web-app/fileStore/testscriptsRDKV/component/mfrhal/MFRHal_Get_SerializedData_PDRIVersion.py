...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_PDRIVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve PDRI Version from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_13</test_case_id>
    <test_objective>To retrieve PDRI Version from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 13 for mfrSERIALIZED_TYPE_PDRIVERSION</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_PDRIVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_PDRIVersion');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 13(mfrSERIALIZED_TYPE_PDRIVERSION)
    tdkTestObj.addParameter("data_type", 13)
    ...
    if (actualresult == expectedresult):
        ...
        pdriVersion = str(tdkTestObj.getResultDetails())
        if (pdriVersion):
            print "MfrHal_GetSerializedData: PDRI Version : ", pdriVersion
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "PDRI Version data retrieved is empty or incorrect"
