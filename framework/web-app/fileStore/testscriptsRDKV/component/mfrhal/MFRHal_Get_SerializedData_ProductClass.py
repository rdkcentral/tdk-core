...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_ProductClass</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Product Class from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_05</test_case_id>
    <test_objective>To retrieve Product Class from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 4 for mfrSERIALIZED_TYPE_PRODUCTCLASS</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_ProductClass</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_ProductClass');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 4(mfrSERIALIZED_TYPE_PRODUCTCLASS)
    tdkTestObj.addParameter("data_type", 4)
    ...
    if (actualresult == expectedresult):
        ...
        productClass = str(tdkTestObj.getResultDetails())
        if (productClass):
            print "MfrHal_GetSerializedData: Product Class : ", productClass
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print "Product Class data retrieved is empty or incorrect"
