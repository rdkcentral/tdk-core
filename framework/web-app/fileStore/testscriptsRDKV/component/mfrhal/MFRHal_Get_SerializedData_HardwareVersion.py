...
<xml>
  ...
  <name>MFRHal_Get_SerializedData_HardwareVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Hardware Version from DUT</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_MFR_HAL_07</test_case_id>
    <test_objective>To retrieve Hardware Version from DUT</test_objective>
    ...
    <input_parameters>mfrSerializedType_t type = 6 for mfrSERIALIZED_TYPE_HARDWAREVERSION</input_parameters>
...
Checkpoint 2. Verify that the serialized data retrieved is not empty</expected_output>
    ...
    <test_script>MFRHal_Get_SerializedData_HardwareVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
mfrhalObj.configureTestCase(ip,port,'MFRHal_Get_SerializedData_HardwareVersion');
...
if "SUCCESS" in result.upper():
    ...
    #Set the serialized data type to 6(mfrSERIALIZED_TYPE_HARDWAREVERSION)
    tdkTestObj.addParameter("data_type", 6)
    ...
    if (actualresult == expectedresult):
        ...
        hardwareVersion = str(tdkTestObj.getResultDetails())
        if (hardwareVersion):
            print("MfrHal_GetSerializedData: Hardware Version : ", hardwareVersion)
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            print("Hardware Version data retrieved is empty or incorrect")
