...
<xml>
  ...
  <name>HWPerformance_Stress-ng_Filesystem</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Execute Stress-ng opensource performance tool it will test Filesystem stress in the system</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HWPerformance_16</test_case_id>
    <test_objective>Execute Stress-ng opensource performance tool will test Filesystem stress in the system</test_objective>
    ...
    <input_parameters> --fork 8 --open 8 --close 8 --seek 8 --wait 8 --kill 8 -t 20s --times --metrics-brief --log-file /tmp/stressng-report.txt</input_parameters>
    ...
    <test_script>HWPerformance_Stress-ng_Filesystem</test_script>
    <skipped>No</skipped>
...
sysUtilObj.configureTestCase(ip,port,'HWPerformance_Stress-ng_Filesystem');
...
if ("SUCCESS" in sysUtilLoadStatus.upper()):
    ...
    App_with_args="stress-ng --fork 8 --open 8 --close 8 --seek 8 --wait 8 --kill 8 -t 20s --times --metrics-brief --log-file /tmp/stressng-report.txt;"
    ...
    if "SUCCESS" in actualresult:
        ...
        message=""
        for filesystem in ["fork","open","close","seek","wait","kill"]:
            Parse_log="sh $TDK_PATH/HWPerf_metric_parser.sh stress-ng_%s;"%(filesystem)
		...
		print "Unable to acquire result for stress-ng_%s;"%(filesystem)
                tdkTestObj.setResultStatus("FAILURE");

        if message:
             print "\n**************HW Performance tools Execution Log - Begin**********\n"
             print "\n**************Stress NG NETWORK Logs*************\n"
             print message
