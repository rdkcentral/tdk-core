...
<xml>
  ...
  <name>RDKV_Telemetry_Description</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device description report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_07</test_case_id>
    <test_objective>To send device description report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device description into server.
    ...
    5. Verify if device description is updated in cJSON report. </automation_approch>
    ...
    5. Device description must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Description</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Description');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.Description"
    profile_name = "RDKV-Profile-4"
    description = "Report to check Device Description"
    name = "Desc"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device description in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    description = data["Report"][0][name]
                    print("Device Description:", description)
                    print("SUCCESS : Able to obtain  \"Device Description\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Device Description\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Device Description from cJSON report")
