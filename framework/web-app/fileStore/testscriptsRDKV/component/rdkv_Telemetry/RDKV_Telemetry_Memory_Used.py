...
<xml>
  ...
  <name>RDKV_Telemetry_Memory_Used</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Memory_Used report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_14</test_case_id>
    <test_objective>To send device Memory_Used report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Memory_Used into server.
    ...
    5. Verify if device Memory_Used is updated in cJSON report. </automation_approch>
    ...
    5. Device Memory_Used must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_Memory_Used</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_Memory_Used');
...
if pre_requisite_set:
    param = "Device.DeviceInfo.MemoryStatus.Used"
    profile_name = "RDKV-Profile-11"
    description = "Report to check Memory Used"
    name = "MemStatUsed"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Memory_Used in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Memory_Used = data["Report"][0][name]
                    print("Memory_Used:", Memory_Used)
                    print("SUCCESS : Able to obtain  \"Memory_Used\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Memory_Used\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Memory_Used from cJSON report")
