...
<xml>
  ...
  <name>RDKV_Telemetry_HardwareVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device Hardware_Version report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_11</test_case_id>
    <test_objective>To send device Hardware_Version report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device Hardware_Version into server.
    ...
    5. Verify if device Hardware_Version is updated in cJSON report. </automation_approch>
    ...
    5. Device Hardware_Version must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_HardwareVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_HardwareVersion');
...
if pre_requisite_set:
    ...
    profile_name = "RDKV-Profile-8"
    description = "Report to check Hardware Version"
    name = "HwVer"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device Hardware_Version in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    Hardware_Version = data["Report"][0][name]
                    print("Hardware Version:", Hardware_Version)
                    print("SUCCESS : Able to obtain  \"Hardware Version\" from cJSON report")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to obtain \"Hardware Version\" from cJSON report")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to get Hardware Version from cJSON report")
