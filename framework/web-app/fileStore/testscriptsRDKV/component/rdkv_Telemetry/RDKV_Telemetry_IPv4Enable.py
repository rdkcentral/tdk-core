...
<xml>
  ...
  <name>RDKV_Telemetry_IPv4Enable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send device IPV4 Enable report to telemetry server and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkv_Telemetry_24</test_case_id>
    <test_objective>To send device IPV4 Enable or not report to telemetry server and verify output</test_objective>
    ...
    <automation_approch>1. Execute rbuscli command to set profile to report device IPV4 Enable into server.
    ...
    5. Verify if device IPV4 Enable is updated in cJSON report. </automation_approch>
    ...
    5. Device IPV4 Enable must be successfully retrieved from cJSON report.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_Telemetry</test_stub_interface>
    <test_script>RDKV_Telemetry_IPv4Enable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Telemetry_IPv4Enable');
...
if pre_requisite_set:
    param = "Device.IP.IPv4Enable"
    profile_name = "RDKV-Profile-21"
    description = "Report to check IPV4 Enable"
    name = "IPv4Enable"
    ...
    if profile_set:
        ...
        if not cJSON_found:
            ...
            print("FAILURE : Unable to find device IPV4 Enable in cJSON report")
            print("\n[TEST STEP RESULT] : FAILURE\n")
            tdkTestObj.setResultStatus("FAILURE")
        else:
            print(cJSON_line)
        ...
        if cJSON_line and name in cJSON_line:
            try:
                ...
                if match:
                    ...
                    IPV4_Enable = data["Report"][0][name]
                    print("IPV4 Enable:", IPV4_Enable)
                    print("SUCCESS : Able to check whether IPV4 is enabled or not")
                    print("\n[TEST STEP RESULT] : SUCCESS\n");
                    tdkTestObj.setResultStatus("SUCCESS");
            except:
                print("FAILURE : Unable to check whether IPV4 is enabled or not")
                print("\n[TEST STEP RESULT] : FAILURE\n")
                tdkTestObj.setResultStatus("FAILURE")
        else:
            print("No JSON found in log line")
            print("Unable to check whether IPV4 is enabled or not from cJSON report")
