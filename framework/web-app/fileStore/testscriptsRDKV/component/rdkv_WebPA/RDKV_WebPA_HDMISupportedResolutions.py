...
<xml>
  ...
  <name>RDKV_WebPA_HDMISupportedResolutions</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the HDMI supported resolutions reported by the device using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_29</test_case_id>
    <test_objective>To validate the HDMI supported resolutions reported by the device using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.Services.STBService.1.Capabilities.HDMI.SupportedResolutions will fetch the WebPA server url from the device.
...
2. API must return the resolutions supported by the device</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_HDMISupportedResolutions</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_HDMISupportedResolutions')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.Services.STBService.1.Capabilities.HDMI.SupportedResolutions"
