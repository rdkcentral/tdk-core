...
<xml>
  ...
  <name>RDKV_WebPA_Get_PowerStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the device power status using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_34</test_case_id>
    <test_objective>To get the device power status using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_COMCAST-COM_PowerStatus will fetch the WebPA server url from the device.
...
2. API must return the power status of the device</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_Get_PowerStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_Get_PowerStatus')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_COMCAST-COM_PowerStatus"
    ...
    if "SUCCESS" in detail:
        ...
        print("Power Status : ", original_value)
        data_type = detail[2]
        if "SUCCESS" in detail:
            tdkTestObj.setResultStatus("SUCCESS")
