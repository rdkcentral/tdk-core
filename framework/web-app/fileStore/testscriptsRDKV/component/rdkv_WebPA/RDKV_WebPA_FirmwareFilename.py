...
<xml>
  ...
  <name>RDKV_WebPA_FirmwareFilename</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the firmware filename reported by the device using the WebPA interface.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_21</test_case_id>
    <test_objective>To validate the firmware filename reported by the device using the WebPA interface.</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_COMCAST-COM_FirmwareFilename will fetch the WebPA server url from the device.
...
2. API must return the Firmware File name</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_FirmwareFilename</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_FirmwareFilename')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_COMCAST-COM_FirmwareFilename"
