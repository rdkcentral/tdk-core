...
<xml>
  ...
  <name>RDKV_WebPA_Get_SoftwareVersion</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the device software version using the WebPA interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_35</test_case_id>
    <test_objective>To get the device software version using the WebPA interface</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.SoftwareVersion will fetch the WebPA server url from the device.
2. TM will verify the output by having a expected output string and cross verify.
...
2. API must return the software version of the device</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_Get_SoftwareVersion</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_Get_SoftwareVersion')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.SoftwareVersion"
    threshold_value = "100"
    ...
    if "SUCCESS" in detail:
        ...
        print("Software Version : ", original_value)
        data_type = detail[2]
        if "SUCCESS" in detail:
            tdkTestObj.setResultStatus("SUCCESS")

            print("\n")
            tdkTestObj = obj.createTestStep('integer_check')
            tdkTestObj.addParameter("value",original_value)
            tdkTestObj.addParameter("threshold_value",threshold_value)
            tdkTestObj.executeTestCase(expectedResult)
            detail = tdkTestObj.getResultDetails()
            print("\nDETAILS : ", detail)
            ...
            if "SUCCESS" in detail:
                tdkTestObj.setResultStatus("SUCCESS")
                print("\n Device software version is obtained successfully")
            else:
