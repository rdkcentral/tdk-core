...
<xml>
  ...
  <name>RDKV_WebPA_Collectd_PortNumber</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to validate the Port Value using WebPA server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>rdkvWebPA_03</test_case_id>
    <test_objective>Test to validate the Port Value using WebPA server</test_objective>
    ...
    <automation_approch>1. Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.collectd.PortNumber will fetch the WebPA server url from the device.
...
3. TM will return SUCCESS or FAILURE based on the result from the above step.</automation_approch>
    <expected_output>1.1. RFC parameter will give the server url from the device.
2.API must return the Port Number</expected_output>
    <priority>High</priority>
    <test_stub_interface></test_stub_interface>
    <test_script>RDKV_WebPA_Collectd_PortNumber</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebPA_Collectd_PortNumber')
...
if "SUCCESS" in result.upper():
    ...
    paramName = "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.collectd.PortNumber"
    testValue = "5436"
