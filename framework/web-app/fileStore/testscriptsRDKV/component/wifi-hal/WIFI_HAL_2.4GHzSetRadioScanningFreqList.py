...
<xml>
  <id/>
  <version>1</version>
  <name>WIFI_HAL_2.4GHzSetRadioScanningFreqList</name>
  <primitive_test_id/>
  <primitive_test_name>WIFI_HAL_SetRadioScanningFreqList</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To invoke wifi_setRadioScanningFreqList() api with radio index 0 to set the radio scanning frequency list</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFI_HAL_38</test_case_id>
    <test_objective>To invoke wifi_setRadioScanningFreqList() api with radio index 0 to set the radio scanning frequency list</test_objective>
...
3.If 2.4GHz is supported, invoke wifi_setRadioScanningFreqList() api with radio index 1 and list of frequency to be set
...
6.Unload the module.</automation_approch>
    ...
    <test_script>WIFI_HAL_2.4GHzSetRadioScanningFreqList</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'WIFI_HAL_2.4GHzSetRadioScanningFreqList');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if "2.4GHz" in SupportedFrequencyBands:
            ...
            tdkTestObj.addParameter("radioIndex",0);
            tdkTestObj.addParameter("freqList",tdkvWifiUtility.set_2ghz_freq_list);
            expectedresult="SUCCESS";
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            print "FREQ LIST TO BE SET : ",tdkvWifiUtility.set_2ghz_freq_list
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
            ...
            print "Frequency band 2.4GHz not supported : Skipping setRadioScanningFreqList validation"
            print "[TEST EXECUTION RESULT] : SUCCESS";
            tdkTestObj.setResultStatus("SUCCESS");
    else:
