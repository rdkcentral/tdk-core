...
<xml>
  ...
  <name>WIFI_HAL_5GHzGetSpecificSSIDInfo</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To invoke wifi_getSpecificSSIDInfo()  with frequency band 2 [ 5GHz ] to get the details of the Neighboring AP  whose SSID matches with the provided SSID</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.If 5GHz is supported, Get the list of Neighboring APs using wifi_getNeighboringWiFiDiagnosticResult()
3.Invoke wifi_getSpecificSSIDInfo() api with frequency band 2 [ 5 GHz ] for all the ssid listed by wifi_getNeighboringWiFiDiagnosticResult()
...
7.Unload the module.</automation_approch>
    ...
    <test_script>WIFI_HAL_5GHzGetSpecificSSIDInfo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'WIFI_HAL_5GHzGetSpecificSSIDInfo');
        ...
        if "5GHz" in SupportedFrequencyBands:
            ...
            if expectedresult in actualresult:
                ...
                for ssid in neighborSSID:
                    ...
                    tdkTestObj.addParameter("band",2);
                    tdkTestObj.executeTestCase(expectedresult);
                    sleep(3);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
