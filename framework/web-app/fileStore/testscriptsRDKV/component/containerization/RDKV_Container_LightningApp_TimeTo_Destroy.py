...
<xml>
  ...
  <name>RDKV_Container_LightningApp_TimeTo_Destroy</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the time taken to destroy LightningApp plugin in container mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_23</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to destroy LightningApp plugin in container mode</test_objective>
...
2. LIGHTNINGAPP_DESTROY_THRESHOLD_VALUE_IN_CONTAINER
...
5. Save current system time and destroy LightningApp using RDKShell.
6. Get the time from triggered event
7. Validate the output with threshold value</automation_approch>
    <expected_output>LightningApp should be destroyed and time taken to destroy LightningApp should be within the expected limit</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_LightningApp_TimeTo_Destroy</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_LightningApp_TimeTo_Destroy');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            lightningapp_destroy_threshold = configValues["LIGHTNINGAPP_DESTROY_THRESHOLD_VALUE_IN_CONTAINER"]
