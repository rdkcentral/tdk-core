...
<xml>
  ...
  <name>RDKV_Container_HtmlApp_ResourceUsage_LoadURL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>HtmlApp URL should be launched successfully in container mode and resource usage must be within the expected limit</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>HtmlApp URL should be launched successfully in container mode and resource usage must be within the expected limit </test_objective>
    ...
    <input_parameters>html_playback_url </input_parameters>
...
5. Launch HtmlApp in running in container mode
6. Set up the url in HtmlApp
7. Check wpeframework.log for the url changed log
9. Calculate resource usage</automation_approch>
    <expected_output>HtmlApp URL should be launched successfully in container mode and resource usage must be within the expected limit</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_HtmlApp_ResourceUsage_LoadURL</test_script>
    <skipped>No</skipped>
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            html_playback_url = configValues["HTMLAPP_PLAYBACK_URL"]
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching HtmlApp in container mode" in output:
                        ...
                        if(html_result in expectedResult):
                            tdkTestObj.setResultStatus("SUCCESS")
                            command = 'cat /opt/logs/wpeframework.log | grep -inr URLChanged.*url.*'+html_playback_url+'| tail -1'
                            #Primitive test case which associated to this Script
                            tdkTestObj = obj.createTestStep('containerization_executeInDUT');
                            #Add the parameters to ssh to the DUT and execute the command
                            tdkTestObj.addParameter("sshMethod", configValues["SSH_METHOD"]);
