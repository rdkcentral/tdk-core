...
<xml>
  ...
  <name>RDKV_Container_Cobalt_Launch_StandbyToOn_StressTest</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate whether Cobalt is in contianer mode after changing Standby to On for 5 iterations</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_59</test_case_id>
    <test_objective>The objective of this test is to validate whether Cobalt is in contianer mode after changing Standby to On for 5 iterations</test_objective>
    ...
    <automation_approch>1. Launch Cobalt in contianer mode as pre condition.
...
6. Verify whether Cobalt is still running in container mode.
</automation_approch>
    <expected_output>Cobalt should be in contianer mode after changing StandbyOn for 5 iterations</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Cobalt_Launch_StandbyToOn_StressTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Cobalt_Launch_StandbyToOn_StressTest');
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching cobalt in container mode" in output:
                        ...
                        if expectedResult in result and preferred_standby == "LIGHT_SLEEP":
                            ...
                            if expectedResult in result and current_power_state == "STANDBY":
                                for count in range(0,max_powerstate_changes):
                                    ...
                                    actualresult = tdkTestObj.getResultDetails()
                                    if expectedResult in actualresult.upper():
                                        print "\n Cobalt is running in container mode without any issues"
                                        tdkTestObj.setResultStatus("SUCCESS")
                                    else:
                        ...
                        print "\n Error in launching Cobalt in container mode"
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print "\n Error in checking the container running state"
                    tdkTestObj.setResultStatus("FAILURE")
            else:
                print "\n Unable to launch Cobalt in container mode"
                tdkTestObj.setResultStatus("FAILURE")
        else:
