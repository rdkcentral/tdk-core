...
<xml>
  ...
  <name>RDKV_Container_HtmlApp_Timeto_Video_PlayPause</name>
  ...
  <test_cases>
    <test_case_id>Containerization_57</test_case_id>
    ...
    HTMLAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER
    HTMLAPP_PLAYBACK_URL
    HTMLAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER</input_parameters>
...
10. Calculate time to pause and play from wpeframework.log
    ...
    <test_script>RDKV_Container_HtmlApp_Timeto_Video_PlayPause</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_HtmlApp_Timeto_Video_PlayPause');
...
if expectedResult in result.upper():
    ...
    configKeyList = ["SSH_METHOD", "SSH_USERNAME", "SSH_PASSWORD", "HTMLAPP_DETAILS", "HTMLAPP_PLAYBACK_URL", "HTMLAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER", "HTMLAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER", "THRESHOLD_OFFSET_CONTAINER"]
    configValues = {}
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            HtmlApp_playback_url = configValues["HTMLAPP_PLAYBACK_URL"]
            HtmlApp_pause_threshold = configValues["HTMLAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER"]
            HtmlApp_play_threshold = configValues["HTMLAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER"]
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching HtmlApp in container mode" in output:
                        ...
                        print("\n Set the URL : {} using HtmlApp")
                        tdkTestObj = obj.createTestStep('containerization_setValue')
                        tdkTestObj.addParameter("method","HtmlApp.1.url")
                        tdkTestObj.addParameter("value",HtmlApp_playback_url)
                        tdkTestObj.executeTestCase(expectedResult)
                        HtmlApp_result = tdkTestObj.getResult()
                        time.sleep(10)
                        if(HtmlApp_result in expectedResult):
                            tdkTestObj.setResultStatus("SUCCESS")
                                                ...
                                                if video_pausedtime_in_millisec > pause_starttime_in_millisec:
                                                    ...
                                                    if result == "SUCCESS":
                                                        ...
                                                        if output != "EXCEPTION" and expectedResult in result and "old: PAUSED" in output:
                                                            ...
                                                            result1,HtmlApp_pause_threshold = getDeviceConfigKeyValue(conf_file,"HTMLAPP_PAUSE_TIME_THRESHOLD_VALUE_CONTAINER")
                                                            result2,HtmlApp_play_threshold = getDeviceConfigKeyValue(conf_file,"HTMLAPP_PLAY_TIME_THRESHOLD_VALUE_CONTAINER")
                                                            offset_status,offset = getDeviceConfigKeyValue(conf_file,"THRESHOLD_OFFSET_CONTAINER")
                                                            if all(value != "" for value in (HtmlApp_pause_threshold,HtmlApp_play_threshold,offset)):
                                                                ...
                                                                print("\n Threshold value for time taken for play operation : {} ms".format(HtmlApp_play_threshold))
                                                                if 0 < int(time_for_video_play) < (int(HtmlApp_play_threshold) + int(offset)):
                                                                ...
                                                                print("\n Threshold value for time taken for pause operation : {} ms".format(HtmlApp_pause_threshold))
                                                                if 0 < int(time_for_video_pause) < (int(HtmlApp_pause_threshold) + int(offset)):
