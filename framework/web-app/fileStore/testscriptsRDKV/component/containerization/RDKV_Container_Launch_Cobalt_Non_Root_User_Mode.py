...
<xml>
  ...
  <name>RDKV_Container_Launch_Cobalt_Non_Root_User_Mode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To launch Cobalt in container mode and verify it runs in non-root user mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_42</test_case_id>
    <test_objective>To launch Cobalt and verify if its in non-root user mode</test_objective>
...
8. Verify if Cobalt is running in non-root user mode</automation_approch>
    <expected_output>Cobalt should be launched in container mode and must run in non-root user mode</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Launch_Cobalt_Non_Root_User_Mode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Launch_Cobalt_Non_Root_User_Mode');
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching cobalt in container mode" in output:
                        ...
                        print "Verify if Cobalt is launched in non-root user mode"
                        command = 'ps -aux | grep cobalt | grep -v root'
                        print "COMMAND : %s" %(command)
                        #Primitive test case which associated to this Script
                        tdkTestObj = obj.createTestStep('containerization_executeInDUT');
                        ...
                        print "output ", output
                        if "Cobalt" in output:
                            print "Cobalt launched in non-root user mode"
                        else:
                            print "Cobalt not launched in non-root user mode"
