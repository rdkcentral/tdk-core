...
<xml>
  ...
  <name>RDKV_Container_Thunder_Restart_HtmlApp</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>HtmlApp should play video in container mode and launch after thunder restart</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_28</test_case_id>
    <test_objective>HtmlApp should play video in container mode and launch after thunder restart</test_objective>
...
3. Reboot the device or restart Thunder
...
11. Verify that HtmlApp is running in container mode.</automation_approch>
    <expected_output>HtmlApp should play video in container mode and launch after thunder restart</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Thunder_Restart_HtmlApp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Thunder_Restart_HtmlApp');
                            ...
                            if "SUCCESS" == result1:
                                ...
                                if expectedResult in result:
                                    ...
                                    if expectedResult not in actualresult.upper():
                                        print "HtmlApp is not running after Thunder Restart"
                                        print "Launch HtmlApp"
                                        tdkTestObj = obj.createTestStep('containerization_launchApplication')
                                        tdkTestObj.addParameter("launch",htmlapp_details)
                                        tdkTestObj.executeTestCase(expectedResult)
                                        ...
                                        if expectedResult in actualresult.upper():
                                            ...
                                            if expectedResult in actualresult.upper():
                                                ...
                                                if "launching HtmlApp in container mode" in output:
                                                    ...
                                                    print "Unable to get the containerization HtmlApp launch logs"
                                                    tdkTestObj.setResultStatus("FAILURE")
                                            else:
                                                print "HtmlApp is not running in container mode"
                                                tdkTestObj.setResultStatus("FAILURE")
                                        else:
                                            print "HtmlApp launch failed"
                                            tdkTestObj.setResultStatus("FAILURE")
                                    else:
                                        print "HtmlApp is running even after Thunder Restart"
                                        tdkTestObj.setResultStatus("FAILURE")
                                else:
