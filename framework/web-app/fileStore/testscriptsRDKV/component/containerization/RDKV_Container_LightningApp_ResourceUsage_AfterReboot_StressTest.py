...
<xml>
  ...
  <name>RDKV_Container_LightningApp_ResourceUsage_AfterReboot_StressTest</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>In a loop of 5 times, Validate resource usage for LightningApp after Reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_66</test_case_id>
    <test_objective>In a loop of 5 times, Validate Resource Usage for LightningApp after Reboot</test_objective>
...
2. In a loop of 5 times, Reboot the device
3. Enable datamodel values
4. Reboot the device or restart WPEFramework service
5. Verify datamodel values
6. Launch LightningApp
7. Verify that LightningApp is running in container mode
8. Validate resource usage</automation_approch>
    ...
    <test_script>RDKV_Container_LightningApp_ResourceUsage_AfterReboot_StressTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_LightningApp_ResourceUsage_AfterReboot_StressTest');
		...
		if expectedResult in actualresult.upper():
			...
			if expectedResult in actualresult.upper():
				...
				if expectedResult in result and resource_usage != "ERROR":
			        ...
			        print "Unable to get the logs 'launching LightningApp in container mode' from wpeframework logs"
				tdkTestObj.setResultStatus("FAILURE")
			else:
