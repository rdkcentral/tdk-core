...
<xml>
  ...
  <name>RDKV_Container_HtmlApp_LifecycleManagement_StressTest</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to do lifecycle management of HtmlApp plugin in container mode for 5 times and validate the resource usage.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_62</test_case_id>
    <test_objective>The objective of this test is to do lifecycle management of HtmlApp plugin in container mode for 5 times and validate the resource usage.</test_objective>
...
8. Set given URL using HtmlApp.1.url method.
...
15. Validate the resource usage</automation_approch>
    ...
    <test_script>RDKV_Container_HtmlApp_LifecycleManagement_StressTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_HtmlApp_LifecycleManagement_StressTest');
...
if expectedResult in result.upper():
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            html_page_url = configValues["HTMLAPP_PLAYBACK_URL"]
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching HtmlApp in container mode" in output:
                        ...
                        plugin_validation_details = ["HtmlApp.1.url",html_page_url]
                        ...
                        print "Unable to get the logs 'launching HtmlApp in container mode' from wpeframework logs"
                        tdkTestObj.setResultStatus("FAILURE")
                else:
