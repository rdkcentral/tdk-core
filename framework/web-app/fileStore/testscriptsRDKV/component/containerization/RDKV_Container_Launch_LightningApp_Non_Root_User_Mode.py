...
<xml>
  ...
  <name>RDKV_Container_Launch_LightningApp_Non_Root_User_Mode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To launch LightningApp in container mode and verify it runs in non-root user mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_41</test_case_id>
    <test_objective>To launch LightningApp and verify if its in non-root user mode</test_objective>
...
8. Verify if LightningApp is running in non-root user mode</automation_approch>
    <expected_output>LightningApp should be launched in container mode and must run in non-root user mode</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Launch_LightningApp_Non_Root_User_Mode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Launch_LightningApp_Non_Root_User_Mode');
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching LightningApp in container mode" in output:
                        ...
                        output = tdkTestObj.getResultDetails()
                        print "output ", output
                        if "LightningApp" in output:
                            print "LightningApp launched in non-root user mode"
                        else:
                            print "LightningApp not launched in non-root user mode"
