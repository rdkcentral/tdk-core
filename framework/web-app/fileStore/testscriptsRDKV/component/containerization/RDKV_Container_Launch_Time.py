...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Calculate launch time for cobalt in container and non-container mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
7. Check wpeframework.log</automation_approch>
    <expected_output>Launch time for cobalt in container and non-container mode must almost be same</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Launch_Time</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Launch_Time');
...
if expectedResult in result.upper():
    ...
    if expectedResult in actualresult.upper():
        ...
        command = 'cat /opt/logs/wpeframework.log | grep "Application:Cobalt took" | tail -1'
