...
<xml>
  ...
  <name>RDKV_Container_Cobalt_TimeTo_Destroy</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate the time taken to destroy Cobalt plugin in container mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_56</test_case_id>
    <test_objective>The objective of this test is to validate the time taken to destroy Cobalt plugin in container mode</test_objective>
...
2. COBALT_DESTROY_THRESHOLD_VALUE_IN_CONTAINER
...
5. Save current system time and destroy Cobalt using RDKShell.
6. Get the time from triggered event
7. Validate the output with threshold value</automation_approch>
    <expected_output>Cobalt should be destroyed and time taken to destroy Cobalt should be within the expected limit</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Cobalt_TimeTo_Destroy</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Cobalt_TimeTo_Destroy');
...
if expectedResult in result.upper():
    ...
    configKeyList = ["SSH_METHOD", "SSH_USERNAME", "SSH_PASSWORD", "COBALT_DETAILS","COBALT_DESTROY_THRESHOLD_VALUE_IN_CONTAINER", "THRESHOLD_OFFSET_IN_CONTAINER"]
    configValues = {}
    ...
    if "FAILURE" != result:
        if "directSSH" == configValues["SSH_METHOD"]:
            ...
            cobalt_destroy_threshold = configValues["COBALT_DESTROY_THRESHOLD_VALUE_IN_CONTAINER"]
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        if expectedResult in actualresult.upper():
            ...
            if expectedResult in actualresult.upper():
                ...
                if expectedResult in actualresult.upper():
                    tdkTestObj.setResultStatus("SUCCESS")
                    print "Destroying Cobalt in container mode:"
                    ...
                    if result == "SUCCESS":
                        ...
                        if Cobalt_status == 'deactivated' and expectedResult in result:
                            print "Cobalt is destroyed successfully"
                            #Check for Container launch logs
                            command = 'cat /opt/logs/wpeframework.log | grep "onDestroyed event received for Cobalt" | tail -1'
                            print "COMMAND : %s" %(command)
                            #Primitive test case which associated to this Script
                            tdkTestObj = obj.createTestStep('containerization_executeInDUT');
                            ...
                            result = tdkTestObj.getResult()
                            output = tdkTestObj.getResultDetails()
                            if expectedResult in result and output != " ":
                                print "onDestroyed event received for Cobalt in wpeframework logs"
                                pattern = r"\b\d{2}:\d{2}:\d{2}\.\d+\b"
                                matches = re.findall(pattern, output)
                                destroyed_time = matches[len(matches)-1]
                                if destroyed_time:
                                    print "\n Validate the time taken to destroy Cobalt in container mode"
                                    if all(value != "" for value in (cobalt_destroy_threshold,threshold_offset)):
                                        ...
                                        if 0 < time_taken_for_destroy < (int(cobalt_destroy_threshold) + int(threshold_offset)) :
                                    ...
                                    print "\n onDestroyed event is not received for Cobalt \n"
                                    tdkTestObj.setResultStatus("FAILURE")
                            else:
                                print "\n Error in gettin the status of the plugins"
                                tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print "Error in destroying Cobalt in container mode"
                        tdkTestObj.setResultStatus("FAILURE")
                else:
                    print "Cobalt is not running in container mode"
                    tdkTestObj.setResultStatus("FAILURE")
            else:
                print "Failed to destroy Cobalt"
                tdkTestObj.setResultStatus("FAILURE")
        else:
