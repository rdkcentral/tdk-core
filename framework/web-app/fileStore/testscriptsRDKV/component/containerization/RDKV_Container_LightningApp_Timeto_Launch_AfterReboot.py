...
<xml>
  ...
  <name>RDKV_Container_LightningApp_Timeto_Launch_AfterReboot</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The objective of this test is to validate time taken to launch LightningApp after reboot.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_51</test_case_id>
    <test_objective>Validate time to launch LightningApp after reboot</test_objective>
...
11. Verify that LightningApp is running in container mode.</automation_approch>
    <expected_output>Time taken to launch LightningApp after reboot</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_LightningApp_Timeto_Launch_AfterReboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_LightningApp_Timeto_Launch_AfterReboot');
                        ...
                        if "launching LightningApp in container mode" in output:
                            ...
                            if launched_time:
                                ...
                                if all(value != "" for value in (lightningapp_launch_threshold,threshold_offset)):
                                    ...
                                    print "\n Threshold value for time taken to launch LightningApp after reboot: {}ms".format(lightningapp_launch_threshold)
