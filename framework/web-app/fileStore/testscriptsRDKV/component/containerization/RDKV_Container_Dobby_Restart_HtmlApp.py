...
<xml>
  ...
  <name>RDKV_Container_Dobby_Restart_HtmlApp</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>HtmlApp should play video in container mode and launch after dobby restart</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>Containerization_31</test_case_id>
    <test_objective>HtmlApp should play video in container mode and launch after dobby restart</test_objective>
...
11. Verify that HtmlApp is running in container mode.</automation_approch>
    <expected_output>HtmlApp should play video in container mode and launch after dobby restart</expected_output>
    <priority>High</priority>
    <test_stub_interface>containerization</test_stub_interface>
    <test_script>RDKV_Container_Dobby_Restart_HtmlApp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Container_Dobby_Restart_HtmlApp');
                ...
                if expectedResult in actualresult.upper():
                    ...
                    if "launching HtmlApp in container mode" in output:
                        ...
                        if(html_result in expectedResult):
                            ...
                            time.sleep(50)
                            if "SUCCESS" == result1:
                                command = 'systemctl restart dobby'
                                tdkTestObj = obj.createTestStep('containerization_executeInDUT');
                                tdkTestObj.addParameter("sshMethod", configValues["SSH_METHOD"]);
                                tdkTestObj.addParameter("credentials", credentials);
                                tdkTestObj.addParameter("command", command);
                                ...
                                if expectedResult in result:
                                    ...
                                    if expectedResult not in actualresult.upper():
                                        print "HtmlApp is not running after Dobby Restart"
                                        print "Launch HtmlApp"
                                        tdkTestObj = obj.createTestStep('containerization_launchApplication')
                                        tdkTestObj.addParameter("launch",htmlapp_details)
                                        tdkTestObj.executeTestCase(expectedResult)
                                        ...
                                        if expectedResult in actualresult.upper():
                                            ...
                                            if expectedResult in actualresult.upper():
                                                ...
                                                if "launching HtmlApp in container mode" in output:
                                                    ...
                                                    print "Unable to get containerization HtmlApp launch logs"
