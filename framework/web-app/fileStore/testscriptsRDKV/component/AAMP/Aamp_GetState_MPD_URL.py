...
<xml>
  ...
  <name>Aamp_GetState_MPD_URL</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>Aamp_AampGetState</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to get the current State value for MPD stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_54</test_case_id>
    <test_objective>To get the current State</test_objective>
...
AampStop
AampGetState</api_or_interface_used>
...
3. Aamp Agent invokes GetState API
...
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should get the valid State value</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_GetState_MPD_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_GetState_MPD_URL');
sysObj.configureTestCase(ip,port,'Aamp_GetState_MPD_URL');
	...
	if expectedResult in actualResult:
		...
		if expectedResult in actualResult:
			...
			print "Result :", details;
	        	...
	        	if expectedResult in actualResult and "PLAYING" in details:
        		        #Set the result status of execution
                		tdkTestObj.setResultStatus("SUCCESS");
				print "Current state: ", details;
				print "State retrieved is valid for MPD stream";
			else:
        		        #Set the result status of execution
                		tdkTestObj.setResultStatus("FAILURE");
				print "State not retrievd correctly for MPD stream";
		else:
