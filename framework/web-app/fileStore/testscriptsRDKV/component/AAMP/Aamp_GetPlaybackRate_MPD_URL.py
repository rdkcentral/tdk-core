...
<xml>
  ...
  <name>Aamp_GetPlaybackRate_MPD_URL</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>Aamp_AampGetPlaybackRate</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to get the current playback rate for MPD stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_67</test_case_id>
    <test_objective>To get the current playback rate</test_objective>
...
AampStop
AampGetPlaybackRate</api_or_interface_used>
    <input_parameters>MPD URL</input_parameters>
    <automation_approch>1. TM loads the Aamp Agent and systemutil Agent.
2. Aamp Agent invokes Tune API with MPD URL
3. Aamp Agent invokes GetPlaybackRate API
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_GetPlaybackRate_MPD_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_GetPlaybackRate_MPD_URL');
sysObj.configureTestCase(ip,port,'Aamp_GetPlaybackRate_MPD_URL');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
	...
	if expectedResult in actualResult:
		...
		if expectedResult in actualResult:
			print "AAMP Tune event received"
			print "[TEST EXECUTION RESULT] : %s" %actualResult;
			#Set the result status of execution
			tdkTestObj.setResultStatus("SUCCESS");

			sleep(30);
                        tdkTestObj = aampObj.createTestStep('Aamp_AampGetState');
                        expectedResult = "SUCCESS";
                        #Execute the test case in STB
                        tdkTestObj.executeTestCase(expectedResult);
                        #Get the result of execution
			...
			if expectedResult in actualResult:
                        	state = tdkTestObj.getResultDetails();
	                        print "State :", state;
				#Set the result status of execution
				tdkTestObj.setResultStatus("SUCCESS");

				tdkTestObj = aampObj.createTestStep('Aamp_AampGetPlaybackRate');
				expectedResult = "SUCCESS";
				#Execute the test case in STB
				tdkTestObj.executeTestCase(expectedResult);
				#Get the result of execution
				...
				if expectedResult in actualResult:
					...
					print "Result :", details;
                                        ...
                                        print rate;
                                        if rate != 0 and "PLAYING" in state or rate ==0 and "PAUSED":
						#Set the result status of execution
						tdkTestObj.setResultStatus("SUCCESS");
						print "Playback rate retrieved is valid for MPD stream";
					elif rate == 0.0 and "PLAYING" in states[index]:
						#Set the result status of execution
						tdkTestObj.setResultStatus("FAILURE");
						print "Current state is playing but playback rate retrieved is 0";
					else:
						#Set the result status of execution
						tdkTestObj.setResultStatus("FAILURE");
						print "Playback rate not retrieved correctly for MPD stream";
				else:
					#Set the result status of execution
	                                tdkTestObj.setResultStatus("FAILURE");
        	                        print "Aamp_AampGetPlaybackRate call failed";
			else:
				#Set the result status of execution
                                tdkTestObj.setResultStatus("FAILURE");
                                print "Aamp_AampGetState call failed";
		else:
