...
<xml>
  ...
  <name>Aamp_Set_PreferredAudioLanguages_Non_IdleState</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to set  preferred language list during non idle player state</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_47</test_case_id>
    <test_objective>Test Script to set  preferred language list during non idle player state</test_objective>
...
5. Generate language code list except current audio language from the available audio tracks.
6.Aamp Agent invokes SetCurrentAudioLanguage  API to set the preferred language list at player non idle state.
7.Aamp Agent invokes AampGetPreferredLanguages API
8.Aamp Agent invokes GetCurrentAudioLanguage API
9. GetPreferredLanguages  should be empty and current language should be the same
10. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should not be able to set the preferred languages at player non-idle state and get Preferred language list should be empty and current language should not get updated.</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Set_PreferredAudioLanguages_Non_IdleState</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Set_PreferredAudioLanguages_Non_IdleState');
sysObj.configureTestCase(ip,port,'Aamp_Set_PreferredAudioLanguages_Non_IdleState');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
        ...
        if "SUCCESS" in tuneStatus:
            ...
            # Setting the preferred language during player non idle state
            if getLangCodesStatus == "SUCCESS" :
                ...
                if expectedResult in actualResult:
                    ...
                    print actualResult;
                    details = tdkTestObj.getResultDetails();
                    if expectedResult in actualResult and "not set" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "Result :", details;
                        print "Preferred language not set when player is non idle"
                        print "[TEST EXECUTION RESULT] : SUCCESS\n"
                        ...
                        if expectedResult in actualResult and new_lang == curr_lang:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "Result :", details;
                            print "Current Audio language is not updated"
                            print "[TEST EXECUTION RESULT] : SUCCESS\n"
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print details;
                            print "Current Audio language is updated"
                            print "[TEST EXECUTION RESULT] : FAILURE\n"
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print details;
