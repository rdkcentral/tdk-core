...
<xml>
  ...
  <name>Aamp_SetandGet_Video_Bitrate_InvalidValue_MPD_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to set and get the video bitrate with invalid value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_62</test_case_id>
    <test_objective>Test Script to set and get the video bitrate with invalid value</test_objective>
...
3. Aamp Agent invokes SetVideoBitrates API with invalid bitrate
4. Aamp Agent invokes GetVideoBitrates API to get the current bitrate
5. TM checks if the value is not set and returns SUCCESS/FAILURE
6. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should not set the invalid video bitrate value</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_SetandGet_Video_Bitrate_InvalidValue_MPD_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_SetandGet_Video_Bitrate_InvalidValue_MPD_URL');
sysObj.configureTestCase(ip,port,'Aamp_SetandGet_Video_Bitrate_InvalidValue_MPD_URL');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
	...
	if expectedResult in actualResult:
		...
		if expectedResult in actualResult:
			...
			#Supported bitrates comes in the range of lakhs. So testing with small value
			rate = "100";
			...
			if expectedResult in actualResult:
				...
				if expectedResult in actualResult and rate not in details:
				 ...
				 print "Aamp_AampSetVideoBitrate call failed";
                        ...
                        #So testing with negative value
                        rate = "-20";
                        tdkTestObj = aampObj.createTestStep('Aamp_AampSetVideoBitrate');
                        tdkTestObj.addParameter("bitRate", int(rate));
                        expectedResult = "SUCCESS";
