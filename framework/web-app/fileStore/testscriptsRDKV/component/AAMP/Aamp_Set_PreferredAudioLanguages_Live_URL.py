...
<xml>
  ...
  <name>Aamp_Set_PreferredAudioLanguages_Live_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to set  preferred language list</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_43</test_case_id>
    <test_objective>Test Script to set  preferred language list</test_objective>
...
10. GetPreferredLanguages  should be same as that of language list set and current language should be the most preferred language
11. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should be able to set the list of preferred language and current language should get updated to most preferred one (i.e) the first language in the list</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Set_PreferredAudioLanguages_Live_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Set_PreferredAudioLanguages_Live_URL');
sysObj.configureTestCase(ip,port,'Aamp_Set_PreferredAudioLanguages_Live_URL');
            ...
            if getLangCodesStatus == "SUCCESS" and stopStatus == "SUCCESS":
                ...
                if len(lang_codes) > 0:
                    if len(lang_codes) > 1:
                        ...
                        print "Shuffled language code : ",lang_codes
                ...
                if expectedResult in actualResult:
                    ...
                    if "SUCCESS" in tuneStatus:
                        ...
                        if expectedResult in actualResult and lang_list in str(details):
                            ...
                            if len(lang_codes) > 0 and new_lang == lang_codes[0]:
                                lang_update = "True"
                            elif new_lang in lang_list:
                                lang_update = "True"
                            else:
                                lang_update = "False"
                            if expectedResult in actualResult and lang_update == "True":
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Result :", details;
                                print "Current Audio language is same as that of most preferred language"
                                print "[TEST EXECUTION RESULT] : SUCCESS\n"

                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print details;
                                print "Current Audio language is not same as most preferred language"
                                print "[TEST EXECUTION RESULT] : FAILURE\n"
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print details;
