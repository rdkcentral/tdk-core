...
<xml>
  ...
  <name>Aamp_Get_VideoZoom_check_In_Pause</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to verify the zoom mode in pause scenario</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test script to verify the zoom mode in pause scenario</test_objective>
...
AampGetVideoZoom
AampGetPlaybackPosition
AampPauseAtPosition</api_or_interface_used>
...
3. Aamp Agent invokes GetVideoZoom API to get the current zoom moe
4. Aamp Agent invokes AampPauseAtPosition API to pause the video
5. Aamp Agent invokes GetVideoZoom API again to get the zoom mode and verify with the before pause scenario.
6. TM checks if the value is equal and returns SUCCESS/FAILURE
7. TM unloads the Aamp Agent</automation_approch>
    <expected_output>Should get the same zoom mode after pause functionality too</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Get_VideoZoom_check_In_Pause</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Get_VideoZoom_check_In_Pause');
...
if ("SUCCESS" in aampLoadStatus.upper()):
    ...
    if result and aampUtilitylib.SearchAampPlayerEvents(tdkTestObj,pattern) == "SUCCESS":
        ...
        if expectedResult in actualResult:
            ...
            #get the current playback position to pass it for pause operation
            ...
            if expectedResult in actualResult:
                ...
                if expectedResult in result:
                    ...
                    if paused:
                        #get the zoom mode & verify its same after pause operation
                        ...
                        if expectedResult in actualResult:
                            zoom_mode  = int(details.split(':')[-1]);
                            if cur_zoommode == zoom_mode:
                                print("Same VideoZoom mode retrived afer pause operation");
                                tdkTestObj.setResultStatus("SUCCESS");
                            else:
                                print("Different VideoZoom mode retrieved after pause operation");
                                tdkTestObj.setResultStatus("FAILURE");
                        else:
                            print("VideoZoom mode not received");
                            tdkTestObj.setResultStatus("FAILURE");
                else:
