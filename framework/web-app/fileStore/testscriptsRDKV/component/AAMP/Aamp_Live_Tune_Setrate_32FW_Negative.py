...
<xml>
  ...
  <name>Aamp_Live_Tune_Setrate_32FW_Negative</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if FW is working for live mpd stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_27</test_case_id>
    <test_objective>Check if FW is working for live mpd stream</test_objective>
...
7.Set the script status as per the Log comparison</automation_approch>
    <except_output>Speed should not get changed</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Live_Tune_Setrate_32FW_Negative</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Live_Tune_Setrate_32FW_Negative');
sysObj.configureTestCase(ip,port,'Aamp_Live_Tune_Setrate_32FW_Negative');
			...
			tdkTestObj.addParameter("rate",32.0);
		        ...
		        result = tdkTestObj.getResult();
                        ...
                        if expectedResult in result:
                                ...
                                if expectedResult in result:
                                        print "Setrate failed for live stream"
                                        print "[TEST EXECUTION RESULT] : SUCCESS";
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("SUCCESS")
                                else:
                                        print "Setrate success for live stream which is not expected"
