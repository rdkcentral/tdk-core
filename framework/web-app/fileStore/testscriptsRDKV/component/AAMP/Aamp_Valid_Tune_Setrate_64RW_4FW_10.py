...
<xml>
  ...
  <name>Aamp_Valid_Tune_Setrate_64RW_4FW_10</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Tune_Setrate_ with 64RW and 4FW</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_10</test_case_id>
    <test_objective>Valid_Tune_Setrate_ for 64RW and 4FW</test_objective>
...
7.Set the script status as per the Log comparison</automation_approch>
    <except_output>AampSetrate worked and speed changed</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Valid_Tune_Setrate_64RW_4FW_10</test_script>
    <skipped>No</skipped>
...
aampobj.configureTestCase(ip,port,'Aamp_Valid_Tune_Setrate_64RW_4FW_10');
sysobj.configureTestCase(ip,port,'Aamp_Valid_Tune_Setrate_64RW_4FW_10');
...
if ("SUCCESS" in aamp_status.upper()) and ("SUCCESS" in sysutil_status.upper()):
	...
	if Expected_Result in result:
		...
		if Expected_Result in result:
			...
			setrates = [-64.0, 4.0]
			for setrate in setrates:
				#AampSetRate call
				tdkTestObj = aampobj.createTestStep('Aamp_AampSetRate');
				tdkTestObj.addParameter("rate",setrate);
			        #Execute the test case in STB
			        tdkTestObj.executeTestCase(Expected_Result);
			        #Get the result of execution
			        result = tdkTestObj.getResult();
				if Expected_Result in result:
					...
					print "AAMP setrate call failed for rate :%s"%setrate;
					#Set the result status of execution
					tdkTestObj.setResultStatus("FAILURE");
