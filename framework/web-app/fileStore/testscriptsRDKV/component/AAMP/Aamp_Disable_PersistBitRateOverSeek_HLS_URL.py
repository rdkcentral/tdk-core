...
<xml>
  ...
  <name>Aamp_Disable_PersistBitRateOverSeek_HLS_URL</name>
  ...
  <test_cases>
    <test_case_id>CT_Aamp_85</test_case_id>
...
5. Aamp Agent invokes SetVideoBitrate API to set with a different bitrate other than the defaul one.
6. Aamp Agent invokes PersistBitRateOverSeek API with the argument false to verify the disable persist functionality.
...
10. TM unloads the Aamp Agent.</automation_approch>
    <expected_output>Should not perist the bitrate value</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Disable_PersistBitRateOverSeek_HLS_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Disable_PersistBitRateOverSeek_HLS_URL');
    ...
    if result and aampUtilitylib.SearchAampPlayerEvents(tdkTestObj,pattern) == "SUCCESS":
        ...
        if result:
            ...
            if len(bitrates) <2:
                ...
                #Disable PersistBitRateOverSeek functionality
                ...
                if expectedResult in actualResult:
                    ...
                    if Expected_Result in result:
                        ...
                        if rate == 0:
                            ...
                            if rate == initial_bitrate:
                                print("Video Bitrate value not persisted.");
                                print("[TEST EXECUTION RESULT] : SUCCESS");
                                tdkTestObj.setResultStatus("SUCCESS");
                            else:
                                print("Video Bitrate persistence value is displaying. It shouldn't persisted.");
                                print("[TEST EXECUTION RESULT] : FAILURE");
                                tdkTestObj.setResultStatus("FAILURE");
                        result,details = executeTest(aampObj, 'Aamp_AampStop');
                else:
