...
<xml>
  ...
  <name>Aamp_Live_Tune_VideoDecoderHandle_Test_13</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Live_Tune_VideoDecoderHandle_Test</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_13</test_case_id>
    <test_objective>Live_Tune_VideoDecoderHandle_Test</test_objective>
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Live_Tune_VideoDecoderHandle_Test_13</test_script>
    <skipped>No</skipped>
...
aampobj.configureTestCase(ip,port,'Aamp_Live_Tune_VideoDecoderHandle_Test_13');
sysobj.configureTestCase(ip,port,'Aamp_Live_Tune_VideoDecoderHandle_Test_13');
...
if ("SUCCESS" in aamp_status.upper()) and ("SUCCESS" in sysutil_status.upper()):
        ...
        if Expected_Result in result:
			...
			pattern="AAMP_EVENT_CC_HANDLE_RECEIVED"
                        ...
                        result=aampUtilitylib.SearchAampPlayerEvents(tdkTestObj,pattern);
                        if Expected_Result in result:
				print "SUCCESS:AAMP Live Tune events are verified:video decoders handle are  retrieved"
                                print "[TEST EXECUTION RESULT] : SUCCESS"
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS")
			else:
		                print "FAILURE:AAMP events are not received:No video decoder handle is retrieved"
        	        	#Set the result status of execution
