...
<xml>
  ...
  <name>Aamp_GetVideo_Bitrate_MPD_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to check if the video bitrate is valid</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_72</test_case_id>
    <test_objective>Test Script to check if the video bitrate is valid</test_objective>
...
4. Aamp Agent invokes GetVideoBitrate API to get the current bitrate
5. TM checks if the value is valid and returns SUCCESS/FAILURE
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_GetVideo_Bitrate_MPD_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_GetVideo_Bitrate_MPD_URL');
sysObj.configureTestCase(ip,port,'Aamp_GetVideo_Bitrate_MPD_URL');
	...
	if expectedResult in actualResult:
		...
		if expectedResult in actualResult:
			...
			tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        tdkTestObj = aampObj.createTestStep('Aamp_AampGetVideoBitrates');
                        expectedResult = "SUCCESS";
                        #Execute the test case in STB
                        tdkTestObj.executeTestCase(expectedResult);
                        #Get the result of execution
			...
			print details;
				...
				if expectedResult in actualResult:
					rate = details.split(':')[-1].strip(' ');
					print rate;
					if rate in bitrates:
 	 					#Set the result status of execution
						tdkTestObj.setResultStatus("SUCCESS");
						print "Video bitrate is valid: ", rate;
					else:
						#Set the result status of execution
						tdkTestObj.setResultStatus("FAILURE");
						print "Video bitrate is not valid";
				else:
					#Set the result status of execution
					tdkTestObj.setResultStatus("FAILURE");
					print "Aamp_AampGetVideoBitrate call failed";
			else:
				 #Set the result status of execution
                                 tdkTestObj.setResultStatus("FAILURE");
