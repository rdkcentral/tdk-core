...
<xml>
  ...
  <name>Aamp_GetPlaybackPosition_HLS_URL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to get the current playback position for HLS stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_56</test_case_id>
...
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_GetPlaybackPosition_HLS_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_GetPlaybackPosition_HLS_URL');
sysObj.configureTestCase(ip,port,'Aamp_GetPlaybackPosition_HLS_URL');
			...
			print "Result :", details;
				...
				if expectedResult in actualResult:
					...
					if diff >= 5.0:
						#Set the result status of execution
						tdkTestObj.setResultStatus("SUCCESS");
						print "Playback position retrieved is valid for HLS stream";
					else:
						#Set the result status of execution
						tdkTestObj.setResultStatus("FAILURE");
						print "Playback position retrieved is not valid for HLS stream";
				else:
					#Set the result status of execution
	                		tdkTestObj.setResultStatus("FAILURE");
					print "Playback position not retrieved";
        		        ...
        		        #Set the result status of execution
                		tdkTestObj.setResultStatus("FAILURE");
				print "Playback position not retrieved for HLS stream";
		else:
