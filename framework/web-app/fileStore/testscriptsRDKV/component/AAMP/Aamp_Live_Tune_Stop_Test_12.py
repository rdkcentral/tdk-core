...
<xml>
  ...
  <name>Aamp_Live_Tune_Stop_Test_12</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Live_Tune_Stop</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_12</test_case_id>
    <test_objective>Live_Tune_Stop_Test</test_objective>
...
7.Set the script status as per the Log comparison</automation_approch>
    <except_output>Aamp tune should be stopped</except_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Live_Tune_Stop_Test_12</test_script>
    <skipped>No</skipped>
...
aampobj.configureTestCase(ip,port,'Aamp_Live_Tune_Stop_Test_12');
sysobj.configureTestCase(ip,port,'Aamp_Live_Tune_Stop_Test_12');
...
if ("SUCCESS" in aamp_status.upper()) and ("SUCCESS" in sysutil_status.upper()):
        ...
        if Expected_Result in result:
                ...
                if Expected_Result in result:
                        ...
                        result = tdkTestObj.getResult();
                        if Expected_Result in result:
				print "Verified AampStop for Live stream"
				print "[TEST EXECUTION RESULT] : %s" %result;
				#Set the result status of execution
				tdkTestObj.setResultStatus("SUCCESS")
			else:
