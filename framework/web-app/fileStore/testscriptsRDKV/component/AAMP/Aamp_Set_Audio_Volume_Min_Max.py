...
<xml>
  ...
  <name>Aamp_Set_Audio_Volume_Min_Max</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to set audio volume as zero and maximum value 100</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_36</test_case_id>
    <test_objective>Test script to set audio volume as zero and maximum value 100</test_objective>
...
3. Aamp Agent invokes SetAudioVolume  API with min volume level 0 and max level 100
4. Checks if GetAudioVolume API  returns the volume level set and set SUCCESS/FAILURE
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should set the  audio volume level as 0 and 100</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Set_Audio_Volume_Min_Max</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Min_Max');
sysObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Min_Max');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            volume_min = 0;
            tdkTestObj.addParameter("volume", volume_min);
            #Execute the test case in STB
            tdkTestObj.executeTestCase(expectedResult);
            #Get the result of execution
            actualResult = tdkTestObj.getResult();
            ...
            if expectedResult in actualResult :
                ...
                if expectedResult in actualResult and volume == volume_min:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("Result :", details);
                    print("Able to set Audio volume as 0(min) successfully")
                    print("[TEST EXECUTION RESULT] : SUCCESS\n")

                    tdkTestObj = aampObj.createTestStep('Aamp_AampSetAudioVolume');
                    expectedResult = "SUCCESS";
                    #As per the aamp src code, maximum audio volume level is 100
                    volume_max = 100;
                    tdkTestObj.addParameter("volume", volume_max);
                    #Execute the test case in STB
                    tdkTestObj.executeTestCase(expectedResult);
                    #Get the result of execution
                    actualResult = tdkTestObj.getResult();
                    ...
                    if expectedResult in actualResult :
                        ...
                        details = tdkTestObj.getResultDetails();
                        volume = int(str(details).split(":")[1].strip())
                        if expectedResult in actualResult and volume == volume_max:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print("Result :", details);
                            print("Able to set Audio volume as 100(max) successfully")
                            print("[TEST EXECUTION RESULT] : SUCCESS\n")
                        else:
                            #Set the result status of execution
