...
<xml>
  ...
  <name>Aamp_GetCurrentDRM_MPD_URL</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>Aamp_AampGetCurrentDRM</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to get the current DRM value for MPD stream</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_51</test_case_id>
    <test_objective>To get the current DRM</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1V3,XI3</test_setup>
    <pre_requisite>configure Aamp_Tune_Config.ini with proper tuning URLs protected with DRM</pre_requisite>
    <api_or_interface_used>AampTune
AampStop
AampGetCurrentDRM</api_or_interface_used>
...
3. Aamp Agent invokes GetCurrentDRM API
...
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should get the valid DRM value</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_GetCurrentDRM_MPD_URL</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_GetCurrentDRM_HLS_URL');
sysObj.configureTestCase(ip,port,'Aamp_GetCurrentDRM_HLS_URL');
...
drmNames = ["Widevine", "CONSEC_agnostic", "PlayReady", "Adobe_Access", "Vanilla_AES"];
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
	...
	if expectedResult in actualResult:
		...
		if expectedResult in actualResult:
			...
			tdkTestObj = aampObj.createTestStep('Aamp_AampGetCurrentDRM');
			expectedResult = "SUCCESS";
		        #Execute the test case in STB
		        tdkTestObj.executeTestCase(expectedResult);
		        #Get the result of execution
			...
			print "Result :", details;
	        	...
	        	if expectedResult in actualResult and details in drmNames:
        		        #Set the result status of execution
                		tdkTestObj.setResultStatus("SUCCESS");
				print "DRM retrieved is valid for MPD stream";
			else:
        		        #Set the result status of execution
                		tdkTestObj.setResultStatus("FAILURE");
				print "DRM not retrievd correctly for MPD stream";
		else:
