...
<xml>
  ...
  <name>Aamp_Set_Audio_Volume_Invalid</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>Aamp_AampSetAudioVolume</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to set the current audio volume with invalid level</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_38</test_case_id>
    <test_objective>Test script to set the current audio volume with invalid level</test_objective>
...
AampStop
AampGetAudioVolume
AampSetAudioVolume</api_or_interface_used>
...
3. Aamp Agent invokes SetAudioVolume  API with invalid volume level -10
4. Checks if GetAudioVolume API  not returns the volume level set and set SUCCESS/FAILURE
5. TM unloads the Aamp Agent and systemutil Agent.</automation_approch>
    <expected_output>Should not set the  audio volume level as -10</expected_output>
    <priority>High</priority>
    <test_stub_interface>libaampstub.so.0.0.0
libsystemutilstub.so.0.0.0</test_stub_interface>
    <test_script>Aamp_Set_Audio_Volume_Invalid</test_script>
    <skipped>No</skipped>
...
aampObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Invalid');
sysObj.configureTestCase(ip,port,'Aamp_Set_Audio_Volume_Invalid');
...
if ("SUCCESS" in aampLoadStatus.upper()) and ("SUCCESS" in sysLoadStatus.upper()):
        ...
        if expectedResult in actualResult:
                ...
                if expectedResult in actualResult:
                        ...
                        tdkTestObj = aampObj.createTestStep('Aamp_AampSetAudioVolume');
                        expectedResult = "SUCCESS";
                        #As per aamp src code, 0-100 is the audio volume level range
                        #choosing -10 as a random value which does not lie within the valid range
                        volume_set = -10;
                        tdkTestObj.addParameter("volume", volume_set);
                        print "Setting Audio volume as %s" %(volume_set)
                        #Execute the test case in STB
                        tdkTestObj.executeTestCase(expectedResult);
                        #Get the result of execution
                        actualResult = tdkTestObj.getResult();
                        ...
                        if expectedResult in actualResult :
                            ...
                            print actualResult;
                            details = tdkTestObj.getResultDetails();
                            volume = int(str(details).split(":")[1].strip())
                            if expectedResult in actualResult and volume != volume_set:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Result :", details;
                                print "Not able to set invalid Audio volume as %s successfully" %(volume_set)
                                print "[TEST EXECUTION RESULT] : SUCCESS\n"
                                ...
                                print details;
                                print "Able to set invalid Audio volume as %s successfully" %(volume_set)
