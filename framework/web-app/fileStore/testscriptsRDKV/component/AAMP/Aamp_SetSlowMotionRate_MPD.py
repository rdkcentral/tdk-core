...
<xml>
  ...
  <name>Aamp_SetSlowMotionRate_MPD</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>SetSlowMotion Playback Rate on MPD stream and verify the same</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Aamp_77</test_case_id>
    <test_objective>SetSlowMotion Playback Rate on MPD stream and verify the same</test_objective>
    <test_type>Positive</test_type>
    <test_setup>X1, Video Accelerator, RPI</test_setup>
    <pre_requisite>MPD stream must be populated in Aamp_Tune_config.ini</pre_requisite>
    <api_or_interface_used>SetRate</api_or_interface_used>
    <input_parameters>MPD stream</input_parameters>
    <automation_approch>1.Tune AAMP with MPD url.
...
6. Invoke AAMP steop to gracefully exit from the player.</automation_approch>
    ...
    <test_script>Aamp_SetSlowMotionRate_MPD</test_script>
    <skipped></skipped>
...
aampobj.configureTestCase(ip,port,'Aamp_SetSlowMotionRate_MPD');
sysobj.configureTestCase(ip,port,'Aamp_SetSlowMotionRate_MPD');
