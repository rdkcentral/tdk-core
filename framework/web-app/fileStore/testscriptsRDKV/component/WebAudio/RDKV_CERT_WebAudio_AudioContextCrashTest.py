...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_CERT_WebAudio_AudioContextCrashTest</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the device browser crashes by creating 50 AudioContext objects</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_02</test_case_id>
    <test_objective>To check if the device browser crashes by creating 50 AudioContext objects</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>AudioContextCrashTest.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should not crash on creating multiple audio context objects.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_CERT_WebAudio_AudioContextCrashTest</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_CERT_WebAudio_AudioContextCrashTest');
...
webaudio_test_url = obj.url+"/fileStore/lightning-apps/webaudio/AudioContextCrashTest.html"
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            if log_check_method == "WebinspectPageLogs":
                ...
                if webinspect_logs !='':
                    webinspect_logs = webinspect_logs[-1]
            else:
                print("\n Script is using wpeframework log to validate the webaudio test")
                grep_line="AudioContextCrashTest | tail -1"
                webinspect_logs = WebAudiolib.webaudio_getLogs_fromDevicelogs(obj,webaudio_test_url,browser,grep_line)

            if webinspect_logs != "":
                if "TDK_LOGS" in webinspect_logs and "AudioContextCrashTest" in webinspect_logs:
                    print (webinspect_logs)
                    print("\nSUCCESS: Successfully fetched the logs from the Html test App")
                    tdkTestObj.setResultStatus("SUCCESS")
                    if "Success" in webinspect_logs and "50" in webinspect_logs:
                        print("\n SUCCESS: The browser didn't crash on creating 50 audiocontext objects\n")
                        tdkTestObj.setResultStatus("SUCCESS")

                    elif "Failure" in webinspect_logs:
                        print ("\n FAILURE: The browser crashed on creating 50 audiocontext objects\n")
                        tdkTestObj.setResultStatus("FAILURE")
                    else:
                        print("\n FAILURE: Failed to create 50 audiocontext objects using html test app\n")
                        tdkTestObj.setResultStatus("FAILURE")
