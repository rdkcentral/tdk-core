...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_ValueSetterWarnings</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the value setter warnings details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_140</test_case_id>
    <test_objective>To get the value setter warnings details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>value-setter-warnings.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the value setter warnings details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_ValueSetterWarnings</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_ValueSetterWarnings');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/value-setter-warnings.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing value-setter-warnings.html file")
            grep_line = "value-setter-warnings | tail -3 | tr -d '\\n'"
            log_filename = "value-setter-warnings"
...
if "value-setter-warnings" in status_dict:
    print(f"value-setter-warnings: {status_dict['value-setter-warnings']}")
