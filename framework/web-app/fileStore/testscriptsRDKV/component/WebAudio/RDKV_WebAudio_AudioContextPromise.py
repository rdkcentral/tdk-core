...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_AudioContextPromise</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the AudioContextPromise details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_22</test_case_id>
    <test_objective>To get the AudioContextPromise details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>audiocontext-promise.html,audiocontext-promise-throwing.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the AudioContextPromise details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_AudioContextPromise</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_AudioContextPromise');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/audiocontext-promise.html'
webaudio_test_url2 = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/audiocontext-promise-throwing.html'
...
def process_webinspect_logs(log_filename, webinspect_logs,status_dict):
    ...
    if "TDK_LOGS" in webinspect_logs and log_filename in webinspect_logs:
        ...
        if "TDK_LOGS : PASS" in webinspect_logs and "TDK_LOGS : FAIL" in webinspect_logs:
            ...
            print("Successfully got the AudioContextPromise details from the device browser.")
            print("\n", webinspect_logs, "\n")
            ...
            print("Failed to get the AudioContextPromise details from the device browser.")
            print("\n", webinspect_logs, "\n")
            print("Test step result: FAILURE")
            tdkTestObj.setResultStatus("FAILURE")
            status_dict[log_filename] = "FAILURE"
