...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>RDKV_WebAudio_AudiobuffersourceDetuneModulatedImpulse</name>
  <primitive_test_id/>
  <primitive_test_name>webaudio_prerequisite</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the audiobuffersource detune modulated impulse details from the device browser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>WebAudio_75</test_case_id>
    <test_objective>To get the audiobuffersource detune modulated impulse details from the device browser</test_objective>
...
All the variables in WebAudioVariables.py must be filled.</pre_requisite>
    ...
    <input_parameters>audiobuffersource-detune-modulated-impulse.html</input_parameters>
...
2. Check for the required logs in wpeframework log or in the webinspect page</automation_approch>
    <expected_output>The browser should be able to get the audiobuffersource detune modulated impulse details</expected_output>
    <priority>High</priority>
    <test_stub_interface>WebAudio</test_stub_interface>
    <test_script>RDKV_WebAudio_AudiobuffersourceDetuneModulatedImpulse</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_WebAudio_AudiobuffersourceDetuneModulatedImpulse');
...
webaudio_test_url = obj.url+WebAudioVariables.wpe_webkit_testcases_path+'/AudioBufferSource/audiobuffersource-detune-modulated-impulse.html'
...
if expectedResult in result.upper():
    ...
    if expectedResult in pre_req_status:
        ...
        if expectedResult in result:
            ...
            print("Processing audiobuffersource-detune-modulated-impulse.html file")
            grep_line = "audiobuffersource-detune-modulated-impulse | tail -3 | tr -d '\\n'"
            log_filename = "audiobuffersource-detune-modulated-impulse"
...
if "audiobuffersource-detune-modulated-impulse" in status_dict:
    print(f"audiobuffersource-detune-modulated-impulse: {status_dict['audiobuffersource-detune-modulated-impulse']}")
