...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>XUPNP_BcastSerialNumEvt</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_BroadcastEvent</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To broadcast Serial Number event and check if xcal-device process receives it.
Testcase ID: CT_XUPNP_38</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_38</test_case_id>
    <test_objective>To broadcast Serial Number event and check if xcal-device process receives it.</test_objective>
...
6. The stub will invokes the RPC method for checking the serial No event is registered and send the results.
...
9. IARMBUS_Agent deregisters from the IARM Bus Daemon.</automation_approch>
    ...
    <test_script>XUPNP_BcastSerialNumEvt</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_BcastSerialNumEvt');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_BcastSerialNumEvt');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in xupnpLoadStatus.upper():
                ...
                tdkTestObj.addParameter("stateId",33);
                tdkTestObj.addParameter("eventLog","Serial Number Update Received");
                tdkTestObj.addParameter("payload","TEST00000123");
                ...
                if "SUCCESS" in actualresult.upper():
                    ...
                    print("Broadcasting SerialNum Event Result : %s"%actualresult);
                    print("Details : %s"%details);
                else:
                    tdkTestObj.addParameter("stateId",33);
                    tdkTestObj.addParameter("eventLog","Serial no");
                    tdkTestObj.addParameter("payload","TEST00000123");
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
