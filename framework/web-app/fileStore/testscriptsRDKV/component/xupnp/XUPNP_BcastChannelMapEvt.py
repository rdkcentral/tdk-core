...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>XUPNP_BcastChannelMapEvt</name>
  <primitive_test_id/>
...
Testcase ID: CT_XUPNP_35</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_35</test_case_id>
    <test_objective>To broadcast channel map event and check if xcal-device process receives it.</test_objective>
...
6. The stub will invokes the RPC method for checking the channel map event is registered and send the results.
...
9. IARMBUS_Agent deregisters from the IARM Bus Daemon.</automation_approch>
    ...
    <test_script>XUPNP_BcastChannelMapEvt</test_script>
    <skipped>No</skipped>
...
iarmObj.configureTestCase(ip,port,'XUPNP_BcastChannelMapEvt');
...
if "SUCCESS" in iarmLoadStatus.upper():
    ...
    if "SUCCESS" in result:
        ...
        if "SUCCESS" in result:
            ...
            xUpnpObj.configureTestCase(ip,port,'XUPNP_BcastChannelMapEvt');
            #Get the result of connection with test component and STB
            ...
            if "SUCCESS" in xupnpLoadStatus.upper():
                ...
                tdkTestObj.addParameter("stateId",0);
                tdkTestObj.addParameter("eventLog","Received channel map update");
                tdkTestObj.addParameter("payload","1234");
                ...
                if "SUCCESS" in actualresult.upper():
                    ...
                    print("Broadcasting ChannelMap Event Result : %s"%actualresult);
                    print("Details : %s"%details);
                else:
                    tdkTestObj.addParameter("stateId",0);
                    tdkTestObj.addParameter("eventLog","Channel map id available");
                    tdkTestObj.addParameter("payload","1234");
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
