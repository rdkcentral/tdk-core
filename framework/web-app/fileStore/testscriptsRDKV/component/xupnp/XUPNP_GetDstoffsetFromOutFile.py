...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>XUPNP_GetDstoffsetFromOutFile</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNP_ReadXDiscOutputFile</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the dst offset value from xdiscovery output file.
Testcase ID: CT_XUPNP_53</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_XUPNP_53</test_case_id>
    <test_objective>To get the dst offset value from xdiscovery output file.</test_objective>
...
2.Process xcal-device and xdiscovery should be running on GW Box and xdiscovery should be running on IPClient Box</pre_requisite>
    ...
    <input_parameters>string paramName = dstoffset</input_parameters>
...
4. TM will receive and display the result.</automation_approch>
    ...
    <test_script>XUPNP_GetDstoffsetFromOutFile</test_script>
    <skipped>No</skipped>
...
xUpnpObj.configureTestCase(ip,port,'XUPNP_GetDstoffsetFromOutFile');
...
if "SUCCESS" in xupnpLoadStatus.upper():
    ...
    tdkTestObj.addParameter("paramName","dstoffset");
    ...
    details = details.replace('\\','')
    details = details.replace('\"','')
    print("GetDstoffset Result : %s"%actualresult);
    print("GetDstoffset Details : %s"%details);
