...
<xml>
  ...
  <name>RDKV_Memcr_YouTubeTV_Validate_Logs_During_Hibernate_Restore_Operations</name>
  ...
  <test_cases>
    <test_case_id>rdkvmemcr_19</test_case_id>
    <test_objective>To validate that the logs generated during the hibernate and restore operations</test_objective>
...
2. Check the status of memcr service
...
5. Check if the youtubeTV app enters a hibernated state by using the suspend API
6. Check that the logs generated during the hibernate and restore operations</automation_approch>
...
6. Suspend and restore API calls should generate appropriate hibernate and restore operation logs</expected_output>
    <priority>Medium</priority>
    <test_stub_interface>Nil</test_stub_interface>
    <test_script>RDKV_Memcr_YouTubeTV_Validate_Logs_During_Hibernate_Restore_Operations</test_script>
    <skipped>Nil</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_Memcr_YouTubeTV_Validate_Logs_During_Hibernate_Restore_Operations');
                    ...
                    if successstatus == 1:
                            ...
                            if str(success).lower() == "true":
                                ...
                                if str(success).lower() == "true" and "hibernated" in appstate:
                                    ...
                                    log_Occurrences = "Hibernate process"
                                    tdkTestObj = obj.createTestStep('memcr_logValidation')
                                    tdkTestObj.addParameter("log_Occurrences",log_Occurrences)
                                    tdkTestObj.executeTestCase(expectedResult)
                                    result = tdkTestObj.getResultDetails()
                                    ...
                                    if str(success).lower() == "true":
                                        ...
                                        for app in apps:
                                            if app["callsign"] == "YouTubeTV":
                                                appstate = app["state"]
                                        success = result.get("success")
                                        if str(success).lower() == "true" and "suspended" in appstate:
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print("SUCCESS : "+method+" API call was successful\n")

                                            log_Occurrences = "Wakeup process"
                                            tdkTestObj = obj.createTestStep('memcr_logValidation')
                                            tdkTestObj.addParameter("log_Occurrences",log_Occurrences)
                                            tdkTestObj.executeTestCase(expectedResult)
                                            result = tdkTestObj.getResultDetails()
