...
<xml>
  ...
  <test_cases>
    ...
    <automation_approch>1. Retrieve the AppHibernate RFC parameter from the device configuration 2. Check the status of the Memcr service 3. Check the status of the YouTube TV app: if it is hibernated, restore it; if it is suspended, launch it; if it is already running, take no action 4. If it is not running at all, proceed to launch the app 5. Verify that the compressed app file (.img format) is created during suspension 6. After restoring, ensure that the compressed app file is no longer present</automation_approch>
