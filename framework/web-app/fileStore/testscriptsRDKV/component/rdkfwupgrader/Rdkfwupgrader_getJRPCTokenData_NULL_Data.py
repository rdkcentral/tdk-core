...
<xml>
  ...
  <name>Rdkfwupgrader_getJRPCTokenData_NULL_Data</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To try to obtain token from testString via rdkfwupgrader API getJRPCTokenData with NULL Data and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_28</test_case_id>
    <test_objective>To try to obtain token from testString via rdkfwupgrader API getJRPCTokenData with NULL Data and verify output</test_objective>
    ...
    <expected_output>API must FAIL due to NULL Data being passed</expected_output>
    <priority>High</priority>
    <test_stub_interface>librdkfwupgraderstub.so.0.0.0</test_stub_interface>
    <test_script>Rdkfwupgrader_getJRPCTokenData_NULL_Data</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_getJRPCTokenData_NULL_Data');
...
if "SUCCESS" in result.upper():
    ...
    if result == "FAILURE":
        ...
        print ("Expected test Result : getJRPCTokenData must fail due to NULL Data")
