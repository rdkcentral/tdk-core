...
<xml>
  ...
  <name>Rdkfwupgrader_GetServerUrlFile_NULL_URL_param</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To pass NULL URL param to rdkfwupgrader API GetServerUrlFile and verify output</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_34</test_case_id>
    <test_objective>To pass NULL URL param to rdkfwupgrader API GetServerUrlFile and verify output</test_objective>
    ...
    3. RDK_fwupgradeAgent will invoke GetServerUrlFile API with the NULL param being passed to pServUrl param.
    ...
    <test_script>Rdkfwupgrader_GetServerUrlFile_NULL_URL_param</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_GetServerUrlFile_NULL_URL_param');
sysUtilObj.configureTestCase(ip,port,'Rdkfwupgrader_GetServerUrlFile_NULL_URL_param');
...
if "SUCCESS" in result.upper() and "SUCCESS" in sysUtilLoadStatus.upper():
    if createTestFile():
        ...
        if result == "FAILURE" and "crash" not in details:
            print("SUCCESS : GetServerURL failed as expected for null param")
            tdkTestObj.setResultStatus("SUCCESS")
        elif "crash" in details:
            print("FAILURE : GetServerURL crashes when called with null param")
            tdkTestObj.setResultStatus("FAILURE")
            obj.initiateReboot();
        else:
            print("FAILURE : GetServerURL is success for null param which is not expected")
            tdkTestObj.setResultStatus("FAILURE")
