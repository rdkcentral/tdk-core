...
<xml>
  ...
  <name>Rdkfwupgrader_isThrottleEnabled_RebootFlag_NULL_check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the behavior when a NULL parameter is passed in the isThrottleEnabled function.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKFWUPGRADE_136</test_case_id>
    <test_objective>To verify the behavior when a NULL parameter is passed in the isThrottleEnabled function.</test_objective>
    ...
    2. RDK_fwupgradeAgent will invoke isThrottleEnabled API with the NULL param being passed to reboot_immediate_flag param.
    ...
    <test_script>Rdkfwupgrader_isThrottleEnabled_RebootFlag_NULL_check</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Rdkfwupgrader_isThrottleEnabled_RebootFlag_NULL_check');
...
if "SUCCESS" in result.upper():
    ...
    tdkTestObj.addParameter("reboot_null_param", 1);
    ...
    if result == "FAILURE" and "crash" not in details:
        print("SUCCESS : isThrottleEnabled failed as expected for null param")
        tdkTestObj.setResultStatus("SUCCESS")
    elif "crash" in details:
        print("FAILURE : isThrottleEnabled crashes when called with null param")
        tdkTestObj.setResultStatus("FAILURE")
        obj.initiateReboot();
    else:
        print("FAILURE : isThrottleEnabled is success for null param which is not expected")
        tdkTestObj.setResultStatus("FAILURE")
