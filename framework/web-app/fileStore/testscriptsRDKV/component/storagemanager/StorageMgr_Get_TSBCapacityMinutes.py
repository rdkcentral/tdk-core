...
<xml>
  ...
  <name>StorageMgr_Get_TSBCapacityMinutes</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to get the device capability for TSB storage in minutes</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.Invoke getTSBCapacityMinutes API
3.It should return devices capability for TSB storage in minutes</automation_approch>
    <expected_output>Should return devices capability for TSB storage in minutes</expected_output>
    <priority>High</priority>
    <test_stub_interface>libstoragemanagerstub.so.0.0.0</test_stub_interface>
    <test_script>StorageMgr_Get_TSBCapacityMinutes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'StorageMgr_Get_TSBCapacityMinutes');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    if expectedResult in actualResult and int(details) != 0:
        ...
        print("TSB Capacity Minutes: ",details)
        print("[TEST EXECUTION RESULT] : SUCCESS\n")
        print("ACTUAL RESULT: StorageMgr_GetTSBCapacityMinutes call was success")
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print("TSB Capacity Minutes: ",details)
        print("ACTUAL RESULT: StorageMgr_GetTSBCapacityMinutes call failed")
