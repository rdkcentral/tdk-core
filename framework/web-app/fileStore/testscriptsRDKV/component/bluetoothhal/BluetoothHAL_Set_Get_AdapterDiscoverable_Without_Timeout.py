...
<xml>
  ...
  <name>BluetoothHAL_Set_Get_AdapterDiscoverable_Without_Timeout</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set the bluetooth adapter to be discoverable with timeout value 0 (no timeout) and get the discoverable status to check if it is set correctly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_09</test_case_id>
    <test_objective>To set the bluetooth adapter to be discoverable with timeout value 0 (no timeout) and get the discoverable status to check if it is set correctly</test_objective>
...
5. Get the discoverable status for the adapter immediately and check if its 1
6. Get the discoverable status for the adapter after 60 seconds and check if its 1
...
Checkpoint 2. Verify the discoverable status for the bluetooth adapter should be 1 all the time.</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Set_Get_AdapterDiscoverable_Without_Timeout</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Set_Get_AdapterDiscoverable_Without_Timeout');
