...
<xml>
  ...
  <name>BluetoothHAL_Get_Supported_Services</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>BluetoothHal_GetSupportedServices</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the list of services supported in a remote bluetooth device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <remarks>Api is not used actively  in bluez5 and was not developed. DELIA-49396</remarks>
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_43</test_case_id>
    <test_objective>To get the list of services supported in a remote bluetooth device</test_objective>
...
9. enBTRCoreRet BTRCore_GetSupportedServices (tBTRCoreHandle hBTRCore, tBTRCoreDevId aBTRCoreDevId, stBTRCoreSupportedServiceList *pProfileList);
10. enBTRCoreRet BTRCore_UnPairDevice (tBTRCoreHandle hBTRCore, tBTRCoreDevId aBTRCoreDevId);</api_or_interface_used>
    <input_parameters>1. adapter_path - retrieved using BTRCore_GetAdapter()
2. power_status - 1 (ON)
3. device_type - 0, for enBTRCoreSpeakers
...
10. Retrieve the list of services supported in the client device using the api BTRCore_GetSupportedServices
11. Unpair the bluetooth client device from the DUT using BTRCore_UnPairDevice api.
12. Retrieve the list of paired devices to ensure that client device is unpaired correctly.
13. Based on the API call return code, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1. Verify the API call is success
Checkpoint 2. Verify that the bluetooth client device  is present in the scanned/paired devices list after scanning/pairing</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Get_Supported_Services</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Get_Supported_Services');
                                    ...
                                    if True == deviceDiscovered:
                                        ...
                                        if (actualresult == expectedresult):
                                            ...
                                            if True == devicePaired:
                                                ...
                                                #Find the list of supported services using api
                                                tdkTestObj = bluetoothhalObj.createTestStep('BluetoothHal_GetSupportedServices');
                                                #Set device ID as the bluetooth client device ID
                                                tdkTestObj.addParameter("device_id", deviceID)
                                                ...
                                                if (actualresult == expectedresult):
                                                    print(("BluetoothHal_GetSupportedServices executed successfully, \n %s is found in the paired devices list" %(bluetoothhallib.deviceName)))
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    #Check for the retrieved services
                                                    scanResult = tdkTestObj.getResultDetails()
                                                    if scanResult and "NO_SERVICE_AVAILABLE" != scanResult:
                                                        serviceList = json.loads(scanResult)
                                                        print(("Supported Services List :\n\t", serviceList))
                                                        tdkTestObj.setResultStatus("SUCCESS")
                                                    else:
                                                        print("Supported Service List retrieved is empty")
                                                        tdkTestObj.setResultStatus("FAILURE")
                                                else:
                                                    print(("BluetoothHal_GetSupportedServices: failed.\n %s not found in paired devices list" %(bluetoothhallib.deviceName)))
                                                    tdkTestObj.setResultStatus("FAILURE")
