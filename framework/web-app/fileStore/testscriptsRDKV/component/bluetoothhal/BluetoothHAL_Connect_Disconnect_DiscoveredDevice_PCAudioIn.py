...
<xml>
  ...
  <name>BluetoothHAL_Connect_Disconnect_DiscoveredDevice_PCAudioIn</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To connect and disconnect with a discovered device when the device type is PCAudioIn</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_32</test_case_id>
    <test_objective>To connect and disconnect with a discovered device when the device type is PCAudioIn</test_objective>
...
Checkpoint 3. Verify that the bluetooth client device  is disconnected successfully from DUT using BTRCore_GetDeviceDisconnected api.</expected_output>
    ...
    <test_script>BluetoothHAL_Connect_Disconnect_DiscoveredDevice_PCAudioIn</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Connect_Disconnect_DiscoveredDevice_PCAudioIn');
