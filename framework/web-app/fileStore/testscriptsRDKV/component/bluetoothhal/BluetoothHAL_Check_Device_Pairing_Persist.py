...
<xml>
  ...
  <name>BluetoothHAL_Check_Device_Pairing_Persist</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether Bluetooth pairing persists even after DUT reboots provided discoverable status is on in client device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_28</test_case_id>
    <test_objective>To check whether Bluetooth pairing persists even after DUT reboots provided discoverable status is on in client device</test_objective>
...
10. Reboot the DUT.
11. After 60 seconds, set the bluetooth client device as discoverable again.
12. Get the list of paired devices using BTRCore_GetListOfPairedDevices api to ensure that the bluetooth client device is paired automatically after reboot with the DUT.
...
Checkpoint 2. Verify that the bluetooth client device  is present in the paired devices list after BTRCore_PairDevice api is executed successfully.
Checkpoint 3. Verify that the bluetooth client device  is present in the paired devices list after DUT reboots successfully
Checkpoint 4. Verify that the bluetooth client device  is not present  in the paired devices list after BTRCore_UnPairDevice api is executed successfully</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Check_Device_Pairing_Persist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Check_Device_Pairing_Persist');
                                        ...
                                        if (actualresult == expectedresult):
                                            ...
                                            if True == devicePaired:
                                                ...
                                                if "FAILURE" not in str(bluetoothctlResult):
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print(("Client Device %s set as discoverable" %(bluetoothhallib.deviceName)))
                                                    #Retrieve the list of paired devices to check if client device is auto paired after DUT reboot
                                                    devicePaired = checkDeviceInPairedList(bluetoothhalObj,deviceID)
                                                    if True == devicePaired:
                                                        print("Client device is successfully paired with DUT after reboot")
                                                        tdkTestObj.setResultStatus("SUCCESS")
                                                        ...
                                                        print("Client device NOT paired with DUT after reboot")
                                                        tdkTestObj.setResultStatus("FAILURE")
                                                else:
