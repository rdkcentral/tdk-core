...
<xml>
  ...
  <name>BluetoothHAL_Get_DeviceTypeClass</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>BluetoothHal_GetDeviceTypeClass</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the device type class for the bluetooth client device</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_44</test_case_id>
    <test_objective>To get the device type class for the bluetooth client device</test_objective>
...
7. enBTRCoreRet BTRCore_GetDeviceTypeClass (tBTRCoreHandle hBTRCore, tBTRCoreDevId aBTRCoreDevId, enBTRCoreDeviceType* apenBTRCoreDevTy, enBTRCoreDeviceClass* apenBTRCoreDevCl);</api_or_interface_used>
...
8. Retrieve the device type class for the discovered device using BTRCore_GetDeviceTypeClass api</automation_approch>
    <expected_output>Checkpoint 1. Verify the API call is success
Checkpoint 2. Verify that the device type class values returned from the API are not empty </expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Get_DeviceTypeClass</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Get_DeviceTypeClass');
	                        ...
	                        print "BluetoothHal_GetListOfScannedDevices executed successfully"
                                ...
                                if scanResult and "NO_DEVICES_FOUND" != scanResult and "deviceName" in scanResult:
                                    ...
                                    if True == deviceDiscovered:
                                        ...
                                        tdkTestObj = bluetoothhalObj.createTestStep('BluetoothHal_GetDeviceTypeClass');
                                        ...
                                        if (actualresult == expectedresult):
                                            print "BluetoothHal_GetDeviceTypeClass executed successfully"
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            deviceClass = int(tdkTestObj.getResultDetails())
                                            if deviceClass:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print "Device Class: ", hex(deviceClass)
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print "Device Type Class not retrieved correctly"
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print "BluetoothHal_GetDeviceTypeClass: failed"
                                    else:
