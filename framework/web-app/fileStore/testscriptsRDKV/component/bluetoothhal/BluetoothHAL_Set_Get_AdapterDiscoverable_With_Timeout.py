...
<xml>
  ...
  <name>BluetoothHAL_Set_Get_AdapterDiscoverable_With_Timeout</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set the bluetooth adapter to be discoverable with timeout value 60 seconds and get the discoverable status to check the if its set correctly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_10</test_case_id>
    <test_objective>To set the bluetooth adapter to be discoverable with timeout value 60 seconds and get the discoverable status to check the if its set correctly</test_objective>
...
3. timeout - 60 seconds</input_parameters>
...
3. Set the timeout for discoverable status as 60 seconds
4. Set the discoverable status for the adapter as 1(discoverable)
5. Get the discoverable status for the adapter immediately and check if its true
6. Get the discoverable status for the adapter after 80 seconds and check if its false
...
Checkpoint 2. Verify the discoverable status for the bluetooth adapter should be 1 immediately after setting the status and it should be 0 after 60 seconds.</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Set_Get_AdapterDiscoverable_With_Timeout</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Set_Get_AdapterDiscoverable_With_Timeout');
...
if "SUCCESS" in result.upper():
    ...
    if (actualresult == expectedresult):
        ...
        if (adapterPath):
            ...
            #Set the discoverable timeout as 60 seconds
            tdkTestObj.addParameter("timeout", 60)
                ...
                if (actualresult == expectedresult):
                    ...
                    if (actualresult == expectedresult):
                        ...
                        if (1 == discoverableStatus):
                            ...
                            #Wait for 80 seconds and then check discoverable status again
                            time.sleep(80)
                            ...
                            if (actualresult == expectedresult):
                                ...
                                if (0 == discoverableStatus):
                                    ...
                                    print(("Adapter %s is discoverable even after 60 seconds" %(adapterPath)))
