...
<xml>
  ...
  <name>BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_LE</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether discoverable devices are not listed in the discovered devices list when the device type is LE</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_24</test_case_id>
    <test_objective>To check whether discoverable devices are not listed in the discovered devices list when the device type is LE</test_objective>
...
2. power_status - 1 (ON)
3. device_type - 4, for enBTRCoreLE
...
8. Ensure that the bluetooth client device is not present in the scanned devices list since its not an LE device.
9. Based on the API call return code, TM return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1. Verify the API call is success
Checkpoint 2. Verify that the bluetooth client device  is not present in the discovered devices list since its not an LE device</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_LE</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Get_ListOf_ScannedDevices_For_DeviceType_LE');
        ...
        if (adapterPath):
            ...
            if (actualresult == expectedresult):
                ...
                if "FAILURE" not in str(bluetoothctlResult):
                    ...
                    #Set the device type as 4 - LE
                    tdkTestObj.addParameter("device_type", 4)
