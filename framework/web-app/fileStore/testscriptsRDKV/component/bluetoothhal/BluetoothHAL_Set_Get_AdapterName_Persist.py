...
<xml>
  ...
  <name>BluetoothHAL_Set_Get_AdapterName_Persist</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether bluetooth adapter name persist after reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_BLUETOOTH_HAL_14</test_case_id>
    <test_objective>To check whether bluetooth adapter name persist after reboot</test_objective>
...
9. BluetoothHal agent will invoke the api BTRCore_SetAdapterName to set the  bluetooth adapter name to the previous value and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1. Verify all API calls are success
Checkpoint 2. Verify that the adapter name is updated correctly.
Checkpoint 3. Verify that the adapter name persist after reboot.</expected_output>
    <priority>High</priority>
    <test_stub_interface>libbluetoothhalstub.so.0</test_stub_interface>
    <test_script>BluetoothHAL_Set_Get_AdapterName_Persist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
bluetoothhalObj.configureTestCase(ip,port,'BluetoothHAL_Set_Get_AdapterName_Persist');
