...
<xml>
  ...
  <name>RDKV_AppArmor_Check_Profiles</name>
...
lighttpd</synopsis>
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_05</test_case_id>
    <test_objective> To Check the profiles are present for following processes
irMgrMain
audioCaptureManager
lighttpd</test_objective>
...
3. Pass if files are present.</automation_approch>
    ...
    <test_script>RDKV_AppArmor_Check_Profiles</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Check_Profiles');
...
if expectedResult in result.upper():
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        command = "sh " + configValues["FilePath"] + "/tdk_apparmor_tests.sh aptest05"
        print("COMMAND : %s" %(command))
        #Primitive test case which associated to this Script
        tdkTestObj = obj.createTestStep('rdkvapparmor_executeInDUT');
        #Add the parameters to ssh to the DUT and execute the command
