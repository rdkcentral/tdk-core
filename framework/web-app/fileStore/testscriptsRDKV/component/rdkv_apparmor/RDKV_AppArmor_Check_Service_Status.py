...
<xml>
  ...
  <name>RDKV_AppArmor_Check_Service_Status</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test apparmor service status using systemctl command</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_04</test_case_id>
    <test_objective>To test if apparmor service status is active</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Check_Service_Status');
...
if expectedResult in result.upper():
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        command = "sh " + configValues["FilePath"] + "/tdk_apparmor_tests.sh aptest01"
        print "COMMAND : %s" %(command)
        #Primitive test case which associated to this Script
        tdkTestObj = obj.createTestStep('rdkvapparmor_executeInDUT');
        #Add the parameters to ssh to the DUT and execute the command
