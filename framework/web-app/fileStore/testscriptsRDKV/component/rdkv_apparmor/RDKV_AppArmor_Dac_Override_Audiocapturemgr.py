...
<xml>
  ...
  <name>RDKV_AppArmor_Dac_Override_Audiocapturemgr</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if Dac_Override is present in audiocapturemgr profile</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_18</test_case_id>
    <test_objective> To Check Dac_Override in audiocapturemgr is present</test_objective>
...
2. Validate Dac_Override is present in audiocapturemgr
3. Pass if dac_override is present</automation_approch>
    <expected_output>Dac_Override is present in audiocapturemgr</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_apparmor</test_stub_interface>
    <test_script>RDKV_AppArmor_Dac_Override_Audiocapturemgr</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Dac_Override_Audiocapturemgr');
