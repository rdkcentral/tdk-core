...
<xml>
  ...
  <name>RDKV_AppArmor_Capability_Check</name>
...
lighttpd</synopsis>
  ...
  <test_cases>
    ...
    <test_objective> To check profile have Admin Capability</test_objective>
...
3. Pass if files are present.</automation_approch>
    ...
    <test_script>RDKV_AppArmor_Capability_Check</test_script>
    <skipped>No</skipped>
...
if expectedResult in result.upper():
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        command = "sh " + configValues["FilePath"] + "/tdk_apparmor_tests.sh aptest07"
        print "COMMAND : %s" %(command)
        #Primitive test case which associated to this Script
        tdkTestObj = obj.createTestStep('rdkvapparmor_executeInDUT');
        #Add the parameters to ssh to the DUT and execute the command
