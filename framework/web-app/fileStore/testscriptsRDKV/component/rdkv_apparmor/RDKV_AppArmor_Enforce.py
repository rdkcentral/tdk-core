...
<xml>
  ...
  <name>RDKV_AppArmor_Enforce</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test if apparmor is in enforce mode for the given profiles</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_01</test_case_id>
    <test_objective>To test if apparmor is in enforce mode for the given profiles</test_objective>
...
2. Enforce AppArmor to the given profiles using tr181 command
3. Validate if the AppArmor is enforced to the given profiles</automation_approch>
    <expected_output>AppArmor should be enforced in the given profiles</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_apparmor</test_stub_interface>
    <test_script>RDKV_AppArmor_Enforce</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Enforce');
