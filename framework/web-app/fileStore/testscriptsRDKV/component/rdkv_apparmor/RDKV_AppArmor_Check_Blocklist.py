...
<xml>
  ...
  <name>RDKV_AppArmor_Check_Blocklist</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check the Apparmor_blocklist is present</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_07</test_case_id>
    <test_objective>To check the Apparmor_blocklist is present</test_objective>
    <test_type>Positive</test_type>
    <test_setup>RPI-HYB,RPI-Client</test_setup>
    <pre_requisite>1. Configure the location of shell scripts in FilePath available in fileStore/ApparmorConfig.config file</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>FilePath</input_parameters>
    <automation_approch>1. SSH to device and run the command 
2. Validate the file is present.</automation_approch>
    <expected_output>path: /etc/secure/Apparmor_blocklist</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_apparmor</test_stub_interface>
    <test_script>RDKV_AppArmor_Check_Blocklist</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Check_Blocklist');
...
if expectedResult in result.upper():
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        command = "sh " + configValues["FilePath"] + "/tdk_apparmor_tests.sh aptest03"
        print "COMMAND : %s" %(command)
        #Primitive test case which associated to this Script
        tdkTestObj = obj.createTestStep('rdkvapparmor_executeInDUT');
        #Add the parameters to ssh to the DUT and execute the command
