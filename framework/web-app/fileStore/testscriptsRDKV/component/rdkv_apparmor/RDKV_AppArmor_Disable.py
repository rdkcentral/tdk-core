...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test if apparmor is in disable mode for the given profiles</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>RDKV_APPARMOR_08</test_case_id>
    <test_objective>To test if apparmor is in disable mode for the given profiles</test_objective>
...
2. Disable AppArmor to the given profiles using tr181 command
3. Validate if the AppArmor is disable to the given profiles</automation_approch>
    <expected_output>AppArmor should be in disabled mode in the given profiles</expected_output>
    <priority>High</priority>
    <test_stub_interface>rdkv_apparmor</test_stub_interface>
    <test_script>RDKV_AppArmor_Disable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RDKV_AppArmor_Disable');
    ...
    if "Active: active" in output and expectedResult in result:
        ...
        if 'Y' in output and expectedResult in result:
            ...
            for i in range(count):
                command = 'tr181 -s -v "'+AppArmor_Profiles[i]+':disable" Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.NonRootSupport.ApparmorBlocklist'
                print("COMMAND : %s" %(command))
                #Primitive test case which associated to this Script
                tdkTestObj = obj.createTestStep('rdkvapparmor_executeInDUT');
                #Add the parameters to ssh to the DUT and execute the command
            ...
            if expectedResult in result:
                ...
                if expectedResult in result:
                    ...
                    output = tdkTestObj.getResultDetails()
                    for i in range(count):
                        if ''+AppArmor_Profiles[i]+':disable' in output:
                            disable = "Success"
                        else:
                            disable = "Failure"
                    if disable == "Success":
