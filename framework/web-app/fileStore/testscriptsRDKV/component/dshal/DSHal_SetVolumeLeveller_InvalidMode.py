...
<xml>
  ...
  <name>DSHal_SetVolumeLeveller_InvalidMode</name>
  ...
  <test_cases>
    ...
    <test_objective>To set the volume leveller in invalid mode and check whether level levels are not set.</test_objective>
...
4. TM checks if the volumeLeveller mode and level are returned as expected and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_SetVolumeLeveller_InvalidMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_SetVolumeLeveller_InvalidMode');
...
if "SUCCESS" in result.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        #Trying to set invalid mode with valid level
