...
<xml>
  ...
  <name>DSHal_IsHDCPEnabled_HDMI</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_IsHDCPEnabled</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the HDCP status of output HDMI</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To get the HDCP status of output HDMI</test_objective>
...
dsIsHDCPEnabled(int handle, bool *pContentProtected)</api_or_interface_used>
    <input_parameters>type - Video port type
index- Video port index
handle - Video port handle
pContentProtected -  HDCP status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsGetVideoPort to get the handle for HDMI port
3 . DSHAL agent will invoke the api dsIsHDCPEnabled to get the status  
4. TM checks if the status is valid and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the HDCP status is valid</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_IsHDCPEnabled_HDMI</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                tdkTestObj = dshalObj.createTestStep('DSHal_IsHDCPEnabled');
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
                print "DSHal_IsHDCPEnabled result: ", actualResult;
                if expectedResult in actualResult:
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print "Failed to get HDCP status";
            else:
                tdkTestObj.setResultStatus("FAILURE");
