...
<xml>
  ...
  <name>DSHal_IsDisplayConnected_InvalidHandle</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_IsDisplayConnected</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the video port display connected status is retrieved for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if the video port display connected status is retrieved for invalid handle</test_objective>
...
dsIsDisplayConnected(int handle, bool *connected)</api_or_interface_used>
...
3 . DSHAL agent will invoke the api dsIsDisplayConnected to get the display connection status
4. TM checks if the API call fails and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_IsDisplayConnected_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsDisplayConnected_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("Video port display connected status not set for invalid hanle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("Video port display connected status set for invalid handle which is not expected");
    else:
        tdkTestObj.setResultStatus("FAILURE");
