...
<xml>
  ...
  <name>DSHal_SetandGet_DialogEnhancement_levelInvalid</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if dialogenhancement level is set to invalid value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if dialogenhancement level is set to invalid value</test_objective>
...
2 . DSHAL agent will invoke the api dsSetDialogEnhancement to set the level to invalid value
...
4. TM checks if the level is same as that set and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_SetandGet_DialogEnhancement_levelInvalid</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetandGet_DialogEnhancement_levelInvalid');
