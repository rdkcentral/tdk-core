...
<xml>
  ...
  <name>DSHal_SetBackgroundColor_None_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set video device background color to None</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To set video device background color to None</test_objective>
...
Checkpoint 2 Verify that the BG color is set to None</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_SetBackgroundColor_None_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetBackgroundColor_None_HDMI');
sysObj.configureTestCase(ip,port,'DSHal_SetBackgroundColor_None_HDMI');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and "SUCCESS" in sysloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                print("Trying to set BGColor to None");
                tdkTestObj = dshalObj.createTestStep('DSHal_SetBackgroundColor');
                tdkTestObj.addParameter("color", colorId["None"]);
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
