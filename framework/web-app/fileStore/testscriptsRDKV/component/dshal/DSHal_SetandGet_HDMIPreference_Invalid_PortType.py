...
<xml>
  ...
  <name>DSHal_SetandGet_HDMIPreference_Invalid_PortType</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if HDMI preference is set for invalid PortType</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
Checkpoint 2 Verify that the HDCP protocol version is not set</expected_output>
    ...
    <test_script>DSHal_SetandGet_HDMIPreference_Invalid_PortType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetandGet_HDMIPreference_Invalid_PortType');
                ...
                if expectedResult in actualResult:
                    ...
                    if expectedResult in actualResult:
                        ...
                        if int(details) != hdcpProtocolVersion["VERSION_2X"]:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print("HdmiPreference not set for invalid PortType");
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("HdmiPreference set for invalid PortType");
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
