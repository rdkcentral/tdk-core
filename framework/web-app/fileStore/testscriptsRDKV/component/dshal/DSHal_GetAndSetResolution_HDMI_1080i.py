...
<xml>
  ...
  <name>DSHal_GetAndSetResolution_HDMI_1080i</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the resolution is set to 1080i</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_109</test_case_id>
    <test_objective>To check if the resolution is set to 1080i</test_objective>
...
3 . DSHAL agent will invoke the api dsSetResolution to set the resolution to 1080i
...
Checkpoint 2 Verify that the resolution is set</expected_output>
    ...
    <test_script>DSHal_GetAndSetResolution_HDMI_1080i</test_script>
    <skipped>No</skipped>
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                ...
                tdkTestObj.addParameter("interlaced",1); #INTERLACED
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
