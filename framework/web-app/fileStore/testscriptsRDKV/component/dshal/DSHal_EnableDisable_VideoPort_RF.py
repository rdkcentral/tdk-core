...
<xml>
  ...
  <name>DSHal_EnableDisable_VideoPort_RF</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To enable and disable RF video port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_22</test_case_id>
    <test_objective>To enable and disable RF video port</test_objective>
...
2 . DSHAL agent will invoke the api dsEnableVideoPort to enable/disable the RF video port status
...
Checkpoint 2 Verify that the status is set</expected_output>
    ...
    <test_script>DSHal_EnableDisable_VideoPort_RF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_EnableDisable_VideoPort_RF');

#Get the result of connection with test component and STB
dshalloadModuleStatus = dshalObj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %dshalloadModuleStatus);

#Check whether RF port is supported by DUT
