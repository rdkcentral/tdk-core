...
<xml>
  ...
  <name>DSHal_SetAndGetAudioMuteEnable_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set AudioMute status to true for HDMI audio port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_41</test_case_id>
    <test_objective>To set AudioMute status to true for HDMI audio port</test_objective>
...
Checkpoint 2 Verify that the mute is set</expected_output>
    ...
    <test_script>DSHal_SetAndGetAudioMuteEnable_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAndGetAudioMuteEnable_HDMI');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and "SUCCESS":
