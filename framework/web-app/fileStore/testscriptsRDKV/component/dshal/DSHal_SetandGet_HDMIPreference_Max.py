...
<xml>
  ...
  <name>DSHal_SetandGet_HDMIPreference_Max</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set and get HDMI preference to HDCP Protocol version maximum index used for hot plug disconnect</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To set and get HDMI preference to HDCP Protocol version maximum index used for hot plug disconnect</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort to get the handle for HDMI video port
3 . DSHAL agent will invoke the api dsSetHdmiPreference to set the HDCP protocol version to maximum index
4 . DSHAL agent will invoke the api dsGetHdmiPreference to get the HDCP protocol version
5. TM checks if the HDCP protocol version is set and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the HDCP protocol version is set</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_SetandGet_HDMIPreference_Max</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetandGet_HDMIPreference_Max');
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                ...
                tdkTestObj.addParameter("hdcpProtocol", hdcpProtocolVersion["VERSION_MAX"]);
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
                ...
                if expectedResult in actualResult:
                    ...
                    if expectedResult in actualResult:
                        ...
                        if int(details) == hdcpProtocolVersion["VERSION_MAX"]:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "HdmiPreference set successfully";
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print "HdmiPreference not set";
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
