...
<xml>
  ...
  <name>DSHal_SetVolumeLeveller_ValidMode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set volume leveller mode and level used in a given audio port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_164</test_case_id>
    <test_objective>To set the volume leveller in normal mode and check whether level levels are set properly.</test_objective>
...
4. TM checks if the volumeLeveller mode and level are returned as expected and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_SetVolumeLeveller_ValidMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_SetVolumeLeveller_ValidMode');
        ...
        for level in [minLevel,maxLevel]:
            ...
            if expectedResult in actualResult:
                ...
                if expectedResult in actualResult:
                    ...
                    print " "
                    if fail_params:
                        print "TEST FAILED due to unexpected %s"%(fail_params);
