...
<xml>
  ...
  <name>DSHal_GetVideoPortHandle_BB_Invalid_PortIndex</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if BB video port handle is retrieved for invalid port index</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_33</test_case_id>
    <test_objective>To check if BB video port handle is retrieved for invalid port index</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort for invalid port index with port type as BB
...
Checkpoint 2 Verify that handle is not retrieved</expected_output>
    ...
    <test_script>DSHal_GetVideoPortHandle_BB_Invalid_PortIndex</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetVideoPortHandle_BB_Invalid_PortIndex');

#Get the result of connection with test component and STB
dshalloadModuleStatus = dshalObj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %dshalloadModuleStatus);

#Check if BB port is supported by the DUT
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        print(details);
        print("VideoPort handle not retrieved for invalid port index for BB");
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print("VideoPort handle retrieved for invalid port index for BB");
