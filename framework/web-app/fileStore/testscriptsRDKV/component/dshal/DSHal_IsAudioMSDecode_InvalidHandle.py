...
<xml>
  ...
  <name>DSHal_IsAudioMSDecode_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioMSDecode status is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_138</test_case_id>
    <test_objective>To check if AudioMSDecode status is set for invalid handle</test_objective>
...
dsIsAudioMSDecode(int handle, bool enabled)</api_or_interface_used>
...
Checkpoint 2 Verify that the status is not retrieved</expected_output>
    ...
    <test_script>DSHal_IsAudioMSDecode_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsAudioMSDecode_InvalidHandle');
