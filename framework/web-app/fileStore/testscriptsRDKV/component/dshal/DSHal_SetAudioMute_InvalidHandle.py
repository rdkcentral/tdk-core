...
<xml>
  ...
  <name>DSHal_SetAudioMute_InvalidHandle</name>
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_136</test_case_id>
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsSetAudioMute(int handle, bool muted)</api_or_interface_used>
    <input_parameters>handle - Audio port handle
muted - mute status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetAudioMute with invalid handle
...
Checkpoint 2 Verify that the status is not set</expected_output>
    ...
    <test_script>DSHal_SetAudioMute_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAudioMute_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        expectedOutput = "SetAudioMute call failed";
        muted = 1;
        #Prmitive test case which associated to this Script
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_SetAudioMute output: ", details);

        if expectedResult in actualResult:# and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioMute status not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetAudioMute success for invalid handle which is not expected");
