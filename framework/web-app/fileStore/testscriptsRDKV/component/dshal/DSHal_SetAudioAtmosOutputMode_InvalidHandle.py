...
<xml>
  ...
  <name>DSHal_SetAudioAtmosOutputMode_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioAtmosOutputMode is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_134</test_case_id>
    <test_objective>To check if AudioAtmosOutputMode is set for invalid handle</test_objective>
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsSetAudioAtmosOutputMode(int handle, bool enable)</api_or_interface_used>
    <input_parameters>handle - Audio port handle
enable - enable value</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetAudioAtmosOutputMode with invalid handle
...
Checkpoint 2 Verify that the value is not set</expected_output>
    ...
    <test_script>DSHal_SetAudioAtmosOutputMode_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAudioAtmosOutputMode_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_SetAudioAtmosOutputMode output: ", details);

        if expectedResult in actualResult and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioAtmosOutputMode not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetAudioAtmosOutputMode success for invalid handle which is not expected");
