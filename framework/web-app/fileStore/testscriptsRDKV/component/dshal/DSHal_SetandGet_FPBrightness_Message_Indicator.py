...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>DSHal_SetandGet_FPBrightness_Message_Indicator</name>
  <primitive_test_id/>
  <primitive_test_name>DSHal_SetFPBrightness</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set and get the brightness of Message indicator</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>To set the brightness of Message indicator and check using get method</test_objective>
...
Checkpoint 2 Verify that the brightness is set</expected_output>
    ...
    <test_script>DSHal_SetandGet_FPBrightness_Message_Indicator</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetandGet_FPBrightness_Message_Indicator');

#Get the result of connection with test component and STB
dshalloadModuleStatus = dshalObj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %dshalloadModuleStatus);

#Check whether the device supports Message Indicator
capable = deviceCapabilities.getconfig(dshalObj,"Indicator","Message");
