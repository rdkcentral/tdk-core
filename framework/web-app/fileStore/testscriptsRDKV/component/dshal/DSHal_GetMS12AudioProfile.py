...
<xml>
  ...
  <name>DSHal_GetMS12AudioProfile</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to get the current audio profile selection.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_196</test_case_id>
    <test_objective >Test script to get the current audio profile selection.</test_objective>
...
5. Stop dsMgr.service</pre_requisite>
    <api_or_interface_used>dsError_t  dsGetMS12AudioProfile(intptr_t handle, char *profile)</api_or_interface_used>
    <input_parameters> handle - audio port handle
profile - address of the value to get the current selected audio profile</input_parameters>
...
3. Invoke the dsGetMS12AudioProfile api to get the supported audio profile.
...
3. verify the supported audio profile got received successfully.
4. If execution is on source device then dsGetMS12AudioProfile API should return dsERR_OPERATION_NOT_SUPPORTED error code</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_GetMS12AudioProfile</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_GetMS12AudioProfile');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        #Prmitive test case which associated to this Script
        tdkTestObj = obj.createTestStep('DSHal_GetMS12AudioProfile');
        #Execute the test case in STB
        tdkTestObj.executeTestCase(expectedResult);
        actualResult = tdkTestObj.getResult();
        ...
        if expectedResult in actualResult:
            ...
            if source_device_failure_handlings:
                ...
                if expectederrorVal in details:
                    print("GetMS12AudioProfile API returned an expected value : dsERR_OPERATION_NOT_SUPPORTED");
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print("GetMS12AudioProfile API returned an error value other than the expected value : dsERR_OPERATION_NOT_SUPPORTED");
                    tdkTestObj.setResultStatus("FAILURE");
            else:
                print(details);
                print("ACTUAL RESULT  : dsGetMS12AudioProfile call is success");
                tdkTestObj.setResultStatus("SUCCESS");
        else:
            print(details);
            if source_device_failure_handlings:
                print("ACTUAL RESULT : GetMS12AudioProfile call is success instead of returning dsERR_OPERATION_NOT_SUPPORTED");
            else:
                print("ACTUAL RESULT  : dsGetMS12AudioProfile call failed")
            tdkTestObj.setResultStatus("FAILURE");
