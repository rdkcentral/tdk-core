...
<xml>
  ...
  <name>DSHal_GetMatrixCoefficients_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check the matrix coefficient value for HDMI port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_96</test_case_id>
    <test_objective>To check the matrix coefficient value for HDMI port</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort to get the video handle for HDMI port
...
Checkpoint 2 Verify that the coefficient value is valid</expected_output>
    ...
    <test_script>DSHal_GetMatrixCoefficients_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetMatrixCoefficients_HDMI');
