...
<xml>
  ...
  <name>DSHal_IsSurroundDecoderEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the surround decoder enabled status</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To get the SurroundDecoder enabled status for audio port</test_objective>
...
2 . DSHAL agent will invoke the api dsIsSurroundDecoderEnabled to get surround decoder status
...
Checkpoint 3 If Surround decoder feature is not supported on source device then this API should return dsERR_OPERATION_NOT_SUPPORTED always.</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_IsSurroundDecoderEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsSurroundDecoderEnabled');
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if source_device_failure_handlings:
                ...
                print("SurroundDecoderEnabled status retrieved : ", details);
                if details:
                    print("SurroundDecoder is Enabled");
                else:
                    print("SurroundDecoder is Disabled");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            details = tdkTestObj.getResultDetails();
            print(details);
