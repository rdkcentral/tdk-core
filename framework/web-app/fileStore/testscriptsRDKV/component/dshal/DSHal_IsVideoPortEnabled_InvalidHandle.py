...
<xml>
  ...
  <name>DSHal_IsVideoPortEnabled_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the video port enable status is retrieved for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if the video port enable status is retrieved for invalid handle</test_objective>
...
4. TM checks if the API call fails and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_IsVideoPortEnabled_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsVideoPortEnabled_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("Video port enable status not retrieved for invalid hanle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("Video port enable status retrieved for invalid handle which is not expected");
    else:
        tdkTestObj.setResultStatus("FAILURE");
