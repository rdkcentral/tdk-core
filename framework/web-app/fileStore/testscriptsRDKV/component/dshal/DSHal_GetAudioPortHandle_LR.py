...
<xml>
  ...
  <name>DSHal_GetAudioPortHandle_LR</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if audio port handle is retrieved for LR</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_3</test_case_id>
    <test_objective>To check if audio port handle is retrieved for LR</test_objective>
...
2 . DSHAL agent will invoke the api dsGetAudioPort for LR0
...
Checkpoint 2 Verify that handle is retrieved</expected_output>
    ...
    <test_script>DSHal_GetAudioPortHandle_LR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetAudioPortHandle_LR');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        print(details);
        print("AudioPort handle retrieved for LR");
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print("AudioPort handle not retrieved for LR");
