...
<xml>
  ...
  <name>DSHal_SetAudioDelay_InvalidHandle</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_SetAudioDelay</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if AudioDelay is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_132</test_case_id>
    <test_objective>To check if AudioDelay is set for invalid handle</test_objective>
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsSetAudioDelay(int handle, const uint32_t audioDelayMs)</api_or_interface_used>
    <input_parameters>handle - Audio port handle
audioDelayMs- Audio delay in milliseconds</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetAudioDelay with invalid handle
...
Checkpoint 2 Verify that the value is not set</expected_output>
    ...
    <test_script>DSHal_SetAudioDelay_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetAudioDelay_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        expectedOutput = "SetAudioDelay call failed";
        audioDelay = 10;
        #Prmitive test case which associated to this Script
        ...
        tdkTestObj.setResultStatus("SUCCESS");
        details = tdkTestObj.getResultDetails();
        print("DSHal_SetAudioDelay output: ", details);

        if expectedResult in actualResult and expectedOutput in details :
            tdkTestObj.setResultStatus("SUCCESS");
            print("AudioDelay not set for invalid handle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("SetAudioDelay success for invalid handle which is not expected");
