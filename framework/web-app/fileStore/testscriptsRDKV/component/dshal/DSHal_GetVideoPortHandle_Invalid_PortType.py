...
<xml>
  ...
  <name>DSHal_GetVideoPortHandle_Invalid_PortType</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if video port handle is retrieved for INVALID port type</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_18</test_case_id>
    <test_objective>To check if video port handle is retrieved for INVALID port type</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort for INVALID port type
...
Checkpoint 2 Verify that handle is not retrieved</expected_output>
    ...
    <test_script>DSHal_GetVideoPortHandle_Invalid_PortType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetVideoPortHandle_Invalid_PortType');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
        ...
        if expectedResult in actualResult:
            ...
            print details;
            print "VideoPort handle not retrieved for INVALID port type";
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print "VideoPort handle retrieved for INVALID port type";
