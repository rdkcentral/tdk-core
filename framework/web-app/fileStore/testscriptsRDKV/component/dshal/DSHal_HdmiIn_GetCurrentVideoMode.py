...
<xml>
  ...
  <name>DSHal_HdmiIn_GetCurrentVideoMode</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_GetHdmiInCurrentVideoMode</primitive_test_name>
  <!--  -->
...
4. Initialize DSHAL subsystems</pre_requisite>
    <api_or_interface_used>dsHdmiInCurrentVideoMode (dsHdmiInStatus_t *pStatus)</api_or_interface_used>
    <input_parameters>pStatus - status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsHdmiInCurrentVideoMode to get the number of hdmi inputs
...
Checkpoint 2 Verify that the hdmiiin status is retrieved</expected_output>
    ...
    <test_script>DSHal_HdmiIn_GetCurrentVideoMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_HdmiIn_GetCurrentVideoMode');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    tdkTestObj = dshalObj.createTestStep('DSHal_GetHdmiInCurrentVideoMode');
