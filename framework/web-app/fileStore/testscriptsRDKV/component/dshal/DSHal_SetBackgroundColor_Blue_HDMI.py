...
<xml>
  ...
  <name>DSHal_SetBackgroundColor_Blue_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set video device background color to Blue</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_52</test_case_id>
    <test_objective>To set video device background color to Blue</test_objective>
...
2 . DSHAL agent will invoke the api dsSetBackgroundColor to set the BG color to Blue
3. TM checks if the color is set and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the BG color is set to Blue</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_SetBackgroundColor_Blue_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetBackgroundColor_Blue_HDMI');
sysObj.configureTestCase(ip,port,'DSHal_SetBackgroundColor_Blue_HDMI');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and "SUCCESS" in sysloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                print "Trying to set BGColor to Blue";	
                tdkTestObj = dshalObj.createTestStep('DSHal_SetBackgroundColor');
                tdkTestObj.addParameter("color", colorId["Blue"]);
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
                ...
                if expectedResult in actualResult:
                    ...
                    cmd = "grep dsSetBackgroundColor " + logName + " | grep '" + str(colorMap["Blue"]) + "$\|" + str(colorId["Blue"]) + "$'"
                    print cmd;
                    ...
                    print "OUTPUT:", color;
                    if colorMap["Blue"] in color or str(colorId["Blue"]) in color:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "BGColor set to Blue";
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "BGColor not set to Blue";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
