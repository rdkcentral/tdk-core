...
<xml>
  ...
  <name>DSHal_GetHDCPReceiverProtocol_HDMI</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_GetHDCPReceiverProtocol</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the HDCP protocol of the display device connected to HDMI port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_39</test_case_id>
    <test_objective>To get the HDCP protocol of the display device connected to HDMI port</test_objective>
...
dsGetHDCPReceiverProtocol (int handle, dsHdcpProtocolVersion_t *protocolVersion)</api_or_interface_used>
...
2 . DSHAL agent will invoke the api dsGetHDCPReceiverProtocol to get the HDCP protocol of the video device
...
Checkpoint 2 Verify that the HDCP protocol is valid</expected_output>
    ...
    <test_script>DSHal_GetHDCPReceiverProtocol_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetHDCPReceiverProtocol_HDMI');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if details == "true":
                tdkTestObj = dshalObj.createTestStep('DSHal_GetHDCPReceiverProtocol');
                #Execute the test case in STB
                tdkTestObj.executeTestCase(expectedResult);
                actualResult = tdkTestObj.getResult();
                print("DSHal_GetHDCPReceiverProtocol result: ", actualResult);
                if expectedResult in actualResult:
                    protocol = tdkTestObj.getResultDetails();
                    print("Receiver protocol: ", protocol);
                    if int(protocol) == hdcpProtocolVersion["VERSION_1X"] or int(protocol) == hdcpProtocolVersion["VERSION_2X"]:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("HDCPReceiverProtocol is valid");
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("HDCPReceiverProtocol is not valid");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("Failed to get HDCPReceiverProtocol");
