...
<xml>
  ...
  <name>DSHal_EnableVideoPort_BB</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the video port enable status is set for unsupported port type</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if the video port enable status is set for BaseBand port type</test_objective>
...
dsEnableVideoPort(int handle, bool enabled)</api_or_interface_used>
...
2 . DSHAL agent will invoke the api dsGetVideoPort to get the handle for port type which is not supported for dsEnableVideoPort API
3 . DSHAL agent will invoke the api dsEnableVideoPort to get the video port enable status
4. TM checks if the API call fails and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_EnableVideoPort_BB</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_EnableVideoPort_BB');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("SUCCESS: BB port enable call is successfull as expected");
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print("Video port enable status not set for BB port type which is not expected");
    else:
        tdkTestObj.setResultStatus("FAILURE");
