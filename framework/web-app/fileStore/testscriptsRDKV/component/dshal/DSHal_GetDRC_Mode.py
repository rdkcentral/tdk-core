...
<xml>
  ...
  <name>DSHal_GetDRC_Mode</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_GetDRCMode</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Test to get the current DRC mode</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test to get the current DRC mode</test_objective>
...
2 . DSHAL agent will invoke the api dsGetDRCMode to get the mode value
3. TM checks if the mode value is retrieved or not and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>Checkpoint 1.Verify the API call is success
Checkpoint 2 Verify that the mode is drc mode is retrieved</expected_output>
    <priority>High</priority>
    <test_stub_interface>libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_GetDRC_Mode</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetDRC_Mode');
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            details = tdkTestObj.getResultDetails();
            print "DRC retrieved successfully ", details
            tdkTestObj.setResultStatus("SUCCESS");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print "DSHal_GetDRC failed";
    else:
        tdkTestObj.setResultStatus("FAILURE");
