...
<xml>
  ...
  <name>DSHal_IsAudioMSDecode_HDMI</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id></primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>DSHal_IsAudioMSDecode</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the AudioMSDecode status for HDMI audio port</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To get the AudioMSDecode status for HDMI audio port</test_objective>
...
dsIsAudioMSDecode(int handle, bool *ms11Enabled)</api_or_interface_used>
    <input_parameters>type - Audio port type
index- Audio port index
handle - Audio port handle
ms11Enabled - MSdecode status</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsIsAudioMSDecode to get MSDecode status
...
Checkpoint 2 Verify that the status is correct</expected_output>
    ...
    <test_script>DSHal_IsAudioMSDecode_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_IsAudioMSDecode_HDMI');
