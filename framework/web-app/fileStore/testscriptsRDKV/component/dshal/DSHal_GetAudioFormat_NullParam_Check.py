...
<xml>
  ...
  <name>DSHal_GetAudioFormat_NullParam_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to check Getaudioformat HAL api by passing null param.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Test script to validate the GetAudioFormat API by passing null param</test_objective>
...
3. Invoke the dsGetAudioFormat api by passing a null param to the audioFormat argument.
4. TM checks if the dsGetAudioFormat returned a corresponding error code retrieved and return SUCCESS/FAILURE status.</automation_approch>
    <expected_output>1. Verify the API call is success
2. verify that handle is received
3. verify the dsGetAudioFormat API got returned a proper error statement</expected_output>
    <priority >High</priority>
    <test_stub_interface >libdshalstub.so.0.0.0</test_stub_interface>
    <test_script>DSHal_GetAudioFormat_NullParam_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_GetAudioFormat_NullParam_Check');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if result:
        ...
        print("playback triggered and playing successfully");
        ...
        print("Test Step: Passing NULL param to the GetAudioFormat API");
        print("Expected result: API should fail with the corresponding invalid param error");
        ...
        if expectedResult in actualResult:
            print(details);
            print("ACTUAL RESULT : dsGetAudioFormat call is failed due to null param being passed ");
            tdkTestObj.setResultStatus("SUCCESS");
            print("[TEST EXECUTION RESULT] : SUCCESS");
        else:
            print("Value returned : ",details);
            print("ACTUAL RESULT : dsGetAudioForma call is success eventhough with the null param");
