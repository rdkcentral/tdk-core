...
<xml>
  ...
  <name>DSHal_GetAudioEncoding_SPDIF</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the audio encoding type of the audio port SPDIF</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_12</test_case_id>
    <test_objective>To get the audio encoding type of the audio port SPDIF</test_objective>
...
Checkpoint 2 Verify that the encoding type is valid</expected_output>
    ...
    <test_script>DSHal_GetAudioEncoding_SPDIF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetAudioEncoding_SPDIF');
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            ...
            if int(details) == audioEncodingType["PCM"] or int(details) == audioEncodingType["AC3"] or int(details) == audioEncodingType["EAC3"]:
                tdkTestObj.setResultStatus("SUCCESS");
                print("AudioEncoding has valid value for SPDIF");
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print("AudioEncoding value is not valid for SPDIF");
        else:
            tdkTestObj.setResultStatus("FAILURE");
