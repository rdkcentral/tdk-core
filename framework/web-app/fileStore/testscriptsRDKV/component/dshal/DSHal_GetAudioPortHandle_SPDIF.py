...
<xml>
  ...
  <name>DSHal_GetAudioPortHandle_SPDIF</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if audio port handle is retrieved for SPDIF</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_2</test_case_id>
    <test_objective>To check if audio port handle is retrieved for SPDIF</test_objective>
...
2 . DSHAL agent will invoke the api dsGetAudioPort for SPDIF0
...
Checkpoint 2 Verify that handle is retrieved</expected_output>
    ...
    <test_script>DSHal_GetAudioPortHandle_SPDIF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetAudioPortHandle_SPDIF');

#Get the result of connection with test component and STB
dshalloadModuleStatus = dshalObj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %dshalloadModuleStatus);

#Check if SPDIF port is supported bby DUT
...
if "SUCCESS" in dshalloadModuleStatus.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        print(details);
        print("AudioPort handle retrieved for SPDIF");
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print("AudioPort handle not retrieved for SPDIF");
