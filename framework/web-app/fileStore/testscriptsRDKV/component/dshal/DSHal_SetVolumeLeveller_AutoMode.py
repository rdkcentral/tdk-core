...
<xml>
  ...
  <name>DSHal_SetVolumeLeveller_AutoMode</name>
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_166</test_case_id>
    <test_objective>To set the volume leveller in auto mode and check whether level levels are set properly.</test_objective>
...
4. TM checks if the volumeLeveller mode and level are returned as expected and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_SetVolumeLeveller_AutoMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_SetVolumeLeveller_AutoMode');
