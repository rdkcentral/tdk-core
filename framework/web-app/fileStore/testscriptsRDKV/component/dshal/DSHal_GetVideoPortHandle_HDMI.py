...
<xml>
  ...
  <name>DSHal_GetVideoPortHandle_HDMI</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if video port handle is retrieved for HDMI</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_15</test_case_id>
    <test_objective>To check if video port handle is retrieved for HDMI</test_objective>
...
2 . DSHAL agent will invoke the api dsGetVideoPort for HDMI0
...
Checkpoint 2 Verify that handle is retrieved</expected_output>
    ...
    <test_script>DSHal_GetVideoPortHandle_HDMI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_GetVideoPortHandle_HDMI');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
        ...
        if expectedResult in actualResult:
            ...
            print details;
            print "VideoPort handle retrieved for HDMI";
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print "VideoPort handle not retrieved for HDMI";
