...
<xml>
  ...
  <name>DSHal_SetSurroundVirtualizer_InvalidBoost</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set the invalid surround virtualizer boost level and check invalid boost is not set</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_162</test_case_id>
    <test_objective>To set invalid surround virtualizer boost level and verify whether the existing virtualizer mode is unhindered.</test_objective>
...
2. DSHAL agent will invoke the api dsSetSurroundVirtualizer to set the valid virtualizer mode and invalid boost.
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_SetSurroundVirtualizer_InvalidBoost');
...
if "SUCCESS" in result.upper() and capable:
    ...
    if expectedResult in actualResult:
        ...
        print("\n Trying to set Invalid Boost")
        #Trying to set invalid boost with valid mode
        #Valid Mode values  : 0-Disable, 1-Normal, 2-AUTO
        for Mode in range(0,3):
            print("\n Mode = %s , Boost = %s "%(Mode,Boost_test))
