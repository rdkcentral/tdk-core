...
<xml>
  ...
  <name>DSHal_SetMISteering_InvalidHandle_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the setMISteering status API by passing invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_194</test_case_id>
    <test_objective>To validate SetMISteering API using invalid handle param</test_objective>
...
enabled - MISteering enabled(1)/disabled(0) value</input_parameters>
    <automation_approch>1. TM loads the DSHAL agent via the test agent.
2 . DSHAL agent will invoke the api dsSetMISteering to set the enable/disable status
...
Checkpoint 2 Verify that the status is correct</expected_output>
    ...
    <test_script>DSHal_SetMISteering_InvalidHandle_Check</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetMISteering_InvalidHandle_Check');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            print("Test step to validate SetMISteering API by passing an invalid handle");
            ...
            if expectedResult in actualResult:
                print(details);
                print("ACTUAL RESULT : dsSetMISteering call is failed due to invalid handle");
                tdkTestObj.setResultStatus("SUCCESS");
                print("[TEST EXECUTION RESULT] : SUCCESS");
            else:
                print("Value returned : ", details);
                print("ACTUAL RESULT : dsSetMISteering call is success eventhough with an invalid handle");
            ...
            details = tdkTestObj.getResultDetails();
            print(details);
            print("Failed to MISteering status");
