...
<xml>
  ...
  <name>DSHal_EnableVideoPort_InvalidHandle</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the video port enable status is set for invalid handle</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_148</test_case_id>
    <test_objective>To check if the video port enable status is set for invalid handle</test_objective>
...
3 . DSHAL agent will invoke the api dsEnableVideoPort to set the video port enable status
4. TM checks if the API call fails and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_EnableVideoPort_InvalidHandle</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_EnableVideoPort_InvalidHandle');
...
if "SUCCESS" in dshalloadModuleStatus.upper():
    ...
    if expectedResult in actualResult:
        ...
        if expectedResult in actualResult:
            tdkTestObj.setResultStatus("SUCCESS");
            print("Video port enable status not set for invalid hanle which is expected");
        else:
            tdkTestObj.setResultStatus("FAILURE");
            print("Video port enable status set for invalid handle which is not expected");
    else:
        tdkTestObj.setResultStatus("FAILURE");
