...
<xml>
  ...
  <name>DSHal_SetandGet_HDMIPreference_Version_Invalid</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if HDMI preference is set to invalid HDCP Protocol version</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check if HDMI preference is set to invalid HDCP Protocol version</test_objective>
...
3 . DSHAL agent will invoke the api dsSetHdmiPreference to set the HDCP protocol version to invalid
...
Checkpoint 2 Verify that the HDCP protocol version is not set</expected_output>
    ...
    <test_script>DSHal_SetandGet_HDMIPreference_Version_Invalid</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_SetandGet_HDMIPreference_Version_Invalid');
