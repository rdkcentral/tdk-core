...
<xml>
  ...
  <name>DSHal_HdmiIn_SelectZoomMode_Full</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if HdmiIn select zoom mode is working for mode full</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_DS_HAL_107</test_case_id>
    <test_objective>To check if HdmiIn select zoom mode is working for mode full</test_objective>
...
3. TM checks if the api call is success and return SUCCESS/FAILURE status.</automation_approch>
    ...
    <test_script>DSHal_HdmiIn_SelectZoomMode_Full</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
dshalObj.configureTestCase(ip,port,'DSHal_HdmiIn_SelectZoomMode_Full');
