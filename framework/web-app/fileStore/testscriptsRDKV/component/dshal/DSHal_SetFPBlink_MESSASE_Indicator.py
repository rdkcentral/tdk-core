...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>DSHal_SetFPBlink_MESSASE_Indicator</name>
  <primitive_test_id/>
...
4. Initialize DSHAL subsystems</pre_requisite>
    ...
    <input_parameters>eIndicator - FP LED indicator (MESSAGE)
uBlinkDuration - time duration
uBlinkIteration - number of times the LED should blink</input_parameters>
    <automation_approch>1.TM loads the DSHAL agent via the test agent.
2.DSHAL agent will invoke the API dsSetFPBlink with parameter LED indicator 0 [MESSAGE LED]
...
5.Unload the module</automation_approch>
    ...
    <test_script>DSHal_SetFPBlink_MESSASE_Indicator</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DSHal_SetFPBlink_MESSAGE_Indicator');

#Get the result of connection with test component and STB
loadModuleStatus = obj.getLoadModuleResult();
print("[LIB LOAD STATUS]  :  %s" %loadModuleStatus);

#Check if the device supports Blink and message inicator
capable = deviceCapabilities.getconfig(obj,"SetBlink") and deviceCapabilities.getconfig(obj,"Indicator","Message");

if "SUCCESS" in loadModuleStatus.upper() and capable:
    obj.setLoadModuleStatus("SUCCESS");
    expectedResult="SUCCESS";
    print("\nTEST STEP1 : To set the Message indicator in Front Panel to Blink")
    print("EXEPECTED RESULT : Should set the Message led to blink for given number of time for given duration")
