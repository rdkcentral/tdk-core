...
<xml>
  ...
  <name>HdmicecHal_TxAsync_Get_PowerStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to get the power status  from HDMI device using asynchronous transmission</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_10</test_case_id>
    <test_objective>Test Script to get the power status  from HDMI device using asynchronous transmission</test_objective>
...
3. Transmit the CEC frame to get  power status
...
6. Unload the module</automation_approch>
    <expected_output>Should transmit and receive  power status CEC frames</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_TxAsync_Get_PowerStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_TxAsync_Get_PowerStatus');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    # Opcode to be sent: GIVE_DEVICE_POWER_STATUS = 0x8F
    # Operand to be received: REPORT_POWER_STATUS = 0x90
