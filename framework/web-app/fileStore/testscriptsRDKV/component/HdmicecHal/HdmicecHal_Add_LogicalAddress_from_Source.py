...
<xml>
  ...
  <name>HdmicecHal_Add_LogicalAddress_from_Source</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>to add the logical address from source device.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_21</test_case_id>
    <test_objective>to add the logical address from source device</test_objective>
    <test_type>positive</test_type>
    <test_setup>Video_Accelerator</test_setup>
    <pre_requisite>1. TDK Agent should be up and running
    2. connect with HDMI CEC enabled TV with source device
    ...
    <expected_output>Driver should return an invalid argument since it wont support for source devices</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_Add_LogicalAddress_from_Source</test_script>
    <skipped>no</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Add_LogicalAddress_from_Source');
...
if "SUCCESS" in result.upper():
    ...
    print("\nTEST STEP : Add the logical address using HdmiCecAddLogicalAddress API");
