...
<xml>
  ...
  <name>HdmicecHal_TxAsync_Get_MenuLanguage</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test Script to get the currently set menu language  using asynchronous transmission</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HdmicecHal_09</test_case_id>
    <test_objective>Test Script to get the currently set menu language  using asynchronous transmission</test_objective>
...
3. Transmit the CEC frame to get menu language
...
6. Unload the module</automation_approch>
    <expected_output>Should transmit and receive menu language CEC frames</expected_output>
    <priority>High</priority>
    <test_stub_interface>libhdmicechalstub.so.0.0.0</test_stub_interface>
    <test_script>HdmicecHal_TxAsync_Get_MenuLanguage</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_TxAsync_Get_MenuLanguage');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    # Opcode to be sent: GET_MENU_LANGUAGE = 0X91
    # Operand to be received: SET_MENU_LANGUAGE = 0X32
    ...
    if expectedResult in actualResult1 and expectedResult in actualResult2:
        ...
        print("\nTEST STEP : Transmit CEC Frame to get currently selected Menu language")
        print("EXEPECTED RESULT : Should receive the Menu language frame")
        tdkTestObj = obj.createTestStep('HdmicecHal_TxAsync');
        menu_language_opcode = "91"
        menu_language_operand = "32"
        print("Opcode to be sent: GET_MENU_LANGUAGE: ",menu_language_opcode)
        print("Operand to be received: SET_MENU_LANGUAGE: ",menu_language_operand)
        tdkTestObj.addParameter("opcode",menu_language_opcode);
        tdkTestObj.executeTestCase(expectedResult);
        ...
        if expectedResult in actualResult:
            ...
            menu_language_operand_received = str(details).split(";")[0].split(":")[1].strip()
            print("Value Retuned:")
            for info in txInfo:
                print(info)
            if menu_language_operand in menu_language_operand_received:
                print("ACTUAL RESULT  : Menu Language frame received")
                print("[TEST EXECUTION RESULT] : SUCCESS\n")
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print("ACTUAL RESULT  : Menu Language frame not received")
