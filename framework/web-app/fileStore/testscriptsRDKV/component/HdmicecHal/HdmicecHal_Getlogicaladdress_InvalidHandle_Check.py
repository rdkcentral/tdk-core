...
<xml>
  ...
  <name>HdmicecHal_Getlogicaladdress_InvalidHandle_Check</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to validate the GetLogicalAddress HAL api by passing an invalid handle.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <rdk_versions>
   ...
   <test_cases>
    <test_case_id>TC_HdmicecHal_24</test_case_id>
    <test_objective>Test script to validate with the invalid handle param</test_objective>
...
3.  HdmiCecOpen should open a CEC driver instance successfully and iarmbus event should be obtained from device ready call back.</pre_requisite>
    ...
    <input_parameters>handle - pass invalid driver handle to validate
    logicalAddresses - pass address to retrieve the logical address value </input_parameters>
    ...
    <test_script>HdmicecHal_Getlogicaladdress_InvalidHandle_Check</test_script>
    <skipped>no</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'HdmicecHal_Getlogicaladdress_InvalidHandle_Check');
...
if "SUCCESS" in loadModuleStatus.upper():
    ...
    print("\nTEST STEP : Verify the HdmiCecGetLogicalAddress API by passing invalid handle param")
    ...
    if expectedResult in actualResult:
        ...
        print(details)
        print("ACTUAL RESULT: HdmiCecGetLogicalAddress call failed due to invalid handle")
