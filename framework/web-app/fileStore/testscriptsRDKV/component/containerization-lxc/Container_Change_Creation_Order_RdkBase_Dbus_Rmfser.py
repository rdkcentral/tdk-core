...
<xml>
  ...
  <name>Container_Change_Creation_Order_RdkBase_Dbus_Rmfser</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify container creation order is correct for the sequence ["rdk-base", "dbus", "rmfserv"]</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Container_16</test_case_id>
    <test_objective>Verify container creation order is correct for the sequence ["rdk-base", "dbus", "rmfserv"]</test_objective>
...
2. SystemUtilAgent will change the contents of the file '/etc/containers_order.conf' to "rdk-base dbus rmfserv"
...
6. TM unloads the SystemUtilAgent</automation_approch>
    ...
    <test_script>Container_Change_Creation_Order_RdkBase_Dbus_Rmfser</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'Container_Change_Creation_Order_RdkBase_Dbus_Rmfser');
...
if "SUCCESS" in loadStatus.upper():
	...
	if result:
		containerList = ["rdk-base", "dbus", "rmfserv"];
