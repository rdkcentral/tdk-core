...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>E2E_PortTriggering_WANtoWLAN_BOTH_TargetFTP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 21 is forwarded to the WLAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule with protocol as BOTH.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_605</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 21 is forwarded to the WLAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule with protocol as BOTH.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - BOTH, Target Protocol - BOTH, Description - MyPTRule
...
18. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_BOTH_TargetFTP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_BOTH_TargetFTP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_BOTH_TargetFTP');
