...
<xml>
  ...
  <name>E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTP_FromLanToWLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that HTTP traffic between Lan to WLAN should be successful when wifi client is associated with wpa3-psk authentication for 6GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_662</test_case_id>
    <test_objective>This test case is to verify that HTTP traffic between Lan to WLAN should be successful when wifi client is assocatiated with wpa3-psk authentication for 6GHZ</test_objective>
...
3. Set security mode to WPA3-Personal,radio enable and ssid enable to true using tdkb_e2e_SetMultipleParams
4. Try to connect to 6GHZ from WIFI client.
5. send HTTP request to wlan client from lan client
6. Revert the values</automation_approch>
    <expected_output>HTTP request between wlan and lan client should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTP_FromLanToWLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTP_FromLanToWLan')
wifi_obj.configureTestCase(ip,port,'E2E_SANITY_WIFI_6GHZ_WPA3Personal_HTTP_FromLanToWLan')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        # Assign the WIFI parameters names to variables
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                ...
                if expectedresult in actualresult:
                    ...
                    step += 1
                    print(f"\nTEST STEP {step}: Get the current ssid, keypassphrase,radioEnable and ssidEnable after setting")
                    print(f"EXPECTED RESULT {step}: Retrieved values should match set values")
                        ...
                        if expectedresult in status:
                            ...
                            if wlanIP:
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}: WLAN IP {wlanIP} is in same range as {curIPAddress}")
                                        print("TEST EXECUTION RESULT : SUCCESS")

                                        # Get the IP address of the lan client
                                        step += 1
                                        print(f"\nTEST STEP {step}: Get the IP address of the lan client after connecting to it")
                                        print(f"EXPECTED RESULT {step}: Should retrieve a valid LAN IP address")
                                        ...
                                        if lanIP:
                                            ...
                                            if expectedresult in status:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: LAN client IP {lanIP} is in same range as {curIPAddress}")
                                                print("TEST EXECUTION RESULT : SUCCESS")
                                                ...
                                                print(f"EXPECTED RESULT {step}: HTTP connectivity should be successful")
                                                status = tdkbE2EUtility.verifyNetworkConnectivity(wlanIP, "WGET_HTTP", lanIP, curIPAddress, "LAN")

                                                if "SUCCESS" in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    print(f"ACTUAL RESULT {step}: HTTP is successful between wired and wireless clients")
                                                    print("TEST EXECUTION RESULT : SUCCESS")
                                                    finalStatus = "SUCCESS"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE")
                                                    print(f"ACTUAL RESULT {step}: HTTP not successful between wired and wireless clients")
                                                    print("TEST EXECUTION RESULT : FAILURE")
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print(f"ACTUAL RESULT {step}: LAN IP address not in the same DHCP range")
                                                print("TEST EXECUTION RESULT : FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print(f"ACTUAL RESULT {step}: Failed to get the IP address of the lan client")
                                            print("TEST EXECUTION RESULT : FAILURE")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print(f"ACTUAL RESULT {step}: WLAN IP address not in same DHCP range")
                                        print("TEST EXECUTION RESULT : FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                            ...
                            print(f"EXPECTED RESULT {step}: WLAN connection should be successfully disconnected")
