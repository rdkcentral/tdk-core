...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_ParentalControl_ManagedServices_5GHZ_BlockFTP_AllowHTTP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG allows other services for Wi-Fi Client associated using 5GHZ radio when only FTP service is blocked through parental control( managed services)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_131</test_case_id>
    <test_objective>Verify that WG allows other services for Wi-Fi Client associated using 5GHZ radio when only FTP service is blocked through parental control( managed services)</test_objective>
...
3. Enable managed service and set WPA2PSK as security mode
4. Add new rule to "Device.X_Comcast_com_ParentalControl.ManagedServices.Service."
5. Set values to all fields in new rule to block ftp
6.Check whether the WLAN client is able to access internet using http
7.Delete the added rule and revert all params values
8.Unload tdkb_e2e module</automation_approch>
    <except_output>The WLAN client should be able to access the internet using the unblocked service HTTP</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedServices_5GHZ_BlockFTP_AllowHTTP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_BlockFTP_AllowHTTP');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_5GHZ_BlockFTP_AllowHTTP');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            # Set the SSID name,password,managedServiceEnable,blockedService and securityMode"
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in actualresult:
                        ...
                        if (int(instance) > 0):
                            ...
                            setValuesList = ['ftp','BOTH','21','21','true'];
                            print("Parameter values that are set: %s" %setValuesList)

                            list1 = [description,'ftp','string']
                            list2 = [protocol,'BOTH','string']
                            list3 = [startport,'21','unsignedInt']
                            list4 = [endPort,'21','unsignedInt']
                                ...
                                if expectedresult in status and setValuesList == newValues:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if wlanIP:
                                            ...
                                            print("TEST STEP 9: Get the current WAN IP address DHCP range")
                                            param = "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress"
                                            tdkTestObj,status,curIPAddress = getParameterValue(obj,param)
                                            print("WAN IP Address: %s" %curIPAddress);
                                            ...
                                            if expectedresult in status and curIPAddress:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    status = addStaticRoute(tdkbE2EUtility.wan_http_ip, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
                                                    if expectedresult in status:
                                                        print("TEST STEP 11:Static route add success")
                                                        tdkTestObj.setResultStatus("SUCCESS");

                                                        print("TEST STEP 12:Check the Http connectivity from WLAN to WAN")

                                                        status = wgetToWAN("WGET_HTTP", wlanIP, curIPAddress)

                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("The allowedservice 'HTTP' can be reached")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                        else:
                                                        ...
                                                        status = delStaticRoute(tdkbE2EUtility.wan_http_ip, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("TEST STEP 13:Static route delete success")
                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("TEST STEP 13:Static route delete failed")
