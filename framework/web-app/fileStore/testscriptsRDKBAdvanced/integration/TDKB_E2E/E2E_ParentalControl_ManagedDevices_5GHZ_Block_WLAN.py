...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_ParentalControl_ManagedDevices_5GHZ_Block_WLAN</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG allows specific Wi-Fi clients associated using 5GHZ radio based on MACaddress through parental control (managed devices)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_132</test_case_id>
    <test_objective>Verify that WG allows specific Wi-Fi clients associated using 5GHZ radio based on MACaddress through parental control (managed devices)</test_objective>
...
4. Enable managed device, set false to Allowall and set WPA2PSK as security mode
5.  Connect to the WIFI client and get the MAC address of the WLAN client
6.Add new rule to Device.X_Comcast_com_ParentalControl.ManagedDevices.Device.
7. Set values to all fields in new rule
8.Check whether the WLAN client is able to access internet using wget
9.Delete the added rule and revert all params values
10.Unload tdkb_e2e module</automation_approch>
    <except_output>The blocked WLAN client should not access the internet</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedDevices_5GHZ_Block_WLAN</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedDevices_5GHZ_Block_WLAN');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedDevices_5GHZ_Block_WLAN');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'true','false','WPA2-Personal'];
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if MAC:
                                                            ...
                                                            if expectedresult in status:
                                                                ...
                                                                if wlanIP:
                                                                    ...
                                                                    if expectedresult in status:
                                                                        ...
                                                                        tdkTestObj.setResultStatus("FAILURE");
                                                                        print("TEST STEP 13:WLAN Client IP address is not in the same Gateway DHCP range")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("Failed to disconnect from wifi ssid")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
            ...
            if expectedresult in actualresult and expectedresult in actualresult1 and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 16: Should set the original ssid,keypassphrase,securityMode,managedDeviceEnable and allowAll");
