...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_BridgeMode_CheckLANIPAddress</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when Bridge mode is enabled, ensure that the Ethernet client gets the WAN IP directly</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_202</test_case_id>
    <test_objective>Verify that when Bridge mode is enabled, ensure that the Ethernet client gets the WAN IP directly</test_objective>
...
5.  Connect to the LAN client
6.Check if ping to default GW IP is success and to DHCP server is failure
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Ping from LAN to Default Gateway WAN IP Address should be success and Ping from LAN to DHCP server should be failure</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_BridgeMode_CheckLANIPAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_BridgeMode_CheckLANIPAddress');
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if lanIP:
                        ...
                        if expectedresult in status and curIPAddress:
                            tdkTestObj.setResultStatus("SUCCESS");
                            #Send ping request from LAN to default gateway wan ip address and DHCP server
                            print("TEST STEP 6:Check the PING from LAN");
                            status = verifyNetworkConnectivity(orgValue[1], "PING", lanIP, orgValue[1], "LAN")
                            if expectedresult in status:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print("Ping successful from LAN to Default Gateway WAN IP Address")
                                status = verifyNetworkConnectivity(curIPAddress, "PING", lanIP, orgValue[1], "LAN")
