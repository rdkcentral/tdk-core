...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_ParentalControl_ManagedServices_2.4GHZ_BlockHTTP_AllowHTTPS</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG allows other services for Wi-Fi Client associated using 2.4GHZ radio when only HTTP service is blocked through parental control (managed Services)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_108</test_case_id>
    <test_objective>Verify that WG allows other services for Wi-Fi Client associated using 2.4GHZ radio when only HTTP service is blocked through parental control (managed Services)</test_objective>
...
5. Set values to all fields in new rule to block http
6.Check whether the WWAN client is able to access internet using https
7.Delete the added rule and revert all params values
8.Unload tdkb_e2e module</automation_approch>
    <except_output>The WWAN client should be able to access the internet using the unblocked service HTTPS</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedServices_2.4GHZ_BlockHTTP_AllowHTTPS</test_script>
    <skipped>No</skipped>
    <release_version>M57</release_version>
    <remarks>WWAN,WAN</remarks>
  </test_cases>
</xml>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_2.4GHZ_BlockHTTP_AllowHTTPS');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_2.4GHZ_BlockHTTP_AllowHTTPS');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'true','WPA2-Personal'];
                                ...
                                if expectedresult in status and setValuesList == newValues:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if wlanIP:
                                            ...
                                            if expectedresult in status and curIPAddress:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("TEST STEP 12:Check the Https connectivity from WWAN to WAN")

                                                        status = wgetToWAN("WGET_HTTPS", wlanIP, curIPAddress)

                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("The allowed service 'HTTPS' can be reached")
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("The allowed service 'HTTPS' cannot be reached")
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("TEST STEP 8:Failed to get the WWAN Client IP address")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
