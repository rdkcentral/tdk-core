...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetIPERF</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is forwarded to the LAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_559</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is forwarded to the LAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</test_objective>
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 5001 is forwarded to the LAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetIPERF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetIPERF');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetIPERF');
