...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidFTPPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 2121 is forwarded to the LAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_640</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 2121 is forwarded to the LAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
11. From WAN client, send an inbound packet to the target port 2121 via the DUT GW WAN IP. The packet should NOT be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 2121 is forwarded to the LAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidFTPPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidFTPPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetInvalidFTPPort');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "12345"
                        triggerEnd = "12345"
                        targetStart = "21"
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    step = step + 1
                                    #FTP with invalid port
                                    invalidPort = "2121"
                                    print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the LAN client that triggered it" %step)
                                    print("EXPECTED RESULT %d: FTP from WAN to the LAN should be failed with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
                                    status = ftpToClientWithPort("LAN", tdkbE2EUtility.gw_wan_ip, "WAN", invalidPort);
