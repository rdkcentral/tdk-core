...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_MACFilter_CheckDefaultSettings</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that by default MAC address based Access control is disabled and allows all wireless clients connected to WG</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_42</test_case_id>
    <test_objective>Verify that by default MAC address based Access control is disabled and allows all wireless clients connected to WG</test_objective>
...
2. Using tdkb_e2e_Get, get MACFilter.FilterAsBlackList, MACFilter.Enable
3.Check if both MACFilter.Enable and  MACFilter.FilterAsBlackList are in disabled state by default
4. If disabled, try to connect the WIFI client
5. Check if the connection was success and a valid ip is assigned to the client.
6.Unload tdkb_e2e module</automation_approch>
    <except_output>Both MACFilter.FilterAsBlackList, MACFilter.Enable should be in disabled state by default and the connection to the WiFi client should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_MACFilter_CheckDefaultSettings</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_MACFilter_CheckDefaultSettings');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in status:
        ...
        currentMacFilterEnable = orgValue[3]
        currentMacFilterBlackList = orgValue[4]

        if currentMacFilterEnable == "false" and currentMacFilterBlackList == "false" and expectedresult in status:
                    ...
                    if expectedresult in status:
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("TEST STEP 7: wlan ip address is in same DHCP range")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                    finalStatus = "SUCCESS";
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("TEST STEP 7: wlan ip address is not in same DHCP range")
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 13: Set the ssid,keypassphrase,Radio enable status")
                print("EXPECTED RESULT 13: Should set the ssid,keypassphrase,Radio enable status")
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print("TEST STEP 1: Get the current macFilterEnable, macFilterBlackList status")
            print("EXPECTED RESULT 1: Should retrieve the current macFilterEnable, macFilterBlackList status as disabled")
            print("ACTUAL RESULT 1: %s" %orgValue);
            print("[TEST EXECUTION RESULT] : FAILURE macFilterEnable, macFilterBlackList status is not disabled")
    else:
