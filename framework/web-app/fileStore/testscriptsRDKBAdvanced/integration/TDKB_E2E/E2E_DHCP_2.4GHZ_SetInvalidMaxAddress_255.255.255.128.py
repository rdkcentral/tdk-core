...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_2.4GHZ_SetInvalidMaxAddress_255.255.255.128</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when DHCP Server Subnet mask is set to 255.255.255.128  Max address cannot be set greater than 10.0.0.127 due to block size of 128</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_176</test_case_id>
    <test_objective>Verify that when DHCP Server Subnet mask is set to 255.255.255.128  Max address cannot be set greater than 10.0.0.127 due to block size of 128</test_objective>
...
4.  Connect to the WIFI client and check whether the invalid maximum ip address is being set or not
5. Revert the values to original value
6.Unload tdkb_e2e module</automation_approch>
    <except_output>Invalid maximum ip address should not be set.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_2.4GHZ_SetInvalidMaxAddress_255.255.255.128</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_2.4GHZ_SetInvalidMaxAddress_255.255.255.128');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'10.0.0.1','255.255.255.128','10.0.0.2','10.0.0.130'];
            ...
            list4 = [lanSubnetMask,'255.255.255.128','string']
            list5 = [minAddress,'10.0.0.2','string']
            list6 = [maxAddress,'10.0.0.130','string']
            ...
            if expectedresult in actualresult and expectedresult not in actualresult1:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 2: Set the ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                print("EXPECTED RESULT 2: Should not set Max address as 10.0.0.130 as the value is greater than block size 128");
                print("ACTUAL RESULT 2: %s" %details);
                print("[TEST EXECUTION RESULT] : SUCCESS");

                setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'10.0.0.1','255.255.255.128','10.0.0.2','10.0.0.126'];
                print("Parameter values that are set: %s" %setValuesList)
                ...
                list4 = [lanSubnetMask,'255.255.255.128','string']
                list5 = [minAddress,'10.0.0.2','string']
                list6 = [maxAddress,'10.0.0.126','string']
                ...
                if expectedresult in actualresult and expectedresult in actualresult1:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("TEST STEP 3: Set the ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                    print("EXPECTED RESULT 3: Should set the ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress");
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Get the current ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                        print("EXPECTED RESULT 4: Should retrieve the current ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                        ...
                        if expectedresult in status:
                            ...
                            print("TEST STEP 6: Get the subnetmask of the wlan client after connecting to wifi")
                            wlanSubnetMask = getWlanSubnetMask(tdkbE2EUtility.wlan_2ghz_interface);
                            if wlanSubnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print("getWlanSubnetMask: SUCCESS")

                                print("TEST STEP 7: Check whether subnet mask is 255.255.255.128")
                                if "255.255.255.128" in wlanSubnetMask:
                                    status = "SUCCESS";
                                else:
                                    status = "FAILURE";
                                if expectedresult in status:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("The subnet mask is 255.255.255.128")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("TEST STEP 8:Disconnect from WIFI SSID: FAILED")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("TEST STEP 7:The subnet mask is not 255.255.255.128")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("TEST STEP 6:Failed to get the Subnet mask")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5:Failed to connect to WIFI SSID")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("TEST STEP 4: Get the current ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                        print("EXPECTED RESULT 4: Should retrieve the current ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress")
            ...
            if expectedresult in actualresult and expectedresult in actualresult1 and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 9: Should set the original ssid,keypassphrase,lanIPAddress,lanSubnetMask,minAddress and maxAddress");
