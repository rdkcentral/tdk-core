...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_OpenSecurity_RadioDisabled_HTTPSTraffic</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that https traffic between WLan to LAN should be denied when wifi client assocation in open authentication for 2.4GHZ and Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_362</test_case_id>
    <test_objective>Verify that https traffic between WLan to LAN should be denied when wifi client assocation in open authentication for 2.4GHZ and Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</test_objective>
...
4. Disable radio and send https request to lan client from wifi client
4. Revert the values</automation_approch>
    ...
    <test_script>E2E_WIFI_2.4GHZ_OpenSecurity_RadioDisabled_HTTPSTraffic</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_OpenSecurity_RadioDisabled_HTTPSTraffic');
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status:
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print("wlanConnectWifiSsid:Failed to connect the wifi ssid")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current ssid,securityMode,radioEnable and ssidEnable")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,securityMode,radioEnable and ssidEnable")
