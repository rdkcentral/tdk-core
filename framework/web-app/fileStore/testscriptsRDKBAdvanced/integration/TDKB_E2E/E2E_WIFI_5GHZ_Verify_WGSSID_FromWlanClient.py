...
<xml>
  ...
  <name>E2E_WIFI_5GHZ_Verify_WGSSID_FromWlanClient</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that SSID name for 5GHZ band obtained through parameter (Device.WiFi.SSID.10101.SSID) matches with the SSID name obtained from Wireless Client.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_354</test_case_id>
    <test_objective>Verify that SSID name for 5GHZ band obtained through parameter (Device.WiFi.SSID.10101.SSID) matches with the SSID name obtained from Wireless Client.</test_objective>
...
2. Using tdkb_e2e_Get, get and save Radio enable and ssid enable status of 5GHz
3. Set  radio enable and ssid enable to true using tdkb_e2e_SetMultipleParams
4. From the GW, get its SSID using Device.WiFi.SSID.2.SSID
5. From the client get SSID of GW using getConnectedSsidName utility and compare with above value
6. Revert the values</automation_approch>
    <except_output>Both SSIDs should be the same</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_Verify_WGSSID_FromWlanClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_Verify_WGSSID_FromWlanClient');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    #Get the connected WG's SSID from client
                                    SSID1 = getConnectedSsidName(tdkbE2EUtility.wlan_5ghz_interface)

                                    ##Get the WG SSID using tr-181 param
                                    getSSID = "Device.WiFi.SSID.%s.SSID" %tdkbE2EUtility.ssid_5ghz_index

                                    tdkTestObj,status, SSID2 = getParameterValue(obj, getSSID)
                                    if expectedresult in status and SSID2:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("TEST STEP 8: Get the SSID of WG")
                                        print("EXPECTED RESULT 8: Should Get the SSID of WG")
                                        print("ACTUAL RESULT 8: %s" %SSID2);
                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                        if SSID1 == SSID2:
                                            print("ACTUAL RESULT 8: Both SSID values are same %s %s" %(SSID1, SSID2));
                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                            finalStatus = "SUCCESS"
                                        else:
                                            print("ACTUAL RESULT 8: Both SSID values are not same %s %s" %(SSID1, SSID2));
