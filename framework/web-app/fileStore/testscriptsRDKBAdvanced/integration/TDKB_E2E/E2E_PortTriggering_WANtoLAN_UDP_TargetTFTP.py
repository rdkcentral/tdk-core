...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_568</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</test_objective>
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP');
