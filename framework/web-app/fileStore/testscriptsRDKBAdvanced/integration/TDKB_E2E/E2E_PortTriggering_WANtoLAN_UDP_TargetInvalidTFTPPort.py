...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>E2E_PortTriggering_WANtoLAN_UDP_TargetInvalidTFTPPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 6969 is forwarded to the LAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_645</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 6969 is forwarded to the LAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
11. From WAN client, send an inbound packet to the target port 6969 via the DUT GW WAN IP. The packet should NOT be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 6969 is forwarded to the LAN machine (which is running a TFTP server at port 69) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_UDP_TargetInvalidTFTPPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetInvalidTFTPPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetInvalidTFTPPort');
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("EXPECTED RESULT %d: TFTP from WAN to the LAN should be failed with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
                                    status = tftpToClient(tdkbE2EUtility.gw_wan_ip, "LAN", tdkbE2EUtility.tftpfile, message, "WAN", invalidPort)
