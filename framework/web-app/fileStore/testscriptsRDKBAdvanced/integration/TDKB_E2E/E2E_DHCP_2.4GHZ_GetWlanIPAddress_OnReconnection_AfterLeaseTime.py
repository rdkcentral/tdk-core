...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_2.4GHZ_GetWlanIPAddress_OnReconnection_AfterLeaseTime</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that Wi-Fi client retains the same IP address when disconnected and reconnected when the Lease period expires</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_178</test_case_id>
    <test_objective>Verify that Wi-Fi client retains the same IP address when disconnected and reconnected when the Lease period expires</test_objective>
...
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Wi-Fi client should retain the same IP address when disconnected and reconnected when the Lease period is expired</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_2.4GHZ_GetWlanIPAddress_OnReconnection_AfterLeaseTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_2.4GHZ_GetWlanIPAddress_OnReconnection_AfterLeaseTime');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'10.0.0.1','255.255.255.0','10.0.0.2','10.0.0.253','120'];
                                        ...
                                        if wlanIP1:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("TEST STEP 12: Checking whether Wi-Fi Client retains the same IP address when disconnected and reconnected when the Lease period is expired")
                                                    if wlanIP == wlanIP1:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("Wi-Fi Client retains the same IP address when disconnected and reconnected when the Lease period is expired")
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("Wi-Fi Client did not retain the same IP address when disconnected and reconnected when the Lease period is expired")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
