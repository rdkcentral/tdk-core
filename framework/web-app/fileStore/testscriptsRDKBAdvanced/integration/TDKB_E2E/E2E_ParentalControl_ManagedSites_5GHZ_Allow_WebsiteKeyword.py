...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_ParentalControl_ManagedSites_5GHZ_Allow_WebsiteKeyword</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG allows other websites for Wi-Fi Client associated using 5GHZ radio except for the sites that has been blocked using keyword (google) through Parental control (managed sites)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_122</test_case_id>
    <test_objective>Verify that WG allows other websites for Wi-Fi Client associated using 5GHZ radio except for the sites that has been blocked using keyword (google) through Parental control (managed sites)</test_objective>
...
6.Check whether the WLAN client is able to access the allowed url keyword using wget
...
8.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_ParentalControl_ManagedSites_5GHZ_Allow_WebsiteKeyword</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedSites_5GHZ_Allow_WebsiteKeyword');
                                        ...
                                        if wlanIP:
                                            ...
                                            if expectedresult in status and curIPAddress:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    status = addStaticRoute(tdkbE2EUtility.allowed_url, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
                                                    if expectedresult in status:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                    ...
                                                    status = delStaticRoute(tdkbE2EUtility.allowed_url, curIPAddress,tdkbE2EUtility.wlan_5ghz_interface);
                                                    if expectedresult in status:
                                                        tdkTestObj.setResultStatus("SUCCESS");
