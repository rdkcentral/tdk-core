...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_562</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 5001 is forwarded to the WLAN machine (which is running an IPERF server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule using UDP protocol.</test_objective>
...
17. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetIPERF');
