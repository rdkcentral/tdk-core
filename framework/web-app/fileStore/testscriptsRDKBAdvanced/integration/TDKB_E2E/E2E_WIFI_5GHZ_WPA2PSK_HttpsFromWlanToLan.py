...
<xml>
  ...
  <name>E2E_WIFI_5GHZ_WPA2PSK_HttpsFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that https traffic between WLAN to LAN should be succesfull when wifi client assocation in wpa2-psk authentication for 5GHZ .</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_247</test_case_id>
    <test_objective>Verify that https traffic between WLAN to LAN should be succesfull when wifi client assocation in wpa2-psk authentication for 5GHZ .</test_objective>
...
5. From the wlan client, try to connect to the LAN client via https and check whether https connection is success when the security mode for 5GHz is set to WPA2PSK
6. Revert the security mode to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Https connection from WLAN client to LAN client should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_WPA2PSK_HttpsFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPA2PSK_HttpsFromWlanToLan');
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("TEST STEP 10:HTTS from WLAN to LAN failed")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
