...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCATWithUDPTrigger</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the PT rule doesn't take effect and inbound traffic is not let through, if the trigger is sent using incorrect protocol.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_570</test_case_id>
    <test_objective>To check if the PT rule doesn't take effect and inbound traffic is not let through, if the trigger is sent using incorrect protocol.</test_objective>
...
10. Send an outbound packet to trigger the port 12345 with a UDP packet which is not the trigger protocol configured in the rule.
11. From WAN client, send an inbound packet to the target port 54321 via the DUT GW WAN IP. The packet should NOT be successfully forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>PT rule should not take effect and inbound traffic should not be let through, if the trigger is sent using incorrect protocol.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCATWithUDPTrigger</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCATWithUDPTrigger');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCATWithUDPTrigger');
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    #Trigger the port
                                    incorrectTriggerProtocol = "UDP"
                                    step = step + 1
                                    print("\nTEST STEP %d: Trigger the Port %s with incorrect protocol %s  through a specified outbound packet from LAN client" %(step, triggerStart,incorrectTriggerProtocol))
                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                    status = triggerPort(tdkbE2EUtility.wan_ip, triggerStart, incorrectTriggerProtocol, "LAN")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("EXPECTED RESULT %d: NETCAT client running in WAN machine should be failure to the server running in LAN with the GW WAN IP of DUT" %step)
                                        status = PTClientRequest("NETCAT", tdkbE2EUtility.wan_ip, targetStart, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");

                                        if "FAILURE" in status:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("ACTUAL RESULT %d: NETCAT from WAN to the LAN is failure with the GW WAN IP of DUT" %step)
                                            print("[TEST EXECUTION RESULT] : SUCCESS");
