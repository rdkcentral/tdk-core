...
<xml>
  ...
  <name>E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV4Address</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the connectivity with invalid IPV4 address</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_486</test_case_id>
    <test_objective>To test the connectivity with invalid IPV4 address</test_objective>
...
10. Enter the invalid ip in IPV4 address column and check whether error popup is displayed
11. Kill selenium hub and node
12. unload module</automation_approch>
    <except_output>With invalid IPV4 address,error popup should be displayed</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV4Address</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV4Address');
                        ...
                        if expectedresult in status:
                            ...
                            if status == "SUCCESS":
                                try:
                                    ...
                                    #To give invalid IPV4 address as input
                                    driver.find_element_by_id("ipv4_address_1").send_keys("0")
                                    driver.find_element_by_id("ipv4_address_2").send_keys("0")
                                    driver.find_element_by_id("ipv4_address_3").send_keys("0")
                                    driver.find_element_by_id("ipv4_address_4").send_keys("0")
                                    # To click on "Check IPV4 address connectivity" button
                                    driver.find_element_by_xpath('//*[@id="check_ipv4"]').click()
                                    time.sleep(40);
                                    value = driver.find_element_by_xpath('//*[@id="connectivity_ipv4"]').text
                                    if value == "Error":
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("TEST STEP 6: Test the connectivity status of invalid ipv4 address")
                                        print("EXPECTED RESULT 6:With an invalid IPv4 address connectivity status is expected to throw Error popup")
                            ...
                            if status == "SUCCESS":
                                print("TEST STEP 7: Executing Post-requiste")
                                tdkTestObj.setResultStatus("SUCCESS");
