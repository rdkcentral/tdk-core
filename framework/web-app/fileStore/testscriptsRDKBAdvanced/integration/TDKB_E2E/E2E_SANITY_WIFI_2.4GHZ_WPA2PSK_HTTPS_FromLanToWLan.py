...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_SANITY_WIFI_2.4GHZ_WPA2PSK_HTTPS_FromLanToWLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that HTTPS traffic between Lan to WLAN should be succesfull when wifi client association in wpa2-psk authentication for 2.4GHZ</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Verify that HTTPS traffic between Lan to WLAN should be succesfull when wifi client association in wpa2-psk authentication for 2.4GHZ</test_objective>
...
4. send HTTPS request to wlan client from wlan client
4. Revert the values</automation_approch>
    <except_output>HTTPS request should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_SANITY_WIFI_2.4GHZ_WPA2PSK_HTTPS_FromLanToWLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_SANITY_WIFI_2.4GHZ_WPA2PSK_HTTPS_FromLanToWLan');
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if "SUCCESS" in status:
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("HTTPS traffic is successful between wired and wireless clients")
                                                finalStatus = "SUCCESS"
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("HTTPS traffic is not successful between wired and wireless clients")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
