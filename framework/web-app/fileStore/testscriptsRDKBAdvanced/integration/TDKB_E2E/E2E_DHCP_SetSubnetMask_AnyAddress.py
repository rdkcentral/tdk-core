...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_SetSubnetMask_AnyAddress</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that subnet mask of DHCPv4 server pool cannot be set with Any IP addresses (ex.0.0.0.0). Ensure that DHCP configuration on client is not affected</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_266</test_case_id>
    <test_objective>Verify that subnet mask of DHCPv4 server pool cannot be set with Any IP addresses (ex.0.0.0.0). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set Subnetmask to any address using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting subnetmask as any address</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetSubnetMask_AnyAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetSubnetMask_AnyAddress');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the LanIPAddress to any address"
                    subnetMask = "%s|0.0.0.0|string" %lanSubnetMask

                    tdkTestObj,status,details = setMultipleParameterValues(obj,subnetMask)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the SubnetMask to any address, 0.0.0.0")
                        ...
                        if expectedresult in retStatus and newSubnetMask != "0.0.0.0":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            if subnetMask and "0.0.0.0" not in subnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print(" subnet mask has not changed as any address")
                                finalStatus = "SUCCESS";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("subnet mask has changed as any address")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current subnetmask")
                            print("EXPECTED RESULT 5: Should retrieve the current subnetmask")
