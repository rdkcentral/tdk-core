...
<xml>
  ...
  <name>E2E_DHCP_SetSubnetMask_PublicAddress</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that subnet mask of DHCPv4 server pool cannot be set with Public IP addresses (ex.203.212.202.114). Ensure that DHCP configuration on client is not affected</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_265</test_case_id>
    <test_objective>Verify that subnet mask of DHCPv4 server pool cannot be set with Public IP addresses (ex.203.212.202.114). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set subnetmask to some public ip using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting subnetmask as public ip address</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetSubnetMask_PublicAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetSubnetMask_PublicAddress');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the subnetmask to some Public IP address"
                    subnetMask = "%s|203.212.202.114|string" %lanSubnetMask

                    tdkTestObj,status,details = setMultipleParameterValues(obj,subnetMask)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the SubnetMask to a public IP")
                        ...
                        if expectedresult in retStatus and newSubnetMask != "203.212.202.114":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            if subnetMask and "203.212.202.114" not in subnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print(" subnet mask has not changed as public ip value")
                                finalStatus = "SUCCESS";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("subnet mask has changed as public ip value")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current subnetmask")
                            print("EXPECTED RESULT 5: Should retrieve the current subnetmask")
