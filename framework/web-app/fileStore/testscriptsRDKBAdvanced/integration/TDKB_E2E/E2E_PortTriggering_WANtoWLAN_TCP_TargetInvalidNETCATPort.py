...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidNETCATPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 123 is forwarded to the WLAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_648</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 123 is forwarded to the WLAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
12. Send an outbound packet to trigger the port 12345 with a TCP packet
13. From WAN client, send an inbound packet to the target port 123 via the DUT GW WAN IP. The packet should NOT be forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 123 is forwarded to the WLAN machine (which is running a NETCAT server at port 54321) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidNETCATPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidNETCATPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidNETCATPort');
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in status and setValuesList == newValues:
                        ...
                        if expectedresult in status:
                            ...
                            print("EXPECTED RESULT %d: The current WLAN IP address of the connected client should be obatined" %step)
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("EXPECTED RESULT %d: NETCAT client runing in WAN machine should NOT connect to the server running in WLAN with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
