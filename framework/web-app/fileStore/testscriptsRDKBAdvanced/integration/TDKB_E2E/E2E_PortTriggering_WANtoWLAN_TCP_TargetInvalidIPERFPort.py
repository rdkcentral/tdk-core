...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidIPERFPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 5050 is forwarded to the WLAN machine (which is running an IPERF server at port 5001) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_647</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 5050 is forwarded to the WLAN machine (which is running an IPERF server at port 5001) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
4. Set current SSID, KeyPassphrase to new files as per the config file and radio enable to true.
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
12. Send an outbound packet to trigger the port 12345 with TCP packet
13. From WAN client, send an inbound packet to the target port 5050 via the DUT GW WAN IP. The packet should NOT be forwarded to the WLAN client.
14. Delete the added route
15. Delete the added port triggering rule
16. Revert SSID, Keypassphrase and radio enable to initial values
17. Revert Device.NAT.X_CISCO_COM_PortTriggers.Enable if required
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 5050 is forwarded to the WLAN machine (which is running an IPERF server at port 5001) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidIPERFPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidIPERFPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidIPERFPort');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 5001:5001 (for IPERF), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12345"
                                        triggerEnd = "12345"
                                        targetStart = "5001"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                #Start the IPERF server in WLAN
                                                step = step + 1
                                                print("\nTEST STEP %d: Start the IPERF server in WLAN client at port %s" %(step, targetStart))
                                                print("EXPECTED RESULT %d: IPERF server should be started successfully in WLAN client" %step)
                                                status = PTinitServer("IPERF", wlanIP, targetStart, targetProtocol, tdkbE2EUtility.server_logfile, source="WLAN")
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("EXPECTED RESULT %d: IPERF client runing in WAN machine should NOT connect to the server running in WLAN with the GW WAN IP of DUT with invalid target port %s" %(step, invalidPort))
                                                        status = PTClientRequest("IPERF", tdkbE2EUtility.wan_ip, invalidPort, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");
                                                        ...
                                                        if "bits/sec" not in serverStatus:
                                                            ...
                                                            print("ACTUAL RESULT %d: IPERF from WAN to the WLAN is success with the GW WAN IP of DUT" %step)
                                                    ...
                                                    status = PTServerClientPostRequisite("IPERF", "WLAN", "WAN")
