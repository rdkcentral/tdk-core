...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetFTP_TriggerOutOfRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 21 is not forwarded to the WLAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_617</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 21 is not forwarded to the WLAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
13. From WAN client, send an inbound packet to the target port 21 via the DUT GW WAN IP. The packet should not be forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 21 is not forwarded to the WLAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetFTP_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetFTP_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetFTP_TriggerOutOfRange');
                            ...
                            if wlanIP != "":
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                                        step = step + 1
                                        triggerStart = "12340"
                                        triggerEnd = "12346"
                                        targetStart = "21"
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("EXPECTED RESULT %d: FTP from WAN to the WLAN should fail with the GW WAN IP of DUT" %step)
                                                    ...
                                                    if expectedresult not in status:
                                                        ...
                                                        print("ACTUAL RESULT %d: FTP from WAN to the WLAN success with the GW WAN IP of DUT" %step)
                                                        print("[TEST EXECUTION RESULT] : FAILURE");
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("ACTUAL RESULT %d: Port trigger failed" %step)
                                                    print("[TEST EXECUTION RESULT] : FAILURE");
