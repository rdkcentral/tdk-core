...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_OpenSecurity_Low_TCPFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association  in open security wlan on 2.4ghz band with firewall set to Low.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_416</test_case_id>
    <test_objective>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association  in open security wlan on 2.4ghz band with firewall set to Low.</test_objective>
...
3. Set the security mode for 2.4GHz to None and firewall to Low using tdkb_e2e_SetMultipleParams
...
8.Unload tdkb_e2e module</automation_approch>
    <except_output>TCP should be success between wired and wireless clients when firewall is Low and in Opensecurity mode</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_OpenSecurity_Low_TCPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_OpenSecurity_Low_TCPFromWlanToLan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,'None','Low'];
