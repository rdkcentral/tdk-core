...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_DHCP_SetLanIPAddress_PublicAddress</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that Gateway IP in DHCPv4 server pool cannot be set with Public IP addresses. (ex.203.212.202.114). Ensure that DHCP configuration on client is not affected</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_254</test_case_id>
    <test_objective>Verify that Gateway IP in DHCPv4 server pool cannot be set with Public IP addresses. (ex.203.212.202.114). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set LanIPAddress to public address using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting lanIPAddress as public address</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetLanIPAddress_PublicAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetLanIPAddress_PublicAddress');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the LanIPAddress to some Public IP address"
                    lanIPValue = "%s|203.212.202.114|string" %lanIPAddress

                    tdkTestObj,status,details = setMultipleParameterValues(obj,lanIPValue)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the lanIPAddress to a public IP, 203.212.202.114")
                        ...
                        if expectedresult in retStatus and newlanIPValue != "203.212.202.114":
                            tdkTestObj.setResultStatus("SUCCESS");
