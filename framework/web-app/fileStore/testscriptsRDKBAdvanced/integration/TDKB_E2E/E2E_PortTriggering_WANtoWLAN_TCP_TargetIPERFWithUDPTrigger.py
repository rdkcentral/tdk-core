...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERFWithUDPTrigger</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_575</test_case_id>
...
12. Send an outbound packet to trigger the port 12345 with a UDP packet which is not the trigger protocol configured in the rule
...
18. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERFWithUDPTrigger</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERFWithUDPTrigger');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERFWithUDPTrigger');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    step = step + 1
                                                    print("\nTEST STEP %d: Trigger the Port %s with incorrect protocol %s through a specified outbound packet from WLAN client" %(step, triggerStart, incorrectTriggerProtocol))
                                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                                    status = triggerPort(tdkbE2EUtility.wan_ip, triggerStart, incorrectTriggerProtocol, "WLAN")
