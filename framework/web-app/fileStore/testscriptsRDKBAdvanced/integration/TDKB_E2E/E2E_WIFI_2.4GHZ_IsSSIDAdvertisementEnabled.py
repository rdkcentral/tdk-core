...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_2.4GHZ_IsSSIDAdvertisementEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether Network name broadcast (SSID advertisement) is enabled by default in WG</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_32</test_case_id>
    <test_objective>Verify whether Network name broadcast (SSID advertisement) is enabled by default in WG</test_objective>
...
3. Set SSIDAdvertisementEnabled to True using tdkb_e2e_SetMultipleParams
3. Try to connect to Wifi client and check if the ssid is listing properly
4. Revert the values of SSIDAdvertisementEnabled</automation_approch>
    <except_output>The ssid name should be listed with SSIDAdvertisementEnabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_IsSSIDAdvertisementEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_IsSSIDAdvertisementEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status and "true" in orgValue[2]:
            ...
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 5: Should set the original ssid and keypassphrase");
