...
<xml>
  <id/>
  <version>4</version>
  <name>E2E_WIFI_MACFilter_Block_2.4GHZ_WIFIClient</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG is able to deny Access for specific Wi-Fi clients of 2.4GHZ SSID that are configured in MAC address filter</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_43</test_case_id>
    <test_objective>Verify that WG is able to deny Access for specific Wi-Fi clients that are configured in MAC address filter</test_objective>
...
3. Enable MACFilter.Enable and MACFilter.FilterAsBlackList
4.  Connect to the WIFI client and get the MAC address of the WLAN  interface
5. Disconnect the WiFi client
6.Add new rule to Device.WiFi.AccessPoint.%s.X_CISCO_COM_MacFilterTable.
7. Set the devicename and mac details of client to be blocked in the MAC filter rule
8.Try to connect the blocked client to the GW SSID, that connection attempt should fail
9.Delete the added rule and revert all params values
10.Unload tdkb_e2e module</automation_approch>
    <except_output>After blocking a WiFi client device, any connection attempt from that device to the GW should fail</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_MACFilter_Block_2.4GHZ_WIFIClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_MACFilter_Block_2.4GHZ_WIFIClient');
obj1.configureTestCase(ip,port,'E2E_WIFI_MACFilter_Block_2.4GHZ_WIFIClient');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        macFilterEnable = "Device.WiFi.AccessPoint.%s.X_CISCO_COM_MACFilter.Enable" %tdkbE2EUtility.ssid_2ghz_index
        macFilterBlackList = "Device.WiFi.AccessPoint.%s.X_CISCO_COM_MACFilter.FilterAsBlackList" %tdkbE2EUtility.ssid_2ghz_index
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'true','true','true'];
            ...
            list5 = [macFilterBlackList,'true','bool']
            ...
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 2: Set the ssid,keypassphrase,Radio enable status,macFilterEnable, macFilterBlackList")
                print("EXPECTED RESULT 2: Should set the ssid,keypassphrase,Radio enable status,macFilterEnable, macFilterBlackList")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if MAC:
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("TEST STEP 8: Adding new Mac filter rule");
                                                ...
                                                if (int(instance) > 0):
                                                    ...
                                                    deviceMAC = "Device.WiFi.AccessPoint.%s.X_CISCO_COM_MacFilterTable.%s.MACAddress" %(tdkbE2EUtility.ssid_2ghz_index, instance)

                                                    setValuesList = [tdkbE2EUtility.ssid_2ghz_name,MAC]
                                                    print("Parameter values that are set: %s" %setValuesList)
                                                    list1 = [deviceName,tdkbE2EUtility.ssid_2ghz_name,'string']
                                                    ...
                                                    if expectedresult in actualresult:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("TEST STEP 9: Set the name and MAC of client device in the filter")
                                                        print("EXPECTED RESULT 9: Should set the name and MAC of client device in the filter")
                                                        ...
                                                        if expectedresult in status and setValuesList == newValues:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("TEST STEP 10: Get the current value of name and MAC of client device in the filter")
                                                            print("EXPECTED RESULT 10: Should retrieve the current name and mac of client device in the filter")
                                                            print("ACTUAL RESULT 10: %s" %newValues);
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");

                                                            #Connect to the wifi ssid from wlan client after enabling MAC filtering
                                                            print("TEST STEP 11: From wlan client, try to Connect to the wifi ssid with mac filtering enabled")
                                                            status = wlanConnectWifiSsid(tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,tdkbE2EUtility.wlan_2ghz_interface);
                                                            if expectedresult not in status:
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                finalStatus = "SUCCESS";
                                                                print("EXPECTED RESULT 11: Connection attempt should fail after enabling the MAC filter")
                                                                print("ACTUAL RESULT 11: Connection from client failed after enabling the MAC filter")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE");
                                                                print("EXPECTED RESULT 11: Connection attempt should fail after enabling the MAC filter")
                                                                print("ACTUAL RESULT 11: Connection from client is success even after enabling the MAC filter")
                                                ...
                                                print("TEST STEP 8: Adding new Mac filter rule");
                                                print("EXPECTED RESULT 8: Should add new rule");
                                                print("ACTUAL RESULT 8: adding new rule failed%s" %details);
                                                print("[TEST EXECUTION RESULT] : FAILURE");
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 13: Set the ssid,keypassphrase,Radio enable status,macFilterEnable, macFilterBlackList")
                print("EXPECTED RESULT 13: Should set the ssid,keypassphrase,Radio enable status,macFilterEnable, macFilterBlackList")
