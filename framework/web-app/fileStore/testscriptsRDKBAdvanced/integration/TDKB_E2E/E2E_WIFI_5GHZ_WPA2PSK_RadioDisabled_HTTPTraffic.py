...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_WPA2PSK_RadioDisabled_HTTPTraffic</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that http traffic between WLan to LAN should be denied when wifi client assocation in wpa2-psk authentication for 5GHZ and Radio interface parameter for 5GHZ [Device.WiFi.Radio.2.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_368</test_case_id>
    <test_objective>Verify that http traffic between WLan to LAN should be denied when wifi client assocation in wpa2-psk authentication for 5GHZ and Radio interface parameter for 5GHZ [Device.WiFi.Radio.2.Enable] is disabled.</test_objective>
...
4. Disable radio and send http request to lan client from wifi client
4. Revert the values</automation_approch>
    <except_output>HTTP request should fail since radio is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_WPA2PSK_RadioDisabled_HTTPTraffic</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPA2PSK_RadioDisabled_HTTPTraffic');
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status and curIPAddress:
                                            ...
                                            print("TEST STEP 10: Check whether lan ip address is in same DHCP range")
                                            status = "SUCCESS"
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in actualresult:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("TEST STEP 11: Set RadioEnable")
                                                    print("EXPECTED RESULT 11: Should set the RadioEnable");
                                                    ...
                                                    if expectedresult in status and setValuesList == newValues:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("TEST STEP 12: Get the current radioEnable")
                                                        print("EXPECTED RESULT 12: Should retrieve the current radioEnable")
                                                        ...
                                                        print("TEST STEP 13: Connect to LAN Client and send HTTP Request to WLAN Client")
