...
<xml>
  ...
  <name>E2E_BridgeMode_CheckClientDisabled_OnReboot</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the Connected clients are disabled when the gateway is in bridge mode and if this behavior persists on reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_481</test_case_id>
    <test_objective>Check if the Connected clients are disabled when the gateway is in bridge mode and if this behavior persists on reboot</test_objective>
...
9. Get the lan ip and check if it is not within the DHCP range
10. Check if Device.Hosts.HostNumberOfEntries and
Device.Hosts.X_CISCO_COM_ConnectedDeviceNumber are both still 0
11. Revert the Lanmode to its initial value
12. Unload tdkb_e2e module</automation_approch>
    <except_output>When gateway is in bridge mode, connected client should be disabled and this behavior should be persisted on reboot</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_BridgeMode_CheckClientDisabled_OnReboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_BridgeMode_CheckClientDisabled_OnReboot');
obj1.configureTestCase(ip,port,'E2E_BridgeMode_CheckClientDisabled_OnReboot');
                    ...
                    if lanIP:
                        ...
                        if expectedresult not in status:
                            ...
                            if expectedresult in status and valueList == ['0','0']:
                                ...
                                obj1.initiateReboot();
                                sleep(300);
                                print("TEST STEP : Get the current LAN IP address DHCP range")
                                param = "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress"
                                tdkTestObj,status,curIPAddress = getParameterValue(obj,param)
                                print("LAN IP Address DHCP range: %s" %curIPAddress);
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult not in status:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("Lan client IP change persists on reboot")
                                            ...
                                            if expectedresult in status and valueList == ['0','0']:
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("SUCCESS: Client is disabled after reboot also")
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("FAILURE: Client is not in disabled state after reboot")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("FAILURE: Lan client IP change not persistant on reboot")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("SUCCESS: Failed to get lan ip after reboot")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("Failed to get DHCP IP range after reboot")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
