...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerOutOfRange</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is not forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_626</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is not forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</test_objective>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
...
11. From WAN client, send an inbound packet to the target port 69 via the DUT GW WAN IP. The packet should not be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 69 is not forwarded to the LAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_UDP_TargetTFTP_TriggerOutOfRange');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        print("[TEST EXECUTION RESULT] : SUCCESS");
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 69:69 (for TFTP), Trigger Protocol - UDP, Target Protocol - UDP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "12340"
                        triggerEnd = "12346"
                        targetStart = "69"
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("EXPECTED RESULT %d: TFTP from WAN to the LAN should fail with the GW WAN IP of DUT" %step)
