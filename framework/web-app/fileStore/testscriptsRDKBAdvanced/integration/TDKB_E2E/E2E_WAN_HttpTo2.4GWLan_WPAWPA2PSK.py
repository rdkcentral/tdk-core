...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WAN_HttpTo2.4GWLan_WPAWPA2PSK</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that http connection from WAN to 2.4GHZ WLAN client with WPA-WPA2-PSK security mode is not success</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_546</test_case_id>
    <test_objective>Verify that http connection from WAN to 2.4GHZ WLAN client with WPA-WPA2-PSK security mode is not success</test_objective>
...
5. From WAN client check if HTTP connection to 2.4G WLAN client ip is success or not with WPA-WPA2-PSK security mode
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WAN_HttpTo2.4GWLan_WPAWPA2PSK</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WAN_HttpTo2.4GWLan_WPAWPA2PSK');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 10:Check the HTTP connection from WAN to 2.4GHZ WLAN client ip")
                                    expectedresult = "FAILURE"
                                    status = verifyNetworkConnectivity(wlanIP, "WGET_HTTP", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                    if expectedresult in status:
                                        print("SUCCESS : HTTP connection not successful from WAN to 2.4GHZ WLAN client ip")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("FAILURE : HTTP connection successful from WAN to 2.4GHZ WLAN client ip")
