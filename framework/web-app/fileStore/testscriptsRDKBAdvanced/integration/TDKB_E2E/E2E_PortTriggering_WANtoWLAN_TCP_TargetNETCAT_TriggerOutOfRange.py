...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCAT_TriggerOutOfRange</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 54321 is not forwarded to the WLAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_621</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that inbound packet with target port 54321 is not forwarded to the WLAN machine (which is running an NETCAT server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
12. Send an outbound packet to trigger the out of range port 12350 with a TCP packet
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 54321 is not forwarded to the WLAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCAT_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCAT_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCAT_TriggerOutOfRange');
