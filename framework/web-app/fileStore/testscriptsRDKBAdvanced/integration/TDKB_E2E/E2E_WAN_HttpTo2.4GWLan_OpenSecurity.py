...
<xml>
  ...
  <name>E2E_WAN_HttpTo2.4GWLan_OpenSecurity</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that http connection from WAN to 2.4GHZ WLAN client with open security mode is not success</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_553</test_case_id>
    <test_objective>Verify that http connection from WAN to 2.4GHZ WLAN client with open security mode is not success</test_objective>
...
5. From WAN client check if HTTP connection to 2.4G WLAN client ip is success or not with open security mode
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WAN_HttpTo2.4GWLan_OpenSecurity</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WAN_HttpTo2.4GWLan_OpenSecurity');
