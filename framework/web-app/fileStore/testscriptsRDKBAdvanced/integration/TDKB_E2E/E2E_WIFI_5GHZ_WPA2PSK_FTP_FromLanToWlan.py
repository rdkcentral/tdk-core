...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_WPA2PSK_FTP_FromLanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpa2-psk security wlan on 5ghz band.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_143</test_case_id>
    <test_objective>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpa2-psk security wlan on 5ghz band.</test_objective>
...
3. Set the security mode for 5GHz to WPA2PSK using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, try to connect to the LAN client via ftp and check whether ftp connection is success and file transfer is done when the security mode for 5GHz is set to WPA2PSK
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WIFI_5GHZ_WPA2PSK_FTP_FromLanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPA2PSK_FTP_FromLanToWlan');
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            #Send FTP request and file transfer from LAN client to WLAN client
                                            ...
                                            if expectedresult in status:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("TEST STEP 10:FTP from LAN to WLAN failed")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
