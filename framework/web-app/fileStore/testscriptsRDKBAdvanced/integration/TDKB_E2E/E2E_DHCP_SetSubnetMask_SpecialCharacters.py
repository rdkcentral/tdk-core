...
<xml>
  ...
  <name>E2E_DHCP_SetSubnetMask_SpecialCharacters</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that subnet mask of DHCPv4 server pool cannot be set with Special characters. (ex.!@#$%). Ensure that DHCP configuration on client is not affected</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_271</test_case_id>
    <test_objective>Verify that subnet mask of DHCPv4 server pool cannot be set with Special characters. (ex.!@#$%). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set subnetmask as some special characters using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting subnetmask as special characters</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetSubnetMask_SpecialCharacters</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetSubnetMask_SpecialCharacters');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the Subnetmask to some special character"
                    subnetMask = "%s|!@*&!|string" %lanSubnetMask

                    tdkTestObj,status,details = setMultipleParameterValues(obj,subnetMask)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the SubnetMask to some special characters")
                        ...
                        if expectedresult in retStatus and newSubnetMask != "!@*&!":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            if subnetMask and "!@*&!" not in subnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print(" subnet mask has not changed as special characters")
                                finalStatus = "SUCCESS";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("subnet mask has changed as special character")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current subnetmask")
                            print("EXPECTED RESULT 5: Should retrieve the current subnetmask")
