...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_WPAWPA2PSK_HttpToRouterPrivateIP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that http connection from 2.4G WLAN client to router's private ip is success with WPA-WPA2-PSK security mode</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_548</test_case_id>
    <test_objective>Verify that http connection from 2.4G WLAN client to router's private ip is success with WPA-WPA2-PSK security mode</test_objective>
...
3.Connect the WiFi client to the 2.4SSID  of gateway with WPA-WPA2-PSK security mode
...
7.Unload tdkb_e2e module</automation_approch>
    <expected_output>Http connection from 2.4 WLAN client to router's private ip is success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_WPAWPA2PSK_HttpToRouterPrivateIP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPAWPA2PSK_HttpToRouterPrivateIP');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 10:Check the HTTP connection from 2.4 WLAN to router's private ip")
                                    status = verifyNetworkConnectivity(curIPAddress, "WGET_HTTP", wlanIP, curIPAddress)
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("verifyNetworkConnectivity: SUCCESS")
                                        print("HTTP connection successful from 2.4 WLAN to router's private ip")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("HTTP connection not successful from 2.4 WLAN to router's private ip")
