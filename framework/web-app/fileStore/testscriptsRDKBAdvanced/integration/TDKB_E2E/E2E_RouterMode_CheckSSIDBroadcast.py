...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_RouterMode_CheckSSIDBroadcast</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling bridge mode both Private SSIDs on 2.4 and 5 GHz should be broadcasted</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_190</test_case_id>
    <test_objective>Verify that on disabling bridge mode both Private SSIDs on 2.4 and 5 GHz should be broadcasted</test_objective>
...
4. Set lanMode as router
5.  Connect to the WIFI client and check whether the ssid is getting listed
6.Unload tdkb_e2e module</automation_approch>
    <except_output>The SSID should get listed when lanMode is router</except_output>
    <priority>High</priority>
    <test_stub_interface>None</test_stub_interface>
    <test_script>E2E_RouterMode_CheckSSIDBroadcast</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_RouterMode_CheckSSIDBroadcast');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_5ghz_name,'router'];
            ...
            if expectedresult in actualresult and expectedresult in actualresult1:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if expectedresult in status_2ghz and expectedresult in status_5ghz:
                        tdkTestObj.setResultStatus("SUCCESS");
