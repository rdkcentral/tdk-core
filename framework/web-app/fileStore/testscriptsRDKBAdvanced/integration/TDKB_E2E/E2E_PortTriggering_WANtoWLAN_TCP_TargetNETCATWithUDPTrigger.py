...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCATWithUDPTrigger</name>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_571</test_case_id>
...
13. From WAN client, send an inbound packet to the target port 54321 via the DUT GW WAN IP. The packet should NOT be successfully forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCATWithUDPTrigger</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCATWithUDPTrigger');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetNETCATWithUDPTrigger');
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        print("EXPECTED RESULT %d: NETCAT client running in WAN machine should be failure to the server running in WLAN with the GW WAN IP of DUT" %step)
                                                        status = PTClientRequest("NETCAT", tdkbE2EUtility.wan_ip, targetStart, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");

                                                        if "FAILURE" in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("ACTUAL RESULT %d: NETCAT from WAN to the WLAN is failure with the GW WAN IP of DUT" %step)
                                                            print("[TEST EXECUTION RESULT] : SUCCESS");
                                                        else:
