...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WLANtoLAN_Telnet_WPA3Personal</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that telnet traffic between WLan to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6 GHz band.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_667</test_case_id>
    <test_objective>Verify that telnet traffic between WLan to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6GHz band.</test_objective>
...
3. Set security mode to WPA3-Personal if security mode is not WPA3-Personal mode,radio enable and ssid enable to true using tdkb_e2e_SetMultipleParams
4. Try to connect to 6GHZ from WIFI client
5. send telnet request to wlan client to Lan client.
6. Revert the values.
7. Unload the module</automation_approch>
    <expected_output>Telnet request from wlan to lan client should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WLANtoLAN_Telnet_WPA3Personal</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WLANtoLAN_Telnet_WPA3Personal')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WLANtoLAN_Telnet_WPA3Personal')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            if exit_flag != 1:
                step += 1
                # Set ssid,keypassphrase, radioEnable and ssidEnable for 2.4ghz"
                ...
                if expectedresult in actualresult:
                    ...
                    print(f"TEST STEP {step}: Get the current ssid,keypassphrase,radioEnable and ssidEnable after setting")
                    print(f"EXPECTED RESULT {step}: Should retrieve the current ssid,keypassphrase,radioEnable and ssidEnable and match with set values")
                                ...
                                if expectedresult in status and curIPAddress:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if lanIP:
                                            ...
                                            step += 1
                                            print(f"TEST STEP {step}: Check whether lan ip is in the same DHCP range")
                ...
                step += 1
                print(f"TEST STEP {step}: Revert to original WiFi parameter values")
                print(f"EXPECTED RESULT {step}: Should successfully revert to original ssid,keypassphrase,radioEnable and ssidEnable")
