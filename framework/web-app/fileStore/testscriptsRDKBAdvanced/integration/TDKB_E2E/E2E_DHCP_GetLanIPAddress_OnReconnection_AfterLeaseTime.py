...
<xml>
  ...
  <name>E2E_DHCP_GetLanIPAddress_OnReconnection_AfterLeaseTime</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that DHCPv4 client retains the same IP address when disconnected and reconnected when the Lease period is expires</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_277</test_case_id>
    <test_objective>Verify that DHCPv4 client retains the same IP address when disconnected and reconnected when the Lease period is expires</test_objective>
...
5. Disconnect the LAN client and wait till the lease time expires and then reconnect the lan client
...
7.Unload tdkb_e2e module</automation_approch>
    <except_output>LAN client should retain the same IP address when disconnected and reconnected when the Lease period is expired</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_GetLanIPAddress_OnReconnection_AfterLeaseTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_GetLanIPAddress_OnReconnection_AfterLeaseTime');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if lanIP:
                        ...
                        if expectedresult in status:
                            ...
                            print("TEST STEP 6: Get the IP address of the lan client after lease time expires")
                            ...
                            if lanIP1:
                                ...
                                status = "SUCCESS"
                                status = checkIpRange(LanIP,lanIP1);
                                if expectedresult in status:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("checkIpRange: SUCCESS")

                                    print("TEST STEP 8: Checking whether LAN Client retains the same IP address when disconnected and reconnected when the Lease period is expired")
                                    if lanIP == lanIP1:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("LAN Client retains the same IP address when disconnected and reconnected when the Lease period is expired")
                                        finalStatus = "SUCCESS";
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("LAN Client did not retain the same IP address when disconnected and reconnected when the Lease period is expired")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
            ...
            if expectedresult in actualresult and expectedresult in actualresult1 and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 9: Should set the original lanIPAddress,lanSubnetMask,minAddress,maxAddress and leaseTime");
