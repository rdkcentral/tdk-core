...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF_WithInvalidPort</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the inbound traffic is not let through when the actual trigger is given via a different port other than in PT rule.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_635</test_case_id>
    <test_objective>To check if the inbound traffic is not let through when the actual trigger is given via a different port other than in PT rule.</test_objective>
...
12. Send an outbound packet to trigger the port 12343 which is different from the port in PT rule with a TCP packet
13. From WAN client, send an inbound packet to the target port 5001 via the DUT GW WAN IP. The packet should NOT be successfully forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF_WithInvalidPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF_WithInvalidPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetIPERF_WithInvalidPort');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    #Trigger with a invalid port
                                                    invalidPort = "12343"
                                                    step = step + 1
                                                    print("\nTEST STEP %d: Trigger with invalid Port %s through a specified outbound packet from WLAN client" %(step, invalidPort))
                                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                                    status = triggerPort(tdkbE2EUtility.wan_ip, invalidPort, triggerProtocol, "WLAN")
                                                    ...
                                                    if expectedresult in status:
                                                        ...
                                                        step = step + 1
                                                        print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the WLAN client that triggered with invalid port" %step)
                                                        print("EXPECTED RESULT %d: IPERF client running in WAN machine should be failed to the server running in WLAN with the GW WAN IP of DUT" %step)
