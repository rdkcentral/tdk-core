...
<xml>
  <id/>
  <version>4</version>
  <name>E2E_WEBUI_WLAN_5GHZ_GetNoOfClientsConnected</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the count of clients connected from the UI page, after WLAN client of 5GHz is connected and verify it.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_477</test_case_id>
    <test_objective>Check if the number of clients connected to WLAN using 5GHZ is successfully updated .</test_objective>
...
11.Check if the number of clients connected to 5GHZ WiFi network is updated or not.
12. Kill selenium hub and node
13.  unload module</automation_approch>
    <except_output>Number of clients connected to 5GHZ WiFi network should to updated successfully.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WEBUI_WLAN_5GHZ_GetNoOfClientsConnected</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_WLAN_5GHZ_GetNoOfClientsConnected');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        clientCount = "Device.WiFi.AccessPoint.%s.AssociatedDeviceNumberOfEntries" %tdkbE2EUtility.ssid_5ghz_index

        #Get the value of the wifi parameters that are currently set.
        paramList=[ssidName,keyPassPhrase,clientCount]
        tdkTestObj,status,orgValue = getMultipleParameterValues(obj,paramList)

        if expectedresult in status:
            tdkTestObj.setResultStatus("SUCCESS");
            print("TEST STEP 1: Get the current ssid and keypassphrase and clients connected count")
            print("EXPECTED RESULT 1: Should retrieve the current ssid and keypassphrase and  clients connected count")
                    ...
                    if status == "SUCCESS":
                        ...
                        if expectedresult in status:
                            ...
                            if wlanIP:
                                ...
                                print("TEST STEP 6: Get the current WLAN IP address DHCP range")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if status == "SUCCESS":
                                            try:
                                                ...
                                                clientCount = driver.find_element_by_xpath("/html/body/div[1]/div[3]/div[3]/div[3]/div[2]/div[4]/span[2]").text
                                                if int(clientCount) == int(orgValue[2])+1 :
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("number of clients connected : ", clientCount);
                                                    finalStatus = "SUCCESS"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("Failed to get the number of clients connected")
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 8: Should set the original ssid and keypassphrase");
