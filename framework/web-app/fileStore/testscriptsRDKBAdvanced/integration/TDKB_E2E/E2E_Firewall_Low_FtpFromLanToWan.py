...
<xml>
  ...
  <name>E2E_Firewall_Low_FtpFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Low FTP access from LAN to WAN should not be blocked</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_293</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Low FTP access from LAN to WAN should not be blocked</test_objective>
...
5. From the lan client, do FTP to the WAN client and check whether ftp is success when the firewall level is set to Low
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_Firewall_Low_FtpFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Low_FtpFromLanToWan');
