...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_OpenSecuritymode</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether WG supports Open Security mode for 5GHZ frequency band.Ensure that client should be able to connect to WG</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_40</test_case_id>
    <test_objective>Verify whether WG supports Open Security mode for 5GHZ frequency band.Ensure that client should be able to connect to WG</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband,Emulator,RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Device.WiFi.AccessPoint.2.Security.ModeEnabled : None</input_parameters>
    <automation_approch>1. Load tdkb_e2e module
2. Using tdkb_e2e_Get, get and save security mode for 5GHz
3. Set Security mode to None using tdkb_e2e_SetMultipleParams
3. Try to connect to 5GHZ from WIFI client and  check whether the security mode there is changed to None
4. Revert the values of Radio enable</automation_approch>
    <except_output>The security mode there should be returned as None</except_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_OpenSecuritymode');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 8: Check whether wlan client connected without any security mode")
                                    security_mode = getSecurityMode(tdkbE2EUtility.ssid_5ghz_name);
                                    if "Open" in security_mode:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        ...
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("getSecurityMode: Failed to get the security mode")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                ...
                print("TEST STEP 2: Set the ssid, and securityMode")
                print("EXPECTED RESULT 2: Should set the ssid,nd securityMode");
                print("ACTUAL RESULT 2: %s" %details);
                print("[TEST EXECUTION RESULT] : FAILURE");
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 5: Should set the original ssid,and securityMode");
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print("TEST STEP 1: Get the current ssid, and securityMode")
            print("EXPECTED RESULT 1: Should retrieve the current ssid, securityMode")
            print("ACTUAL RESULT 1: %s" %orgValue);
            print("[TEST EXECUTION RESULT] : FAILURE");
    else:
