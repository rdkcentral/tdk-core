...
<xml>
  ...
  <name>E2E_PF_FirewallMedium_FTPFromWanToWlan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify Port Forwarding with FTP service running on Wi-Fi Client and Firewall mode set to Medium FTP access should be successful from WAN to WLAN</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_84</test_case_id>
    <test_objective>Verify Port Forwarding with FTP service running on Wi-Fi Client and Firewall mode set to Medium FTP access should be successful from WAN to WLAN</test_objective>
...
13.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>FTP connection from WAN client to WAN ip of gateway, after enabling port forwarding should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_PF_FirewallMedium_FTPFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PF_FirewallMedium_FTPFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_PF_FirewallMedium_FTPFromWanToWlan');
                                            ...
                                            #Set the result status of execution
                                                ...
                                                if (instance1 > 0):
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        print("TEST STEP 11:Check the FTP connectivity from WAN client to WAN ip of GW ")
                                                        time.sleep(60);
                                                        status = ftpToClient("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN");
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            finalStatus = "SUCCESS";
                                                            print("FTP from WAN client to WAN ip of GW: SUCCESS")

                                                        else:
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("TEST STEP 11:FTP from WAN client to WAN ip of GW failed")
