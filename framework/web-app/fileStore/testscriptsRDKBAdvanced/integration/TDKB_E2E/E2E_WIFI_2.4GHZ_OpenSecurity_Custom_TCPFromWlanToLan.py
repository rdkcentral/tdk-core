...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_2.4GHZ_OpenSecurity_Custom_TCPFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client where client association in open security wlan on 2.4ghz band with firewall set to Custom.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_404</test_case_id>
    <test_objective>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client where client association in open security wlan on 2.4ghz band with firewall set to Custom.</test_objective>
...
3. Set the security mode for 2.4GHz to None and firewall to Custom using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, do TCP to the LAN client and check whether  TCP is success when the security mode for 2.4GHz is set to None
6.Validate the bandwidth obtained from TCP server and client
7. Revert the security mode to original value
8.Unload tdkb_e2e module</automation_approch>
    <except_output>TCP should be success between wired and wireless clients when firewall is Custom and in Opensecurity mode</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_OpenSecurity_Custom_TCPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_OpenSecurity_Custom_TCPFromWlanToLan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            print("TEST STEP 1: Get the current ssid,firewall and securityMode")
            print("EXPECTED RESULT 1: Should retrieve the current ssid,firewall and securityMode")
            print("ACTUAL RESULT 1: %s" %orgValue);
            print("[TEST EXECUTION RESULT] : SUCCESS");

            # Set securityMode for 2ghz"
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,'None','Custom'];
            ...
            firewallParam = "%s|Custom|string" %firewallLevel
            ...
            if expectedresult in actualresult and expectedresult in firewallResult:
                tdkTestObj.setResultStatus("SUCCESS");
                print("TEST STEP 2: Set the ssid,firewall and securityMode")
                print("EXPECTED RESULT 2: Should set the ssid,firewall and securityMode");
                ...
                if expectedresult in status and setValuesList == newValues:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print("TEST STEP 3: Get the current ssid,firewall and securityMode")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,firewall and securityMode")
            ...
            if expectedresult in actualresult and expectedresult in firewallResult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 15: Should set the original ssid,firewall and securityMode");
