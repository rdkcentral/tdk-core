...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_WPAWPA2PSK_PingRouterPublicIP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ping from 5G WLAN client to router's public ip is success with security mode WPA-WPA2-PSK</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_545</test_case_id>
    <test_objective>Verify that ping from 5G WLAN client to router's public ip is success with security mode WPA-WPA2-PSK</test_objective>
...
3.Connect the WiFi client to the 5GSSID  of gateway
4. Check if the WiFi client's ip received from gateway is in valid range
5. From WLAN client check if ping to gateway's public ip is success or not with security mode WPA-WPA2-PSK
6. Disconnect the WiFi client
7.Unload tdkb_e2e module</automation_approch>
    <expected_output>ping from 5G WLAN client to router's public ip is success with security mode WPA-WPA2-PSK</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_WPAWPA2PSK_PingRouterPublicIP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPAWPA2PSK_PingRouterPublicIP');
                        ...
                        if wlanIP:
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("TEST STEP 10:Check the PING from 5G WLAN to router's public ip")
                                    status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "PING", wlanIP, curIPAddress)
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("verifyNetworkConnectivity: SUCCESS")
                                        print("Ping successful from 5G WLAN to router's public ip")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("Ping not successful from 5G WLAN to router's public ip")
