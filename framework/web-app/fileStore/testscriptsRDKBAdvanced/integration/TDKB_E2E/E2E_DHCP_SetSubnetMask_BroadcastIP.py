...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_SetSubnetMask_BroadcastIP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that subnet mask of DHCPv4 server pool cannot be set with Broadcast IP address. (ex.255.255.255.255). Ensure that DHCP configuration on client is not affected</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_270</test_case_id>
    <test_objective>Verify that subnet mask of DHCPv4 server pool cannot be set with Broadcast IP address. (ex.255.255.255.255). Ensure that DHCP configuration on client is not affected</test_objective>
...
4. Set subnetmask to broadcast ip address using tdkb_e2e_SetMultipleParams and it should fail
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>DHCP configuration on client should not be affected by setting subnetmask as broadcast ip</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_SetSubnetMask_BroadcastIP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_SetSubnetMask_BroadcastIP');
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    # Set the LanIPAddress to Broadcast IP address"
                    subnetMask = "%s|255.255.255.255|string" %lanSubnetMask

                    tdkTestObj,status,details = setMultipleParameterValues(obj,subnetMask)
                    if expectedresult not in status:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print("TEST STEP 4: Set the SubnetMask to Broadcast IP, 255.255.255.255")
                        ...
                        if expectedresult in retStatus and newSubnetMask != "255.255.255.255":
                            tdkTestObj.setResultStatus("SUCCESS");
                            ...
                            if subnetMask and "255.255.255.255" not in subnetMask:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print(" subnet mask has not changed as Broadcast IP")
                                finalStatus = "SUCCESS";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("subnet mask has changed as Broadcast IP")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5: Get the current subnetmask")
                            print("EXPECTED RESULT 5: Should retrieve the current subnetmask")
