...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_ParentalControl_ManagedServices_LAN_BlockHTTP_AllowHTTPS</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that WG allows other services when only HTTP service is blocked through parental control (managed Services)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_397</test_case_id>
    <test_objective>Verify that WG allows other services when only HTTP service is blocked through parental control (managed Services)</test_objective>
...
6.Check whether the LAN client is able to establish https connection
7.Delete the added rule and revert all params values
8.Unload tdkb_e2e module</automation_approch>
    <except_output>LAN client should be able to establish connection with the allowed service https </except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_ParentalControl_ManagedServices_LAN_BlockHTTP_AllowHTTPS</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_LAN_BlockHTTP_AllowHTTPS');
obj1.configureTestCase(ip,port,'E2E_ParentalControl_ManagedServices_LAN_BlockHTTP_AllowHTTPS');
                                        ...
                                        if expectedresult in status and curIPAddress:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("TEST STEP 11:Check the HTTPS connectivity from LAN to WAN")

                                                    status = wgetToWAN("WGET_HTTPS", lanIP, curIPAddress, "LAN")
