...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_RadioDisabled_TCP_WLanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that tcp traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_213</test_case_id>
    <test_objective>Verify that tcp traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</test_objective>
...
3. Set security mode to WPA2PSK,radio enable and ssid enable to true using tdkb_e2e_SetMultipleParams
3. Try to connect to 2.4GHZ from WIFI client and Lan Client
4. Disable radio and send TCP request from wlan client to lan client
4. Revert the values</automation_approch>
    <except_output>TCP request should fail since radio is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_RadioDisabled_TCP_WLanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_RadioDisabled_TCP_WLanToLan');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in status and setValuesList == newValues:
                                                    ...
                                                    #Send TCP request from WLAN client to LAN client
                                                    print("TEST STEP 12: Connect to LAN Client and send TCP Request from WLAN Client")
                                                    status = tcp_udpInClients("WLAN","LAN",lanIP,wlanIP);
                                                    if "SUCCESS" not in status:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("TCP traffic not successful between wired and wireless clients")
                                                        finalStatus = "SUCCESS"
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("TCP traffic successful between wired and wireless clients")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("TEST STEP 11: Get the current radioEnable")
                                                    print("EXPECTED RESULT 11: Should retrieve the current radioEnable")
