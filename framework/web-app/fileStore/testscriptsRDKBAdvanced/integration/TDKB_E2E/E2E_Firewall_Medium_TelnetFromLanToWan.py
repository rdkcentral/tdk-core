...
<xml>
  ...
  <name>E2E_Firewall_Medium_TelnetFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Medium Telnet access from LAN to WAN is success</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_297</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Medium Telnet access from LAN to WAN is success</test_objective>
...
5. From the wlan client, do TELNET to the WAN client and check whether telnet is success when the firewall level is set to Medium
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Medium_TelnetFromLanToWan');
    ...
    if expectedresult in status:
        ...
        if expectedresult in retStatus:
            ...
            if expectedresult in firewallResult:
                ...
                if expectedresult in retStatus and newFirewallValue == "Medium":
                    tdkTestObj.setResultStatus("SUCCESS");
