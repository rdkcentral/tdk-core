...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_DisableDMZ_FirewallLow_FTPFromWanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling DMZ FTP should not be allowed from WAN to WLAN</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_188</test_case_id>
    <test_objective>Verify that on disabling DMZ FTP should not be allowed from WAN to WLAN</test_objective>
...
13. Check if the ftp connection has failed.
...
16.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    <except_output>On enabling DMZ, FTP connection from WAN client to WAN ip of gateway should be success. On disabling DMZ, FTP connection should fail.</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e, advancedconfig</test_stub_interface>
    <test_script>E2E_DisableDMZ_FirewallLow_FTPFromWanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_FTPFromWanToWlan');
obj1.configureTestCase(ip,port,'E2E_DisableDMZ_FirewallLow_FTPFromWanToWlan');
                                                        ...
                                                        if expectedresult in status:
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("SUCCESS: FTP from WAN to WAN ip of GW is blocked")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("TEST STEP 13:FTP from WLAN to WAN ip of GW failed")
                                                                    print("FAILURE: FTP from WAN to WAN ip of GW is not blocked")
                                                            else:
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                details = tdkTestObj.getResultDetails();
                                                                print("[TEST STEP 11]: Disabling DMZ");
