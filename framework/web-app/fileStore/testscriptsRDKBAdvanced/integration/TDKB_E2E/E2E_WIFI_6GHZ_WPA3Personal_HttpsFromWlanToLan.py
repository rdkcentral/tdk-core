...
<xml>
  ...
  <name>E2E_WIFI_6GHZ_WPA3Personal_HttpsFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that https traffic between WLAN to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6GHZ.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_671</test_case_id>
    <test_objective>Verify that https traffic between WLAN to LAN should be successful when the wireless client is associated with wpa3-psk authentication for 6GHz.</test_objective>
...
6. From the wlan client, try to connect to the lan client via https and check whether https connection is success when the security mode for 6GHz is set to WPA-Personal
7. Unload tdkb_e2e module</automation_approch>
    <expected_output>Https connection from WLAN client to LAN client should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_6GHZ_WPA3Personal_HttpsFromWlanToLan</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_HttpsFromWlanToLan')
wifi_obj.configureTestCase(ip,port,'E2E_WIFI_6GHZ_WPA3Personal_HttpsFromWlanToLan')
                                    ...
                                    if expectedresult in status:
                                        ...
                                        if lanIP:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                #Send HTTPs request to LAN client from WLAN client
                                                step += 1
                                                print(f"TEST STEP {step}: Check the HTTPS connectivity from WLAN to LAN")
                                                print(f"EXPECTED RESULT {step}: WLAN client should reach LAN client via HTTPS")
                                                status = tdkbE2EUtility.verifyNetworkConnectivity(lanIP, "WGET_HTTPS", wlanIP, curIPAddress)
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS")
