...
<xml>
  <id/>
  <version>4</version>
  <name>E2E_WIFI_5GHZ_WPAWPA2PSK_PingFromLanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that ICMP Traffic should be successfull with source as wired client and destination as wireless client where client association in wpawpa2-psk security wlan on 5ghz band.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_163</test_case_id>
    <test_objective>Verify that ICMP Traffic should be successfull with source as wired client and destination as wireless client where client association in wpawpa2-psk security wlan on 5ghz band.</test_objective>
...
3. Set the security mode for 5GHz to wpawpa2-psk  using tdkb_e2e_SetMultipleParams
...
5. From the lan client, do ping to the WLAN client and check whether  ping is success when the security mode for 5GHz is set to wpawpa2-psk
6. Revert the security mode to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>From the lan client, do ping to the WLAN client and check whether ping is success when the security mode for 5GHz is set to wpawpa2-psk</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_WPAWPA2PSK_PingFromLanToWlan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPAWPA2PSK_PingFromLanToWlan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'WPA-WPA2-Personal'];
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            print("TEST STEP 10:Check the PING from LAN to WLAN");
                                            ...
                                            if expectedresult in status:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("TEST STEP 10:Ping from LAN to WLAN failed")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
