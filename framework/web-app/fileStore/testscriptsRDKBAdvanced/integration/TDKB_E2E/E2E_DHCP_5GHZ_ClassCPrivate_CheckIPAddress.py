...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_DHCP_5GHZ_ClassCPrivate_CheckIPAddress</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on configuring DHCP Server to Class C Private IP range (192.168.x.x - 192.168.255.255) Wi-Fi Client connected via 5GHz radio successfully receives IP address</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_99</test_case_id>
    <test_objective>Verify that on configuring DHCP Server to Class C Private IP range (192.168.x.x - 192.168.255.255) Wi-Fi Client connected via 5GHz radio successfully receives IP address</test_objective>
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>The IP obtained by the wifi client should be in Class C private range</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_5GHZ_ClassCPrivate_CheckIPAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_5GHZ_ClassCPrivate_CheckIPAddress');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'192.168.0.1','255.255.255.0','192.168.0.2','192.168.0.253'];
