...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_DHCP_ClassBPrivate_CheckLanIPAddress</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that DHCPv4 server pool can be set with Class B private IP address range 172.16.x.x - 172.31.255.255. Ensure that ethernet client obtains IP within the range</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_193</test_case_id>
    <test_objective>Verify that DHCPv4 server pool can be set with Class B private IP address range 172.16.x.x - 172.31.255.255. Ensure that ethernet client obtains IP within the range</test_objective>
...
6.Unload tdkb_e2e module</automation_approch>
    <except_output>The IP obtained by the lan client should be in Class B private range</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_ClassBPrivate_CheckLanIPAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_ClassBPrivate_CheckLanIPAddress');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = ['172.16.0.1','255.255.255.0','172.16.0.2','172.16.0.253'];
            print("Parameter values that are set: %s" %setValuesList)


            list1 = [lanIPAddress,'172.16.0.1','string']
            list2 = [lanSubnetMask,'255.255.255.0','string']
            list3 = [minAddress,'172.16.0.2','string']
            list4 = [maxAddress,'172.16.0.253','string']
