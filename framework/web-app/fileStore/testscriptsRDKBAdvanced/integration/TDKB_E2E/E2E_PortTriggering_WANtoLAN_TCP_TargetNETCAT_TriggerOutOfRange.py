...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerOutOfRange</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_620</test_case_id>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
10. Send an outbound packet to trigger the out of range port 12350 with a TCP packet
11. From WAN client, send an inbound packet to the target port 54321 via the DUT GW WAN IP. The packet should not be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 54321 is not forwarded to the LAN machine (which is running a NETCAT server) after it sends an outbound packet through the GW that triggers the out of range port 12350 which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetNETCAT_TriggerOutOfRange');
            ...
            if lanIP != "":
                ...
                if expectedresult in status and curIPAddress:
                    ...
                    if expectedresult in status:
                        ...
                        print("[TEST EXECUTION RESULT] : SUCCESS");
                        #Set the Port Triggering rule and enable it:- Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 54321:54321 (for NETCAT), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
                        step = step + 1
                        triggerStart = "12340"
                        triggerEnd = "12346"
                        targetStart = "54321"
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("\nTEST STEP %d: Trigger the Out of Range Port %s through a specified outbound packet from LAN client" %(step, trigger_OutOfRange))
                                    print("EXPECTED RESULT %d: Port should be triggered successfully" %step)
                                    status = triggerPort(tdkbE2EUtility.wan_ip, trigger_OutOfRange, triggerProtocol, "LAN")
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        ...
                                        print("EXPECTED RESULT %d: NETCAT client running in WAN machine should not connect to the server running in LAN with the GW WAN IP of DUT" %step)
                                        status = PTClientRequest("NETCAT", tdkbE2EUtility.wan_ip, triggerStart, targetProtocol, tdkbE2EUtility.client_logfile, "WAN");
