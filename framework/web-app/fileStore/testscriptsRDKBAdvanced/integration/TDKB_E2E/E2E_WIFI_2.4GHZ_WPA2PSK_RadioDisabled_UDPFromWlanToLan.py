...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_WPA2PSK_RadioDisabled_UDPFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that udp traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_214</test_case_id>
    <test_objective>Verify that udp traffic between wireless and wired clients should be denied when Radio interface parameter for 2.4GHZ [Device.WiFi.Radio.1.Enable] is disabled.</test_objective>
...
4. Disable radio and usinf iperf check udp traffic from wlan to lan
5. Check if UDP communication failed
4. Revert the values</automation_approch>
    <except_output>UDP communication between WLAN and LAN client should fail</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_WPA2PSK_RadioDisabled_UDPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPA2PSK_RadioDisabled_UDPFromWlanToLan');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if expectedresult in status and setValuesList == newValues:
                                                    ...
                                                    print("TEST STEP 12:Check UDP from WLAN to LAN");
                                                    ...
                                                    if "SUCCESS" in status and lossPercentage=="0%":
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("UDP is success from WLAN to LAN")
                                                        print("FAILURE: UDP from WLAN to LAN is success")
                                                    else:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print("TEST STEP 12:SUCCESS :UDP from WLAN to LAN failed")
                                                        finalStatus = "SUCCESS"
