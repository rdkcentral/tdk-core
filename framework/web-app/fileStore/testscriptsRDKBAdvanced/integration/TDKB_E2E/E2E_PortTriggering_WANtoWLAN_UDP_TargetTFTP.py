...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_PortTriggering_WANtoWLAN_UDP_TargetTFTP</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the WLAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_569</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW takes effect so that an inbound packet with target port 69 is forwarded to the WLAN machine (which is running a TFTP server) after it sends an outbound packet through the GW that triggers the 12345 port configured in the rule.</test_objective>
...
18. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoWLAN_UDP_TargetTFTP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetTFTP');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_UDP_TargetTFTP');
