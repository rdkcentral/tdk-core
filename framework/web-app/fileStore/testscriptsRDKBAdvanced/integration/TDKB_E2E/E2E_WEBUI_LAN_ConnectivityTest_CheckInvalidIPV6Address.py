...
<xml>
  ...
  <name>E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV6Address</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the connectivity with invalid IPV6 address</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_487</test_case_id>
    <test_objective>To test the connectivity with invalid IPV6 address</test_objective>
...
10. Enter the invalid ip in IPV6 address column and test whether error popup is displayed
11. Kill selenium hub and node
12. unload module</automation_approch>
    <except_output>With invalid IPV6 address,error poup should be displayed</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV6Address</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WEBUI_LAN_ConnectivityTest_CheckInvalidIPV6Address');
                        ...
                        if expectedresult in status:
                            ...
                            if status == "SUCCESS":
                                try:
                                    ...
                                    #To give invalid IPV6 address as input
                                    driver.find_element_by_id("ipv6_address_1").send_keys("a")
                                    driver.find_element_by_id("ipv6_address_2").send_keys("b")
                                    driver.find_element_by_id("ipv6_address_3").send_keys("c")
                                    driver.find_element_by_id("ipv6_address_4").send_keys("d")
                                    driver.find_element_by_id("ipv6_address_5").send_keys("1")
                                    driver.find_element_by_id("ipv6_address_6").send_keys("2")
                                    driver.find_element_by_id("ipv6_address_7").send_keys("3")
                                    driver.find_element_by_id("ipv6_address_8").send_keys("4")
                                    # To click on "Check IPV6 address connectivity" button
                                    driver.find_element_by_xpath('//*[@id="check_ipv6"]').click()
                                    time.sleep(40);
                                    value = driver.find_element_by_xpath('//*[@id="connectivity_ipv6"]').text
                                    if value == "Error":
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("TEST STEP 6: Test the coonectivity status of invalid ipv6 address")
                                        print("EXPECTED RESULT 6:With an invalid IPv6 address connectivity status is expected to throw Error popup")
                                        print("ACTUAL RESULT 6: %s" %value);
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 6: Failed to get the connectivity status as Error")
