...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_DisconnectWiFi_GetAssociatedDeviceNumberOfEntries</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disconnecting a Wireless Client through 5 GHz radio value of parameter (Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries) should be decremented accordingly</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_169</test_case_id>
    <test_objective>Verify that on disconnecting a Wireless Client through 5 GHz radio value of parameter (Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries) should be decremented accordingly</test_objective>
...
Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries</input_parameters>
...
4. Get the current associated number of entries and check whether it is incremented by one after the wifi connect
5. Disconnect from the wifi connected
6.  Get the current associated number of entries and check whether it is decremented by one after the wifi disconnect
7. Revert the values of encryption method and security mode
8.Unload tdkb_e2e module</automation_approch>
    <except_output>Device.WiFi.AccessPoint.2.AssociatedDeviceNumberOfEntries should be decremented by one after successfully disconnected from wifi</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_DisconnectWiFi_GetAssociatedDeviceNumberOfEntries</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_DisconnectWiFi_GetAssociatedDeviceNumberOfEntries');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = ['true',tdkbE2EUtility.ssid_5ghz_name,'AES','WPA2-Personal',tdkbE2EUtility.ssid_5ghz_pwd];
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print("TEST STEP 4: Get the current Associated number of entries before connecting to WIFI")
                    param = "Device.WiFi.AccessPoint.%s.AssociatedDeviceNumberOfEntries" %tdkbE2EUtility.ssid_5ghz_index
                    tdkTestObj,status,associatedNumberOfEntries = getParameterValue(obj,param)
                    print("Current Associated number of entries: %s" %associatedNumberOfEntries);
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in status:
                            ...
                            if wlanIP:
                                ...
                                print("TEST STEP 7: Get the current Associated number of entries after connecting to WIFI")
                                param = "Device.WiFi.AccessPoint.%s.AssociatedDeviceNumberOfEntries" %tdkbE2EUtility.ssid_5ghz_index
                                tdkTestObj,status,associatedNumberOfEntriesAfterWiFiConnect = getParameterValue(obj,param)
                                print("Current Associated number of entries after wifi connect: %s" %associatedNumberOfEntriesAfterWiFiConnect);

                                #Increment intial Associated number of entries to check whether the current Associated number of entries incremented by 1 after wifi connect
                                if expectedresult in status and (int(associatedNumberOfEntries)+1) == int(associatedNumberOfEntriesAfterWiFiConnect):
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("Associated number of entries incremented after wifi connect")
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("TEST STEP 9: Get the current Associated number of entries after disconnecting from WIFI")
                                        param = "Device.WiFi.AccessPoint.%s.AssociatedDeviceNumberOfEntries" %tdkbE2EUtility.ssid_5ghz_index
                                        tdkTestObj,status,associatedNumberOfEntriesAfterWiFiDisconnect = getParameterValue(obj,param)
                                        print("Current Associated number of entries after wifi disconnect: %s" %associatedNumberOfEntriesAfterWiFiDisconnect);

                                        #Increment intial Associated number of entries to check whether the current Associated number of entries decremented by 1 after wifi disconnect
                                        if expectedresult in status and (int(associatedNumberOfEntriesAfterWiFiConnect)-1) == int(associatedNumberOfEntriesAfterWiFiDisconnect):
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("Associated number of entries decremented after wifi disconnect")
                                            finalStatus = "SUCCESS"
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("TEST STEP 9: Failed to get the current Associated number of entries after disconnecting from WIFI/Associated number of entries not decremented")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 8: Failed to Disconnect from the wifi ssid")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("TEST STEP 7: Failed to get the current Associated number of entries after connecting to WIFI/Associated number of entries not incremented")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print("TEST STEP 4: Failed to get the current Associated number of entries before connecting to WIFI")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current SSID, KeyPassphrase, Radio status, encryption method, security mode")
                    print("EXPECTED RESULT 3: Should retrieve the current SSID, KeyPassphrase, Radio status, encryption method, security mode")
