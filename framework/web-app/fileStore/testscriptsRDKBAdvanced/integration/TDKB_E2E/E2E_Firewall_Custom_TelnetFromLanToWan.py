...
<xml>
  ...
  <name>E2E_Firewall_Custom_TelnetFromLanToWan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that when Firewall Config is set to Custom Telnet access from LAN to WAN should be success</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_302</test_case_id>
    <test_objective>Verify that when Firewall Config is set to Custom Telnet access from LAN to WAN should be success</test_objective>
...
3. Set the firewall level to Custom using tdkb_e2e_SetMultipleParams
4.  Check whether LAN client is connected to GW and it is assigned a valid ip by the GW
5. From the lan client, do TELNET to the WAN client and check whether telnet is blocked when the firewall level is set as custom
6. Revert the firewall level to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>Telnet from LAN to WAN should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_Firewall_Custom_TelnetFromLanToWan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_Firewall_Custom_TelnetFromLanToWan');
    ...
    if expectedresult in status:
        ...
        if expectedresult in retStatus:
            ...
            if expectedresult in firewallResult:
                ...
                if expectedresult in retStatus and newFirewallValue == "Custom":
                    tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        if expectedresult in status and curIPAddress:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("TEST STEP 10:Check the Telnet from LAN to WAN")
                                    status = telnetToClient("WAN",tdkbE2EUtility.wan_ip);
                                    time.sleep(20);
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("SUCCESS: Telnet from LAN to WAN is success")
                                        ...
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("TEST STEP 10: failed, Telnet from LAN to WAN is blocked")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
