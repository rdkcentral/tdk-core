...
<xml>
  <id/>
  <version>2</version>
  <name>E2E_WIFI_5GHZ_CheckSSIDBroadcast_RadioDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that when 5 GHz radio is disabled corresponding SSID should not be broadcasted in Client side.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_135</test_case_id>
    <test_objective>Verify that when 5 GHz radio is disabled corresponding SSID should not be broadcasted in Client side.</test_objective>
...
2. Using tdkb_e2e_Get, get and save security mode and 5GHZ radio enable status
3. Set security mode to WPAWPA2PSK and Disable Radio using tdkb_e2e_SetMultipleParams
4. Check whether SSID for 5GHZ is broadcasting or not
5. Revert the values to original value
6.Unload tdkb_e2e module</automation_approch>
    <except_output>SSID for 5GHZ should not  broadcast since Radio is disabled</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_CheckSSIDBroadcast_RadioDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_CheckSSIDBroadcast_RadioDisabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,'WPA2-Personal','false'];
