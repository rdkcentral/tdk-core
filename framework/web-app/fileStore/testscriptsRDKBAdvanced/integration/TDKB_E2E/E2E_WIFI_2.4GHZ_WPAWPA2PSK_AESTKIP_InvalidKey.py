...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_WPAWPA2PSK_AESTKIP_InvalidKey</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether WG supports WPAWPA2-PSK (TKIP/AES)á encryption security mechanism using invalid key for 2.4GHZ SSID. Ensure that client should not be able to connect to WG</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_04</test_case_id>
    <test_objective>Verify whether WG supports WPAWPA2-PSK (TKIP/AES)Ã&#131;Â¡ encryption security mechanism using invalid key for 2.4GHZ SSID. Ensure that client should not be able to connect to WG</test_objective>
...
3. Try to connect to Wifi client and check whether the wifi client is not connected to the DUT using AES+TKIP and WPA-WPA2-Personal and invalid wifi password
4. Revert the values of encryption method and security mode</automation_approch>
    <except_output>WIFI client should not be able to connect to the DUT with the using AES+TKIP encryption method and WPA-WPA2-Personal security mode and invalid wifi password</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_WPAWPA2PSK_AESTKIP_InvalidKey</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPAWPA2PSK_AESTKIP_InvalidKey');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = ['true',tdkbE2EUtility.ssid_2ghz_name,'AES+TKIP','WPA-WPA2-Personal',tdkbE2EUtility.ssid_2ghz_pwd];
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print("TEST STEP 4: From wlan client, Connect to the wifi ssid using invalid wifi password")
                    status = wlanConnectWifiSsid(tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_invalid_pwd,tdkbE2EUtility.wlan_2ghz_interface);
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 5: Should set the original SSID, KeyPassphrase, Radio status, encryption method, security mode");
