...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_DisablePF_FirewallLow_HTTPFromWanToWlan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that on disabling Port Forwarding HTTP traffic should not passthrough from WAN to WLAN</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_183</test_case_id>
    <test_objective>Verify that on disabling Port Forwarding HTTP traffic should not passthrough from WAN to WLAN</test_objective>
...
12. From wan client try to establish http connection with the WAN ip of GW(it should fail)
...
13.Unload tdkb_e2e and advancedconfig modules</automation_approch>
    ...
    <test_script>E2E_DisablePF_FirewallLow_HTTPFromWanToWlan</test_script>
    <skipped>No</skipped>
                                    ...
                                    if expectedresult in actualresult:
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            #Set the result status of execution
                                                ...
                                                print("[EXPECTED RESULT9]: Should add new rule to Port Mapping");
                                                ...
                                                if (instance1 > 0):
                                                    ...
                                                    if expectedresult in actualresult:
                                                        ...
                                                        time.sleep(60);
                                                        print("TEST STEP :Check the HTTP from WAN to WLAN after enabling port mapping")
                                                        status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "WGET_HTTP", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                                        if expectedresult in status:
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("HTTP from WAN to WLAN: SUCCESS")
                                                            ...
                                                            if expectedresult in actualresult:
                                                                ...
                                                                tdkTestObj.setResultStatus("SUCCESS");
                                                                details = tdkTestObj.getResultDetails();
                                                                print("[TEST STEP 11]: Disbaling Port Mapping");
                                                                ...
                                                                time.sleep(60);
                                                                print("TEST STEP :Check the HTTP from WAN to WLAN after disabling port mapping")
                                                                status = verifyNetworkConnectivity(tdkbE2EUtility.gw_wan_ip, "WGET_HTTP", tdkbE2EUtility.wan_ip, curIPAddress, "WAN")
                                                                if expectedresult not in status:
                                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                                    finalStatus = "SUCCESS";
                                                                    print("SUCCESS, HTTP from WAN to WLAN failed")
                                                                else:
                                                                    tdkTestObj.setResultStatus("FAILURE");
                                                                    print("TEST STEP: FAILURE: HTTP from WAN to WLAN is success")
                                                            else:
                                                                tdkTestObj.setResultStatus("FAILURE");
                                                                details = tdkTestObj.getResultDetails();
                                                            ...
                                                            tdkTestObj.setResultStatus("FAILURE");
                                                            print("TEST STEP :HTTP from WAN to WLAN failed")
