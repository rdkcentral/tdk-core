...
<xml>
  ...
  <name>E2E_DHCP_6GHZ_ClassCPrivate_CheckIPAddress</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that on configuring DHCP Server to Class C Private IP range (192.168.x.x - 192.168.255.255) Wi-Fi Client connected via 6GHz radio successfully receives IP address</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <rdk_versions />
  <test_cases>
    <test_case_id>TC_TDKB_E2E_681</test_case_id>
    <test_objective>Verify that on configuring DHCP Server to Class C Private IP range (192.168.x.x - 192.168.255.255) Wi-Fi Client connected via 6GHz radio successfully receives IP address</test_objective>
...
6. Unload tdkb_e2e module</automation_approch>
    <expected_output>The IP obtained by the wifi client should be in Class C private range</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_6GHZ_ClassCPrivate_CheckIPAddress</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,"E2E_DHCP_6GHZ_ClassCPrivate_CheckIPAddress")
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_6ghz_name,tdkbE2EUtility.ssid_6ghz_pwd,"192.168.0.1","255.255.255.0","192.168.0.2","192.168.0.253"]
