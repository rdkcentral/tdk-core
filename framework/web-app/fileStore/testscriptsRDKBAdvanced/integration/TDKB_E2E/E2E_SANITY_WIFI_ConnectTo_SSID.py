        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print("TEST STEP 4: From wlan client, Connect to the 2.4GHZ wifi ssid")
                            ...
                            if expectedresult in status and curIPAddress:
                                ...
                                if expectedresult in status:
                                    ...
                                    if expectedresult in status:
                                        ...
                                        print("TEST STEP 9: From wlan client, Connect to the 5GHZ wifi ssid")
                                        status = wlanConnectWifiSsid(tdkbE2EUtility.ssid_5ghz_name,tdkbE2EUtility.ssid_5ghz_pwd,tdkbE2EUtility.wlan_5ghz_interface);
                                        if expectedresult in status:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print("wlanConnectWifiSsid: SUCCESS")

                                            print("TEST STEP 10: Get the IP address of the wlan client after connecting to wifi")
                                            wlanIP = getWlanIPAddress(tdkbE2EUtility.wlan_5ghz_interface);
                                            if wlanIP:
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("getWlanIPAddress: SUCCESS")

                                                print("TEST STEP 11: Check whether wlan ip address is in same DHCP range")
                                                status = "SUCCESS"
                                                status = checkIpRange(curIPAddress,wlanIP);
                                                if expectedresult in status:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    ...
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print("wlan ip address is not in same DHCP range for 5GHZ")
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("Failed to get the wlan IP address for 5GHZ")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print("Failed to connect to the 5GHZ SSID")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("Disconnect from WIFI SSID:FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("wlan ip address is not in same DHCP range for 2.4GHZ")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("Failed to get the current LAN IP address DHCP range")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("Failed to get the wlan IP address for 2.4GHZ")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("Failed to connect to the 2.4GHZ SSID")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current ssid,keypassphrase of 2.4GHZ and 5GHz")
                    print("EXPECTED RESULT 3: Should retrieve the current ssid,keypassphrase of 2.4GHZ and 5GHz")
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 13: Should set the original ssid,keypassphrase of 2.4GHZ and 5GHz");
