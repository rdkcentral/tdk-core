...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_5GHZ_WPAWPA2PSK_FTP_FromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpawpa2-psk security wlan on 5ghz band.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_151</test_case_id>
    <test_objective>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpawpa2-psk security wlan on 5ghz band.</test_objective>
...
3. Set the security mode for 5GHz to WPAWPA22PSK using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, try to connect to the LAN client via ftp .Check whether ftp connection and file transfer are success when the security mode for 5GHz is set to WPAWPA2PSK
6. Revert the security mode to original value
7.Unload tdkb_e2e module</automation_approch>
    <except_output>ftp connection from WLAN client to LAN client and file transfer should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_5GHZ_WPAWPA2PSK_FTP_FromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_5GHZ_WPAWPA2PSK_FTP_FromWlanToLan');
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            print("TEST STEP 10:Check File download via FTP from WLAN to LAN");
                                            ...
                                            if expectedresult in status:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("TEST STEP 10: File download via FTP from WLAN to LAN failed")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
