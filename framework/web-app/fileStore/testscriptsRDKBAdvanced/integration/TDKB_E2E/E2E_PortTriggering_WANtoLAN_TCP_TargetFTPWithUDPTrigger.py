...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithUDPTrigger</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_557</test_case_id>
...
15. Unload the modules</automation_approch>
    ...
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithUDPTrigger</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithUDPTrigger');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTPWithUDPTrigger');
                    ...
                    if expectedresult in status:
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                step = step + 1
                                print("\nTEST STEP %d: Trigger the Port %s with incorrect protocol %s through an outbound packet from LAN client" %(step, triggerStart, incorrectTriggerProtocol))
                                ...
                                if expectedresult in status:
                                    ...
                                    step = step + 1
                                    print("\nTEST STEP %d: Check if the specified inbound packets are NOT forwarded to the LAN client that triggered with incorrect protocol" %step)
                                    print("EXPECTED RESULT %d: FTP from WAN to the LAN should be failure with the GW WAN IP of DUT" %step)
                                    status = ftpToClient("LAN", tdkbE2EUtility.gw_wan_ip, "WAN");

                                    if expectedresult not in status:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("ACTUAL RESULT %d: FTP from WAN to the LAN is failed with the GW WAN IP of DUT" %step)
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                    else:
