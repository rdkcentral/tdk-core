...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_WPA2PSK_Custom_TCPFromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association in wpa2-psk security wlan on 2.4ghz band with firewall set to custom.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Verify that TCP Traffic should be successfull with destination as wired client and source as wireless client  where client association in wpa2-psk security wlan on 2.4ghz band with firewall set to custom.</test_objective>
...
3. Set the security mode for 2.4GHz to WPA2PSK and firewall to Custom using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, do TCP to the LAN client and check whether  TCP is success when the security mode for 2.4GHz is set to WPA2PSK
...
8.Unload tdkb_e2e module</automation_approch>
    <except_output>TCP should be success between wired and wireless clients when firewall is Custom and Security mode to WPA2PSK</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_WIFI_2.4GHZ_WPA2PSK_Custom_TCPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPA2PSK_Custom_TCPFromWlanToLan');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
            ...
            setValuesList = [tdkbE2EUtility.ssid_2ghz_name,tdkbE2EUtility.ssid_2ghz_pwd,'WPA2-Personal','Custom'];
