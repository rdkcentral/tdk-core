...
<xml>
  <id/>
  <version>1</version>
  <name>E2E_WIFI_2.4GHZ_WPA2PSK_FTP_FromWlanToLan</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpa2-psk security wlan on 2.4ghz band.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_148</test_case_id>
    <test_objective>Verify that largersize file can be downloaded sucessfuly from ftpserver when wifi client assocation in wpa2-psk security wlan on 2.4ghz band.</test_objective>
...
3. Set the security mode for 2.4GHz to WPA2PSK using tdkb_e2e_SetMultipleParams
...
5. From the wlan client, try to connect to the LAN client via ftp and check whether ftp connection and file transfer are success when the security mode for 2.4GHz is set to WPA2PSK
...
7.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_WIFI_2.4GHZ_WPA2PSK_FTP_FromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPA2PSK_FTP_FromWlanToLan');
                                ...
                                if expectedresult in status:
                                    ...
                                    if lanIP:
                                        ...
                                        if expectedresult in status:
                                            ...
                                            print("TEST STEP 10:Check FTP and File download request from WLAN to LAN");
                                            ...
                                            if expectedresult in status:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print("TEST STEP 10:File download request via FTP from WLAN to LAN failed")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
