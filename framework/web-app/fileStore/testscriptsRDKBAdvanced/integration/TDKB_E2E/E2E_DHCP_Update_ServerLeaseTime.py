...
<xml>
  ...
  <name>E2E_DHCP_Update_ServerLeaseTime</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify the DHCPv4 server lease time can be updated in Wireless gateway</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_274</test_case_id>
    <test_objective>Verify the DHCPv4 server lease time can be updated in Wireless gateway</test_objective>
...
4. Set leasetime to a valid value using tdkb_e2e_SetMultipleParams
...
6.Unload tdkb_e2e module</automation_approch>
    ...
    <test_script>E2E_DHCP_Update_ServerLeaseTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_Update_ServerLeaseTime');
    ...
    if expectedresult in status:
        ...
        if expectedresult in status:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    if lanIP:
                        ...
                        if expectedresult in status:
                            ...
                            print("TEST STEP 6: Get the DHCP server lease time of LAN client")
                            leaseTimeInClient = getLanDhcpDetails(tdkbE2EUtility.lan_lease_time);
                            newLeaseTime = newValues[4]
                            if leaseTimeInClient:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print("getDhcpLeaseTime SUCCESS")

                                print("TEST STEP 7: Check whether lease time from client is matching with the set value")
                                if leaseTimeInClient == newLeaseTime:
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print("Lease time from client is matching with the set value")
                                    finalStatus = "SUCCESS";
                                else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print("lease time from client is not matching with the set value")
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print("TEST STEP 6:Failed to get the DHCP server lease time of LAN client")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
            ...
            if expectedresult in actualresult and expectedresult in actualresult1 and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 8: Should set the original lanIPAddress,lanSubnetMask,minAddress,maxAddress and leaseTime");
