...
<xml>
  ...
  <name>E2E_DHCP_Check_ConfigUpdate_InLANClient</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that DHCPv4 server configurations are updated without rebooting the client</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_283</test_case_id>
    <test_objective>Verify that DHCPv4 server configurations are updated without rebooting the client</test_objective>
...
4.  Connect to the LAN client and check whether the values set as the subnetmask and lan ip address is reflected in client or not
5. Revert the values to original value
6.Unload tdkb_e2e module</automation_approch>
    <except_output>The values set as the subnetmask and lan ip address should be reflected in client</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_DHCP_Check_ConfigUpdate_InLANClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_DHCP_Check_ConfigUpdate_InLANClient')
        ...
        if expectedresult in status:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in status and setValuesList == newValues:
                    ...
                    print("TEST STEP 4:Connect to LAN Client and get the subnet mask")
                    ...
                    if subnetMask and "255.255.0.0" in subnetMask:
                        ...
                        print("TEST STEP 5:Connect to LAN Client and get the IP address")
                        ...
                        if lanIP:
                            ...
                            if expectedresult in status:
                                ...
                                print("TEST STEP 6: LanIP obtained is not in the same DHCP range")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("TEST STEP 5:Failed to connect to LAN Client")
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print("subnet mask has not changed as expected")
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print("TEST STEP 3: Get the current lanIPAddress,lanSubnetMask,minAddress and maxAddress")
                    print("EXPECTED RESULT 3: Should retrieve the current lanIPAddress,lanSubnetMask,minAddress and maxAddress")
            ...
            if expectedresult in actualresult and expectedresult in finalStatus:
                tdkTestObj.setResultStatus("SUCCESS");
                print("EXPECTED RESULT 7: Should set the original lanIPAddress,lanSubnetMask,minAddress and maxAddress");
