...
<xml>
  ...
  <name>E2E_PortTriggering_WANtoLAN_TCP_TargetFTP_TriggerOutOfRange</name>
...
6. To the added instance, configure the rule: Trigger Port(start:end) - 12340:12346, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
11. From WAN client, send an inbound packet to the target port 21 via the DUT GW WAN IP. The packet should not be forwarded to the LAN client.
...
15. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should take effect so that an inbound packet with target port 21 is not forwarded to the LAN machine (which is running an FTP server) after it sends an outbound packet through the GW that triggers the out of range port 12350 port which is not configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoLAN_TCP_TargetFTP_TriggerOutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTP_TriggerOutOfRange');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoLAN_TCP_TargetFTP_TriggerOutOfRange');
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in status:
                                ...
                                if expectedresult in status:
                                    ...
                                    print("EXPECTED RESULT %d: FTP from WAN to the LAN should fail with the GW WAN IP of DUT" %step)
                                    status = ftpToClient("LAN", tdkbE2EUtility.gw_wan_ip, "WAN");
                                    if expectedresult in status:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("ACTUAL RESULT %d: FTP from WAN to the LAN success with the GW WAN IP of DUT" %step)
