...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidFTPPort</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 2121 is forwarded to the WLAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_646</test_case_id>
    <test_objective>To check if the Port Triggering rule in GW does not take effect when an inbound packet with an invalid target port 2121 is forwarded to the WLAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</test_objective>
...
8. To the added instance, configure the rule: Trigger Port(start:end) - 12345:12345, TargetPort(start:end) - 21:21 (for FTP), Trigger Protocol - TCP, Target Protocol - TCP, Description - MyPTRule
...
13. From WAN client, send an inbound packet to the target port 2121 via the DUT GW WAN IP. The packet should NOT be forwarded to the WLAN client.
...
18. Unload the modules</automation_approch>
    <expected_output>Port Triggering rule in GW should not take effect when an inbound packet with an invalid target port 2121 is forwarded to the WLAN machine (which is running an FTP server at port 21) after it sends an outbound packet through the GW that triggers the port configured in the rule.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkb_e2e</test_stub_interface>
    <test_script>E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidFTPPort</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidFTPPort');
obj1.configureTestCase(ip,port,'E2E_PortTriggering_WANtoWLAN_TCP_TargetInvalidFTPPort');
                                        ...
                                        if expectedresult in status:
                                            ...
                                            if expectedresult in status:
                                                ...
                                                if expectedresult in status:
                                                    ...
                                                    print("EXPECTED RESULT %d: FTP from WAN to the WLAN should be failed with the GW WAN IP of DUT using invalid target port %s" %(step, invalidPort))
                                                    status = ftpToClientWithPort("WLAN", tdkbE2EUtility.gw_wan_ip, "WAN", invalidPort);
