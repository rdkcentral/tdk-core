...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>E2E_ETHAGENT_WEBUI_WAN_DisconnectLANClient_CheckConnectedDevices</name>
  <primitive_test_id/>
  <primitive_test_name>tdkb_e2e_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Disconnect LAN client and verify the count changes on displayed message while hovering mouse on "Internet" tab
</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_520</test_case_id>
    <test_objective>Disconnect LAN client and verify the count changes on displayed message while hovering mouse on "Internet" tab</test_objective>
...
4. Check if connected device number retrieved through namespace is same as number of connected devices in UI while hovering the mouse on Internet Tab
5. Disconnect LAN Client.
6. Check if connected devices in UI decreases by 1
7. unload module</automation_approch>
    <except_output>After disconnecting LAN Client,count changes on displayed message while hovering mouse on "Internet" tab should be decreased</except_output>
    <priority>High</priority>
    <test_stub_interface>None</test_stub_interface>
    <test_script>E2E_ETHAGENT_WEBUI_WAN_DisconnectLANClient_CheckConnectedDevices</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_ETHAGENT_WEBUI_WAN_DisconnectLANClient_CheckConnectedDevices');
obj1.configureTestCase(ip,port,'E2E_ETHAGENT_WEBUI_WAN_DisconnectLANClient_CheckConnectedDevices');
                    ...
                    if expectedresult in status and curIPAddress:
                        ...
                        if expectedresult in status:
                            ...
                            if expectedresult in actualresult:
                                ...
                                if status == "SUCCESS":
                                    try:
                                        print("Get the number of connected devices by hovering the mouse on Internet tab")
                                        ...
                                        if noOfConnectedDevice == connectedDeviceNumber:
                                            ...
                                            if status == "SUCCESS":
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print("TEST STEP 8: Check if lan client is disconnected");
                                                print("EXPECTED RESULT 8 : LAN client should be disconnected")
                                                print("ACTUAL RESULT 8 :LAN client is disconnected")
                                                #Get the result of execution
                                                print("[TEST EXECUTION RESULT] : SUCCESS");
                                                print("Waiting to reflect the changes in UI")
                                                time.sleep(500);
                                                driver.refresh();
                                                print("Get the connected device from UI after disconnecting lanclient")
                                                element = driver.find_element_by_link_text('Internet')
                                                hover = ActionChains(driver).move_to_element(element)
                                                hover.perform()
                                                time.sleep(5);
                                                ...
                                                print("Connected status :%s" %hoverText)
                                                connectedDevice=hoverText.splitlines()[1]
                                                newnoOfConnectedDevice=connectedDevice.split()[0]
                                                print("No of connected devices via WEBUI :%s" %newnoOfConnectedDevice)
                                                if int(newnoOfConnectedDevice) == int(int(noOfConnectedDevice) - 1):
                                                    #Set the result status of execution
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("TEST STEP 9: Check if number of connected device decreases after disconnecting the lan client");
                                                    print("EXPECTED RESULT 9: number of connected device should decrease after disconnecting the lan client")
                                                    print("ACTUAL RESULT 9:number of connected device decreases after disconnecting the lan client")
                                                    #Get the result of execution
                                                    print("[TEST EXECUTION RESULT] : SUCCESS");
                                                else:
                                                    #Set the result status of execution
                                                    ...
                                                    print("TEST STEP 9: Check if number of connected device decreases after disconnecting the lan client");
                                                    print("EXPECTED RESULT 9: number of connected device should decrease after disconnecting the lan client")
                                                    print("ACTUAL RESULT 9:number of connected device does not decrease after disconnecting the lan client")
                                                    #Get the result of execution
                                                    print("[TEST EXECUTION RESULT] : FAILURE");
                                                status= bringupInterface(tdkbE2EUtility.lan_interface,"LAN");
                                                if status == "SUCCESS":
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print("TEST STEP 10: Revert the changes by connecting the client");
                                                    print("EXPECTED RESULT 10 : LAN client should be connected")
                                                    print("ACTUAL RESULT 10 :LAN client is connected")
                                                    #Get the result of execution
                                                    print("[TEST EXECUTION RESULT] : SUCCESS");
                                                    ...
                                                    print("TEST STEP 10: Revert the changes by connecting the client");
                                                    print("EXPECTED RESULT 10 : LAN client should be connected")
                                                    print("ACTUAL RESULT 10:Failed to connect LAN Client")
                                                    #Get the result of execution
                                                    print("[TEST EXECUTION RESULT] : FAILURE");
                                        else:
                                            #Set the result status of execution
