...
<xml>
  ...
  <name>E2E_WIFI_2.4GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify that udp traffic between wireless and wired clients should be denied when SSID parameter for 2.4GHZ [Device.WiFi.SSID.1.Enable] is disabled.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_E2E_228</test_case_id>
    <test_objective>Verify that udp traffic between wireless and wired clients should be denied when SSID parameter for 2.4GHZ [Device.WiFi.SSID.1.Enable] is disabled.</test_objective>
...
4. Disable 2.4ssid and usinf iperf check tcp traffic from wlan to lan
5. Check if UDPcommunication failed
4. Revert the values</automation_approch>
    ...
    <test_script>E2E_WIFI_2.4GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'E2E_WIFI_2.4GHZ_WPA2PSK_SSIDDisabled_UDPFromWlanToLan');
                                                ...
                                                if expectedresult in actualresult:
                                                    ...
                                                    if expectedresult in status and setValuesList == newValues:
                                                        ...
                                                        if "SUCCESS" in status and lossPercentage=="0%":
                                                            ...
                                                            tdkTestObj.setResultStatus("SUCCESS");
                                                            print("TEST STEP 10:SUCESS: UDP from WLAN to LAN failed")
                                                            finalStatus = "SUCCESS"
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print("TEST STEP 13: Get the current SSIDEnable")
